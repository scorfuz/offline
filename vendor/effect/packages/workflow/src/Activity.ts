...
> => {
  ...
  > = {
    ...
    executeEncoded: Effect.matchEffect(executeWithoutInterrupt, {
      onFailure: (error) =>
        Effect.flatMap(
          Effect.orDie(Schema.encode(self.errorSchema as any)(error)),
          Effect.fail
        ),
      onSuccess: (value) =>
        Effect.orDie(Schema.encode(self.successSchema)(value))
    }),
    commit() {
      return execute
    }
...
const interruptRetryPolicy = Schedule.exponential(100, 1.5).pipe(
  ...
  Schedule.whileInput((cause: Cause.Cause<unknown>) =>
 ...
 * @category Attempts
 */
export class CurrentAttempt extends Context.Reference<CurrentAttempt>()(
