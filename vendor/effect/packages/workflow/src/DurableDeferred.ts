...
>(options: {
  ...
  success: Schema.Schema<
    Effects[number] extends Effect.Effect<infer S, infer _E, infer _R>
      ? S
      : never,
    SI,
    SR
  >
  error: Schema.Schema<
    Effects[number] extends Effect.Effect<infer _S, infer E, infer _R>
      ? E
      : never,
    EI,
    ER
  ...
  Effects[number] extends Effect.Effect<infer _A, infer _E, infer _R>
    ? _A
    : never,
  Effects[number] extends Effect.Effect<infer _A, infer _E, infer _R>
    ? _E
    : never,
  | (Effects[number] extends Effect.Effect<infer _A, infer _R, infer R>
      ? R
      : never)
  | SR
  | ER
 ...
 * @category Token
 */
export const TokenTypeId: unique symbol = Symbol.for(
  "@effect/workflow/DurableDeferred/Token"
)
 ...
 * @category Token
 */
export const Token: Schema.brand<typeof Schema.String, typeof TokenTypeId> =
...
export const tokenFromPayload: {
  <W extends Workflow.Any>(options: {
    readonly workflow: W
    readonly payload: Schema.Simplify<
      Schema.Struct.Constructor<W["payloadSchema"]["fields"]>
    >
  }): <Success extends Schema.Schema.Any, Error extends Schema.Schema.All>(
    self: DurableDeferred<Success, Error>
  ) => Effect.Effect<Token>
