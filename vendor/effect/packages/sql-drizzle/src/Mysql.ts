...
) => Effect.Effect<MySqlRemoteDatabase, never, Client.SqlClient> = (config) =>
  Effect.gen(function* () {
    const db = drizzle(yield* makeRemoteCallback, config)
    return db
  })
 ...
 * @category layers
 */
export const layer: Layer.Layer<MysqlDrizzle, never, Client.SqlClient> =
  Layer.effect(MysqlDrizzle, make())
...
export const layerWithConfig: (
  config: DrizzleConfig
) => Layer.Layer<MysqlDrizzle, never, Client.SqlClient> = (config) =>
