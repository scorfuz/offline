...
class ORM extends Effect.Service<ORM>()("ORM", {
  effect: Mysql.make({ schema: { users } })
}) {
  static Client = this.Default.pipe(
    Layer.provideMerge(MysqlContainer.ClientLive)
  )
}
...
describe.sequential("Mysql", () => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        const rows = yield* Effect.promise(() =>
          db
            .insert(users)
            .values({ name: "Alice", snakeCase: "snake" })
            .$returningId()
        )
  ...
  it.effect(
    "remote callback multiple values",
    () =>
      Effect.gen(function* () {
        const sql = yield* SqlClient.SqlClient
        const db = yield* Mysql.MysqlDrizzle
