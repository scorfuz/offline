...
describe.concurrent("Doc", () => {
  describe.concurrent("constructors", () => {
    ...
    it("flatAlt", () => {
      ...
      expect(
        Doc.render(prettyDo(statements), {
          style: "pretty",
    ...
    it("union", () => {
      ...
      expect(
        Doc.render(doc, { style: "pretty", options: { lineWidth: 1 } })
      ).toBe("b")
    })
    ...
    it("line", () => {
      ...
      expect(Doc.render(Doc.group(doc), { style: "pretty" })).toBe(
    ...
    it("softLine", () => {
      ...
      expect(
        Doc.render(doc, { style: "pretty", options: { lineWidth: 10 } })
      ).toBe(
        String.stripMargin(
          `|lorem ipsum
         |dolor sit amet`
    ...
    it("hardLine", () => {
      ...
      expect(
        Doc.render(doc, { style: "pretty", options: { lineWidth: 1000 } })
      ).toBe(
        String.stripMargin(
          `|lorem ipsum
           |dolor sit amet`
    ...
    it("withPageWidth", () => {
          ...
          switch (pageWidth._tag) {
            case "AvailablePerLine": {
              ...
              return Doc.squareBracketed(
                Doc.text(
                  `Width: ${lineWidth}, Ribbon Fraction: ${ribbonFraction}`
                )
              )
            }
            case "Unbounded": {
      ...
      const doc = Doc.vsep([0, 4, 8].map((n) => Doc.indent(prefix, n)))
      expect(
        Doc.render(doc, { style: "pretty", options: { lineWidth: 32 } })
      ).toBe(
        String.stripMargin(
  ...
  describe.concurrent("refinements", () => {
    ...
    it("isFlatAlt", () => {
      expect(Doc.isFlatAlt(Doc.flatAlt(Doc.char("a"), Doc.char("b")))).toBe(
        true
      )
  ...
  describe.concurrent("alternative combinators", () => {
    describe.concurrent("group", () => {
      it("should ensure that the `left` document is less wide than the `right`", () => {
        const doc = Doc.group(
          Doc.flatAlt(Doc.text("even wider"), Doc.text("too wide"))
        )
        // If the `right` document does not fit the page, the algorithm falls
        // back to an even wider layout
        expect(
          Doc.render(doc, { style: "pretty", options: { lineWidth: 7 } })
        ).toBe("even wider")
      })
      ...
      it("should flatten the right document", () => {
        const doc = Doc.group(
          Doc.flatAlt(
            Doc.char("x"),
            Doc.hcat([Doc.char("y"), Doc.line, Doc.char("y")])
          )
        )
      ...
      it("should never render an unflattenable `right` document", () => {
        const doc = Doc.group(
          Doc.flatAlt(
            Doc.char("x"),
            Doc.hcat([Doc.char("y"), Doc.hardLine, Doc.char("y")])
          )
        )
  ...
  describe.concurrent("sep combinators", () => {
    it("hsep", () => {
      ...
      expect(
        Doc.render(doc, { style: "pretty", options: { lineWidth: 5 } })
    ...
    it("sep", () => {
      ...
      expect(
        Doc.render(doc, { style: "pretty", options: { lineWidth: 20 } })
      ).toBe(
        String.stripMargin(
          `|prefix text
           |to
  ...
  describe.concurrent("alignment combinators", () => {
    ...
    it("hang", () => {
      ...
      expect(
        Doc.render(doc, { style: "pretty", options: { lineWidth: 24 } })
      ).toBe(
        String.stripMargin(
    ...
    it("encloseSep", () => {
      const doc = Doc.hsep([
        ...
        Doc.align(
          Doc.encloseSep(
            ["1", "20", "300", "4000"].map((n) =>
              n.length === 1 ? Doc.char(n) : Doc.text(n)
            ),
            Doc.lbracket,
            Doc.rbracket,
            Doc.comma
