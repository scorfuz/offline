  ...
  describe("render", () => {
    it("should render an annotated document in tree-form", () => {
      ...
      const header = (
        doc: Doc.Doc<SimpleHtml>,
        level: number
      ): Doc.Doc<SimpleHtml> => Doc.annotate(doc, { _tag: "Header", level })
      ...
      const document = Doc.vsep([
        ...
        paragraph(
          Doc.hsep([
            Doc.text("and"),
            italicized(Doc.text("this is italicized!"))
          ])
        )
      ])

      expect(render(Layout.pretty(document, Layout.defaultOptions))).toBe(
        String.stripMargin(
