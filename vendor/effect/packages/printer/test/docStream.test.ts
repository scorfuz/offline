...
describe.concurrent("DocStream", () => {
  ...
  it("isCharStream", () => {
    expect(DocStream.isCharStream(DocStream.char(DocStream.empty, "a"))).toBe(
      true
    )
  ...
  it("isTextStream", () => {
    expect(DocStream.isTextStream(DocStream.text(DocStream.empty, "foo"))).toBe(
      true
    )
  ...
  it("isLineStream", () => {
    expect(DocStream.isLineStream(DocStream.line(DocStream.empty, 4))).toBe(
      true
    )
  ...
  it("isPushAnnotationStream", () => {
    expect(
      DocStream.isPushAnnotationStream(
        DocStream.pushAnnotation(DocStream.empty, 1)
      )
    ).toBe(true)
  ...
  it("isPopAnnotationStream", () => {
    expect(
      DocStream.isPopAnnotationStream(DocStream.popAnnotation(DocStream.empty))
    ).toBe(true)
