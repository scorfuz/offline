...
const protoEqual = {
  ...
  LineTree: (self: DocTree.LineTree<any>, that: unknown) =>
    isDocTree(that) &&
    that._tag === "LineTree" &&
    self.indentation === that.indentation,
...
/** @internal */
export const isEmptyTree = <A>(
  self: DocTree.DocTree<A>
): self is DocTree.EmptyTree<A> => self._tag === "EmptyTree"

/** @internal */
export const isCharTree = <A>(
  self: DocTree.DocTree<A>
): self is DocTree.CharTree<A> => self._tag === "CharTree"

/** @internal */
export const isTextTree = <A>(
  self: DocTree.DocTree<A>
): self is DocTree.TextTree<A> => self._tag === "TextTree"

/** @internal */
export const isLineTree = <A>(
  self: DocTree.DocTree<A>
): self is DocTree.LineTree<A> => self._tag === "LineTree"

/** @internal */
export const isAnnotationTree = <A>(
  self: DocTree.DocTree<A>
): self is DocTree.AnnotationTree<A> => self._tag === "AnnotationTree"

/** @internal */
export const isConcatTree = <A>(
  self: DocTree.DocTree<A>
): self is DocTree.ConcatTree<A> => self._tag === "ConcatTree"
...
const renderSimplyDecoratedSafe = <A, M>(
  ...
  switch (self._tag) {
    ...
    case "AnnotationTree": {
      return Effect.map(
        Effect.suspend(() =>
          renderSimplyDecoratedSafe(self.tree, M, renderText, renderAnnotation)
        ),
    ...
    case "ConcatTree": {
      ...
      return Arr.reduce(
        tail,
        Effect.suspend(() =>
          renderSimplyDecoratedSafe(head, M, renderText, renderAnnotation)
        ),
        (acc, tree) =>
          Effect.zipWith(
            acc,
            Effect.suspend(() =>
              renderSimplyDecoratedSafe(tree, M, renderText, renderAnnotation)
            ),
            M.combine
          )
      )
...
const parserSucceed =
  <S, A>(value: A): DocTreeParser<S, A> =>
  (stream) =>
    Option.some([value, stream] as const)

const parserMap =
  <S, A, B>(self: DocTreeParser<S, A>, f: (a: A) => B): DocTreeParser<S, B> =>
  (stream) =>
    Option.map(self(stream), ([a, s]) => [f(a), s] as const)

const parserFlatMap =
  <S, A, B>(
    self: DocTreeParser<S, A>,
    f: (a: A) => DocTreeParser<S, B>
  ): DocTreeParser<S, B> =>
  (stream) =>
...
): DocTreeParser<S, ReadonlyArray<A>> {
  return (stream) =>
    pipe(
      parser(stream),
...
const nextToken = <A>(): DocTreeParser<
  DocStream.DocStream<A>,
  docTreeToken.DocTreeToken<A>
> => {
  return (stream) => {
    switch (stream._tag) {
      case "FailedStream": {
      ...
      case "CharStream": {
        return Option.some([
          docTreeToken.char(stream.char),
          stream.stream
        ] as const)
      }
      case "TextStream": {
        return Option.some([
          docTreeToken.text(stream.text),
          stream.stream
        ] as const)
      }
      case "LineStream": {
        return Option.some([
          docTreeToken.line(stream.indentation),
          stream.stream
        ] as const)
      }
...
/** @internal */
export const parser = <A>(): DocTreeParser<
  DocStream.DocStream<A>,
  DocTree.DocTree<A>
