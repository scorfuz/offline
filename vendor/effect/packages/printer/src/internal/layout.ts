...
): Layout.Layout.Options => ({ pageWidth })

/** @internal */
export const defaultOptions: Layout.Layout.Options = options(
  InternalPageWidth.defaultPageWidth
)
        ...
        case "Cons": {
          switch (self.document._tag) {
            ...
            case "Line": {
              ...
              const nextIndent =
                InternalDocStream.isEmptyStream(stream) ||
                InternalDocStream.isLineStream(stream)
                  ? 0
                  : self.indent
              return InternalDocStream.line(stream, nextIndent)
            }
            case "FlatAlt": {
              const next = InternalPipeline.cons(
                self.indent,
                self.document.left,
                self.pipeline
              )
            ...
            case "Cat": {
              const inner = InternalPipeline.cons(
                self.indent,
                self.document.right,
                self.pipeline
              )
              const outer = InternalPipeline.cons(
                self.indent,
                self.document.left,
                inner
              )
            ...
            case "Union": {
              const leftPipeline = InternalPipeline.cons(
                self.indent,
                self.document.left,
                self.pipeline
              )
              const rightPipeline = InternalPipeline.cons(
                self.indent,
                self.document.right,
                self.pipeline
              )
            ...
            case "Annotated": {
              ...
              return InternalDocStream.pushAnnotation(
                stream,
                self.document.annotation
              )
            }
          }
        }
    ...
    switch (head._tag) {
      ...
      case "WithPageWidth": {
        return yield* compactSafe(
          List.cons(head.react(InternalPageWidth.unbounded), tail),
          i
        )
      }
      case "Nesting": {
...
export const unbounded = <A>(self: Doc.Doc<A>): DocStream.DocStream<A> =>
  wadlerLeijen(self, (stream) => !failsOnFirstLine(stream), {
