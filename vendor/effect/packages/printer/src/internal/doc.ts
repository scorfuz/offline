...
/** @internal */
export const DocTypeId: Doc.DocTypeId = Symbol.for(
  DocSymbolKey
) as Doc.DocTypeId
...
const protoHash = {
  Fail: (_: Doc.Fail<any>) =>
    Hash.combine(Hash.hash(DocSymbolKey))(
      Hash.hash("@effect/printer/Doc/Fail")
    ),
  Empty: (_: Doc.Empty<any>) =>
    Hash.combine(Hash.hash(DocSymbolKey))(
      Hash.hash("@effect/printer/Doc/Empty")
    ),
  Char: (self: Doc.Char<any>) =>
    Hash.combine(Hash.hash(DocSymbolKey))(Hash.string(self.char)),
  Text: (self: Doc.Text<any>) =>
    Hash.combine(Hash.hash(DocSymbolKey))(Hash.string(self.text)),
  Line: (_: Doc.Line<any>) =>
    Hash.combine(Hash.hash(DocSymbolKey))(
      Hash.hash("@effect/printer/Doc/Line")
    ),
...
const protoEqual = {
  Fail: (_: Doc.Fail<any>, that: unknown) =>
    isDoc(that) && that._tag === "Fail",
  Empty: (_: Doc.Empty<any>, that: unknown) =>
    isDoc(that) && that._tag === "Empty",
  ...
  Line: (_: Doc.Line<any>, that: unknown) =>
    isDoc(that) && that._tag === "Line",
...
/** @internal */
export const isFail = <A>(self: Doc.Doc<A>): self is Doc.Fail<A> =>
  self._tag === "Fail"

/** @internal */
export const isEmpty = <A>(self: Doc.Doc<A>): self is Doc.Empty<A> =>
  self._tag === "Empty"

/** @internal */
export const isChar = <A>(self: Doc.Doc<A>): self is Doc.Char<A> =>
  self._tag === "Char"

/** @internal */
export const isText = <A>(self: Doc.Doc<A>): self is Doc.Text<A> =>
  self._tag === "Text"

/** @internal */
export const isLine = <A>(self: Doc.Doc<A>): self is Doc.Line<A> =>
  self._tag === "Line"

/** @internal */
export const isFlatAlt = <A>(self: Doc.Doc<A>): self is Doc.FlatAlt<A> =>
  self._tag === "FlatAlt"

/** @internal */
export const isCat = <A>(self: Doc.Doc<A>): self is Doc.Cat<A> =>
  self._tag === "Cat"

/** @internal */
export const isNest = <A>(self: Doc.Doc<A>): self is Doc.Nest<A> =>
  self._tag === "Nest"

/** @internal */
export const isUnion = <A>(self: Doc.Doc<A>): self is Doc.Union<A> =>
  self._tag === "Union"

/** @internal */
export const isColumn = <A>(self: Doc.Doc<A>): self is Doc.Column<A> =>
  self._tag === "Column"

/** @internal */
export const isWithPageWidth = <A>(
  self: Doc.Doc<A>
): self is Doc.WithPageWidth<A> => self._tag === "WithPageWidth"

/** @internal */
export const isNesting = <A>(self: Doc.Doc<A>): self is Doc.Nesting<A> =>
  self._tag === "Nesting"
...
/** @internal */
export const cats = <A>(docs: Iterable<Doc.Doc<A>>): Doc.Doc<A> =>
  group(vcat(docs))
...
/** @internal */
export const seps = <A>(docs: Iterable<Doc.Doc<A>>): Doc.Doc<A> =>
  group(vsep(docs))
...
export const group = <A>(self: Doc.Doc<A>): Doc.Doc<A> => {
  switch (self._tag) {
    ...
    default: {
      ...
      return InternalFlatten.isFlattened(flattened)
        ? union(flattened.value, self)
        : self
    }
  }
}
...
/** @internal */
export const flatten = <A>(self: Doc.Doc<A>): Doc.Doc<A> =>
  Effect.runSync(flattenSafe(self))
...
/** @internal */
export const changesUponFlattening = <A>(
  self: Doc.Doc<A>
): Flatten.Flatten<Doc.Doc<A>> =>
  ...
  Effect.gen(function* () {
    switch (self._tag) {
      ...
      case "Cat": {
        ...
        if (
          InternalFlatten.isNeverFlat(left) ||
          InternalFlatten.isNeverFlat(right)
        ) {
          return InternalFlatten.neverFlat
        }
        if (
          InternalFlatten.isFlattened(left) &&
          InternalFlatten.isFlattened(right)
        ) {
          return InternalFlatten.flattened(cat(left.value, right.value))
        }
        if (
          InternalFlatten.isFlattened(left) &&
          InternalFlatten.isAlreadyFlat(right)
        ) {
          return InternalFlatten.flattened(cat(left.value, self.right))
        }
        if (
          InternalFlatten.isAlreadyFlat(left) &&
          InternalFlatten.isFlattened(right)
        ) {
      ...
      case "Column": {
        const doc = column((position) =>
          Effect.runSync(flattenSafe(self.react(position)))
        )
        return InternalFlatten.flattened(doc)
      }
      case "WithPageWidth": {
        const doc = pageWidth((pageWidth) =>
          Effect.runSync(flattenSafe(self.react(pageWidth)))
        )
        return InternalFlatten.flattened(doc)
      }
      case "Nesting": {
        const doc = nesting((level) =>
          Effect.runSync(flattenSafe(self.react(level)))
        )
        return InternalFlatten.flattened(doc)
      }
      case "Annotated": {
        return yield* pipe(
          changesUponFlatteningSafe(self.doc),
          Effect.map(
            InternalFlatten.map((doc) => annotate(doc, self.annotation))
          )
        )
      }
    }
  ...
  switch (self._tag) {
    ...
    case "Column": {
      return Effect.succeed(
        column((position) =>
          Effect.runSync(alterAnnotationsSafe(self.react(position), f))
        )
      )
    }
    case "WithPageWidth": {
      return Effect.succeed(
        pageWidth((pageWidth) =>
          Effect.runSync(alterAnnotationsSafe(self.react(pageWidth), f))
        )
      )
    }
    case "Nesting": {
      return Effect.succeed(
        nesting((level) =>
          Effect.runSync(alterAnnotationsSafe(self.react(level), f))
        )
      )
    }
    case "Annotated": {
      return Effect.map(alterAnnotationsSafe(self.doc, f), (doc) =>
        Arr.reduceRight(Arr.fromIterable(f(self.annotation)), doc, (doc, b) =>
          annotate(doc, b)
        )
      )
    }
...
export const singleQuoted = <A>(self: Doc.Doc<A>): Doc.Doc<A> =>
  surround(self, squote, squote)

/** @Internal */
export const doubleQuoted = <A>(self: Doc.Doc<A>): Doc.Doc<A> =>
  surround(self, dquote, dquote)

/** @internal */
export const parenthesized = <A>(self: Doc.Doc<A>): Doc.Doc<A> =>
  surround(self, lparen, rparen)

/** @internal */
export const angleBracketed = <A>(self: Doc.Doc<A>): Doc.Doc<A> =>
  surround(self, langle, rangle)
...
/** @internal */
export const curlyBraced = <A>(self: Doc.Doc<A>): Doc.Doc<A> =>
  surround(self, lbrace, rbrace)
...
/** @internal */
export const reflow = (s: string, char = " "): Doc.Doc<never> =>
