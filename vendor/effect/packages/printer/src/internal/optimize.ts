      ...
      switch (self._tag) {
        ...
        case "Cat": {
          ...
          if (
            ...
            const inner = yield* optimize(
              InternalDoc.cat(self.left, self.right.left)
            )
          ...
          if (
            ...
            const inner = yield* optimize(
              InternalDoc.cat(self.left.right, self.right)
            )
        ...
        case "Column": {
          return depth._tag === "Shallow"
            ? self
            : InternalDoc.column((position) =>
                Effect.runSync(optimizeSafe(self.react(position), depth))
              )
        }
        case "WithPageWidth": {
          return depth._tag === "Shallow"
            ? self
            : InternalDoc.pageWidth((pageWidth) =>
                Effect.runSync(optimizeSafe(self.react(pageWidth), depth))
              )
        }
        case "Nesting": {
          return depth._tag === "Shallow"
            ? self
            : InternalDoc.nesting((level) =>
                Effect.runSync(optimizeSafe(self.react(level), depth))
              )
        }
        case "Annotated": {
          return InternalDoc.annotate(
            yield* optimize(self.doc),
            self.annotation
          )
        }
      }
