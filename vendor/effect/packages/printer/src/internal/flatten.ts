...
const protoHash = {
  ...
  NeverFlat: (_: Flatten.AlreadyFlat<any>) =>
    Hash.combine(Hash.string("@effect/printer/Flattened/NeverFlat"))(
      Hash.string(FlattenSymbolKey)
    )
}
...
/** @internal */
export const isFlattened = <A>(
  self: Flatten.Flatten<A>
): self is Flatten.Flattened<A> => self._tag === "Flattened"

/** @internal */
export const isAlreadyFlat = <A>(
  self: Flatten.Flatten<A>
): self is Flatten.AlreadyFlat<A> => self._tag === "AlreadyFlat"

/** @internal */
export const isNeverFlat = <A>(
  self: Flatten.Flatten<A>
): self is Flatten.NeverFlat<A> => self._tag === "NeverFlat"
