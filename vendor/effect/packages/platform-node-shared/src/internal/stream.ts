...
export const fromReadable = <E, A = Uint8Array>(
  ...
  Stream.fromChannel(fromReadableChannel<E, A>(evaluate, onError, options))
      ...
      Effect.async((resume) => {
        ...
        stream.on("data", (chunk) => {
          ...
          if (maxBytesNumber && bytes > maxBytesNumber) {
            resume(
              Effect.fail(options.onFailure(new Error("maxBytes exceeded")))
            )
          }
        })
      }),
  ...
  Channel.suspend(() => {
    ...
    return Channel.embedInput(
      ...
      writeInput<IE, I>(
        duplex,
        (cause) =>
          Effect.sync(() => MutableRef.set(exit, Exit.failCause(cause))),
        options
      )
    )
  })
...
): Channel.Channel<Chunk.Chunk<A>, unknown, E> =>
  Channel.suspend(() =>
    unsafeReadableRead(evaluate(), onError, MutableRef.make(undefined), options)
  )
...
export const writeEffect =
  <A>(writable: Writable | NodeJS.WritableStream, encoding?: BufferEncoding) =>
  (chunk: Chunk.Chunk<A>) =>
    chunk.length === 0
      ? Effect.void
      : Effect.async<void>((resume) => {
...
const unsafeReadableRead = <A, E>(
  ...
  const read = Channel.suspend(function loop(): Channel.Channel<
    Chunk.Chunk<A>,
    unknown,
    E
  > {
