        ...
        () =>
          Effect.async<Net.Socket, Socket.SocketError, never>((resume) => {
            ...
            conn.on("error", (cause) => {
              resume(
                Effect.fail(
                  new Socket.SocketGenericError({ reason: "Open", cause })
                )
              )
            })
          })
...
export const fromDuplex = <RO>(
  ...
  Effect.withFiberRuntime<Socket.Socket, never, Exclude<RO, Scope.Scope>>(
    (fiber) => {
      ...
      const run = <R, E, _>(
        ...
        Effect.scopedWith(
          Effect.fnUntraced(function* (scope) {
            ...
            function onError(cause: Error) {
              Deferred.unsafeDone(
                fiberSet.deferred,
                Effect.fail(
                  new Socket.SocketGenericError({ reason: "Read", cause })
                )
              )
            }
          ...
          Effect.mapInputContext((input: Context.Context<R>) =>
            Context.merge(openContext, input)
          ),
          Effect.ensuring(
            Effect.sync(() => {
              latch.unsafeClose()
          ...
          Effect.async<void, Socket.SocketError>((resume) => {
            ...
            currentSocket!.write(chunk, (cause) => {
              resume(
                cause
                  ? Effect.fail(
                      new Socket.SocketGenericError({ reason: "Write", cause })
                    )
                  : Effect.void
              )
            })
...
export const makeNetChannel = <IE = never>(
  ...
  unknown
> =>
  Channel.unwrapScoped(Effect.map(makeNet(options), Socket.toChannelWith<IE>()))
