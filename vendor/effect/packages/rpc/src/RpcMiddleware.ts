...
> extends TagClass.Base<
  ...
  TagClass.Wrap<Options> extends true
    ? RpcMiddlewareWrap<TagClass.Provides<Options>, TagClass.Failure<Options>>
...
export declare namespace TagClass {
   ...
   * @category models
   */
  export type RequiredForClient<Options> = Options extends {
   ...
   * @category models
   */
  export type Wrap<Options> = Options extends { readonly wrap: true }
    ? true
    : false
 ...
 * @category models
 */
export interface TagClassAnyWithProps extends Context.Tag<
  any,
  RpcMiddleware<any, any> | RpcMiddlewareWrap<any, any>
> {
  readonly [TypeId]: TypeId
  readonly optional: boolean
  readonly provides?: Context.Tag<any, any>
...
export const layerClient = <Id, S, R, EX = never, RX = never>(
  ...
  service:
    | RpcMiddlewareClient<R>
    | Effect.Effect<RpcMiddlewareClient<R>, EX, RX>
      ...
      return Context.unsafeMake(
        new Map([
          [
            `${tag.key}/Client`,
            (options: any) =>
              Effect.mapInputContext(middleware(options), (requestContext) =>
