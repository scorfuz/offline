...
> = Effect.fnUntraced(function* <Rpcs extends Rpc.Any>(
      ...
      Effect.withFiberRuntime((requestFiber) => {
        ...
        switch (message._tag) {
          ...
          case "Interrupt": {
            ...
            return fiber
              ? Fiber.interruptAsFork(fiber, fiberIdClientInterrupt)
  ...
  const handleRequest = (
    ...
    if (client.fibers.has(request.id)) {
      return Effect.flatMap(Fiber.await(client.fibers.get(request.id)!), () =>
    ...
    if (enableTracing) {
      const parentSpan = requestFiber.currentContext.unsafeMap.get(
  ...
  const streamEffect = (
    ...
    stream:
      ...
      | Effect.Effect<Mailbox.ReadonlyMailbox<any, any>, any, Scope.Scope>
  ) => {
      ...
      return stream.pipe(
        Effect.flatMap((mailbox) =>
          Effect.whileLoop({
            while: () => !done,
            body: constant(
              Effect.flatMap(mailbox.takeAll, ([chunk, done_]) => {
                done = done_
...
const applyMiddleware = <A, E, R>(
  ...
  for (const tag of rpc.middlewares) {
    if (tag.wrap) {
      ...
      const middleware = Context.unsafeGet(context, tag) as RpcMiddleware<
        any,
        any
      >
...
> = Effect.fnUntraced(function* <Rpcs extends Rpc.Any>(
  ...
  yield* Effect.fork(
    Effect.interruptible(
      Effect.whileLoop({
        while: constTrue,
        body: constant(
          Effect.flatMap(disconnects.take, (clientId) => {
            clients.delete(clientId)
  ...
  type Schemas = {
    readonly decode: (
      u: unknown
    ) => Effect.Effect<Rpc.Payload<Rpcs>, ParseError>
    readonly encodeChunk: (
      u: ReadonlyArray<unknown>
    ) => Effect.Effect<NonEmptyReadonlyArray<unknown>, ParseError>
    readonly encodeExit: (
      u: unknown
    ) => Effect.Effect<
  ...
  const handleEncode = <A, R>(
    ...
    (collector
      ...
      Effect.flatMap((a) =>
        send(client.id, onSuccess(a), collector && collector.unsafeClear())
      ),
      Effect.catchAllCause((cause) => {
        client.schemas.delete(requestId)
        const defect = Cause.squash(
          Cause.map(cause, TreeFormatter.formatErrorSync)
        )
  ...
  const sendRequestDefect = (
    ...
    Effect.catchAllCause(
      encodeDefect(defect).pipe(
        Effect.flatMap((encodedDefect) =>
          send(client.id, {
            _tag: "Exit",
  ...
  return yield* run((clientId, request) => {
    ...
    switch (request._tag) {
      ...
      case "Ping": {
        return Effect.catchAllCause(send(client.id, constPong), (cause) =>
...
export const layer = <Rpcs extends Rpc.Any>(
  ...
  never,
  never,
  Protocol | Rpc.ToHandler<Rpcs> | Rpc.Context<Rpcs> | Rpc.Middleware<Rpcs>
> =>
  Layer.scopedDiscard(
    Effect.forkScoped(Effect.interruptible(make(group, options)))
  )
 ...
 * @category protocol
 */
export class Protocol extends Context.Tag("@effect/rpc/RpcServer/Protocol")<
  Protocol,
  {
    readonly run: (
...
export const makeProtocolSocketServer = Effect.gen(function* () {
  ...
  yield* server
    .run(Effect.fnUntraced(onSocket, Effect.scoped))
 ...
 * @category protocol
 */
export const makeProtocolWebsocket: <I = HttpRouter.Default>(options: {
  readonly path: HttpRouter.PathInput
...
> = Effect.fnUntraced(function* <I = HttpRouter.Default>(options: {
  ...
  const router = yield* options.routerTag ??
    (HttpRouter.Default as any as Context.Tag<
      I,
      HttpRouter.HttpRouter.Service<any, any>
    >)
 ...
 * @category protocol
 */
export const makeProtocolWebsocketRouter: (options: {
  readonly path: HttpRouter.PathInput
}) => Effect.Effect<
  Protocol["Type"],
  never,
...
export const layerProtocolWebsocket = <I = HttpRouter.Default>(options: {
  ...
  const routerTag =
    options.routerTag ??
    (HttpRouter.Default as any as HttpRouter.HttpRouter.TagClass<
      I,
      string,
      any,
      any
...
> = Effect.gen(function* () {
  ...
  const httpApp: HttpApp.Default<never, Scope.Scope> = Effect.gen(function* () {
    ...
    >(
      isBinary
        ? Effect.map(request.arrayBuffer, (ab) => new Uint8Array(ab))
        : request.text
    )
    const id = clientId++
    ...
    const client: Client = {
      write: !includesFraming
        ? (response) => mailbox.offer(response)
        : (response) => {
            try {
    ...
    if (done) {
      return HttpServerResponse.uint8Array(
        mergeUint8Arrays(initialChunk as Chunk.Chunk<Uint8Array>),
        {
          contentType: serialization.contentType
        }
      )
    ...
    return HttpServerResponse.stream(
      Stream.fromChunk(initialChunk as Chunk.Chunk<Uint8Array>).pipe(
        Stream.concat(
          Mailbox.toStream(mailbox as Mailbox.ReadonlyMailbox<Uint8Array>)
        )
      ),
      { contentType: serialization.contentType }
    )
 ...
 * @category protocol
 */
export const makeProtocolHttp = Effect.fnUntraced(function* <
  I = HttpRouter.Default
>(options: {
  readonly path: HttpRouter.PathInput
  readonly routerTag?: HttpRouter.HttpRouter.TagClass<I, string, any, any>
}) {
...
> = Protocol.make(
  Effect.fnUntraced(function* (writeRequest) {
    const fiber = yield* Effect.withFiberRuntime<Fiber.RuntimeFiber<void>>(
 ...
 * @category protocol
 */
export const makeProtocolStdio = Effect.fnUntraced(function* <
  EIn,
  EOut,
  RIn,
  ROut
...
export const layerProtocolStdio = <EIn, EOut, RIn, ROut>(options: {
  ...
  Protocol,
  never,
  RpcSerialization.RpcSerialization | RIn | ROut
 ...
 * @category Interruption
 */
export const fiberIdTransientInterrupt = FiberId.make(
  -503,
  0
) as FiberId.Runtime
