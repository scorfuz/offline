...
export declare namespace RpcClient {
   ...
   * @category client
   */
  export type NonPrefixed<Rpcs extends Rpc.Any> = Exclude<
    Rpcs,
  ...
  > = {
    ...
    >(
      ...
      options?: [Rpc.SuccessSchema<Current>] extends [
...
>(
    ...
    Effect.withFiberRuntime<any, any, any>((parentFiber) => {
      ...
      return Effect.onInterrupt(
        ...
        (interruptors) => {
          ...
          entries.delete(id)
          const ids = Array.from(interruptors).flatMap((id) =>
            Array.from(FiberId.toSet(id))
          )
  ...
  const onStreamRequest = Effect.fnUntraced(function* (
    ...
    yield* Scope.addFinalizerExit(scope, (exit) => {
      ...
      return sendInterrupt(
        id,
        Exit.isFailure(exit)
          ? Array.from(Cause.interruptors(exit.cause)).flatMap((id) =>
              Array.from(FiberId.toSet(id))
            )
          : [],
        context
  ...
  const getRpcClientMiddleware = (
    rpc: Rpc.AnyWithProps
  ): ((request: Request<Rpcs>) => Effect.Effect<Request<Rpcs>>) => {
      ...
      case "Chunk": {
        ...
        return entry.mailbox.offerAll(message.values).pipe(
          ...
          Effect.catchAllCause((cause) =>
            entry.mailbox.done(Exit.failCause(cause))
          )
        )
      }
      case "Exit": {
  ...
  if (options.flatten) {
    ...
    for (const rpc of group.requests.values()) {
      ...
      if (prefix !== undefined && !(prefix in client)) {
        ;(client as any)[prefix] = {} as Mutable<
          RpcClient.Prefixed<Rpcs, typeof prefix>
        >
      }
  ...
  type ClientEntry = {
    ...
    readonly decodeChunk:
      | ((
        ...
        ) => Effect.Effect<NonEmptyReadonlyArray<any>, ParseError, unknown>)
      | undefined
  }
  const entries = new Map<RequestId, ClientEntry>()
          ...
          const entry: ClientEntry = {
            ...
            decodeChunk: Option.isSome(schemas)
              ? Schema.decodeUnknown(
                  Schema.NonEmptyArray(schemas.value.success)
                )
              : undefined
          }
  ...
  yield* run((message) => {
    switch (message._tag) {
      ...
      case "Exit": {
        ...
        if (!entry) return Effect.void
        entries.delete(requestId)
        return Schema.decode(Rpc.exitSchema(entry.rpc as any))(
          message.exit
        ).pipe(
          Effect.locally(FiberRef.currentContext, entry.context),
          Effect.orDie,
...
export const withHeadersEffect: {
  ...
  <A, E, R, E2, R2>(
    ...
    Effect.flatMap(headers, (headers) => withHeaders(effect, headers))
)
 ...
 * @category protocol
 */
export class Protocol extends Context.Tag("@effect/rpc/RpcClient/Protocol")<
  Protocol,
  {
    readonly run: (
        ...
        return client.post("", { body }).pipe(
          Effect.flatMap((r) =>
            Stream.runForEachChunk(r.stream, (chunk) => {
              const responses = Chunk.toReadonlyArray(chunk).flatMap(
                parser.decode
  ...
  Protocol.make(
    Effect.fnUntraced(function* (writeResponse) {
      ...
      const entries = new Map<
        string,
        {
          readonly worker: Worker.BackingWorker<
