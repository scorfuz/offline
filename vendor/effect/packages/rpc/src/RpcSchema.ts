 ...
 * @category Stream
 */
export const StreamSchemaId: unique symbol = Symbol.for(
  "@effect/rpc/RpcSchema/Stream"
)
 ...
 * @category Stream
 */
export const isStreamSchema = (
  schema: Schema.Schema.All
): schema is Stream<any, any> =>
...
export const getStreamSchemas = (
  ...
  ast.annotations[StreamSchemaId]
    ? Option.some(ast.annotations[StreamSchemaId] as any)
    : Option.none()
...
const isStream = (u: unknown): u is Stream_.Stream<unknown, unknown> =>
  hasProperty(u, Stream_.StreamTypeId)
...
const parseStream =
  <A, E, RA, RE>(
    ...
    decodeFailure: (
      u: unknown,
      overrideOptions?: AST.ParseOptions
    ) => Effect.Effect<E, ParseResult.ParseIssue, RE>
  ) =>
