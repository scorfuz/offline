...
export interface RpcGroup<in out R extends Rpc.Any> extends Pipeable {
  ...
  toLayer<Handlers extends HandlersFrom<R>, EX = never, RX = never>(
    ...
    Rpc.ToHandler<R>,
    EX,
    Exclude<RX, Scope> | HandlersContext<R, Handlers>
  >
  ...
  >(
    ...
    Rpc.Handler<Tag>,
    EX,
    Exclude<RX, Scope> | HandlerContext<R, Tag, Handler>
  >
...
const RpcGroupProto = {
  ...
  accessHandler(this: RpcGroup<any>, tag: string) {
    return Effect.contextWith((parentContext: Context.Context<any>) => {
      const rpc = this.requests.get(tag)!
      const { context, handler } = parentContext.unsafeMap.get(
        rpc.key
      ) as Rpc.Handler<any>
      return (
        payload: Rpc.Payload<any>,
        options: {
  ...
  annotateRpcsContext(this: RpcGroup<any>, context: Context.Context<any>) {
    ...
    for (const [tag, rpc] of this.requests) {
      requests.set(
        tag,
        rpc.annotateContext(Context.merge(context, rpc.annotations))
      )
    }
    return makeProto({
      requests,
...
const resolveInput = <Rpcs extends ReadonlyArray<Rpc.Any>>(
  ...
  for (const rpc of rpcs) {
    requests.set(
      rpc._tag,
      Schema.isSchema(rpc) ? Rpc.fromTaggedRequest(rpc as any) : (rpc as any)
    )
  }
  return requests
}
