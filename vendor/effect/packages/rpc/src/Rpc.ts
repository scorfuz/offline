...
> extends Pipeable {
  ...
  setSuccess<S extends Schema.Schema.Any>(
    schema: S
  ): Rpc<Tag, Payload, S, Error, Middleware>

  /**
   * Set the schema for the error response of the rpc.
   */
  setError<E extends Schema.Schema.Any>(
    schema: E
  ): Rpc<Tag, Payload, Success, E, Middleware>
  ...
  setPayload<P extends Schema.Struct<any> | Schema.Struct.Fields>(
    ...
    Tag,
    P extends Schema.Struct<infer _>
      ? P
  ...
  middleware<M extends RpcMiddleware.TagClassAny>(
    middleware: M
  ): Rpc<Tag, Payload, Success, Error, Middleware | M>
  ...
  prefix<const Prefix extends string>(
    prefix: Prefix
  ): Rpc<`${Prefix}${Tag}`, Payload, Success, Error, Middleware>
...
export type SuccessExit<R> =
  SuccessSchema<R> extends infer S
    ? S extends RpcSchema.Stream<infer _A, infer _E>
      ? void
 ...
 * @category models
 */
export type SuccessChunkEncoded<R> =
  SuccessEncoded<R> extends Stream<infer _A, infer _E, infer _Env> ? _A : never
 ...
 * @category models
 */
export type ExitEncoded<R, Defect = unknown> = Schema.ExitEncoded<
  SuccessExitEncoded<R>,
  ErrorExitEncoded<R>,
  Defect
>
 ...
 * @category models
 */
export type AddError<R extends Any, Error extends Schema.Schema.All> =
  R extends Rpc<
    infer _Tag,
    infer _Payload,
    infer _Success,
 ...
 * @category models
 */
export interface From<S extends AnyTaggedRequestSchema> extends Rpc<
  S["_tag"],
  S,
  S["success"],
  S["failure"]
...
export type Prefixed<Rpcs extends Any, Prefix extends string> =
  Rpcs extends Rpc<
    ...
    infer _Middleware
  >
    ? Rpc<`${Prefix}${_Tag}`, _Payload, _Success, _Error, _Middleware>
    : never
...
>(
  ...
  options?: {
    ...
    readonly primaryKey?: [Payload] extends [Schema.Struct.Fields]
      ? (
          payload: Schema.Simplify<Schema.Struct.Type<NoInfer<Payload>>>
        ) => string
      : never
  }
): Rpc<
...
const exitSchemaCache = globalValue(
  "@effect/rpc/Rpc/exitSchemaCache",
 ...
 * @category Wrapper
 */
export const WrapperTypeId: unique symbol = Symbol.for(
 ...
 * @category Wrapper
 */
export const fork: <A extends object>(
  value: A
) => A extends Wrapper<infer _> ? A : Wrapper<A> = wrap({ fork: true })
...
export const uninterruptible: <A extends object>(
  value: A
) => A extends Wrapper<infer _> ? A : Wrapper<A> = wrap({
