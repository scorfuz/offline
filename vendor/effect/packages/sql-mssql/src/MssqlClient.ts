 ...
 * @since 1.0.0
 */
export const MssqlClient = Context.GenericTag<MssqlClient>(
...
const TransactionConnection =
  Client.TransactionConnection as unknown as Context.Tag<
    ...
    const makeConnection = Effect.gen(function* () {
      ...
      yield* Effect.async<void, SqlError>((resume) => {
        conn.connect((cause) => {
          if (cause) {
            resume(
              Effect.fail(new SqlError({ cause, message: "Failed to connect" }))
            )
          } else {
            resume(Effect.void)
          }
      ...
      Object.assign(
        ...
        }),
        {
          ...
          >(
            ...
            Effect.scoped(
              Effect.flatMap(pool.get, (_) => _.call(procedure, transformRows))
            ),
          withoutTransforms() {
            ...
            const client = Object.assign(statement, this, statement, {
              ...
              >(
                ...
                Effect.scoped(
                  Effect.flatMap(pool.get, (_) => _.call(procedure, undefined))
                )
            })
