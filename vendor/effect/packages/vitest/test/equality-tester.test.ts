...
describe("toMatchObject", () => {
  ...
  it("option", () => {
    ...
    expect({
      x: Option.some({ a: 1, b: 2 }),
      y: Option.none()
    }).not.toMatchObject({ x: Option.some({ b: 1 }) })
    expect({ x: Option.none(), y: Option.none() }).not.toMatchObject({
      x: Option.some({})
    })
  })
  ...
  it("either", () => {
    ...
    expect(Either.right({ a: 1, b: 2 })).not.toMatchObject(
      Either.left({ a: 1 })
    )
    expect(Either.left({ a: 1, b: 2 })).not.toMatchObject(
      Either.right({ a: 1 })
    )
  })
...
describe.each(["toStrictEqual", "toEqual"] as const)("%s", (matcher) => {
  ...
  it("exit", () => {
    ...
    expect(Exit.die("failure")).not[matcher](Exit.fail("failure1"))
    expect(
      Exit.failCause(Cause.sequential(Cause.fail("f1"), Cause.fail("f2")))
    ).not[matcher](
