 ...
 * @since 1.0.0
 */
export type API = { scopedFixtures: V.TestAPI<{}>["scoped"] } & {
...
export namespace Vitest {
   ...
   * @since 1.0.0
   */
  export interface MethodsNonLive<
    R = never,
    ExcludeTestServices extends boolean = false
  > extends API {
    ...
    readonly scoped: Vitest.Tester<
      | (ExcludeTestServices extends true ? never : TestServices.TestServices)
      | Scope.Scope
      | R
    >
    readonly layer: <R2, E>(
 ...
 * @since 1.0.0
 */
export const scoped: Vitest.Tester<TestServices.TestServices | Scope.Scope> =
  internal.scoped
