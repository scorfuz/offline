...
> =>
  Effect.gen(function* () {
    ...
    const EntrySql = Schema.transform(EntrySqlEncoded, EventJournal.Entry, {
      ...
      encode(toI) {
        return {
          ...
          payload: toI.payload,
          timestamp: new Date(
            EventJournal.entryIdMillis(toI.id as EventJournal.EntryId)
          )
        }
      }
    })
    ...
    const insertEntry = SqlSchema.void({
      Request: EntrySql,
      execute: (entry) =>
        sql`INSERT INTO ${sql(entryTable)} ${sql.insert(entry)} ON CONFLICT DO NOTHING`
          .withoutTransform
    })
    ...
    const insertRemotes = SqlSchema.void({
      ...
      execute: (entry) =>
        sql`INSERT INTO ${sql(remotesTable)} ${sql.insert(entry)} ON CONFLICT DO NOTHING`
          .withoutTransform
    })
    ...
    return EventJournal.EventJournal.of({
      entries:
        sql`SELECT * FROM ${sql(entryTable)} ORDER BY timestamp ASC`.withoutTransform.pipe(
      ...
      writeFromRemote: (options) =>
        Effect.gen(function* () {
          ...
          yield* sql<{
            id: Uint8Array
          }>`SELECT id FROM ${sql(entryTable)} WHERE ${sql.in(
            "id",
            entries.map((e) => e.id)
          )}`.pipe(
            Effect.tap((rows) => {
          ...
          const uncommited = options.entries.filter(
      ...
      nextRemoteSequence: (remoteId) =>
        sql<{
          max: number
        }>`SELECT MAX(sequence) AS max FROM ${sql(remotesTable)} WHERE remote_id = ${remoteId}`.pipe(
      ...
      destroy: Effect.gen(function* () {
        ...
        Effect.mapError(
          (cause) =>
            new EventJournal.EventJournalError({ cause, method: "destory" })
        )
      )
    })
  })
