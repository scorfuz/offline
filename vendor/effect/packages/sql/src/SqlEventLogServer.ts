...
import {
  ...
  EventLogEncryption,
  layerSubtle
} from "@effect/experimental/EventLogEncryption"
        ...
        Effect.gen(function* () {
          ...
          for (const batch of forInsert) {
            const encryptedEntries = yield* pipe(
              sql`INSERT INTO ${sql(table)} ${sql.insert(batch.entries)} ON CONFLICT DO NOTHING`
                .withoutTransform,
              Effect.zipRight(
      ...
      changes: (publicKey, startSequence) =>
        Effect.gen(function* () {
          ...
          yield* queue.takeBetween(1, Number.MAX_SAFE_INTEGER).pipe(
            Effect.tap((chunk) =>
              mailbox.offerAll(
                Chunk.filter(chunk, (_) => _.sequence >= startSequence)
              )
            ),
            Effect.forever,
            Effect.forkScoped,
...
const decodeEntries = Schema.decodeUnknown(
  Schema.Array(EncryptedRemoteEntryFromSql)
)
...
export const layerStorage = (options?: {
  ...
  SqlError,
  SqlClient.SqlClient | EventLogEncryption
...
export const layerStorageSubtle = (options?: {
  ...
  layerStorage(options).pipe(Layer.provide(layerSubtle))
