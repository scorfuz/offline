...
/** @internal */
export const clientTag = Context.GenericTag<Client.SqlClient>(
  "@effect/sql/SqlClient"
)
...
export function make({
  ...
  transactionAcquirer,
  transformRows
}: Client.SqlClient.MakeOptions): Effect.Effect<
  Client.SqlClient,
  never,
  ...
  return Effect.gen(function* () {
    ...
    const withTransaction = makeWithTransaction({
      ...
      acquireConnection: Effect.flatMap(Scope.make(), (scope) =>
        Effect.map(
          Scope.extend(transactionAcquirer!, scope),
...
export const makeWithTransaction =
  <I, S>(options: {
    ...
    readonly acquireConnection: Effect.Effect<
      readonly [Scope.CloseableScope | undefined, S],
      Error.SqlError
    >
    ...
    readonly savepoint: (
      conn: NoInfer<S>,
      id: number
    ) => Effect.Effect<void, Error.SqlError>
        ...
        (span) =>
          Effect.withFiberRuntime<A, E | Error.SqlError, R>((fiber) => {
            ...
            return Effect.flatMap(conn, ([scope, conn]) =>
              (id === 0
                ? options.begin(conn)
                : options.savepoint(conn, id)
              ).pipe(
                Effect.zipRight(
                  Effect.locally(
                    restore(effect),
                    FiberRef.currentContext,
                    Context.add(context, options.transactionTag, [
                      conn,
                      id
                    ]).pipe(Context.add(Tracer.ParentSpan, span))
                  )
                ),
                Effect.exit,
                Effect.flatMap((exit) => {
