...
/** @internal */
export const isParameter = (u: Statement.Segment): u is Statement.Parameter =>
  Predicate.isTagged(u, "Parameter")

/** @internal */
export const isCustom =
  <A extends Statement.Custom<any, any, any, any>>(kind: A["kind"]) =>
  (u: unknown): u is A =>
    u instanceof CustomImpl && u.kind === kind

/** @internal */
export const currentTransformer = globalValue(
  "@effect/sql/Statement/currentTransformer",
  () => FiberRef.unsafeMake(Option.none<Statement.Statement.Transformer>())
)
...
/** @internal */
export const withTransformerDisabled = <A, E, R>(
  effect: Effect.Effect<A, E, R>
) => Effect.locally(effect, currentTransformer, Option.none())

/** @internal */
export const setTransformer = (f: Statement.Statement.Transformer) =>
  ...
  Effect.withFiberRuntime<X, E, R>((fiber) => {
    ...
    return Effect.flatMap(
      transform.value(
        ...
        fiber.getFiberRefs(),
        span
      ) as Effect.Effect<StatementPrimitive<A>>,
      f
    )
  })
...
export class StatementPrimitive<A>
  extends Effectable.Class<ReadonlyArray<A>, Error.SqlError>
  implements Statement.Statement<A>
{
      ...
      Effect.flatMap(
        ...
        (span) =>
          withStatement(this, span, (statement) => {
            ...
            span.attribute(ATTR_DB_QUERY_TEXT, sql)
            return Effect.map(this.acquirer, (_) =>
              _.executeStream(sql, params, this.transformRows)
            )
          })
      )
    )
  ...
  get unprepared(): Effect.Effect<ReadonlyArray<A>, Error.SqlError> {
    return this.withConnection("executeUnprepared", (connection, sql, params) =>
      connection.executeUnprepared(sql, params, this.transformRows)
    )
  }
  ...
  commit(): Effect.Effect<ReadonlyArray<A>, Error.SqlError> {
    return this.withConnection("execute", (connection, sql, params) =>
...
function identifierWrap(
  sql: string | Statement.Identifier | Statement.Fragment
...
class RecordUpdateHelperImpl implements Statement.RecordUpdateHelper {
  ...
  returning(sql: string | Statement.Identifier | Statement.Fragment) {
    return new RecordUpdateHelperImpl(
      this.value,
      this.alias,
      identifierWrap(sql)
    )
...
class RecordUpdateHelperSingleImpl
  implements Statement.RecordUpdateHelperSingle
{
  ...
  returning(sql: string | Statement.Identifier | Statement.Fragment) {
    return new RecordUpdateHelperSingleImpl(
      this.value,
      this.omit,
      identifierWrap(sql)
    )
...
class CustomImpl<T extends string, A, B, C> implements Statement.Custom<
  T,
  A,
  B,
  C
...
export const custom =
  ...
  (i0: C["i0"], i1: C["i1"], i2: C["i2"]): Statement.Fragment =>
...
const constructorCache = globalValue(
  ...
  () => ({
    transforms: new WeakMap<
      Connection.Connection.Acquirer,
...
export const statement = (
  ...
  return new StatementPrimitive<Connection.Row>(
    segments,
    acquirer,
    compiler,
    spanAttributes,
...
function convertLiteralOrFragment(
  clause: string | Statement.Fragment
): Array<Statement.Segment> {
...
export function join(literal: string, addParens = true, fallback = "") {
  ...
  return (
    clauses: ReadonlyArray<string | Statement.Fragment>
  ): Statement.Fragment => {
    if (clauses.length === 0) {
...
const statementCacheNoTransformSymbol = Symbol.for(
  "@effect/sql/Statement/statementCacheNoTransform"
)
...
class CompilerImpl implements Statement.Compiler {
  ...
  compile(
    ...
    const placeholder =
      placeholderOverride ??
      ((u: unknown) => this.parameterPlaceholder(++placeholderCount, u))
    const placeholderNoIncrement = (u: unknown) =>
      this.parameterPlaceholder(placeholderCount, u)
...
export const makeCompiler = <
  C extends Statement.Custom<any, any, any, any> = any
>(options: {
  readonly dialect: Statement.Dialect
