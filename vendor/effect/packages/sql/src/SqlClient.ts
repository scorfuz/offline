...
) => Effect<SqlClient, never, Reactivity> = internal.make
...
export const makeWithTransaction: <I, S>(options: {
  ...
  readonly spanAttributes: ReadonlyArray<readonly [string, unknown]>
  readonly acquireConnection: Effect<
    readonly [CloseableScope | undefined, S],
    SqlError
  >
...
export const TransactionConnection: Tag<
  ...
  readonly [conn: Connection, depth: number]
> = internal.TransactionConnection

/**
 * @since 1.0.0
 */
export class SafeIntegers extends Context.Reference<SafeIntegers>()(
