 ...
 * @category date & time
 */
export const DateWithNow = VariantSchema.Overrideable(
  Date,
  Schema.DateTimeUtcFromSelf,
  {
...
> =>
  Effect.gen(function* () {
    ...
    const insertResolver = yield* SqlResolver.ordered(
...
select * from ${sql(options.tableName)} where ${sql(idColumn)} = LAST_INSERT_ID();`.unprepared.pipe(
    ...
    const insertVoidResolver = yield* SqlResolver.void(
    ...
    const deleteResolver = yield* SqlResolver.void(
