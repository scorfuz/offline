 ...
 * @category requests
 */
export interface SqlRequest<T extends string, A, E> extends Request.Request<
  A,
  E | ParseError
> {
  readonly _tag: T
...
const SqlRequestProto = {
  ...
  [Hash.symbol](this: SqlRequest<string, unknown, unknown>): number {
    return Hash.cached(
      this,
      Hash.combine(Hash.hash(this.input))(Hash.string(this._tag))
    )
  }
}
...
const makeRequest = <T extends string, I, II, A, E>(
  ...
  const self = Object.create(SqlRequestProto) as Types.Mutable<
    SqlRequest<T, A, E>
  >
  self._tag = tag
  self.spanLink = {
...
const partitionRequests = <T extends string, A, E>(
  requests: ReadonlyArray<SqlRequest<T, A, E>>
) => {
...
const partitionRequestsById =
  <I, II>() =>
  <T extends string, A, E>(requests: ReadonlyArray<SqlRequest<T, A, E>>) => {
    const len = requests.length
...
> extends RequestResolver.RequestResolver<SqlRequest<T, A, E>> {
  ...
  readonly request: (
    input: I
  ) => Effect.Effect<SqlRequest<T, A, E>, ParseError, R>
}
...
const makeResolver = <T extends string, A, E, I, II, RI, R>(
        ...
        return Effect.useSpan(
          ...
          (span) =>
            Effect.withFiberRuntime<A, E | ParseError, RI>((fiber) => {
              ...
              return Effect.flatMap(encode(input), (encoded) =>
                Effect.request(
                  makeRequest<T, I, II, A, E>(tag, input, encoded, span),
                  resolver
                )
              )
            })
    ...
    return Object.assign(self, {
      request(input: I) {
        return Effect.withFiberRuntime<SqlRequest<T, A, E>, ParseError, RI>(
          (fiber) => {
            const span = fiber.currentContext.unsafeMap.get(
              Tracer.ParentSpan.key
            )
            return Effect.map(encode(input), (encoded) =>
              makeRequest(tag, input, encoded, span)
            )
          }
        )
      },
      cachePopulate(input: I, value: A) {
        return Effect.cacheRequestResult(
          makeRequest(tag, input, null as any, null as any),
          Exit.succeed(value)
        )
      },
  ...
  return withContext === true
    ? Effect.map(Effect.context<R>(), make)
...
>(
  ...
  const resolver = RequestResolver.makeBatched(
    (requests: NonEmptyArray<SqlRequest<T, Option.Option<A>, E>>) => {
      const [inputs, spanLinks, idMap] = partitionRequestsById<I, II>()(
        requests
      )
      return options.execute(inputs as any).pipe(
        Effect.bindTo("rawResults"),
