    ...
    readonly openai?:
      | {
          ...
          readonly imageDetail?:
            | typeof Generated.ImageDetail.Encoded
            | undefined
        }
      | undefined
  }
...
export const make = Effect.fnUntraced(function* (options: {
  ...
  ) => Effect.Effect<typeof Generated.CreateResponse.Encoded, AiError.AiError> =
          ...
          switch (part.type) {
            ...
            case "reasoning": {
              ...
              if (Predicate.isNotUndefined(options?.itemId)) {
                ...
                const summaryParts: Mutable<
                  typeof Generated.ReasoningItem.fields.summary.Encoded
                > = []
...
type OpenAiToolChoice =
  typeof Generated.CreateResponse.fields.tool_choice.from.Encoded
...
> = Effect.fnUntraced(function* (options) {
  ...
  if (typeof options.toolChoice === "object" && "tool" in options.toolChoice) {
    toolChoice = Predicate.isUndefined(
      OpenAiTool.getProviderDefinedToolName(options.toolChoice.tool)
    )
...
const isFileId = (data: string, config: Config.Service): boolean =>
  Predicate.isNotUndefined(config.fileIdPrefixes) &&
  config.fileIdPrefixes.some((prefix) => data.startsWith(prefix))

const getItemId = (
  part: Prompt.TextPart | Prompt.ToolCallPart
...
const prepareInclude = (
  ...
  const codeInterpreterTool = options.tools.find(
    (tool) =>
      Tool.isProviderDefined(tool) && tool.id === "openai.code_interpreter"
  ) as Tool.AnyProviderDefined | undefined
