      ...
      HttpClient.mapRequest((request) =>
        request.pipe(
          HttpClientRequest.prependUrl(
            options.apiUrl ?? `https://bedrock-runtime.${region}.amazonaws.com`
          ),
        ...
        Effect.fnUntraced(function* (request) {
          ...
          const signer = new AwsV4Signer({
            ...
            ...(options.sessionToken
              ? { sessionToken: Redacted.value(options.sessionToken) }
              : {})
          })
          const { headers: signedHeaders } = yield* Effect.promise(() =>
            signer.sign()
          )
          const headers = Headers.merge(
            originalHeaders,
            Headers.fromInput(signedHeaders)
          )
...
export const layer = (options: {
  ...
  Layer.scoped(AmazonBedrockClient, make(options))
