...
export const ProviderOptionsKey =
  "@effect/ai-google/GoogleLanguageModel/ProviderOptions"

/** @internal */
export const ProviderMetadataKey =
  "@effect/ai-google/GoogleLanguageModel/ProviderMetadata"

const finishReasonMap: Record<
  typeof CandidateFinishReason.Type,
...
/** @internal */
export const jsonSchemaToOpenApiSchema = (
  schema: JsonSchema7
  ...
  if ("type" in schema && schema.type === "string") {
    ...
    if (stringSchema.minLength !== undefined)
      result.minLength = stringSchema.minLength.toString()
    if (stringSchema.maxLength !== undefined)
      result.maxLength = stringSchema.maxLength.toString()
  ...
  if ("type" in schema && schema.type === "integer") {
    ...
    if (integerSchema.minimum !== undefined)
      result.minimum = integerSchema.minimum
    if (integerSchema.maximum !== undefined)
      result.maximum = integerSchema.maximum
  ...
  if ("type" in schema && schema.type === "array") {
    ...
    if (arraySchema.minItems !== undefined)
      result.minItems = arraySchema.minItems.toString()
    if (arraySchema.maxItems !== undefined)
      result.maxItems = arraySchema.maxItems.toString()
...
const extractAnnotations = (
  schema: JsonSchemaAnnotations
): Partial<typeof Schema.Encoded> => {
