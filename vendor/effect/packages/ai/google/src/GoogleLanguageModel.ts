...
export class Config extends Context.Tag(
  "@effect/ai-google/GoogleLanguageModel/Config"
  ...
  export interface FinishPartMetadata extends ProviderMetadata {
    readonly google?:
      | {
          ...
          readonly safetyRatings?:
            | ReadonlyArray<Generated.SafetyRating>
            | undefined
    ...
    const responseSchema =
      responseFormat.type === "json"
        ? InternalUtilities.jsonSchemaToOpenApiSchema(
            Tool.getJsonSchemaFromSchemaAst(responseFormat.schema.ast)
          )
        : undefined
...
> = Effect.fnUntraced(function* (options, config) {
            ...
            case "file": {
              ...
              if (part.data instanceof URL) {
                return yield* new AiError.MalformedInput({
                  ...
                  method: "prepareMessages",
                  description:
                    "File data URLs are unsupported in assistant messages"
                })
              }
              parts.push({
                inlineData: {
  ...
  if (
    ...
    ;(messages[0].parts as Array<typeof Generated.Part.Encoded>).unshift({
      text: `${systemText}\n\n`
    })
  ...
  return {
    system: !isGemmaModel && system.length > 0 ? { parts: system } : undefined,
    messages
  }
})
...
const makeResponse: (
  response: Generated.GenerateContentResponse
) => Effect.Effect<
...
> = Effect.fnUntraced(function* (response) {
  ...
  for (const part of contentParts) {
    if (
      "text" in part &&
      Predicate.isNotUndefined(part.text) &&
      part.text.length > 0
    ) {
    ...
    if (
      ...
      Predicate.isNotUndefined(part.executableCode)
    ) {
    ...
    if (
      ...
      Predicate.isNotUndefined(part.codeExecutionResult)
    ) {
      if (Predicate.isUndefined(lastCodeExecutionToolCallId)) {
        return yield* new AiError.MalformedOutput({
          ...
          method: "makeResponse",
          description:
            "Received code_execution tool call result without " +
...
> = Effect.fnUntraced(function* (options, config) {
  ...
  const userDefinedTools = options.tools.filter((tool) =>
    Tool.isUserDefined(tool)
  )
  const providerDefinedTools = options.tools.filter((tool) =>
  ...
  if (userDefinedTools.length > 0 && providerDefinedTools.length > 0) {
    return yield* new AiError.MalformedInput({
      ...
      method: "prepareTools",
      description:
        "Unsupported Functionality: cannot mix provider-defined tools " +
  ...
  if (providerDefinedTools.length > 0) {
    ...
    const googleTools: Mutable<
      Omit<typeof Generated.Tool.Encoded, "functionDeclarations">
    > = {}
      ...
      switch (tool.id) {
        ...
        case "google.google_search_retrieval": {
          googleTools.googleSearchRetrieval = {
            dynamicRetrievalConfig: tool.args
          }
          break
        }
  ...
  for (const tool of userDefinedTools) {
    tools.push({
      ...
      parameters: InternalUtilities.jsonSchemaToOpenApiSchema(
        Tool.getJsonSchema(tool as any)
      )
    })
  }
...
const extractSources = Effect.fnUntraced(function* (
...
const getToolCalls = Effect.fnUntraced(function* (
  contentParts: ReadonlyArray<Generated.Part>
) {
  const idGenerator = yield* IdGenerator.IdGenerator
