...
>() {
  ...
  static readonly getOrUndefined: Effect.Effect<
    typeof GoogleConfig.Service | undefined
  > = Effect.map(Effect.context<never>(), (context) =>
    context.unsafeMap.get(GoogleConfig.key)
  )
}
...
>(2, (self, transformClient) =>
  Effect.flatMap(GoogleConfig.getOrUndefined, (config) =>
    Effect.provideService(self, GoogleConfig, { ...config, transformClient })
  )
)
