...
describe("Chat", () => {
  it.scoped(
    "should persist chat history to the backing persistence store",
    () =>
      Effect.gen(function* () {
        const storeId = "chat"
        const chatId = "1"
        ...
        const storeHistory = yield* store
          .get(chatId)
          .pipe(
            Effect.flatten,
  ...
  it.scoped(
    "should prefer the message identifier of the most recent assistant message",
    () =>
      Effect.gen(function* () {
        const storeId = "chat"
        const chatId = "2"
  ...
  it.scoped(
    "should raise an error when retrieving a chat that does not exist",
    () =>
      Effect.gen(function* () {
        const persistence = yield* Chat.Persistence
