...
>() {
  ...
  static readonly layer: Layer.Layer<McpServer | McpServerClient> =
    Layer.scoped(McpServer, McpServer.make) as any
}
...
> => {
  ...
  return Effect.gen(function* () {
    const registry = yield* McpServer
    const context =
      yield* Effect.context<Exclude<R | ParamsR, McpServerClient>>()
    const completions: Record<
      string,
      (
        input: string
  ...
  ClientRpcs.toLayer(
    Effect.gen(function* () {
      ...
      return {
        ...
        "prompts/list": () =>
          Effect.sync(() => new ListPromptsResult({ prompts: server.prompts })),
        "resources/list": () =>
          Effect.sync(
        ...
        "tools/list": () =>
          Effect.sync(() => new ListToolsResult({ tools: server.tools })),
