 ...
 * @category Context
 */
export class Chat extends Context.Tag("@effect/ai/Chat")<Chat, Service>() {}
...
export const empty: Effect.Effect<Service> = Effect.gen(function* () {
  ...
  const provideContextStream = <A, E, R>(
      ...
      return Stream.fromChannel(
        Channel.acquireUseRelease(
          semaphore.take(1).pipe(
            ...
            Effect.map((history) =>
              Prompt.merge(history, Prompt.make(options.prompt))
            )
          ),
          (prompt) =>
...
export const fromExport = (
  data: unknown
): Effect.Effect<Service, ParseError, LanguageModel.LanguageModel> =>
...
export const makePersisted = Effect.fnUntraced(function* (options: {
    ...
    const saveChat = Effect.fnUntraced(
      function* (prevHistory: Prompt.Prompt) {
        ...
        if (
          ...
          messageId = lastMessage.options[Persistence.key]?.messageId as
            | string
            | undefined
        }
  ...
  const makeTTL = (
    ...
    Option.fromNullable(timeToLive).pipe(Option.map(Duration.decode))

  const get = Effect.fn("PersistedChat.get")(function* (
    chatId: string,
    options?: {
      readonly timeToLive?: Duration.DurationInput | undefined
    }
  ...
  const getOrCreate = Effect.fn("PersistedChat.getOrCreate")(function* (
    chatId: string,
    options?: {
      readonly timeToLive?: Duration.DurationInput | undefined
    }
...
export const layerPersisted = (options: {
  ...
  Layer.scoped(Persistence, makePersisted(options))
