...
export interface GenerateTextOptions<Tools extends Record<string, Tool.Any>> {
  ...
  readonly toolkit?:
    ...
    | Effect.Effect<Toolkit.WithHandler<Tools>, any, any>
    | undefined
    ...
    >(
      ...
      if (Predicate.isUndefined(options.toolkit)) {
        ...
        return params
          ...
          .pipe(Stream.mapChunksEffect(decode)) as Stream.Stream<
          Response.StreamPart<Tools>,
          AiError.AiError | ParseResult.ParseError,
          IdGenerator
        >
      ...
      // If there is a toolkit resolve and apply it to the provider options
      const toolkit = Effect.isEffect(options.toolkit)
...
const resolveToolkit = <Tools extends Record<string, Tool.Any>, E, R>(
  ...
  Effect.isEffect(toolkit) ? toolkit : Effect.succeed(toolkit)
...
const resolveStructuredOutput = Effect.fnUntraced(function* <A, I, R>(
