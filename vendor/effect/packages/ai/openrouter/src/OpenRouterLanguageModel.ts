        ...
        responseFormat.type === "text"
          ...
          : {
              ...
              json_schema: {
                ...
                schema: Tool.getJsonSchemaFromSchemaAst(
    ...
    switch (message.role) {
      ...
      case "user": {
        if (
          ...
          const content: Array<
            typeof Generated.ChatMessageContentItem.Encoded
          > = []
              ...
              case "file": {
                if (part.mediaType.startsWith("image/")) {
                  ...
                  content.push({
                    ...
                    cache_control:
                      part.data instanceof URL ? cacheControl : undefined
                  })
                }
                break
              }
      ...
      case "assistant": {
        ...
        const reasoningDetails: Array<
          typeof Generated.ReasoningDetail.Encoded
        > = []
...
const prepareTools: (options: LanguageModel.ProviderOptions) => Effect.Effect<
  ...
  AiError.AiError
> = Effect.fnUntraced(function* (options: LanguageModel.ProviderOptions) {
...
const makeResponse: (
  response: Generated.ChatResponse
  ...
  Effect.fnUntraced(function* (response) {
    ...
    if (
      Predicate.isNotNullable(message.reasoning) &&
    ...
    if (
      Predicate.isNotNullable(message.reasoning_details) &&
      ...
      for (const detail of message.reasoning_details) {
        switch (detail.type) {
          case "reasoning.summary": {
            if (
              Predicate.isNotUndefined(detail.summary) &&
          ...
          case "reasoning.encrypted": {
            if (
              Predicate.isNotUndefined(detail.data) &&
          ...
          case "reasoning.text": {
            if (
              Predicate.isNotUndefined(detail.text) &&
    ...
    if (
      Predicate.isNotNullable(message.content) &&
...
const makeStreamResponse: (
  stream: Stream.Stream<ChatStreamingResponseChunk, AiError.AiError>
) => Effect.Effect<Stream.Stream<Response.StreamPartEncoded, AiError.AiError>> =
    ...
    return stream.pipe(
      Stream.mapEffect(
        Effect.fnUntraced(function* (event) {
          ...
          if (
            Predicate.isUndefined(choice) &&
            Predicate.isUndefined(event.usage)
          ) {
            return yield* new AiError.MalformedOutput({
              module: "OpenRouterLanguageModel",
              method: "makeResponse",
          ...
          const emitReasoningPart = (
            delta: string,
            metadata: OpenRouterReasoningInfo | undefined = undefined
          ) => {
          ...
          if (
            Predicate.isNotNullable(delta?.reasoning_details) &&
            ...
            Predicate.isNotNullable(delta?.reasoning) &&
          ...
          if (
            Predicate.isNotNullable(delta?.content) &&
          ...
          if (
            Predicate.isNotNullable(delta?.tool_calls) &&
...
const getCacheControl = (
  ...
  part.options.openrouter?.cacheControl
