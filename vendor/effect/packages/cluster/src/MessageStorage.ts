 ...
 * @category context
 */
export class MessageStorage extends Context.Tag(
...
export const makeEncoded: (
  encoded: Encoded
) => Effect.Effect<MessageStorage["Type"], never, Snowflake.Generator> =
  Effect.fnUntraced(function* (encoded: Encoded) {
    ...
    const storage: MessageStorage["Type"] = yield* make({
      saveRequest: (message) =>
        Message.serializeEnvelope(message).pipe(
          ...
          Effect.flatMap((result) => {
            if (
              result._tag === "Success" ||
              result.lastReceivedReply._tag === "None"
            ) {
      ...
      unprocessedMessages: (shardIds) => {
        ...
        return Effect.flatMap(
          Effect.suspend(() =>
            encoded.unprocessedMessages(shards, clock.unsafeCurrentTimeMillis())
          ),
          decodeMessages
        )
      },
    ...
    const decodeReplies = (
      ...
      const decodeReply: Effect.Effect<void | Reply.Reply<any>> =
        Effect.catchAll(
          Effect.suspend(() => {
...
export class MemoryDriver extends Effect.Service<MemoryDriver>()(
  "@effect/cluster/MessageStorage/MemoryDriver",
  {
    ...
    effect: Effect.gen(function* () {
      ...
      const unprocessedWith = (
        predicate: Predicate<Envelope.Envelope.Encoded>
      ) => {
        ...
        for (const envelope of unprocessed) {
          ...
          if (envelope._tag === "Request") {
            ...
            messages.push({
              envelope,
              lastSentReply: Option.fromNullable(
      ...
      const encoded: Encoded = {
        ...
        saveReply: (reply_) =>
          Effect.sync(() => {
            const reply = JSON.parse(
              JSON.stringify(reply_)
            ) as Reply.ReplyEncoded<any>
        ...
        requestIdForPrimaryKey: (primaryKey) =>
          Effect.sync(() => {
            ...
            return Option.fromNullable(entry?.envelope.requestId).pipe(
...
).pipe(Layer.provideMerge(MemoryDriver.Default))
