 ...
 * @category context
 */
export class Runners extends Context.Tag("@effect/cluster/Runners")<
  Runners,
  {
    /**
  ...
  function notifyWith<E>(
    ...
    if (!persisted) {
      return Effect.dieMessage(
        "Runners.notify only supports persisted messages"
      )
    }
    ...
    if (message._tag === "OutgoingEnvelope") {
      ...
      return storage
        ...
        .pipe(
          ...
          Effect.zipRight(
            entry
              ? Effect.zipRight(entry.latch.open, afterPersist(message, false))
...
> = Layer.scoped(Runners, makeNoop).pipe(
...
> = Layer.scoped(Runners, makeRpc).pipe(Layer.provide(Snowflake.layerGenerator))

/**
 * @since 1.0.0
 * @category Client
 */
export class RpcClientProtocol extends Context.Tag(
  "@effect/cluster/Runners/RpcClientProtocol"
)<
  RpcClientProtocol,
  (
    address: RunnerAddress
  ) => Effect.Effect<RpcClient_.Protocol["Type"], never, Scope>
>() {}
