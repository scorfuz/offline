...
>(
  ...
  options: {
    ...
    readonly defectRetryPolicy?:
      | Schedule.Schedule<any, unknown, never>
      | undefined
  ...
  const context = yield* Effect.context<
    Rpc.Context<Rpcs> | Rpc.Middleware<Rpcs> | RX
  >()
        ...
        const writeRef = yield* ResourceRef.from(
          ...
          Effect.fnUntraced(function* (scope) {
            ...
            const handlers = yield* entity.protocol
              .toHandlersContext(buildHandlers)
              .pipe(
                Effect.provide(
                  context.pipe(
                  ...
                  switch (response._tag) {
                    case "Exit": {
                      ...
                      if (
                        ...
                        (isShuttingDown ||
                          Uninterruptible.forServer(request.rpc.annotations))
                      ) {
                        if (!isShuttingDown) {
                          return server
                            .write(0, {
  ...
  const maxIdleTime = Duration.toMillis(
    options.maxIdleTime ?? config.entityMaxIdleTime
  )
  ...
  const runFork = Runtime.runFork(
    yield* Effect.runtime<never>().pipe(Effect.interruptible)
  )
...
const makeMessageDecode = <Type extends string, Rpcs extends Rpc.Any>(
  ...
  const decodeRequest = (
    ...
    const lastSentReply = Option.isSome(message.lastSentReply)
      ? Effect.asSome(
          Schema.decode(Reply.Reply(rpc as any))(message.lastSentReply.value)
        )
      : Effect.succeedNone
