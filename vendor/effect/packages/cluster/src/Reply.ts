 ...
 * @category models
 */
export type ReplyEncoded<R extends Rpc.Any> =
  | WithExitEncoded<R>
  | ChunkEncoded<R>
...
const schemaCache = new WeakMap<
  Rpc.Any,
  Schema.Schema<Reply<Rpc.Any>, ReplyEncoded<Rpc.Any>, Rpc.Context<Rpc.Any>>
>()
...
export const Reply = <R extends Rpc.Any>(
  rpc: R
): Schema.Schema<Reply<R>, ReplyEncoded<R>, Rpc.Context<R>> => {
...
export class Chunk<R extends Rpc.Any> extends Data.TaggedClass("Chunk")<{
  ...
  static schema<R extends Rpc.Any>(
    rpc: R
  ): Schema.Schema<Chunk<R>, ChunkEncoded<R>, Rpc.Context<R>> {
    const successSchema = (
...
export class WithExit<R extends Rpc.Any> extends Data.TaggedClass("WithExit")<{
  ...
  static schema<R extends Rpc.Any>(
    rpc: R
  ): Schema.Schema<WithExit<R>, WithExitEncoded<R>, Rpc.Context<R>> {
    return Schema.transform(
      Schema.Struct({
        _tag: Schema.Literal("WithExit"),
        requestId: SnowflakeFromString,
      ...
      Schema.declare(
        (u): u is WithExit<R> => isReply(u) && u._tag === "WithExit"
      ),
      {
...
export const serialize = <R extends Rpc.Any>(
  ...
  return MalformedMessage.refail(
    Effect.locally(
      Schema.encode(schema)(self.reply),
...
export const serializeLastReceived = <R extends Rpc.Any>(
  ...
  return Effect.asSome(
    MalformedMessage.refail(
      Effect.locally(
        Schema.encode(schema)(self.lastReceivedReply.value),
        FiberRef.currentContext,
        self.context
      )
    )
