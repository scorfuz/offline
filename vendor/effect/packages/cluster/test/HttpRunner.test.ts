    ...
    it.scoped("path '/' produces http://host:port/", () =>
      Effect.gen(function* () {
        ...
        const layer = HttpRunner.layerClientProtocolHttp({ path: "/" }).pipe(
          Layer.provide(
            Layer.succeed(HttpClient.HttpClient, makeUrlCapturingClient(urlRef))
          ),
          Layer.provide(RpcSerialization.layerNdjson)
        )

        const makeProtocol = yield* Effect.provide(
          Runners.RpcClientProtocol,
          layer
        )
        const protocol = yield* makeProtocol(
          RunnerAddress.make("localhost", 3000)
        )
