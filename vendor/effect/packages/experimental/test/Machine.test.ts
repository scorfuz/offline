...
class Decrement extends Schema.TaggedRequest<Decrement>()("Decrement", {
  failure: Schema.Never,
  success: Schema.Number,
  payload: {}
}) {}
...
class DelayedIncrementBy extends Schema.TaggedRequest<DelayedIncrementBy>()(
  "DelayedIncrementBy",
  {
    failure: Schema.Never,
    success: Schema.Void,
...
class Multiply extends Schema.TaggedRequest<Multiply>()("Multiply", {
  failure: Schema.Never,
  success: Schema.Number,
  payload: {}
}) {}

class FailBackground extends Schema.TaggedRequest<FailBackground>()(
  "FailBackground",
  {
    failure: Schema.Never,
    success: Schema.Void,
...
const counter = Machine.makeWith<number, number>()((input, previous) =>
  Machine.procedures
    .make(previous ?? input, {
    ...
    .pipe(
      ...
      Machine.procedures.add<IncrementBy>()(
        "IncrementBy",
        ({ request, state }) =>
          Effect.sync(() => {
  ...
  (input, previous) =>
    Machine.serializable
      ...
      .pipe(
        ...
        Machine.serializable.add(FailBackground, ({ forkWith, state }) =>
          forkWith(Effect.fail("error"), state)
        )
      )
)

const delayedCounter = Machine.makeWith<number, number>()((input, previous) =>
  Machine.procedures
    .make(previous ?? input, {
      identifier: `Counter(${input})`
    })
    .pipe(
      Machine.procedures.addPrivate<IncrementBy>()(
        "IncrementBy",
        ({ request, state }) =>
          Effect.sync(() => {
            const count = state + request.number
...
const withContext = Machine.make((input: number, previous?: number) =>
  Effect.gen(function* () {
...
const timerLoop = Machine.make(
  Effect.gen(function* () {
    ...
    return Machine.procedures
      .make(0)
      .pipe(
        Machine.procedures.addPrivate<Increment>()("Increment", (ctx) =>
          ctx
            .send(new Increment())
            .pipe(
              Effect.delay(20),
...
const deferReply = Machine.make(
  Machine.procedures.make(0).pipe(
    Machine.procedures.add<Increment>()("Increment", (ctx) => {
...
describe("Machine", () => {
  test("counter", () =>
    Effect.gen(function* () {
      ...
      Effect.scoped,
      Machine.withTracingEnabled(true),
  ...
  test("init context", () =>
    ...
    }).pipe(Effect.scoped, Effect.provide(Multiplier.Live), Effect.runPromise))
  ...
  test("forkWithState", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(
        yield* booted.send(new DelayedIncrementBy({ number: 2, delay: 10 })),
        undefined
      )
      assert.strictEqual(yield* booted.get, 2)
      yield* Effect.sleep(10)
