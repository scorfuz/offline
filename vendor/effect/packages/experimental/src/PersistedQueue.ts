...
export interface PersistedQueue<in out A, out R = never> {
  ...
  readonly take: <XA, XE, XR>(
    ...
    XA,
    XE | PersistedQueueError | ParseResult.ParseError,
    R | XR
  >
}
 ...
 * @category Factory
 */
export class PersistedQueueFactory extends Context.Tag(
...
export const make = <A, I, R>(options: {
  ...
  Effect.flatMap(PersistedQueueFactory, (factory) => factory.make(options))
            ...
            Effect.fnUntraced(function* (restore) {
              ...
              const exit = yield* decodeUnknown(item.element).pipe(
                Effect.flatMap((value) =>
                  f(value, { id: item.id, attempts: item.attempts })
                ),
                restore,
                Effect.exit
              )
 ...
 * @category Store
 */
export class PersistedQueueStore extends Context.Tag(
