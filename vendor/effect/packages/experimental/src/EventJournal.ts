 ...
 * @category tags
 */
export class EventJournal extends Context.Tag(
...
export const ErrorTypeId: unique symbol = Symbol.for(
  "@effect/experimental/EventJournal/ErrorId"
)
 ...
 * @category errors
 */
export class EventJournalError extends Schema.TaggedClass<EventJournalError>(
  "@effect/experimental/EventJournal/Error"
)("EventJournalError", {
 ...
 * @category remote
 */
export const RemoteIdTypeId: unique symbol = Symbol.for(
  "@effect/experimental/EventJournal/RemoteId"
)
 ...
 * @category remote
 */
export const RemoteId = Schema.Uint8ArrayFromSelf.pipe(
  Schema.brand(RemoteIdTypeId)
)
...
export const makeRemoteId = (): RemoteId =>
  Uuid.v4({}, new Uint8Array(16)) as RemoteId

/**
 * @since 1.0.0
 * @category entry
 */
export const EntryIdTypeId: unique symbol = Symbol.for(
  "@effect/experimental/EventJournal/EntryId"
)
 ...
 * @category entry
 */
export const EntryId = Schema.Uint8ArrayFromSelf.pipe(
...
export class RemoteEntry extends Schema.Class<RemoteEntry>(
  "@effect/experimental/EventJournal/RemoteEntry"
)({
          ...
          for (const [compacted, remoteEntries] of brackets) {
            for (const originEntry of compacted) {
              ...
              for (let i = journal.length - 1; i >= -1; i--) {
                ...
                if (
                  entry !== undefined &&
                  entry.createdAtMillis > entryMillis
                ) {
                  continue
                }
                for (let j = i + 2; j < journal.length; j++) {
                  ...
                  if (
                    entry.event === originEntry.event &&
                    entry.primaryKey === originEntry.primaryKey
                  ) {
...
export const layerIndexedDb = (options?: {
  ...
  Layer.scoped(EventJournal, makeIndexedDb(options))
