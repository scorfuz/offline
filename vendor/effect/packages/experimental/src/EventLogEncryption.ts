 ...
 * @category encrytion
 */
export class EventLogEncryption extends Context.Tag(
...
export const makeEncryptionSubtle = (
  crypto: Crypto
): Effect.Effect<typeof EventLogEncryption.Service> =>
  Effect.sync(() => {
    ...
    return EventLogEncryption.of({
      ...
      decrypt: (identity, entries) =>
        Effect.gen(function* () {
          ...
          return decoded.map(
            (entry, i) =>
              new RemoteEntry({ remoteSequence: entries[i].sequence, entry })
          )
        }),
      sha256: (data) =>
