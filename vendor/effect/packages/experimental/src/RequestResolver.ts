...
interface DataLoaderItem<A extends Request.Request<any, any>> {
  readonly request: A
  readonly resume: (
    effect: Effect.Effect<Request.Request.Success<A>, Request.Request.Error<A>>
  ) => void
}
...
>(
  2,
  Effect.fnUntraced(function* <A extends Request.Request<any, any>>(
    self: RequestResolver.RequestResolver<A, never>,
    options: {
      readonly window: Duration.DurationInput
      readonly maxBatchSize?: number
    ...
    const runtime = yield* Effect.runtime<never>().pipe(Effect.interruptible)
    ...
    yield* Scope.addFinalizer(
      scope,
      Effect.suspend(() => (fiber ? Fiber.interrupt(fiber) : Effect.void))
    )

    return RequestResolver.fromEffect((request: A) =>
      Effect.async<Request.Request.Success<A>, Request.Request.Error<A>>(
        (resume) => {
...
export declare namespace PersistedRequest {
   ...
   */
  export type Any =
    | PersistedRequest<any, any, any, any, any>
    | PersistedRequest<any, never, never, any, any>
}
...
export const persisted: {
  <Req extends PersistedRequest.Any>(options: {
    ...
    readonly timeToLive: (
      ...args: Persistence.ResultPersistence.TimeToLiveArgs<Req>
  ...
  > =>
    Effect.gen(function* () {
      ...
      const set = (
        ...
        Effect.ignoreLogged(storage.set(request as any, result))
      ...
      return RequestResolver.makeBatched((requests: Arr.NonEmptyArray<Req>) =>
        Effect.flatMap(partition(requests), ([remaining, results]) => {
          const completeCached = Effect.forEach(
            results,
            ([request, result]) =>
              Request.complete(request, result as any) as Effect.Effect<void>,
            { discard: true }
          )
