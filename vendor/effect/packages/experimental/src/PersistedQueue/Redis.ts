...
interface RedisWithQueue extends Redis {
  ...
  complete(keyPending: string, keyLock: string, id: string): Promise<void>
  failed(
    keyPending: string,
    keyLock: string,
    keyFailed: string,
...
`,
  ...
  const lockRefreshMillis = options.lockRefreshInterval
    ? Duration.toMillis(options.lockRefreshInterval)
    : 30_000
  const lockExpirationMillis = options.lockExpiration
    ? Duration.toMillis(options.lockExpiration)
    : 90_000
      ...
      Effect.uninterruptibleMask((restore) =>
        RcMap.get(mailboxes, options.name).pipe(
          Effect.flatMap(({ mailbox, pollLatch, takenLatch, takers }) => {
            ...
            return Effect.tap(
              restore(mailbox.take as Effect.Effect<Element>),
              () => {
                takers.current--
                if (takers.current === 0) {
            ...
            return Effect.addFinalizer(
              Exit.match({
                ...
                onSuccess: () => {
                  activeLockKeys.delete(lock)
                  return Effect.promise(() =>
                    redis.complete(keyPending(options.name), lock, element.id)
                  )
                }
              })
            )
