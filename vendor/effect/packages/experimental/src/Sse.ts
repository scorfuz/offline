...
export const makeChannel = <IE, Done>(options?: {
  ...
  Done
> => {
  const events = Mailbox.make<Event, IE | Retry>(
  ...
  > = Channel.catchAll(events, (error) =>
    Retry.is(error)
      ? Effect.sleep(error.duration).pipe(Effect.as(withRetry), Channel.unwrap)
      : Channel.fail(error)
  )
 ...
 * @category type ids
 */
export const RetryTypeId: unique symbol = Symbol.for(
  "@effect/experimental/Sse/Retry"
)
