...
> extends Effect.Effect<SerializableProcedureList<State, Public, Private, R>> {
  ...
  readonly public: ReadonlyArray<
    Procedure.SerializableProcedure<Public, State, R>
  >
  readonly private: ReadonlyArray<
    Procedure.SerializableProcedure<Private, State, R>
  >
...
export const add: {
  ...
  >(
    ...
    handler: Procedure.Handler<
      Req,
      Types.NoInfer<State>,
      Types.NoInfer<Public> | Types.NoInfer<Private>,
      R2
    >
  ): <R>(
  ...
  > =>
    ProcedureList.addProcedure(
      self,
      Procedure.makeSerializable<any, any>()(schema, handler)
    ) as any
)
