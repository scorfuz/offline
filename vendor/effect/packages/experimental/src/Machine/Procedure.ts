...
export const SerializableTypeId: unique symbol = Symbol.for(
  "@effect/experimental/Machine/SerializableProcedure"
)
...
export const isSerializable = (
  u: unknown
): u is SerializableProcedure<any, any, any> =>
 ...
 * @category models
 */
export interface SerializableProcedure<
  Request extends Schema.TaggedRequest.All,
  State,
  R
 ...
 * @category symbols
 */
export const NoReply = Symbol.for(
  "@effect/experimental/Machine/Procedure/NoReply"
)
...
export declare namespace Procedure {
  ...
  export interface BaseContext {
    ...
    readonly unsafeSend: <Req extends TaggedRequest.Any>(
      request: Req
    ) => Effect.Effect<void>
  ...
  > extends BaseContext {
    ...
    readonly sendAwait: <Req extends Requests>(
      request: Req
    ) => Effect.Effect<Request.Success<Req>, Request.Error<Req>>
    readonly forkWith: {
      (
   ...
   * @category models
   */
  export type InferRequest<P> =
    P extends Procedure<infer Req, infer _, infer _> ? Req : never

  /**
   * @since 1.0.0
   * @category models
   */
  export type InferContext<P> =
    P extends Procedure<infer _, infer _, infer R> ? R : never
}
...
export const makeSerializable =
  <Requests extends TaggedRequest.Any, State>() =>
  <Req extends Schema.TaggedRequest.All, IS, R, RS>(
