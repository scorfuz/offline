 ...
 * @category schema
 */
export const SchemaTypeId: unique symbol = Symbol.for(
  "@effect/experimental/EventLog/EventLogSchema"
)
...
export const HandlersTypeId: unique symbol = Symbol.for(
  "@effect/experimental/EventLog/Handlers"
)
...
const makeHandlers = <Events extends Event.Any>(options: {
  ...
  readonly context: Context.Context<any>
}): Handlers<never, Events> =>
  Object.assign(Object.create(handlersProto), options)
...
> =>
  Effect.gen(function* () {
    ...
    const handlers = Effect.isEffect(result)
      ? yield* result as any as Effect.Effect<any>
      : result
    const registry = yield* Registry
    yield* registry.add(handlers)
  }).pipe(Layer.scopedDiscard, Layer.provide(Registry.layer)) as any
...
export const groupCompaction = <Events extends Event.Any, R>(
  ...
  never,
  never,
  Identity | EventJournal | R | Event.Context<Events>
> =>
  Effect.gen(function* () {
      ...
      effect: Effect.fnUntraced(function* ({ entries, write }) {
            ...
            const entry = new Entry(
              {
                ...
                event: tag,
                payload: yield* Schema.encode(event.payloadMsgPack)(
                  payload
                ).pipe(
        ...
        for (const entry of entries) {
          const payload = yield* Schema.decodeUnknown(
            (group.events[entry.event] as any).payloadMsgPack
          )(entry.payload).pipe(
            Effect.locally(FiberRef.currentContext, context)
...
export class Registry extends Context.Tag(
  ...
  Registry,
  {
    ...
    readonly handlers: Effect.Effect<
      Record.ReadonlyRecord<string, Handlers.Item<any>>
    >
  }
>() {
  /**
...
>() {
  ...
  static decodeString = (s: string): Identity["Type"] =>
    Schema.decodeSync(Identity.SchemaFromString)(s)
  /**
   * @since 1.0.0
   */
  static encodeString = (identity: Identity["Type"]): string =>
    Schema.encodeSync(Identity.SchemaFromString)(identity)
}
 ...
 * @category tags
 */
export class EventLog extends Context.Tag(
...
const make = Effect.gen(function* () {
  ...
  const runRemote = Effect.fnUntraced(
    function* (remote: EventLogRemote) {
              ...
              effect: Effect.fnUntraced(
                function* ({ conflicts, entry }) {
                  ...
                  const decodePayload = Schema.decode(
                    handlers[entry.event].event
                      .payloadMsgPack as unknown as Schema.Schema<any>
                  )
      ...
      yield* write
      return yield* Queue.takeBetween(
  ...
  const writeHandler = Effect.fnUntraced(
    function* (
      ...
      const payload = yield* Effect.orDie(
        Schema.encode(
          handlers[options.event].event
            .payloadMsgPack as unknown as Schema.Schema<Uint8Array>
        )(options.payload)
      )
    ...
    (effect, handler) =>
      Effect.mapInputContext(effect, (context) =>
        Context.merge(handler.context, context)
      )
  )
...
> = Layer.scoped(EventLog, make).pipe(
