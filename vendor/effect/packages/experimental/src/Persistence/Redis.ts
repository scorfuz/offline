      ...
      Effect.sync(() => {
        ...
        const parse = (method: string) => (str: string | null) => {
          ...
          return Effect.try({
            try: () => Option.some(JSON.parse(str)),
              ...
              Persistence.PersistenceBackingError.make(method, error)
          })
        }
        return identity<Persistence.BackingPersistenceStore>({
          ...
          clear: Effect.tryPromise({
            try: () =>
              redis.keys(`${prefix}:*`).then((keys) => redis.del(keys)),
            catch: (error) =>
...
export const layerResult = (
  ...
  Persistence.layerResult.pipe(Layer.provide(layer(options)))
...
export const layerResultConfig = (
  ...
  Persistence.layerResult.pipe(Layer.provide(layerConfig(options)))
