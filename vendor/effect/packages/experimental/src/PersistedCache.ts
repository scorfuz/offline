...
class CacheRequest<
  K extends Persistence.ResultPersistence.KeyAny
> extends Data.Class<{
  readonly key: K
...
>(options: {
  ...
  PersistedCache<K>,
  never,
  | Schema.SerializableWithResult.Context<K>
  | R
    ...
    Effect.bind("inMemoryCache", ({ store }) =>
      Cache.make({
        lookup: (request: CacheRequest<K>) => {
          ...
          return request.span._tag === "Some"
            ? Effect.withParentSpan(effect, request.span.value)
            : effect
        },
      ...
      identity<PersistedCache<K>>({
        get: (key) =>
          Effect.serviceOption(Tracer.ParentSpan).pipe(
            Effect.flatMap((span) =>
              inMemoryCache.get(new CacheRequest({ key, span }))
            )
          ),
        invalidate: (key) =>
          store
            ...
            .pipe(
              Effect.zipRight(
                inMemoryCache.invalidate(
                  new CacheRequest({ key, span: Option.none() })
                )
              )
            )
      })
