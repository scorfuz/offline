 ...
 * @category tags
 */
export const Client = Context.GenericTag<Client, ClientImpl>(
...
> = Effect.gen(function* () {
        ...
        metrics.push({
          ...
          state: {
            occurrences: Object.fromEntries(
              metricPair.metricState.occurrences.entries()
            )
          }
        })
      }
  ...
  yield* Effect.addFinalizer(() =>
    requests
      .offer(metricsSnapshot())
      .pipe(
        Effect.zipRight(
          Effect.fiberIdWith((id) => requests.failCause(Cause.interrupt(id)))
        )
      )
  )
  yield* requests
 ...
 * @category layers
 */
export const layer: Layer.Layer<Client, never, Socket.Socket> = Layer.scoped(
  Client,
  make
)
 ...
 * @category constructors
 */
export const makeTracer: Effect.Effect<Tracer.Tracer, never, Client> =
  Effect.gen(function* () {
