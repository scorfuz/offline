 ...
 * @category schemas
 */
export interface ExternalSpanFrom extends Schema.Schema.Encoded<
  typeof ExternalSpan
> {}
 ...
 * @category schemas
 */
export const metric = <Tag extends string, S, IS, R>(
  tag: Tag,
...
export const Summary = metric(
  ...
  Schema.Struct({
    ...
    quantiles: Schema.Array(
      Schema.Tuple(Schema.Number, Schema.Option(Schema.Number))
    ),
    count: Schema.Number,
    min: Schema.Number,
    max: Schema.Number,
