...
> extends Pipeable {
  ...
  readonly initialize: Machine.Initialize<
    Input,
    State,
    Public,
    Private,
  ...
  readonly retryPolicy:
    | Schedule.Schedule<unknown, InitErr | MachineDefect, R>
    | undefined
}
 ...
 * @category type ids
 */
export const SerializableTypeId: unique symbol = Symbol.for(
  "@effect/experimental/Machine/Serializable"
)
 ...
 * @category type ids
 */
export const ActorTypeId: unique symbol = Symbol.for(
  "@effect/experimental/Machine/Actor"
)
 ...
 * @category errors
 */
export class MachineDefect extends Schema.TaggedError<MachineDefect>()(
  "MachineDefect",
  {
    cause: Schema.Defect
  }
  ...
  ): Effect.Effect<A, MachineDefect, R> {
    return Effect.catchAllCause(Effect.orDie(effect), (cause) =>
      Effect.fail(new MachineDefect({ cause: Cause.squash(cause) }))
    )
  }
}
 ...
 * @category tags
 */
export class MachineContext extends Context.Tag(
  "@effect/experimental/Machine/Context"
)<MachineContext, Procedure.Procedure.BaseContext>() {}
 ...
 * @category models
 */
export interface Actor<M extends Machine.Any> extends Subscribable.Subscribable<
  Machine.State<M>
> {
...
export const make: {
  ...
  >(
    ...
    void,
    InitErr,
    Exclude<R, Scope.Scope | MachineContext>
  >
  <
    State,
    Public extends Procedure.TaggedRequest.Any,
...
> => ({
  [TypeId]: TypeId,
  initialize: Effect.isEffect(initialize)
    ? () => initialize
    : (initialize as any),
...
export const makeSerializable: {
  ...
  >(
    ...
    initialize:
      | Effect.Effect<
          ...
          R
        >
      | Machine.InitializeSerializable<
          void,
          State,
          Public,
          Private,
  ...
  >(
    ...
    initialize: Machine.InitializeSerializable<
      Input,
      State,
      Public,
      Private,
...
export const retry: {
  <
    ...
    Out,
    In extends Machine.InitError<M> | MachineDefect,
    R
  >(
...
} = dual(2, <A, E, R>(self: Effect.Effect<A, E, R>, enabled: boolean) =>
  Effect.locally(self, currentTracingEnabled, enabled)
)
 ...
 * @category runtime
 */
export const boot = <M extends Machine.Any>(
  self: M,
  ...
  M extends { readonly [SerializableTypeId]: SerializableTypeId }
    ? SerializableActor<M>
    : Actor<M>,
  never,
  ...
  Effect.gen(function* () {
    ...
    let runState: {
      ...
      readonly decodeRequest: (
        u: unknown
      ) => Effect.Effect<Machine.Public<M>, ParseResult.ParseError>
    } = {
      identifier: "Unknown",
    ...
    const requestContext = <R extends Machine.Public<M>>(request: R) =>
      Effect.sync(() => {
        ...
        const deferred = Deferred.unsafeMake<
          Request.Success<R>,
          Request.Error<R>
        >(fiber.id())
        const span: Tracer.AnySpan | undefined = context.unsafeMap.get(
          Tracer.ParentSpan.key
        )
        const addSpans = FiberRefs.getOrDefault(
          fiberRefs,
    ...
    const send = <R extends Machine.Public<M>>(request: R) =>
      Effect.flatMap(requestContext(request), (item) => {
        if (!item[3]) {
    ...
    const sendIgnore = <R extends Machine.Public<M>>(request: R) =>
      Effect.flatMap(requestContext(request), (item) => {
        ...
        const [, deferred, span] = item
        return Effect.useSpan(
          `Machine.sendIgnore ${request._tag}`,
          {
            parent: span,
            attributes: {
              "effect.machine": runState.identifier,
    ...
    const sendUnknown = (u: unknown) =>
      Effect.suspend(() =>
        runState.decodeRequest(u).pipe(
          Effect.flatMap((req) =>
            Effect.flatMap(Effect.exit(send(req)), (exit) =>
              Schema.serializeExit(req, exit)
            )
          ),
    ...
    const run = Effect.gen(function* () {
      ...
      const forkReplace: {
        ...
        <A, E, R>(
          ...
          Effect.asVoid(FiberMap.run(fiberMap, id, MachineDefect.wrap(effect)))
      )
      ...
      const forkOneWith: {
        ...
        <A, E, R>(
          ...
          Effect.map(forkOne(effect, id), (_) => [_, state] as const)
      )

      const contextProto: Procedure.Procedure.ContextProto<
        Machine.Public<M>,
        Machine.State<M>
      > = {
        sendAwait: send,
      ...
      > = Object.fromEntries(
        procedures.private
          .map((p) => [p.tag, p])
          .concat(procedures.public.map((p) => [p.tag, p]))
      )
      ...
      runState = {
        ...
        decodeRequest: Schema.decodeUnknown(
          Schema.Union(
            ...Arr.filter(procedures.public, Procedure.isSerializable).map(
              (p) => p.schema
            )
          )
        )
      ...
      const process = pipe(
        ...
        Effect.flatMap(([request, deferred, span, addSpan]) =>
          Effect.flatMap(Deferred.isDone(deferred), (done) => {
            ...
            if (addSpan) {
              handler = Effect.withSpan(
                handler,
                `Machine.process ${request._tag}`,
                {
                  kind: "server",
                  parent: span,
                  attributes: {
      ...
      yield* pipe(
        Effect.all(
          [process, FiberSet.join(fiberSet), FiberMap.join(fiberMap)],
        ...
        Effect.tapErrorCause((cause) =>
          FiberRef.getWith(
            ...
            Option.match({
              ...
              onSome: (level) =>
                Effect.log(
                  `Unhandled Machine (${runState.identifier}) failure`,
                  cause
                ).pipe(Effect.locally(FiberRef.currentLogLevel, level))
            })
          )
        ),
        Effect.catchAllDefect((cause) =>
          Effect.fail(new MachineDefect({ cause }))
        )
      )
    ...
    const fiber = yield* pipe(
      run,
      self.retryPolicy ? Effect.retry(self.retryPolicy) : identity,
      Effect.forkScoped,
    ...
    return identity<SerializableActor<M>>(
      Object.assign(Object.create(ActorProto), {
        machine: self,
        input: input!,
...
>(
  self: Actor<
    SerializableMachine<State, Public, Private, Input, InitErr, R, SR>
  >
): Effect.Effect<
  [input: unknown, state: unknown],
  ParseResult.ParseError,
  SR
> =>
  Effect.zip(
...
> =>
  Effect.flatMap(
    Schema.decodeUnknown(Schema.Tuple(self.schemaInput, self.schemaState))(
      snapshot
    ),
