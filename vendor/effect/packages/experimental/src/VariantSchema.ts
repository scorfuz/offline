...
> extends Schema.Union<{
  readonly [K in keyof Members]: [Members[K]] extends [Schema.Schema.All]
    ? Members[K]
    : never
}> {}
...
export const Overrideable = <From, IFrom, RFrom, To, ITo, R>(
  ...
  options: {
    readonly generate: (
      _: Option.Option<ITo>
    ) => Effect.Effect<From, ParseResult.ParseIssue, R>
  ...
  Schema.transformOrFail(
    ...
    Schema.Union(
      Schema.Undefined,
      to as Schema.brand<Schema.Schema<To, ITo>, "Override">
    ),
    {
      decode: (_) =>
        options.decode
          ? ParseResult.decode(options.decode)(_)
          : ParseResult.succeed(undefined),
      encode: (dt) =>
        options.generate(dt === undefined ? Option.none() : Option.some(dt))
    }
  ).pipe(
    Schema.propertySignature,
    Schema.withConstructorDefault(
      options.constructorDefault ?? (constUndefined as any)
    )
  )
