...
export const ErrorTypeId: unique symbol = Symbol.for(
  "@effect/experimental/PersistenceError"
)
 ...
 * @category errors
 */
export class PersistenceParseError extends TypeIdError(
  ErrorTypeId,
  "PersistenceError"
)<{
 ...
 * @category errors
 */
export class PersistenceBackingError extends TypeIdError(
  ErrorTypeId,
  "PersistenceError"
)<{
 ...
 * @category type ids
 */
export const BackingPersistenceTypeId: unique symbol = Symbol.for(
...
export interface BackingPersistenceStore {
  ...
  readonly getMany: (
    key: Array<string>
  ) => Effect.Effect<Array<Option.Option<unknown>>, PersistenceError>
  readonly set: (
    key: string,
    value: unknown,
 ...
 * @category tags
 */
export const BackingPersistence: Context.Tag<
  BackingPersistence,
  BackingPersistence
> = Context.GenericTag<BackingPersistence>(
  "@effect/experimental/BackingPersistence"
)
 ...
 * @category type ids
 */
export const ResultPersistenceTypeId: unique symbol = Symbol.for(
...
export interface ResultPersistenceStore {
  ...
  readonly setMany: <R, IE, E, IA, A>(
    entries: Iterable<
      readonly [ResultPersistence.Key<R, IE, E, IA, A>, Exit.Exit<A, E>]
    >
  ) => Effect.Effect<void, PersistenceError, R>
...
export declare namespace ResultPersistence {
   ...
   * @category models
   */
  export interface Key<R, IE, E, IA, A>
    extends Schema.WithResult<A, IA, E, IE, R>, PrimaryKey.PrimaryKey {}
  /**
   * @since 1.0.0
   * @category models
   */
  ...
  export type TimeToLiveArgs<A> = A extends infer K
    ? K extends Persistable<infer _A, infer _E>
      ? [request: K, exit: Exit.Exit<_A["Type"], _E["Type"]>]
      : never
    : never
}
 ...
 * @category tags
 */
export const ResultPersistence: Context.Tag<
  ResultPersistence,
  ResultPersistence
> = Context.GenericTag<ResultPersistence>(
  "@effect/experimental/ResultPersistence"
)
      ...
      make: (options) =>
        Effect.gen(function* () {
          ...
          const parse = <R, IE, E, IA, A>(
            ...
            Effect.mapError(Schema.deserializeExit(key, value), (_) =>
              PersistenceParseError.make(method, _.issue)
            )
          const encode = <R, IE, E, IA, A>(
            ...
            Effect.mapError(Schema.serializeExit(key, value), (_) =>
              PersistenceParseError.make(method, _.issue)
            )
          ...
          return identity<ResultPersistenceStore>({
              ...
              return encode("set", key, value).pipe(
                Effect.flatMap((encoded) =>
                  storage.set(
                    makeKey(key),
                    encoded,
                    Duration.isFinite(ttl) ? Option.some(ttl) : Option.none()
                  )
                )
              )
            },
            setMany: Effect.fnUntraced(function* (entries) {
              const encodedEntries =
                Arr.empty<
                  readonly [string, unknown, Option.Option<Duration.Duration>]
                >()
              ...
              return yield* storage
                .setMany(encodedEntries)
                .pipe(
                  Effect.catchAll((error) =>
                    Effect.fail(PersistenceBackingError.make("setMany", error))
                  )
                )
            }),
...
export const layerMemory: Layer.Layer<BackingPersistence> = Layer.sync(
  ...
  () => {
    const stores = new Map<
      string,
      Map<string, readonly [unknown, expires: number | null]>
    >()
        ...
        Effect.map(Effect.clock, (clock) => {
          ...
          const unsafeGet = (key: string): Option.Option<unknown> => {
            ...
            if (value === undefined) {
              ...
              value[1] !== null &&
              value[1] <= clock.unsafeCurrentTimeMillis()
            ) {
              map.delete(key)
              return Option.none()
            }
 ...
 * @category layers
 */
export const layerKeyValueStore: Layer.Layer<
  BackingPersistence,
  never,
      ...
      make: (storeId) =>
        Effect.map(Effect.clock, (clock) => {
          ...
          const get = (method: string, key: string) =>
            Effect.flatMap(
              Effect.mapError(store.get(key), (error) =>
                ...
                onSome: (s) =>
                  Effect.flatMap(
                    ...
                    (_) => {
                      ...
                      if (
                        expires !== null &&
                        expires <= clock.unsafeCurrentTimeMillis()
                      ) {
                        return Effect.as(
          ...
          return identity<BackingPersistenceStore>({
            get: (key) => get("get", key),
            getMany: (keys) =>
              Effect.forEach(keys, (key) => get("getMany", key), {
            ...
            set: (key, value, ttl) =>
              Effect.flatMap(
                ...
                (u) =>
                  Effect.mapError(store.set(key, u), (error) =>
                    PersistenceBackingError.make("set", error)
                  )
              ),
            setMany: (entries) =>
              Effect.forEach(
                ...
                Effect.mapError((error) =>
                  PersistenceBackingError.make("setMany", error)
                )
              ),
            remove: (key) =>
              Effect.mapError(store.remove(key), (error) =>
                PersistenceBackingError.make("remove", error)
              ),
            clear: Effect.mapError(store.clear, (error) =>
              PersistenceBackingError.make("clear", error)
            )
          })
        })
    })
...
export const layerResultMemory: Layer.Layer<ResultPersistence> =
  layerResult.pipe(Layer.provide(layerMemory))

/**
 * @since 1.0.0
 * @category layers
 */
export const layerResultKeyValueStore: Layer.Layer<
  ResultPersistence,
  never,
  KeyValueStore.KeyValueStore
> = layerResult.pipe(Layer.provide(layerKeyValueStore))
...
export const unsafeTtlToExpires = (
  ...
  ttl._tag === "None"
    ? null
    : clock.unsafeCurrentTimeMillis() + Duration.toMillis(ttl.value)
