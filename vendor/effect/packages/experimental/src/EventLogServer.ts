...
> = Effect.gen(function* () {
  ...
  function* handler(socket: Socket.Socket) {
    ...
    const write = (response: typeof ProtocolResponse.Type) =>
      Effect.gen(() => writeGen(response))
    ...
    yield* socket
      .run((data) => handleRequest(decodeRequest(data)))
...
export class PersistedEntry extends Schema.Class<PersistedEntry>(
  "@effect/experimental/EventLogServer/PersistedEntry"
)({
  entryId: EntryId,
...
export class Storage extends Context.Tag(
  "@effect/experimental/EventLogServer/Storage"
)<
  Storage,
  {
    ...
    readonly changes: (
      ...
      Mailbox.ReadonlyMailbox<EncryptedRemoteEntry>,
      never,
      Scope.Scope
    >
  }
 ...
 * @category storage
 */
export const layerStorageMemory: Layer.Layer<Storage> = Layer.scoped(
  Storage,
  makeStorageMemory
)
