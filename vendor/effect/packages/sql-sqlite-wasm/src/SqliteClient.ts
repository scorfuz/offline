...
const registered = globalValue(
  "@effect/sql-sqlite-wasm/registered",
  () => new Set<string>()
)
  ...
  Effect.gen(function* () {
    ...
    const makeConnection = Effect.gen(function* () {
      ...
      const run = (
        ...
        const rows = Effect.withFiberRuntime<
        ...
        >((fiber) => {
          ...
          return send(
            id,
            [id, sql, params],
            fiber.getFiberRef(currentTransferables)
          )
        })
 ...
 * @since 1.0.0
 */
export const currentTransferables: FiberRef.FiberRef<
  ReadonlyArray<Transferable>
> = globalValue("@effect/sql-sqlite-wasm/currentTransferables", () =>
