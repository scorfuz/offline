...
const makeClient = (
  ...
  Effect.gen(function* () {
    ...
    class ConnectionImpl implements Connection {
      ...
      executeStream(
        ...
        return Effect.gen(function* () {
          ...
          const pull = Effect.async<Chunk.Chunk<any>, Option.Option<SqlError>>(
            (resume) => {
    ...
    const reserveRaw = Effect.async<Pg.PoolClient, SqlError, Scope.Scope>(
      (resume) => {
            ...
            Effect.fnUntraced(function* (emit) {
              ...
              yield* Effect.addFinalizer(() =>
                Effect.promise(() => {
                  ...
                  return client.query(
                    `UNLISTEN ${Pg.escapeIdentifier(channel)}`
                  )
                })
              )
              yield* Effect.tryPromise({
                try: () =>
                  client.query(`LISTEN ${Pg.escapeIdentifier(channel)}`),
                catch: (cause) =>
...
export const layerConfig = (
  ...
  ConfigError.ConfigError | SqlError
> =>
  Layer.scopedContext(
