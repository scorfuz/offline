...
describe("Product", () => {
  it("tuple", () => {
    expect(Product.tuple(ProductInstance)(fa, fb, fc)).type.toBe<
      RAW<
    ...
    expect(Product.tuple(ProductInstance)()).type.toBe<
      RAW<unknown, never, []>
    >()
  })

  it("struct", () => {
    expect(Product.struct(ProductInstance)({ fa, fb, fc })).type.toBe<
      RAW<
        { a: string } & { b: number } & { c: boolean },
        "a" | "b" | "c",
        { fa: string; fb: number; fc: boolean }
      >
    >()
    // should allow empty structs
    expect(Product.struct(ProductInstance)({})).type.toBe<
      RAW<unknown, never, {}>
    >()
  })
})
