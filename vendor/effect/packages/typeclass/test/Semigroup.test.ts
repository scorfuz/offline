...
describe.concurrent("Semigroup", () => {
  ...
  describe.concurrent("min", () => {
    ...
    it("should return the last minimum", () => {
      type Item = { a: number }
      const A = Semigroup.min(
        pipe(
          Number.Order,
          order.mapInput((_: Item) => _.a)
        )
      )
      const item: Item = { a: 1 }
  ...
  it("productMany", () => {
    ...
    U.deepStrictEqual(S.combine(["a", "b", "c"], ["d", "e", "f"]), [
      "ad",
      "be",
      "cf"
    ])
  ...
  it("productAll", () => {
    const S = Semigroup.Product.productAll([
      StringInstances.Semigroup,
      StringInstances.Semigroup
    ])
