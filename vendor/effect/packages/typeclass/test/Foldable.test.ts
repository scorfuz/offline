...
describe.concurrent("Foldable", () => {
  ...
  it("combineMap", () => {
    ...
    U.deepStrictEqual(
      combineMap(NumberInstances.MonoidSum)([1, 2, 3], U.double),
      12
    )
  })

  it("reduceKind", () => {
    const reduceKind = Foldable.reduceKind(ArrayInstances.Foldable)(
      OptionInstances.Monad
    )
    U.deepStrictEqual(
      reduceKind([], "-", () => O.none()),
      O.some("-")
    ...
    U.deepStrictEqual(
      reduceKind(["a", "b", "c"], "-", (b, a) =>
        a === "b" ? O.none() : O.some(b + a)
      ),
      O.none()
    )
  })
  ...
  it("coproductMapKind", () => {
    ...
    U.deepStrictEqual(
      pipe(
        ["a", "b", "c"],
        coproductMapKind((a) => (a === "b" ? O.none() : O.some(a)))
      ),
      O.some("a")
    )
  })
