...
describe.concurrent("Filterable", () => {
  ...
  it("partitionMapComposition", () => {
    ...
    const f = (s: string) =>
      s.length > 1 ? E.right(s.length) : E.left(s + "!")
    U.deepStrictEqual(partitionMap([], f), [[], []])
    U.deepStrictEqual(partitionMap([O.none()], f), [[O.none()], [O.none()]])
    U.deepStrictEqual(partitionMap([O.some("a")], f), [
      [O.some("a!")],
      [O.none()]
    ])
    U.deepStrictEqual(partitionMap([O.some("aa")], f), [
      [O.none()],
      [O.some(2)]
    ])
  })
  ...
  it("compact", () => {
    ...
    assert.deepStrictEqual(
      compact([O.some(1), O.some(2), O.some(3)]),
      [1, 2, 3]
    )
    assert.deepStrictEqual(compact([O.some(1), O.none(), O.some(3)]), [1, 3])
  })
