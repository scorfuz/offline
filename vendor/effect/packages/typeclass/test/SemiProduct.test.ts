...
describe.concurrent("SemiProduct", () => {
  it("productMany should be equivalent to `ap`", () => {
    const curry =
      (f: Function, n: number, acc: ReadonlyArray<unknown>) => (x: unknown) => {
  ...
  describe.concurrent("productComposition", () => {
    ...
    it("Option", () => {
      ...
      U.deepStrictEqual(
        product(O.some(O.some(1)), O.some(O.none())),
        O.some(O.none())
      )
      U.deepStrictEqual(
        product(O.some(O.none()), O.some(O.some(2))),
        O.some(O.none())
      )
      U.deepStrictEqual(
  ...
  describe.concurrent("productManyComposition", () => {
    it("ReadonlyArray", () => {
      ...
      expect(
        productMany([O.some(1), O.none()], [[O.some(2), O.none()]])
      ).toEqual([O.some([1, 2]), O.none(), O.none(), O.none()])
      expect(
        productMany(
          [O.some(1), O.some(2)],
          [[O.some(3), O.some(4)], [O.some(5)]]
        )
      ).toEqual([
        O.some([1, 3, 5]),
        O.some([1, 4, 5]),
    ...
    it("Option", () => {
      ...
      U.deepStrictEqual(
        productMany(O.some(O.none()), [O.some(O.none())]),
        O.some(O.none())
      )
      U.deepStrictEqual(
  ...
  describe.concurrent("andThenBind", () => {
    it("Covariant (Option)", () => {
      ...
      U.deepStrictEqual(
        pipe(O.some({ a: 1 }), andThenBind("b", O.none())),
        O.none()
      )
      U.deepStrictEqual(
        pipe(O.some({ a: 1 }), andThenBind("b", O.some(2))),
        O.some({ a: 1, b: 2 })
      )
    })
  ...
  describe.concurrent("appendElement", () => {
    it("Covariant (Option)", () => {
      ...
      expect(pipe(O.some([1, 2]), appendElement(O.some(3)))).toEqual(
        O.some([1, 2, 3])
      )
    })
    ...
    it("Contravariant (Predicate)", () => {
      ...
      const p = pipe(
        P.tuple(String.isString, String.isString),
        appendElement(Number.isNumber)
      )
  ...
  describe.concurrent("nonEmptyTuple", () => {
    it("Covariant (Option)", () => {
      ...
      expect(nonEmptyTuple(O.some("a"), O.some(1), O.some(true))).toEqual(
        O.some(["a", 1, true])
      )
      U.deepStrictEqual(
        nonEmptyTuple(O.some("a"), O.some(1), O.none()),
        O.none()
      )
    })
  ...
  describe.concurrent("nonEmptyStruct", () => {
    ...
    it("Invariant (Semigroup)", () => {
      ...
      const S = nonEmptyStruct({
        x: StringInstances.Semigroup,
        y: NumberInstances.SemigroupSum
      })
      U.deepStrictEqual(S.combine({ x: "a", y: 2 }, { x: "b", y: 3 }), {
        x: "ab",
        y: 5
      })
    })
