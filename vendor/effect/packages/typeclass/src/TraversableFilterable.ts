 ...
 * @since 0.24.0
 */
export interface TraversableFilterable<
...
export const traversePartitionMap =
  ...
  (F) =>
  (self, f) =>
    F.map(T.traverse(F)(self, f), filterable.separate(T))
...
export const traverseFilterMap =
  ...
  (F) =>
  (self, f) =>
    F.map(T.traverse(F)(self, f), filterable.compact(T))

/**
 * @since 0.24.0
 */
export const traverseFilter =
  <T extends TypeLambda>(T: TraversableFilterable<T>) =>
  <F extends TypeLambda>(
    F: Applicative<F>
  ): {
  ...
  <F extends TypeLambda>(
    ...
    dual(2, (self, predicate) =>
      T.traversePartitionMap(F)(self, (b) =>
        F.map(
          predicate(b),
          (keep): E.Either<typeof b, typeof b> =>
            keep ? E.right(b) : E.left(b)
        )
      )
    )
