...
const product = <A, B>(
  ...
  if (
    ArrayInstances.isEmptyReadonlyArray(self) ||
    ArrayInstances.isEmptyReadonlyArray(that)
  ) {
...
const productMany =
  semiProduct.productMany<ArrayInstances.ReadonlyArrayTypeLambda>(map, product)
...
const traverse = <F extends TypeLambda>(
  ...
  dual(
    ...
    <A, R, O, E, B>(
      ...
      F.productAll(ArrayInstances.fromIterable(self).map(f))
  )
 ...
 * @since 0.24.0
 */
export const SemiApplicative: semiApplicative.SemiApplicative<ArrayInstances.ReadonlyArrayTypeLambda> =
  {
    imap,
    map,
    product,
 ...
 * @since 0.24.0
 */
export const TraversableFilterable: traversableFilterable.TraversableFilterable<ArrayInstances.ReadonlyArrayTypeLambda> =
  {
