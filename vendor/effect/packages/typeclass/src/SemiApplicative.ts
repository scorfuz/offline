 ...
 * @since 0.24.0
 */
export interface SemiApplicative<F extends TypeLambda>
...
export const getSemigroup =
  ...
  <A, R, O, E>(S: Semigroup<A>): Semigroup<Kind<F, R, O, E, A>> =>
    semigroup.make(
      (self, that) =>
        F.map(F.product(self, that), ([a1, a2]) => S.combine(a1, a2)),
      (self, collection) =>
        F.map(F.productMany(self, collection), ([head, ...tail]) =>
          S.combineMany(head, tail)
        )
    )
 ...
 * @since 0.24.0
 */
export const zipWith = <F extends TypeLambda>(
  F: SemiApplicative<F>
): {
  <R2, O2, E2, B, A, C>(
  ...
  dual(
    ...
    <R1, O1, E1, A, R2, O2, E2, B, C>(
      ...
      F.map(F.product(self, that), ([a, b]) => f(a, b))
  )
...
export const ap = <F extends TypeLambda>(
  ...
  dual(
    ...
    <R1, O1, E1, A, B, R2, O2, E2>(
      ...
      zipWith(F)(self, that, (f, a) => f(a))
  )

/**
 * @since 0.24.0
 */
export const zipLeft = <F extends TypeLambda>(
  F: SemiApplicative<F>
): {
  <R2, O2, E2, _>(
 ...
 * @since 0.24.0
 */
export const zipRight = <F extends TypeLambda>(
  F: SemiApplicative<F>
): {
  <R2, O2, E2, B>(
    that: Kind<F, R2, O2, E2, B>
