 ...
 * @since 0.24.0
 */
export const partitionMapComposition =
  ...
  <FR, FO, FE, GR, GO, GE, A, B, C>(
    ...
    Kind<F, FR, FO, FE, Kind<G, GR, GO, GE, C>>
  ] => {
...
export const compact = <F extends TypeLambda>(
  ...
  self: Kind<F, R, O, E, Option.Option<A>>
) => Kind<F, R, O, E, A>) => F.filterMap(identity)
...
export const filter: <F extends TypeLambda>(
  ...
  dual(
    ...
    <R, O, E, A>(
      ...
      Filterable.filterMap(self, (b) =>
        predicate(b) ? Option.some(b) : Option.none()
      )
  )
...
export const partition = <F extends TypeLambda>(
  ...
  dual(
    ...
    <R, O, E, B extends A, A = B>(
      ...
      F.partitionMap(self, (b) =>
        predicate(b) ? Either.right(b) : Either.left(b)
      )
  )
