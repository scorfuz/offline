...
> =>
  Effect.Do.pipe(
    ...
    Effect.map(({ fs, path }) => {
      ...
      const parseConfig =
        <A>(primitive: Config.Config.Primitive<A>) =>
        (value: string) =>
      ...
      ) => Effect.flatMap(fs.readFileString(filePath), parseConfig(primitive))

      const resolveEnumerableDirs = (segments: ReadonlyArray<string>) =>
        segments.length === 0 ? [] : [path.join(...segments)]

      const resolveFilePath = (pathSegments: ReadonlyArray<string>) =>
        path.join(rootDirectory, ...pathSegments)
      ...
      const handlePlatformError =
        (pathSegments: ReadonlyArray<string>) => (error: PlatformError) =>
      ...
      return ConfigProvider.fromFlat(
        ConfigProvider.makeFlat({
          ...
          enumerateChildren: (pathSegments) =>
            Effect.forEach(resolveEnumerableDirs(pathSegments), (dir) =>
...
export const layerDotEnvAdd: (
  path: string
) => Layer.Layer<never, never, FileSystem.FileSystem> = internal.layerDotEnvAdd
