 ...
 * @category guards
 */
export const isHttpApi = (u: unknown): u is HttpApi.Any =>
...
> extends Pipeable {
   ...
   */
  add<A extends HttpApiGroup.HttpApiGroup.Any>(
    group: A
  ): HttpApi<Id, Groups | A, E, R>
  /**
   * Add another `HttpApi` to the `HttpApi`.
   */
  addHttpApi<
    Id2 extends string,
    Groups2 extends HttpApiGroup.HttpApiGroup.Any,
    E2,
    R2
  >(
...
export declare namespace HttpApi {
   ...
   * @category models
   */
  export type AnyWithProps = HttpApi<
    string,
...
>(
  ...
  const apiErrors = extractMembers(
    self.errorSchema.ast,
    new Map(),
    HttpApiSchema.getStatusErrorAST
  )
  const groups = Object.values(
    self.groups
  ) as any as Array<HttpApiGroup.HttpApiGroup.AnyWithProps>
  for (const group of groups) {
    const groupErrors = extractMembers(
      group.errorSchema.ast,
      apiErrors,
      HttpApiSchema.getStatusErrorAST
    )
    ...
    const endpoints = Object.values(group.endpoints) as Iterable<
      HttpApiEndpoint.HttpApiEndpoint<string, HttpMethod>
    >
    for (const endpoint of endpoints) {
      ...
      const errors = extractMembers(
        endpoint.errorSchema.ast,
        groupErrors,
        HttpApiSchema.getStatusErrorAST
      )
      options.onEndpoint({
  ...
  function process(type: AST.AST) {
    ...
    members.set(status, {
      ...
      ast: (current ? current.ast : Option.none()).pipe(
        ...
        Option.orElse(() =>
          !emptyDecodeable && AST.isVoidKeyword(AST.encodedAST(type))
            ? Option.none()
            : Option.some(type)
        )
      )
...
const getDescriptionOrIdentifier = (
  ...
  return Option.fromNullable(
    annotations[AST.DescriptionAnnotationId] ??
      (annotations[AST.IdentifierAnnotationId] as any)
  )
}
 ...
 * @category tags
 */
export class AdditionalSchemas extends Context.Tag(
  "@effect/platform/HttpApi/AdditionalSchemas"
)<AdditionalSchemas, ReadonlyArray<Schema.Schema.All>>() {}
