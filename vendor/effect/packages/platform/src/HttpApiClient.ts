...
>(
  ...
  HttpApiMiddleware.HttpApiMiddleware.Without<
    ApiR | HttpApiGroup.ClientContext<Groups>
  >
> =>
  Effect.gen(function* () {
      ...
      onEndpoint(onEndpointOptions) {
        ...
        errors.forEach(({ ast }, status) => {
          ...
          decodeMap[status] = (response) =>
            Effect.flatMap(decode(response), Effect.fail)
        })
        successes.forEach(({ ast }, status) => {
          decodeMap[status] =
            ast._tag === "None" ? responseAsVoid : schemaToResponse(ast.value)
        })
...
>(
  ...
  HttpApiMiddleware.HttpApiMiddleware.Without<
    ...
    | HttpApiEndpoint.ContextWithName<
        HttpApiGroup.EndpointsWithName<Groups, GroupName>,
        EndpointName
      >
    | HttpApiEndpoint.ErrorContextWithName<
        HttpApiGroup.EndpointsWithName<Groups, GroupName>,
        EndpointName
      >
  >
...
const parseJsonOrVoid = Schema.transformOrFail(Schema.String, Schema.Unknown, {
  strict: true,
  decode: (i, _, ast) => {
...
const parseJsonArrayBuffer = Schema.compose(
...
const schemaFromArrayBuffer = (
  ...
  if (ast._tag === "Union") {
    return Schema.Union(
      ...ast.types.map((ast) =>
        schemaFromArrayBuffer(ast, HttpApiSchema.getEncoding(ast, encoding))
      )
    )
  }
  ...
  switch (encoding.kind) {
    ...
    case "UrlParams": {
      return Schema.compose(
        StringFromArrayBuffer,
        UrlParams.schemaParse(schema as any)
      ) as any
    }
    case "Uint8Array": {
...
const responseAsVoid = (_response: HttpClientResponse.HttpClientResponse) =>
  Effect.void
    ...
    encode(toI, _, ast) {
      switch (encoding.kind) {
        case "Json": {
          try {
            return ParseResult.succeed(
              HttpBody.text(JSON.stringify(toI), encoding.contentType)
            )
          } catch {
            return ParseResult.fail(
              new ParseResult.Type(ast, toI, "Could not encode as JSON")
            )
          }
        }
        case "Text": {
        ...
        case "Uint8Array": {
          ...
          return ParseResult.succeed(
            HttpBody.uint8Array(toI, encoding.contentType)
          )
        }
      }
    }
