...
export const isWorkerError = (u: unknown): u is WorkerError =>
  Predicate.hasProperty(u, WorkerErrorTypeId)
...
export class WorkerError extends Schema.TaggedError<WorkerError>()(
   ...
   * @since 1.0.0
   */
  static readonly encodeCause: (
    a: Cause.Cause<WorkerError>
  ) => Schema.CauseEncoded<WorkerErrorFrom, unknown> = Schema.encodeSync(
    this.Cause
  )
   ...
   * @since 1.0.0
   */
  static readonly decodeCause: (
    u: Schema.CauseEncoded<WorkerErrorFrom, unknown>
  ) => Cause.Cause<WorkerError> = Schema.decodeSync(this.Cause)
