...
>(
  ...
  return <E>(
    self: HttpIncomingMessage<E>
  ): Effect.Effect<A, ParseResult.ParseError, R> => parse(self.headers)
}
 ...
 * @category fiber refs
 */
export class MaxBodySize extends Context.Reference<MaxBodySize>()(
...
>(2, (effect, size) =>
  Effect.provideService(effect, MaxBodySize, Option.map(size, FileSystem.Size))
)
