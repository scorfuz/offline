 ...
 * @category router
 */
export class Router extends HttpRouter.Tag(
  "@effect/platform/HttpApiBuilder/Router"
)<Router>() {}
...
> = Effect.gen(function* () {
  ...
  const router = applyMiddleware(
    middleware,
    yield* HttpRouter.toHttpApp(yield* Router.router)
  )
  ...
  return router.pipe(
    ...
    Effect.catchAllCause((cause) =>
      Effect.matchEffect(
        Effect.provide(encodeError(Cause.squash(cause)), context),
        {
...
>(api: HttpApi.HttpApi<Id, Groups, E, R>) {
  ...
  return (
    ...
    Effect.catchAllCause(applyMiddleware(middlewareMap, effect), (cause) =>
      Effect.matchEffect(
        Effect.provide(encodeError(Cause.squash(cause)), context),
        {
          onFailure: () => Effect.failCause(cause),
...
> extends Pipeable {
  ...
  handleRaw<Name extends HttpApiEndpoint.HttpApiEndpoint.Name<Endpoints>, R1>(
    name: Name,
    handler: HttpApiEndpoint.HttpApiEndpoint.HandlerRawWithName<
      Endpoints,
      Name,
      E,
      R1
...
>(
  ...
  build: (
    handlers: Handlers.FromGroup<
      ApiError,
      ApiR,
      HttpApiGroup.HttpApiGroup.WithName<Groups, Name>
    >
  ...
  Router.use((router) =>
    Effect.gen(function* () {
      ...
      const handlers: Handlers<any, any, any> = Effect.isEffect(result)
        ...
        : result
      const groupMiddleware = makeMiddlewareMap(
        (group as any).middlewares,
        context
      )
      const routes: Array<HttpRouter.Route<any, any>> = []
      for (const item of handlers.handlers) {
        const middleware = makeMiddlewareMap(
          (item as any).endpoint.middlewares,
          context,
          groupMiddleware
        )
        routes.push(
          handlerToRoute(
            ...
            middleware,
            function (request) {
              return Effect.mapInputContext(item.handler(request), (input) =>
...
export const handler = <
  ...
  const Name extends HttpApiGroup.HttpApiGroup.EndpointsWithName<
    Groups,
    GroupName
  >["name"],
  R
...
const requestPayload = (
...
): Effect.Effect<unknown, never, FileSystem | Path | Scope> => {
  ...
  return Effect.map(
    Effect.orDie(request.arrayBuffer),
    (buffer) => new Uint8Array(buffer)
  )
}

type MiddlewareMap = Map<
  string,
  {
      ...
      if (key !== undefined) {
        ...
        return ps !== undefined
          ...
          : ast.indexSignatures.some(
              (is) =>
                Schema.is(Schema.make(is.parameter))(key) &&
                isSingleStringType(is.type)
            )
      }
      return false
    }
...
export function normalizeUrlParams(
  ...
  for (const key in params) {
    ...
    out[key] =
      Array.isArray(value) || isSingleStringType(ast, key) ? value : [value]
  }
  return out
}
...
const handlerToRoute = (
  ...
  const isMultipartStream = endpoint.payloadSchema.pipe(
    Option.map(
      ({ ast }) => HttpApiSchema.getMultipartStream(ast) !== undefined
    ),
  ...
  const decodePayload =
    isFullRequest || isMultipartStream
      ? Option.none()
      ...
      Effect.gen(function* () {
        ...
        if (decodePayload._tag === "Some") {
          ...
          request.payload = Option.match(multipartLimits, {
            ...
            onSome: (limits) =>
              Multipart.withLimitsStream(httpRequest.multipartStream, limits)
          })
        }
        ...
        if (endpoint.urlParamsSchema._tag === "Some") {
          ...
          request.urlParams = yield* Schema.decodeUnknown(schema)(
            normalizeUrlParams(urlParams, schema.ast)
          )
        }
        const response = yield* handler(request)
        return HttpServerResponse.isServerResponse(response)
          ? response
...
const applyMiddleware = <A extends Effect.Effect<any, any, any>>(
  ...
  for (const entry of middleware.values()) {
    const effect =
      HttpApiMiddleware.SecurityTypeId in entry.tag
        ? makeSecurityMiddleware(entry as any)
        : entry.effect
    if (entry.tag.optional) {
      ...
      handler = Effect.matchEffect(effect, {
        onFailure: () => previous,
        onSuccess:
          entry.tag.provides !== undefined
            ? (value) =>
                Effect.provideService(
                  previous,
                  entry.tag.provides as any,
                  value
                )
            : (_) => previous
      }) as any
...
const securityMiddlewareCache = globalValue<
  WeakMap<any, Effect.Effect<any, any, any>>
>("securityMiddlewareCache", () => new WeakMap())
...
const makeSecurityMiddleware = (entry: {
  ...
  for (const [key, security] of Object.entries(entry.tag.security)) {
    ...
    effect =
      effect === undefined
        ? middleware
        : Effect.catchAll(effect, () => middleware)
  }
  if (effect === undefined) {
    effect = Effect.void
  }
    ...
    switch (encoding.kind) {
      case "Json": {
        return Effect.mapError(
          ...
          (error) =>
            new ParseResult.Type(ast, error, "Could not encode to JSON")
        )
      }
      case "Text": {
        return ParseResult.succeed(
          HttpServerResponse.text(data as any, {
            status,
            contentType: encoding.contentType
          })
        )
      ...
      case "UrlParams": {
        return ParseResult.succeed(
          HttpServerResponse.urlParams(data as any, {
            status,
            contentType: encoding.contentType
          })
        )
 ...
 * @category middleware
 */
export class Middleware extends Context.Tag(
...
const middlewareAdd = (
  ...
  Effect.gen(function* () {
    ...
    yield* service.add((httpApp) =>
      Effect.mapInputContext(middleware(httpApp), (input) =>
        Context.merge(context, input)
      )
    )
  })
...
export const middleware: {
  <EX = never, RX = never>(
    middleware:
      | MiddlewareFn<never>
      | Effect.Effect<MiddlewareFn<never>, EX, RX>,
    options?: {
  ...
  <R, EX = never, RX = never>(
    middleware:
      | MiddlewareFn<never, R>
      | Effect.Effect<MiddlewareFn<never, R>, EX, RX>,
    options: {
    ...
    never,
    EX,
    Exclude<HttpRouter.HttpRouter.ExcludeProvided<R> | RX, Scope>
  >
  <
    ApiId extends string,
    Groups extends HttpApiGroup.HttpApiGroup.Any,
  ...
  >(
    ...
    middleware:
      | MiddlewareFn<NoInfer<Error>>
      | Effect.Effect<MiddlewareFn<NoInfer<Error>>, EX, RX>,
    options?: {
      readonly withContext?: false | undefined
    }
  ): Layer.Layer<never, EX, Exclude<RX, Scope>>
  ...
  >(
    api: HttpApi.HttpApi<ApiId, Groups, Error, ErrorR>,
    middleware:
      | MiddlewareFn<NoInfer<Error>, R>
      | Effect.Effect<MiddlewareFn<NoInfer<Error>, R>, EX, RX>,
    options: {
      readonly withContext: true
    }
  ): Layer.Layer<
  ...
  ...args:
    | [
        middleware:
          ...
          | Effect.Effect<MiddlewareFn<any, any>, any, any>,
        options?:
          | {
  ...
  const withContext = apiFirst
    ? args[2]?.withContext === true
    : (args as any)[1]?.withContext === true
  ...
  return (
    Effect.isEffect(middleware)
      ...
      : Layer.scopedDiscard(add(middleware as any))
  ).pipe(Layer.provide(Middleware.layer))
}
  ...
  options?:
    | {
        ...
        readonly additionalPropertiesStrategy?:
          | OpenApi.AdditionalPropertiesStrategy
          | undefined
      }
    | undefined
