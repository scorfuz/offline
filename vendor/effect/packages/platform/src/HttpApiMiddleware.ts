 ...
 * @category type ids
 */
export const SecurityTypeId: unique symbol = Symbol.for(
  "@effect/platform/HttpApiMiddleware/Security"
)
 ...
 * @category guards
 */
export const isSecurity = (u: TagClassAny): u is TagClassSecurityAny =>
 ...
 * @category models
 */
export interface HttpApiMiddleware<Provides, E> extends Effect.Effect<
  Provides,
  E,
...
export declare namespace HttpApiMiddleware {
   ...
   * @category models
   */
  export type Error<A> = A extends {
    readonly [TypeId]: { readonly failure: infer E }
  }
    ? E
    : never
   ...
   * @category models
   */
  export type ErrorContext<A> = A extends {
    readonly [TypeId]: { readonly failureContext: infer R }
  }
    ? R
    : never
...
export declare namespace TagClass {
   ...
   * @category models
   */
  export type FailureContext<Options> = Schema.Schema.Context<
    FailureSchema<Options>
  >
   ...
   * @category models
   */
  export type FailureService<Options> =
    Optional<Options> extends true ? unknown : Failure<Options>

  /**
   * @since 1.0.0
   * @category models
   */
  export type Optional<Options> = Options extends { readonly optional: true }
    ? true
    : false
...
export const Tag =
  ...
  (
    ...
    TagClass_.failure =
      options?.optional === true || options?.failure === undefined
        ? Schema.Never
        : options.failure
    if (options?.provides) {
    ...
    if (options?.security) {
      if (Object.keys(options.security).length === 0) {
        throw new Error(
          "HttpApiMiddleware.Tag: security object must not be empty"
        )
      }
