...
export const AnnotationStatus: unique symbol = Symbol.for(
  "@effect/platform/HttpApiSchema/AnnotationStatus"
)
...
export const AnnotationEncoding: unique symbol = Symbol.for(
  "@effect/platform/HttpApiSchema/AnnotationEncoding"
)
...
const getAnnotation = <A>(ast: AST.AST, key: symbol): A | undefined =>
  mergedAnnotations(ast)[key] as A
 ...
 * @category annotations
 */
export const getMultipart = (
  ast: AST.AST
): Multipart_.withLimits.Options | undefined =>
 ...
 * @category annotations
 */
export const getMultipartStream = (
  ast: AST.AST
): Multipart_.withLimits.Options | undefined =>
...
export const getParam = (
  ast: AST.AST | Schema.PropertySignature.AST
): string | undefined => {
  const annotations =
...
export const getStatusSuccessAST = (ast: AST.AST): number =>
  getStatus(ast, isVoid(ast) ? 204 : 200)

/**
 * @since 1.0.0
 * @category reflection
 */
export const getStatusSuccess = <A extends Schema.Schema.Any>(
  self: A
): number => getStatusSuccessAST(self.ast)
 ...
 * @category reflection
 */
export const getStatusError = <A extends Schema.Schema.All>(self: A): number =>
  getStatusErrorAST(self.ast)
...
export const UnionUnifyAST = (self: AST.AST, that: AST.AST): AST.AST =>
  AST.Union.make(
    Array.from(
      new Set<AST.AST>([...extractUnionTypes(self), ...extractUnionTypes(that)])
    )
  )
...
export const UnionUnify = <
  A extends Schema.Schema.All,
  B extends Schema.Schema.All
>(
  self: A,
  that: B
): Schema.Schema<
...
export const param: {
  <Name extends string>(
    ...
    schema: S &
      ([Schema.Schema.Encoded<S> & {}] extends [string]
        ? unknown
 ...
 */
export const Empty = (status: number): typeof Schema.Void =>
  Schema.Void.annotations(annotations({ status }))

/**
 * @since 1.0.0
 * @category empty response
 */
export interface asEmpty<S extends Schema.Schema.Any> extends Schema.transform<
  typeof Schema.Void,
  S
> {}
 ...
 * @category multipart
 */
export const MultipartTypeId: unique symbol = Symbol.for(
  "@effect/platform/HttpApiSchema/Multipart"
)
 ...
 * @category multipart
 */
export interface Multipart<S extends Schema.Schema.Any> extends Schema.Schema<
 ...
 * @category multipart
 */
export const Multipart = <S extends Schema.Schema.Any>(
  self: S,
  options?: {
 ...
 * @category multipart
 */
export const MultipartStreamTypeId: unique symbol = Symbol.for(
  "@effect/platform/HttpApiSchema/MultipartStream"
)
 ...
 * @category multipart
 */
export const MultipartStream = <S extends Schema.Schema.Any>(
  self: S,
  options?: {
    readonly maxParts?: Option.Option<number> | undefined
    readonly maxFieldSize?: FileSystem.SizeInput | undefined
...
export const Uint8Array = (options?: {
  ...
  withEncoding(Schema.Uint8ArrayFromSelf, { kind: "Uint8Array", ...options })
...
export const deunionize = (
  ...
  if (ast._tag === "Union") {
    for (const astType of ast.types) {
      ...
      const memberSchema = Schema.make(
        AST.annotations(astType, {
          ...ast.annotations,
          ...astType.annotations
        })
      )
 ...
 * @since 1.0.0
 */
export interface EmptyErrorUnify<
  A extends { [Unify.typeSymbol]?: any }
> extends Effect.EffectUnify<A> {
  EmptyError?: () => A[Unify.typeSymbol] extends
    | EmptyError<infer Self, infer _Tag>
    | infer _
    ? Self
    : never
}
...
export const EmptyError =
  ...
  <const Tag extends string>(options: {
    ...
    const symbol = Symbol.for(
      `@effect/platform/HttpApiSchema/EmptyError/${options.tag}`
    )
