 ...
 */
export type HttpApp<
  A = ServerResponse.HttpServerResponse,
  E = never,
  R = never
> = Effect.Effect<A, E, R | ServerRequest.HttpServerRequest>
...
export const toHandled = <E, R, _, EH, RH>(
  ...
  void,
  never,
  Exclude<R | RH | ServerRequest.HttpServerRequest, Scope.Scope>
> => {
  ...
  const withErrorHandling = Effect.catchAllCause(responded, (cause) =>
  ...
  const withMiddleware = unify(
    middleware === undefined
      ...
      : Effect.matchCauseEffect(
          internalMiddleware.tracer(middleware(withErrorHandling)),
          {
            onFailure: (cause): Effect.Effect<void, EH, RH> =>
              Effect.withFiberRuntime((fiber) => {
                ...
                return Effect.matchCauseEffect(
                  ServerError.causeResponse(cause),
                  {
                    onFailure: (_cause) =>
                      handleResponse(
                        request,
                        ServerResponse.empty({ status: 500 })
                      ),
 ...
 * @category fiber refs
 */
export const currentPreResponseHandlers: FiberRef.FiberRef<
  Option.Option<PreResponseHandler>
...
) => Effect.Effect<void> = internal.appendPreResponseHandler
...
export const toWebHandler: <E>(
  ...
  request: Request,
  context?: Context.Context<never> | undefined
) => Promise<Response> = toWebHandlerRuntime(Runtime.defaultRuntime)
  ...
  options: {
    readonly toHandler: (
      ...
      Effect.Effect<
        ServerResponse.HttpServerResponse,
        E,
        R | Scope.Scope | ServerRequest.HttpServerRequest
      >,
      EX
    >
