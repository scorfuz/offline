...
export interface HttpRouter<E = never, R = never>
  extends
    App.Default<E | Error.RouteNotFound, Exclude<R, RouteContext>>,
    Inspectable {
...
export declare namespace HttpRouter {
  ...
  export interface TagClass<
    ...
    E,
    R
  > extends Context.Tag<Self, Service<E, R>> {
 ...
 */
export const RouteContext: Context.Tag<RouteContext, RouteContext> =
...
>(
  ...
  internal.schemaPathParams

/**
 * @since 1.0.0
 * @category router config
 */
export const currentRouterConfig: FiberRef<Partial<RouterConfig>> =
  internal.currentRouterConfig
 ...
 * @category router config
 */
export const setRouterConfig: (
  config: Partial<RouterConfig>
) => Layer.Layer<never> = internal.setRouterConfig
...
export const concatAll: <
  Routers extends ReadonlyArray<HttpRouter<unknown, unknown>>
>(
  ...routers: Routers
) => [Routers[number]] extends [HttpRouter<infer E, infer R>]
  ? HttpRouter<E, R>
  : never = internal.concatAll
...
export const mountApp: {
  <R1, E1>(
    ...
    E1 | E,
    HttpRouter.ExcludeProvided<R1> | HttpRouter.ExcludeProvided<R>
  >
  <E, R, E1, R1>(
    self: HttpRouter<E, R>,
    path: `/${string}`,
    ...
    E | E1,
    HttpRouter.ExcludeProvided<R> | HttpRouter.ExcludeProvided<R1>
  >
} = internal.mountApp
...
export const route: (method: Method.HttpMethod | "*") => {
  <R1, E1>(
    ...
    E1 | E,
    | R
    | Exclude<R1, ServerRequest.HttpServerRequest | RouteContext | Scope.Scope>
  >
  <E, R, E1, R1>(
    self: HttpRouter<E, R>,
    path: PathInput,
 ...
 * @category tags
 */
export class Default extends Tag(
  "@effect/platform/HttpRouter/Default"
)<Default>() {}
...
export const toHttpApp: <E, R>(
  self: HttpRouter<E, R>
) => Effect.Effect<App.Default<E | Error.RouteNotFound, R>> = internal.toHttpApp
