...
export const makeManager = Effect.gen(function* () {
  ...
  return WorkerManager.of({
    ...
    spawn<I, O, E>({ encode, initialMessage }: Worker.Worker.Options<I>) {
      return Effect.gen(function* () {
        const id = idCounter++
        let requestIdCounter = 0
        const requestMap = new Map<
          number,
          | Mailbox.Mailbox<O, E | WorkerError>
          | Deferred.Deferred<O, E | WorkerError>
        >()
        ...
        if (initialMessage) {
          yield* Effect.sync(initialMessage).pipe(
            ...
            Effect.mapError(
              (cause) => new WorkerError({ reason: "spawn", cause })
            )
          )
        }
...
export const makePool = <I, O, E>(options: Worker.WorkerPool.Options<I>) =>
  Effect.gen(function* () {
    ...
    const pool: Worker.WorkerPool<I, O, E> = {
      ...
      execute: (message: I) =>
        Stream.unwrapScoped(
          Effect.map(backing.get, (worker) => worker.execute(message))
        ),
    ...
    const execute = <Req extends I>(message: Req) => {
      ...
      return pipe(
        ...
        Stream.catchAll((error) =>
          Effect.flatMap(parseFailure(error), Effect.fail)
        ),
...
export const makePoolSerialized = <I extends Schema.TaggedRequest.All>(
  ...
  Effect.gen(function* () {
    ...
    const acquire = pipe(
      ...
      Effect.tap((worker) =>
        Effect.addFinalizer(() => Effect.sync(() => workers.delete(worker)))
      ),
      options.onCreate
        ? Effect.tap(
            options.onCreate as (
              worker: Worker.SerializedWorker<I>
    ...
    PlatformWorker.of({
      ...
      spawn<I, O>(id: number) {
        return Effect.gen(function* () {
          ...
          const run = <A, E, R>(
            handler: (
              _: Worker.BackingWorker.Message<O>
            ) => Effect.Effect<A, E, R>
