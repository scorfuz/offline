...
/** @internal */
export const TypeId: Multiplex.TypeId = Symbol.for(
  ...
  implements Multiplex.HttpMultiplex<E, R>
{
  ...
  constructor(
    ...
    let execute: (
      request: ServerRequest.HttpServerRequest
    ) => App.Default<E | Error.RouteNotFound, R> = (request) =>
    ...
    for (let i = apps.length - 1; i >= 0; i--) {
      ...
      execute = (request) =>
        Effect.flatMap(predicate(request), (match) =>
          match ? app : previous(request)
        )
    }
...
>(3, (self, predicate, app) => make([...self.apps, [predicate, app]] as any))
