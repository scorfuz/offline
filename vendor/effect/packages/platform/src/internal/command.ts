...
/** @internal */
export const isCommand = (u: unknown): u is Command.Command =>
...
>(2, (self, input) =>
  stdin(self, Stream.fromChunk(Chunk.of(new TextEncoder().encode(input))))
)

/** @internal */
export const flatten = (
  self: Command.Command
): ReadonlyArray.NonEmptyReadonlyArray<Command.StandardCommand> =>
  Array.from(
    flattenLoop(self)
  ) as unknown as ReadonlyArray.NonEmptyReadonlyArray<Command.StandardCommand>

/** @internal */
const flattenLoop = (
  self: Command.Command
): Chunk.NonEmptyChunk<Command.StandardCommand> => {
  switch (self._tag) {
    case "StandardCommand": {
...
> =>
  Effect.flatMap(commandExecutor.CommandExecutor, (executor) =>
    executor.lines(command, encoding)
  )
...
const makeStandard = (
  options: Omit<Command.StandardCommand, keyof Command.Command.Proto>
...
const makePiped = (
  options: Omit<Command.PipedCommand, keyof Command.Command.Proto>
): Command.PipedCommand => Object.assign(Object.create(PipedProto), options)
...
export const start = (
  ...
  PlatformError,
  CommandExecutor.CommandExecutor | Scope
> =>
  Effect.flatMap(commandExecutor.CommandExecutor, (executor) =>
...
export const stream = (
  ...
  Stream.flatMap(commandExecutor.CommandExecutor, (executor) =>
...
export const streamLines = (
  ...
  Stream.flatMap(commandExecutor.CommandExecutor, (executor) =>
    executor.streamLines(command, encoding)
  )
...
>(
  ...
  (command, encoding) =>
    Effect.flatMap(commandExecutor.CommandExecutor, (executor) =>
      executor.string(command, encoding)
    )
)
...
>(2, (self, cwd) => {
  switch (self._tag) {
    ...
    case "PipedCommand": {
      return pipeTo(
        workingDirectory(self.left, cwd),
        workingDirectory(self.right, cwd)
      )
    }
  }
})
