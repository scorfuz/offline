...
/** @internal */
export const TypeId: Server.TypeId = Symbol.for(
  "@effect/platform/HttpServer"
) as Server.TypeId

/** @internal */
export const serverTag = Context.GenericTag<Server.HttpServer>(
  "@effect/platform/HttpServer"
)
...
/** @internal */
export const isServer = (u: unknown): u is Server.HttpServer =>
...
>(
  ...
  > =>
    Layer.scopedDiscard(
      Effect.flatMap(serverTag, (server) => server.serve(httpApp, middleware!))
    ) as any
)
...
export const serveEffect = dual<
  {
    (): <E, R>(
      ...
      | Server.HttpServer
      | Scope.Scope
      | Exclude<R, ServerRequest.HttpServerRequest>
    >
    <E, R, App extends App.Default<any, any>>(
      middleware: Middleware.HttpMiddleware.Applied<App, E, R>
    ): (
...
>(
  ...
  (<E, R, App extends App.Default<any, any>>(
    ...
    | Scope.Scope
  > =>
    Effect.flatMap(serverTag, (server) =>
      server.serve(httpApp, middleware)
    )) as any
)
...
export const addressWith = <A, E, R>(
  ...
  Effect.flatMap(serverTag, (server) => effect(server.address))
...
/** @internal */
export const logAddress: Effect.Effect<void, never, Server.HttpServer> =
...
export const makeTestClient = addressWith((address) =>
  Effect.flatMap(Client.HttpClient, (client) => {
    ...
    return Effect.succeed(
      Client.mapRequest(client, ClientRequest.prependUrl(url))
    )
  })
)
...
).pipe(Layer.provideMerge(internalFileSystem.layerNoop({})))
