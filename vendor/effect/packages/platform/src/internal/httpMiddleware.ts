...
/** @internal */
export const make = <M extends Middleware.HttpMiddleware>(middleware: M): M =>
  middleware
...
export const withLoggerDisabled = <A, E, R>(
  ...
  Effect.zipRight(FiberRef.set(loggerDisabled, true), self)

/** @internal */
export const currentTracerDisabledWhen = globalValue(
  Symbol.for("@effect/platform/HttpMiddleware/tracerDisabledWhen"),
  () =>
    FiberRef.unsafeMake<Predicate.Predicate<ServerRequest.HttpServerRequest>>(
      constFalse
    )
)
...
>(2, (self, urls) =>
  Layer.locally(self, currentTracerDisabledWhen, (req) =>
...
export const SpanNameGenerator =
  Context.Reference<Middleware.SpanNameGenerator>()(
    "@effect/platform/HttpMiddleware/SpanNameGenerator",
    {
      defaultValue: () => (request: ServerRequest.HttpServerRequest) =>
    ...
    return Effect.withLogSpan(
      Effect.flatMap(Effect.exit(httpApp), (exit) => {
        if (fiber.getFiberRef(loggerDisabled)) {
          ...
          const [response, cause] = ServerError.causeResponseStripped(
            exit.cause
          )
...
export const xForwardedHeaders = make((httpApp) =>
  Effect.updateService(httpApp, ServerRequest.HttpServerRequest, (request) => {
    ...
    return request.modify({
      headers:
        host !== undefined
          ? Headers.set(request.headers, "host", host)
          : undefined,
      remoteAddress
    })
  })
...
export const searchParamsParser = <E, R>(httpApp: App.Default<E, R>) =>
  ...
  >((fiber) => {
    ...
    const params = ServerRequest.searchParamsFromURL(
...
export const cors = (options?: {
  ...
  const preResponseHandler = (
    ...
    Effect.succeed(
      ServerResponse.setHeaders(response, headersFromRequest(request))
    )
  ...
  return <E, R>(httpApp: App.Default<E, R>): App.Default<E, R> =>
    Effect.withFiberRuntime((fiber) => {
      ...
      return Effect.zipRight(
        internalHttpApp.appendPreResponseHandler(preResponseHandler),
        httpApp
      )
    })
}
