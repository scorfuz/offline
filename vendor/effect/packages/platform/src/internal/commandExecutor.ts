...
/** @internal */
export const TypeId: _CommandExecutor.TypeId = Symbol.for(
  "@effect/platform/CommandExecutor"
) as _CommandExecutor.TypeId
...
/** @internal */
export const CommandExecutor = GenericTag<_CommandExecutor.CommandExecutor>(
  "@effect/platform/CommandExecutor"
)
...
export const makeExecutor = (
  ...
  const streamLines: _CommandExecutor.CommandExecutor["streamLines"] = (
    command,
    encoding
  ) => {
    ...
    return Stream.splitLines(
      Stream.mapChunks(
        stream(command),
        Chunk.map((bytes) => decoder.decode(bytes))
      )
    )
  }
  return {
    ...
    exitCode: (command) =>
      Effect.scoped(
        Effect.flatMap(start(command), (process) => process.exitCode)
      ),
    stream,
    string: (command, encoding = "utf-8") => {
      ...
      return pipe(
        ...
        Effect.flatMap((process) =>
          Stream.run(process.stdout, collectUint8Array)
        ),
