...
/** @internal */
export const TypeId: ServerRequest.TypeId = Symbol.for(
  "@effect/platform/HttpServerRequest"
) as ServerRequest.TypeId

/** @internal */
export const serverRequestTag =
  Context.GenericTag<ServerRequest.HttpServerRequest>(
    "@effect/platform/HttpServerRequest"
  )
...
/** @internal */
export const upgrade = Effect.flatMap(
  serverRequestTag,
  (request) => request.upgrade
)

/** @internal */
export const upgradeChannel = <IE = never>() =>
  Channel.unwrap(Effect.map(upgrade, Socket.toChannelWith<IE>()))

/** @internal */
export const multipartPersisted = Effect.flatMap(
  serverRequestTag,
...
export const schemaBodyForm = <A, I extends Partial<Multipart.Persisted>, R>(
  ...
  const parseUrlParams = IncomingMessage.schemaBodyUrlParams(
    schema as Schema.Schema<A, any, R>,
    options
  )
  ...
  > {
    return Stream.pipeThroughChannel(
      Stream.mapError(
        this.stream,
        (cause) =>
          new Multipart.MultipartError({ reason: "InternalError", cause })
      ),
