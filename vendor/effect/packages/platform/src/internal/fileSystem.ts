...
export const Size = (bytes: SizeInput) =>
  typeof bytes === "bigint" ? (bytes as Size_) : (BigInt(bytes) as Size_)
...
const stream = (
  ...
  const bytesToRead =
    bytesToRead_ !== undefined ? Size(bytesToRead_) : undefined
  ...
  > {
    ...
    return Channel.flatMap(
      ...
      Option.match({
        onNone: () => Channel.void,
        onSome: (buf) =>
          Channel.flatMap(Channel.write(Chunk.of(buf)), (_) =>
