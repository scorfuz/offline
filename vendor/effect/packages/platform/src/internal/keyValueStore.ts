...
/** @internal */
export const keyValueStoreTag = Context.GenericTag<KeyValueStore.KeyValueStore>(
  "@effect/platform/KeyValueStore"
)
...
export const make: (
  ...
  keyValueStoreTag.of({
    ...
    modifyUint8Array: (key, f) =>
      Effect.flatMap(impl.getUint8Array(key), (o) => {
        if (Option.isNone(o)) {
          return Effect.succeedNone
        }
        const newValue = f(o.value)
...
export const makeStringOnly: (
  impl: Pick<KeyValueStore.KeyValueStore, "get" | "remove" | "clear" | "size"> &
...
export const prefix = dual<
  (
    prefix: string
  ) => <S extends KeyValueStore.KeyValueStore.AnyStore>(self: S) => S,
...
const makeSchemaStore = <A, I, R>(
  ...
  const set = (key: string, value: A) =>
    Effect.flatMap(encode(value), (json) => store.set(key, json))
    ...
    get: (key: string) =>
      Effect.sync(() =>
        Option.fromNullable(store.get(key)).pipe(
          Option.map((value) =>
            typeof value === "string" ? value : Encoding.encodeBase64(value)
          )
        )
      ),
...
export const layerSchema = <A, I, R>(
  ...
  const layer = Layer.effect(
    tag,
    Effect.map(keyValueStoreTag, (store) => store.forSchema(schema))
  )
...
const storageError = (
  props: Omit<
    ConstructorParameters<typeof PlatformError.SystemError>[0],
    "reason" | "module"
  >
) =>
