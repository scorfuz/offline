...
/** @internal */
export const TypeId: ServerResponse.TypeId = Symbol.for(
  "@effect/platform/HttpServerResponse"
) as ServerResponse.TypeId
...
class ServerResponseImpl
  extends Effectable.StructuralClass<ServerResponse.HttpServerResponse>
  implements ServerResponse.HttpServerResponse
{
  ...
  [respondableSymbol](): Effect.Effect<
...
export const isServerResponse = (
  u: unknown
): u is ServerResponse.HttpServerResponse =>
...
>(
  ...
  Template.Interpolated.Context<A[number]>
> =>
  Effect.map(Effect.context<any>(), (context) =>
    stream(
...
export const json = (
  ...
  const headers = options?.headers
    ...
    : Headers.empty
  return Effect.map(
    internalBody.json(body, getContentType(options, headers)),
    (body) =>
      new ServerResponseImpl(
        options?.status ?? 200,
        options?.statusText,
...
export const schemaJson = <A, I, R>(
  ...
  > => {
    const headers = options?.headers
      ...
      : Headers.empty
    return Effect.map(
      encode(body, getContentType(options, headers)),
      (body) =>
        new ServerResponseImpl(
          options?.status ?? 200,
          options?.statusText,
...
const httpPlatform = Context.GenericTag<Platform.HttpPlatform>(
...
> =>
  Effect.flatMap(httpPlatform, (platform) =>
...
export const fileWeb = (
  ...
  options?:
    | (ServerResponse.Options.WithContent & FileSystem.StreamOptions)
    | undefined
): Effect.Effect<
  ServerResponse.HttpServerResponse,
  never,
...
>(2, (self, cookies) =>
  Effect.map(
    Cookies.setAll(self.cookies, cookies),
    (cookies) =>
      new ServerResponseImpl(
        self.status,
        self.statusText,
    ...
    case "Stream": {
      return new Response(
        Stream.toReadableStreamRuntime(
          body.stream,
          options?.runtime ?? Runtime.defaultRuntime
        ),
        {
          status: response.status,
          statusText: response.statusText,
