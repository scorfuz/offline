...
/** @internal */
export const TypeId: Router.TypeId = Symbol.for(
  "@effect/platform/HttpRouter"
) as Router.TypeId

/** @internal */
export const RouteTypeId: Router.RouteTypeId = Symbol.for(
  "@effect/platform/HttpRouter/Route"
) as Router.RouteTypeId
...
/** @internal */
export const RouteContext = Context.GenericTag<Router.RouteContext>(
  "@effect/platform/HttpRouter/RouteContext"
)

const isRouter = (u: unknown): u is Router.HttpRouter<unknown, unknown> =>
...
class RouterImpl<E = never, R = never>
  extends Effectable.StructuralClass<
...
export const toHttpApp = <E, R>(
  ...
  Effect.map(FiberRef.get(currentRouterConfig), (config) => {
    ...
    const mounts = Chunk.toReadonlyArray(self.mounts).map(
      ([path, app, options]) =>
        [
          path,
    ...
    >((fiber) => {
      ...
      if (mountsLen > 0) {
        ...
        const pathname =
          searchIndex === -1 ? request.url : request.url.slice(0, searchIndex)
        ...
        for (let i = 0; i < mountsLen; i++) {
          ...
          if (pathname === path || pathname.startsWith(path + "/")) {
            ...
            return Effect.locally(
              Effect.flatMap(
                routeContext.route.handler,
                Respondable.toResponse
              ) as App.Default<E, R>,
      ...
      context.unsafeMap.set(
        ServerRequest.ParsedSearchParams.key,
        result.searchParams
      )
      context.unsafeMap.set(
        RouteContext.key,
        new RouteContextImpl(route, result.params)
      )
      ...
      const handlerResponse = Effect.flatMap(
        route.handler,
        Respondable.toResponse
      )
      return Effect.locally(
...
class RouteImpl<E = never, R = never>
  extends Inspectable.Class
  implements Router.Route<E, R>
{
...
/** @internal */
export const empty: Router.HttpRouter<never> = new RouterImpl(
  Chunk.empty(),
  Chunk.empty()
)
...
>(
  2,
  (self, route) =>
    new RouterImpl(Chunk.append(self.routes, route) as any, self.mounts)
)
...
export const concatAll = <
  Routers extends ReadonlyArray<Router.HttpRouter<E, R>>,
  E,
  R
>(
  ...routers: Routers
  ...
  new RouterImpl(
    routers.reduce(
      (cur, acc) => Chunk.appendAll(cur, acc.routes),
      Chunk.empty<Router.Route<E, R>>()
    ),
    routers.reduce(
...
const removeTrailingSlash = (path: Router.PathInput): Router.PathInput =>
...
export const prefixAll = dual<
  ...
  ) => <E, R>(self: Router.HttpRouter<E, R>) => Router.HttpRouter<E, R>,
  <E, R>(
    self: Router.HttpRouter<E, R>,
    prefix: Router.PathInput
...
>(2, (self, prefix) => {
  ...
  return new RouterImpl(
    Chunk.map(
      ...
      (route) =>
        new RouteImpl(
          ...
          route.path === "/"
            ? prefix
            : ((prefix + route.path) as Router.PathInput),
          route.handler,
          Option.orElse(
    ...
    Chunk.map(self.mounts, ([path, app]) => [
...
>(3, (self, path, that) => concat(self, prefixAll(that, path)))
...
>(
  ...
  <E, R, E1, R1>(
    ...
    new RouterImpl<any, any>(
      self.routes,
      Chunk.append(self.mounts, [removeTrailingSlash(path), that, options])
    ) as any
)
...
>(
  ...
  (self, f) =>
    new RouterImpl<any, any>(
      ...
      Chunk.map(self.mounts, ([path, app]) => [path, f(app as any)])
    )
)
...
export const transform = dual<
  <E, R, R1, E1>(
    f: (
      self: Router.Route.Handler<E, R>
    ) => App.HttpApp<Respondable.Respondable, E1, R1>
  ) => (
    self: Router.HttpRouter<E, R>
  ) => Router.HttpRouter<E1, Router.HttpRouter.ExcludeProvided<R1>>,
  <E, R, R1, E1>(
...
>(
  ...
  (self, f) =>
    new RouterImpl<any, any>(
      ...
      Chunk.map(self.mounts, ([path, app]) => [
        path,
        Effect.flatMap(f(app as any), Respondable.toResponse)
      ])
    )
)
...
export const catchTags: {
  ...
  >(
    ...
    | Router.HttpRouter.ExcludeProvided<
      ...
      >
  >
} = dual(2, (self: Router.HttpRouter<any, any>, cases: {}) =>
  use(self, Effect.catchTags(cases))
)
...
export const Tag =
  ...
  > => {
    ...
    const TagClass_ = TagClass as any as Mutable<
      Router.HttpRouter.TagClass<Self, Name, E, R>
    >
    Object.setPrototypeOf(
      TagClass,
      Object.getPrototypeOf(Context.GenericTag<Self, any>(id))
    )
    TagClass.key = id
    ...
    TagClass_.serve = (middleware) =>
      TagClass_.unwrap(HttpServer.serve(middleware as any))
