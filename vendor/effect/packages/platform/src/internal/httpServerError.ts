...
/** @internal */
export const isServerError = (u: unknown): u is Error.HttpServerError =>
...
export const causeResponse = <E>(
  ...
  let effect: Effect.Effect<HttpServerResponse> =
    Effect.succeed(internalServerError)
  ...
  Cause.reduce(cause, void 0 as void, (_, current) => {
    const withoutInterrupt = Cause.isInterruptType(stripped)
      ? Cause.empty
      : stripped
    switch (current._tag) {
      case "Fail": {
      ...
      case "Die": {
        ...
        stripped = isResponse
          ? withoutInterrupt
          : combineCauses(withoutInterrupt, current)
        hasResponse = hasResponse || isResponse
        break
      }
      case "Interrupt": {
        isClientInterrupt =
          isClientInterrupt || current.fiberId === clientAbortFiberId
  ...
  const responseEffect =
    !hasResponse && Cause.isInterruptType(stripped)
      ? Effect.succeed(isClientInterrupt ? clientAbortError : serverAbortError)
      : effect
  const strippedCause: Cause.Cause<E> =
    !hasResponse && Cause.isInterruptType(stripped) && isClientInterrupt
  ...
  return Effect.map(responseEffect, (response) => {
    ...
    return [
      response,
      Cause.sequential(strippedCause, Cause.die(response))
    ] as const
  })
}

const combineCauses = <A = never, B = never>(
  left: Cause.Cause<A>,
  right: Cause.Cause<B>
): Cause.Cause<A | B> => {
...
export const exitResponse = <E>(
  exit: Exit.Exit<HttpServerResponse, E>
): HttpServerResponse => {
