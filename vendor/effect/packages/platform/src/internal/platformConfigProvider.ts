...
export const layerDotEnv = (
  ...
  fromDotEnv(path).pipe(Effect.map(Layer.setConfigProvider), Layer.unwrapEffect)
...
export const layerDotEnvAdd = (
  ...
  Effect.gen(function* () {
    const dotEnvConfigProvider = yield* Effect.orElseSucceed(
      fromDotEnv(path),
      () => null
    )
    ...
    if (dotEnvConfigProvider === null) {
      yield* Effect.logDebug(
        `File '${path}' not found, skipping dotenv ConfigProvider.`
      )
      return Layer.empty
    }

    const currentConfigProvider = yield* FiberRef.get(
