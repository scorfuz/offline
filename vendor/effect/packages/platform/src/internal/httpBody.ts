...
export const json = (
  body: unknown,
  contentType?: string
): Effect.Effect<Body.Uint8Array, Body.HttpBodyError> =>
  Effect.try({
...
export const jsonSchema = <A, I, R>(
  ...
  return (
    body: A,
    contentType?: string
  ): Effect.Effect<Body.Uint8Array, Body.HttpBodyError, R> =>
    Effect.flatMap(
      Effect.mapError(encode(body), (error) =>
        HttpBodyError({ _tag: "SchemaError", error })
      ),
...
/** @internal */
export const formData = (body: FormData): Body.FormData =>
  new FormDataImpl(body)

/** @internal */
export const formDataRecord = (entries: Body.FormDataInput): Body.FormData => {
  ...
  for (const [key, value] of Object.entries(entries)) {
    if (Array.isArray(value)) {
      for (const item of value) {
        ...
        formData.append(
          key,
          typeof value === "object" ? (item as any) : String(item)
        )
      }
    } else if (value != null) {
      formData.append(
        key,
        typeof value === "object" ? (value as any) : String(value)
      )
    }
  }
...
export const stream = (
  ...
  new StreamImpl(body, contentType ?? "application/octet-stream", contentLength)
