...
/** @internal */
export const TypeId: Platform.TypeId = Symbol.for(
  "@effect/platform/HttpPlatform"
) as Platform.TypeId

/** @internal */
export const tag = Context.GenericTag<Platform.HttpPlatform>(
  "@effect/platform/HttpPlatform"
)
        ...
        return pipe(
          ...
          Effect.map(({ etag, info }) => {
            ...
            if (info.mtime._tag === "Some") {
              ;(headers as any)["last-modified"] =
                info.mtime.value.toUTCString()
            }
            const contentLength =
