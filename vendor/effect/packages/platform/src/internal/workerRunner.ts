...
/** @internal */
export const PlatformRunnerTypeId: WorkerRunner.PlatformRunnerTypeId =
  Symbol.for(
...
/** @internal */
export const CloseLatch = Context.Reference<WorkerRunner.CloseLatch>()(
  "@effect/platform/WorkerRunner/CloseLatch",
  {
...
export const make = Effect.fnUntraced(function* <I, E, R, O>(
  ...
  const backing = yield* platform.start<
    Worker.Worker.Request<I>,
    Worker.Worker.Response<E>
  >(closeLatch)
  ...
  yield* backing.run(
    (portId, [id, kind, data, span]): Effect.Effect<void, WorkerError, R> => {
      if (kind === 1) {
                ...
                pipe(
                  options?.encodeOutput
                    ? Effect.provideService(
                        options.encodeOutput(input, out),
                ...
                Stream.runForEachChunk((chunk) => {
                  ...
                  return pipe(
                    Effect.forEach(chunk, (data) =>
                      options.encodeOutput!(input, data)
                    ),
            ...
            restore(effect).pipe(
              Effect.catchIf(isWorkerError, (error) =>
                backing.send(portId, [
                  id,
                  3,
                  WorkerError.encodeCause(Cause.fail(error))
                ])
              ),
...
> =>
  Effect.gen(function* () {
    ...
    const parseRequest = Schema.decodeUnknown(schema) as (
      _: unknown
    ) => Effect.Effect<A>
...
>(
  ...
  | WorkerRunner.SerializedRunner.HandlersContext<Handlers>
> =>
  Layer.scopedDiscard(makeSerialized(schema, handlers)).pipe(
    Layer.provide(layerCloseLatch)
  )
...
): Effect.Effect<void, E | WorkerError, R> =>
  Effect.scopedWith(
    Effect.fnUntraced(function* (scope) {
      const context = yield* Layer.buildWithScope(
        Layer.provideMerge(layer, layerCloseLatch),
        scope
      )
