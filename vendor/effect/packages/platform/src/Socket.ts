 ...
 * @category type ids
 */
export const CloseEventTypeId: unique symbol = Symbol.for(
  "@effect/platform/Socket/CloseEvent"
)
 ...
 * @category errors
 */
export class SocketGenericError extends TypeIdError(
  SocketErrorTypeId,
  "SocketError"
)<{
 ...
 * @category errors
 */
export class SocketCloseError extends TypeIdError(
  SocketErrorTypeId,
  "SocketError"
)<{
 ...
 * @category tags
 */
export const WebSocket: Context.Tag<WebSocket, globalThis.WebSocket> =
  Context.GenericTag("@effect/platform/Socket/WebSocket")
  ...
  fromWebSocket(
    Effect.acquireRelease(
      (typeof url === "string" ? Effect.succeed(url) : url).pipe(
        Effect.flatMap((url) =>
          Effect.map(WebSocketConstructor, (f) => f(url, options?.protocols))
        )
      ),
...
export const layerWebSocket = (
  ...
  Layer.effect(Socket, makeWebSocket(url, options))
        ...
        Effect.fnUntraced(function* (scope) {
          ...
          yield* Effect.tryPromise({
            try: async () => {
              ...
              isSocketError(cause)
                ? cause
                : new SocketGenericError({ reason: "Read", cause })
