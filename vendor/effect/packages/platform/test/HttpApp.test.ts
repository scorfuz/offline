...
describe("Http/App", () => {
  describe("toWebHandler", () => {
    ...
    test("schemaJson supports contentType option", async () => {
      const encode = HttpServerResponse.schemaJson(
        Schema.Struct({ foo: Schema.String })
      )
      const handler = HttpApp.toWebHandler(
        encode({ foo: "bar" }, { contentType: "application/merge-patch+json" })
      )
      const response = await handler(new Request("http://localhost:3000/"))
      strictEqual(
        response.headers.get("Content-Type"),
    ...
    test("cookies", async () => {
      const handler = HttpApp.toWebHandler(
        HttpServerResponse.unsafeJson({ foo: "bar" }).pipe(
          ...
          HttpServerResponse.unsafeSetCookie("test", "123", {
            secure: true,
            httpOnly: true,
            sameSite: "strict"
          })
    ...
    test("stream runtime", async () => {
      const handler = HttpApp.toWebHandlerRuntime(
        ...
        HttpServerResponse.stream(
          FiberRef.get(FiberRef.currentConcurrency).pipe(
            Stream.map(String),
            Stream.encodeText
          )
        )
