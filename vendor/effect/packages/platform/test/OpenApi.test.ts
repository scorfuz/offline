...
type Parts = {
  ...
  readonly securitySchemes?:
    | Record<string, OpenApi.OpenAPISecurityScheme>
    | undefined
...
const getSpec = (parts: Parts, options?: Options): OpenApi.OpenAPISpec => {
  const additionalPropertiesStrategy =
    (options?.additionalPropertiesStrategy ?? "strict") === "allow"
  return {
    openapi: "3.1.0",
...
describe("OpenApi", () => {
  describe("fromApi", () => {
    describe("HttpApi", () => {
      ...
      it("addHttpApi", () => {
        const anotherApi = HttpApi.make("api")
          .add(
            HttpApiGroup.make("group").add(
        ...
        const api = HttpApi.make("api")
          .add(
            HttpApiGroup.make("group1")
              .add(
                HttpApiEndpoint.get("getGroup1Root", "/").addSuccess(
                  Schema.String
                )
              )
          ...
          .add(
            HttpApiGroup.make("group2").add(
              HttpApiEndpoint.get("getB", "/b").addSuccess(Schema.String)
            )
          )
          .prefix("/prefix")
          ...
          const api = HttpApi.make("api")
            .add(
              HttpApiGroup.make("group")
                ...
                .add(
                  HttpApiEndpoint.get("getA", "/a").addSuccess(Schema.String)
                )
            )
            .addError(Schema.String)
          expectSpecPaths(api, {
              ...
              servers: [
                {
                  ...
                  description: "example",
                  variables: {
                    a: { default: "b", enum: ["c"], description: "d" }
                  }
                }
              ],
    ...
    describe("HttpGroup", () => {
      ...
      it("prefix", () => {
        const api = HttpApi.make("api").add(
          HttpApiGroup.make("group")
            .add(HttpApiEndpoint.get("getRoot", "/").addSuccess(Schema.String))
            .add(HttpApiEndpoint.get("getA", "/a").addSuccess(Schema.String))
            .prefix("/prefix")
        )
    ...
    describe("HttpApiEndpoint.get", () => {
      describe("addSuccess", () => {
        ...
        it("Class API", () => {
          ...
          const api = HttpApi.make("api").add(
            HttpApiGroup.make("group").add(
              HttpApiEndpoint.get("get", "/").addSuccess(User)
            )
          )
          expectParts(api, {
            schemas: {
        ...
        it("NoContent", () => {
          const api = HttpApi.make("api").add(
            HttpApiGroup.make("group")
              .add(
                HttpApiEndpoint.get("getRoot", "/").addSuccess(
                  HttpApiSchema.NoContent
                )
              )
              .add(
                HttpApiEndpoint.get("getA", "/a").addSuccess(
      ...
      it("setPath as template string", () => {
        ...
        const api = HttpApi.make("api").add(
          HttpApiGroup.make("group").add(
            HttpApiEndpoint.get("get")`/a/${a}/b/${b}`.addSuccess(Schema.String)
          )
        )
        expectSpecPaths(api, {
          "/a/{0}/b/{1}": {
      ...
      it("setPath as template string with HttpApiSchema.param", () => {
        ...
        const api = HttpApi.make("api").add(
          HttpApiGroup.make("group").add(
            HttpApiEndpoint.get(
              "get"
            )`/a/${HttpApiSchema.param("a", a)}/b/${HttpApiSchema.param("b", b)}`.addSuccess(
              Schema.String
            )
          )
        )
          ...
          HttpApiGroup.make("group")
            .add(
              HttpApiEndpoint.get("get", "/")
                ...
                .annotate(OpenApi.Transform, (spec) => ({
                  ...spec,
                  operationId: spec.operationId + "-transformed"
                }))
            )
            .add(
            ...
            HttpApiEndpoint.post("post", "/")
              ...
              .setPayload(
                Schema.Union(
                  ...
                  HttpApiSchema.Multipart(Schema.Struct({ c: Schema.String }))
                )
              )
          )
        )
    ...
    describe("HttpApiEndpoint.del", () => {
      it("addSuccess", () => {
        const api = HttpApi.make("api").add(
          HttpApiGroup.make("group").add(
            HttpApiEndpoint.del("del", "/").addSuccess(Schema.String)
          )
        )
        expectSpecPaths(api, {
          "/": {
