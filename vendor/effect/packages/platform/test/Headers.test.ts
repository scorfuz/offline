...
describe("Headers", () => {
  describe("Redactable", () => {
    it("one key", () => {
      ...
      const r = Inspectable.withRedactableContext(fiberRefs, () =>
        Inspectable.toStringUnknown(headers)
      )
    ...
    it.effect("logs redacted", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.log(headers).pipe(Effect.annotateLogs({ headers }))
    ...
    it.effect("logs redacted structured", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.log(headers).pipe(Effect.annotateLogs({ headers }))
        strictEqual(
          Redacted.isRedacted(messages[0].message.authorization),
          true
        )
        strictEqual(
          Redacted.isRedacted(messages[0].annotations.headers.authorization),
          true
        )
      })
    )
  ...
  describe("redact", () => {
    it("one key", () => {
      ...
      strictEqual(
        Redacted.value(redacted.authorization as Redacted.Redacted),
        "Bearer some-token"
      )
    })
    ...
    it("multiple keys", () => {
      ...
      const redacted = Headers.redact(headers, [
        "Authorization",
        "authorization",
        "X-Api-Token",
        "x-api-key"
      ...
      strictEqual(
        Redacted.value(redacted["x-api-key"] as Redacted.Redacted),
        "some-key"
      )
    })
  ...
  describe("remove", () => {
    ...
    it("multiple keys", () => {
      ...
      const removed = Headers.remove(headers, [
        "Authorization",
        "authorization",
        "X-Api-Token",
        "x-api-key"
