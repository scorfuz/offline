...
describe("HttpTraceContext", () => {
  describe("w3c", () => {
    ...
    it("should return none when traceparent header is invalid", () => {
      // Invalid format
      const invalidFormat = TraceContext.w3c(
        Headers.fromInput({
      ...
      // Invalid version
      const non00Version = TraceContext.w3c(
        Headers.fromInput({
      ...
      // x included in trace
      const resultTraceNonHex = TraceContext.w3c(
        Headers.fromInput({
      ...
      // 33 character trace
      const traceLarge = TraceContext.w3c(
        Headers.fromInput({
          traceparent:
            "00-99e04eb3282f5adee84c335ca51626daa-886b16145ac0f399-01"
        })
      )
      assertNone(traceLarge)

      // 31 character trace
      const traceSmall = TraceContext.w3c(
        Headers.fromInput({
      ...
      // x included span
      const resultSpanNonHex = TraceContext.w3c(
        Headers.fromInput({
      ...
      // 15 characters span
      const spanSmall = TraceContext.w3c(
        Headers.fromInput({
      ...
      // 17 characters span
      const spanLarge = TraceContext.w3c(
        Headers.fromInput({
