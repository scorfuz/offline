...
describe("UrlParams", () => {
  describe("makeUrl", () => {
    it.effect("makes a URL", () =>
      Effect.gen(function* () {
        const url = yield* UrlParams.makeUrl(
    ...
    it.effect("does not throw if `location` is set to `undefined`", () =>
      Effect.gen(function* () {
        ...
        let url = yield* UrlParams.makeUrl(
          "https://example.com",
          [],
          Option.none()
        )
    ...
    it.effect("does not fail if `location` is partially defined", () =>
      Effect.gen(function* () {
        ...
        const url1 = yield* UrlParams.makeUrl(
          "https://example.com",
          [],
          Option.none()
        )
        ...
        const url2 = yield* UrlParams.makeUrl(
          "https://example.com",
          [],
          Option.none()
        )
        ...
        const url3 = yield* UrlParams.makeUrl(
          "https://example.com",
          [],
          Option.none()
        )
  ...
  describe("toRecord", () => {
    it("works when empty", () => {
      deepStrictEqual(UrlParams.toRecord(UrlParams.empty), {})
    })
  ...
  describe("setAll", () => {
    it("overwrites provided keys while preserving others", () => {
      ...
      deepStrictEqual(UrlParams.toRecord(UrlParams.setAll(prev, next)), {
        baz: "c",
        foo: "a",
        bar: "b"
      })
  ...
  describe("set", () => {
    it("overwrites only the targeted key", () => {
      ...
      deepStrictEqual(UrlParams.toRecord(UrlParams.set(params, "foo", "a")), {
        baz: "c",
        foo: "a"
      })
    })
  ...
  describe("appendAll", () => {
    it("appends all entries while keeping order", () => {
      ...
      deepStrictEqual(UrlParams.toRecord(appended), {
        foo: ["a", "b"],
        bar: "c"
      })
    })
  ...
  describe("remove", () => {
    it("removes every instance of the provided key", () => {
      ...
      deepStrictEqual(UrlParams.toRecord(UrlParams.remove(params, "foo")), {
        bar: "c"
      })
    })
  })
  ...
  describe("schemaParse", () => {
    const schema = UrlParams.schemaParse(
      Schema.Struct({
