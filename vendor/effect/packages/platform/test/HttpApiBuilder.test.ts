...
const assertNormalizedUrlParams = <UrlParams extends Schema.Schema.Any>(
  schema: UrlParams &
    HttpApiEndpoint.HttpApiEndpoint.ValidateUrlParams<UrlParams>,
  params: Record<string, string | Array<string>>,
  expected: Record<string, string | Array<string>>
) => {
  deepStrictEqual(
    HttpApiBuilder.normalizeUrlParams(params, schema.ast),
    expected
  )
}
  ...
  describe("normalizeUrlParams", () => {
    describe("Property Signatures", () => {
      ...
      it("optional(NonEmptyArray(String))", () => {
        const schema = Schema.Struct({
          a: Schema.optional(Schema.NonEmptyArray(Schema.String))
        })
        assertNormalizedUrlParams(schema, { a: "a" }, { a: ["a"] })
        assertNormalizedUrlParams(schema, { a: ["a"] }, { a: ["a"] })
      })
      ...
      describe("Union", () => {
        ...
        it("Literal + Tuple", () => {
          const schema = Schema.Struct({
            a: Schema.Union(Schema.Literal("a"), Schema.Tuple(Schema.String))
          })
          assertNormalizedUrlParams(schema, { a: "a" }, { a: "a" })
          assertNormalizedUrlParams(schema, { a: ["a"] }, { a: ["a"] })
        })
        ...
        it("String + Tuple", () => {
          const schema = Schema.Struct({
            a: Schema.Union(Schema.String, Schema.Tuple(Schema.String))
          })
          assertNormalizedUrlParams(schema, { a: "a" }, { a: "a" })
          assertNormalizedUrlParams(schema, { a: ["a"] }, { a: ["a"] })
        })
    ...
    it("Refinement", () => {
      const schema = Schema.Struct({ a: Schema.Array(Schema.String) }).pipe(
        Schema.filter(() => true)
      )
      assertNormalizedUrlParams(schema, { a: "a" }, { a: ["a"] })
      assertNormalizedUrlParams(schema, { a: ["a"] }, { a: ["a"] })
    ...
    it("Transformation", () => {
      ...
      const schema = Schema.transform(struct, struct, {
        strict: true,
        decode: identity,
        encode: identity
      })
