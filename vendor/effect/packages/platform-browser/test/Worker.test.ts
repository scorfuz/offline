...
describe.sequential("Worker", () => {
  ...
  it("Serialized", () =>
    Effect.gen(function* () {
      ...
      const people = yield* pool
        .execute(new GetPersonById({ id: 123 }))
      ...
      Effect.provide(
        BrowserWorker.layer(
          () =>
            new globalThis.Worker(
              new URL("./fixtures/serializedWorker.ts", import.meta.url)
            )
        )
      ),
      Effect.runPromise
  ...
  it("tracing", () =>
    Effect.gen(function* () {
      ...
      const span = yield* pool
        .executeEffect(new GetSpan())
