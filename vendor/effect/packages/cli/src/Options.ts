...
export declare namespace All {
  ...
  export type ReturnIterable<T extends Iterable<OptionsAny>> = [T] extends [
    Iterable<Options.Variance<infer A>>
  ]
    ? Options<Array<A>>
    : never
  ...
  export type ReturnObject<T> = [T] extends [{ [K: string]: OptionsAny }]
    ? Options<{
        -readonly [K in keyof T]: [T[K]] extends [Options.Variance<infer _A>]
          ? _A
          : never
      }>
    : never
  ...
  > = [Arg] extends [ReadonlyArray<OptionsAny>]
    ? ReturnTuple<Arg>
 ...
 */
export const isOptions: (u: unknown) => u is Options<unknown> =
  InternalOptions.isOptions
...
>(
  arg: Arg
) => All.Return<Arg> = InternalOptions.all
...
export const boolean: (
  name: string,
  options?: Options.BooleanOptionsConfig
) => Options<boolean> = InternalOptions.boolean
 ...
 * @category constructors
 */
export const date: (name: string) => Options<globalThis.Date> =
  InternalOptions.date
...
export const directory: (
  name: string,
  config?: Options.PathOptionsConfig
) => Options<string> = InternalOptions.directory
...
export const file: (
  name: string,
  config?: Options.PathOptionsConfig
) => Options<string> = InternalOptions.file
...
export const fileContent: (
  name: string
) => Options<readonly [path: string, content: Uint8Array]> =
...
) => Options<unknown> = InternalOptions.fileParse
...
export const fileText: (
  name: string
) => Options<readonly [path: string, content: string]> =
  InternalOptions.fileText
 ...
 * @category constructors
 */
export const integer: (name: string) => Options<number> =
  InternalOptions.integer
 ...
 * @category combinators
 */
export const repeated: <A>(self: Options<A>) => Options<Array<A>> =
  InternalOptions.repeated
