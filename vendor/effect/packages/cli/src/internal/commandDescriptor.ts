...
/** @internal */
export type Op<Tag extends string, Body = {}> = Descriptor.Command<never> &
  Body & {
    readonly _tag: Tag
  }
...
/** @internal */
export type Instruction = Standard | GetUserInput | Map | Subcommands

/** @internal */
export interface Standard extends Op<
  "Standard",
  {
...
/** @internal */
export interface GetUserInput extends Op<
  "GetUserInput",
  {
    readonly name: string
    readonly description: HelpDoc.HelpDoc
...
/** @internal */
export interface Subcommands extends Op<
  "Subcommands",
  {
...
/** @internal */
export const isStandard = (self: Instruction): self is Standard =>
  self._tag === "Standard"
...
/** @internal */
export const isSubcommands = (self: Instruction): self is Subcommands =>
  self._tag === "Subcommands"
...
export const getBashCompletions = <A>(
  ...
  getBashCompletionsInternal(self as Instruction, executable)
...
export const getFishCompletions = <A>(
  ...
  getFishCompletionsInternal(self as Instruction, executable)
...
export const getZshCompletions = <A>(
  ...
  getZshCompletionsInternal(self as Instruction, executable)
...
export const withSubcommands = dual<
  ...
  >(
    ...
    Descriptor.Command.ComputeParsedType<
      A &
        Readonly<{
          subcommand: Option.Option<Descriptor.Command.Subcommands<Subcommands>>
        }>
    >
  >,
  <
...
>(2, (self, subcommands) => {
  ...
  op._tag = "Subcommands"
  op.parent = self
  op.children = Arr.map(subcommands, ([id, command]) =>
    map(command, (a) => [id, a])
  )
  return op
})
    ...
    case "Standard": {
      const header = InternalHelpDoc.isEmpty(self.description)
        ...
        : InternalHelpDoc.sequence(
            InternalHelpDoc.h1("DESCRIPTION"),
            self.description
          )
      ...
      return InternalHelpDoc.sequence(
        header,
        InternalHelpDoc.sequence(argsSection, optionsSection)
      )
    }
    case "GetUserInput": {
    ...
    case "Subcommands": {
      const getUsage = (
        ...
        switch (command._tag) {
          case "Standard":
          case "GetUserInput": {
            const usage = InternalHelpDoc.getSpan(
              InternalUsage.getHelp(getUsageInternal(command))
            )
          ...
          case "Subcommands": {
            ...
            return Option.match(Arr.head(parentUsage), {
              onNone: () =>
                Arr.flatMap(command.children, (child) =>
                  getUsage(child, preceding)
                ),
              onSome: ([usage]) => {
                const childrenUsage = Arr.flatMap(command.children, (child) =>
                  getUsage(child, Arr.append(preceding, usage))
                )
      ...
      const printSubcommands = (
        ...
        const maxUsageLength = Arr.reduceRight(subcommands, 0, (max, [usage]) =>
          Math.max(InternalSpan.size(usage), max)
        )
        const documents = Arr.map(subcommands, ([usage, desc]) =>
          InternalHelpDoc.p(
            InternalSpan.spans([
              usage,
              InternalSpan.text(
                " ".repeat(maxUsageLength - InternalSpan.size(usage) + 2)
              ),
              desc
            ])
          )
        ...
        throw new Error(
          "[BUG]: Subcommands.usage - received empty list of subcommands to print"
        )
      }
      return InternalHelpDoc.sequence(
        ...
        InternalHelpDoc.sequence(
          ...
          printSubcommands(
            Arr.flatMap(self.children, (child) => getUsage(child, Arr.empty()))
          )
        )
      )
    }
  ...
  const parseCommandLine = (
    ...
    Arr.matchLeft(args, {
      ...
      onNonEmpty: (head, tail) => {
        ...
        const normalizedCommandName = InternalCliConfig.normalizeCase(
          config,
          self.name
        )
        return Effect.succeed(tail).pipe(
          ...
          Effect.catchTag("NoSuchElementException", () => {
            const error = InternalHelpDoc.p(
              `Missing command name: '${self.name}'`
            )
            return Effect.fail(InternalValidationError.commandMismatch(error))
          })
        )
  ...
  switch (self._tag) {
    case "Standard": {
      ...
      > =>
        Arr.matchLeft(args, {
          ...
          onNonEmpty: (argv0) => {
            const normalizedArgv0 = InternalCliConfig.normalizeCase(
              config,
              argv0
            )
            const normalizedCommandName = InternalCliConfig.normalizeCase(
            ...
            if (normalizedArgv0 === normalizedCommandName) {
              ...
              const options = InternalBuiltInOptions.builtInOptions(
                self,
                usage,
                help
              )
              const argsWithoutCommand = Arr.drop(args, 1)
              return InternalOptions.processCommandLine(
                options,
                argsWithoutCommand,
                config
              ).pipe(
                ...
                Effect.catchTag("NoSuchElementException", () => {
                  const error = InternalHelpDoc.p(
                    "No built-in option was matched"
                  )
                  return Effect.fail(
                    InternalValidationError.noBuiltInMatch(error)
                  )
                }),
      ...
      const parseUserDefinedArgs = (
        ...
        FileSystem.FileSystem | Path.Path | Terminal.Terminal
      > =>
        parseCommandLine(self, args).pipe(
          Effect.flatMap((commandOptionsAndArgs) => {
            const [optionsAndArgs, forcedCommandArgs] = splitForcedArgs(
    ...
    case "Subcommands": {
      ...
      const helpDirectiveForParent = Effect.sync(() => {
        return InternalCommandDirective.builtIn(
      ...
      const helpDirectiveForChild = parseChildren.pipe(
        Effect.flatMap((directive) => {
          if (
            ...
            const newDirective = InternalCommandDirective.builtIn(
              InternalBuiltInOptions.showHelp(
                InternalUsage.concat(
          ...
          return Effect.fail(
            InternalValidationError.invalidArgument(InternalHelpDoc.empty)
          )
        })
      )
      const wizardDirectiveForParent = Effect.sync(() =>
        InternalCommandDirective.builtIn(
          InternalBuiltInOptions.showWizard(self)
        )
      )
          ...
          Effect.flatMap((directive) => {
            switch (directive._tag) {
              case "BuiltIn": {
                ...
                if (InternalBuiltInOptions.isShowWizard(directive.option)) {
                  return Effect.orElse(
                    wizardDirectiveForChild,
                    () => wizardDirectiveForParent
                  )
                }
                return Effect.succeed(directive)
              }
              case "UserDefined": {
                ...
                if (Arr.isNonEmptyReadonlyArray(args)) {
                  return parseChildrenWith(args).pipe(
                    Effect.flatMap((childDirective) => {
                      if (
                        !InternalCommandDirective.isUserDefined(childDirective)
                      ) {
                    ...
                    Effect.catchAll((err) => {
                      if (InternalValidationError.isCommandMismatch(err)) {
                        ...
                        return Effect.fail(
                          InternalValidationError.commandMismatch(error)
                        )
                      }
                ...
                return Effect.succeed(
                  InternalCommandDirective.userDefined(directive.leftover, {
                    ...(directive.value as any),
...
const withDescriptionInternal = (
  ...
  switch (self._tag) {
    case "Standard": {
      const helpDoc =
        typeof description === "string" ? HelpDoc.p(description) : description
      const op = Object.create(proto)
      op._tag = "Standard"
      op.name = self.name
    ...
    case "Map": {
      return mapEffect(
        withDescriptionInternal(self.command, description),
        self.f
      )
    }
    case "Subcommands": {
    ...
    switch (self._tag) {
      ...
      case "Standard": {
        return Effect.gen(function* () {
          const logCurrentCommand = Ref.get(commandLineRef).pipe(
            Effect.flatMap((commandLine) => {
              const currentCommand = InternalHelpDoc.p(
                pipe(
                  InternalSpan.strong(
                    InternalSpan.highlight("COMMAND:", Color.cyan)
                  ),
          ...
          if (isStandard(self)) {
            ...
            if (
              !InternalOptions.isEmpty(
                self.options as InternalOptions.Instruction
              )
            ) {
              ...
              const options = yield* InternalOptions.wizard(
                self.options,
                config
              )
      ...
      case "Subcommands": {
        const description = InternalHelpDoc.p(
          "Select which command you would like to execute"
        )
        ...
        return loop(self.parent, commandLineRef).pipe(
          Effect.zipRight(
            InternalSelectPrompt.select({ message, choices }).pipe(
              Effect.tap(([name]) =>
                Ref.update(commandLineRef, Arr.append(name))
              ),
              Effect.zipLeft(Console.log()),
              Effect.flatMap(([, nextIndex]) =>
                loop(self.children[nextIndex], commandLineRef)
              )
            )
          )
        )
...
const traverseCommand = <S>(
  ...
  SynchronizedRef.make(initialState).pipe(
    Effect.flatMap((ref) => {
      const loop = (
        self: Instruction,
        ...
        switch (self._tag) {
          ...
          case "Subcommands": {
            ...
            const nextParentCommands = Arr.appendAll(
              parentCommands,
              parentNames
            )
            ...
            return loop(
              ...
              Effect.zipRight(
                Effect.forEach(self.children, (child) =>
...
const getBashCompletionsInternal = (
  ...
  traverseCommand(
    ...
    (state, info) => {
      ...
      const optionNames = InternalOptions.getNames(
        options as InternalOptions.Instruction
      )
      const optionCases = isStandard(info.command)
        ? InternalOptions.getBashCompletions(
            info.command.options as InternalOptions.Instruction
          )
        : Arr.empty()
      ...
      const lines = Arr.append(state, [funcLines, cmdLines] as [
        ReadonlyArray<string>,
        ReadonlyArray<string>
      ])
...
const getFishCompletionsInternal = (
  ...
  traverseCommand(self, Arr.empty(), (state, info) => {
    ...
    const argsCompletions = isStandard(info.command)
      ? InternalArgs.getFishCompletions(
          info.command.args as InternalArgs.Instruction
        )
      : Arr.empty()
  ...
  switch (self._tag) {
    ...
    case "GetUserInput": {
      ...
      const optionCompletions = pipe(
        InternalOptions.getZshCompletions(
          options as InternalOptions.Instruction
        ),
    ...
    case "Subcommands": {
      ...
      const childLines = pipe(
        ...
        Arr.appendAll(
          Arr.isEmptyReadonlyArray(parentCommands) ? Arr.empty() : Arr.of(";;")
        )
      )
