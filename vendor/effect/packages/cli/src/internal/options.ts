...
/** @internal */
export type Op<Tag extends string, Body = {}> = Options.Options<never> &
  Body & {
    readonly _tag: Tag
  }
...
/** @internal */
export interface Single extends Op<
  "Single",
  {
...
/** @internal */
export interface KeyValueMap extends Op<
  "KeyValueMap",
  {
...
/** @internal */
export interface Map extends Op<
  "Map",
  {
...
/** @internal */
export interface Both extends Op<
  "Both",
  {
...
/** @internal */
export interface OrElse extends Op<
  "OrElse",
  {
    readonly left: Options.Options<unknown>
    readonly right: Options.Options<unknown>
...
/** @internal */
export interface WithFallback extends Op<
  "WithFallback",
  {
    readonly options: Options.Options<unknown>
...
/** @internal */
export interface Variadic extends Op<
  "Variadic",
  {
    readonly argumentOption: Single
...
/** @internal */
export interface WithDefault extends Op<
  "WithDefault",
  {
    readonly options: Options.Options<unknown>
...
/** @internal */
export const isInstruction = <_>(
  self: Options.Options<_>
): self is Instruction => self as any

/** @internal */
export const isEmpty = (self: Instruction): self is Empty =>
  self._tag === "Empty"

/** @internal */
export const isSingle = (self: Instruction): self is Single =>
  self._tag === "Single"

/** @internal */
export const isKeyValueMap = (self: Instruction): self is KeyValueMap =>
  self._tag === "KeyValueMap"
...
/** @internal */
export const isOrElse = (self: Instruction): self is OrElse =>
  self._tag === "OrElse"

/** @internal */
export const isWithDefault = (self: Instruction): self is WithDefault =>
  self._tag === "WithDefault"

/** @internal */
export const isWithFallback = (self: Instruction): self is WithFallback =>
  self._tag === "WithFallback"
...
export const all: <
  const Arg extends
    | Iterable<Options.Options<any>>
    | Record<string, Options.Options<any>>
>(
  arg: Arg
) => Options.All.Return<Arg> = function () {
...
export const choiceWithValue = <const C extends ReadonlyArray<[string, any]>>(
  ...
  makeSingle(name, Arr.empty(), InternalPrimitive.choice(choices))

/** @internal */
export const date = (name: string): Options.Options<Date> =>
  makeSingle(name, Arr.empty(), InternalPrimitive.date)
...
export const fileContent = (
  ...
  mapEffect(file(name, { exists: "yes" }), (path) =>
    Effect.mapError(InternalFiles.read(path), (msg) =>
      InternalValidationError.invalidValue(InternalHelpDoc.p(msg))
    )
  )
...
export const fileParse = (
  ...
  mapEffect(fileText(name), ([path, content]) =>
    Effect.mapError(InternalFiles.parse(path, content, format), (error) =>
      InternalValidationError.invalidValue(InternalHelpDoc.p(error))
    )
  )
...
export const fileText = (
  ...
  mapEffect(file(name, { exists: "yes" }), (path) =>
    Effect.mapError(InternalFiles.readString(path), (error) =>
      InternalValidationError.invalidValue(InternalHelpDoc.p(error))
    )
  )
...
>(3, (self, f, message) =>
  mapEffect(self, (a) =>
    Option.match(f(a), {
      onNone: () =>
        Either.left(
          InternalValidationError.invalidValue(InternalHelpDoc.p(message))
        ),
      onSome: Either.right
    })
  )
...
export const float = (name: string): Options.Options<number> =>
  makeSingle(name, Arr.empty(), InternalPrimitive.float)
...
export const keyValueMap = (
  ...
  if (!isSingle(option as Instruction)) {
    throw new Error(
      "InvalidArgumentException: only single options can be key/value maps"
    )
  } else {
...
export const text = (name: string): Options.Options<string> =>
  makeSingle(name, Arr.empty(), InternalPrimitive.text)
...
>(
  2,
  (self, times) => makeVariadic(self, Option.some(times), Option.none()) as any
)
...
>(
  2,
  (self, times) => makeVariadic(self, Option.none(), Option.some(times)) as any
)
...
export const isBool = <A>(self: Options.Options<A>): boolean =>
  isBoolInternal(self as Instruction)
...
/** @internal */
export const getIdentifier = <A>(
  self: Options.Options<A>
): Option.Option<string> => getIdentifierInternal(self as Instruction)
...
export const optional = <A>(
  self: Options.Options<A>
): Options.Options<Option.Option<A>> =>
...
export const repeated = <A>(
  self: Options.Options<A>
): Options.Options<Array<A>> => makeVariadic(self, Option.none(), Option.none())
...
const allTupled = <const T extends ArrayLike<Options.Options<any>>>(
  arg: T
): Options.Options<{
  ...
  switch (self._tag) {
    ...
    case "Single": {
      return InternalHelpDoc.descriptionList(
        Arr.of([
          InternalHelpDoc.getSpan(
            InternalUsage.getHelp(getUsageInternal(self))
          ),
    ...
    case "KeyValueMap": {
      ...
      return InternalHelpDoc.mapDescriptionList(
        ...
        (span, oldBlock) => {
          const header = InternalHelpDoc.p(
    ...
    case "Variadic": {
      ...
      return InternalHelpDoc.mapDescriptionList(help, (oldSpan, oldBlock) => {
        ...
        return [
          InternalSpan.concat(oldSpan, newSpan),
          InternalHelpDoc.sequence(oldBlock, newBlock)
        ]
      })
    }
    case "WithDefault": {
          ...
          const optionalDescription = Option.isOption(self.fallback)
            ? Option.match(self.fallback, {
                ...
                onSome: (fallbackValue) => {
                  const inspectableValue = Predicate.isObject(fallbackValue)
                    ? fallbackValue
                    : String(fallbackValue)
                  const displayValue = Inspectable.toStringUnknown(
                    inspectableValue,
                    0
                  )
                  return InternalHelpDoc.p(
                    `This setting is optional. Defaults to: ${displayValue}`
                  )
                }
              })
    ...
    case "WithFallback": {
      const helpDoc: HelpDoc.HelpDoc = Config.isConfig(self.effect)
        ...
        : InternalPrompt.isPrompt(self.effect)
          ? InternalHelpDoc.p(
              "Will prompt the user for input if this option is not provided."
            )
...
const getMinSizeInternal = (self: Instruction): number => {
  switch (self._tag) {
    ...
    case "Variadic": {
      ...
      const argumentOptionMinSize = getMinSizeInternal(
        self.argumentOption as Instruction
      )
...
const getMaxSizeInternal = (self: Instruction): number => {
  switch (self._tag) {
    ...
    case "Variadic": {
      const selfMaxSize = Option.getOrElse(
        self.max,
        () => Number.MAX_SAFE_INTEGER / 2
      )
      const optionsMaxSize = getMaxSizeInternal(
        self.argumentOption as Instruction
      )
  ...
  switch (self._tag) {
    ...
    case "Single": {
      const acceptedValues = InternalPrimitive.isBool(self.primitiveType)
        ? Option.none()
        : Option.orElse(InternalPrimitive.getChoices(self.primitiveType), () =>
    ...
    case "Variadic": {
      return InternalUsage.repeated(
        getUsageInternal(self.argumentOption as Instruction)
      )
    }
    case "WithDefault":
    case "WithFallback": {
      return InternalUsage.optional(
        getUsageInternal(self.options as Instruction)
      )
    }
  }
}
...
const makeVariadic = <A>(
  ...
  if (!isSingle(argumentOption as Instruction)) {
    throw new Error(
      "InvalidArgumentException: only single options can be variadic"
    )
  }
  const op = Object.create(proto)
  op._tag = "Variadic"
...
const toParseableInstruction = (
  self: Instruction
): Array<ParseableInstruction> => {
  switch (self._tag) {
    case "Empty": {
      return Arr.empty()
    }
...
> => {
  switch (self._tag) {
    ...
    case "Single": {
      const singleNames = Arr.filterMap(getNames(self), (name) =>
        HashMap.get(args, name)
      )
      if (Arr.isNonEmptyReadonlyArray(singleNames)) {
        ...
        if (Arr.isEmptyReadonlyArray(tail)) {
          if (Arr.isEmptyReadonlyArray(head)) {
            return InternalPrimitive.validate(
              self.primitiveType,
              Option.none(),
              config
            ).pipe(
              Effect.mapError((e) =>
                InternalValidationError.invalidValue(InternalHelpDoc.p(e))
              )
            )
          }
          if (
          ...
          return Effect.fail(
            InternalValidationError.multipleValuesDetected(
              InternalHelpDoc.empty,
              head
            )
          )
      ...
      const error = InternalHelpDoc.p(
        `Expected to find option: '${self.fullName}'`
      )
    ...
    case "KeyValueMap": {
      const extractKeyValue = (
        ...
        if (
          Arr.isNonEmptyReadonlyArray(split) &&
          split.length === 2 &&
          split[1] !== ""
        ) {
        ...
        const error = InternalHelpDoc.p(
          `Expected a key/value pair but received '${value}'`
        )
        return Effect.fail(InternalValidationError.invalidArgument(error))
      }
      return parseInternal(self.argumentOption, args, config).pipe(
        Effect.matchEffect({
          onFailure: (e) =>
            InternalValidationError.isMultipleValuesDetected(e)
              ...
              : Effect.fail(e),
          onSuccess: (kv) =>
            extractKeyValue(kv as string).pipe(Effect.map(HashMap.make))
        })
      )
    }
    case "Map": {
            ...
            parseInternal(self.right as Instruction, args, config).pipe(
              Effect.matchEffect({
                ...
                onSuccess: () => {
                  ...
                  return Effect.fail(
                    InternalValidationError.invalidValue(error)
                  )
                }
              })
            )
    ...
    case "Variadic": {
      ...
      const matchedArgument = Arr.filterMap(getNames(self), (name) =>
        HashMap.get(args, name)
      )
        ...
        Effect.catchTag("MissingValue", (e) =>
          self.effect.pipe(
            Effect.catchAll((e2) => {
              ...
              if (
                ConfigError.isConfigError(e2) &&
                !ConfigError.isMissingDataOnly(e2)
              ) {
  ...
  switch (self._tag) {
    ...
    case "KeyValueMap": {
      ...
      return InternalListPrompt.list({
        ...
        Effect.flatMap((args) => {
          const identifier = Option.getOrElse(
            getIdentifierInternal(self),
            () => ""
          )
          return parseInternal(
            self,
            HashMap.make([identifier, args]),
            config
          ).pipe(Effect.as(Arr.prepend(args, identifier)))
        }),
        Effect.zipLeft(Console.log())
      )
    }
    ...
    case "OrElse": {
      ...
      const choices = Arr.getSomes([
        Option.map(getIdentifierInternal(self.left as Instruction), (title) =>
          makeChoice(title, self.left as Instruction)
        ),
        Option.map(getIdentifierInternal(self.right as Instruction), (title) =>
          makeChoice(title, self.right as Instruction)
        )
      ])
            ...
            : Ref.make(Arr.empty<string>()).pipe(
                Effect.flatMap((ref) =>
                  wizardInternal(
                    ...
                    Effect.flatMap((args) =>
                      Ref.update(ref, Arr.appendAll(args))
                    ),
                    Effect.repeatN(n - 1),
    ...
    case "WithDefault": {
      ...
      const defaultHelp = InternalHelpDoc.p(
        `This option is optional - use the default?`
      )
      const message = pipe(
        getHelpInternal(self.options as Instruction),
    ...
    case "WithFallback": {
      ...
      const defaultHelp = InternalHelpDoc.p(
        `Try load this option from the environment?`
      )
      const message = pipe(
        getHelpInternal(self.options as Instruction),
        InternalHelpDoc.sequence(defaultHelp)
    ...
    onNonEmpty: (head, tail) =>
      parseCommandLine(head, input, config).pipe(
        Effect.flatMap(({ leftover, parsed }) =>
          Option.match(parsed, {
            onNone: () =>
              findOptions(leftover, tail, config).pipe(
                Effect.map(
                  ([nextArgs, nextOptions, map]) =>
...
const parseCommandLine = (
  ...
  switch (self._tag) {
        ...
        Effect.flatMap((args) =>
          Arr.matchLeft(args, {
            ...
            onNonEmpty: (head, tail) => {
              ...
              const normalizedNames = Arr.map(getNames(self), (name) =>
                normalize(name)
              )
              ...
              if (Arr.contains(normalizedNames, normalizedHead)) {
                  ...
                  return Arr.matchLeft(tail, {
                    ...
                    onNonEmpty: (value, leftover) => {
                      if (InternalPrimitive.isTrueValue(value)) {
                        ...
                        return Effect.succeed<ParsedCommandLine>({
                          parsed,
                          leftover
                        })
                      }
                ...
                return Arr.matchLeft(tail, {
                  onEmpty: () => {
                    ...
                    return Effect.fail(
                      InternalValidationError.missingValue(error)
                    )
                  },
                  onNonEmpty: (value, leftover) => {
                    const parsed = Option.some({
              ...
              if (head.startsWith("-")) {
                if (
                  ...
                  return Effect.fail(
                    InternalValidationError.correctedFlag(error)
                  )
                }
                const error = InternalHelpDoc.p(
                  `Expected to find option: '${self.fullName}'`
              ...
              const optionName =
                equalsValue !== undefined ? FLAG_REGEX.exec(rawArg)![1] : rawArg
    ...
    case "KeyValueMap": {
      const normalizedNames = Arr.map(getNames(self.argumentOption), (name) =>
        InternalCliConfig.normalizeCase(config, name)
      )
      return Arr.matchLeft(args, {
        onEmpty: () =>
          Effect.succeed<ParsedCommandLine>({
            parsed: Option.none(),
            leftover: args
          }),
        onNonEmpty: (head, tail) => {
          const loop = (
            ...
            while (Arr.isNonEmptyReadonlyArray(leftover)) {
              ...
              const normalizedName = InternalCliConfig.normalizeCase(
                config,
                name
              )
              ...
              if (
                leftover.length >= 2 &&
                Arr.contains(normalizedNames, normalizedName)
              ) {
          ...
          if (Arr.contains(normalizedNames, normalizedName)) {
            ...
            return Effect.succeed({
              parsed: Option.some({ name: head, values }),
              leftover
            })
          }
          ...
          return Effect.succeed({
            parsed: Option.some({ name: optionName, values }),
            leftover
          })
        }
      })
...
const merge = (
  ...
  if (Arr.isNonEmptyReadonlyArray(map2)) {
    ...
    const newMap = Option.match(HashMap.get(map1, head[0]), {
      ...
      onSome: (elems) =>
        HashMap.set(map1, head[0], Arr.appendAll(elems, head[1]))
    })
...
/** @internal */
export const getBashCompletions = (
  self: Instruction
): ReadonlyArray<string> => {
  switch (self._tag) {
    case "Empty": {
...
export const getFishCompletions = (self: Instruction): Array<string> => {
  switch (self._tag) {
    ...
    case "Single": {
      ...
      const order = Order.mapInput(
        Order.boolean,
        (tuple: readonly [boolean, string]) => !tuple[0]
      )
      return pipe(
        ...
        Arr.appendAll(
          description.length === 0 ? Arr.empty() : Arr.of(`-d '${description}'`)
        ),
        Arr.join(" "),
        Arr.of
      )
  ...
  switch (self._tag) {
    ...
    case "Single": {
      ...
      return Arr.map(
        names,
        (name) =>
          `${conflicts}${multiple}${name}[${escape(description)}]${possibleValues}`
      )
    }
    case "KeyValueMap": {
      return getZshCompletions(self.argumentOption as Instruction, {
        ...state,
        multiple: true
      })
    }
    ...
    case "OrElse": {
      ...
      const left = getZshCompletions(self.left as Instruction, {
        ...state,
        conflicts: Arr.appendAll(state.conflicts, rightNames)
      })
      const right = getZshCompletions(self.right as Instruction, {
        ...state,
    ...
    case "Variadic": {
      return Option.isSome(self.max) && self.max.value > 1
        ? getZshCompletions(self.argumentOption as Instruction, {
            ...state,
            multiple: true
          })
