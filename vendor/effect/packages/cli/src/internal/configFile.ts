...
> =>
  Effect.gen(function* () {
    ...
    const searchPaths =
      options?.searchPaths && options.searchPaths.length
        ? options.searchPaths
        : ["."]
    const extensions =
      options?.formats && options.formats.length
        ? options.formats.flatMap((_) => fileExtensions[_])
        : allFileExtensions
    const filePaths = yield* Effect.filter(
      searchPaths.flatMap((searchPath) =>
        extensions.map((ext) => path.join(searchPath, `${fileName}.${ext}`))
      ),
    ...
    const providers = yield* Effect.forEach(filePaths, (path) =>
      pipe(
        ...
        Effect.mapError((_) =>
          ConfigFileError(`Could not read file (${path})`)
        ),
        Effect.flatMap((content) =>
          Effect.mapError(InternalFiles.parse(path, content), (message) =>
            ConfigFileError(message)
          )
        ),
    ...
    return providers.reduce((acc, provider) =>
      ConfigProvider.orElse(acc, () => provider)
    )
  })
      ...
      Layer.fiberRefLocallyScopedWith(
        ...
        (services) => {
          ...
          return Context.add(
            services,
            ConfigProvider.ConfigProvider,
            ConfigProvider.orElse(current, () => provider)
          )
        }
      )
    ),
...
const ConfigFileErrorProto = Object.assign(
  Object.create(Cause.YieldableError.prototype),
  {
