...
interface FileOptions extends Required<
  Omit<Prompt.Prompt.FileOptions, "startingPath">
> {
...
function handleClear(options: FileOptions) {
  return (state: State, _: Prompt.Prompt.Action<State, string>) => {
    return Effect.gen(function* () {
      ...
      const message = showConfirmation(state.confirm)
        ? CONFIRM_MESSAGE
        : options.message
...
function renderPrompt(
  ...
  const annotateLine = (line: string): Doc.AnsiDoc =>
    Doc.annotate(Doc.text(line), Ansi.bold)
...
function renderNextFrame(state: State, options: FileOptions) {
  return Effect.gen(function* () {
    ...
    if (showConfirmation(state.confirm)) {
      ...
      const trailingSymbol = Doc.annotate(
...
export const file = (
  ...
  const initialState: Effect.Effect<State, never, Prompt.Prompt.Environment> =
    Effect.gen(function* () {
