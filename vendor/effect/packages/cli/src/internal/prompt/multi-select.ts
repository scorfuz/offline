  ...
  for (let index = toDisplay.startIndex; index < toDisplay.endIndex; index++) {
    ...
    if (index < metaOptions.length) {
      ...
      const description = renderChoiceDescription(
        choice as Prompt.Prompt.SelectChoice<A>,
        isHighlighted
      )
      documents.push(
        prefix.pipe(
...
function processSpace<A>(state: State, options: SelectOptions<A>) {
...
export function handleClear<A>(options: SelectOptions<A>) {
  return Effect.gen(function* () {
    ...
    const text =
      "\n".repeat(Math.min(options.choices.length + 2, options.maxPerPage)) +
      options.message +
      1
...
export const multiSelect = <const A>(
  ...
  return InternalPrompt.custom(
    { index: 0, selectedIndices: initialSelected, error: Option.none() },
    {
