...
/** @internal */
type Op<Tag extends string, Body = {}> = Prompt.Prompt<never> &
  Body & {
    readonly _tag: Tag
  }
...
/** @internal */
export interface Loop extends Op<
  "Loop",
  {
    readonly initialState:
      | unknown
      | Effect.Effect<unknown, never, Prompt.Prompt.Environment>
...
const allTupled = <const T extends ArrayLike<Prompt.Prompt<any>>>(
  arg: T
): Prompt.Prompt<{
...
export const all: <
  const Arg extends
    | Iterable<Prompt.Prompt<any>>
    | Record<string, Prompt.Prompt<any>>
>(
  arg: Arg
) => Prompt.All.Return<Arg> = function () {
  if (arguments.length === 1) {
    if (isPrompt(arguments[0])) {
      ...
      const entries = Object.entries(
        arguments[0] as Readonly<{ [K: string]: Prompt.Prompt<any> }>
      )
...
const runLoop = Effect.fnUntraced(
  function* (
    ...
    let state = Effect.isEffect(loop.initialState)
      ...
      : loop.initialState
    let action: Prompt.Prompt.Action<unknown, unknown> = Action.NextFrame({
      state
    })
    while (true) {
