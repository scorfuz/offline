...
const render = (
  ...
  switch (self._tag) {
    ...
    case "Named": {
      const typeInfo = config.showTypes
        ? Option.match(self.acceptedValues, {
            ...
            onSome: (s) =>
              InternalSpan.concat(InternalSpan.space, InternalSpan.text(s))
          })
        : InternalSpan.empty
    ...
    case "Repeated": {
      return Arr.map(render(self.usage, config), (span) =>
        InternalSpan.concat(span, InternalSpan.text("..."))
      )
    }
    case "Alternation": {
      if (
      ...
      return Arr.flatMap(render(self.left, config), (left) =>
        Arr.map(render(self.right, config), (right) =>
          InternalSpan.spans([left, InternalSpan.text("|"), right])
        )
      )
    }
    case "Concat": {
      ...
      const separator =
        Arr.isNonEmptyReadonlyArray(leftSpan) &&
        Arr.isNonEmptyReadonlyArray(rightSpan)
          ? InternalSpan.space
          : InternalSpan.empty
      return Arr.flatMap(leftSpan, (left) =>
        Arr.map(rightSpan, (right) =>
          InternalSpan.spans([left, separator, right])
        )
      )
    }
  }
