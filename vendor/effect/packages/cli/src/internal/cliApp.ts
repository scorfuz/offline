...
/** @internal */
export const make = <A>(
  config: CliApp.CliApp.ConstructorArgs<A>
): CliApp.CliApp<A> => {
                  ...
                  return Arr.matchLeft(directive.leftover, {
                    ...
                    onNonEmpty: (head) => {
                      ...
                      return Effect.zipRight(
                        printDocs(error),
                        Effect.fail(InternalValidationError.invalidValue(error))
                      )
                    }
                  })
                }
...
const printDocs = (error: HelpDoc.HelpDoc): Effect.Effect<void> =>
  Console.error(InternalHelpDoc.toAnsiText(error))
    ...
    case "ShowHelp": {
      ...
      const usage = InternalHelpDoc.sequence(
        ...
        pipe(
          ...
          Arr.map((span) =>
            InternalHelpDoc.p(
              InternalSpan.concat(InternalSpan.text("$ "), span)
            )
          ),
          Arr.reduceRight(InternalHelpDoc.empty, (left, right) =>
            InternalHelpDoc.sequence(left, right)
          )
        )
      )
      const helpDoc = pipe(
    ...
    case "ShowWizard": {
      ...
      const instructions = InternalHelpDoc.sequence(
        InternalHelpDoc.p(
          InternalSpan.spans([
            InternalSpan.text(
              "The wizard mode will assist you with constructing commands for"
            ),
            InternalSpan.space,
      ...
      const description = InternalHelpDoc.descriptionList([
        [InternalSpan.text("Instructions"), instructions]
      ])
      ...
      return Console.log(text).pipe(
        Effect.zipRight(
          InternalCommand.wizard(builtIn.command, wizardPrefix, config)
        ),
        Effect.tap((args) =>
          Console.log(InternalHelpDoc.toAnsiText(renderWizardArgs(args)))
        ),
        Effect.flatMap((args) =>
          InternalTogglePrompt.toggle({
            ...
            Effect.flatMap((shouldRunCommand) => {
              // Use first 2 elements from originalArgs (runtime + script) to preserve paths with spaces
              ...
              return shouldRunCommand
                ? Console.log().pipe(
                    Effect.zipRight(run(self, finalArgs, execute))
                  )
                : Effect.void
            })
          )
...
const prefixCommand = <A>(self: Command.Command<A>): ReadonlyArray<string> => {
  let command: InternalCommand.Instruction | undefined =
    self as InternalCommand.Instruction
...
const renderWizardArgs = (args: ReadonlyArray<string>) => {
  ...
  return InternalHelpDoc.blocks([
    InternalHelpDoc.p(
      InternalSpan.strong(InternalSpan.code("Wizard Mode Complete!"))
    ),
