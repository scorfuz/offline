...
/** @internal */
export type Op<Tag extends string, Body = {}> = Primitive.Primitive<never> &
  Body & {
    readonly _tag: Tag
  }
...
/** @internal */
export interface Bool extends Op<
  "Bool",
  {
...
/** @internal */
export interface Choice extends Op<
  "Choice",
  {
...
/** @internal */
export interface Path extends Op<
  "Path",
  {
...
/** @internal */
export const isBoolType = (self: Instruction): self is Bool =>
  self._tag === "Bool"

/** @internal */
export const isChoiceType = (self: Instruction): self is Choice =>
  self._tag === "Choice"

/** @internal */
export const isDateTimeType = (self: Instruction): self is DateTime =>
  self._tag === "DateTime"

/** @internal */
export const isFloatType = (self: Instruction): self is Float =>
  self._tag === "Float"

/** @internal */
export const isIntegerType = (self: Instruction): self is Integer =>
  self._tag === "Integer"

/** @internal */
export const isPathType = (self: Instruction): self is Path =>
  self._tag === "Path"

/** @internal */
export const isSecretType = (self: Instruction): self is Path =>
  self._tag === "Path"

/** @internal */
export const isTextType = (self: Instruction): self is Text =>
  self._tag === "Text"
...
/** @internal */
export const getChoices = <A>(
  self: Primitive.Primitive<A>
): Option.Option<string> => getChoicesInternal(self as Instruction)
...
const validateInternal = (
  ...
  switch (self._tag) {
    case "Bool": {
      return Option.map(value, (str) =>
        InternalCliConfig.normalizeCase(config, str)
      ).pipe(
        Option.match({
          onNone: () =>
            Effect.orElseFail(
          ...
          onSome: (value) =>
            isTrueValue(value)
              ...
              : isFalseValue(value)
                ...
                : Effect.fail(
                    `Unable to recognize '${value}' as a valid boolean`
                  )
        })
      )
    }
    case "Choice": {
      return Effect.orElseFail(
        ...
        Effect.flatMap((value) =>
          Arr.findFirst(self.alternatives, ([choice]) => choice === value)
        ),
        Effect.mapBoth({
          onFailure: () => {
            const choices = pipe(
    ...
    case "DateTime": {
      return attempt(
        ...
        Schema.decodeUnknown(Schema.Date)
      )
    }
    case "Float": {
      return attempt(
        value,
        getTypeNameInternal(self),
        Schema.decodeUnknown(Schema.NumberFromString)
      )
    }
    case "Integer": {
      ...
      return attempt(
        value,
        getTypeNameInternal(self),
        Schema.decodeUnknown(intFromString)
      )
    }
    case "Path": {
    ...
    case "Redacted": {
      ...
      ).pipe(Effect.map((value) => EffectRedacted.make(value)))
    }
    case "Secret": {
      return attempt(
        value,
        getTypeNameInternal(self),
        Schema.decodeUnknown(Schema.String)
      ).pipe(Effect.map((value) => EffectSecret.fromString(value)))
    }
    case "Text": {
      return attempt(
        value,
...
const attempt = <E, A>(
  ...
  Effect.orElseFail(
    ...
    Effect.flatMap((value) =>
      Effect.orElseFail(parse(value), () => `'${value}' is not a ${typeName}`)
    )
  )
...
const wizardInternal = (
  ...
  switch (self._tag) {
    ...
    case "Redacted": {
      const primitiveHelp = InternalHelpDoc.p(
        "Enter some text (value will be redacted)"
      )
      const message = InternalHelpDoc.sequence(help, primitiveHelp)
