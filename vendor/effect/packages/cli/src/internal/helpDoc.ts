...
): helpDoc is HelpDoc.Paragraph => helpDoc._tag === "Paragraph"

/** @internal */
export const isDescriptionList = (
  helpDoc: HelpDoc.HelpDoc
): helpDoc is HelpDoc.DescriptionList => helpDoc._tag === "DescriptionList"

/** @internal */
export const isEnumeration = (
  helpDoc: HelpDoc.HelpDoc
): helpDoc is HelpDoc.Enumeration => helpDoc._tag === "Enumeration"

/** @internal */
export const isSequence = (
  helpDoc: HelpDoc.HelpDoc
): helpDoc is HelpDoc.Sequence => helpDoc._tag === "Sequence"
...
/** @internal */
export const blocks = (
  helpDocs: Iterable<HelpDoc.HelpDoc>
): HelpDoc.HelpDoc => {
...
const toAnsiDocInternal = (self: HelpDoc.HelpDoc): Doc.AnsiDoc => {
  switch (self._tag) {
    ...
    case "Enumeration": {
      const elements = self.elements.map((doc) =>
        Doc.cat(Doc.text("- "), toAnsiDocInternal(doc))
      )
