...
const parseConfig = (
  ...
  function parseValue(
    value:
      ...
      | Options.Options<any>
      | ReadonlyArray<
          Args.Args<any> | Options.Options<any> | Command.Command.Config
        >
...
const getDescriptor = <Name extends string, R, E, A>(
...
const makeDerive = <Name extends string, R, E, A>(
  ...
  command.transform = options.transform
    ? (effect: Effect.Effect<void, E, R>, opts: A) =>
...
const makeDescriptor = <const Config extends Command.Command.Config>(
  ...
  return InternalDescriptor.map(
    InternalDescriptor.make(
      name,
      InternalOptions.all(options),
      InternalArgs.all(args)
    ),
...
/** @internal */
export const make: {
  <Name extends string>(name: Name): Command.Command<Name, never, never, {}>
  ...
  <Name extends string, const Config extends Command.Command.Config, R, E>(
    ...
    config: Config,
    handler: (
      _: Types.Simplify<Command.Command.ParseConfig<Config>>
...
>(2, (self, subcommands) => {
  ...
  function handler(args: {
    ...
    readonly subcommand: Option.Option<
      readonly [id: Context.Tag<any, any>, value: unknown]
    >
  }) {
    if (args.subcommand._tag === "Some") {
      const [tag, value] = args.subcommand.value
      const subcommand = subcommandMap.get(tag)!
      const subcommandEffect = subcommand.transform(
        subcommand.handler(value),
        value
      )
      return Effect.provideService(subcommandEffect, self.tag, args as any)
    }
