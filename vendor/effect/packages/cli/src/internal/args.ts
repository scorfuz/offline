...
/** @internal */
export type Op<Tag extends string, Body = {}> = Args.Args<never> &
  Body & {
    readonly _tag: Tag
  }
...
/** @internal */
export interface WithFallbackConfig extends Op<
  "WithFallbackConfig",
  {
    readonly args: Args.Args<unknown>
...
/** @internal */
export const isInstruction = <_>(self: Args.Args<_>): self is Instruction =>
  self as any
...
/** @internal */
export const isVariadic = (self: Instruction): self is Variadic =>
  self._tag === "Variadic"
...
/** @internal */
export const isWithFallbackConfig = (
  self: Instruction
): self is WithFallbackConfig => self._tag === "WithFallbackConfig"
  ...
  if (arguments.length === 1) {
    if (isArgs(arguments[0])) {
      ...
      const entries = Object.entries(
        arguments[0] as Readonly<{ [K: string]: Args.Args<any> }>
      )
...
export const boolean = (
  ...
  makeSingle(
    ...
    InternalPrimitive.boolean(Option.none())
  )
...
export const choice = <A>(
  ...
  makeSingle(
    Option.fromNullable(config?.name),
    InternalPrimitive.choice(choices)
  )

/** @internal */
export const date = (
  config?: Args.Args.BaseArgsConfig
): Args.Args<globalThis.Date> =>
...
export const fileContent = (
  ...
  mapEffect(file({ ...config, exists: "yes" }), (path) =>
    Effect.mapError(InternalFiles.read(path), (e) => InternalHelpDoc.p(e))
  )
...
export const fileParse = (
  ...
  mapEffect(fileText(config), ([path, content]) =>
    Effect.mapError(InternalFiles.parse(path, content, config?.format), (e) =>
      InternalHelpDoc.p(e)
    )
  )
...
export const fileText = (
  ...
  mapEffect(file({ ...config, exists: "yes" }), (path) =>
    Effect.mapError(InternalFiles.readString(path), (e) => InternalHelpDoc.p(e))
  )
...
export const redacted = (
  config?: Args.Args.BaseArgsConfig
): Args.Args<Redacted.Redacted> =>
  makeSingle(Option.fromNullable(config?.name), InternalPrimitive.redacted)
...
/** @internal */
export const getMinSize = <A>(self: Args.Args<A>): number =>
  getMinSizeInternal(self as Instruction)

/** @internal */
export const getMaxSize = <A>(self: Args.Args<A>): number =>
...
/** @internal */
export const repeated = <A>(self: Args.Args<A>): Args.Args<Array<A>> =>
...
const allTupled = <const T extends ArrayLike<Args.Args<any>>>(
  arg: T
): Args.Args<{
...
const getMaxSizeInternal = (self: Instruction): number => {
  switch (self._tag) {
    ...
    case "Variadic": {
      ...
      return Math.floor(
        Option.getOrElse(self.max, () => Number.MAX_SAFE_INTEGER / 2) *
          argsMaxSize
      )
    }
  }
      ...
      return validateInternal(self.args as Instruction, args, config).pipe(
        Effect.flatMap(([leftover, a]) =>
          Effect.matchEffect(self.f(a), {
            onFailure: (doc) =>
              Effect.fail(InternalValidationError.invalidArgument(doc)),
...
const withDescriptionInternal = (
  ...
  switch (self._tag) {
    ...
    case "Single": {
      const desc = InternalHelpDoc.sequence(
        self.description,
        InternalHelpDoc.p(description)
      )
  ...
  switch (self._tag) {
    ...
    case "Map": {
      return wizardInternal(self.args as Instruction, config).pipe(
        Effect.tap((args) =>
          validateInternal(self.args as Instruction, args, config)
        )
      )
    }
    case "Both": {
    ...
    case "WithDefault": {
      const defaultHelp = InternalHelpDoc.p(
        `This argument is optional - use the default?`
      )
      const message = pipe(
        getHelpInternal(self.args as Instruction),
  ...
  switch (self._tag) {
    ...
    case "Single": {
      ...
      return possibleValues.length === 0
        ? Arr.empty()
        : Arr.of(
            `${multiple}${optional}${self.name}${description}${possibleValues}`
          )
    }
    case "Map": {
    ...
    case "Variadic": {
      return Option.isSome(self.max) && self.max.value > 1
        ? getZshCompletions(self.args as Instruction, {
            ...state,
            multiple: true
          })
    ...
    case "WithDefault":
    case "WithFallbackConfig": {
      return getZshCompletions(self.args as Instruction, {
        ...state,
        optional: true
      })
    }
