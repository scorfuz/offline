...
describe("Primitive", () => {
  describe("Bool", () => {
    it("validates that truthy text representations of a boolean return true", () =>
      fc.assert(
        fc.asyncProperty(trueValuesArb, (str) =>
          Effect.gen(function* () {
    ...
    it("validates that falsy text representations of a boolean return false", () =>
      fc.assert(
        fc.asyncProperty(falseValuesArb, (str) =>
          Effect.gen(function* () {
            const bool = Primitive.boolean(Option.none())
            const result = yield* Primitive.validate(
              bool,
    ...
    it("validates that invalid boolean representations are rejected", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.flip(
          Primitive.validate(bool, Option.some("bad"), CliConfig.defaultConfig)
        )
    ...
    it("validates that the default value will be used if a value is not provided", () =>
      fc.assert(
        fc.asyncProperty(fc.boolean(), (value) =>
          Effect.gen(function* () {
  ...
  describe("Choice", () => {
    ...
    it("does not validate a choice that is not one of the alternatives", () =>
      fc.assert(
        fc.asyncProperty(pairsArb, ([tuple, pairs]) =>
          Effect.gen(function* () {
  ...
  describe("Text", () => {
    it("validates all user-defined text", () =>
      fc.assert(
        fc.asyncProperty(fc.string(), (str) =>
          Effect.gen(function* () {
            const result = yield* Primitive.validate(
              Primitive.text,
              Option.some(str),
  ...
  describe(`${primitiveTypeName}`, () => {
    ...
    it(`validates that invalid values are rejected`, () =>
      Effect.gen(function* () {
        ...
        expect(result).toBe(
          `'bad' is not a ${Primitive.getTypeName(primitive)}`
        )
      }).pipe(runEffect))
  })
}
...
const trueValuesArb = fc
  .constantFrom("true", "1", "y", "yes", "on")
  .map(randomizeCharacterCases)
const falseValuesArb = fc
  .constantFrom("false", "0", "n", "no", "off")
  .map(randomizeCharacterCases)

const pairsArb = fc
  .array(fc.tuple(fc.string(), fc.float()), { minLength: 2, maxLength: 100 })
