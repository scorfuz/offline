...
describe("Command", () => {
  describe("Standard Commands", () => {
    it("should validate a command with options followed by arguments", () =>
      Effect.gen(function* () {
        ...
        const result1 = yield* Descriptor.parse(
          Tail.command,
          args1,
          CliConfig.defaultConfig
        )
        const result2 = yield* Descriptor.parse(
          Grep.command,
          args2,
          CliConfig.defaultConfig
        )
        ...
        expect(result1).toEqual(
          CommandDirective.userDefined(Array.empty(), expected1)
        )
        expect(result2).toEqual(
          CommandDirective.userDefined(Array.empty(), expected2)
        )
      }).pipe(runEffect))

    it("should provide auto-correct suggestions for misspelled options", () =>
      Effect.gen(function* () {
        const args1 = Array.make(
          "grep",
          "--afte",
          "2",
          "--before",
        ...
        const result1 = yield* Effect.flip(
          Descriptor.parse(Grep.command, args1, CliConfig.defaultConfig)
        )
        const result2 = yield* Effect.flip(
          Descriptor.parse(Grep.command, args2, CliConfig.defaultConfig)
        )
        const result3 = yield* Effect.flip(
          Descriptor.parse(Grep.command, args3, CliConfig.defaultConfig)
        )
        expect(result1).toEqual(
    ...
    it("should return an error if an option is missing", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.flip(
          Descriptor.parse(Grep.command, args, CliConfig.defaultConfig)
        )
  ...
  describe("Commands with Clustered Options", () => {
    it("should treat clustered boolean options as un-clustered options", () =>
      Effect.gen(function* () {
        ...
        expect(result1).toEqual(
          CommandDirective.userDefined(Array.empty(), expected)
        )
        expect(result2).toEqual(
          CommandDirective.userDefined(Array.empty(), expected)
        )
    ...
    it("should not uncluster wrong clusters", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Descriptor.parse(
          WordCount.command,
          args,
          CliConfig.defaultConfig
        )
  ...
  describe("Subcommands without Options or Arguments", () => {
      ...
      Effect.gen(function* () {
        ...
        const expected = {
          ...
          subcommand: Option.some([
            "remote",
            { name: "remote", options: void 0, args: void 0 }
          ])
        }
        expect(result).toEqual(
          CommandDirective.userDefined(Array.empty(), expected)
    ...
    it("matches the second subcommand without any surplus arguments", () =>
      Effect.gen(function* () {
        ...
        const expected = {
          ...
          subcommand: Option.some([
            "log",
            { name: "log", options: void 0, args: void 0 }
          ])
        }
        expect(result).toEqual(
          CommandDirective.userDefined(Array.empty(), expected)
    ...
    it("should return an error message for an unknown subcommand", () =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.flip(
          Descriptor.parse(git, args, CliConfig.defaultConfig)
        )
        expect(result).toEqual(
          ValidationError.commandMismatch(
            HelpDoc.p("Invalid subcommand for git - use one of 'remote', 'log'")
          )
        )
      }).pipe(runEffect))
  })
        ...
        [
          ...
          Descriptor.make("sub").pipe(
            Descriptor.withSubcommands([
              [
                "subsub",
                Descriptor.make("subsub", Options.boolean("i"), Args.text())
              ]
            ])
          )
        ]
  ...
  describe("Help Documentation", () => {
    it("should allow adding help documentation to a command", () =>
      Effect.gen(function* () {
        ...
        expect(result).toEqual(
          CommandDirective.userDefined(Array.empty(), expectedValue)
        )
    ...
    it("should allow adding help documentation to subcommands", () => {
      ...
      const expected = HelpDoc.sequence(
        HelpDoc.h1("DESCRIPTION"),
        HelpDoc.p("this is some help")
      )
  ...
  describe("Built-In Options Processing", () => {
    ...
    const directiveType = <A>(
      directive: CommandDirective.CommandDirective<A>
    ): string => {
  ...
  describe("End of Command Options Symbol", () => {
    ...
    it("should properly handle the end of command options symbol", () =>
      Effect.gen(function* () {
        ...
        const result3 = yield* Descriptor.parse(
          command,
          args3,
          CliConfig.defaultConfig
        )
        ...
        expect(result3).toEqual(
          CommandDirective.userDefined(Array.empty(), expected3)
        )
      }).pipe(runEffect))
  })
  ...
  describe("Completions", () => {
    ...
    it("should create completions for the bash shell", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() =>
          expect(result).toMatchFileSnapshot("./snapshots/bash-completions")
        )
      }).pipe(runEffect))
    ...
    it("should create completions for the zsh shell", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() =>
          expect(result).toMatchFileSnapshot("./snapshots/zsh-completions")
        )
      }).pipe(runEffect))
    ...
    it("should create completions for the fish shell", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() =>
          expect(result).toMatchFileSnapshot("./snapshots/fish-completions")
        )
      }).pipe(runEffect))
  })
})
