...
describe("Options", () => {
  ...
  it("should return a HelpDoc if an option is not an exact match and it's a short option", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.flip(
        process(age, args, CliConfig.defaultConfig)
      )
      expect(result).toEqual(
        ValidationError.missingValue(
          HelpDoc.p("Expected to find option: '--age'")
        )
      )
    }).pipe(runEffect))
  ...
  it("should return a HelpDoc if there is a collision between arguments", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.flip(
        process(options, args, CliConfig.defaultConfig)
      )
      expect(result).toEqual(
  ...
  it("validates a boolean option with negation", () =>
    Effect.gen(function* () {
      const option = Options.boolean("verbose", {
        aliases: ["v"],
        negationNames: ["silent", "s"]
      })
      ...
      const result4 = yield* process(
        option,
        ["--silent"],
        CliConfig.defaultConfig
      )
      const result5 = yield* process(option, ["-s"], CliConfig.defaultConfig)
      const result6 = yield* Effect.flip(
        process(option, ["--verbose", "--silent"], CliConfig.defaultConfig)
      )
      const result7 = yield* Effect.flip(
        process(option, ["-v", "-s"], CliConfig.defaultConfig)
      )
  ...
  it("does not validate collision of boolean options with negation", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.flip(
        process(option, args, CliConfig.defaultConfig)
      )
      expect(result).toEqual(
        ValidationError.invalidValue(
          HelpDoc.p(
  ...
  it("validates an option with choices that map to values", () =>
    Effect.gen(function* () {
      ...
      const cat = new Cat()
      const dog = new Dog()
      const option: Options.Options<Animal> = Options.choiceWithValue(
        "animal",
        [
          ["dog", dog],
          ["cat", cat]
  ...
  it("does not validate when no valid values are passed", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.either(
        process(firstName, args, CliConfig.defaultConfig)
      )
      expect(result).toEqual(
        Either.left(
          ValidationError.missingValue(
            HelpDoc.p("Expected to find option: '--firstName'")
          )
        )
      )
    }).pipe(runEffect))
  ...
  it("does not validate when an option is passed without a corresponding value", () =>
    Effect.gen(function* () {
      ...
      expect(result).toEqual(
        Either.left(
          ValidationError.missingValue(
            HelpDoc.p("Expected a value following option: '--firstName'")
          )
        )
      )
    }).pipe(runEffect))
  ...
  it("does not validate an invalid option value", () =>
    Effect.gen(function* () {
      ...
      expect(result).toEqual(
        ValidationError.invalidValue(HelpDoc.p("'abc' is not a integer"))
      )
    }).pipe(runEffect))
  ...
  it("validates using all and returns the specified structure", () =>
    Effect.gen(function* () {
      ...
      const option2 = Options.all([
        Options.text("firstName"),
        Options.text("lastName")
      ])
      const args = Array.make("--firstName", "John", "--lastName", "Doe")
  ...
  it("orElse - no options provided", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.flip(
        process(option, [], CliConfig.defaultConfig)
      )
  ...
  it("keyValueMap - validates a missing option", () =>
    Effect.gen(function* () {
      const result = yield* Effect.flip(
        process(defs, [], CliConfig.defaultConfig)
      )
      expect(result).toEqual(
        ValidationError.missingValue(
          HelpDoc.p("Expected to find option: '--defs'")
        )
      )
    }).pipe(runEffect))
  ...
  it("keyValueMap - validates key/values with equals in alias value", () =>
    Effect.gen(function* () {
      ...
      expect(result).toEqual([
        ["--verbose"],
        HashMap.make(["key1", "v1"], ["key2", "v2=vv"])
      ])
    }).pipe(runEffect))
  ...
  it("keyValueMap - validates key/values with equals in aliased longer value", () =>
    Effect.gen(function* () {
      ...
      expect(result).toEqual([
        ["--verbose"],
        HashMap.make(["key1", "v1"], ["key2", "v2=1+1"])
      ])
    }).pipe(runEffect))
  ...
  it("repeated", () =>
    Effect.gen(function* () {
      ...
      const result1 = yield* process(option, [], CliConfig.defaultConfig)
      const result2 = yield* process(option, args2, CliConfig.defaultConfig)
      const result3 = yield* Effect.flip(
        process(option, args3, CliConfig.defaultConfig)
      )
      const result4 = yield* Effect.flip(
        process(option, args4, CliConfig.defaultConfig)
      )
      ...
      expect(result3).toEqual(
        ValidationError.invalidValue(HelpDoc.p("'v2' is not a integer"))
      )
      expect(result4).toEqual(
        ValidationError.invalidValue(HelpDoc.p("'v2' is not a integer"))
      )
  ...
  it("atLeast", () =>
    Effect.gen(function* () {
      ...
      const args2 = ["--foo", "1"]
      const result1 = yield* process(option, args1, CliConfig.defaultConfig)
      const result2 = yield* Effect.flip(
        process(option, args2, CliConfig.defaultConfig)
      )
      expect(result1).toEqual([Array.empty(), [1, 2]])
      expect(result2).toEqual(
        ValidationError.invalidValue(
          HelpDoc.p("Expected at least 2 value(s) for option: '--foo'")
        )
      )
    }).pipe(runEffect))
  ...
  it("atMost", () =>
    Effect.gen(function* () {
      ...
      expect(result2).toEqual(
        ValidationError.invalidValue(
          HelpDoc.p("Expected at most 2 value(s) for option: '--foo'")
        )
      )
    }).pipe(runEffect))
  ...
  it("between", () =>
    Effect.gen(function* () {
      ...
      const result1 = yield* Effect.flip(
        process(option, args1, CliConfig.defaultConfig)
      )
      const result2 = yield* process(option, args2, CliConfig.defaultConfig)
      ...
      expect(result4).toEqual(
        ValidationError.invalidValue(
          HelpDoc.p("Expected at most 3 value(s) for option: '--foo'")
        )
      )
    }).pipe(runEffect))
  ...
  it("validates with a Schema", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(result, [
        [],
        BigDecimal.unsafeFromString("100.50").pipe(BigDecimal.normalize)
      ])
    }).pipe(runEffect))
  ...
  it("displays default value in help when default wrapped in Option.Some (primitive)", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        expect(helpDoc).toMatchFileSnapshot(
          "./snapshots/help-output/options-default-primitive"
        )
      )
    }).pipe(runEffect))
  ...
  it("displays default value in help when default wrapped in Option.Some (object)", () =>
    Effect.gen(function* () {
      ...
      const option = Options.withDefault(
        Options.text("config"),
        Option.some(defaultObject)
      )
      const helpDoc = Options.getHelp(option)
      yield* Effect.promise(() =>
        expect(helpDoc).toMatchFileSnapshot(
          "./snapshots/help-output/options-default-object"
        )
      )
    }).pipe(runEffect))
  ...
  it("displays no default value in help when default is not Option.Some", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        expect(helpDoc).toMatchFileSnapshot(
          "./snapshots/help-output/options-no-default"
        )
      )
    }).pipe(runEffect))
  ...
  describe("options after positional arguments", () => {
    ...
    it("parses keyValueMap option with multiple values after positional args", () =>
      Effect.gen(function* () {
        ...
        expect(result).toEqual([
          ["positional"],
          HashMap.make(["key1", "value1"], ["key2", "value2"])
        ])
      }).pipe(runEffect))
  })
})
