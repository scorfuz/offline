      ...
      Effect.gen(function* () {
        ...
        expect(lines).toEqual(
          expect.arrayContaining([
            expect.stringContaining(unsubmittedValue),
            expect.stringContaining(submittedValue)
          ])
        )

        expect(
          lines.findIndex((line) => line.includes(unsubmittedValue))
        ).toBeLessThan(lines.findIndex((line) => line.includes(submittedValue)))
      }).pipe(runEffect))
    ...
    it("should accept the default value when the tab is pressed", () =>
      Effect.gen(function* () {
        ...
        expect(lines).toEqual(
          expect.arrayContaining([
            expect.stringContaining(unsubmittedValue),
            expect.stringContaining(enteredValue)
          ])
        )

        expect(
          lines.findIndex((line) => line.includes(unsubmittedValue))
        ).toBeLessThan(lines.findIndex((line) => line.includes(enteredValue)))
      }).pipe(runEffect))
  })
    ...
    it("should not render the default value when the default is provided", () =>
      Effect.gen(function* () {
        ...
        expect(lines).not.toEqual(
          expect.arrayContaining([expect.stringContaining("default-value")])
        )
      }).pipe(runEffect))
  })
