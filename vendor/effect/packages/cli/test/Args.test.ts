...
describe("Args", () => {
  it("validates an valid argument with a default", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Args.validate(
        args,
        Array.empty(),
  ...
  it("validates an valid optional argument", () =>
    Effect.gen(function* () {
      ...
      let result = yield* Args.validate(
        args,
        Array.empty(),
        CliConfig.defaultConfig
      )
  ...
  it("does not validate an invalid argument even when there is a default", () =>
    Effect.gen(function* () {
      ...
      expect(result).toEqual(
        ValidationError.invalidArgument(HelpDoc.p("'abc' is not a integer"))
      )
    }).pipe(runEffect))
  ...
  it("should validate an existing file that is expected to exist", () =>
    Effect.gen(function* () {
      ...
      const args = Args.file({ name: "files", exists: "yes" }).pipe(
        Args.repeated
      )
      const result = yield* Args.validate(
        args,
    ...
    Effect.gen(function* () {
      ...
      expect(result).toEqual(
        ValidationError.invalidArgument(
          HelpDoc.p(`Path '${filePath}' must exist`)
        )
      )
    }).pipe(runEffect))
  ...
  it("should validate a non-existent file that is expected not to exist", () =>
    Effect.gen(function* () {
      const path = yield* Path.Path
      const filePath = path.join(__dirname, "NotExist.test.ts")
      const args = Args.file({ name: "files", exists: "no" }).pipe(
        Args.repeated
      )
      const result = yield* Args.validate(
        args,
  ...
  it("displays default value in help when default wrapped in Option.Some (primitive)", () =>
    Effect.gen(function* () {
      const option = Args.withDefault(
        Args.integer({ name: "value" }),
        Option.some(123)
      )
      const helpDoc = Args.getHelp(option)
      yield* Effect.promise(() =>
        expect(helpDoc).toMatchFileSnapshot(
          "./snapshots/help-output/args-default-primitive"
        )
      )
    }).pipe(runEffect))
  ...
  it("displays default value in help when default wrapped in Option.Some (object)", () =>
    Effect.gen(function* () {
      ...
      const option = Args.withDefault(
        Args.text({ name: "config" }),
        Option.some(defaultObject)
      )
      const helpDoc = Args.getHelp(option)
      yield* Effect.promise(() =>
        expect(helpDoc).toMatchFileSnapshot(
          "./snapshots/help-output/args-default-object"
        )
      )
    }).pipe(runEffect))
  ...
  it("displays no default value in help when default is not Option.Some", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        expect(helpDoc).toMatchFileSnapshot(
          "./snapshots/help-output/args-no-default"
        )
      )
    }).pipe(runEffect))
})
