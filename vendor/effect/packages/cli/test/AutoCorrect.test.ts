...
describe("AutoCorrect", () => {
  it("should calculate the correct Levenstein distance between two strings", () => {
    expect(
      AutoCorrect.levensteinDistance("", "", CliConfig.defaultConfig)
    ).toBe(0)
    expect(
      AutoCorrect.levensteinDistance("--force", "", CliConfig.defaultConfig)
    ).toBe(7)
    expect(
      AutoCorrect.levensteinDistance("", "--force", CliConfig.defaultConfig)
    ).toBe(7)
    expect(
      AutoCorrect.levensteinDistance(
        "--force",
        "force",
    ...
    expect(
      AutoCorrect.levensteinDistance("foo", "bar", CliConfig.defaultConfig)
    ).toBe(3)
