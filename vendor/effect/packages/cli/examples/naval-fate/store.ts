...
export const NavalFateStore =
  Context.GenericTag<NavalFateStore>("NavalFateStore")

export const make = Effect.gen(function* () {
  const shipsStore = yield* Effect.map(KeyValueStore.KeyValueStore, (store) =>
    store.forSchema(Schema.ReadonlyMap({ key: Schema.String, value: Ship }))
  )
  const minesStore = yield* Effect.map(KeyValueStore.KeyValueStore, (store) =>
    store.forSchema(Schema.Array(Mine))
  )

  const getShips = shipsStore
    .get("ships")
    .pipe(
      Effect.map(Option.getOrElse<ReadonlyMap<string, Ship>>(() => new Map())),
      Effect.orDie
    )
  const getMines = minesStore
    .get("mines")
    .pipe(
  ...
  const moveShip: NavalFateStore["moveShip"] = (name, x, y) =>
    Effect.gen(function* () {
      ...
      const mineAtCoords = Arr.findFirst(mines, (mine) =>
        mine.hasCoordinates(x, y)
      )
  ...
  const removeMine: NavalFateStore["removeMine"] = (x, y) =>
    Effect.gen(function* () {
      const mines = yield* getMines
      const mineAtCoords = Arr.findFirstIndex(mines, (mine) =>
        mine.hasCoordinates(x, y)
      )
