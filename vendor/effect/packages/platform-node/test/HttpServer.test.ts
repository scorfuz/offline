...
describe("HttpServer", () => {
  ...
  it.scoped("schema HttpLayerRouter", () =>
    Effect.gen(function* () {
      const handler = yield* HttpLayerRouter.toHttpEffect(
        HttpLayerRouter.use(
          Effect.fnUntraced(function* (router) {
            yield* router.add(
              "GET",
              "/todos/:id",
              Effect.flatMap(HttpLayerRouter.schemaParams(IdParams), ({ id }) =>
                todoResponse({ id, title: "test" })
              )
            )
          })
  ...
  it.scoped("mount", () =>
    Effect.gen(function* () {
      const child = HttpRouter.empty.pipe(
        HttpRouter.get(
          "/",
          Effect.map(HttpServerRequest.HttpServerRequest, (_) =>
  ...
  it.scoped("file", () =>
    Effect.gen(function* () {
      ...
      )).pipe(Effect.tapErrorCause(Effect.logError), HttpServer.serveEffect())
      const client = yield* HttpClient.HttpClient
  ...
  it.scoped("schemaBodyUrlParams error", () =>
    Effect.gen(function* () {
      yield* HttpRouter.empty.pipe(
        ...
        HttpRouter.catchTag("ParseError", (error) =>
          HttpServerResponse.unsafeJson({ error }, { status: 400 })
        ),
        HttpServer.serveEffect()
      )
      const client = yield* HttpClient.HttpClient
    ...
    Effect.gen(function* () {
      ...
      yield* HttpServerResponse.empty().pipe(
        ...
        HttpServer.serveEffect((app) =>
          Effect.onExit(app, (exit) => Deferred.complete(latch, exit))
        )
      )
      const client = yield* HttpClient.HttpClient
      ...
      const cause = yield* Deferred.await(latch).pipe(
        Effect.sandbox,
        Effect.flip
      )
    ...
    it.scoped("respondable schema", () =>
      Effect.gen(function* () {
        ...
        const res = yield* client
          .get("/user")
          .pipe(Effect.flatMap(HttpClientResponse.schemaBodyJson(User)))
  ...
  it.scoped("concat", () =>
    Effect.gen(function* () {
      ...
      const [responseA, responseMountA, responseB, responseMountB] =
        yield* Effect.all([
          HttpClient.get("/a"),
          HttpClient.get("/ma"),
          HttpClient.get("/b"),
  ...
  it.scoped("concatAll", () =>
    Effect.gen(function* () {
      yield* HttpRouter.concatAll(routerA, routerB).pipe(
        HttpServer.serveEffect()
      )
      const [responseA, responseMountA, responseB, responseMountB] =
        yield* Effect.all([
