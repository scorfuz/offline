...
describe("HttpApi", () => {
    ...
    it.effect("HttpApiDecodeError", () =>
      Effect.gen(function* () {
        ...
        const error = yield* client.users
          .upload({ path: {}, payload: new FormData() })
          .pipe(Effect.flip)
    ...
    it.effect("security middleware sets current user", () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make(
          Cookies.empty.pipe(Cookies.unsafeSet("token", "foo"))
        )
        const client = yield* HttpApiClient.makeWith(Api, {
  ...
  it.effect("error from plain text", () => {
    class RateLimitError extends Schema.TaggedError<RateLimitError>(
      "RateLimitError"
    )("RateLimitError", Schema.Struct({ message: Schema.String })) {}
...
class GlobalError extends Schema.TaggedClass<GlobalError>()(
  "GlobalError",
  {}
) {}
class GroupError extends Schema.TaggedClass<GroupError>()("GroupError", {}) {}
class UserError extends Schema.TaggedClass<UserError>()(
  "UserError",
  {},
  HttpApiSchema.annotations({ status: 400 })
) {}
class NoStatusError extends Schema.TaggedClass<NoStatusError>()(
  "NoStatusError",
  {}
) {}
...
class GroupsApi extends HttpApiGroup.make("groups")
  .add(
    HttpApiEndpoint.get(
      "findById"
    )`/${HttpApiSchema.param("id", Schema.NumberFromString)}`.addSuccess(Group)
  )
  .add(
  ...
  .add(
    HttpApiEndpoint.post(
      "handle"
    )`/handle/${HttpApiSchema.param("id", Schema.NumberFromString)}`
      .setPayload(
        Schema.Struct({
          name: Schema.String
        })
  ...
  .add(
    HttpApiEndpoint.post(
      "handleRaw"
    )`/handleraw/${HttpApiSchema.param("id", Schema.NumberFromString)}`
      .setPayload(
        Schema.Struct({
          name: Schema.String
        })
...
class UsersApi extends HttpApiGroup.make("users")
  ...
  .add(
    HttpApiEndpoint.post("create")`/`
      .setPayload(Schema.Struct(Struct.omit(User.fields, "id", "createdAt")))
      .setUrlParams(
        Schema.Struct({
...
class TopLevelApi extends HttpApiGroup.make("root", { topLevel: true }).add(
  HttpApiEndpoint.get("healthz")`/healthz`.addSuccess(
    HttpApiSchema.NoContent.annotations({ description: "Empty" })
  )
) {}
...
const HttpUsersLive = HttpApiBuilder.group(Api, "users", (handlers) =>
  Effect.gen(function* () {
    ...
    return handlers
      .handle("findById", (_) =>
        _.path.id === -1 ? CurrentUser : repo.findById(_.path.id)
      )
      .handle("create", (_) =>
        _.payload.name === "boom"
