    ...
    return Server.make({
      ...
      serve: (httpApp, middleware) =>
        Effect.gen(function* () {
          ...
          const upgradeHandler = yield* makeUpgradeHandler(
            wss,
            httpApp,
            middleware!
          )
...
class ServerRequestImpl
  extends HttpIncomingMessageImpl<Error.RequestError>
  implements ServerRequest.HttpServerRequest
{
  ...
  > {
    ...
    this.multipartEffect = Effect.runSync(
      Effect.cached(MultipartNode.persisted(this.source, this.source.headers))
    )
    return this.multipartEffect
  }
...
): Http.IncomingMessage => (self as ServerRequestImpl).source
