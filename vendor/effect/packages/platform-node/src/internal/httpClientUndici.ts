...
/** @internal */
export const Dispatcher = Context.GenericTag<
  NodeClient.Dispatcher,
  Undici.Dispatcher
...
/** @internal */
export const dispatcherLayerGlobal = Layer.sync(Dispatcher, () =>
  Undici.getGlobalDispatcher()
)

/** @internal */
export const undiciOptionsTagKey =
...
export const make = (dispatcher: Undici.Dispatcher): Client.HttpClient =>
  Client.make((request, url, signal, fiber) => {
    ...
    const options: Undici.Dispatcher.RequestOptions =
      context.unsafeMap.get(undiciOptionsTagKey) ?? {}
...
/** @internal */
export const layerWithoutDispatcher = Client.layerMergedContext(
  Effect.map(Dispatcher, make)
)
