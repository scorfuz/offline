...
/** @internal */
export const HttpAgent = Context.GenericTag<NodeClient.HttpAgent>(
  "@effect/platform-node/Http/NodeClient/HttpAgent"
)
...
): Effect.Effect<NodeClient.HttpAgent, never, Scope.Scope> =>
  Effect.map(
    Effect.all([
...
export const makeAgentLayer = (
  options?: Https.AgentOptions
): Layer.Layer<NodeClient.HttpAgent> =>
...
const fromAgent = (agent: NodeClient.HttpAgent): Client.HttpClient =>
  Client.make((request, url, signal) => {
    ...
    return pipe(
      Effect.zipRight(
        sendBody(nodeRequest, request, request.body),
...
class ClientResponseImpl
  extends HttpIncomingMessageImpl<Error.ResponseError>
