    ...
    describe("sequential vs parallel", () => {
      it("deepStrictEqual", { fails: true }, () => {
        assert.deepStrictEqual(
          Cause.sequential(Cause.empty, Cause.empty),
          Cause.parallel(Cause.empty, Cause.empty)
        )
      })
    })
  })
  ...
  describe("Cause", () => {
    describe("sequential vs parallel", () => {
      it("toEqual", { fails: false }, () => {
        expect(Cause.sequential(Cause.empty, Cause.empty)).toEqual(
          Cause.parallel(Cause.empty, Cause.empty)
        )
      })

      it("toStrictEqual", { fails: false }, () => {
        expect(Cause.sequential(Cause.empty, Cause.empty)).toStrictEqual(
          Cause.parallel(Cause.empty, Cause.empty)
        )
      })
    })
...
describe("utils", () => {
  ...
  it("assertInclude", () => {
    ...
    Util.throws(
      () => Util.assertInclude(undefined, "a"),
      (err) => {
        Util.assertTrue(err instanceof Error)
        Util.strictEqual(
          err.message,
