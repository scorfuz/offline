...
describe.concurrent("Micro", () => {
  ...
  describe("fromOption", () => {
    ...
    it("from a none", () =>
      Option.none().pipe(
        ...
        Micro.flip,
        Micro.tap((error) =>
          assertInstanceOf(error, Micro.NoSuchElementException)
        ),
        Micro.runPromise
      ))
  })
  ...
  describe("forEach", () => {
    ...
    it("unbounded", () =>
      Micro.gen(function* () {
        const results = yield* Micro.forEach(
          [1, 2, 3],
          (_) => Micro.succeed(_),
    ...
    it("inherit unbounded", () =>
      Micro.gen(function* () {
        const handle = yield* Micro.forEach(
          [1, 2, 3],
          (_) => Micro.succeed(_).pipe(Micro.delay(50)),
          {
            concurrency: "inherit"
          }
        ).pipe(Micro.withConcurrency("unbounded"), Micro.fork)
    ...
    it("unbounded interrupt", () =>
      Micro.gen(function* () {
        const done: Array<number> = []
        const fiber = yield* Micro.forEach(
          [1, 2, 3],
          (i) =>
            Micro.sync(() => {
              done.push(i)
              return i
            }).pipe(Micro.delay(150)),
          { concurrency: "unbounded" }
        ).pipe(Micro.fork)
  ...
  describe("all", () => {
    ...
    it.effect("iterable", () =>
      Micro.gen(function* () {
        const results = (yield* Micro.all(
          new Set([Micro.succeed(1), Micro.succeed(2), Micro.succeed(3)])
  ...
  describe("filter", () => {
    it.live("odd numbers", () =>
      Micro.gen(function* () {
        const results = yield* Micro.filter([1, 2, 3, 4, 5], (_) =>
          Micro.succeed(_ % 2 === 1)
        )
    ...
    it.live("iterable", () =>
      Micro.gen(function* () {
        const results = yield* Micro.filter(new Set([1, 2, 3, 4, 5]), (_) =>
          Micro.succeed(_ % 2 === 1)
        )
        deepStrictEqual(results, [1, 3, 5])
      })
  ...
  it.effect("raceAll", () =>
    Micro.gen(function* () {
      ...
      const result = yield* Micro.raceAll(
  ...
  it("raceAllFirst", () =>
    Micro.gen(function* () {
      const interrupted: Array<number> = []
      const result = yield* Micro.raceAllFirst(
        [500, 300, 200, 0, 100].map((ms) =>
          (ms === 0 ? Micro.fail("boom") : Micro.succeed(ms)).pipe(
            Micro.delay(ms),
            Micro.onInterrupt(
  ...
  describe("valid Effect", () => {
    ...
    it.effect("context", () =>
      Effect.gen(function* () {
        const result = yield* Micro.service(ATag).pipe(Micro.map((_) => _))
    ...
    it.effect("interruption", () =>
      Effect.gen(function* () {
        const fiber = yield* Micro.never.pipe(Effect.fork)
  ...
  describe("repeat", () => {
    it.effect("is stack safe", () =>
      Micro.void.pipe(Micro.repeat({ times: 10000 }))
    )

    it.effect("is interruptible", () =>
      Micro.void.pipe(Micro.forever, Micro.timeoutOption(50))
    )
  ...
  describe("retry", () => {
    it.live("nothing on success", () =>
      Micro.gen(function* () {
        let count = 0
        yield* Micro.sync(() => count++).pipe(Micro.retry({ times: 10000 }))
        strictEqual(count, 1)
      })
    )
  ...
  describe("timeoutOption", () => {
    ...
    it.effect(
      ...
      () =>
        Micro.gen(function* () {
          yield* Micro.void.pipe(
            Micro.timeoutOption(20_000),
            Micro.uninterruptible
          )
        }),
      { timeout: 1000 }
  ...
  describe("failure rendering", () => {
    it.effect("renders non-error defects", () =>
      Micro.gen(function* () {
        ...
        assertInclude(
          failure.stack,
          `MicroCause.Die: ${JSON.stringify({ some: "error" })}`
        )
    ...
    it.effect("renders non-errors", () =>
      Micro.gen(function* () {
        ...
        assertInclude(
          failure.stack,
          `MicroCause.Fail: ${JSON.stringify({ some: "error" })}`
        )
        assertInclude(failure.stack, "at test trace (")
      })
    )
  ...
  describe("interruption", () => {
      ...
      Micro.gen(function* () {
        ...
        const fiber = yield* Micro.never.pipe(
          ...
          Micro.flatMap((result) =>
            Micro.sync(() => {
              recovered =
                result._tag === "Failure" && result.cause._tag === "Interrupt"
            })
          ),
          Micro.uninterruptible,
          Micro.fork
    ...
    it.live("acquireUseRelease use inherits interrupt status", () =>
      Micro.gen(function* () {
        ...
        const fiber = yield* Micro.acquireUseRelease(
          ...
          () => Micro.void
        ).pipe(Micro.uninterruptible, Micro.fork)
        yield* Micro.yieldFlush
        yield* Micro.fiberInterrupt(fiber)
        assertTrue(ref)
      })
    ...
    it.live("async cannot resume on interrupt", () =>
      Micro.gen(function* () {
        ...
        }).pipe(Micro.onInterrupt(Micro.sleep(30)), Micro.fork)
        yield* Micro.yieldFlush
        yield* Micro.fiberInterrupt(fiber)
        deepStrictEqual(fiber.unsafePoll(), Micro.exitInterrupt)
      })
  ...
  describe("stack safety", () => {
    it.effect("recursion", () => {
      ...
      return loop.pipe(Micro.timeoutOption(50))
    })
  })
  ...
  describe("zip", () => {
    it.effect("concurrent: false", () => {
      ...
      const task1 = Micro.succeed("a").pipe(
        Micro.delay(5),
        Micro.tap(() => executionOrder.push("task1"))
      )
      const task2 = Micro.succeed(1).pipe(
        Micro.delay(1),
        Micro.tap(() => executionOrder.push("task2"))
      )
  ...
  describe("catchCauseIf", () => {
    it.effect("first argument as success", () =>
      Micro.gen(function* () {
        const result = yield* Micro.catchCauseIf(
          Micro.succeed(1),
          () => false,
          () => Micro.fail("e2")
        )
    ...
    it.effect("first argument as failure and predicate return false", () =>
      Micro.gen(function* () {
        const result = yield* Micro.flip(
          Micro.catchCauseIf(
            Micro.fail("e1" as const),
            () => false,
            () => Micro.fail("e2" as const)
          )
        )
  ...
  describe("catchAll", () => {
    it.effect("first argument as success", () =>
      Micro.gen(function* () {
        const result = yield* Micro.catchAll(Micro.succeed(1), () =>
          Micro.fail("e2" as const)
        )
        deepStrictEqual(result, 1)
      })
    ...
    it.effect("first argument as failure", () =>
      Micro.gen(function* () {
        const result = yield* Micro.flip(
          Micro.catchAll(Micro.fail("e1" as const), () =>
            Micro.fail("e2" as const)
          )
        )
        deepStrictEqual(result, "e2")
  ...
  describe("catchAllCause", () => {
    it.effect("first argument as success", () =>
      Micro.gen(function* () {
        const result = yield* Micro.catchAllCause(Micro.succeed(1), () =>
          Micro.fail("e2" as const)
        )
        deepStrictEqual(result, 1)
      })
    ...
    it.effect("first argument as failure", () =>
      Micro.gen(function* () {
        const result = yield* Micro.flip(
          Micro.catchAllCause(Micro.fail("e1" as const), () =>
            Micro.fail("e2" as const)
          )
        )
        deepStrictEqual(result, "e2")
  ...
  describe("schedules", () => {
    ...
    it("scheduleAddDelay", () => {
      const out = dryRun(
        Micro.scheduleAddDelay(Micro.scheduleRecurs(5), () => 10)
      )
    ...
    it("scheduleWithMaxDelay", () => {
      const out = dryRun(
        Micro.scheduleWithMaxDelay(Micro.scheduleExponential(10), 400)
      )
    ...
    it("scheduleWithMaxElapsed", () => {
      const out = dryRun(
        Micro.scheduleWithMaxElapsed(Micro.scheduleExponential(10), 400)
      )
