...
const asEffect = <A, E>(either: Either.Either<A, E>): Effect.Effect<A, E> =>
  either

const expectGetRefinementExpected = (
  schema: S.Schema.Any,
  expected: string
) => {
...
describe("ParseResult", () => {
  const typeParseError1 = ParseResult.parseError(
    new ParseResult.Type(S.String.ast, null)
  )
  const typeParseError2 = ParseResult.parseError(
    new ParseResult.Type(S.Number.ast, null)
  )

  it("getRefinementExpected", () => {
    expectGetRefinementExpected(
      S.Number.pipe(S.filter(() => true)),
      "{ number | filter }"
    )
    expectGetRefinementExpected(S.Number.pipe(S.int()), "an integer")
    expectGetRefinementExpected(
      S.Number.pipe(S.int(), S.positive()),
      "a positive number"
    )
  ...
  describe("ParseError", () => {
    it("toString()", () => {
      ...
      assertLeft(
        S.decodeUnknownEither(schema)({}).pipe(
...
└─ ["a"]
   └─ is missing`
    ...
    it("toJSON()", () => {
      ...
      assertLeft(
        S.decodeUnknownEither(schema)({}).pipe(
          Either.mapLeft((e) => (e as any).toJSON())
        ),
        {
          _id: "ParseError",
...
└─ ["a"]
    ...
    it("Error.stack", () => {
      assertTrue(
        ParseResult.parseError(
          new ParseResult.Type(S.String.ast, 1)
        ).stack?.startsWith(`ParseError: Expected string, actual 1`)
      )
    })
  ...
  it("eitherOrUndefined", () => {
    deepStrictEqual(
      ParseResult.eitherOrUndefined(Either.right(1)),
      Either.right(1)
    )
    deepStrictEqual(
      ParseResult.eitherOrUndefined(Either.left("err")),
      Either.left("err")
    )
  ...
  it("flatMap", () => {
    ...
    assertFailure(
      Effect.runSyncExit(
        ParseResult.flatMap(Either.right(1), () => Effect.fail("err"))
      ),
      Cause.fail("err")
    )
  })
  ...
  it("map", () => {
    deepStrictEqual(
      ParseResult.map(Either.right(1), (n) => n + 1),
      asEffect(Either.right(2))
    )
    deepStrictEqual(
      ParseResult.map(Either.left(typeParseError1), (n) => n + 1),
      asEffect(Either.left(typeParseError1))
    )
    deepStrictEqual(
      Either.right(1).pipe(ParseResult.map((n) => n + 1)),
      Either.right(2)
    )
    assertSuccess(
      Effect.runSyncExit(ParseResult.map(Effect.succeed(1), (n) => n + 1)),
      2
    )
    assertFailure(
      Effect.runSyncExit(
        ParseResult.map(Effect.fail(typeParseError1), (n) => n + 1)
      ),
  ...
  it("mapError", () => {
    deepStrictEqual(
      ParseResult.mapError(Either.right(1), () => typeParseError2),
      asEffect(Either.right(1))
    )
    deepStrictEqual(
    ...
    // pipeable
    deepStrictEqual(
      Either.right(1).pipe(ParseResult.mapError(() => typeParseError2)),
      Either.right(1)
    )
    assertSuccess(
      Effect.runSyncExit(
        ParseResult.mapError(Effect.succeed(1), () => typeParseError2)
      ),
      1
    )
    assertFailure(
  ...
  it("mapBoth", () => {
    deepStrictEqual(
      ParseResult.mapBoth(Either.right(1), {
    ...
    assertSuccess(
      Effect.runSyncExit(
        ParseResult.mapBoth(Effect.succeed(1), {
          onFailure: () => typeParseError2,
          onSuccess: (n) => n + 1
        })
      ),
  ...
  it("orElse", () => {
    deepStrictEqual(
      ParseResult.orElse(Either.right(1), () => Either.right(2)),
      asEffect(Either.right(1))
    )
    deepStrictEqual(
      ParseResult.orElse(Either.left(typeParseError1), () => Either.right(2)),
      asEffect(Either.right(2))
    )
    // pipeable
    deepStrictEqual(
      Either.right(1).pipe(ParseResult.orElse(() => Either.right(2))),
      Either.right(1)
    )
    assertSuccess(
      Effect.runSyncExit(
        ParseResult.orElse(Effect.succeed(1), () => Either.right(2))
      ),
      1
    )
    assertSuccess(
...
describe("ParseIssue.actual", () => {
  it("transform decode", () => {
    const result = S.decodeEither(
      S.transformOrFail(S.NumberFromString, S.Boolean, {
        strict: true,
    ...
    strictEqual(
      (result.left.issue as ParseResult.Transformation).issue.actual,
      1
    )
  })

  it("transform encode", () => {
    const result = S.encodeEither(
      S.transformOrFail(S.Boolean, S.NumberFromString, {
        strict: true,
        decode: (n, _, ast) => ParseResult.fail(new ParseResult.Type(ast, n)),
        encode: (b, _, ast) => ParseResult.fail(new ParseResult.Type(ast, b))
      })
  ...
  it("compose decode", () => {
    const result = S.decodeEither(
      S.compose(S.NumberFromString, S.Number.pipe(S.negative()))
    )("1")
    if (Either.isRight(result)) throw new Error("Expected failure")
    strictEqual(result.left.issue.actual, "1")
    strictEqual(
  ...
  it("compose encode", () => {
    const result = S.encodeEither(
      S.compose(S.String.pipe(S.length(5)), S.NumberFromString)
    )(1)
    if (Either.isRight(result)) throw new Error("Expected failure")
    strictEqual(result.left.issue.actual, 1)
    strictEqual(
  ...
  it("mergeInternalOptions", () => {
    strictEqual(
      ParseResult.mergeInternalOptions(undefined, undefined),
      undefined
    )
    deepStrictEqual(ParseResult.mergeInternalOptions({}, undefined), {})
    deepStrictEqual(ParseResult.mergeInternalOptions(undefined, {}), {})
    deepStrictEqual(
      ParseResult.mergeInternalOptions({ errors: undefined }, undefined),
      { errors: undefined }
    )
    deepStrictEqual(
      ParseResult.mergeInternalOptions(undefined, { errors: undefined }),
      { errors: undefined }
    )
    deepStrictEqual(
      ParseResult.mergeInternalOptions({ errors: "all" }, { errors: "first" }),
      {
        errors: "first"
      }
    )
    deepStrictEqual(
      ParseResult.mergeInternalOptions(
    ...
    deepStrictEqual(ParseResult.mergeInternalOptions({}, { exact: false }), {
      exact: false
    })
    deepStrictEqual(
      ParseResult.mergeInternalOptions({ exact: true }, { exact: false }),
      { exact: false }
    )

    deepStrictEqual(
      ParseResult.mergeInternalOptions({ isEffectAllowed: true }, {}),
  ...
  describe("getLiterals", () => {
    ...
    it("Struct", () => {
      deepStrictEqual(
        ParseResult.getLiterals(S.Struct({ _tag: S.Literal("a") }).ast, true),
    ...
    it("Tuple", () => {
      deepStrictEqual(
        ParseResult.getLiterals(S.Tuple(S.Literal("a"), S.String).ast, true),
        [[0, new AST.Literal("a")]]
      )
    })
    ...
    it("Refinement", () => {
      deepStrictEqual(
        ParseResult.getLiterals(
          S.Struct({ _tag: S.Literal("a") }).pipe(S.filter(() => true)).ast,
          true
        ),
        [["_tag", new AST.Literal("a")]]
      )
    ...
    it("property Transform (encode)", () => {
      deepStrictEqual(
        ParseResult.getLiterals(
          S.Struct({
            _tag: S.transform(S.Literal("a"), S.Literal("b"), {
              strict: true,
              decode: () => "b" as const,
              encode: () => "a" as const
            })
    ...
    it("Class (decode)", () => {
      ...
      deepStrictEqual(ParseResult.getLiterals(A.ast, true), [
        ["_tag", new AST.Literal("a")]
      ])
    })
    ...
    it("Class (encode)", () => {
      class A extends S.Class<A>("A")({ _tag: S.Literal("a") }) {}
      deepStrictEqual(ParseResult.getLiterals(A.ast, false), [
        ["_tag", new AST.Literal("a")]
      ])
    })
  })
  ...
  describe("getSearchTree", () => {
    it("primitive + primitive", () => {
      deepStrictEqual(
        ParseResult.getSearchTree([S.String.ast, S.Number.ast], true),
        {
          keys: {},
    ...
    it("should handle multiple tags", () => {
      ...
      deepStrictEqual(ParseResult.getSearchTree([a.ast, b.ast, c.ast], true), {
        keys: {
          category: {
            buckets: {
              catA: [a.ast]
