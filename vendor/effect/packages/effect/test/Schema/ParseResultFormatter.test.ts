...
const expectSyncTree = <A, I>(
  ...
  const actual = S.decodeUnknownEither(schema)(
    input,
...
describe("Formatters output", () => {
  it("Effect async message", async () => {
    ...
    const input = null
    await expectAsyncTree(schema, input, "custom message")
    await expectAsyncIssues(schema, input, [
      {
        _tag: "Type",
        path: [],
  ...
  it("Effect sync messages", () => {
    ...
    const input = null
    expectSyncTree(schema, input, "custom message")
   ...
   └─ is unexpected, expected: "a"`,
    ...
    describe("Declaration", () => {
      it("default message", () => {
        ...
        const input = null
        expectSyncTree(schema, input, "Expected Option<string>, actual null")
        expectSyncIssues(schema, input, [
          {
            _tag: "Type",
            path: [],
      ...
      it("default message with identifier", () => {
        ...
        const input = null
        expectSyncTree(schema, input, "Expected identifier, actual null")
        expectSyncIssues(schema, input, [
          {
            _tag: "Type",
            path: [],
...
└─ Expected string, actual 1`
    ...
    describe("String", () => {
      it("default message", () => {
        ...
        const input = null
        expectSyncTree(schema, input, "Expected string, actual null")
        expectSyncIssues(schema, input, [
          {
            _tag: "Type",
            path: [],
      ...
      it("default message with identifier", () => {
        ...
        const input = null
        expectSyncTree(schema, input, "Expected ID, actual null")
        expectSyncIssues(schema, input, [
          {
            _tag: "Type",
            path: [],
    ...
    describe("Transformation", () => {
      it("default message", () => {
        const schema = S.transformOrFail(S.String, S.String, {
          strict: true,
   ...
   └─ Expected string, actual null`
      ...
      it("default message with message field (kind=Transformation)", () => {
        const schema = S.transformOrFail(S.String, S.String, {
          ...
          decode: (s, _, ast) =>
            ParseResult.fail(
              new ParseResult.Type(ast, s, "transformation failure")
            ),
          encode: ParseResult.succeed
        })
        const input = ""
...
└─ Transformation process failure
   └─ transformation failure`
      ...
      it("custom message (kind=From, override=false)", () => {
        const schema = S.transform(S.String, S.String, {
          strict: true,
          decode: identity,
          encode: identity
...
└─ Encoded side transformation failure
   └─ Expected string, actual null`
      ...
      it("custom message with inner custom message (kind=From, override=false)", () => {
        ...
        const input = null
        expectSyncTree(schema, input, "inner custom message")
        expectSyncIssues(schema, input, [
          {
            _tag: "Transformation",
            path: [],
      ...
      it("custom message (kind=To, override=false)", () => {
        const schema = S.transform(S.String, S.NonEmptyString, {
          strict: true,
          decode: identity,
          encode: identity
        }).annotations({ message: () => "custom message" })
   ...
   └─ NonEmptyString
      ...
      it("custom message (kind=Transformation, override=false)", () => {
        const schema = S.transformOrFail(S.String, S.String, {
          strict: true,
          decode: (s, _, ast) =>
            ParseResult.fail(new ParseResult.Type(ast, s, "message field")),
          encode: ParseResult.succeed
        }).annotations({ message: () => "custom message" })
        const input = ""
        expectSyncTree(schema, input, "custom message")
   ...
   └─ Expected string, actual null`
      ...
      it("custom message with inner custom message (kind=From, override=false)", () => {
        const schema = S.String.pipe(
          S.minLength(1, { message: () => "inner custom message" }),
          S.maxLength(2)
        ).annotations({ message: () => "custom message" })
        const input = ""
        expectSyncTree(schema, input, "inner custom message")
...
└─ Expected number, actual null`
    ...
    describe("Tuple", () => {
      it("parent custom message with override=false", () => {
        const schema = S.Tuple(S.String).annotations({
          message: () => "custom message"
        })
        const input1 = [1]
        expectSyncTree(
...
└─ [0]
   └─ Expected string, actual 1`
      ...
      it("parent custom message with override=true", () => {
        ...
        const input1 = [1]
        expectSyncTree(schema, input1, "custom message")
        expectSyncIssues(schema, input1, [
          {
            _tag: "Composite",
            path: [],
...
└─ ["as"]
   └─ B
      ...
      it("parent custom message with override=true", () => {
        ...
        const input2 = { as: [] }
        expectSyncTree(schema, input2, "custom message")
        expectSyncIssues(schema, input2, [
          {
            _tag: "Composite",
            path: [],
...
└─ [1]
   └─ A | null
      ...
      it("inner/outer", () => {
        ...
        expectSyncTree(schema, null, `Expected A, actual null`)
        expectSyncTree(
          schema,
          [1, undefined],
          `A
...
└─ [1]
  ...
  it("Effect as message", () => {
    ...
    class Translator extends Context.Tag("Translator")<
      Translator,
      {
    ...
    assertLeft(
      Either.mapLeft(result, (error) =>
        Effect.runSync(ParseResult.TreeFormatter.formatError(error))
      ),
      "Invalid string"
    )
      ...
      Either.mapLeft(result, (error) =>
        Effect.runSync(
          ParseResult.TreeFormatter.formatError(error).pipe(
            Effect.provideService(Translator, {
              locale: "it",
              translations
            })
          )
