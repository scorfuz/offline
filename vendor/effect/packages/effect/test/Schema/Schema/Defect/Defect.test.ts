...
describe("Defect", () => {
  describe("decoding", () => {
    it("a string", async () => {
      await Util.assertions.decoding.succeed(S.Defect, "error", "error")
    })
    ...
    it("an object with a message", () => {
      ...
      deepStrictEqual(
        err,
        new Error("message", { cause: { message: "message" } })
      )
    })
    ...
    it("an object with a message and a name", () => {
      const err = S.decodeUnknownSync(S.Defect)({
        message: "message",
        name: "name"
      })
    ...
    it("a null object without a message", async () => {
      ...
      await Util.assertions.decoding.succeed(S.Defect, defect, '{"a":1}')
    })
  })

  describe("encoding", () => {
    it("a string", async () => {
      await Util.assertions.encoding.succeed(S.Defect, "error", "error")
    })

    it("an object", async () => {
      await Util.assertions.encoding.succeed(S.Defect, { a: 1 }, '{"a":1}')
    })
    ...
    it("a null object", async () => {
      ...
      await Util.assertions.encoding.succeed(S.Defect, defect, '{"a":1}')
    })

    it("an Error", async () => {
      await Util.assertions.encoding.succeed(S.Defect, new Error("message"), {
        message: "message",
        name: "Error"
      })
    })
