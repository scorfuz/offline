...
└─ ["a"]
   └─ Expected number, actual undefined`
    ...
    describe("Tuple", () => {
      it("e", async () => {
        const schema = S.partialWith(S.Tuple(S.NumberFromString), {
          exact: true
        })
...
└─ [0]
   └─ number | undefined
    ...
    it("Union", async () => {
      const schema = S.partialWith(S.Union(S.Array(S.Number), S.String), {
        exact: true
      })
