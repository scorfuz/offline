...
describe("withDecodingDefault", () => {
  describe("PropertySignatureDeclaration", () => {
    ...
    it("should prune undefined from the type", () => {
      ...
      strictEqual(
        String(prop1),
        `PropertySignature<":", string, never, "?:", string | undefined>`
      )

      const prop2 = S.optional(S.NumberFromString).pipe(
        S.withDecodingDefault(() => 0)
      )
      strictEqual(
        String(prop2),
        `PropertySignature<":", number, never, "?:", NumberFromString | undefined>`
      )
    })
  })
  ...
  describe("PropertySignatureTransformation", () => {
    it("optional", async () => {
      const prop = S.optional(S.String).pipe(
        S.fromKey("b"),
        S.withDecodingDefault(() => "")
      )
      const schema = S.Struct({ a: prop })
    ...
    it("should prune undefined from the type", () => {
      const prop1 = S.optional(S.String).pipe(
        S.fromKey("a"),
        S.withDecodingDefault(() => "")
      )
      strictEqual(
        String(prop1),
        `PropertySignature<":", string, "a", "?:", string | undefined>`
      )
      ...
      strictEqual(
        String(prop2),
        `PropertySignature<":", number, "a", "?:", NumberFromString | undefined>`
      )
    })
  })
})
