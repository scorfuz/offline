...
describe("record", () => {
  ...
  it("should compute the partial result", () => {
    const schema = S.Record({ key: S.String, value: S.Number })
    const all = S.decodeUnknownEither(schema)(
      { a: 1, b: "b", c: 2, d: "d" },
      { errors: "all" }
    )
    ...
    const first = S.decodeUnknownEither(schema)(
      { a: 1, b: "b", c: 2, d: "d" },
      { errors: "first" }
    )
   ...
   └─ is missing`
    ...
    it("Record('a' | `prefix-${string}`, number)", async () => {
      const schema = S.Record({
        key: S.Union(
          S.Literal("a"),
          S.TemplateLiteral(S.Literal("prefix-"), S.String)
        ),
        value: S.Number
      })
      await Util.assertions.decoding.succeed(schema, { a: 1 })
...
└─ ["prefix-b"]
   └─ Expected number, actual "b"`
    ...
    it("Record(keyof struct({ a, b }), number)", async () => {
      const schema = S.Record({
        key: S.keyof(S.Struct({ a: S.String, b: S.String })),
        value: S.Number
      })
      await Util.assertions.decoding.succeed(schema, { a: 1, b: 2 })
...
└─ ["a"]
   └─ Expected number, actual "a"`
    ...
    it("Record(Symbol('a') | Symbol('b'), number)", async () => {
      ...
      const b = Symbol.for("effect/Schema/test/b")
      const schema = S.Record({
        key: S.Union(S.UniqueSymbolFromSelf(a), S.UniqueSymbolFromSelf(b)),
        value: S.Number
      })
      await Util.assertions.decoding.succeed(schema, { [a]: 1, [b]: 2 })
...
└─ [Symbol(effect/Schema/test/a)]
   └─ is missing`
    ...
    it("Record(${string}-${string}, number)", async () => {
      const schema = S.Record({
        key: S.TemplateLiteral(S.String, S.Literal("-"), S.String),
        value: S.Number
      })
      await Util.assertions.decoding.succeed(schema, {})
...
└─ ["a"]
   └─ string | number
    ...
    it("should support branded keys", async () => {
      const schema = S.Record({
        key: S.NonEmptyString.pipe(S.brand("UserId")),
        value: S.Number
      })
      await Util.assertions.decoding.succeed(schema, {})
      await Util.assertions.decoding.succeed(schema, { a: 1 })
