...
const expectPattern = (
  ...
  strictEqual(
    AST.getTemplateLiteralCapturingRegExp(ast).source,
    expectedPattern
  )
}
...
└─ Type side transformation failure
   └─ readonly [Int, "a"]
    ...
    it(`NumberFromString + "a" + NonEmptyString`, async () => {
      ...
      await Util.assertions.decoding.succeed(schema, "100ab23a", [
        100,
        "a",
        "b23a"
      ])
   ...
   └─ Expected \`null\`, actual "nulla"`
    ...
    it(`"c" + (\`a\${string}b\`|"e") + "d"`, async () => {
      const schema = S.TemplateLiteralParser(
        "c",
        S.Union(
          S.TemplateLiteralParser("a", S.NonEmptyString, "b"),
          S.Literal("e")
        ),
        "d"
      )
...
└─ Encoded side transformation failure
   └─ Expected \`c\${\`a\${string}b\` | "e"}d\`, actual "ed"`
    ...
    it(`"c" + (\`a\${number}b\`|"e") + "d"`, async () => {
      ...
      await Util.assertions.decoding.succeed(schema, "ced", ["c", "e", "d"])
      await Util.assertions.decoding.succeed(schema, "ca1bd", [
        "c",
        ["a", 1, "b"],
        "d"
      ])
...
└─ Encoded side transformation failure
   └─ Expected \`<\${\`h\${"1" | "2"}\`}>\`, actual "<h3>"`
    ...
    it('(`<${`h${"1" | "2"}`}>` <-> readonly ["<", `h${"1" | "2"}`, ">"])', async () => {
      const schema = S.TemplateLiteralParser(
        "<",
        S.TemplateLiteralParser("h", S.Literal(1, 2)),
        ">"
      )
