    ...
    describe("discriminated unions", () => {
      describe("structs", () => {
        it("should handle discriminators for each struct", async () => {
          const schema = S.Union(
            S.Struct({ a: S.Literal(1), c: S.String }).annotations({
              identifier: "ID1"
            }),
            S.Struct({ b: S.Literal(2), d: S.Number }).annotations({
              identifier: "ID2"
            })
          )
          await Util.assertions.decoding.fail(
   ...
   └─ ["b"]
      └─ Expected 2, actual 3`
        ...
        it("should handle structs with multiple discriminators", async () => {
          const schema = S.Union(
            S.Struct({
              category: S.Literal("catA"),
              tag: S.Literal("a")
            }).annotations({ identifier: "IDa" }),
            S.Struct({
              category: S.Literal("catA"),
              tag: S.Literal("b")
            }).annotations({ identifier: "IDb" }),
            S.Struct({
              category: S.Literal("catA"),
              tag: S.Literal("c")
            }).annotations({ identifier: "IDc" })
          )
          await Util.assertions.decoding.fail(
            schema,
            null,
   ...
   └─ ["tag"]
      └─ Expected "b" | "c", actual "d"`
        ...
        it("should handle nested unions", async () => {
          const a = S.Struct({ _tag: S.Literal("a") }).annotations({
            identifier: "IDa"
          })
          const b = S.Struct({ _tag: S.Literal("b") }).annotations({
            identifier: "IDb"
          })
          const A = S.Struct({ a: S.Literal("A"), c: S.String }).annotations({
            identifier: "IDA"
          })
          const B = S.Struct({ b: S.Literal("B"), d: S.Number }).annotations({
            identifier: "IDB"
          })
   ...
   └─ [1]
      └─ Expected string, actual 0`
        ...
        it("should handle tuples with multiple discriminators", async () => {
          const schema = S.Union(
            S.Tuple(S.Literal("catA"), S.Literal("a"), S.String).annotations({
              identifier: "IDa"
            }),
            S.Tuple(S.Literal("catA"), S.Literal("b"), S.Number).annotations({
              identifier: "IDb"
            }),
            S.Tuple(S.Literal("catA"), S.Literal("c"), S.Boolean).annotations({
              identifier: "IDc"
            })
          ).annotations({ identifier: "ID" })
   ...
   └─ [2]
      └─ Expected string, actual 0`
        ...
        it("should handle discriminated tuple + tuple", async () => {
          const schema = S.Union(
            S.Tuple(S.Literal(-1), S.Literal(0)).annotations({
              identifier: "ID1"
            }),
            S.Tuple(S.NonNegativeInt, S.NonNegativeInt).annotations({
              identifier: "ID2"
            })
          ).annotations({ identifier: "ID" })
...
└─ ID3
  ...
  describe("encoding", () => {
    ...
    it("should handle members with exact optional property signatures", async () => {
      ...
      const ac = S.Struct({
        a: S.String,
        c: S.optionalWith(S.Number, { exact: true })
      })
