...
describe("extend", () => {
  describe("String", () => {
    ...
    it("String Branded Refinement & String Branded Refinement", () => {
      const startsWith = Schema.String.pipe(
        Schema.startsWith("start:"),
        Schema.brand("start:")
      )
      const endsWith = Schema.String.pipe(
        Schema.endsWith(":end"),
        Schema.brand(":end")
      )
      ...
      strictEqual(
        String(schema),
        `startsWith("start:") & Brand<"start:"> & endsWith(":end") & Brand<":end">`
      )
  ...
  describe("Number", () => {
    ...
    it("Number Branded Refinement & Number Branded Refinement", () => {
      ...
      strictEqual(
        String(schema.ast),
        `greaterThan(0) & Brand<"> 0"> & lessThan(2) & Brand<"< 2">`
      )
...
└─ ["nested"]
  ...
  describe("TypeLiteralTransformation", () => {
    ...
    it("TypeLiteralTransformation & Refinement", async () => {
      ...
      const B = Schema.Struct({ b: Schema.String }).pipe(
        Schema.filter(() => true)
      )
      assertExtend(A, B, [
    ...
    it("TypeLiteralTransformation & TypeLiteralTransformation", async () => {
      ...
      const B = Schema.Struct({
        c: Schema.optionalWith(Schema.Number, {
          exact: true,
          default: () => 0
        }),
        d: Schema.Boolean
...
└─ Encoded side transformation failure
  ...
  describe("Union", () => {
    ...
    it("Union of structs with defaults & Union of structs with defaults", async () => {
      ...
      const B = Schema.Union(
        Schema.Struct({
          e: Schema.optionalWith(Schema.String, {
            exact: true,
            default: () => "e"
          }),
          f: Schema.String
        ...
        Schema.Struct({
          g: Schema.optionalWith(Schema.String, {
            exact: true,
            default: () => "g"
          }),
          h: Schema.String
  ...
  describe("Refinements", () => {
    it("Struct & Refinement", async () => {
      ...
      const schema = Schema.extend(A, B)
      await Util.assertions.decoding.fail(schema, { a: "a", b: -1 }, `R filter`)
    })
    ...
    it("Struct & Refinement (two filters)", async () => {
      ...
      const schema = Schema.extend(A, B)
      await Util.assertions.decoding.fail(schema, { a: "a", b: -1 }, `filter1`)
      await Util.assertions.decoding.fail(schema, { a: "a", b: 11 }, `filter2`)
    })

    it("Refinement & Refinement", async () => {
      const A = Schema.Struct({ a: Schema.String }).pipe(
        Schema.filter((input) => input.a.length > 0, {
          message: () => "R1 filter"
        })
      )
      const B = Schema.Struct({ b: Schema.Number }).pipe(
      ...
      const schema = Schema.extend(A, B)
      await Util.assertions.decoding.fail(schema, { a: "", b: 1 }, `R1 filter`)
      await Util.assertions.decoding.fail(
        schema,
        { a: "a", b: -1 },
        `R2 filter`
    ...
    it("Union of structs & Refinement", async () => {
      ...
      const B = Schema.Struct({ c: Schema.Boolean }).pipe(
        Schema.filter((input) => input.c === true, {
          message: () => "R filter"
        })
      )
    ...
    throws(
      () =>
        Schema.Record({
          ...
          Schema.extend(
            Schema.Record({ key: Schema.SymbolFromSelf, value: Schema.Boolean })
          )
        ),
      new Error(`Duplicate index signature
