...
describe("omit", () => {
  describe("omit specific tests", () => {
    it("Struct & Record", () => {
      ...
      const schema = S.Struct({ a: S.NumberFromString, b: S.Number }, record)
  ...
  it("Refinement", () => {
    const schema = S.Struct({ a: S.NumberFromString, b: S.Number }).pipe(
      S.filter(() => true)
    )
    const omitted = schema.pipe(S.omit("b"))
  ...
  describe("Struct", () => {
    it("required properties", () => {
      ...
      Util.assertions.ast.equals(
        omitted,
        S.Struct({ [a]: S.String, b: S.NumberFromString })
      )
    })
  ...
  it("Union", () => {
    ...
    Util.assertions.ast.equals(
      omitted,
      S.Struct({ a: S.Union(S.String, S.Number) })
    )
  })
  ...
  it("suspend", async () => {
    ...
    await Util.assertions.decoding.succeed(omitted, {
      as: [{ a: "a", as: [] }]
    })
