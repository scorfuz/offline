...
describe("getNumberIndexedAccess", () => {
  describe("Tuple", () => {
    it("decodes and encodes required elements in a tuple", async () => {
      const schema = S.getNumberIndexedAccess(
        S.Tuple(S.NumberFromString, S.DurationFromNanos)
      )
    ...
    it("decodes and encodes a tuple with an optional element", async () => {
      const schema = S.getNumberIndexedAccess(
        S.Tuple(S.NumberFromString, S.optionalElement(S.DurationFromNanos))
      )
  ...
  it("Union", async () => {
    const schema = S.getNumberIndexedAccess(
      S.Union(S.Array(S.Number), S.Array(S.String))
    )
