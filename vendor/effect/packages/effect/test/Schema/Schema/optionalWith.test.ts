...
└─ ["a"]
   └─ NumberFromString
    ...
    it("never", async () => {
      strictEqual(
        S.optionalWith(S.Never, { exact: true }).from.ast,
        AST.neverKeyword
      )
      const schema = S.Struct({
        a: S.optionalWith(S.Never, { exact: true }),
        b: S.Number
      })
...
└─ Encoded side transformation failure
  ...
  describe(`optionalWith > { as: "Option" }`, () => {
    it("decoding / encoding", async () => {
      const schema = S.Struct({
        a: S.optionalWith(S.NumberFromString, { as: "Option" })
      })
      await Util.assertions.decoding.succeed(schema, {}, { a: O.none() })
      await Util.assertions.decoding.succeed(
        schema,
  ...
  describe(`optionalWith > { as: "Option", onNoneEncoding: () => O.some(undefined) }`, () => {
    it("decoding / encoding", async () => {
      const schema = S.Struct({
        a: S.optionalWith(S.NumberFromString, {
          as: "Option",
          onNoneEncoding: () => O.some(undefined)
        })
      })
      await Util.assertions.decoding.succeed(schema, {}, { a: O.none() })
      await Util.assertions.decoding.succeed(
