...
describe("Cause", () => {
  ...
  it("decoding", async () => {
    ...
    await Util.assertions.decoding.succeed(
      ...
      Cause.die(
        new Error("error", { cause: { stack: "fail", message: "error" } })
      )
    )
    await Util.assertions.decoding.succeed(
      schema,
...
└─ Type side transformation failure
   └─ Cause<string>
    ...
    it("using the built-in Defect schema as defect argument", async () => {
      const schema = S.Cause({ error: S.NumberFromString, defect: S.Defect })
      const schemaUnknown = S.Cause({
        error: S.NumberFromString,
        defect: S.Unknown
      })

      await Util.assertions.encoding.succeed(schema, Cause.fail(1), {
        _tag: "Fail",
        error: "1"
      })
      await Util.assertions.encoding.succeed(schema, Cause.empty, {
        _tag: "Empty"
      })
      await Util.assertions.encoding.succeed(
        schema,
      ...
      await Util.assertions.encoding.succeed(
        schema,
        Cause.interrupt(
          FiberId.composite(FiberId.runtime(1, 1000), FiberId.none)
        ),
        {
          _tag: "Interrupt",
          fiberId: {
