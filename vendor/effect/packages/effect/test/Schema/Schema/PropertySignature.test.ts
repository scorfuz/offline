...
describe("PropertySignature", () => {
  ...
  describe("annotations", () => {
    it("propertySignature(S.string)", () => {
      const schema = S.Struct({
        a: S.propertySignature(S.String)
          .annotations({ description: "a description" })
    ...
    it("propertySignature(S.NumberFromString)", () => {
      const schema = S.Struct({
        a: S.propertySignature(S.NumberFromString)
          .annotations({ description: "a description" })
          .annotations({
            title: "a title"
          })
    ...
    it("optionalWith(S.string, { default })", () => {
      const schema = S.Struct({
        a: S.optionalWith(S.String, { default: () => "" })
          .annotations({ description: "a description" })
          .annotations({
            title: "a title"
          })
  ...
  it("add a decoding default to an optional field", async () => {
    const ps: S.PropertySignature<":", number, never, "?:", string, never> =
      S.makePropertySignature(
        new S.PropertySignatureTransformation(
          new S.FromPropertySignature(
            S.NumberFromString.ast,
            true,
            true,
            {},
            undefined
...
└─ Encoded side transformation failure
  ...
  it("empty string as optional", async () => {
    const ps: S.PropertySignature<"?:", string, never, ":", string, never> =
      S.makePropertySignature(
        new S.PropertySignatureTransformation(
  ...
  it("encoding default", async () => {
    const ps: S.PropertySignature<"?:", number, never, ":", number, never> =
      S.makePropertySignature(
        new S.PropertySignatureTransformation(
