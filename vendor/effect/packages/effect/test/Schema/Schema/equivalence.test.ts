...
export const propertyType = <A, I>(
  ...
  const symmetry = fc.property(
    arb,
    arb,
    (a, b) => equivalence(a, b) === equivalence(b, a)
  )
...
schema (NeverKeyword): never`)
  ...
  describe("tuple", () => {
    ...
    describe("optional element support", () => {
      ...
      it("e? e?", () => {
        const schema = S.Tuple(
          S.optionalElement(MyString),
          S.optionalElement(MyNumber)
        )
        const equivalence = S.equivalence(schema)
