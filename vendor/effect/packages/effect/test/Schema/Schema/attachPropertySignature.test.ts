...
describe("attachPropertySignature", () => {
  ...
  it("with a transformation", async () => {
    const From = S.Struct({
      radius: S.Number,
      _isVisible: S.optionalWith(S.Boolean, { exact: true })
    })
    const To = S.Struct({ radius: S.Number, _isVisible: S.Boolean })

    const schema = S.transformOrFail(From, To, {
      strict: true,
      decode: (input) =>
        ParseResult.mapError(S.decodeUnknown(To)(input), (e) => e.issue),
      encode: ({ _isVisible, ...rest }) => ParseResult.succeed(rest)
    }).pipe(S.attachPropertySignature("_tag", "Circle"))

    await Util.assertions.decoding.succeed(
      schema,
      { radius: 10, _isVisible: true },
      {
...
└─ Type side transformation failure
  ...
  it("decoding error message", async () => {
    const schema = S.Struct({
      ...
      .pipe(S.attachPropertySignature("_tag", "a"))
