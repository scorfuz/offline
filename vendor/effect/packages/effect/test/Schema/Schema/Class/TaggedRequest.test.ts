...
describe("TaggedRequest", () => {
  ...
  it("TaggedRequest extends SerializableWithExit", () => {
    ...
    deepStrictEqual(S.serialize(req).pipe(Effect.runSync), {
      _tag: "MyRequest",
      id: 1
    })
    assertTrue(
    ...
    deepStrictEqual(
      S.deserializeExit(req, { _tag: "Success", value: "123" }).pipe(
        Effect.runSync
      ),
      Exit.succeed(123)
    )
  ...
  it("TaggedRequest context", () => {
    ...
    deepStrictEqual(
      S.serialize(req).pipe(Effect.provideService(Id, 1), Effect.runSync),
      { _tag: "MyRequest", id: 1 }
    )
    deepStrictEqual(
    ...
    deepStrictEqual(
      S.deserializeExit(req, {
        _tag: "Failure",
        cause: { _tag: "Fail", error: "fail" }
      }).pipe(Effect.provideService(Name, "fail"), Effect.runSync),
      Exit.fail("fail")
    )
  })
  ...
  describe("should support annotations when declaring the Class", () => {
    it("single argument", async () => {
      class A extends S.TaggedRequest<A>()(
        "A",
        {
          failure: S.String,
          success: S.Number,
