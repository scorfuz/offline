   ...
   └─ NonEmptyString
    ...
    it("should support defaults", () => {
      ...
      class A extends S.Class<A>("A")({
        ...
        [b]: S.propertySignature(S.Number).pipe(
    ...
    it("should support lazy defaults", () => {
      let i = 0
      class A extends S.Class<A>("A")({
        a: S.propertySignature(S.Number).pipe(
          S.withConstructorDefault(() => ++i)
        )
      }) {}
    ...
    it("should treat `undefined` as missing field", () => {
      class A extends S.Class<A>("A")({
        a: S.propertySignature(S.UndefinedOr(S.String)).pipe(
          S.withConstructorDefault(() => "")
        )
      }) {}
...
details: Duplicate key "a"`)
  ...
  it("can be extended with TaggedClass fields", () => {
    ...
    deepStrictEqual(
      { ...new D({ a: "a", b: "b", c: true }) },
      { _tag: "D", a: "a", b: "b", c: true }
    )
  })
  ...
  it("should accept a refinement of a Struct as argument", async () => {
    ...
    class A extends S.Class<A>("A")(
      S.Struct(fields).pipe(
        S.filter(({ a, b }) => (a === b ? undefined : "a should be equal to b"))
      )
    ) {}
    Util.expectFields(A.fields, fields)
...
└─ Predicate refinement failure
  ...
  it("Data.Class", () => {
    ...
    strictEqual(
      String(personAge),
      `PersonWithAge({ "id": 1, "name": "John", "age": 30 })`
    )
  ...
  describe("encode", () => {
    ...
    it("class a class with a getter", async () => {
      ...
      await Util.assertions.encoding.succeed(B, new A({ n: 1 }), {
        n: "1",
        s: "s"
      })
    })
