...
describe("TaggedClass", () => {
  ...
  it("should expose the fields and the tag", () => {
    ...
    deepStrictEqual(S.Struct(TA.fields).make({ a: "a" }), {
      _tag: "TA",
      a: "a"
    })
  ...
  it("a custom _tag field should be not allowed", () => {
    throws(
      () => {
        class _TA extends S.TaggedClass<_TA>()("TA", {
          _tag: S.Literal("X"),
          a: S.String
        }) {}
...
└─ Encoded side transformation failure
  ...
  it("encoding", async () => {
    class TA extends S.TaggedClass<TA>()("TA", { a: S.NonEmptyString }) {}
    await Util.assertions.encoding.succeed(TA, new TA({ a: "a" }), {
      _tag: "TA",
      a: "a"
    })
    await Util.assertions.encoding.succeed(TA, { _tag: "TA", a: "a" } as any, {
      _tag: "TA",
      a: "a"
    })
...
└─ Encoded side transformation failure
  ...
  it("baseline", () => {
    ...
    Util.assertions.parseError(
      () =>
        S.decodeUnknownSync(TaggedPersonWithAge)({
          id: 1,
          name: "John",
          age: 30
        }),
