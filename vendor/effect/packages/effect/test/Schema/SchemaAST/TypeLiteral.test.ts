...
details: An index signature parameter type must be \`string\`, \`symbol\`, a template literal type or a refinement of the previous types`
  ...
  describe("toString", () => {
    it("Struct (immutable)", () => {
      strictEqual(
        S.Struct({ a: S.String, b: S.Number }).ast.toString(),
    ...
    it("Struct (mutable)", () => {
      strictEqual(
        S.mutable(S.Struct({ a: S.String, b: S.Number })).ast.toString(),
        `{ a: string; b: number }`
      )
    })
    ...
    it("Record (immutable)", () => {
      strictEqual(
        S.Record({ key: S.String, value: S.Number }).ast.toString(),
    ...
    it("Record (mutable)", () => {
      strictEqual(
        S.mutable(S.Record({ key: S.String, value: S.Number })).ast.toString(),
        `{ [x: string]: number }`
      )
    })
  })
