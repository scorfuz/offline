...
describe("guards", () => {
  ...
  it("isTemplateLiteral", () => {
    assertTrue(
      AST.isTemplateLiteral(S.TemplateLiteral(S.Literal("a"), S.String).ast)
    )
  ...
  it("isUniqueSymbol", () => {
    assertTrue(
      AST.isUniqueSymbol(
        S.UniqueSymbolFromSelf(Symbol.for("effect/Schema/test/a")).ast
      )
    )
  ...
  it("isParameter", () => {
    ...
    assertTrue(
      AST.isParameter(S.TemplateLiteral(S.String, S.Literal("-"), S.String).ast)
    )
    assertTrue(AST.isParameter(S.String.pipe(S.minLength(2)).ast))
    assertTrue(
      AST.isParameter(
        S.TemplateLiteral(S.Literal("a", "b"), S.Literal("c")).ast
      )
    )
