...
describe("AST.Union", () => {
  it("flatten should un-nest union members", () => {
    const asts = AST.flatten([
      S.Union(S.Literal("a", "b"), S.Literal("c", "d")).ast
    ])
  ...
  it("unify should remove never from members", () => {
    strictEqual(
      AST.Union.unify([AST.neverKeyword, AST.neverKeyword]),
      AST.neverKeyword
    )
    strictEqual(
      AST.Union.unify([AST.neverKeyword, AST.stringKeyword]),
      AST.stringKeyword
    )
    strictEqual(
      AST.Union.unify([AST.stringKeyword, AST.neverKeyword]),
      AST.stringKeyword
    )
    deepStrictEqual(
      AST.Union.unify([
  ...
  describe("toString", () => {
    ...
    it("descriptions of nested unions should be preserved", () => {
      ...
      strictEqual(
        String(S.Union(nested1, nested2, S.String)),
