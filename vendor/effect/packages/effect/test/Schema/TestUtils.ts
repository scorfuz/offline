...
export const NumberFromChar = S.Char.pipe(
...
export const AsyncDeclaration = S.declare(
  ...
  {
    decode: () => (u) =>
      Effect.andThen(Effect.sleep("10 millis"), Effect.succeed(u)),
    encode: () => (u) =>
      Effect.andThen(Effect.sleep("10 millis"), Effect.succeed(u))
  },
  {
...
export const AsyncString = effectify(S.String).annotations({
  identifier: "AsyncString"
})

const Name = Context.GenericTag<"Name", string>("Name")

export const DependencyString = S.transformOrFail(S.String, S.String, {
  strict: true,
  decode: (s) => Effect.andThen(Name, s),
  encode: (s) => Effect.andThen(Name, s)
}).annotations({ identifier: "DependencyString" })

export const expectFields = (f1: S.Struct.Fields, f2: S.Struct.Fields) => {
  const ks1 = Reflect.ownKeys(f1)
    .sort()
    .map((k) => [k, f1[k].ast.toString()])
  const ks2 = Reflect.ownKeys(f2)
    .sort()
    .map((k) => [k, f2[k].ast.toString()])
...
function effectifyDecode<R>(
  ...
  return (fromA, options, ast, fromI) =>
    ParseResult.flatMap(Effect.sleep("10 millis"), () =>
  ...
  switch (ast._tag) {
    case "TupleType":
      return new AST.TupleType(
        ...
        ast.rest.map(
          (annotatedAST) =>
            new AST.Type(
              effectifyAST(annotatedAST.type),
    ...
    case "TypeLiteral":
      return new AST.TypeLiteral(
        ...
        ast.indexSignatures.map((is) => {
          return new AST.IndexSignature(
            is.parameter,
            effectifyAST(is.type),
            is.isReadonly
          )
        }),
        ast.annotations
    ...
    case "Union":
      return AST.Union.make(
        ast.types.map((ast) => effectifyAST(ast)),
        ast.annotations
      )
    case "Suspend":
    ...
    case "Transformation":
      return new AST.Transformation(
        ...
        new AST.FinalTransformation(
          effectifyDecode(
            ParseResult.getFinalTransformation(ast.transformation, true)
          ),
          effectifyDecode(
            ParseResult.getFinalTransformation(ast.transformation, false)
          )
        ),
        ast.annotations
      )
    ...
    new AST.FinalTransformation(
      (a, options) =>
        Effect.flatMap(Effect.sleep("10 millis"), () =>
          ParseResult.mapError(decode(a, options), (e) => e.issue)
        ),
      (a, options) =>
        Effect.flatMap(Effect.sleep("10 millis"), () =>
          ParseResult.mapError(encode(a, options), (e) => e.issue)
        )
    )
  )
}
