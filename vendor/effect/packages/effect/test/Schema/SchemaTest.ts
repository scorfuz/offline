...
export type UntypedParameters = Omit<
  FastCheck.Parameters<any>,
  "examples" | "reporter" | "asyncReporter"
>

// Configuration context for assertion behaviors
export class AssertConfig extends Context.Tag("AssertConfig")<
  AssertConfig,
  {
    readonly arbitrary?: {
...
export class Assert extends Context.Tag("Assert")<
  Assert,
  {
...
export const assertions = Effect.gen(function* () {
  ...
  const out = {
    ast: {
      equals: <A, I, R>(
        a: Schema.Schema<A, I, R>,
        b: Schema.Schema<A, I, R>
      ) => {
    ...
    arbitrary: {
      ...
      validateGeneratedValues<A, I, R>(
        ...
        const params = Predicate.isObject(
          config.arbitrary?.validateGeneratedValues
        )
          ? {
              ...config.arbitrary?.validateGeneratedValues?.params,
              ...options?.params
            }
          : options?.params
     ...
     * is equal to `a`.
     */
    testRoundtripConsistency<A, I>(
      schema: Schema.Schema<A, I, never>,
      options?: {
        readonly ignoreEncodingErrors?:
          | ((issue: ParseResult.ParseIssue) => boolean)
          | undefined
        readonly params?: FastCheck.Parameters<[A]> | undefined
      }
    ) {
      ...
      FastCheck.assert(
        FastCheck.property(arb, (a) => {
          const roundtrip = encode(a).pipe(
            ...
            Effect.flatMap((i) =>
              decode(i).pipe(
                Effect.mapError((issue) => ["decoding", issue] as const)
              )
            ),
            Effect.either,
            Effect.runSync
    ...
    decoding: {
      ...
      async succeed<const A, I>(
        ...
        const decoded = ParseResult.decodeUnknown(schema)(
          input,
    ...
    encoding: {
      ...
      async succeed<const A, const I>(
        ...
        const encoded = ParseResult.encodeUnknown(schema)(
          input,
          options?.parseOptions
        )
        return out.effect.succeed(
      ...
      async fail<A>(promise: Promise<A>, message: string) {
        try {
          ...
          if (
            ...
            Cause.isCause(e[Runtime.FiberFailureCauseId])
          ) {
            const cause = e[Runtime.FiberFailureCauseId]
            if (
              Cause.isFailType(cause) &&
              Predicate.hasProperty(cause.error, "message")
            ) {
    ...
    effect: {
       ...
       */
      async succeed<const A, E>(effect: Effect.Effect<A, E>, a: A) {
        deepStrictEqual(
      ...
      async fail<A>(
        ...
        const effectWithMessage = Effect.gen(function* () {
          ...
          if (Either.isLeft(decoded)) {
            const message = yield* ParseResult.TreeFormatter.formatIssue(
              decoded.left
            )
    ...
    either: {
       ...
       */
      async fail<R>(
        either: Either.Either<R, ParseResult.ParseIssue>,
        message: string
      ) {
    ...
    asserts: {
      succeed<A, I, R>(
        ...
        deepStrictEqual(
          Schema.asserts(schema, options?.parseOptions)(input),
          undefined
        )
      },
