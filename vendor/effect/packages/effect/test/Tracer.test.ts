...
describe("Tracer", () => {
  it.effect("includes trace when errored", () =>
    Effect.gen(function* () {
      ...
      assertInclude(
        maybeSpan!.attributes.get("code.stacktrace") as string,
        "Tracer.test.ts:22:26"
      )
    })
  )
  ...
  describe("withSpan", () => {
    ...
    it.effect("parent", () =>
      Effect.gen(function* () {
        const span = yield* Effect.withSpan("B")(
          Effect.withSpan("A")(Effect.currentSpan)
        )

        deepStrictEqual(span.name, "A")
        deepStrictEqual(
          Option.map(span.parent, (span) => (span as Span).name),
          Option.some("B")
        )
      })
    )
    ...
    it.effect("parent when root is set", () =>
      Effect.gen(function* () {
        const span = yield* Effect.withSpan("B")(
          Effect.withSpan("A", { root: true })(Effect.currentSpan)
        )
    ...
    it.effect("external parent", () =>
      Effect.gen(function* () {
        const span = yield* Effect.withSpan("A", {
          parent: {
            _tag: "ExternalSpan",
            spanId: "000",
            traceId: "111",
        ...
        deepStrictEqual(span.name, "A")
        deepStrictEqual(
          Option.map(span.parent, (span) => span.spanId),
          Option.some("000")
        )
      })
    )
    ...
    it.effect("correct time", () =>
      Effect.gen(function* () {
        const spanFiber = yield* Effect.fork(
          Effect.withSpan("A")(
            Effect.delay(Duration.seconds(1))(Effect.currentSpan)
          )
        )
  ...
  it.effect("annotateSpans", () =>
    Effect.gen(function* () {
      const span = yield* Effect.annotateSpans(
        Effect.withSpan("A")(Effect.currentSpan),
        "key",
        "value"
      )
  ...
  it.effect("logger", () =>
    Effect.gen(function* () {
      ...
      deepStrictEqual(span.name, "A")
      assertNone(span.parent)
      deepStrictEqual((span as NativeSpan).events, [
        [
          "event",
          10000n,
          {
  ...
  it.effect("withParentSpan", () =>
    Effect.gen(function* () {
      const span = yield* Effect.currentSpan
      deepStrictEqual(
        span.parent.pipe(Option.map((_) => _.spanId)),
        Option.some("456")
      )
    }).pipe(
      Effect.withSpan("A"),
  ...
  it.effect("Layer.parentSpan", () =>
    Effect.gen(function* () {
      ...
      const parent = yield* Option.filter(
        span.parent,
        (span): span is Span => span._tag === "Span"
      )
  ...
  it.effect("linkSpans", () =>
    Effect.gen(function* () {
      ...
      const currentSpan = yield* pipe(
        Effect.currentSpan,
        Effect.withSpan("A", {
          links: [{ _tag: "SpanLink", span: childB, attributes: {} }]
        }),
