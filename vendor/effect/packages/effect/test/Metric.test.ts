...
const makePollingGauge = (name: string, increment: number) => {
  ...
  const metric = MetricPolling.make(
    gauge,
    Metric.value(gauge).pipe(Effect.map((gauge) => gauge.value + increment))
  )
...
describe("Metric", () => {
  describe("Counter", () => {
    ...
    it.effect("cannot decrement incremental", () =>
      Effect.gen(function* () {
        const name = nextName()
        const counter = Metric.counter(name, { incremental: true }).pipe(
          Metric.taggedWithLabels(labels)
        )
        const result = yield* Metric.increment(counter).pipe(
          Effect.zipRight(Metric.increment(counter)),
    ...
    it.effect("custom increment by bigint value as aspect", () =>
      Effect.gen(function* () {
        const name = nextName()
        const counter = Metric.counter(name, { bigint: true }).pipe(
          Metric.taggedWithLabels(labels)
        )
    ...
    it.effect("custom increment referential transparency", () =>
      Effect.gen(function* () {
        ...
        const result = yield* pipe(
          Effect.succeed(10),
          Effect.withMetric(
            pipe(Metric.counter(name), Metric.taggedWithLabels(labels))
          ),
          Effect.zipRight(
            pipe(
              Effect.succeed(5),
    ...
    it.effect("count + taggedWith", () =>
      Effect.gen(function* () {
        ...
        const base = pipe(
          Metric.counter(name),
          Metric.tagged("static", "0"),
          Metric.withConstantInput(1)
        )
        const counter = pipe(
          base,
          Metric.taggedWithLabelsInput((input: string) => [
            MetricLabel.make("dyn", input)
          ])
        )
        const result = yield* pipe(
          Effect.succeed("hello"),
          Effect.withMetric(counter),
          Effect.zipRight(
            pipe(Effect.succeed("!"), Effect.withMetric(counter))
          ),
          Effect.zipRight(
            pipe(Effect.succeed("!"), Effect.withMetric(counter))
          ),
    ...
    it.effect("tags are a region setting", () =>
      Effect.gen(function* () {
        ...
        const result = yield* pipe(
          ...
          Effect.zipRight(
            pipe(counter, Metric.tagged("key", "value"), Metric.value)
          )
        )
        deepStrictEqual(result, MetricState.counter(1))
      })
  ...
  describe("Frequency", () => {
    it.effect("custom occurrences as aspect", () =>
      Effect.gen(function* () {
        ...
        const result = yield* pipe(
          ...
          Effect.zipRight(
            pipe(Effect.succeed("hello"), Effect.withMetric(frequency))
          ),
          Effect.zipRight(
            pipe(Effect.succeed("world"), Effect.withMetric(frequency))
          ),
          Effect.zipRight(Metric.value(frequency))
        )
        deepStrictEqual(
          result.occurrences,
          new Map([["hello", 2] as const, ["world", 1] as const])
        )
      })
    )
    ...
    it.effect("custom occurrences with mapInput", () =>
      Effect.gen(function* () {
        ...
        const result = yield* pipe(
          ...
          Effect.withMetric(frequency),
          Effect.zipRight(
            pipe(Effect.succeed(1), Effect.withMetric(frequency))
          ),
          Effect.zipRight(
            pipe(Effect.succeed(2), Effect.withMetric(frequency))
          ),
          Effect.zipRight(Metric.value(frequency))
        )
        deepStrictEqual(
          result.occurrences,
          new Map([["1", 2] as const, ["2", 1] as const])
        )
      })
    )
    it.effect("occurences + taggedWith", () =>
      Effect.gen(function* () {
        ...
        const frequency = pipe(
          base,
          Metric.taggedWithLabelsInput((s: string) => [
            MetricLabel.make("dyn", s)
          ])
        )
    ...
    it.effect("custom observe as aspect", () =>
      Effect.gen(function* () {
        ...
        const result = yield* pipe(
          ...
          Effect.zipRight(
            pipe(Effect.succeed(3), Effect.withMetric(histogram))
          ),
    ...
    it.effect("custom observe with mapInput", () =>
      Effect.gen(function* () {
        ...
        const result = yield* pipe(
          ...
          Effect.withMetric(histogram),
          Effect.zipRight(
            pipe(Effect.succeed("xyz"), Effect.withMetric(histogram))
          ),
          Effect.zipRight(Metric.value(histogram))
        )
        strictEqual(result.count, 2)
  ...
  describe("Summary", () => {
    ...
    it.effect("custom observe with mapInput", () =>
      Effect.gen(function* () {
        ...
        const result = yield* pipe(
          ...
          Effect.withMetric(summary),
          Effect.zipRight(
            pipe(Effect.succeed("xyz"), Effect.withMetric(summary))
          ),
          Effect.zipRight(Metric.value(summary))
        )
        strictEqual(result.count, 2)
    ...
    it.effect(
      "should return correct quantile when first chunk overshoots",
      () =>
        Effect.gen(function* () {
          const name = nextName()
          ...
          yield* Effect.forEach(
            samples,
            (value) => Metric.update(summary, value),
            { discard: true }
          )
  ...
  describe("Polling", () => {
    it.scopedLive("launch should be interruptible", () =>
      Effect.gen(function* () {
        ...
        const schedule = pipe(
          Schedule.forever,
          Schedule.delayed(() => Duration.millis(250))
        )
    ...
    it.scoped(
      "launch should update the internal metric using the provided Schedule",
      () =>
        Effect.gen(function* () {
          const name = yield* pipe(
            Clock.currentTimeMillis,
    ...
    it.scoped(
      "collectAll should generate a metric that polls all the provided metrics",
      () =>
        Effect.gen(function* () {
          ...
          const fiber = yield* pipe(
            metric,
            MetricPolling.launch(Schedule.recurs(pollingCount))
          )
          yield* Fiber.join(fiber)
  ...
  it.effect("with a description", () =>
    Effect.gen(function* () {
      ...
      const pair1 = yield* Array.findFirst(snapshot, (key) =>
        Equal.equals(key.metricKey, MetricKey.counter(name))
      )
    ...
    it.effect("infers types in Effectful pipes", () => {
      ...
      return Effect.Do.pipe(
        ...
        Effect.map(({ results }) => {
          ...
          deepStrictEqual(
            results.summary.quantiles.map((x) => x[1]).map(Option.getOrNull),
            [1, 1, 4]
          )
          deepStrictEqual(
