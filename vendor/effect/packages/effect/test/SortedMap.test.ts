...
function makeSortedMap(
  ...numbers: Array<readonly [number, number]>
): SM.SortedMap<Key, Value> {
  const entries = numbers.map(([k, v]) => [key(k), value(v)] as const)
  return SM.fromIterable(entries, (self: Key, that: Key) =>
    self.id > that.id ? 1 : self.id < that.id ? -1 : 0
  )
}
...
function makeNumericSortedMap(
  ...
  return SM.fromIterable(numbers, (self: number, that: number) =>
...
describe("SortedMap", () => {
  ...
  it("map", () => {
    ...
    const result2 = Array.from(
      pipe(
        map2,
        SM.map((key, value) => key.id + value.id)
      )
    )
  ...
  it("partition", () => {
    ...
    deepStrictEqual(Array.from(satisfying), [
      [key(1), value(10)],
      [key(2), value(20)],
      [key(3), value(30)]
    ])
    deepStrictEqual(Array.from(excl), [
      [key(4), value(40)],
      [key(5), value(50)]
    ])
    ...
    deepStrictEqual(Array.from(satisfying2), [
      [key(1), value(10)],
      [key(2), value(20)],
      [key(3), value(30)],
      [key(4), value(40)],
    ...
    deepStrictEqual(Array.from(excl2), [])
    ...
    deepStrictEqual(Array.from(excl3), [
      [key(1), value(10)],
      [key(2), value(20)],
      [key(3), value(30)],
      [key(4), value(40)],
    ...
    deepStrictEqual(Array.from(satisfying3), [])
  })
  ...
  it("reduce", () => {
    ...
    const result1 = pipe(
      map1,
      SM.reduce("", (acc, value) => acc + value.id)
    )
    ...
    const result2 = pipe(
      map2,
      SM.reduce("", (acc, value, key) => acc + key.id + value.id)
    )
