...
describe("Queue", () => {
  ...
  it.effect("dropping - with offerAll, check ordering", () =>
    Effect.gen(function* () {
      ...
      const result1 = yield* Queue.offerAll(
        queue,
        Array.makeBy(256, (i) => i + 1)
      )
      const result2 = yield* Queue.takeAll(queue)
      assertFalse(result1)
      deepStrictEqual(
        Chunk.toReadonlyArray(result2),
        Array.makeBy(128, (i) => i + 1)
      )
    })
  )
  ...
  it.effect("awaitShutdown - once", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(
        ...
        Effect.zipRight(Deferred.succeed(deferred, true)),
        Effect.fork
      )
  ...
  it.effect("awaitShutdown - multiple", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(
        Queue.awaitShutdown(queue),
        Effect.zipRight(Deferred.succeed(deferred1, true)),
        Effect.fork
      )
      yield* pipe(
        Queue.awaitShutdown(queue),
        Effect.zipRight(Deferred.succeed(deferred2, true)),
        Effect.fork
      )
      yield* Queue.shutdown(queue)
  ...
  it.effect("offers are suspended by back pressure", () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* pipe(
        Queue.offer(queue, 2),
        Effect.zipRight(Ref.set(ref, false)),
        Effect.fork
      )
  ...
  it.effect("back pressured offers are retrieved", () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* Effect.forkAll(
        values.map((n) => Queue.offer(queue, n))
      )
  ...
  it.effect("offerAll with pending takers", () =>
    Effect.gen(function* () {
      ...
      const takers = yield* Effect.forkAll(
        Array.makeBy(100, () => Queue.take(queue))
      )
  ...
  it.effect("offerAll with pending takers, check ordering", () =>
    Effect.gen(function* () {
      ...
      const takers = yield* Effect.forkAll(
        Array.makeBy(64, () => Queue.take(queue))
      )
  ...
  it.effect(
    "offerAll with pending takers, check ordering of taker resolution",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("parallel takes and sequential offers", () =>
    Effect.gen(function* () {
      const queue = yield* Queue.bounded<number>(100)
      const fiber = yield* Effect.forkAll(
        Array.makeBy(10, () => Queue.take(queue))
      )
      yield* Array.makeBy(10, (i) => Queue.offer(queue, i + 1)).reduce(
        (acc, curr) => pipe(acc, Effect.zipRight(curr)),
  ...
  it.effect("sequential take and offer", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(
        Queue.offer(queue, "don't "),
        Effect.zipRight(Queue.offer(queue, "give up :D"))
      )
      const result = yield* Fiber.join(fiber)
  ...
  it.effect(
    "takeBetween returns an empty list if boundaries are inverted",
    () =>
      Effect.gen(function* () {
        const queue = yield* Queue.bounded<number>(100)
        yield* Queue.offer(queue, 1)
  ...
  it.effect(
    "takeBetween returns an empty list if boundaries are negative",
    () =>
      Effect.gen(function* () {
        const queue = yield* Queue.bounded<number>(100)
        yield* Queue.offer(queue, 1)
  ...
  it.effect(
    "takeBetween blocks until a required minimum of elements is collected",
    () =>
      Effect.gen(function* () {
        const queue = yield* Queue.bounded<number>(100)
  ...
  it.effect("takeBetween returns elements in the correct order", () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* pipe(
        values,
        Effect.forEach((n) => Queue.offer(queue, n)),
        Effect.fork
      )
      const result = yield* Queue.takeBetween(
        queue,
        values.length,
        values.length
      )
  ...
  it.effect(
    "takeN returns an empty list if a negative number or zero is specified",
    () =>
      Effect.gen(function* () {
        const queue = yield* Queue.bounded<number>(100)
  ...
  it.effect(
    "takeN blocks until the required number of elements is available",
    () =>
      Effect.gen(function* () {
        const queue = yield* Queue.bounded<number>(100)
        const updater = pipe(Queue.offer(queue, 10), Effect.forever)
  ...
  it.effect(
    "should return the specified number of elements from a non-empty queue",
    () =>
      Effect.gen(function* () {
        const queue = yield* Queue.bounded<number>(100)
        yield* Queue.offer(queue, 1)
  ...
  it.effect(
    "should handle an empty queue with max higher than queue size",
    () =>
      Effect.gen(function* () {
        const queue = yield* Queue.bounded<number>(100)
