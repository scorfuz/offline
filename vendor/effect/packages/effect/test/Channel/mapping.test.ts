...
describe("Channel", () => {
  ...
  it.effect(
    "concatMap - multiple concatMaps with failure in first and catchAll in second",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Channel.write(void 0),
          Channel.concatMap(() => Channel.write(Channel.fail("error"))),
          Channel.concatMap(
            Channel.catchAllCause(() => Channel.fail("error2"))
          ),
          Channel.runCollect,
          Effect.exit
        )
  ...
  it.effect("concatMap - custom 1", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        ...
        Channel.runCollect,
        Effect.map(([chunk]) =>
          pipe(Chunk.toReadonlyArray(chunk), Array.getSomes)
        )
      )
