...
describe("Channel", () => {
  it.effect(
    "mapOut is stack safe",
    () =>
      Effect.gen(function* () {
        const N = 10_000
        const [chunk, value] = yield* pipe(
          Chunk.range(1, N),
          Chunk.reduce(Channel.write<number>(1), (channel, n) =>
            pipe(
              channel,
              Channel.mapOut((i) => i + n)
            )
          ),
          Channel.runCollect
        )
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const [chunk, value] = yield* pipe(
          Chunk.range(1, N),
          Chunk.reduce(Channel.write<number>(0), (channel, n) =>
            pipe(
              channel,
