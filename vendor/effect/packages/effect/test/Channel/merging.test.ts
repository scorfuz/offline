    ...
    Effect.gen(function* () {
      const [chunk, value] = yield* pipe(
        ...
        Channel.mergeWith({
          ...
          // TODO: remove
          onSelfDone: (leftDone) =>
            MergeDecision.AwaitConst(Effect.suspend(() => leftDone)),
          onOtherDone: (rightDone) =>
            MergeDecision.AwaitConst(Effect.suspend(() => rightDone))
        }),
        Channel.runCollect
      )
        ...
        Channel.mergeWith({
          ...
          onSelfDone: (leftDone) =>
            MergeDecision.Await((rightDone) =>
              Effect.suspend(() => Exit.zip(leftDone, rightDone))
            ),
          onOtherDone: (rightDone) =>
  ...
  it.effect("mergeWith - interrupts losing side", () =>
    Effect.gen(function* () {
      ...
      const merged = Channel.mergeWith(left, {
        ...
        // TODO: remove
        onSelfDone: (leftDone) =>
          MergeDecision.Done(Effect.suspend(() => leftDone)),
        onOtherDone: (_rightDone) =>
          MergeDecision.Done(
            pipe(
              Ref.get(interrupted),
              Effect.flatMap((isInterrupted) =>
                isInterrupted ? Effect.void : Effect.fail(void 0)
              )
            )
          )
      })
