...
export const mapper = <A, B>(
  ...
  return Channel.readWith({
    onInput: (a: A) => Channel.flatMap(Channel.write(f(a)), () => mapper(f)),
    ...
    Effect.gen(function* () {
          ...
          > =>
            Channel.readWith({
              onInput: (input: number) =>
                pipe(
                  Channel.fromEffect(
                    Ref.update(ref, (array) => [...array, input])
                  ),
      ...
      const channel = pipe(
        ...
        Channel.pipeTo(
          pipe(
            mapper(g),
            Channel.concatMap((ns) => Channel.writeAll(...ns)),
            Channel.asVoid
          )
        ),
  ...
  it.effect("reading with resources", () =>
    Effect.gen(function* () {
      ...
      const event = (label: string) =>
        Ref.update(ref, (array) => [...array, label])
      const left = pipe(
        Channel.acquireReleaseOut(event("Acquire outer"), () =>
          event("Release outer")
        ),
        Channel.concatMap(() =>
          pipe(
