  ...
  it.effect("ensuring - last finalizers are deferred to the scope", () =>
    Effect.gen(function* () {
      ...
      const [eventsInScope, eventsOutsideScope] = yield* pipe(
        ...
        Effect.flatMap((pull) =>
          pipe(Effect.exit(pull), Effect.zipRight(Ref.get(ref)))
        ),
        Effect.scoped,
  ...
  it.effect("ensuring - mixture of concatMap and ensuring", () =>
    Effect.gen(function* () {
      ...
      const channel = pipe(
        ...
        Channel.concatMap((i) =>
          pipe(Channel.write(First(i)), Channel.ensuring(event("First write")))
        ),
  ...
  it.effect("ensuring - finalizer ordering 2", () =>
    Effect.gen(function* () {
      ...
      const channel = pipe(
        ...
        Channel.concatMap((n) =>
          pipe(Channel.write(n), Channel.ensuring(event(`close ${n}`)))
        )
      )
