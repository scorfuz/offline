...
export interface WatchableLookup<Key, Value, Error = never> {
  ...
  createdResources: () => Effect.Effect<
    HashMap.HashMap<
      Key,
      Chunk.Chunk<ObservableResource.ObservableResource<Error, Value>>
    >
  >
  firstCreatedResource: (
    key: Key
  ) => Effect.Effect<ObservableResource.ObservableResource<Error, Value>>
  ...
  assertCalledTimes: (
    key: Key,
    sizeAssertion: (value: number) => void
  ) => Effect.Effect<void>
...
): Effect.Effect<WatchableLookup<Key, Value>> =>
  makeEffect((key) => Effect.succeed(concreteLookup(key)))

export const makeVoid = (): Effect.Effect<WatchableLookup<void, void>> =>
  make<void, void>((_: void) => void 0)
    ...
    ([blocked, resources]): WatchableLookup<Key, Value, Error> => {
      function lookup(key: Key): Effect.Effect<Value, Error, Scope.Scope> {
        return Effect.flatten(
          Effect.gen(function* () {
            const observableResource = yield* ObservableResource.makeEffect(
              concreteLookup(key)
            )
            yield* Ref.update(resources, (resourceMap) => {
              const newResource = pipe(
                ...
                Option.getOrElse(() =>
                  Chunk.empty<
                    ObservableResource.ObservableResource<Error, Value>
                  >()
                ),
      ...
      const firstCreatedResource = (key: Key) =>
        Effect.map(Ref.get(resources), (map) =>
          Chunk.unsafeHead(HashMap.unsafeGet(map, key))
        )
      const getCalledTimes = (key: Key) =>
        Effect.map(createdResources(), (map) =>
      ...
      const resourcesCleaned = (
        ...
        Effect.forEach(resources, (resource) =>
          Effect.suspend(() => resource.assertAcquiredOnceAndCleaned())
        )
      const assertCalledTimes = (
        key: Key,
        sizeAssertion: (value: number) => void
      ) =>
        Effect.flatMap(getCalledTimes(key), (n) =>
          Effect.sync(() => sizeAssertion(n))
        )
      ...
      const assertAtLeastOneResourceNotCleanedForKey = (key: Key) =>
        Effect.flatMap(createdResources(), (resources) => {
          ...
          return pipe(
            ...
            Effect.map((atLeastOneNotCleaned) =>
              Effect.sync(() => assertTrue(atLeastOneNotCleaned))
            )
          )
        })
