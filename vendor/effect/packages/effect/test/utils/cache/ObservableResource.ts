  ...
  assertNotAcquired(): Effect.Effect<void> {
    return Effect.map(this.getState, ([numAcquisition, numCleaned]) => {
      strictEqual(
        numAcquisition,
        0,
        "Resource acquired when it should not have"
      )
...
export const makeVoid = (): Effect.Effect<ObservableResource<never, void>> =>
  make(void 0)

export const make = <V>(
  value: V
): Effect.Effect<ObservableResource<never, V>> =>
      ...
      const scoped = Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          ...
          yield* Scope.addFinalizer(
            child,
            Ref.update(resourceAcquisitionReleasing, (n) => n + 1)
          )
