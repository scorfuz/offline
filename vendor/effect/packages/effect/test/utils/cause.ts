...
export const causesArb = <E>(
  ...
  const fiberId: fc.Arbitrary<FiberId.FiberId> = fc
  ...
  const sequential = (n: number): fc.Arbitrary<Cause.Cause<E>> => {
    return fc
      .integer({ min: 1, max: n - 1 })
      .chain((i) =>
        causesN(i).chain((left) =>
          causesN(n - i).map((right) => Cause.sequential(left, right))
        )
      )
  }
  ...
  const parallel = (n: number): fc.Arbitrary<Cause.Cause<E>> => {
    return fc
      ...
      .chain((i) =>
        causesN(i).chain((left) =>
          causesN(n - i).map((right) => Cause.parallel(left, right))
        )
      )
  }
...
export const errorCauseFunctions: fc.Arbitrary<
  (s: string) => Cause.Cause<string>
> = fc.func(causes)

export const equalCauses: fc.Arbitrary<
  readonly [Cause.Cause<string>, Cause.Cause<string>]
> = fc.tuple(causes, causes, causes).chain(([a, b, c]) => {
  ...
  > = [
    ...
    [Cause.parallel(a, b), Cause.parallel(b, a)],
    [a, Cause.sequential(a, Cause.empty)],
    [a, Cause.parallel(a, Cause.empty)]
  ]
  return fc
    .integer({ min: 0, max: causeCases.length - 1 })
    .map((i) => causeCases[i])
})
