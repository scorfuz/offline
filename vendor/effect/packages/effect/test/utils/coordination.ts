...
export const chunkCoordination = <A>(
  ...
  Effect.gen(function* () {
    ...
    const queue =
      yield* Queue.unbounded<Exit.Exit<Chunk.Chunk<A>, Option.Option<never>>>()
    const ps = yield* Queue.unbounded<void>()
    const ref = yield* Ref.make<
      Chunk.Chunk<Chunk.Chunk<Exit.Exit<Chunk.Chunk<A>, Option.Option<never>>>>
    >(
      pipe(
        chunks,
        Chunk.dropRight(1),
          ...
          pipe(
            ...
            Option.map((chunk) =>
              Chunk.unsafeFromArray<
                Exit.Exit<Chunk.Chunk<A>, Option.Option<never>>
              >([Exit.succeed(chunk), Exit.fail(Option.none())])
            ),
            Option.match({
              onNone: () =>
                Chunk.empty<
                  Chunk.Chunk<Exit.Exit<Chunk.Chunk<A>, Option.Option<never>>>
                >(),
              onSome: Chunk.of
            })
          )
