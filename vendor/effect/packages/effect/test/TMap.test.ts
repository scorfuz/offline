...
const mapEntriesArb: fc.Arbitrary<Array<readonly [string, number]>> = fc
  .uniqueArray(fc.char())
  .chain((keys) =>
    fc.uniqueArray(fc.integer()).map((values) => pipe(keys, Array.zip(values)))
  )
...
describe("TMap", () => {
  ...
  it("keys - collect all keys", () =>
    fc.assert(
      fc.asyncProperty(mapEntriesArb, async (entries) => {
        const transaction = pipe(
        ...
        strictEqual(
          pipe(result, Array.differenceWith(equivalentElements())(keys)).length,
          0
        )
        strictEqual(
          pipe(keys, Array.differenceWith(equivalentElements())(result)).length,
          0
        )
      })
    ))
      ...
      const transaction = pipe(
        ...
        STM.flatMap((map) =>
          STM.all({
            result1: pipe(
              map,
              TMap.merge("a", 2, (x, y) => x + y)
            ),
            result2: pipe(
              map,
              TMap.merge("b", 2, (x, y) => x + y)
            )
          })
        )
      )
  ...
  it.effect("removeIf > { discard: true }", () =>
    Effect.gen(function* () {
      const transaction = STM.gen(function* () {
        ...
        yield* pipe(
          map,
          TMap.removeIf((key) => key === "aa", { discard: true })
        )
        const a = yield* pipe(map, TMap.has("a"))
        const aa = yield* pipe(map, TMap.has("aa"))
        const aaa = yield* pipe(map, TMap.has("aaa"))
  ...
  it.effect("retainIf > { discard: true }", () =>
    Effect.gen(function* () {
      const transaction = STM.gen(function* () {
        ...
        yield* pipe(
          map,
          TMap.retainIf((key) => key === "aa", { discard: true })
        )
        const a = yield* pipe(map, TMap.has("a"))
        const aa = yield* pipe(map, TMap.has("aa"))
        const aaa = yield* pipe(map, TMap.has("aaa"))
  ...
  it.effect("set - add many keys with negative hash codes", () =>
    Effect.gen(function* () {
      const entries = Array.makeBy(1_000, (i) => i + 1).map(
        (i) => [new HashContainer(i), i] as const
      )
      const transaction = pipe(
        TMap.empty<HashContainer, number>(),
        STM.tap((map) =>
          STM.all(
            entries.map((entry) => pipe(map, TMap.set(entry[0], entry[1])))
          )
        ),
        STM.flatMap(TMap.toArray)
      )
      const result = yield* STM.commit(transaction)
      strictEqual(
        pipe(result, Array.differenceWith(equivalentElements())(entries))
          .length,
        0
      )
      strictEqual(
        pipe(entries, Array.differenceWith(equivalentElements())(result))
          .length,
        0
      )
    })
  ...
  it.effect("setIfAbsent", () =>
    Effect.gen(function* () {
      ...
      strictEqual(
        pipe(result, Array.differenceWith(equivalentElements())(expected))
          .length,
        0
      )
      strictEqual(
        pipe(expected, Array.differenceWith(equivalentElements())(result))
          .length,
        0
      )
    })
      ...
      fc.asyncProperty(mapEntriesArb, async (entries) => {
        ...
        strictEqual(
          pipe(
            ...
            Array.differenceWith(equivalentElements())(entries)
          ).length,
          0
        )
        strictEqual(
          pipe(
            entries,
            Array.differenceWith(equivalentElements())(
              Chunk.toReadonlyArray(result)
            )
          ).length,
          0
  ...
  it.effect("transform - handles keys with negative hash codes", () =>
    Effect.gen(function* () {
      ...
      const expected = [
        [new HashContainer(2), 2],
        [new HashContainer(4), 4],
        [new HashContainer(6), 6]
      ]
      strictEqual(
        pipe(result, Array.differenceWith(equivalentElements())(expected))
          .length,
  ...
  it.effect("transformSTM", () =>
    Effect.gen(function* () {
      const transaction = pipe(
        TMap.make(["a", 1], ["aa", 2], ["aaa", 3]),
        STM.tap((map) =>
          TMap.transformSTM(map, (key, value) =>
            STM.succeed([key.replaceAll("a", "b"), value * 2])
          )
        ),
        STM.flatMap(TMap.toArray)
      )
  ...
  it.effect("transformSTM - and shrink", () =>
    Effect.gen(function* () {
      const transaction = pipe(
        TMap.make(["a", 1], ["aa", 2], ["aaa", 3]),
        STM.tap((map) =>
          TMap.transformSTM(map, (_, value) => STM.succeed(["key", value * 2]))
        ),
        STM.flatMap(TMap.toArray)
      )
      const result = yield* STM.commit(transaction)
  ...
  it.effect("transformValuesSTM", () =>
    Effect.gen(function* () {
      const transaction = pipe(
        TMap.make(["a", 1], ["aa", 2], ["aaa", 3]),
        STM.tap((map) =>
          TMap.transformValuesSTM(map, (value) => STM.succeed(value * 2))
        ),
        STM.flatMap(TMap.toArray)
      )
      const result = yield* STM.commit(transaction)
  ...
  it.effect("updateWith", () =>
    Effect.gen(function* () {
      const transaction = STM.gen(function* () {
        ...
        yield* pipe(
          map,
          TMap.updateWith<string, number>("b", () => Option.none())
        )
        yield* pipe(
          map,
          TMap.updateWith("c", () => Option.some(3))
        )
        yield* pipe(
          map,
          TMap.updateWith<string, number>("d", () => Option.none())
        )
  ...
  it("values - collect all values", () =>
    fc.assert(
      fc.asyncProperty(mapEntriesArb, async (entries) => {
        ...
        strictEqual(
          pipe(result, Array.differenceWith(equivalentElements())(values))
            .length,
          0
        )
        strictEqual(
          pipe(values, Array.differenceWith(equivalentElements())(result))
            .length,
          0
        )
      })
