...
const chain =
  (depth: number) =>
  (next: (stm: STM.STM<number>) => STM.STM<number>): Effect.Effect<number> => {
...
const chainError = (depth: number): Effect.Effect<never, number> => {
  const loop = (
    _n: number,
    _acc: STM.STM<never, number, never>
...
describe("STM", () => {
  ...
  it.effect("collectAll - ordering", () =>
    Effect.gen(function* () {
      const transaction = pipe(
        ...
        STM.flatMap((queue) =>
          STM.all(Array.from({ length: 3 }, () => TQueue.take(queue)))
        )
      )
      const result = yield* STM.commit(transaction)
  ...
  it.effect(
    "collectAll - collects a list of transactional effects to a single transaction",
    () =>
      Effect.gen(function* () {
        const chunk: Chunk.Chunk<number> = Chunk.range(1, 100)
        const iterable = yield* Effect.succeed(
          pipe(chunk, Chunk.map(TRef.make))
        )
        const refs = yield* STM.all(iterable)
        const result = yield* Effect.forEach(refs, TRef.get, {
    ...
    Effect.gen(function* () {
      ...
      const transaction = STM.gen(function* () {
        ...
        const results = yield* pipe(
          array,
          STM.filter((n) =>
            pipe(ref, TRef.update(Chunk.append(n)), STM.as(n % 2 === 0))
          )
        )
  ...
  it.effect(
    "forEach - performs an action on each chunk element and return a single transaction",
    () =>
      Effect.gen(function* () {
        ...
        const expected = pipe(
          chunk,
          Chunk.reduceRight(0, (acc, curr) => acc + curr)
        )
  ...
  it.effect("fold - handles both failure and success", () =>
    Effect.gen(function* () {
      const transaction = STM.all({
        success: pipe(
          STM.succeed("yes"),
          STM.match({ onFailure: () => -1, onSuccess: () => 1 })
        ),
        failure: pipe(
          STM.fail("no"),
          STM.match({ onFailure: () => -1, onSuccess: () => 1 })
  ...
  it.effect(
    "foldSTM - folds over the `STM` effect, and handle failure and success",
    () =>
      Effect.gen(function* () {
        const transaction = STM.all({
          success: pipe(
          ...
          failure: pipe(
            STM.fail("no"),
            STM.matchSTM({
              onFailure: STM.succeed,
              onSuccess: () => STM.succeed("yes")
            })
          )
        })
        const { failure, success } = yield* STM.commit(transaction)
  ...
  it.effect("mapBoth - success value", () =>
    Effect.gen(function* () {
      const transaction = pipe(
        STM.succeed(1),
        STM.mapBoth({ onFailure: () => -1, onSuccess: (n) => `${n} as string` })
      )
      const result = yield* STM.commit(transaction)
  ...
  it.effect("mapBoth - success value", () =>
    Effect.gen(function* () {
      const transaction = pipe(
        STM.fail(-1),
        STM.mapBoth({ onFailure: (n) => `${n} as string`, onSuccess: () => 0 })
      )
      const result = yield* Effect.exit(STM.commit(transaction))
  ...
  it.effect(
    "orElseEither - orElseEither returns result of the first successful transaction",
    () =>
      Effect.gen(function* () {
        const result1 = yield* pipe(
          STM.retry,
          STM.orElseEither(() => STM.succeed(42))
        )
        const result2 = yield* pipe(
          STM.succeed(1),
          STM.orElseEither(() => STM.succeed("no"))
        )
        const result3 = yield* pipe(
          STM.succeed(2),
          STM.orElseEither(() => STM.retry)
        )
  ...
  it.effect(
    "orElseFail - fails with the specified error once left retries",
    () =>
      Effect.gen(function* () {
        const transaction = pipe(
          STM.retry,
  ...
  it.effect(
    "orElseSucceed - succeeds with the specified error once left retries",
    () =>
      Effect.gen(function* () {
        const transaction = pipe(
          STM.retry,
  ...
  it.effect(
    "orElseSucceed - succeeds with the specified error once left fails",
    () =>
      Effect.gen(function* () {
        const transaction = pipe(
          STM.fail(true),
  ...
  it.effect("unsome - converts Some in E to error in E", () =>
    Effect.gen(function* () {
      const transaction = STM.unsome(
        STM.fromEither(Either.left(Option.some("Ouch")))
      )
      const result = yield* Effect.exit(STM.commit(transaction))
      deepStrictEqual(result, Exit.fail("Ouch"))
    })
  ...
  it.effect("partition - collects successes and failures", () =>
    Effect.gen(function* () {
      ...
      const transaction = pipe(
        input,
        STM.partition((n) => (n % 2 === 0 ? STM.fail(n) : STM.succeed(n)))
      )
      const [left, right] = yield* STM.commit(transaction)
  ...
  it.effect("partition - evaluates effects in the correct order", () =>
    Effect.gen(function* () {
      ...
      const transaction = STM.gen(function* () {
        ...
        yield* pipe(
          input,
          STM.partition((n) => pipe(ref, TRef.update(Chunk.append(n))))
        )
  ...
  it("reduce - with a successful step function sums the list properly", () =>
    fc.assert(
      fc.asyncProperty(fc.array(fc.integer()), async (array) => {
        const transaction = pipe(
          array,
          STM.reduce(0, (acc, curr) => STM.succeed(acc + curr))
        )
    ...
    fc.assert(
      fc.asyncProperty(
        ...
        async (array) => {
          const transaction = pipe(
            array,
            STM.reduce(0, () => STM.fail("Ouch"))
          )
          const result = await Effect.runPromise(
            Effect.exit(STM.commit(transaction))
          )
          deepStrictEqual(result, Exit.fail("Ouch"))
        }
      )
    ...
    fc.assert(
      fc.asyncProperty(
        ...
        async (array) => {
          const transaction = pipe(
            array,
            STM.reduce(Chunk.empty<number>(), (acc, curr) =>
              STM.succeed(pipe(acc, Chunk.append(curr)))
            )
          )
          const result = await Effect.runPromise(STM.commit(transaction))
  ...
  it("reduceRight - with a successful step function sums the list properly", () =>
    fc.assert(
      fc.asyncProperty(fc.array(fc.integer()), async (array) => {
        const transaction = pipe(
          array,
          STM.reduceRight(0, (acc, curr) => STM.succeed(acc + curr))
        )
        const result = await Effect.runPromise(STM.commit(transaction))
        strictEqual(
          result,
    ...
    fc.assert(
      fc.asyncProperty(
        ...
        async (array) => {
          const transaction = pipe(
            array,
            STM.reduceRight(0, () => STM.fail("Ouch"))
          )
          const result = await Effect.runPromise(
            Effect.exit(STM.commit(transaction))
          )
    ...
    fc.assert(
      fc.asyncProperty(
        ...
        async (array) => {
          const transaction = pipe(
            array,
            STM.reduceRight(Chunk.empty<number>(), (acc, curr) =>
              STM.succeed(pipe(acc, Chunk.prepend(curr)))
            )
          )
          const result = await Effect.runPromise(STM.commit(transaction))
          deepStrictEqual(Array.from(result), array)
  ...
  it.effect("rejectSTM - doesnt collect value", () =>
    Effect.gen(function* () {
      const transaction = pipe(
        STM.succeed(0),
        STM.rejectSTM((n) =>
          n !== 0 ? Option.some(STM.succeed("Ouch")) : Option.none()
        )
      )
      const result = yield* STM.commit(transaction)
      strictEqual(result, 0)
  ...
  it.effect(
    "tap - applies the function to the result preserving the original result",
    () =>
      Effect.gen(function* () {
        const transaction = pipe(
          ...
          STM.flatMap(([refA, refB]) =>
            STM.all({
              result1: pipe(
                TRef.get(refA),
                STM.tap((n) => pipe(refB, TRef.set(n + 1)))
              ),
  ...
  it.effect(
    "tapBoth - applies the function to success values preserving the original result",
    () =>
      Effect.gen(function* () {
        const transaction = STM.gen(function* () {
  ...
  it.effect(
    "tapBoth - applies the function to error values preserving the original error",
    () =>
      Effect.gen(function* () {
        const transaction = STM.gen(function* () {
          const tapSuccess = yield* TDeferred.make<number>()
  ...
  it.effect(
    "tapError - should apply the function to the error result preserving the original error",
    () =>
      Effect.gen(function* () {
        const transaction = STM.gen(function* () {
  ...
  it.effect("validateAll - accumulate errors and ignore successes", () =>
    Effect.gen(function* () {
      ...
      const transaction = pipe(
        input,
        STM.validateAll((n) => (n % 2 === 0 ? STM.succeed(n) : STM.fail(n)))
      )
      const result = yield* pipe(
        STM.commit(transaction),
        Effect.mapError((chunk) => Array.from(chunk)),
  ...
  it.effect(
    "validateFirst - runs sequentially and short circuits on first success validation",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("zipWith - perform an action on two computations", () =>
    Effect.gen(function* () {
      const transaction = pipe(
        STM.succeed(578),
        STM.zipWith(STM.succeed(2), (a, b) => a + b)
      )
      const result = yield* STM.commit(transaction)
  ...
  it.effect("stack-safety - long collect chains", () =>
    Effect.gen(function* () {
      const result = yield* chain(10_000)(
        STM.collect((n) => Option.some(n + 1))
      )
      strictEqual(result, 10_000)
    })
  )
  ...
  it.effect("stack-safety - long collectSTM chains", () =>
    Effect.gen(function* () {
      const result = yield* chain(10_000)(
        STM.collectSTM((n) => Option.some(STM.succeed(n + 1)))
      )
      strictEqual(result, 10_000)
    })
  )
  ...
  it.effect("stack-safety - long fold chains", () =>
    Effect.gen(function* () {
      const result = yield* chain(10_000)(
        STM.match({ onFailure: () => 0, onSuccess: (n) => n + 1 })
      )
      strictEqual(result, 10_000)
    })
  )
  ...
  it.effect("stack-safety - long foldSTM chains", () =>
    Effect.gen(function* () {
      const result = yield* chain(10_000)(
        STM.matchSTM({
          onFailure: () => STM.succeed(0),
          onSuccess: (n) => STM.succeed(n + 1)
        })
      )
      strictEqual(result, 10_000)
    })
        ...
        Effect.gen(function* () {
          ...
          const fiber = yield* Effect.forkAll(
            Array.from({ length: 10 }, () =>
              compute3TRefN(99, refs[0], refs[1], refs[2])
            )
          )
          yield* Fiber.join(fiber)
  ...
  it.effect(
    "condition locks - resume directly when the condition is already satisfied",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "condition locks - resume directly when the condition is already satisfied and change the ref with non-satisfying value",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("condition locks - run both transactions sequentially", () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* pipe(
        Array.from({ length: 10 }, () =>
          pipe(toReceiver, Effect.zipRight(toSender))
        ),
        Effect.forkAll()
      )
      yield* pipe(
  ...
  it.effect("condition locks - run both transactions concurrently #2", () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* pipe(
        toReceiver,
        Effect.zip(toSender, { concurrent: true }),
        Effect.fork
      )
      yield* pipe(
        sender,
  ...
  it.effect(
    "condition locks - atomically run a transaction with a TRef for 20 fibers, each one checking and incrementing the value",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "condition locks - atomically run a transaction that could not be satisfied",
    () => {
      const barrier = new UnpureBarrier()
      return Effect.gen(function* () {
        const ref = yield* TRef.make(0)
  ...
  it.effect(
    "condition locks - interrupt one fiber executing a transaction should terminate all transactions",
    () => {
      const barrier = new UnpureBarrier()
      return Effect.gen(function* () {
        const ref = yield* TRef.make(0)
    ...
    Effect.gen(function* () {
      ...
      assertTrue(
        Either.isLeft(result) &&
          pipe(result.left, Cause.contains(Cause.interrupt(fiberId)))
      )
    })
  )
