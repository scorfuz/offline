...
const assertDivide = (x: string, y: string, z: string) => {
  assertEquals(
    BigDecimal.divide($(x), $(y)).pipe(Option.getOrThrow),
    $(z),
    `Expected ${x} / ${y} to be ${z}`
  )
  assertEquals(
    BigDecimal.unsafeDivide($(x), $(y)),
    $(z),
    `Expected ${x} / ${y} to be ${z}`
  )
}
...
describe("BigDecimal", () => {
  ...
  it("remainder", () => {
    assertEquals(
      BigDecimal.remainder($("5"), $("2")).pipe(Option.getOrThrow),
      $("1")
    )
    assertEquals(
      BigDecimal.remainder($("4"), $("2")).pipe(Option.getOrThrow),
      $("0")
    )
    assertEquals(
      BigDecimal.remainder($("123.456"), $("0.2")).pipe(Option.getOrThrow),
      $("0.056")
    )
  ...
  it("unsafeRemainder", () => {
    ...
    throws(
      () => BigDecimal.unsafeRemainder($("5"), $("0")),
      new RangeError("Division by zero")
    )
  })
  ...
  it("normalize", () => {
    deepStrictEqual(
      ...
      BigDecimal.unsafeMakeNormalized(0n, 0)
    )
    deepStrictEqual(
      BigDecimal.normalize($("0.123000")),
      BigDecimal.unsafeMakeNormalized(123n, 3)
    )
    deepStrictEqual(
      BigDecimal.normalize($("123.000")),
      BigDecimal.unsafeMakeNormalized(123n, 0)
    )
    deepStrictEqual(
      BigDecimal.normalize($("-0.000123000")),
      BigDecimal.unsafeMakeNormalized(-123n, 6)
    )
    deepStrictEqual(
      BigDecimal.normalize($("-123.000")),
      BigDecimal.unsafeMakeNormalized(-123n, 0)
    ...
    deepStrictEqual(
      ...
      BigDecimal.unsafeMakeNormalized(123n, -5)
    )
  })
  ...
  it("sumAll", () => {
    ...
    assertEquals(
      BigDecimal.sumAll([$("2.5"), $("1500"), $("123.456")]),
      $("1625.956")
    )
  })
})
    ...
    fc.assert(
      fc.property(arbWithZero, (bd) => {
        return BigDecimal.equals(
          BigDecimal.unsafeFromString(BigDecimal.format(bd)),
          bd
        )
      })
    )
  ...
  it("toExponential should harmonize with Number.prototype.toExponential", () => {
    ...
    fc.assert(
      fc.property(actualNumbers, (n) => {
        return (
          n.toExponential() ===
          BigDecimal.toExponential(BigDecimal.unsafeFromNumber(n))
        )
      })
    )
  })
