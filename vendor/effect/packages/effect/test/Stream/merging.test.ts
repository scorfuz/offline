...
describe("Stream", () => {
  it.effect("merge - slower stream", () =>
    Effect.gen(function* () {
      ...
      const stream2 = Stream.tap(Stream.make(5, 6, 7, 8), () =>
  ...
  it.effect(
    "mergeHaltLeft - terminates as soon as the first stream terminates",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("mergeHaltEither - interrupts pulling on finish", () =>
    Effect.gen(function* () {
      ...
      const stream2 = Stream.fromEffect(
        pipe(Effect.sleep(Duration.seconds(5)), Effect.as(4))
      )
      const result = yield* pipe(
        stream1,
        Stream.merge(stream2, { haltStrategy: "left" }),
  ...
  it.effect(
    "mergeHaltRight - terminates as soon as the second stream terminates",
    () =>
      Effect.gen(function* () {
        const queue1 = yield* Queue.unbounded<number>()
        const queue2 = yield* Queue.unbounded<number>()
        ...
        yield* pipe(
          Queue.offer(queue2, 1),
          Effect.zipRight(TestClock.adjust(Duration.seconds(1)))
        )
        yield* pipe(
          Queue.offer(queue2, 2),
          Effect.zipRight(TestClock.adjust(Duration.seconds(1)))
        )
        yield* pipe(
  ...
  it.effect(
    "mergeHaltEither - terminates as soon as either stream terminates",
    () =>
      Effect.gen(function* () {
        const queue1 = yield* Queue.unbounded<number>()
        const queue2 = yield* Queue.unbounded<number>()
    ...
    Effect.gen(function* () {
      ...
      const { result1, result2 } = yield* Effect.all({
        ...
        result2: pipe(
          ...
          Effect.zipWith(Stream.runCollect(stream2), (chunk1, chunk2) =>
