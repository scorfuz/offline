...
const withPermitsScoped = (permits: number) => (semaphore: Effect.Semaphore) =>
  Effect.acquireRelease(semaphore.take(permits), (n) => semaphore.release(n))
...
describe("Stream", () => {
  ...
  it.effect(
    "branchAfter - emits data if less than n elements are collected",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Stream.fromChunk(Chunk.range(1, 5)),
          Stream.branchAfter(6, (chunk) =>
            Stream.prepend(Stream.identity<number>(), chunk)
          ),
          Stream.runCollect
        )
        deepStrictEqual(Array.from(result), [1, 2, 3, 4, 5])
  ...
  it.effect(
    "branchAfter - applies the new stream once on remaining upstream",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Stream.fromIterable(Chunk.range(1, 5)),
          Stream.rechunk(2),
          Stream.branchAfter(1, (chunk) =>
            Stream.prepend(Stream.identity<number>(), chunk)
          ),
          Stream.runCollect
        )
  ...
  it.effect("execute", () =>
    Effect.gen(function* () {
      ...
      yield* Stream.runDrain(
        Stream.execute(Ref.set(ref, Chunk.fromIterable([1])))
      )
  ...
  it.effect("flatMap - right identity", () =>
    Effect.gen(function* () {
      const { result1, result2 } = yield* Effect.all({
        result1: pipe(
          Stream.make(1),
          Stream.flatMap((n) => Stream.make(n)),
          Stream.runCollect
        ),
  ...
  it.effect("flatMap - associativity", () =>
    Effect.gen(function* () {
      ...
      const { result1, result2 } = yield* Effect.all({
        ...
        result2: pipe(
          stream,
          Stream.flatMap((n) => pipe(f(n), Stream.flatMap(g))),
          Stream.runCollect
        )
      })
      deepStrictEqual(Array.from(result1), Array.from(result2))
  ...
  it.effect("flatMap - finalizer ordering #2", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(
        ...
        Stream.flatMap(() =>
          Stream.acquireRelease(push("open 2"), () => push("close 2"))
        ),
        Stream.runDrain
      )
      const result = yield* Ref.get(ref)
  ...
  it.effect(
    "flatMap - early release finalizer concatenation is preserved",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make(Chunk.empty<number>())
        const push = (n: number) => Ref.update(ref, Chunk.prepend(n))
  ...
  it.effect("flatMapPar - guarantee ordering", () =>
    Effect.gen(function* () {
      ...
      const { result1, result2 } = yield* Effect.all({
        ...
        result2: pipe(
          stream,
          Stream.flatMap((n) => Stream.make(n, n), { concurrency: 2 }),
          Stream.runCollect
        )
      })
      deepStrictEqual(Array.from(result1), Array.from(result2))
    ...
    Effect.gen(function* () {
      ...
      const result = yield* pipe(
        Stream.make(
          ...
          Stream.fromEffect(
            pipe(Deferred.await(latch), Effect.zipRight(Effect.fail("Ouch")))
          )
        ),
    ...
    Effect.gen(function* () {
      ...
      const result = yield* pipe(
        Stream.make(
          ...
          Stream.fromEffect(
            pipe(Deferred.await(latch), Effect.zipRight(Effect.die(defect)))
          )
        ),
        Stream.flatMap(identity, { concurrency: 2 }),
        Stream.runDrain,
  ...
  it.effect("flatMapPar - finalizer ordering", () =>
    Effect.gen(function* () {
      const ref = yield* Ref.make(Chunk.empty<string>())
      const push = (message: string) => Ref.update(ref, Chunk.append(message))
      const inner = Stream.acquireRelease(push("Inner Acquire"), () =>
        push("Inner Release")
      )
      yield* pipe(
        Stream.acquireRelease(
          pipe(push("Outer Acquire"), Effect.as(inner)),
          () => push("Outer Release")
        ),
        Stream.flatMap(identity, { concurrency: 2 }),
        Stream.runDrain
  ...
  it.effect("flatMapParSwitch - guarantee ordering with parallelism", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(
        ...
        Stream.flatMap(
          (n) => {
            if (n > 8) {
              return pipe(
                Stream.acquireRelease(Effect.void, () =>
      ...
      const result = yield* pipe(Ref.get(ref), semaphore.withPermits(4))
    ...
    Effect.gen(function* () {
      ...
      const result = yield* pipe(
        Stream.make(
          ...
          Stream.fromEffect(
            pipe(Deferred.await(latch), Effect.zipRight(Effect.die(error)))
          )
        ),
        Stream.flatMap(identity, { concurrency: 2, switch: true }),
        Stream.runDrain,
  ...
  it.effect("flattenTake - stop collecting on Exit.Failure", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Stream.make(Take.chunk(Chunk.make(1, 2)), Take.of(3), Take.end),
        Stream.flattenTake,
        Stream.runCollect
      )
