...
describe("Stream", () => {
  ...
  it.effect("absolve - round trip #1", () =>
    Effect.gen(function* () {
      const xss = Stream.fromIterable(
        pipe(Chunk.range(1, 10), Chunk.map(Either.right))
      )
      const stream = pipe(
        xss,
        Stream.concat(Stream.succeed(Either.left("Ouch"))),
      ...
      const { result1, result2 } = yield* Effect.all({
        ...
        result2: pipe(
          Stream.mapEffect(stream, identity),
          Stream.either,
          Stream.runCollect
        )
      })
  ...
  it.effect(
    "catchAllCause - releases all resources by the time the failover stream has started",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "catchAllCause - propagates the right Exit value to the failing stream (ZIO #3609)",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make<Exit.Exit<unknown, unknown>>(Exit.void)
        yield* pipe(
          Stream.acquireRelease(Effect.void, (_, exit) => Ref.set(ref, exit)),
    ...
    Effect.gen(function* () {
      ...
      const result = yield* pipe(
        ...
        Stream.catchSome((error) =>
          error === "boom" ? Option.some(stream2) : Option.none()
        ),
        Stream.runCollect
      )
      deepStrictEqual(Array.from(result), [1, 2, 3, 4])
  ...
  it.effect(
    "catchSome - fails stream when partial function does not match",
    () =>
      Effect.gen(function* () {
        const stream1 = pipe(
          Stream.make(1, 2),
        ...
        const result = yield* pipe(
          stream1,
          Stream.catchSome((error) =>
            error === "boomer" ? Option.some(stream2) : Option.none()
          ),
          Stream.runCollect,
          Effect.either
        )
  ...
  it.effect(
    "catchSomeCause - fails stream when partial function does not match",
    () =>
      Effect.gen(function* () {
        const stream1 = pipe(
          Stream.make(1, 2),
        ...
        const result = yield* pipe(
          stream1,
          Stream.catchSomeCause((cause) =>
            Cause.isEmpty(cause) ? Option.some(stream2) : Option.none()
          ),
          Stream.runCollect,
          Effect.either
        )
  ...
  it.effect("orElseEither", () =>
    Effect.gen(function* () {
      const stream1 = pipe(Stream.make(1), Stream.concat(Stream.fail("boom")))
  ...
  it.effect(
    "orElseIfEmpty - ignores default value when stream is not empty",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Stream.make(1),
  ...
  it.effect(
    "orElseIfEmptyStream - consume default stream if stream is empty",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Stream.empty,
  ...
  it.effect(
    "orElseIfEmptyStream - should throw the correct error from the default stream",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Stream.empty,
