...
const chunkArb = <A>(
  ...
  fc.array(arb, constraints).map(Chunk.fromIterable)
...
describe("Stream", () => {
  ...
  it.effect("finalizer - happy path", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(
        Stream.acquireRelease(Ref.update(ref, Chunk.append("Acquire")), () =>
          Ref.update(ref, Chunk.append("Release"))
        ),
        Stream.flatMap(() =>
          Stream.finalizer(Ref.update(ref, Chunk.append("Use")))
        ),
        ...
        Effect.flatMap((pull) =>
          pipe(
            ...
            Effect.forEach(() =>
              pipe(
                Effect.either(pull),
                Effect.map(Either.map(Chunk.toReadonlyArray))
              )
            )
          )
        ),
  ...
  it.effect("range - emits no values when start > end", () =>
    Effect.gen(function* () {
      const result = yield* pipe(Stream.range(2, 1), Stream.runCollect)
      deepStrictEqual(Array.from(result), [])
    })
  )
  ...
  it.effect("range - emits 1 value when start === end", () =>
    Effect.gen(function* () {
      const result = yield* pipe(Stream.range(1, 1), Stream.runCollect)
  ...
  it.effect("range - emits values in chunks of chunkSize", () =>
    Effect.gen(function* () {
      ...
      deepStrictEqual(Array.from(result), [1 + 2, 3 + 4, 5 + 6, 7 + 8, 9])
    })
  )
