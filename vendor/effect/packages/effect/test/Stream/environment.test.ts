...
describe("Stream", () => {
    ...
    Effect.gen(function* () {
      const result = yield* pipe(
        ...
        Stream.provideContext(
          pipe(Context.empty(), Context.add(StringService, { string: "test" }))
        ),
        Stream.runHead,
        Effect.flatten
      )
  ...
  it.effect("contextWithEffect - fails", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Stream.contextWithEffect((_: Context.Context<StringService>) =>
          Effect.fail("boom")
        ),
        Stream.provideContext(
          pipe(Context.empty(), Context.add(StringService, { string: "test" }))
  ...
  it.effect("contextWithStream - fails", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Stream.contextWithStream((_: Context.Context<StringService>) =>
          Stream.fail("boom")
        ),
        Stream.provideContext(
          pipe(Context.empty(), Context.add(StringService, { string: "test" }))
  ...
  it.effect("serviceWithEffect", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Stream.mapEffect(StringService, (service) =>
          Effect.succeed(service.string)
        ),
        Stream.provideLayer(Layer.succeed(StringService, { string: "test" })),
        Stream.runCollect
      )
  ...
  it.effect("serviceWithStream", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Stream.flatMap(StringService, (service) =>
          Stream.succeed(service.string)
        ),
        Stream.provideLayer(Layer.succeed(StringService, { string: "test" })),
        Stream.runCollect
      )
  ...
  it.effect("deep provide", () =>
    Effect.gen(function* () {
      ...
      const effect = Effect.acquireRelease(
        pipe(
          StringService,
          Effect.tap((s) => Effect.sync(() => messages.push(s.string)))
        ),
        () =>
          pipe(
            StringService,
  ...
  it.effect("withSpan", () =>
    Effect.gen(function* () {
      const spans = yield* pipe(
        Stream.make(1, 2, 3),
        Stream.mapEffect((i) =>
          Effect.withSpan(Effect.currentSpan, `span.${i}`)
        ),
