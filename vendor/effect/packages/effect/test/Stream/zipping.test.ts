...
): fc.Arbitrary<Chunk.Chunk<A>> =>
  fc.array(arb, constraints).map(Chunk.unsafeFromArray)
...
export const splitChunks = <A>(
  chunks: Chunk.Chunk<Chunk.Chunk<A>>
): fc.Arbitrary<Chunk.Chunk<Chunk.Chunk<A>>> => {
  const split = (
    chunks: Chunk.Chunk<Chunk.Chunk<A>>
  ): fc.Arbitrary<Chunk.Chunk<Chunk.Chunk<A>>> =>
    fc.integer({ min: 0, max: Math.max(chunks.length - 1, 0) }).chain((i) => {
      ...
      return fc
        .integer({ min: 0, max: Math.max(chunk.length - 1, 0) })
        .map((j) => {
  ...
  return fc.oneof(
    fc.constant(chunks),
    split(chunks).chain((chunks) => splitChunks(chunks))
  )
}
...
describe("Stream", () => {
  ...
  it.effect(
    "zip - does not pull too much when one of the streams is done",
    () =>
      Effect.gen(function* () {
        const left = pipe(
    ...
    Effect.gen(function* () {
      ...
      const { result1, result2 } = yield* Effect.all({
        ...
        result2: pipe(
          ...
          Effect.map(Chunk.map((a, i) => [a, i] as const))
        )
      })
  ...
  it.effect("zipLatest", () =>
    Effect.gen(function* () {
      ...
      const output =
        yield* Queue.bounded<Take.Take<readonly [number, number]>>(1)
      yield* pipe(
      ...
      deepStrictEqual(Array.from(chunk1), [
        [0, 0],
        [0, 1]
      ])
      deepStrictEqual(Array.from(chunk2), [
        [1, 1],
        [2, 1]
      ])
    })
  ...
  it.effect(
    "zipLatestWith - handles empty pulls properly (JVM Only - LOL)",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Stream.unfold(0, (n) =>
  ...
  it("zipLatestWith - preserves partial ordering of stream elements", () => {
    const sortedChunkArb = chunkArb(fc.integer({ min: 1, max: 100 })).map(
      Chunk.sort(Number.Order)
    )
    const sortedChunksArb = sortedChunkArb.chain((chunk) =>
      splitChunks(Chunk.of(chunk))
    )
    return fc.assert(
      fc.asyncProperty(
        sortedChunksArb,
        ...
        async (left, right) => {
          ...
          const [isSorted] = Chunk.isEmpty(result)
            ? [true]
            : pipe(
                result,
                Chunk.drop(1),
                Chunk.reduce(
                  ...
                  ([isSorted, last], curr) =>
                    [isSorted && last <= curr, curr] as const
                )
              )
          assertTrue(isSorted)
        }
    ...
    fc.assert(
      fc.asyncProperty(fc.array(chunkArb(fc.integer())), async (chunks) => {
        ...
        const { result1, result2 } = await Effect.runPromise(
          Effect.all({
            result1: pipe(stream, Stream.zipWithNext, Stream.runCollect),
  ...
  it("zipWithPreviousAndNext - should output same values as zipping with both previous and next element", () =>
    fc.assert(
      fc.asyncProperty(
        fc.array(chunkArb(fc.integer()), { minLength: 0, maxLength: 5 }),
        async (chunks) => {
          const stream = Stream.fromChunks(...chunks)
          const previous = pipe(
            Stream.make(Option.none()),
