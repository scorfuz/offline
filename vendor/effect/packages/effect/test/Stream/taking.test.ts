...
describe("Stream", () => {
    ...
    Effect.gen(function* () {
      ...
      const { result1, result2 } = yield* Effect.all({
        ...
        result2: pipe(
          ...
          Effect.map(Chunk.takeRight(take))
        )
      })
      deepStrictEqual(Array.from(result1), Array.from(result2))
    })
    ...
    Effect.gen(function* () {
      const result = yield* pipe(
        ...
        Stream.takeUntilEffect((n) =>
          n === 2 ? Effect.fail("boom") : Effect.succeed(false)
        ),
        Stream.either,
        Stream.runCollect
      )
  ...
  it.effect(
    "takeWhile - does not stop when hitting an empty chunk (ZIO #4272)",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          ...
          Stream.mapChunks(
            Chunk.flatMap((n) =>
              n === 2 ? Chunk.empty<number>() : Chunk.of(n)
            )
          ),
