...
describe("Stream", () => {
    ...
    Effect.gen(function* () {
      ...
      yield* pipe(
        ...
        Stream.runForEachWhile(() =>
          Ref.modify(ref, (sum) => [sum >= 9 ? false : true, sum + 1] as const)
        )
      )
  ...
  it.effect("repeat - Schedule.CurrentIterationMetadata", () =>
    Effect.gen(function* () {
      const ref = yield* Ref.make(
        Chunk.empty<undefined | Schedule.IterationMetadata>()
      )
      const fiber = yield* pipe(
        Stream.fromEffect(
          Schedule.CurrentIterationMetadata.pipe(
            Effect.flatMap((currentIterationMetadata) =>
  ...
  it.effect(
    "repeatEffectOption - emit elements until pull fails with None",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make(0)
        const result = yield* pipe(
          Stream.repeatEffectOption(
            pipe(
              ...
              Effect.flatMap((n) =>
                n >= 5 ? Effect.fail(Option.none()) : Effect.succeed(n)
              )
            )
          ),
          Stream.take(10),
  ...
  it.effect(
    "repeatEffectOption - stops evaluating the effect once it fails with None",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make(0)
        yield* pipe(
          ...
          Stream.toPull,
          Effect.flatMap((pull) =>
            pipe(Effect.ignore(pull), Effect.zipRight(Effect.ignore(pull)))
          ),
          Effect.scoped
        )
        const result = yield* Ref.get(ref)
  ...
  it.effect(
    "repeatEffectWithSchedule - should perform repetitions in addition to the first execution (one repetition)",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
  ...
  it.effect(
    "repeatEffectWithSchedule - should perform repetitions in addition to the first execution (zero repetitions)",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
  ...
  it.effect(
    "repeatEffectWithSchedule - emits before delaying according to the schedule",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make(0)
  ...
  it.effect("repeatElementsWith", () =>
    Effect.gen(function* () {
      ...
      const result = yield* pipe(
        Stream.make("A", "B", "C"),
        Stream.repeatElementsWith(schedule, {
          onElement: identity,
          onSchedule: String
        }),
        Stream.runCollect
