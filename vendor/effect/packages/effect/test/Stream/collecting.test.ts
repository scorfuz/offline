    ...
    Effect.gen(function* () {
      const result = yield* pipe(
        ...
        Stream.filterMap((either) =>
          Either.isRight(either) ? Option.some(either.right) : Option.none()
        ),
        Stream.runCollect
      )
  ...
  it.effect("collectWhile - short circuits", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        ...
        Stream.filterMapWhile((option) =>
          Option.isNone(option) ? Option.some(1) : Option.none()
        ),
        Stream.runDrain,
        Effect.either
      )
  ...
  it.effect("collectWhileEffect - short circuits", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        ...
        Stream.concat(Stream.fail("Ouch")),
        Stream.filterMapWhileEffect((option) =>
          Option.isNone(option) ? Option.some(Effect.succeed(1)) : Option.none()
        ),
        Stream.runDrain,
        Effect.either
      )
