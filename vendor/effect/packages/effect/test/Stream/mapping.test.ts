...
describe("Stream", () => {
  ...
  it.effect("mapAccumEffect - happy path", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Stream.make(1, 1, 1),
        Stream.mapAccumEffect(0, (acc, curr) =>
          Effect.succeed([acc + curr, acc + curr])
        ),
        Stream.runCollect
      )
      deepStrictEqual(Array.from(result), [1, 2, 3])
    ...
    Effect.gen(function* () {
      const result = yield* pipe(
        ...
        Stream.mapAccumEffect(void 0, (_, n) =>
          n === 3 ? Effect.fail("boom") : Effect.succeed([void 0, n] as const)
        ),
        Stream.either,
        Stream.runCollect
      )
    ...
    Effect.gen(function* () {
      ...
      const { result1, result2 } = yield* Effect.all({
        ...
        result2: pipe(
          ...
          Effect.map(Chunk.flatMap((n) => f(n)))
        )
      })
      deepStrictEqual(Array.from(result1), Array.from(result2))
    })
  ...
  it.effect("mapConcatChunkEffect - happy path", () =>
    Effect.gen(function* () {
      ...
      const { result1, result2 } = yield* Effect.all({
        result1: pipe(
          stream,
          Stream.mapConcatChunkEffect((n) => Effect.succeed(f(n))),
          Stream.runCollect
        ),
        result2: pipe(
          Stream.runCollect(stream),
  ...
  it.effect("mapConcatEffect - happy path", () =>
    Effect.gen(function* () {
      ...
      const { result1, result2 } = yield* Effect.all({
        result1: pipe(
          stream,
          Stream.mapConcatEffect((n) => Effect.succeed(f(n))),
          Stream.runCollect
        ),
        result2: pipe(
          Stream.runCollect(stream),
  ...
  it.effect("mapEffect - laziness on chunks", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Stream.make(1, 2, 3),
        Stream.mapEffect((n) =>
          n === 3 ? Effect.fail("boom") : Effect.succeed(n)
        ),
        Stream.either,
        Stream.runCollect
      )
  ...
  it.effect("mapEffectPar - guarantees ordering", () =>
    Effect.gen(function* () {
      ...
      const { result1, result2 } = yield* Effect.all({
        result1: pipe(
          stream,
          Stream.mapEffect(Effect.succeed),
          Stream.runCollect
        ),
        result2: pipe(
          stream,
          Stream.mapEffect(Effect.succeed, { concurrency: n }),
          Stream.runCollect
        )
      })
      deepStrictEqual(Array.from(result1), Array.from(result2))
  ...
  it.effect("mapEffectPar - awaits child fibers properly", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        ...
        Stream.mapEffect(() => pipe(Effect.succeed(1), Effect.repeatN(200)), {
          concurrency: 8
        }),
        Stream.runDrain,
        Effect.exit
  ...
  it.effect(
    "mapEffectPar - interrupts pending tasks when one of the tasks fails",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "mapEffectPar - propagates the correct error with subsequent calls to mapEffectPar (ZIO #4514)",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Stream.fromIterable(Chunk.range(1, 50)),
          Stream.mapEffect(
            (n) => (n < 10 ? Effect.succeed(n) : Effect.fail("boom")),
            { concurrency: 20 }
          ),
  ...
  it.effect("mapEffectPar - propagates the error of the original stream", () =>
    Effect.gen(function* () {
      const fiber = yield* pipe(
        ...
        Stream.mapEffect(() => Effect.sleep(Duration.seconds(1)), {
          concurrency: 2
        }),
        Stream.runDrain,
        Effect.fork
  ...
  it.effect("mapEffectParUnordered - mapping with failure is failure", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        ...
        Stream.mapEffect(() => Effect.fail("fail"), {
          concurrency: 10,
          unordered: true
        }),
        Stream.runDrain,
  ...
  it.effect("mapEffect with key", () =>
    Effect.gen(function* () {
      const fiber = yield* pipe(
        ...
        Stream.mapEffect((n) => Effect.delay(Effect.succeed(n), n), {
          key: identity
        }),
        Stream.runCollect,
        Effect.fork
