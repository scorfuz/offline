...
describe("Stream", () => {
  ...
  it.effect(
    "drainFork - interrupts the background stream when the foreground exits",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "drainFork - fails the foreground stream if the background fails with a typed error",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Stream.never,
  ...
  it.effect(
    "drainFork - fails the foreground stream if the background fails with a defect",
    () =>
      Effect.gen(function* () {
