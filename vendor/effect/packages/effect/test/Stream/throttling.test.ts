...
describe("Stream", () => {
  ...
  it.effect("debounce - should take latest chunk within waitTime", () =>
    Effect.gen(function* () {
      ...
      const stream = pipe(
        Stream.fromQueue(coordination.queue),
        Stream.filterMapWhile(
          Exit.match({ onSuccess: Option.some, onFailure: Option.none })
        ),
        Stream.debounce(Duration.seconds(1)),
        Stream.tap(() => coordination.proceed)
      )
  ...
  it.effect("debounce - should interrupt fibers properly", () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* pipe(
        ...
        // TODO: remove
        Stream.flatMap((exit) =>
          Stream.fromEffectOption(Effect.suspend(() => exit))
        ),
  ...
  it.effect(
    "debounce - should interrupt children fiber on stream interruption",
    () =>
      Effect.gen(function* () {
