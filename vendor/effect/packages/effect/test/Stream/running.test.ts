...
describe("Stream", () => {
    ...
    Effect.gen(function* () {
      ...
      yield* pipe(
        ...
        Stream.runForEachWhile((n) =>
          Ref.modify(ref, (sum) =>
            sum >= expected ? ([false, sum] as const) : [true, sum + n]
          )
        )
      )
      const result = yield* Ref.get(ref)
  ...
  it.effect("runLast - non-empty stream", () =>
    Effect.gen(function* () {
      const result = yield* pipe(Stream.make(1, 2, 3, 4), Stream.runLast)
      assertSome(result, 4)
    })
  )
  ...
  it.effect("runScoped - properly closes resources", () =>
    Effect.gen(function* () {
      ...
      const resource = Effect.acquireRelease(Effect.succeed(1), () =>
        Ref.set(ref, true)
      )
      const stream = pipe(
        Stream.scoped(resource),
        Stream.flatMap((a) => Stream.make(a, a, a))
      )
      const [result, state] = yield* pipe(
        ...
        Effect.flatMap((chunk) =>
          pipe(
            Ref.get(ref),
            Effect.map((closed) => [chunk, closed] as const)
          )
        ),
        Effect.scoped
      )
