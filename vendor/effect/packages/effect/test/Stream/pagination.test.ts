...
describe("Stream", () => {
    ...
    Effect.gen(function* () {
      ...
      const result = yield* pipe(
        Stream.paginateEffect(
          s,
          ([n, nums]): Effect.Effect<
            readonly [number, Option.Option<readonly [number, Array<number>]>]
          > =>
            nums.length === 0
  ...
  it.effect("paginateChunk", () =>
    Effect.gen(function* () {
      const s: readonly [Chunk.Chunk<number>, Array<number>] = [
        Chunk.of(0),
        [1, 2, 3, 4, 5]
      ]
      const pageSize = 2
      ...
      const result = yield* pipe(
        Stream.paginateChunkEffect(s, ([chunk, nums]) =>
          nums.length === 0
            ? Effect.succeed([
                chunk,
                Option.none<readonly [Chunk.Chunk<number>, Array<number>]>()
              ] as const)
            : Effect.succeed([
                chunk,
                Option.some([
