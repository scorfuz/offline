...
const weirdStringForSplitLines: fc.Arbitrary<ReadonlyArray<string>> = fc
  .array(fc.string().filter((s) => s !== "\n" && s !== "\r"))
  .map((strings) => {
...
const testSplitLines = (
  ...
  const str = input
    .flatMap((chunk) => Chunk.toReadonlyArray(chunk).join(""))
    .join("")
...
describe("Stream", () => {
      ...
      Effect.gen(function* () {
        ...
        const { result1, result2 } = yield* Effect.all({
          ...
          result2: pipe(
            ...
            Effect.map((chunk) =>
              pipe(Chunk.of(chunk), Chunk.filter(Chunk.isNonEmpty))
            )
          )
        })
        deepStrictEqual(
  ...
  it("splitLines - preserves data", () =>
    fc.assert(
      fc.asyncProperty(weirdStringForSplitLines, async (lines) => {
