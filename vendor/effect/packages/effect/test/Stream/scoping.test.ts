...
describe("Stream", () => {
  ...
  it.effect("acquireRelease - short circuits", () =>
    Effect.gen(function* () {
      ...
      const stream = Stream.acquireRelease(
        Effect.succeed(Chunk.range(0, 2)),
        () => Ref.set(ref, true)
      ).pipe(Stream.flatMap(Stream.fromIterable), Stream.take(2))
      const result = yield* Stream.runCollect(stream)
      const released = yield* Ref.get(ref)
      assertTrue(released)
  ...
  it.effect("acquireRelease - no acquisition when short circuiting", () =>
    Effect.gen(function* () {
      ...
      const stream = Stream.make(1).pipe(
        Stream.concat(
          Stream.acquireRelease(Ref.set(ref, true), () => Effect.void)
        ),
        Stream.take(0)
      )
  ...
  it.effect("acquireRelease - releases when there are defects", () =>
    Effect.gen(function* () {
      const ref = yield* Ref.make(false)
      yield* Stream.acquireRelease(Effect.void, () => Ref.set(ref, true)).pipe(
  ...
  it.effect(
    "acquireRelease - flatMap associativity does not effect lifetime",
    () =>
      Effect.gen(function* () {
        const leftAssoc = yield* Stream.acquireRelease(Ref.make(true), (ref) =>
          Ref.set(ref, false)
        ).pipe(
        ...
        const rightAssoc = yield* Stream.acquireRelease(Ref.make(true), (ref) =>
          Ref.set(ref, false)
        ).pipe(
          Stream.flatMap((ref) =>
  ...
  it.effect("acquireRelease - propagates errors", () =>
    Effect.gen(function* () {
      const result = yield* Stream.acquireRelease(Effect.void, () =>
  ...
  it.effect("ensuring", () =>
    Effect.gen(function* () {
      ...
      yield* Stream.acquireRelease(
        ...
        Stream.crossRight(
          Stream.fromEffect(Ref.update(ref, Chunk.append("Use")))
        ),
        Stream.ensuring(Ref.update(ref, Chunk.append("Ensuring"))),
        Stream.runDrain
      )
      const result = yield* Ref.get(ref)
      deepStrictEqual(Chunk.toArray(result), [
        "Acquire",
        "Use",
        "Release",
        "Ensuring"
  ...
  it.effect("scoped - preserves the interruptibility of an effect", () =>
    Effect.gen(function* () {
      const isInterruptible1 = yield* Effect.checkInterruptible(
        Effect.succeed
  ...
  it("unwrapScoped", async () => {
    ...
    const program = Effect.gen(function* () {
      ...
      const fiber = yield* stream(deferred, ref).pipe(
        Stream.runDrain,
        Effect.fork
      )
