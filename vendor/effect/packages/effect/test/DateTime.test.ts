...
const assertSomeIso = (
  ...
  const iso = value.pipe(
    Option.map((value) => DateTime.formatIso(DateTime.toUtc(value)))
  )
...
describe("DateTime", () => {
  ...
  describe("makeZonedFromString", () => {
    it.effect("parses time + zone", () =>
      Effect.gen(function* () {
        const dt = yield* DateTime.makeZonedFromString(
    ...
    it.effect("roundtrip", () =>
      Effect.gen(function* () {
        ...
        deepStrictEqual(
          dt.zone,
          DateTime.zoneUnsafeMakeNamed("Pacific/Auckland")
        )
        strictEqual(dt.toJSON(), "2024-07-21T08:12:34.112Z")
      })
    )
