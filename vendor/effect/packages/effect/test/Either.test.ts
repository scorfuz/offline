...
describe("Either", () => {
  ...
  it("toJSON", () => {
    deepStrictEqual(Either.right(1).toJSON(), {
      _id: "Either",
      _tag: "Right",
      right: 1
    })
    deepStrictEqual(Either.left("e").toJSON(), {
      _id: "Either",
      _tag: "Left",
      left: "e"
    })
  ...
  it("inspect", () => {
    if (typeof window === "undefined") {
      ...
      deepStrictEqual(
        inspect(Either.right(1)),
        inspect({ _id: "Either", _tag: "Right", right: 1 })
      )
      deepStrictEqual(
        inspect(Either.left("e")),
        inspect({ _id: "Either", _tag: "Left", left: "e" })
      )
    }
  })
  ...
  it("liftPredicate", () => {
    ...
    assertRight(
      pipe(
        1,
        Either.liftPredicate(isPositivePredicate, onPositivePredicateError)
      ),
      1
    )
    assertLeft(
    ...
    assertRight(
      pipe(
        1,
        Either.liftPredicate(isNumberRefinement, onNumberRefinementError)
      ),
      1
    )
    assertLeft(
  ...
  it("getEquivalence", () => {
    const isEquivalent = Either.getEquivalence({
      right: Num.Equivalence,
      left: Str.Equivalence
    })
  ...
  it("fromNullable", () => {
    ...
    deepStrictEqual(
      Either.fromNullable(undefined, () => "fallback"),
      Either.left("fallback")
    )
    deepStrictEqual(
  ...
  it("fromOption", () => {
    deepStrictEqual(
      Either.fromOption(Option.none(), () => "none"),
      Either.left("none")
    )
    deepStrictEqual(
      Either.fromOption(Option.some(1), () => "none"),
      Either.right(1)
    )
  })
  ...
  it("try", () => {
    ...
    deepStrictEqual(
      Either.try({ try: () => 1, catch: (e) => new Error(String(e)) }),
      Either.right(1)
    )
    deepStrictEqual(
      Either.try({
  ...
  it("getOrThrowWith", () => {
    strictEqual(
      pipe(
        Either.right(1),
        Either.getOrThrowWith((e) => new Error(`Unexpected Left: ${e}`))
      ),
      1
    )
    throws(() =>
  ...
  it("getOrThrow", () => {
    ...
    throws(
      () => pipe(Either.left("e"), Either.getOrThrow),
  ...
  it("andThen", () => {
    ...
    assertRight(
      Either.andThen(Either.right(1), () => Either.right(2)),
      2
    )
    assertRight(Either.andThen(Either.right(1), Either.right(2)), 2)
    assertRight(
      Either.andThen(Either.right(1), () => 2),
      2
    )
    assertRight(Either.andThen(Either.right(1), 2), 2)
    assertRight(
      Either.andThen(Either.right(1), (a) => a),
      1
    )
  })
  ...
  it("zipWith", () => {
    assertLeft(
      pipe(
        Either.left(0),
        Either.zipWith(Either.right(2), (a, b) => a + b)
      ),
      0
    )
    assertLeft(
      pipe(
        Either.right(1),
        Either.zipWith(Either.left(0), (a, b) => a + b)
      ),
      0
    )
    assertRight(
  ...
  it("all", () => {
    ...
    assertRight(Either.all({ a: Either.right(1), b: Either.right(true) }), {
      a: 1,
      b: true
    })
    ...
    it("bind", () => {
      ...
      assertLeft(
        pipe(
          Either.right(1),
          Either.bindTo("a"),
          Either.bind("b", () => Either.left("left"))
        ),
        "left"
      )
      assertLeft(
