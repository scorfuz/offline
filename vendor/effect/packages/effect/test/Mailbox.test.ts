...
describe("Mailbox", () => {
  it.effect("offerAll with capacity", () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* mailbox.offerAll([1, 2, 3, 4]).pipe(Effect.fork)
  ...
  it.effect("offer dropping", () =>
    Effect.gen(function* () {
      ...
      assertFalse(result)
      deepStrictEqual(
        Chunk.toReadonlyArray((yield* mailbox.takeAll)[0]),
        [1, 2]
      )
    })
  )
  ...
  it.effect("done completes takes", () =>
    Effect.gen(function* () {
      const mailbox = yield* Mailbox.make<number>(2)
      const fiber = yield* mailbox.takeAll.pipe(Effect.fork)
      yield* Effect.yieldNow()
      yield* mailbox.done(Exit.void)
      deepStrictEqual(
        yield* fiber.await,
        Exit.succeed([Chunk.empty(), true] as const)
      )
    })
  )
  ...
  it.effect("fail", () =>
    Effect.gen(function* () {
      ...
      const takeArr = Effect.map(mailbox.takeAll, ([_]) =>
        Chunk.toReadonlyArray(_)
      )
