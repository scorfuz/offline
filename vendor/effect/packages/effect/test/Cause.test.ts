...
describe("Cause", () => {
  ...
  describe("UnknownException", () => {
    ...
    it("uses a default message when none is provided", () => {
      strictEqual(
        new Cause.UnknownException("my message").message,
    ...
    it("accepts a custom override message", () => {
      strictEqual(
        new Cause.UnknownException(new Error("my error"), "my message").message,
        "my message"
      )
    })
  })
  ...
  it("[internal] prettyErrorMessage converts errors into readable JSON-like strings", () => {
    ...
    strictEqual(
      internal.prettyErrorMessage(new Error2()),
      `{"_tag":"WithMessage","message":"my message"}`
    )
    class Error3 {
    ...
    strictEqual(
      internal.prettyErrorMessage(new Error3()),
  ...
  describe("Cause prototype", () => {
    describe("toJSON / [NodeInspectSymbol]", () => {
      ...
      it("Interrupt", () => {
        ...
        expectJSON(
          Cause.interrupt(
            FiberId.composite(FiberId.none, FiberId.runtime(1, 0))
          ),
          {
            _id: "Cause",
            _tag: "Interrupt",
      ...
      it("Sequential", () => {
        expectJSON(
          Cause.sequential(Cause.fail("failure 1"), Cause.fail("failure 2")),
          {
            _id: "Cause",
            _tag: "Sequential",
            left: {
      ...
      it("Parallel", () => {
        expectJSON(
          Cause.parallel(Cause.fail("failure 1"), Cause.fail("failure 2")),
          {
            _id: "Cause",
            _tag: "Parallel",
            left: {
    ...
    describe("toString", () => {
      ...
      it("Fail", () => {
        ...
        assertInclude(
          String(Cause.fail(new Error("my failure"))),
          "Error: my failure"
        )
      })
      ...
      it("Die", () => {
        ...
        assertInclude(
          String(Cause.die(new Error("die message"))),
          "Error: die message"
        )
      })
      ...
      it("Interrupt", () => {
        strictEqual(
          String(Cause.interrupt(FiberId.none)),
          `All fibers interrupted without errors.`
        )
        strictEqual(
          String(Cause.interrupt(FiberId.runtime(1, 0))),
          `All fibers interrupted without errors.`
        )
        strictEqual(
          String(
      ...
      it("Sequential", () => {
        ...
        const actual = String(
          Cause.sequential(
            Cause.fail(new Error("failure 1")),
            Cause.fail(new Error("failure 2"))
          )
        )
    ...
    describe("Equal.symbol implementation", () => {
      ...
      it("is symmetric", () => {
        fc.assert(
          fc.property(causes, causes, (causeA, causeB) => {
            strictEqual(
      ...
      it("generates identical hashes for equal causes", () => {
        fc.assert(
          fc.property(equalCauses, ([causeA, causeB]) => {
  ...
  describe("Getters", () => {
    it("isEmpty", () => {
      ...
      assertTrue(
        Cause.isEmpty(Cause.parallel(empty, Cause.sequential(empty, empty)))
      )
      assertTrue(
        Cause.isEmpty(Cause.sequential(empty, Cause.parallel(empty, empty)))
      )
      ...
      assertFalse(
        Cause.isEmpty(Cause.parallel(empty, Cause.sequential(empty, failure)))
      )
      assertFalse(
        Cause.isEmpty(Cause.sequential(empty, Cause.parallel(empty, failure)))
      )
    })
    ...
    it("isFailure", () => {
      ...
      assertTrue(
        Cause.isFailure(Cause.parallel(empty, Cause.sequential(empty, failure)))
      )
      assertTrue(
        Cause.isFailure(Cause.sequential(empty, Cause.parallel(empty, failure)))
      )

      assertFalse(
        Cause.isFailure(Cause.sequential(empty, Cause.parallel(empty, empty)))
      )
    })
    ...
    it("isDie", () => {
      ...
      assertTrue(
        Cause.isDie(Cause.parallel(empty, Cause.sequential(empty, defect)))
      )
      assertTrue(
        Cause.isDie(Cause.sequential(empty, Cause.parallel(empty, defect)))
      )

      assertFalse(
        Cause.isDie(Cause.sequential(empty, Cause.parallel(empty, empty)))
      )
    })
    ...
    it("isInterrupted", () => {
      ...
      assertTrue(
        Cause.isInterrupted(
          Cause.parallel(empty, Cause.sequential(empty, interruption))
        )
      )
      assertTrue(
        Cause.isInterrupted(
          Cause.sequential(empty, Cause.parallel(empty, interruption))
        )
      )
      ...
      assertTrue(
        Cause.isInterrupted(
          Cause.parallel(failure, Cause.sequential(empty, interruption))
        )
      )
      assertTrue(
        Cause.isInterrupted(
          Cause.sequential(failure, Cause.parallel(empty, interruption))
        )
      )

      assertFalse(
        Cause.isInterrupted(
          Cause.sequential(empty, Cause.parallel(empty, empty))
        )
      )
    })
    ...
    it("isInterruptedOnly", () => {
      ...
      assertFalse(
        Cause.isInterruptedOnly(Cause.sequential(failure, interruption))
      )
      assertFalse(
        Cause.isInterruptedOnly(Cause.parallel(failure, interruption))
      )
      assertFalse(
        Cause.isInterruptedOnly(
          Cause.parallel(failure, Cause.sequential(empty, interruption))
    ...
    describe("failures", () => {
      it("should return a Chunk of all recoverable errors", () => {
        ...
        expectFailures(
          Cause.sequential(interruption, Cause.parallel(empty, failure)),
          ["error"]
        )
      })
      ...
      it("fails safely for large parallel cause constructions", () => {
        const n = 10_000
        const cause = Array.from({ length: n - 1 }, () =>
          Cause.fail("fail")
        ).reduce(Cause.parallel, Cause.fail("fail"))
    ...
    it("defects", () => {
      ...
      expectDefects(
        Cause.sequential(interruption, Cause.parallel(empty, defect)),
        ["defect"]
      )
    })

    it("interruptors", () => {
      const expectInterruptors = <E>(
        cause: Cause.Cause<E>,
        expected: Array<FiberId.FiberId>
      ) => {
    ...
    it("size", () => {
      ...
      strictEqual(
        Cause.size(Cause.parallel(Cause.fail("error1"), Cause.fail("error2"))),
        2
      )
      strictEqual(
        Cause.size(
      ...
      strictEqual(
        Cause.size(
          Cause.sequential(interruption, Cause.parallel(defect, failure))
        ),
        3
      )
    })
    ...
    it("failureOption", () => {
      const expectFailureOption = <E>(
        cause: Cause.Cause<E>,
        expected: Option.Option<E>
      ) => {
      ...
      expectFailureOption(
        Cause.sequential(failure, defect),
        Option.some("error")
      )
      expectFailureOption(
        Cause.sequential(interruption, Cause.parallel(empty, failure)),
    ...
    it("flipCauseOption", () => {
      ...
      assertSome(
        Cause.flipCauseOption(Cause.fail(Option.some("error"))),
        Cause.fail("error")
      )
      // sequential
      assertSome(
        Cause.flipCauseOption(
          Cause.sequential(
            Cause.fail(Option.some("error1")),
            Cause.fail(Option.some("error2"))
          )
        ),
        Cause.sequential(Cause.fail("error1"), Cause.fail("error2"))
      )
      ...
      assertNone(
        Cause.flipCauseOption(
          Cause.sequential(Cause.fail(Option.none()), Cause.fail(Option.none()))
        )
      )
      // parallel
      assertSome(
      ...
      assertNone(
        Cause.flipCauseOption(
          Cause.parallel(Cause.fail(Option.none()), Cause.fail(Option.none()))
        )
      )
    })

    it("dieOption", () => {
      const expectDieOption = <E>(
        cause: Cause.Cause<E>,
    ...
    it("interruptOption", () => {
      const expectInterruptOption = <E>(
        cause: Cause.Cause<E>,
        expected: Option.Option<FiberId.FiberId>
      ) => {
    ...
    it("keepDefects", () => {
      ...
      assertSome(
        Cause.keepDefects(
          Cause.sequential(
            Cause.die("defect1"),
            Cause.parallel(failure, Cause.die("defect2"))
          )
        ),
        Cause.sequential(Cause.die("defect1"), Cause.die("defect2"))
      )
    ...
    it("linearize", () => {
      const expectLinearize = <E>(
        cause: Cause.Cause<E>,
        expected: Array<Cause.Cause<E>>
      ) => {
      ...
      expectLinearize(Cause.sequential(failure, defect), [
        Cause.sequential(failure, defect)
      ])
      expectLinearize(Cause.parallel(failure, defect), [
        Cause.parallel(failure, defect)
      ])
      expectLinearize(
    ...
    it("stripFailures", () => {
      const expectStripFailures = <E>(
        cause: Cause.Cause<E>,
        expected: Cause.Cause<never>
      ) => {
      ...
      expectStripFailures(
        Cause.sequential(failure, defect),
        Cause.sequential(empty, defect)
      )
      expectStripFailures(
        Cause.parallel(failure, defect),
        Cause.parallel(empty, defect)
      )
    })
    ...
    it("stripSomeDefects", () => {
      ...
      assertSome(
        stripNumberFormatException(Cause.sequential(cause1, cause2)),
        cause2
      )
      assertSome(
        stripNumberFormatException(Cause.sequential(cause2, cause1)),
        cause2
      )
      assertSome(
      ...
      assertSome(
        stripNumberFormatException(Cause.parallel(cause1, cause2)),
        cause2
      )
      assertSome(
        stripNumberFormatException(Cause.parallel(cause2, cause1)),
        cause2
      )
      assertSome(
  ...
  describe("Sequencing", () => {
    describe("flatMap", () => {
      ...
      it("obeys right identity", () => {
        fc.assert(
          fc.property(errors, errorCauseFunctions, (error, f) => {
        ...
        fc.assert(
          fc.property(
            ...
            (cause, f, g) => {
              ...
              const right = cause.pipe(
                Cause.flatMap((error) => f(error).pipe(Cause.flatMap(g)))
              )
              assertTrue(Equal.equals(left, right))
            }
          )
    ...
    it("flatten", () => {
      const expectFlatten = <E>(
        cause: Cause.Cause<Cause.Cause<E>>,
  ...
  describe("Elements", () => {
    it("contains", () => {
      const expectContains = <E, E2>(
        cause: Cause.Cause<E>,
        expected: Cause.Cause<E2>
      ) => {
      ...
      const expectFind = <E>(
        ...
        deepStrictEqual(
          Cause.find(cause, (cause) =>
            Cause.isFailType(cause) && Predicate.isString(cause.error)
  ...
  describe("Destructors", () => {
    it("squash", () => {
      ...
      expectSquash(
        empty,
        new Cause.InterruptedException("Interrupted by fibers: ")
      )
      expectSquash(failure, "error")
      expectSquash(defect, "defect")
      expectSquash(
        interruption,
        new Cause.InterruptedException("Interrupted by fibers: #1")
      )
  ...
  describe("Filtering", () => {
    it("filter", () => {
        ...
        deepStrictEqual(
          Cause.filter(
            ...
            (cause) =>
              ...
              Predicate.isString(cause.error) &&
              cause.error === "error"
          ),
          expected
        )
      ...
      expectFilter(
        Cause.sequential(failure, failure),
        Cause.sequential(failure, failure)
      )
      ...
      expectFilter(
        Cause.parallel(failure, failure),
        Cause.parallel(failure, failure)
      )
  ...
  describe("Formatting", () => {
    it("prettyErrors", () => {
      ...
      deepStrictEqual(Cause.prettyErrors(failure), [
        internal.makePrettyError("error")
      ])
      deepStrictEqual(Cause.prettyErrors(defect), [
    ...
    describe("pretty", () => {
      ...
      describe("renderErrorCause: false", () => {
        const expectPretty = <E>(
          cause: Cause.Cause<E>,
          expected: string | undefined
        ) => {
          deepStrictEqual(Cause.pretty(cause), expected)
          deepStrictEqual(
            Cause.pretty(cause, { renderErrorCause: false }),
            expected
          )
        }
        ...
        it("Interrupt", () => {
          strictEqual(
            Cause.pretty(Cause.interrupt(FiberId.none)),
            "All fibers interrupted without errors."
          )
          strictEqual(
            Cause.pretty(Cause.interrupt(FiberId.runtime(1, 0))),
            "All fibers interrupted without errors."
          )
          strictEqual(
            Cause.pretty(
      ...
      describe("renderErrorCause: true", () => {
        describe("Fail", () => {
          it("no cause", () => {
            const pretty = Cause.pretty(Cause.fail(new Error("my message")), {
          ...
          it("string cause", () => {
            const pretty = Cause.pretty(
              Cause.fail(new Error("my message", { cause: "my cause" })),
              {
                renderErrorCause: true
              }
            )
          ...
          it("error cause", () => {
            const pretty = Cause.pretty(
              Cause.fail(
                new Error("my message", { cause: new Error("my cause") })
              ),
              {
                renderErrorCause: true
              }
            ...
            const pretty = Cause.pretty(
              Cause.fail(
                new Error("my message", {
                  cause: new Error("my cause", { cause: "nested cause" })
                })
              ),
              {
                renderErrorCause: true
        ...
        describe("Die", () => {
          it("with span", () => {
            const exit: any = Effect.die(
              new Error("my message", { cause: "my cause" })
            ).pipe(Effect.withSpan("[myspan]"), Effect.exit, Effect.runSync)
            const cause = exit.cause
