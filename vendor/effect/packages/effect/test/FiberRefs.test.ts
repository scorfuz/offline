...
describe("FiberRefs", () => {
  it.scoped("propagate FiberRef values across fiber boundaries", () =>
    Effect.gen(function* () {
      ...
      const producer = yield* FiberRef.set(fiberRef, true).pipe(
        Effect.zipRight(
          Effect.getFiberRefs.pipe(Effect.flatMap((a) => Queue.offer(queue, a)))
        ),
        Effect.fork
      )
      const consumer = yield* pipe(
        ...
        Effect.flatMap((fiberRefs) =>
          Effect.setFiberRefs(fiberRefs).pipe(
  ...
  it("interruptedCause", () => {
    ...
    const newParentFiberRefs = FiberRefs.joinAs(
      parentFiberRefs,
      parent,
      childFiberRefs
    )
    deepStrictEqual(
      FiberRefs.get(newParentFiberRefs, FiberRef.interruptedCause),
  ...
  describe("currentLogAnnotations", () => {
    ...
    it.effect("annotateLogsScoped", () =>
      Effect.gen(function* () {
        ...
        strictEqual(
          HashMap.size(yield* FiberRef.get(FiberRef.currentLogAnnotations)),
          0
        )
