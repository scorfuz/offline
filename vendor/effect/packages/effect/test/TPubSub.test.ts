...
const sort: (array: ReadonlyArray<number>) => ReadonlyArray<number> = Arr.sort(
  number.Order
)
...
describe("TPubSub", () => {
        ...
        async (n, as) => {
          ...
          deepStrictEqual(
            ...
            as
              .slice(0, n)
              .map((n) => -n)
              .slice(0, Array.from(result1).filter((n) => n < 0).length)
          )
          deepStrictEqual(
            Array.from(result2).filter((n) => n > 0),
          ...
          deepStrictEqual(
            ...
            as
              .slice(0, n)
              .map((n) => -n)
              .slice(0, Array.from(result2).filter((n) => n < 0).length)
          )
        }
      )
    ))
  ...
  it("unbounded - one to one", () =>
    fc.assert(
      fc.asyncProperty(fc.array(fc.integer({ min: 1 })), async (as) => {
        const program = Effect.gen(function* () {
          const deferred = yield* Deferred.make<void>()
