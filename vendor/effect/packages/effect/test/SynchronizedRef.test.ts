...
describe("SynchronizedRef", () => {
  ...
  it.effect("getAndUpdateSomeEffect - happy path", () =>
    Effect.gen(function* () {
      ...
      const result1 = yield* SynchronizedRef.getAndUpdateSomeEffect(
        ref,
        (state) =>
          isClosed(state) ? Option.some(Effect.succeed(Changed)) : Option.none()
      )
  ...
  it.effect("getAndUpdateSomeEffect - twice", () =>
    Effect.gen(function* () {
      ...
      const result1 = yield* SynchronizedRef.getAndUpdateSomeEffect(
        ref,
        (state) =>
          isActive(state) ? Option.some(Effect.succeed(Changed)) : Option.none()
      )
    ...
    Effect.gen(function* () {
      ...
      const result = yield* pipe(
        SynchronizedRef.getAndUpdateSomeEffect(ref, (state) =>
          isActive(state) ? Option.some(Effect.fail(failure)) : Option.none()
        ),
        Effect.exit
      )
      deepStrictEqual(result, Exit.fail(failure))
  ...
  it.effect("getAndUpdateSomeEffect - interrupt parent fiber and update", () =>
    Effect.gen(function* () {
      ...
      ).pipe(Effect.zipRight(Deferred.await(latch)))
