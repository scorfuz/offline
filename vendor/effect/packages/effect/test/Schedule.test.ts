...
describe("Schedule", () => {
  it.effect(
    "collect all inputs into a list as long as the condition f holds",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "collect all inputs into a list as long as the effectful condition f holds",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "collect all inputs into a list until the effectful condition f fails",
    () =>
      Effect.gen(function* () {
        const result = yield* repeat(
          Schedule.collectUntil((n) => n < 10 && n > 1)
        )
  ...
  it.effect("union composes", () =>
    Effect.gen(function* () {
      ...
      const alsoWednesday = mondayOrWednesday.pipe(
        Schedule.intersect(wednesdayOrFriday)
      )
      ...
      deepStrictEqual(
        Chunk.toReadonlyArray(actual),
        Chunk.toReadonlyArray(expected)
      )
    })
  )
  it.effect(
    "either should not wait if neither schedule wants to continue",
    () =>
      Effect.gen(function* () {
        const schedule = Schedule.stop.pipe(
          Schedule.union(
            Schedule.spaced("2 seconds").pipe(Schedule.intersect(Schedule.stop))
          ),
    ...
    Effect.gen(function* () {
      const schedule = (ref: Ref.Ref<number>) => {
        return Schedule.recurs(3).pipe(
          Schedule.onDecision(() => Ref.update(ref, (n) => n + 1))
        )
      }
      const ref = yield* Ref.make(0)
      yield* pipe(
        Ref.getAndUpdate(ref, (n) => n + 1),
        Effect.repeat(schedule(ref))
      )
  ...
  it.effect(
    "union of two schedules should continue as long as either wants to continue",
    () =>
      Effect.gen(function* () {
        const schedule = Schedule.recurWhile((b: boolean) => b).pipe(
          Schedule.union(Schedule.fixed("1 seconds"))
        )
        const input = Chunk.make(true, false, false, false, false)
        const result = yield* runCollect(
          schedule.pipe(Schedule.compose(Schedule.elapsed)),
          input
        )
  ...
  it.effect("Schedule.fixed should compute delays correctly", () =>
    Effect.gen(function* () {
      const inputs = Chunk.make(
        [0, undefined] as const,
        [6500, undefined] as const
      )
  ...
  it.effect("intersection of schedules recurring in bounded intervals", () =>
    Effect.gen(function* () {
      const schedule = Schedule.hourOfDay(4).pipe(
        Schedule.intersect(Schedule.minuteOfHour(20))
      )
      const now = yield* Effect.sync(() => Date.now())
      const input = Array.range(1, 5)
      const delays = yield* pipe(
        Schedule.delays(schedule),
        Schedule.run(now, input)
      )
      const actual = Chunk.toReadonlyArray(
        scanLeft(delays, now, (now, delay) => now + Duration.toMillis(delay))
      ).slice(1)
      assertTrue(
        actual.map((n) => new Date(n).getHours()).every((n) => n === 4)
      )
      assertTrue(
        actual.map((n) => new Date(n).getMinutes()).every((n) => n === 20)
      )
    })
  )
  ...
  describe("simulate a schedule", () => {
    it.effect("without timing out", () =>
      Effect.gen(function* () {
        ...
        const result = yield* pipe(
          ...
          Effect.flatMap((now) =>
            schedule.pipe(Schedule.run(now, Array.makeBy(5, constVoid)))
          )
        )
    ...
    it.effect(
      "respect Schedule.recurs even if more input is provided than needed",
      () =>
        Effect.gen(function* () {
          const schedule = Schedule.recurs(2).pipe(
            Schedule.intersect(Schedule.exponential("1 minutes"))
          )
          const result = yield* Clock.currentTimeMillis.pipe(
            Effect.flatMap((now) =>
              schedule.pipe(Schedule.run(now, Array.range(1, 10)))
            )
          )
          deepStrictEqual(Chunk.toReadonlyArray(result), [
    ...
    it.effect(
      "respect Schedule.upTo even if more input is provided than needed",
      () =>
        Effect.gen(function* () {
  ...
  describe("repeat on success according to a provided strategy", () => {
    ...
    it.effect(
      "for 'recurWhileEffect(cond)' repeats while the effectful cond still holds",
      () =>
        Effect.gen(function* () {
          const result = yield* repeat(
            Schedule.recurWhileEffect((n) => Effect.succeed(n > 10))
          )
          strictEqual(result, 1)
        })
    )
    it.effect(
      "for 'recurUntil(cond)' repeats until the cond is satisfied",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "for 'recurUntilEffect(cond)' repeats until the effectful cond is satisfied",
      () =>
        Effect.gen(function* () {
          const result = yield* repeat(
            Schedule.recurUntilEffect((n) => Effect.succeed(n > 10))
          )
  ...
  describe("repetitions", () => {
    it.effect("forever", () =>
      Effect.gen(function* () {
        const [actual, expected] = yield* checkRepetitions(
  ...
  describe("retries", () => {
    it.effect("for up to 10 times", () =>
      Effect.gen(function* () {
        ...
        const io = Effect.sync(() => {
          ...
          Effect.flatMap(() =>
            i < 5 ? Effect.fail("KeepTryingError") : Effect.succeed(i)
          )
        )
    ...
    it.effect("if fallback succeeded - retryOrElse", () =>
      Effect.gen(function* () {
        ...
        const result = yield* pipe(
          alwaysFail(ref),
          Effect.retryOrElse(Schedule.once, ioSucceed)
        )
    ...
    it.effect("fixed delay with error predicate", () =>
      Effect.gen(function* () {
        ...
        const effect = Effect.sync(() => {
          ...
          Effect.flatMap(() =>
            i < 5 ? Effect.fail("KeepTryingError") : Effect.fail("GiveUpError")
          )
        )
        ...
        const program = effect.pipe(
          Effect.retryOrElse(strategy, (s, n) =>
            TestClock.currentTimeMillis.pipe(
    ...
    it.effect("fibonacci delay", () =>
      Effect.gen(function* () {
        const schedule = Schedule.fibonacci("100 millis").pipe(
          Schedule.compose(Schedule.elapsed)
        )
    ...
    it.effect("fixed delay", () =>
      Effect.gen(function* () {
        const schedule = Schedule.fixed("100 millis").pipe(
          Schedule.compose(Schedule.elapsed)
        )
        const result = yield* runCollect(schedule, Array.makeBy(5, constVoid))
        const expected = [0, 100, 200, 300, 400].map(Duration.millis)
    ...
    it.effect("fixed delay with zero delay", () =>
      Effect.gen(function* () {
        const schedule = Schedule.fixed(Duration.zero).pipe(
          Schedule.compose(Schedule.elapsed)
        )
        const result = yield* runCollect(schedule, Array.makeBy(5, constVoid))
    ...
    it.effect("windowed", () =>
      Effect.gen(function* () {
        const schedule = Schedule.windowed("100 millis").pipe(
          Schedule.compose(Schedule.elapsed)
        )
        const result = yield* runCollect(schedule, Array.makeBy(5, constVoid))
        const expected = [0, 100, 200, 300, 400].map(Duration.millis)
    ...
    it.effect("modified linear delay", () =>
      Effect.gen(function* () {
        const schedule = Schedule.linear("100 millis").pipe(
          Schedule.modifyDelayEffect((_, duration) =>
            Effect.succeed(duration.pipe(Duration.times(2)))
          ),
          Schedule.compose(Schedule.elapsed)
        )
        const result = yield* runCollect(schedule, Array.makeBy(5, constVoid))
    ...
    it.effect("exponential delay with other factor", () =>
      Effect.gen(function* () {
        const schedule = Schedule.exponential("100 millis", 3).pipe(
          Schedule.compose(Schedule.elapsed)
        )
        const result = yield* runCollect(schedule, Array.makeBy(5, constVoid))
    ...
    it.effect(
      "retry a failed action 2 times and call `ensuring` should run the specified finalizer as soon as the schedule is complete",
      () =>
        Effect.gen(function* () {
          const deferred = yield* Deferred.make<void>()
  ...
  describe("cron-like scheduling - repeats at point of time (minute of hour, day of week, ...)", () => {
    it.effect("recur every second minute using cron", () =>
      Effect.gen(function* () {
        ...
        yield* pipe(
          ...
          Effect.tap((instant) =>
            Ref.update(ref, Array.append(format(instant)))
          ),
    ...
    it.effect(
      "recur at time matching cron expression (second granularity)",
      () =>
        Effect.gen(function* () {
          const ref = yield* Ref.make<ReadonlyArray<string>>([])
          yield* TestClock.setTime(new Date(2024, 0, 1, 0, 0, 0).getTime())
    ...
    it.effect("recur at 01 second of each minute", () =>
      Effect.gen(function* () {
        ...
        const inTimeSecondMillis = new Date(
          new Date(originOffset).setSeconds(1)
        ).setMilliseconds(1)
        ...
        const input = Chunk.make(
          ...
          beforeTime,
          afterTime
        ).pipe(Chunk.map((n) => [n, void 0] as const))
        const result = yield* pipe(
        ...
        const afterTimeExpected = new Date(expectedDate).setMinutes(
          expectedDate.getMinutes() + 1
        )
    ...
    it.effect("recur at 01 minute of each hour", () =>
      Effect.gen(function* () {
        const originOffset = new Date(
          new Date(new Date().setHours(0)).setSeconds(0)
        ).setMilliseconds(0)
        const inTimeMinuteMillis = new Date(
          new Date(originOffset).setMinutes(1)
        ).setMilliseconds(1)
        ...
        const afterTimeExpected = new Date(expected).setHours(
          expected.getHours() + 1
        )
        const expectedOutput = Chunk.make(
          expected.getTime(),
    ...
    it.effect("recur at 01 hour of each day", () =>
      Effect.gen(function* () {
        ...
        const inTimeHourSecond = new Date(
          new Date(originOffset).setHours(1)
        ).setSeconds(1)
        ...
        const expected = expectedDate.getTime()
        const afterTimeExpected = new Date(expectedDate).setDate(
    ...
    it.effect("recur at Tuesday of each week", () =>
      Effect.gen(function* () {
        ...
        const nextTuesday = new Date(expectedTuesday).setDate(
          expectedTuesday.getDate() + 7
        )
        const expectedOutput = Chunk.make(
    ...
    it.effect("recur in the 2nd day of each month", () =>
      Effect.gen(function* () {
        ...
        const inTimeDate1 = new Date(
          new Date(originOffset).setDate(2)
        ).setHours(1)
        ...
        const input = Chunk.make(inTimeDate1, inTimeDate2, before, after).pipe(
        ...
        const expectedSecondInTime = new Date(expectedFirstInTime).setMonth(
          expectedFirstInTime.getMonth() + 1
        )
        const expectedBefore = new Date(originOffset).setDate(2)
        const expectedAfter = new Date(
          new Date(expectedBefore).setDate(2)
        ).setMonth(new Date(expectedBefore).getMonth() + 1)
        const expected = Chunk.make(
    ...
    it.effect("union with cron like schedules", () =>
      Effect.gen(function* () {
        ...
        const schedule = Schedule.spaced("20 seconds").pipe(
          Schedule.union(Schedule.secondOfMinute(30))
        )
        yield* pipe(
          TestClock.currentTimeMillis,
          Effect.tap((instant) =>
            Ref.update(ref, (seconds) => [...seconds, instant / 1000])
          ),
          Effect.repeat(schedule),
          Effect.fork
        )
    ...
    it.effect(
      "throw IllegalArgumentException on invalid `second` argument of `secondOfMinute`",
      () =>
        Effect.gen(function* () {
          const input = Chunk.of(Date.now())
          const exit = yield* Effect.exit(
            runCollect(Schedule.secondOfMinute(60), input)
          )
    ...
    it.effect(
      "throw IllegalArgumentException on invalid `minute` argument of `minuteOfHour`",
      () =>
        Effect.gen(function* () {
          const input = Chunk.of(Date.now())
          const exit = yield* Effect.exit(
            runCollect(Schedule.minuteOfHour(60), input)
          )
          const exception = new Cause.IllegalArgumentException(
    ...
    it.effect(
      "throw IllegalArgumentException on invalid `hour` argument of `hourOfDay`",
      () =>
        Effect.gen(function* () {
          const input = Chunk.of(Date.now())
          const exit = yield* Effect.exit(
            runCollect(Schedule.hourOfDay(24), input)
          )
          const exception = new Cause.IllegalArgumentException(
    ...
    it.effect(
      "throw IllegalArgumentException on invalid `day` argument of `dayOfWeek`",
      () =>
        Effect.gen(function* () {
          const input = Chunk.of(Date.now())
          const exit = yield* Effect.exit(
            runCollect(Schedule.dayOfWeek(8), input)
          )
          const exception = new Cause.IllegalArgumentException(
    ...
    it.effect(
      "throw IllegalArgumentException on invalid `day` argument of `dayOfMonth`",
      () =>
        Effect.gen(function* () {
          const input = Chunk.of(Date.now())
          const exit = yield* Effect.exit(
            runCollect(Schedule.dayOfMonth(32), input)
          )
          const exception = new Cause.IllegalArgumentException(
...
const checkDelays = <Env>(
  ...
  readonly [Chunk.Chunk<Duration.Duration>, Chunk.Chunk<Duration.Duration>],
  never,
  Env
> => {
  return Effect.gen(function* () {
...
const checkRepetitions = <Env>(
  ...
  readonly [Chunk.Chunk<number>, Chunk.Chunk<number>],
  never,
  Env
> => {
  return Effect.gen(function* () {
  ...
  return run(
    Schedule.driver(schedule).pipe(
      Effect.flatMap((driver) =>
        runCollectLoop(driver, Chunk.fromIterable(input), Chunk.empty())
      )
    )
  )
}
...
const runManually = <Env, In, Out>(
  ...
  readonly [Chunk.Chunk<readonly [number, Out]>, Option.Option<Out>],
  never,
  Env
> => {
...
const runManuallyLoop = <Env, In, Out>(
  schedule: Schedule.Schedule<Out, In, Env>,
  state: unknown,
  inputs: Chunk.Chunk<readonly [number, In]>,
  acc: Chunk.Chunk<readonly [number, Out]>
): Effect.Effect<
  readonly [Chunk.Chunk<readonly [number, Out]>, Option.Option<Out>],
  never,
  Env
