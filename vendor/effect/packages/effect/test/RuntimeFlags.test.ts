...
const arbRuntimeFlags = fc
  .uniqueArray(arbRuntimeFlag)
  .map((flags) => RuntimeFlags.make(...flags))
...
describe("RuntimeFlags", () => {
  ...
  it("inverse patching", () => {
    ...
    strictEqual(RuntimeFlags.patch(flags, patch2), RuntimeFlags.none)
  })

  it("diff", () => {
    const flags1 = RuntimeFlags.make(RuntimeFlags.RuntimeMetrics)
    const flags2 = RuntimeFlags.make(
      RuntimeFlags.RuntimeMetrics,
      RuntimeFlags.OpSupervision
    )
    strictEqual(
  ...
  it("flags within a set of RuntimeFlags is enabled", () => {
    fc.assert(
      fc.property(arbRuntimeFlags, (flags) => {
        const result = Array.from(RuntimeFlags.toSet(flags)).every((flag) =>
          RuntimeFlags.isEnabled(flags, flag)
        )
        assertTrue(result)
      })
    )
  ...
  it("patching a diff between `none` and a set of flags is an identity", () => {
    fc.assert(
      fc.property(arbRuntimeFlags, (flags) => {
        ...
        strictEqual(RuntimeFlags.patch(RuntimeFlags.none, diff), flags)
      })
    )
  })
