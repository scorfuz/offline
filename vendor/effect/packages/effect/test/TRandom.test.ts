...
const floatsArb: fc.Arbitrary<readonly [number, number]> = fc
  .tuple(
...
const intsArb: fc.Arbitrary<readonly [number, number]> = fc
...
describe("TRandom", () => {
  it("nextIntBetween - generates integers in the specified range", () =>
    fc.assert(
      fc.asyncProperty(intsArb, async ([min, max]) => {
  ...
  it("nextRange - generates numbers in the specified range", () =>
    fc.assert(
      fc.asyncProperty(floatsArb, async ([min, max]) => {
        const result = await pipe(
          STM.commit(TRandom.nextRange(min, max)),
          Effect.provide(TRandom.live),
          Effect.runPromise
