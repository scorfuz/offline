...
describe("Fiber", () => {
    ...
    Effect.gen(function* () {
      const rootContains = (
        ...
        return pipe(
          ...
          Effect.map(Array.contains(fiber))
        )
      }
      ...
      yield* pipe(
        rootContains(fiber1),
        Effect.flatMap((a) =>
          a ? rootContains(fiber2) : Effect.succeed(false)
        ),
  ...
  it.effect(
    "await does not return until all fibers have completed execution",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make(0)
        const fiber = yield* Effect.forkAll(
          Array.makeBy(100, () => Ref.set(ref, 10))
        )
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        result.forEach((_) =>
          assertTrue(Exit.isSuccess(_) && _.value === undefined)
        )
      }),
    10000
  )
  it.effect(
    "joinAll - stack safety",
    () =>
      Effect.gen(function* () {
