...
describe("Sink", () => {
  it.effect("fold - empty", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Stream.empty,
        Stream.transduce(
          Sink.fold<number, number>(0, constTrue, (x, y) => x + y)
        ),
        Stream.runCollect
      )
  ...
  it.effect("foldLeft equivalence with Chunk.reduce", () =>
    Effect.gen(function* () {
      ...
      const result1 = yield* pipe(
        stream,
        Stream.run(Sink.foldLeft("", (s, n) => s + `${n}`))
      )
      const result2 = yield* pipe(
        stream,
        Stream.runCollect,
        Effect.map(Chunk.reduce("", (s, n) => s + `${n}`))
      )
  ...
  it.effect("foldEffect - empty", () =>
    Effect.gen(function* () {
      const sink = Sink.foldEffect(0, constTrue, (x, y: number) =>
        Effect.succeed(x + y)
      )
      const result = yield* pipe(
        Stream.empty,
        ...
        pipe(
          ...
          Effect.flatMap((ref) =>
            pipe(
              stream,
              Stream.transduce(
                Sink.foldEffect(0, constTrue, (_, y: number) =>
                  pipe(Ref.update(ref, Chunk.append(y)), Effect.as(30))
                )
              ),
              Stream.runCollect,
              Effect.flatMap((exit) =>
  ...
  it.effect("foldUntilEffect", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Stream.make(1, 1, 1, 1, 1, 1),
        Stream.transduce(
          Sink.foldUntilEffect(0, 3, (x, y) => Effect.succeed(x + y))
        ),
        Stream.runCollect
      )
      deepStrictEqual(Array.from(result), [3, 3, 0])
      ...
      const result = yield* pipe(
        ...
        Stream.transduce(
          Sink.foldWeightedDecomposeEffect({
            ...
            cost: (_, n) => Effect.succeed(n),
            decompose: (n) =>
              Effect.succeed(n > 1 ? Chunk.make(n - 1, 1) : Chunk.of(n)),
            body: (acc, curr) => Effect.succeed(pipe(acc, Chunk.append(curr)))
          })
        ),
        Stream.runCollect
    ...
    Effect.gen(function* () {
      const sink = pipe(
        ...
        Sink.foldSink({
          ...
          onSuccess: (_) =>
            absurd<
              Sink.Sink<readonly [Array<number>, string], number, never, string>
            >(_)
        })
      )
