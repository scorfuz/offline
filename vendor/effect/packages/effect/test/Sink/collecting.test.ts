...
describe("Sink", () => {
  ...
  it.effect("collectAllToSetN - respects the given limit", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Stream.runCollect(stream)
      deepStrictEqual(Array.from(Chunk.map(result, (set) => Array.from(set))), [
        [1, 2, 3],
        [4]
      ])
    })
  ...
  it.effect(
    "collectAllToMapN - collects as long as map size doesn't exceed the limit",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Stream.fromChunks(
  ...
  it.effect("collectAllUntilEffect", () =>
    Effect.gen(function* () {
      const sink = Sink.collectAllUntilEffect((n: number) =>
        Effect.succeed(n > 4)
      )
      const input = Chunk.make(
        Chunk.make(3, 4, 5, 6, 7, 2),
    ...
    Effect.gen(function* () {
      const sink = pipe(
        ...
        Sink.collectAllWhileWith({
          ...
          body: (acc, option) =>
            Option.isSome(option) ? pipe(acc, Chunk.append(option.value)) : acc
        })
      )
