...
const findSink = <A>(a: A): Sink.Sink<A, A, A, void> =>
  pipe(
    Sink.fold<Option.Option<A>, A>(Option.none(), Option.isNone, (_, v) =>
      a === v ? Option.some(a) : Option.none()
    ),
    Sink.mapEffect(
      Option.match({
...
const zipParLaw = <A, B, C, E>(
  ...
  pipe(
    Effect.all({
      ...
      zbc: pipe(
        stream,
        Stream.run(pipe(sink1, Sink.zip(sink2, { concurrent: true }))),
        Effect.either
      )
    }),
    Effect.map(({ zb, zbc, zc }) =>
      Either.match(zbc, {
        onLeft: (e) =>
          (Either.isLeft(zb) && zb.left === e) ||
          (Either.isLeft(zc) && zc.left === e),
        onRight: ([b, c]) =>
          Either.isRight(zb) &&
          zb.right === b &&
          Either.isRight(zc) &&
