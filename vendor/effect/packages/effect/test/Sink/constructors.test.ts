...
describe("Sink", () => {
  ...
  it.effect(
    "fromQueueWithShutdown - should enqueue all elements and shutdown the queue",
    () =>
      Effect.gen(function* () {
        const queue = yield* pipe(
          Queue.unbounded<number>(),
          Effect.map(createQueueSpy)
        )
        yield* pipe(
          Stream.make(1, 2, 3),
          Stream.run(Sink.fromQueue(queue, { shutdown: true }))
        )
  ...
  it.effect(
    "fromPubSub(_, { shutdown: true }) - should shutdown the pubsub",
    () =>
      Effect.gen(function* () {
        const pubsub = yield* PubSub.unbounded<number>()
        yield* pipe(
          Stream.make(1, 2, 3),
          Stream.run(Sink.fromPubSub(pubsub, { shutdown: true }))
        )
...
const createQueueSpy = <A>(queue: Queue.Queue<A>): Queue.Queue<A> =>
  new QueueSpy(queue)
