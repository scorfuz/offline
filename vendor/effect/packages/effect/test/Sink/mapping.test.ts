...
describe("Sink", () => {
  ...
  it.effect("mapInputChunks - happy path", () =>
    Effect.gen(function* () {
      const sink = pipe(
        Sink.collectAll<number>(),
        Sink.mapInputChunks<string, number>(
          Chunk.map((_) => Number.parseInt(_))
        )
      )
      const result = yield* pipe(Stream.make("1", "2", "3"), Stream.run(sink))
      deepStrictEqual(Array.from(result), [1, 2, 3])
  ...
  it.effect("mapInputEffect - error in transformation", () =>
    Effect.gen(function* () {
        ...
        Sink.mapInputEffect((s: string) =>
          Effect.try(() => {
            ...
            if (Number.isNaN(result)) {
              throw new Cause.RuntimeException(
                `Cannot parse "${s}" to an integer`
              )
            }
            return result
          })
      ...
      deepStrictEqual(
        result.error,
        new Cause.RuntimeException('Cannot parse "a" to an integer')
      )
    })
  )
    ...
    Effect.gen(function* () {
      const result = yield* pipe(
        ...
        Stream.run(
          pipe(
            Sink.succeed(1),
            Sink.mapEffect((n) => Effect.succeed(n + 1))
          )
        )
      )
    ...
    Effect.gen(function* () {
      const result = yield* pipe(
        ...
        Stream.run(
          pipe(
            Sink.succeed(1),
            Sink.mapEffect(() => Effect.fail("fail"))
          )
        ),
        Effect.flip
      )
