...
describe("Sink", () => {
      ...
      const result = yield* pipe(
        ...
        Effect.forEach((n) =>
          pipe(
            ...
            Stream.run(sink),
            Effect.map((option) =>
              Equal.equals(option, Option.some(Option.some(10)))
            )
          )
        )
      )
  ...
  it.effect("findEffect - empty stream terminates with none", () =>
    Effect.gen(function* () {
      ...
      const result = yield* pipe(Stream.fromIterable([]), Stream.run(sink))
      assertNone(result)
    })
  )
  ...
  it.effect("findEffect - unsatisfied condition terminates with none", () =>
    Effect.gen(function* () {
      ...
      const result = yield* pipe(Stream.fromIterable([1, 2]), Stream.run(sink))
      assertNone(result)
    })
  )
  ...
  it.effect(
    "splitWhere - should split a stream on a predicate and run each part into the sink",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "splitWhere - should split a stream on a predicate and run each part into the sink, in several chunks",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "splitWhere - should not yield an empty sink if split on the first element",
    () =>
      Effect.gen(function* () {
        const stream = Stream.make(1, 2, 3, 4, 5, 6, 7, 8)
        const result = yield* pipe(
