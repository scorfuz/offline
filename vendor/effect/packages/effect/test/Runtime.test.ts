...
describe("Runtime", () => {
  it.effect("setFiberRef", () =>
    Effect.gen(function* () {
      ...
      const runtime = Runtime.defaultRuntime.pipe(Runtime.setFiberRef(ref, 1))
  ...
  it.scoped("deleteFiberRef", () =>
    Effect.gen(function* () {
      ...
      const runtime = yield* Layer.toRuntime(
        Layer.effectDiscard(FiberRef.set(ref, { value: 1 }))
      )

      let result = Runtime.runSync(runtime)(FiberRef.get(ref))
      deepStrictEqual(result, { value: 1 })

      result = Runtime.runSync(Runtime.deleteFiberRef(runtime, ref))(
        FiberRef.get(ref)
      )
  ...
  it("runSync", () => {
    deepStrictEqual(
      Runtime.runSync(Runtime.defaultRuntime)(Effect.succeed(1)),
      1
    )
    deepStrictEqual(
      Runtime.runSync(Runtime.defaultRuntime, Effect.succeed(1)),
  ...
  it("runSyncExit", () => {
    deepStrictEqual(
      Runtime.runSyncExit(Runtime.defaultRuntime)(Effect.succeed(1)),
      Exit.succeed(1)
    )
    deepStrictEqual(
      Runtime.runSyncExit(Runtime.defaultRuntime, Effect.succeed(1)),
    ...
    deepStrictEqual(
      Runtime.runSyncExit(Runtime.defaultRuntime)(Effect.fail(1)),
      Exit.fail(1)
    )
    deepStrictEqual(
      Runtime.runSyncExit(Runtime.defaultRuntime, Effect.fail(1)),
  ...
  it("runPromise", async () => {
    ...
    throwsAsync(async () => {
      await Runtime.runPromise(Runtime.defaultRuntime)(
        Effect.tryPromise({
          try: () => new Promise((_, reject) => reject(1)),
          catch: () => "error"
        })
      )
    })
  ...
  it.effect(
    "runPromise isolates AsyncLocalStorage across concurrent calls",
    () =>
      Effect.gen(function* () {
        type RequestStore = {
        ...
        const readAlsOnScheduledContinuation = Effect.withFiberRuntime<Result>(
          (fiber) =>
        ...
        const runRequest = (userId: string): Promise<Result> =>
          requestStore.run({ userId }, () =>
            Runtime.runPromise(runtime)(readAlsOnScheduledContinuation)
          )

        const results = yield* Effect.promise(() =>
          Promise.all([runRequest("user-A"), runRequest("user-B")])
        )
  ...
  it("runPromiseExit/signal", async () => {
    ...
    assertTrue(
      Exit.isInterrupted(
        await Runtime.runPromiseExit(Runtime.defaultRuntime)(Effect.never, {
          signal: aborted
        })
      )
    )
