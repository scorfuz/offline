...
const eventArb: fc.Arbitrary<Event> = fc
  .tuple(fc.integer({ min: -10, max: 10 }), fc.asciiString({ minLength: 1 }))
  .map(([time, description]) => ({ time, description }))

const eventsArb: fc.Arbitrary<Array<Event>> = fc.array(eventArb)

const predicateArb: fc.Arbitrary<(event: Event) => boolean> = fc
  .func(fc.boolean())
  .map((f) => (e: Event) => f(e))
...
describe("TPriorityQueue", () => {
  ...
  it("offerAll and takeAll", () =>
    fc.assert(
      fc.asyncProperty(eventsArb, async (events) => {
        ...
        const result = await Effect.runPromise(STM.commit(transaction))
        strictEqual(
          pipe(result, Arr.differenceWith(equivalentElements())(events)).length,
          0
        )
        strictEqual(
          pipe(events, Arr.differenceWith(equivalentElements())(result)).length,
          0
        )
  ...
  it("removeIf", () =>
    fc.assert(
      fc.asyncProperty(eventsArb, predicateArb, async (events, f) => {
        const transaction = pipe(
        ...
        strictEqual(
          Arr.differenceWith(equivalentElements())(result, filtered).length,
          0
        )
        strictEqual(
          Arr.differenceWith(equivalentElements())(filtered, result).length,
          0
        )
    ...
    fc.assert(
      fc.asyncProperty(eventsArb, async (events) => {
        const transaction = pipe(
          ...
          STM.flatMap((queue) =>
            STM.all(
              pipe(TPriorityQueue.take(queue), STM.replicate(events.length))
            )
          ),
          STM.map((chunk) => Array.from(chunk))
        )
    ...
    fc.assert(
      fc.asyncProperty(
        ...
        async ([events, n]) => {
          ...
          const result = await Effect.runPromise(STM.commit(transaction))
          strictEqual(
            pipe(result, Arr.differenceWith(equivalentElements())(events))
              .length,
            0
          )
          strictEqual(
            pipe(events, Arr.differenceWith(equivalentElements())(result))
              .length,
            0
          )
          deepStrictEqual(result, pipe(result, Arr.sort(orderByTime)))
