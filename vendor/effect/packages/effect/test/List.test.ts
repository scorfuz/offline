...
describe("List", () => {
  ...
  it("appendAll", () => {
    deepStrictEqual(
      List.appendAll(List.make(1, 2), List.make(3, 4)),
      List.make(1, 2, 3, 4)
    )
  })
  ...
  it("flatMap", () => {
    deepStrictEqual(
      List.flatMap(List.empty(), (n) => List.make(n - 1, n + 1)),
      List.empty()
    )
    deepStrictEqual(
    ...
    deepStrictEqual(
      List.flatMap(List.make(1, 2, 3, 4), () => List.empty()),
      List.empty()
    )
  })
  ...
  it("map", () => {
    deepStrictEqual(
      List.map(List.empty(), (n) => n + 1),
      List.empty()
    )
    deepStrictEqual(
      List.map(List.make(1, 2, 3, 4), (n) => n + 1),
      List.make(2, 3, 4, 5)
    )
  })
  ...
  it("mapWithIndex", () => {
    deepStrictEqual(
      List.map(List.empty(), (n, i) => [i, n + 1]),
      List.empty()
    )
    deepStrictEqual(
      List.map(List.make(1, 2, 3, 4), (n, i) => [i, n ** 2]),
      List.make([0, 1], [1, 4], [2, 9], [3, 16])
    )
  })

  it("partition", () => {
    deepStrictEqual(
      List.partition(List.make(1, 2, 3, 4), (n) => n > 2),
  ...
  it("partitionMap", () => {
    deepStrictEqual(
      List.partitionMap(List.make(1, 2, 3, 4), (n) =>
        n > 2 ? Either.right(n) : Either.left(n)
      ),
      [List.make(1, 2), List.make(3, 4)]
    )
  })
  ...
  it("prependAll", () => {
    deepStrictEqual(
      List.prependAll(List.empty(), List.make(1, 2)),
      List.make(1, 2)
    )
    deepStrictEqual(
      List.prependAll(List.make(1, 2), List.empty()),
      List.make(1, 2)
    )
    deepStrictEqual(
      List.prependAll(List.make(3), List.make(1, 2)),
      List.make(1, 2, 3)
    )
  })
  ...
  it("prependAllReversed", () => {
    deepStrictEqual(
      List.prependAllReversed(List.empty(), List.make(1, 2)),
      List.make(2, 1)
    )
    deepStrictEqual(
      List.prependAllReversed(List.make(1, 2), List.empty()),
      List.make(1, 2)
    )
    deepStrictEqual(
      List.prependAllReversed(List.make(3), List.make(1, 2)),
      List.make(2, 1, 3)
    )
  })
  ...
  it("reduce", () => {
    deepStrictEqual(
      List.reduce(List.empty(), "-", (b, a) => b + a),
      "-"
    )
    deepStrictEqual(
      List.reduce(List.make("a", "b", "c"), "-", (b, a) => b + a),
      "-abc"
    )
  })
  ...
  it("splitAt", () => {
    deepStrictEqual(List.splitAt(List.make(1, 2, 3, 4), 2), [
      List.make(1, 2),
      List.make(3, 4)
    ])
  })
  ...
  it("pipe()", () => {
    deepStrictEqual(
      List.empty<string>().pipe(List.prepend("a")),
      List.make("a")
    )
  })
  ...
  it("toJSON", () => {
    ...
    deepStrictEqual(List.make(0, 1, 2).toJSON(), {
      _id: "List",
      _tag: "Cons",
      values: [0, 1, 2]
    })
  ...
  it("inspect", () => {
    ...
    deepStrictEqual(
      inspect(List.empty()),
      inspect({ _id: "List", _tag: "Nil" })
    )
    deepStrictEqual(
      inspect(List.make(0, 1, 2)),
      inspect({ _id: "List", _tag: "Cons", values: [0, 1, 2] })
    )
  })
  ...
  it("compact", () => {
    ...
    deepStrictEqual(
      List.compact(List.make(Option.some(1), Option.some(2), Option.some(3))),
      List.make(1, 2, 3)
    )
    deepStrictEqual(
      List.compact(List.make(Option.some(1), Option.none(), Option.some(3))),
      List.make(1, 3)
    )
  })
  ...
  it("filter", () => {
    ...
    strictEqual(
      testStructuralSharing(List.filter(List.empty(), isEven), List.empty()),
      0
    )
    ...
    deepStrictEqual(
      List.filter(List.make(2, 4, 1, 6, 3), isEven),
      List.make(2, 4, 6)
    )
    const share3 = List.of(6)
    const r4 = List.filter(
      List.appendAll(List.make(2, 4, 1, 3), share3),
      isEven
    )
