...
describe("TSet", () => {
  ...
  it.effect("empty", () =>
    Effect.gen(function* () {
      const transaction = pipe(TSet.empty<number>(), STM.flatMap(TSet.isEmpty))
      const result = yield* STM.commit(transaction)
      assertTrue(result)
    })
  )
  ...
  it.effect("has - existing element", () =>
    Effect.gen(function* () {
      const transaction = pipe(TSet.make(1, 2, 3, 4), STM.flatMap(TSet.has(1)))
      const result = yield* STM.commit(transaction)
      assertTrue(result)
    })
  )
  ...
  it.effect("has - non-existing element", () =>
    Effect.gen(function* () {
      const transaction = pipe(TSet.empty<number>(), STM.flatMap(TSet.has(1)))
      const result = yield* STM.commit(transaction)
      assertFalse(result)
    })
  )
  ...
  it.effect("retainIf > { discard: true }", () =>
    Effect.gen(function* () {
      const transaction = STM.gen(function* () {
        ...
        yield* pipe(
          set,
          TSet.retainIf((s) => s === "aa", { discard: true })
        )
        const a = yield* pipe(set, TSet.has("a"))
        const aa = yield* pipe(set, TSet.has("aa"))
        const aaa = yield* pipe(set, TSet.has("aaa"))
  ...
  it.effect("removeIf > { discard: true }", () =>
    Effect.gen(function* () {
      const transaction = STM.gen(function* () {
        ...
        yield* pipe(
          set,
          TSet.removeIf((s) => s === "aa", { discard: true })
        )
        const a = yield* pipe(set, TSet.has("a"))
        const aa = yield* pipe(set, TSet.has("aa"))
        const aaa = yield* pipe(set, TSet.has("aaa"))
  ...
  it.effect("size", () =>
    Effect.gen(function* () {
      const transaction = pipe(TSet.make(1, 2, 3, 4), STM.flatMap(TSet.size))
      const result = yield* STM.commit(transaction)
