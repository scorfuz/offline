...
describe("Trie", () => {
  ...
  it("longestPrefixOf", () => {
    ...
    deepStrictEqual(
      Trie.longestPrefixOf(trie, "sells"),
      Option.some(["sells", 1])
    )
    deepStrictEqual(
      Trie.longestPrefixOf(trie, "shell"),
      Option.some(["she", 2])
    )
    deepStrictEqual(
      Trie.longestPrefixOf(trie, "shellsort"),
      Option.some(["shells", 0])
    )
  })
    ...
    strictEqual(
      Equal.equals(
        Trie.filterMap(trie, (v, k) =>
          k.length > 3 ? Option.some(v) : Option.none()
        ),
        trieMapK
      ),
      true
