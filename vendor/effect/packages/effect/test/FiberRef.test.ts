...
describe("FiberRef", () => {
  ...
  it.scoped("getAndUpdateSome - changing the value", () =>
    Effect.gen(function* () {
      const fiberRef = yield* FiberRef.make(initial)
      const value1 = yield* FiberRef.getAndUpdateSome(fiberRef, () =>
        Option.some(update)
      )
      const value2 = yield* FiberRef.get(fiberRef)
      strictEqual(value1, initial)
  ...
  it.scoped("modifySome - not changing the value", () =>
    Effect.gen(function* () {
      const fiberRef = yield* FiberRef.make(initial)
      const value1 = yield* FiberRef.modifySome(fiberRef, 2, () =>
        Option.none()
      )
      const value2 = yield* FiberRef.get(fiberRef)
  ...
  it.scoped("updateSomeAndGet - changing the value", () =>
    Effect.gen(function* () {
      const fiberRef = yield* FiberRef.make(initial)
      const value1 = yield* FiberRef.updateSomeAndGet(fiberRef, () =>
        Option.some(update)
      )
      const value2 = yield* FiberRef.get(fiberRef)
      strictEqual(value1, update)
  ...
  it.scoped("locally - restores original value", () =>
    Effect.gen(function* () {
      const fiberRef = yield* FiberRef.make(initial)
      const local = yield* Effect.locally(
        fiberRef,
        update
  ...
  it.scoped("locally - restores parent's value", () =>
    Effect.gen(function* () {
      const fiberRef = yield* FiberRef.make(initial)
      const child = yield* Effect.locally(
        fiberRef,
        update
      )(FiberRef.get(fiberRef).pipe(Effect.fork))
  ...
  it.scoped("locally - restores undefined value", () =>
    Effect.gen(function* () {
      ...
      const localValue = yield* Effect.locally(
        fiberRef,
        update
      )(FiberRef.get(fiberRef))
      const value = yield* FiberRef.get(fiberRef)
  ...
  it.scopedLive("the value of the loser is inherited in zipPar", () =>
    Effect.gen(function* () {
      ...
      const winner = FiberRef.set(fiberRef, update1).pipe(
  ...
  it.scoped("nothing gets inherited with a failure in zipPar", () =>
    Effect.gen(function* () {
      ...
      const failure1 = FiberRef.set(fiberRef, update).pipe(
        Effect.zipRight(Effect.fail(":-("))
      )
      const failure2 = FiberRef.set(fiberRef, update).pipe(
        Effect.zipRight(Effect.fail(":-O"))
      )
      yield* pipe(
        success,
        Effect.zip(failure1.pipe(Effect.zip(failure2, { concurrent: true })), {
          concurrent: true
        }),
  ...
  it.scoped(
    "the value of all fibers in inherited when running many effects with collectAllPar",
    () =>
      Effect.gen(function* () {
        const n = 1000
        const fiberRef = yield* FiberRef.make(0, {
        ...
        yield* Effect.all(
          Array.from({ length: n }, () =>
            FiberRef.update(fiberRef, (n) => n + 1)
          ),
          {
  ...
  it.scoped("its value is inherited after simple race", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(
        ...
        Effect.race(FiberRef.set(fiberRef, update2))
      )
      const result = yield* FiberRef.get(fiberRef)
  ...
  it.scopedLive("its value is inherited after a race with a bad winner", () =>
    Effect.gen(function* () {
      ...
      const badWinner = FiberRef.set(fiberRef, update1).pipe(
        Effect.zipRight(
          Effect.fail("ups").pipe(
            Effect.ensuring(Deferred.succeed(latch, void 0))
          )
        )
      )
  ...
  it.scoped("its value is not inherited after a race of losers", () =>
    Effect.gen(function* () {
      const fiberRef = yield* FiberRef.make(initial)
      const loser1 = FiberRef.set(fiberRef, update1).pipe(
        Effect.zipRight(Effect.fail("ups1"))
      )
      const loser2 = FiberRef.set(fiberRef, update2).pipe(
        Effect.zipRight(Effect.fail("ups2"))
      )
  ...
  it.scoped(
    "the value of the winner is inherited when racing two effects with raceAll",
    () =>
      Effect.gen(function* () {
        const fiberRef = yield* FiberRef.make(initial)
        const latch = yield* Deferred.make<void>()
        ...
        const value1 = yield* pipe(
          FiberRef.get(fiberRef),
          Effect.zipLeft(FiberRef.set(fiberRef, initial))
        )
  ...
  it.scoped(
    "the value of the winner is inherited when racing many effects with raceAll",
    () =>
      Effect.gen(function* () {
        const n = 63
        const fiberRef = yield* FiberRef.make(initial)
        ...
        yield* pipe(
          Chunk.unsafeFromArray(losers1),
          Chunk.prepend(winner1),
          Effect.raceAll
        )
        const value1 = yield* pipe(
          FiberRef.get(fiberRef),
        ...
        const winner2 = FiberRef.set(fiberRef, update1)
        const losers2 = FiberRef.set(fiberRef, update1).pipe(
          Effect.zipRight(Effect.fail(":-O")),
          Effect.replicate(n)
        )
        yield* pipe(
          Chunk.unsafeFromArray(losers2),
          Chunk.prepend(winner2),
          Effect.raceAll
        )
        const value2 = yield* pipe(
          FiberRef.get(fiberRef),
  ...
  it.scoped("nothing gets inherited when racing failures with raceAll", () =>
    Effect.gen(function* () {
      const fiberRef = yield* FiberRef.make(initial)
      const loser = FiberRef.set(fiberRef, update).pipe(
        Effect.zipRight(Effect.fail("darn"))
      )
      yield* pipe(
        Effect.raceAll([loser, ...Array.from({ length: 63 }, () => loser)]),
        Effect.orElse(() => Effect.void)
      )
      const result = yield* FiberRef.get(fiberRef)
      strictEqual(result, initial)
  ...
  it.scoped("fork patch is applied when a fiber is unsafely run", () =>
    Effect.gen(function* () {
      const fiberRef = yield* FiberRef.make<boolean>(true, {
        fork: Fun.constTrue
      })
      const deferred = yield* Deferred.make<boolean>()
      const runtime: Runtime.Runtime<never> =
        yield* Effect.runtime<never>().pipe(Effect.locally(fiberRef, false))
      yield* Effect.sync(() =>
        FiberRef.get(fiberRef).pipe(
  ...
  it.scoped("fork patch is applied when a fiber is unsafely forked", () =>
    Effect.gen(function* () {
      ...
      const deferred = yield* Deferred.make<boolean>()
      const runtime: Runtime.Runtime<never> = yield* Effect.locally(
      ...
      const fiber = yield* Effect.sync(() =>
        Runtime.runFork(runtime)(
          Effect.intoDeferred(FiberRef.get(fiberRef), deferred)
        )
      )
