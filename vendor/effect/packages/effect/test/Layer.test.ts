...
describe("Layer", () => {
  ...
  it.effect("preserves identity of acquired resources", () =>
    Effect.gen(function* () {
      const ChunkTag = Context.GenericTag<Ref.Ref<Chunk.Chunk<string>>>(
        "Ref.Ref<Chunk.Chunk<string>>"
      )
      ...
      const layer = Layer.scoped(
        ...
        ).pipe(Effect.tap(() => Effect.void))
      )
      yield* pipe(
        Layer.build(layer),
        Effect.flatMap((context) =>
          Ref.update(context.pipe(Context.get(ChunkTag)), Chunk.append("test"))
        ),
        Effect.scoped
      )
  ...
  it.scoped("sharing itself with merge", () =>
    Effect.gen(function* () {
      ...
      const result = yield* env.pipe(
        Effect.flatMap((context) =>
          Effect.try(() => context.pipe(Context.get(Service1Tag)))
        )
      )
  ...
  it.effect("finalizers", () =>
    Effect.gen(function* () {
      ...
      assertTrue(
        Array.from(result)
          .slice(0, 2)
          .find((s) => s === acquire1) !== undefined
      )
      assertTrue(
        Array.from(result)
          .slice(0, 2)
          .find((s) => s === acquire2) !== undefined
      )
      assertTrue(
        Array.from(result)
          .slice(2, 4)
          .find((s) => s === release1) !== undefined
      )
      assertTrue(
        Array.from(result)
          .slice(2, 4)
          .find((s) => s === release2) !== undefined
      )
    })
  )
  it.effect("caching values in dependencies", () =>
    Effect.gen(function* () {
      ...
      const aLayer = Layer.function(
        ConfigTag,
        ATag,
        (config) => new A(config.value)
      )
      class B {
        constructor(readonly value: number) {}
      }
  ...
  it.effect("orElse - uses an alternative layer", () =>
    Effect.gen(function* () {
      ...
      const layer1 = makeLayer1(ref)
      const layer2 = makeLayer2(ref)
      const env = Layer.fail("failed!").pipe(
  ...
  it.effect("handles errors gracefully", () =>
    Effect.gen(function* () {
      ...
      const layer = Layer.merge(
        layer1,
        layer4.pipe(Layer.provide(Layer.merge(layer2, layer3)))
      )
      const result = yield* pipe(
        Effect.void,
  ...
  it.effect("can map a layer to an unrelated type", () =>
    Effect.gen(function* () {
      ...
      const layer2 = Layer.function(StringTag, ServiceBTag, (name) => ({
        name
      }))
      const live = layer2.pipe(
        Layer.provide(
          Layer.map(layer1, (context) =>
            Context.make(StringTag, context.pipe(Context.get(ServiceATag)).name)
          )
        )
      )
      ...
      yield* pipe(
        ...
        Effect.flatMap((layer) =>
          Effect.context<Service1>().pipe(
            Effect.provide(layer),
            Effect.flatMap(() =>
              Effect.context<Service1>().pipe(Effect.provide(layer))
            )
          )
        ),
        Effect.scoped
  ...
  it.scoped("fiberRef changes are memoized", () =>
    Effect.gen(function* () {
      ...
      const layer3 = layer2.pipe(Layer.provide(layer1), Layer.merge(layer1))
  ...
  it.effect("provides a partial environment to an effect", () =>
    Effect.gen(function* () {
      ...
      const needsString = needsNumberAndString.pipe(
        Effect.provide(providesNumber)
      )
  ...
  it.effect("to provides a partial environment to another layer", () =>
    Effect.gen(function* () {
      const StringTag = Context.GenericTag<string>("string")
      const NumberRefTag =
        Context.GenericTag<Ref.Ref<number>>("Ref.Ref<number>")
      interface FooService {
        readonly ref: Ref.Ref<number>
        readonly string: string
        readonly get: Effect.Effect<readonly [number, string]>
      }
  ...
  it.effect("passthrough passes the inputs through to the next layer", () =>
    Effect.gen(function* () {
      ...
      const live = Layer.passthrough(layer).pipe(
        Layer.provide(Layer.succeed(NumberTag, { value: 1 }))
      )
  ...
  it.effect("project", () =>
    Effect.gen(function* () {
      ...
      const ageLayer = personLayer.pipe(
        Layer.project(PersonTag, AgeTag, (_) => ({ age: _.age }))
      )
  ...
  it.effect("retry", () =>
    Effect.gen(function* () {
      ...
      const layer = Layer.effectContext(effect).pipe(
  ...
  it.effect("tap peeks at an acquired resource", () =>
    Effect.gen(function* () {
      ...
      const layer = Layer.succeed(BarTag, { bar: "bar" }).pipe(
        Layer.tap((context) =>
          Ref.set(ref, context.pipe(Context.get(BarTag)).bar)
        )
      )
  ...
  it.effect("allows passing partial service", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* service
          .two()
          .pipe(Effect.catchAllDefect(Effect.fail), Effect.flip)
  ...
  describe("MemoMap", () => {
    it.effect("memoizes layer across builds", () =>
      Effect.gen(function* () {
        const ref = yield* makeRef()
        const layer1 = makeLayer1(ref)
        const layer2 = makeLayer2(ref).pipe(Layer.provide(layer1))
...
export const makeLayer1 = (
  ref: Ref.Ref<Chunk.Chunk<string>>
): Layer.Layer<Service1> => {
  return Layer.scoped(
    Service1Tag,
    Effect.acquireRelease(
...
export const makeLayer2 = (
  ref: Ref.Ref<Chunk.Chunk<string>>
): Layer.Layer<Service2> => {
  return Layer.scoped(
    Service2Tag,
    Effect.acquireRelease(
...
export const makeLayer3 = (
  ref: Ref.Ref<Chunk.Chunk<string>>
): Layer.Layer<Service3> => {
  return Layer.scoped(
    Service3Tag,
    Effect.acquireRelease(
