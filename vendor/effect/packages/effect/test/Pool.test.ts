...
describe("Pool", () => {
  ...
  it.scoped("max pool size", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(
        Effect.scoped(
          Effect.zipRight(Pool.get(pool), Deferred.await(deferred))
        ),
        Effect.fork,
        Effect.repeatN(14)
      )
  ...
  it.scoped("max pool size creation strategy", () =>
    Effect.gen(function* () {
      ...
      const scope = yield* Scope.make()
      yield* Pool.get(pool).pipe(Effect.repeatN(14), Scope.extend(scope))
  ...
  it.scoped("finalizer is called for failed allocations", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(pool.get).pipe(Effect.ignore)
