  ...
  describe("string | symbol APIs", () => {
    ...
    it("modifyOption", () => {
      assertNone(
        pipe(
          Record.empty<string>(),
          Record.modifyOption("a", (n) => n + 1)
        )
      )
      assertSome(
        pipe(
          stringRecord,
          Record.modifyOption("a", (n: number) => n + 1)
        ),
        { a: 2, [symA]: null }
      )
      assertSome(
        pipe(
          stringRecord,
          Record.modifyOption("a", (n: number) => String(n))
        ),
        { a: "1", [symA]: null }
      )
      ...
      assertNone(
        pipe(
          Record.empty<symbol>(),
          Record.modifyOption(symA, (n) => n + 1)
        )
      )
      assertSome(
        pipe(
          symbolRecord,
          Record.modifyOption(symA, (n: number) => n + 1)
        ),
        { [symA]: 2, [symB]: 2 }
      )
      assertSome(
        pipe(
          symbolRecord,
          Record.modifyOption(symA, (n: number) => String(n))
        ),
        { [symA]: "1", [symB]: 2 }
      )
    })
    ...
    it("replaceOption", () => {
      ...
      assertSome(pipe(stringRecord, Record.replaceOption("a", 2)), {
        a: 2,
        [symA]: null
      })
      assertSome(pipe(stringRecord, Record.replaceOption("a", true)), {
        a: true,
        [symA]: null
      })

      assertNone(pipe(Record.empty<symbol>(), Record.replaceOption(symA, 2)))
      assertSome(pipe(symbolRecord, Record.replaceOption(symA, 2)), {
        [symA]: 2,
        [symB]: 2
      })
      assertSome(pipe(symbolRecord, Record.replaceOption(symA, true)), {
        [symA]: true,
        [symB]: 2
      })
    })
    ...
    it("replace", () => {
      ...
      deepStrictEqual(Record.replace(stringRecord, "a", 2), {
        a: 2,
        [symA]: null
      })

      deepStrictEqual(Record.replace(symbolRecord, symC, 3), symbolRecord)
      deepStrictEqual(Record.replace(symbolRecord, symA, 2), {
        [symA]: 2,
        [symB]: 2
      })
    })
  ...
  describe("string only APIs", () => {
    ...
    it("filterMap", () => {
      ...
      const filtered = Record.filterMap(x, (value, key) =>
        value > 2 ? Option.some(key) : Option.none()
      )
    ...
    it("partitionMap", () => {
      const f = (n: number) =>
        n > 2 ? Either.right(n + 1) : Either.left(n - 1)
      deepStrictEqual(Record.partitionMap({}, f), [{}, {}])
      deepStrictEqual(Record.partitionMap({ a: 1, b: 3, [symA]: null }, f), [
        { a: 0 },
        { b: 4 }
      ])
    })
    ...
    it("getEquivalence", () => {
      deepStrictEqual(
        Record.getEquivalence(Num.Equivalence)({ a: 1 }, { a: 1 }),
        true
      )
      deepStrictEqual(
        Record.getEquivalence(Num.Equivalence)({ a: 1 }, stringRecord),
        true
      )
      deepStrictEqual(
        Record.getEquivalence(Num.Equivalence)({ a: 1 }, { a: 2 }),
        false
      )
      deepStrictEqual(
        Record.getEquivalence(Num.Equivalence)({ a: 1 }, { b: 1 }),
        false
      )
      const noPrototype = Object.create(null)
      deepStrictEqual(
        Record.getEquivalence(Num.Equivalence)(noPrototype, { b: 1 }),
        false
      )
    })
