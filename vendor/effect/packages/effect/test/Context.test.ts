...
describe("Context", () => {
    ...
    if (typeof window === "undefined") {
      throws(
        ...
        (e) => {
          ...
          assertMatch(
            e.message,
            /Service not found: C \(defined at (.*)Context.test.ts:29:19\)/
          )
        }
      )
      throws(
        ...
        (e) => {
          ...
          assertMatch(
            e.message,
            /Service not found: D \(defined at (.*)Context.test.ts:31:32\)/
          )
        }
      )
    }
  ...
  it("picks services in env and merges", () => {
    ...
    const pruned = pipe(env, Context.pick(A, B))
  ...
  it("omits services from env", () => {
    ...
    const pruned = pipe(env, Context.omit(A, B))
