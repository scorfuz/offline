...
describe("ScopedCache", () => {
            ...
            Effect.flatMap((cache) =>
              pipe(
                Effect.forEach(
                  ...
                  (n) =>
                    Effect.scoped(Effect.zipRight(cache.get(n), Effect.void)),
  ...
  it.effect(
    "invalidate - should properly remove and clean a resource from the cache",
    () =>
      Effect.gen(function* () {
        const capacity = 100
        const observablesResources = yield* Effect.forEach(
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            ...
            yield* Effect.forEach(
              ...
              (observableResource) =>
                observableResource.assertAcquiredOnceAndNotCleaned()
            )
  ...
  it.effect(
    "invalidate - should not invalidate anything before effect is evaluated",
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            const cache = yield* scopedCache
            yield* Effect.scoped(
              Effect.zipRight(cache.get(void 0), Effect.void)
            )
            const invalidateEffect = cache.invalidate(void 0)
            const cacheContainsKey42BeforeInvalidate =
              yield* cache.contains(void 0)
            yield* observablesResource.assertAcquiredOnceAndNotCleaned()
            yield* Effect.scoped(
              Effect.zipRight(cache.get(void 0), Effect.void)
            )
            yield* invalidateEffect
            const cacheContainsKey42AfterInvalidate =
              yield* cache.contains(void 0)
  ...
  it.effect(
    "invalidateAll - should properly remove and clean all resource from the cache",
    () =>
      Effect.gen(function* () {
        const capacity = 100
        const observablesResources = yield* Effect.forEach(
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            ...
            const { hits, misses, size } = yield* cache.cacheStats
            yield* Effect.forEach(observablesResources, (observableResource) =>
              observableResource.assertAcquiredOnceAndCleaned()
            )
  ...
  it.effect(
    "get - should not put anything in the cache before the scoped effect returned by get is used",
    () =>
      Effect.gen(function* () {
        const observablesResource = yield* ObservableResource.makeVoid()
        const scopedCache = ScopedCache.make({
  ...
  it("get - when used sequentially, should properly call correct lookup", () =>
    fc.assert(
      fc.asyncProperty(fc.integer(), (salt) => {
        const program = Effect.gen(function* () {
          const scopedCache = ScopedCache.make({
            capacity: 10,
            timeToLive: Duration.infinity,
          ...
          yield* Effect.scoped(
            Effect.gen(function* () {
              const cache = yield* scopedCache
              const actual = yield* Effect.forEach(Array.range(1, 10), (n) =>
                Effect.scoped(Effect.flatMap(cache.get(n), Effect.succeed))
              )
  ...
  it.effect(
    "get - sequential use of the scoped effect returned by a single call to get should create only one resource",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "get - sequential use of a failing scoped effect should cache the error and immediately call the resource finalizer",
    () =>
      Effect.gen(function* () {
        const watchableLookup = yield* WatchableLookup.makeEffect<
          void,
          never,
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            const cache = yield* scopedCache
            yield* watchableLookup.assertCalledTimes(void 0, (n) =>
              strictEqual(n, 0)
            )
            const resourceScopedProxy = cache.get(void 0)
            yield* watchableLookup.assertCalledTimes(void 0, (n) =>
  ...
  it.effect(
    "get - concurrent use of the scoped effect returned by a single call to get should create only one resource",
    () =>
      Effect.gen(function* () {
        const subResource = yield* ObservableResource.makeVoid()
        const scopedCache = ScopedCache.make({
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            ...
            const acquire1 = Effect.provide(
              scoped,
              Context.make(Scope.Scope, scope1)
            )
            const release1: Scope.Scope.Finalizer = (exit) =>
              Scope.close(scope1, exit)
            const acquire2 = Effect.provide(
              scoped,
              Context.make(Scope.Scope, scope2)
            )
            const release2: Scope.Scope.Finalizer = (exit) =>
              Scope.close(scope2, exit)
            yield* subResource.assertNotAcquired()
            yield* acquire2
            yield* subResource.assertAcquiredOnceAndNotCleaned()
            yield* acquire1
  ...
  it.effect(
    "get - concurrent use on a failing scoped effect should cache the error and immediately call the resource finalizer",
    () =>
      Effect.gen(function* () {
        const watchableLookup = yield* WatchableLookup.makeEffect<
          void,
  ...
  it.effect(
    "get - when two scoped effects returned by two calls to get live longer than the cache, the resource should be cleaned only when it is not in use anymore",
    () =>
      Effect.gen(function* () {
        const subResource = yield* ObservableResource.makeVoid()
        const scopedCache = ScopedCache.make({
        ...
        const scope1 = yield* Scope.make()
        const scope2 = yield* Scope.make()
        const [release1, release2] = yield* Effect.scoped(
          Effect.gen(function* () {
            const cache = yield* scopedCache
  ...
  it.effect(
    "get - when two scoped effects obtained by a single scoped effect returned by a single call to get live longer than the cache, the resource should be cleaned only when it is not in use anymore",
    () =>
      Effect.gen(function* () {
        const subResource = yield* ObservableResource.makeVoid()
        const scopedCache = ScopedCache.make({
  ...
  it("get - should clean old resources if the cache size is exceeded", () => {
    const arb = fc
      ...
      .chain((cacheSize) =>
        fc
          .integer({ min: cacheSize, max: cacheSize + 3 })
          .map((numCreatedKey) => [cacheSize, numCreatedKey] as const)
      )
    return fc.assert(
      fc.asyncProperty(arb, ([cacheSize, numCreatedKey]) => {
        const program = Effect.gen(function* () {
          const watchableLookup = yield* WatchableLookup.make<number, void>(
            () => void 0
          )
          const scopedCache = ScopedCache.make({
            capacity: cacheSize,
          ...
          yield* Effect.scoped(
            Effect.gen(function* () {
                  ...
                  : pipe(
                      ...
                      Array.flatMap((key) =>
                        Chunk.toReadonlyArray(
                          HashMap.unsafeGet(createdResources, key)
                        )
                      ),
                      Array.map((resource) =>
                        resource.assertAcquiredOnceAndCleaned()
                      )
                    )
              ...
              const newestResourceNotCleanedYet = pipe(
                ...
                Array.map((resource) =>
                  resource.assertAcquiredOnceAndNotCleaned()
                )
              )
  ...
  it.effect(
    "get - the scoped effect returned by get should recall lookup function if resource is too old and release the previous resource",
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            ...
            yield* watchableLookup.assertFirstNCreatedResourcesCleaned(
              void 0,
              1
            )
          })
  ...
  it.effect(
    "get - should recall lookup function if resource is too old and release old resource when using the scoped effect multiple times",
    () =>
      Effect.gen(function* () {
        const watchableLookup = yield* WatchableLookup.makeVoid()
        yield* Effect.scoped(
  ...
  it.effect(
    "get - when resource is expired but still used it should wait until resource is not cleaned anymore to clean immediately",
    () =>
      Effect.gen(function* () {
        const watchableLookup = yield* WatchableLookup.makeVoid()
        yield* Effect.scoped(
          Effect.gen(function* () {
            ...
            const release: Scope.Scope.Finalizer = (exit) =>
            ...
            const firstCreatedResource =
              yield* watchableLookup.firstCreatedResource(void 0)
        ...
        const scopedCache = yield* ScopedCache.make({
          ...
          lookup: (i: number) =>
            TestServices.provideLive(
              Effect.delay(Effect.succeed(i), Duration.millis(10))
            )
        })
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const option = yield* pipe(
          scopedCache.getOptionComplete(1),
          Effect.scoped
        )
        assertNone(option)
      })
  ...
  it.effect("refresh - should update the cache with a new value", () =>
    Effect.gen(function* () {
      ...
      const [val1, val2, val3] = yield* Effect.scoped(
        Effect.gen(function* () {
          const cache = yield* scopedCache
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* Chunk.unsafeHead(
            createdResources
          ).assertAcquiredOnceAndCleaned()
          yield* Chunk.unsafeGet(
            createdResources,
            1
  ...
  it.effect(
    "refresh - should update the cache with a new value even if the last get or refresh failed",
    () =>
      Effect.gen(function* () {
        ...
        const retrieve = (number: Ref.Ref<number>) => (key: number) =>
          pipe(
            ...
            Effect.flatMap((n) =>
              n % 3 === 0 ? Effect.fail(error) : Effect.succeed(key * n)
            )
          )
        const seed = 2
        const key = 1
  ...
  it.effect(
    "refresh - should create and acquire subresource if the key doesn't exist in the cache",
    () =>
      Effect.gen(function* () {
        const capacity = 100
        const scopedCache = ScopedCache.make({
  ...
  it.effect(
    "refresh - should not clean the resource if it's not yet expired until the new resource is ready",
    () =>
      Effect.gen(function* () {
        const watchableLookup = yield* WatchableLookup.makeVoid()
        yield* Effect.scoped(
              ...
              Effect.repeat(
                pipe(
                  ...
                  Schedule.whileOutput((elapsed) =>
                    Duration.lessThan(elapsed, Duration.millis(100))
                  )
                )
              )
            )
  ...
  it.effect(
    "refresh - should clean the resource if it's expired and not in used",
    () =>
      Effect.gen(function* () {
        const watchableLookup = yield* WatchableLookup.makeVoid()
        yield* Effect.scoped(
  ...
  it.effect(
    "refresh - should wait to clean expired resource until it's not in use anymore",
    () =>
      Effect.gen(function* () {
        const watchableLookup = yield* WatchableLookup.makeVoid()
        yield* Effect.scoped(
