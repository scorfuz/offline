...
describe("BigInt", () => {
  ...
  it("sqrt", () => {
    throws(
      () => BigInt_.unsafeSqrt(-1n),
      new RangeError("Cannot take the square root of a negative number")
    )
  })

  it("toNumber", () => {
    assertSome(
      BigInt_.toNumber(BigInt(Number.MAX_SAFE_INTEGER)),
      Number.MAX_SAFE_INTEGER
    )
    assertNone(BigInt_.toNumber(BigInt(Number.MAX_SAFE_INTEGER) + BigInt(1)))
    assertSome(
      BigInt_.toNumber(BigInt(Number.MIN_SAFE_INTEGER)),
  ...
  it("fromNumber", () => {
    assertSome(
      BigInt_.fromNumber(Number.MAX_SAFE_INTEGER),
      BigInt(Number.MAX_SAFE_INTEGER)
    )
    assertNone(BigInt_.fromNumber(Number.MAX_SAFE_INTEGER + 1))
    assertSome(
      BigInt_.fromNumber(Number.MIN_SAFE_INTEGER),
