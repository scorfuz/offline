...
const OrdMember: Order.Order<Member> = pipe(
  Str.Order,
  Order.mapInput((member) => member.id)
)

function makeNumericSortedSet(
  ...numbers: Array<number>
): SortedSet.SortedSet<number> {
  return SortedSet.fromIterable(numbers, (self, that: number) =>
...
describe("SortedSet", () => {
  it("fromIterable", () => {
    deepStrictEqual(
      Array.from(SortedSet.fromIterable(["c", "a", "b"], Str.Order)),
      ["a", "b", "c"]
    )
    deepStrictEqual(
      Array.from(pipe(["c", "a", "b"], SortedSet.fromIterable(Str.Order))),
      ["a", "b", "c"]
    )
  })
  ...
  it("inspect", () => {
    ...
    deepStrictEqual(
      inspect(set),
      inspect({ _id: "SortedSet", values: [0, 1, 2] })
    )
  })
  ...
  it("add", () => {
    ...
    deepStrictEqual(Array.from(set), [
  ...
  it("difference", () => {
    ...
    deepStrictEqual(Array.from(pipe(set1, SortedSet.difference(set2))), [
      new Member("worker_000000")
    ])
    deepStrictEqual(Array.from(pipe(set1, SortedSet.difference(set3))), [])
  })
  ...
  it("flatMap", () => {
    ...
    const result = pipe(
      set1,
      SortedSet.flatMap(OrdMember, (a) => [...set2, a])
    )
  ...
  it("intersection", () => {
    ...
    const set3 = [new Member("worker_000005")]
  ...
  it("partition", () => {
    ...
    const result = pipe(
      set,
      SortedSet.partition(
        (member) => member.id.endsWith("1") || member.id.endsWith("3")
      )
    )
  ...
  it("pipe()", () => {
    strictEqual(
      SortedSet.make(Str.Order)("c", "a", "b").pipe(SortedSet.size),
      3
    )
  })
  ...
  it("Equal.symbol", () => {
    assertTrue(
      Equal.equals(SortedSet.empty(Str.Order), SortedSet.empty(Str.Order))
    )
