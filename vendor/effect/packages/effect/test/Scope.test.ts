  ...
  return pipe(
    ...
    Effect.ensuring(
      Effect.scopeWith((scope) =>
        scope.addFinalizer(() =>
          Ref.update(ref, (actions) => [...actions, release(id)])
        )
      )
    )
  )
...
describe("Scope", () => {
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        pipe(
          resource(1, ref),
          Effect.flatMap((id) =>
            Ref.update(ref, (actions) => [...actions, use(id)])
          )
        )
      )
  ...
  it.effect("runs finalizers in parallel", () =>
    Effect.gen(function* () {
      ...
      const result = yield* pipe(
        ...
        Effect.zipRight(
          Effect.addFinalizer(() => Deferred.await(deferred)),
          {
            concurrent: true,
  ...
  it.effect("runs finalizers in parallel when the scope is closed", () =>
    Effect.gen(function* () {
        ...
        pipe(
          ...
          Effect.flatMap(([resource1, resource2]) =>
            pipe(
              ...
              Effect.zip(
                Ref.update(ref, (actions) => [...actions, use(resource2)]),
                { concurrent: true }
              )
            )
          )
      ...
      assertTrue(
        result
          .slice(0, 2)
          .some((action) => isAcquire(action) && action.id === 1)
      )
      assertTrue(
        result
          .slice(0, 2)
          .some((action) => isAcquire(action) && action.id === 2)
      )
      assertTrue(
        result.slice(2, 4).some((action) => isUse(action) && action.id === 1)
      )
      assertTrue(
        result.slice(2, 4).some((action) => isUse(action) && action.id === 2)
      )
      assertTrue(
        result
          .slice(4, 6)
          .some((action) => isRelease(action) && action.id === 1)
      )
      assertTrue(
        result
          .slice(4, 6)
          .some((action) => isRelease(action) && action.id === 2)
      )
    })
  )
  it.effect("preserves order of nested sequential finalizers", () =>
    Effect.gen(function* () {
      const ref = yield* Ref.make<ReadonlyArray<Action>>([])
      const left = Effect.sequentialFinalizers(
        pipe(resource(1, ref), Effect.zipRight(resource(2, ref)))
      )
      const right = Effect.sequentialFinalizers(
        pipe(resource(3, ref), Effect.zipRight(resource(4, ref)))
      )
      yield* Effect.scoped(
        Effect.parallelFinalizers(
          pipe(left, Effect.zip(right, { concurrent: true }))
        )
      )
      const actions = yield* Ref.get(ref)
      const action1Index = actions.findIndex(
        (action) => action.op === OP_RELEASE && action.id === 1
      )
      const action2Index = actions.findIndex(
        (action) => action.op === OP_RELEASE && action.id === 2
      )
      const action3Index = actions.findIndex(
        (action) => action.op === OP_RELEASE && action.id === 3
      )
      const action4Index = actions.findIndex(
        (action) => action.op === OP_RELEASE && action.id === 4
      )
    ...
    Effect.gen(function* () {
      ...
      yield* pipe(
        ...
        Effect.zipRight(
          pipe(
            right,
            Effect.flatMap(([release, _]) => release)
          )
        )
      )
      const actions = yield* Ref.get(ref)
  ...
  it.effect("using", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(
        ...
        Effect.using(() =>
          pipe(
            Ref.update(ref1, (actions) => [...actions, use(1)]),
