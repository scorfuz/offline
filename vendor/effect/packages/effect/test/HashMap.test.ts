...
describe("HashMap", () => {
  ...
  it("inspect", () => {
    if (typeof window === "undefined") {
      ...
      deepStrictEqual(
        inspect(map),
        inspect({ _id: "HashMap", values: [[0, "a"]] })
      )
    }
  })
  ...
  it("hasBy", () => {
    ...
    assertTrue(
      pipe(
        map,
        HM.hasBy((v) => Equal.equals(v, value("a")))
      )
    )
    ...
    assertFalse(
      pipe(
        map,
        HM.hasBy((v) => Equal.equals(v, value("b")))
      )
    )
  })
  ...
  it("modifyAt", () => {
    ...
    const result = pipe(
      ...
      HM.modifyAt(key(0), (maybe) =>
        Option.isSome(maybe) ? Option.some(value("test")) : Option.none()
      )
    )
  ...
  it("some", () => {
    ...
    deepStrictEqual(
      HM.some(mapWith3LettersMax, (value) => value.length > 3),
      false
    )
    deepStrictEqual(
      pipe(
        mapWith3LettersMax,
        HM.some((value) => value.length > 3)
      ),
      false
    )

    deepStrictEqual(
      HM.some(mapWith3LettersMax, (value) => value.length > 1),
      true
    )
    ...
    deepStrictEqual(
      HM.some(
        mapWith3LettersMax,
        (value, key) => value.length > 1 && key === 0
      ),
      false
    )
    ...
    deepStrictEqual(
      HM.some(
        mapWith3LettersMax,
        (value, key) => value.length > 1 && key === 1
      ),
      true
    )
  })
  ...
  it("every", () => {
    ...
    deepStrictEqual(
      HM.every(mapWith3LettersMax, (value) => value.length > 2),
      false
    )
    deepStrictEqual(
      pipe(
        mapWith3LettersMax,
        HM.every((value) => value.length > 2)
      ),
      false
    )

    deepStrictEqual(
      HM.every(mapWith3LettersMax, (value) => value.length >= 1),
      true
    )
  ...
  it("reduce", () => {
    ...
    const result1 = pipe(
      map1,
      HM.reduce("", (acc, { s }) => (acc.length > 0 ? `${acc},${s}` : s))
    )
  ...
  it("pipe()", () => {
    strictEqual(
      HM.empty().pipe(HM.set("key", "value")).pipe(HM.size),
      HM.make(["key", "value"]).pipe(HM.size)
    )
  })
  ...
  it("findFirst", () => {
    ...
    assertSome(
      HM.findFirst(map, (_v, k) => k.n === 0),
      [key(0), value("a")]
    )
    assertSome(
      HM.findFirst(map, (v, _k) => v.s === "bb"),
      [key(1), value("bb")]
    )
  ...
  it("countBy", () => {
    ...
    strictEqual(
      HM.countBy(map, (_v, k) => k.n % 2 === 1),
      2
    )
    strictEqual(
      HM.countBy(map, (v, k) => k.n % 2 === 1 && v.s === "a"),
      1
    )
    strictEqual(
      HM.countBy(map, (v, k) => k.n % 2 === 1 && v.s === "b"),
      0
    )
