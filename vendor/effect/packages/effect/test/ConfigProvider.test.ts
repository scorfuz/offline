...
const webScrapingTargetsConfigWithDefault = Config.all({
  targets: Config.chunk(Config.string()).pipe(
    Config.withDefault(
      Chunk.make("https://effect.website2", "https://github.com/Effect-TS2")
    )
  )
})
...
describe("ConfigProvider", () => {
  ...
  it.effect("top-level lists with multi-character sequence delimiters", () =>
    Effect.gen(function* () {
      ...
      const result = yield* ConfigProvider.fromMap(map, {
        seqDelim: "///"
  ...
  it.effect(
    "top-level lists with special regex multi-character sequence delimiter",
    () =>
      Effect.gen(function* () {
        const map = new Map([
          ["hostPorts.host", "localhost|||localhost|||localhost"],
  ...
  it.effect(
    "top-level lists with special regex character sequence delimiter",
    () =>
      Effect.gen(function* () {
        const map = new Map([
          ["hostPorts.host", "localhost*localhost*localhost"],
  ...
  it.effect(
    "top-level list with different number of elements per key fails",
    () =>
      Effect.gen(function* () {
        const map = new Map([
  ...
  it.effect("collection of atoms falls back to default", () =>
    Effect.gen(function* () {
      const map = new Map()
      const result = yield* provider(map).load(
        webScrapingTargetsConfigWithDefault
      )
      deepStrictEqual(result, {
        targets: Chunk.make(
          "https://effect.website2",
          "https://github.com/Effect-TS2"
        )
      })
    })
  )
  ...
  it.effect(
    "indexed sequence - multiple product types with missing fields",
    () =>
      Effect.gen(function* () {
        const config = Config.array(
          Config.all({
        ...
        const result = yield* Effect.exit(
          ConfigProvider.fromMap(map).load(config)
        )
        assertTrue(
          Exit.isFailure(result) &&
            ...
            result.effect_instruction_i0.error.message ===
              "Expected employees.[1].id to exist in the provided map" &&
            Equal.equals(
  ...
  it.effect(
    "indexed sequence - multiple product types with optional fields",
    () =>
      Effect.gen(function* () {
        const config = Config.array(
          Config.all({
        ...
        const result = yield* ConfigProvider.fromMap(map).load(config)
        deepStrictEqual(result, [
          { age: Option.some(1), id: 2 },
          { age: Option.none(), id: 4 }
        ])
      })
  )

  it.effect(
    "indexed sequence - multiple product types with sequence fields",
    () =>
      Effect.gen(function* () {
        const config = Config.array(
          Config.all({
  ...
  it.effect(
    "indexed sequence - product type of indexed sequences with reusable config",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("indexed sequence - map of indexed sequences", () =>
    Effect.gen(function* () {
      ...
      const config = Config.hashMap(
        Config.array(employee, "employees"),
        "departments"
      )
      const map = new Map([
  ...
  it.effect("indexed sequence - map", () =>
    Effect.gen(function* () {
      ...
      const result = yield* ConfigProvider.fromMap(map).load(config)
      deepStrictEqual(
        result.map((table) => Array.from(table)),
        [
          [
            ["age", 10],
            ["id", 0]
  ...
  it.effect("accessing a non-existent key fails", () =>
    Effect.gen(function* () {
      const map = new Map([["k1.k3", "v"]])
      const config = Config.string("k2").pipe(Config.nested("k1"))
  ...
  it.effect("mapInputPath", () =>
    Effect.gen(function* () {
      const configProvider = ConfigProvider.fromMap(
        new Map([["KEY", "VALUE"]])
      ).pipe(ConfigProvider.mapInputPath((path) => path.toUpperCase()))
  ...
  it.effect("kebabCase", () =>
    Effect.gen(function* () {
      const configProvider = ConfigProvider.fromMap(
        new Map([["kebab-case", "value"]])
      ).pipe(ConfigProvider.kebabCase)
  ...
  it.effect("lowerCase", () =>
    Effect.gen(function* () {
      const configProvider = ConfigProvider.fromMap(
        new Map([["lowercase", "value"]])
      ).pipe(ConfigProvider.lowerCase)
  ...
  it.effect("nested", () =>
    Effect.gen(function* () {
      const configProvider1 = ConfigProvider.fromMap(
        new Map([["nested.key", "value"]])
      )
      const config1 = Config.string("key").pipe(Config.nested("nested"))
      const configProvider2 = ConfigProvider.fromMap(
        new Map([["nested.key", "value"]])
      ).pipe(ConfigProvider.nested("nested"))
  ...
  it.effect("nested - multiple layers of nesting", () =>
    Effect.gen(function* () {
      const configProvider1 = ConfigProvider.fromMap(
        new Map([["parent.child.key", "value"]])
      )
      ...
      const configProvider2 = ConfigProvider.fromMap(
        new Map([["parent.child.key", "value"]])
      ).pipe(ConfigProvider.nested("child"), ConfigProvider.nested("parent"))
      const config2 = Config.string("key")
      const result1 = yield* configProvider1.load(config1)
      const result2 = yield* configProvider2.load(config2)
      strictEqual(result1, "value")
  ...
  it.effect("orElse - with flat data", () =>
    Effect.gen(function* () {
      ...
      const result31 = yield* configProvider.load(
        Config.option(Config.string("key3"))
      )
      const result32 = yield* Effect.either(
        configProvider.load(Config.string("key3"))
      )
      ...
      deepStrictEqual(
        ...
        Either.left(
          ConfigError.Or(
            ConfigError.MissingData(
              ["key3"],
              "Expected key3 to exist in the provided map"
            ),
            ConfigError.MissingData(
              ["key3"],
              "Expected key3 to exist in the provided map"
  ...
  it.effect("orElse - with indexed sequences", () =>
    Effect.gen(function* () {
      ...
      const config1 = arrayConfig.pipe(
        Config.nested("child"),
        Config.nested("parent1")
      )
      const config2 = arrayConfig.pipe(
        Config.nested("child"),
        Config.nested("parent2")
      )
  ...
  it.effect("orElse - with indexed sequences and each provider unnested", () =>
    Effect.gen(function* () {
      ...
      const config3 = arrayConfig.pipe(
        Config.nested("child"),
        Config.nested("parent3")
      )
  ...
  it.effect(
    "orElse - with index sequences and combined provider unnested",
    () =>
      Effect.gen(function* () {
        const configProvider = ConfigProvider.fromMap(
          new Map([
  ...
  it.effect("secret", () =>
    Effect.gen(function* () {
      ...
      const configProvider = ConfigProvider.fromMap(
        new Map([["greeting", value]])
      )
      const result = yield* configProvider.load(Config.secret("greeting"))
      deepStrictEqual(
        result,
        Secret.make(value.split("").map((c) => c.charCodeAt(0)))
      )
    })
  )
  ...
  it.effect("snakeCase", () =>
    Effect.gen(function* () {
      const configProvider = ConfigProvider.fromMap(
        new Map([["snake_case", "value"]])
      ).pipe(ConfigProvider.snakeCase)
  ...
  it.effect("unnested", () =>
    Effect.gen(function* () {
      ...
      const configProvider2 = ConfigProvider.fromMap(
        new Map([["key", "value"]])
      ).pipe(ConfigProvider.unnested("nested"))
  ...
  it.effect("upperCase", () =>
    Effect.gen(function* () {
      const configProvider = ConfigProvider.fromMap(
        new Map([["UPPERCASE", "value"]])
      ).pipe(ConfigProvider.upperCase)
  ...
  it.effect("within", () =>
    Effect.gen(function* () {
      const configProvider = ConfigProvider.fromMap(
        new Map([
          ["nesting1.key1", "value1"],
          ["nesting2.KEY2", "value2"]
        ])
      ).pipe(
        ConfigProvider.within(
          ["nesting2"],
          ConfigProvider.mapInputPath((s) => s.toUpperCase())
        )
      )
      const config = Config.string("key1").pipe(
        Config.nested("nesting1"),
        Config.zip(Config.string("key2").pipe(Config.nested("nesting2")))
      )
      const result = yield* configProvider.load(config)
