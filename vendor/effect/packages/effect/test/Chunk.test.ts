...
describe("Chunk", () => {
  ...
  it("modifyOption", () => {
    assertNone(
      pipe(
        Chunk.empty(),
        Chunk.modifyOption(0, (n: number) => n * 2)
      )
    )
    assertSome(
      pipe(
  ...
  it("replaceOption", () => {
    ...
    assertSome(
      pipe(Chunk.make(1, 2, 3), Chunk.replaceOption(0, 2)),
      Chunk.make(2, 2, 3)
    )
  })
  ...
  it("removeOption", () => {
    ...
    assertSome(
      pipe(Chunk.make(1, 2, 3), Chunk.removeOption(0)),
      Chunk.make(2, 3)
    )
  })
    ...
    describe("Given an iterable", () => {
      ...
      it("should process it", () => {
        assertEquals(
          ...
          Chunk.unsafeFromArray([1, 2, 3, 4, 5])
        )
      })
    })
        ...
        fc.assert(
          fc.property(
            ...
            (arr1, arr2) => {
              ...
              )(Chunk.unsafeFromArray(arr1))
              throws(() =>
                pipe(chunk, Chunk.unsafeGet(arr1.length + arr2.length))
              )
            }
          )
        )
      ...
      it("should return the value", () => {
        fc.assert(
          fc.property(
            ...
            (a, b) => {
              const c = [...a, ...b]
              const d = Chunk.appendAll(Chunk.unsafeFromArray(b))(
                Chunk.unsafeFromArray(a)
              )
  ...
  describe("take", () => {
    describe("Given a Chunk with more elements than the amount taken", () => {
      it("should return the subset", () => {
        assertEquals(
          pipe(Chunk.unsafeFromArray([1, 2, 3]), Chunk.take(2)),
          Chunk.unsafeFromArray([1, 2])
        )
      })
    })
  ...
  it("partitionMap", () => {
    assertTuple(Chunk.partitionMap(Chunk.empty(), identity), [
      Chunk.empty(),
      Chunk.empty()
    ])
    assertTuple(
      Chunk.partitionMap(
        Chunk.make(Either.right(1), Either.left("a"), Either.right(2)),
        identity
      ),
  ...
  it("separate", () => {
    ...
    assertTuple(
      Chunk.separate(
        Chunk.make(Either.right(1), Either.left("e"), Either.right(2))
      ),
  ...
  it("filterMapWhile", () => {
    assertEquals(
      Chunk.filterMapWhile(Chunk.make(1, 3, 4, 5), (n) =>
        n % 2 === 1 ? Option.some(n) : Option.none()
      ),
      Chunk.make(1, 3)
    )
  })
  ...
  it("differenceWith", () => {
    ...
    deepStrictEqual(
      differenceWith(Chunk.make({ id: 1 }, { id: 2 }), chunk),
      Chunk.make({ id: 3 })
    )
  ...
  it("difference", () => {
    ...
    assertEquals(
      Chunk.difference(Chunk.make(1, 2, 3, 4, 5), curr),
      Chunk.make(7, 9)
    )
