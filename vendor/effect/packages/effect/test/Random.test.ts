...
describe("Random", () => {
  ...
  it.live("choice", () =>
    Effect.gen(function* () {
      ...
      const randomItems = yield* Random.choice([1, 2, 3]).pipe(
        Array.replicate(100),
        Effect.all
      )
      strictEqual(
        Array.intersection(randomItems, [1, 2, 3]).length,
        randomItems.length
      )

      assertTrue(
        [1, 2, 3].includes(yield* Random.choice(Chunk.fromIterable([1, 2, 3])))
      )
    })
  )
