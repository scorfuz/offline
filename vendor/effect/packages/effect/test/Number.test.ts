...
describe("Number", () => {
  ...
  it("unsafeDivide", () => {
    ...
    strictEqual(
      pipe(six, Number.unsafeDivide(two)),
      Number.unsafeDivide(six, two)
    )
    ...
    strictEqual(Number.unsafeDivide(six, 0), Infinity)
    strictEqual(Number.unsafeDivide(0, 0), NaN)
  })
  ...
  it("remainder", () => {
    ...
    strictEqual(Number.remainder(-3, 2), -1)
    strictEqual(Number.remainder(-4, 2), -0)
    strictEqual(Number.remainder(-2.8, -0.2), -0)
    strictEqual(Number.remainder(-2, -0.2), -0)
    strictEqual(Number.remainder(-1.5, -0.2), -0.1)
    strictEqual(Number.remainder(0, -0.2), 0)
    strictEqual(Number.remainder(1, -0.2), 0)
    strictEqual(Number.remainder(2.6, -0.2), 0)
    strictEqual(Number.remainder(3.1, -0.2), 0.1)
  })
    ...
    it("preserves mathematical properties for valid inputs", () => {
      ...
      for (const val of validInputs) {
        ...
        strictEqual(
          Math.floor(result),
          result,
          `Result for ${val} should be an integer`
        )
        ...
        strictEqual(
          (result & (result - 1)) === 0,
          true,
          `Result for ${val} should be a power of 2`
        )
