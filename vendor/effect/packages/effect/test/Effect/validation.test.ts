...
describe("Effect", () => {
  ...
  it.effect("validate - combines both cause", () =>
    Effect.gen(function* () {
      ...
      deepStrictEqual(
        result,
        Either.left(Cause.sequential(Cause.fail(1), Cause.fail(2)))
      )
    })
  )
  it.effect("validateWith - succeeds", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Effect.succeed(1),
        Effect.validateWith(Effect.succeed(2), (a, b) => a + b)
      )
  ...
  it.effect("validateAll/discard - returns all errors if never valid", () =>
    Effect.gen(function* () {
      ...
      const result = yield* pipe(
        array,
        Effect.validateAll(Effect.fail, { discard: true }),
        Effect.flip
      )
      deepStrictEqual(Array.from(result), array)
    })
  ...
  it.effect("validateAll/concurrency - returns all errors if never valid", () =>
    Effect.gen(function* () {
      ...
      const result = yield* pipe(
        array,
        Effect.validateAll(Effect.fail, { concurrency: "unbounded" }),
        Effect.flip
      )
      deepStrictEqual(Array.from(result), array)
    })
  ...
  it.effect(
    "validateAll/concurrency - accumulate errors and ignore successes",
    () =>
      Effect.gen(function* () {
        const array = Array.from({ length: 10 }, (_, i) => i)
        const result = yield* pipe(
  ...
  it.effect("validateAll/concurrency - accumulate successes", () =>
    Effect.gen(function* () {
      ...
      const result = yield* pipe(
        array,
        Effect.validateAll(Effect.succeed, { concurrency: "unbounded" })
      )
      deepStrictEqual(Array.from(result), array)
    })
  )
  it.effect(
    "validateAll/concurrency+discard - returns all errors if never valid",
    () =>
      Effect.gen(function* () {
        const array = Array.from({ length: 10 }, () => 0)
        const result = yield* pipe(
  ...
  it.effect("validateFirst - returns all errors if never valid", () =>
    Effect.gen(function* () {
      ...
      const result = yield* pipe(
        array,
        Effect.validateFirst(Effect.fail),
        Effect.flip
      )
      deepStrictEqual(Array.from(result), array)
    })
  ...
  it.effect("validateFirst - returns [] as error if the input is empty", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        [],
        Effect.validateFirst(Effect.succeed),
        Effect.flip
      )
  ...
  describe("", () => {
    it.effect(
      "validateFirst/concurrency - returns all errors if never valid",
      () =>
        Effect.gen(function* () {
          const array = Array.from({ length: 1000 }, () => 0)
          const result = yield* pipe(
            array,
            Effect.validateFirst(Effect.fail, { concurrency: "unbounded" }),
            Effect.flip
          )
          deepStrictEqual(Array.from(result), array)
        })
    ...
    it.effect("validateFirst/concurrency - returns success if valid", () =>
      Effect.gen(function* () {
        ...
        const result = yield* pipe(
          array,
          Effect.validateFirst(f, { concurrency: "unbounded" })
        )
        strictEqual(result, 6)
      })
    )
