...
export interface GetParentChildren extends Request.Request<
  ReadonlyArray<Child>
> {
...
export const GetParentChildren =
  Request.tagged<GetParentChildren>("GetParentChildren")
...
export const children: ReadonlyMap<number, ReadonlyArray<Child>> = new Map(
  Array.map(parents, (p) => [p.id, Array.of({ id: p.id * 10, parentId: p.id })])
)
...
const AllResolver = Resolver.makeBatched(
  (
    requests: Array<
      GetParentChildren | GetAllParents | GetChildExtra | GetChildInfo
    >
  ) =>
    Effect.flatMap(Requests, (r) => {
      ...
      return counted(
        Effect.all([
          Effect.forEach(
            requests.filter(
              (_): _ is GetParentChildren => _._tag === "GetParentChildren"
            ),
            (request) => Request.succeed(request, children.get(request.id)!)
          ),
          Effect.forEach(
            requests.filter(
              (_): _ is GetChildExtra => _._tag === "GetChildExtra"
            ),
            (request) =>
          ...
          Effect.forEach(
            requests.filter(
              (_): _ is GetChildInfo => _._tag === "GetChildInfo"
            ),
            (request) =>
              Request.succeed(request, {
                id: request.id * 10,
          ...
          Effect.forEach(
            requests.filter(
              (_): _ is GetAllParents => _._tag === "GetAllParents"
            ),
...
export const getChildren = (id: number) =>
  Effect.request(GetParentChildren({ id }), AllResolver)
export const getChildInfo = (id: number) =>
  Effect.request(GetChildInfo({ id }), AllResolver)
export const getChildExtra = (id: number) =>
  Effect.request(GetChildExtra({ id }), AllResolver)
        ...
        (parent) =>
          Effect.flatMap(getChildren(parent.id), (children) =>
            Effect.forEach(
              children,
              (child) =>
                Effect.zip(getChildInfo(child.id), getChildExtra(child.id), {
                  concurrent: true,
                  batching: "inherit"
                }),
              {
