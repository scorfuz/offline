...
export class GetNameById extends Request.TaggedClass("GetNameById")<
  string,
  string,
  {
    readonly id: number
...
const counted = <A, E, R>(self: Effect.Effect<A, E, R>) =>
  Effect.tap(self, () => Effect.map(Counter, (c) => c.count++))

const UserResolver = Resolver.makeBatched((requests: Array<UserRequest>) =>
  Effect.flatMap(Requests, (r) => {
    r.count += requests.length
    return counted(
      Effect.forEach(requests, (request) => delay(processRequest(request)), {
        discard: true
      })
    )
  })
).pipe(Resolver.batchN(15), Resolver.contextFromServices(Counter, Requests))
...
export const getUserNameById = (id: number) =>
  Effect.request(new GetNameById({ id }), UserResolver)

export const getUserNameByIdPiped = (id: number) =>
  pipe(new GetNameById({ id }), Effect.request(UserResolver))

export const getAllUserNamesN = (concurrency: Concurrency) =>
  getAllUserIds.pipe(
    Effect.flatMap(
      Effect.forEach(getUserNameById, { concurrency, batching: true })
    ),
    Effect.onInterrupt(() =>
      FiberRef.getWith(interrupts, (i) => Effect.sync(() => i.interrupts++))
    )
  )
...
export const getAllUserIdsTagged = Effect.request(
  GetAllIds({}),
  UserResolverTagged
)
export const getUserNameByIdTagged = (id: number) =>
  Effect.request(new GetNameById({ id }), UserResolverTagged)
...
describe("Effect", () => {
  it.effect(
    "avoid false interruption when concurrency happens in resolver",
    () =>
      Effect.gen(function* () {
        class RequestUserById extends Request.TaggedClass("RequestUserById")<
          number,
          never,
          {
            id: string
        ...
        const resolver = Resolver.makeBatched((i) => {
          count++
          return Effect.forEach(i, Request.complete(Exit.succeed(1)), {
    ...
    provideEnv(
      Effect.gen(function* () {
        const [a, b] = yield* pipe(
          Effect.zip(getUserNameById(userIds[0]), getUserNameById(userIds[1]), {
            concurrent: true,
            batching: false
          }),
  ...
  it.effect("batching preserves individual & identical requests", () =>
    provideEnv(
      Effect.gen(function* () {
        yield* pipe(
          Effect.all(
            [getUserNameById(userIds[0]), getUserNameById(userIds[0])],
            {
              concurrency: "unbounded",
              batching: true,
              discard: true
