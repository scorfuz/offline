...
describe("Effect", () => {
  ...
  it.effect(
    "filter/negate - filters a collection using an effectual predicate, removing all elements that satisfy the predicate",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make<ReadonlyArray<number>>([])
        const results = yield* pipe(
  ...
  it.effect(
    "filter/concurrency - filters a collection in parallel using an effectual predicate",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "filter/concurrency+negate - filters a collection in parallel using an effectual predicate, removing all elements that satisfy the predicate",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          [2, 4, 6, 3, 5, 6, 10, 11, 15, 17, 20, 22, 23, 25, 28],
  ...
  it.effect("filterOrFail - without orFailWith", () =>
    Effect.gen(function* () {
      ...
      const goodCaseDataFirst = yield* Effect.filterOrFail(
        Effect.succeed(0),
        (n) => n === 0
      )
      const badCase = yield* pipe(
