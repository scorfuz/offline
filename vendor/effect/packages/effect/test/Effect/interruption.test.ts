...
describe("Effect", () => {
    ...
    Effect.gen(function* () {
      ...
      assertTrue(
        Exit.isFailure(result) &&
          Cause.isInterruptedOnly(result.effect_instruction_i0)
      )
    })
  )
  ...
  it.effect("asyncEffect is interruptible", () =>
    Effect.gen(function* () {
      const fiber = yield* pipe(
        Effect.asyncEffect<never, never, never, never, never, never>(
          () => Effect.never
        ),
        Effect.fork
      )
  ...
  it.effect("async is interruptible", () =>
    Effect.gen(function* () {
      const fiber = yield* pipe(
        Effect.async<void, never, never>(constVoid),
        Effect.fork
      )
      const result = yield* Fiber.interrupt(fiber)
      assertTrue(
    ...
    Effect.gen(function* () {
      ...
      const program = Effect.gen(function* () {
        ...
        return yield* pipe(
          ...
          Effect.zipLeft(TestClock.adjust(Duration.seconds(1)), {
            concurrent: true
          })
        )
      })
  ...
  it.effect(
    "acquireUseRelease disconnect release called on interrupt in separate fiber",
    () =>
      Effect.gen(function* () {
        const deferred1 = yield* Deferred.make<void>()
        const deferred2 = yield* Deferred.make<void>()
  ...
  it.effect("interrupted cause persists after catching", () =>
    Effect.gen(function* () {
      const process = (
        list: Chunk.Chunk<Exit.Exit<any>>
      ): Chunk.Chunk<Exit.Exit<any>> => {
              ...
              pipe(
                Effect.uninterruptibleMask((restore) =>
                  pipe(
                    ...
                    Effect.onExit((exit) =>
                      Ref.update(exits, Chunk.prepend(exit))
                    )
                  )
                ),
                Effect.asVoid
        ...
        pipe(
          ...
          Array.reduce(
            ...
            (acc, curr) =>
              acc &&
              Exit.isFailure(curr) &&
              Cause.isInterruptedOnly(curr.effect_instruction_i0)
          )
        )
      )
    })
  ...
  it.effect("interruption of raced", () =>
    Effect.gen(function* () {
      ...
      const raced = yield* pipe(
        make(latch1),
        Effect.race(make(latch2)),
        Effect.fork
      )
      yield* pipe(
        Deferred.await(latch1),
      ...
      const fiber = yield* withLatch((release) =>
        pipe(
          ...
          Effect.matchCauseEffect({
            onFailure: (cause) =>
              Ref.set(recovered, Cause.isInterrupted(cause)),
  ...
  it.effect("sandbox of interruptible", () =>
    Effect.gen(function* () {
      const recovered = yield* Ref.make<
        Option.Option<Either.Either<never, boolean>>
      >(Option.none())
      const fiber = yield* withLatch((release) =>
        pipe(
          release,
          ...
          Effect.flatMap((either) =>
            Ref.set(
              recovered,
              Option.some(pipe(either, Either.mapLeft(Cause.isInterrupted)))
            )
          ),
          Effect.uninterruptible,
          Effect.fork
    ...
    Effect.gen(function* () {
      ...
      const fiber = yield* withLatch((release) =>
        pipe(
          ...
          Effect.exit,
          Effect.flatMap((exit) =>
            Ref.set(recovered, Option.some(Exit.isInterrupted(exit)))
          ),
          Effect.uninterruptible,
          Effect.fork
        )
    ...
    Effect.gen(function* () {
      ...
      const fiber = yield* withLatch((release) =>
        pipe(
          release,
          Effect.zipRight(
            pipe(Effect.never, Effect.interruptible, Effect.exit)
          ),
  ...
  it.live(
    "disconnected effect that is then interrupted eventually performs interruption",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make(false)
        const deferred1 = yield* Deferred.make<void>()
  ...
  it.effect("cause reflects interruption", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        withLatch((release) =>
          pipe(release, Effect.zipRight(Effect.fail("foo")), Effect.fork)
        ),
