...
describe("Effect", () => {
  ...
  it.effect("simple asyncEffect must return", () =>
    Effect.gen(function* () {
      const result = yield* Effect.asyncEffect<
        number,
        never,
        never,
        never,
        ...
        Effect.async<void, never, never>((cb) => {
          Runtime.runCallback(runtime)(
            pipe(
              ...
              Effect.zipRight(
                Effect.sync(() =>
                  cb(Ref.update(unexpectedPlace, Chunk.prepend(1)))
                )
              )
            )
          )
        ...
        Effect.async<void, never, never>((resume) => {
          Runtime.runCallback(runtime)(
            pipe(
              ...
              Effect.zipRight(
                Effect.sync(() =>
                  resume(Ref.update(unexpectedPlace, Chunk.prepend(1)))
                )
              )
            )
          )
  ...
  it.effect("asyncEffect can fail before registering", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Effect.asyncEffect<unknown, unknown, never, never, string, never>(
          (_) => {
