...
const ExampleErrorFail: Effect.Effect<never, Error, never> =
  Effect.fail(ExampleError)

const deepErrorEffect = (
  n: number
...
describe("Effect", () => {
  ...
  it.effect("attempt - fail", () =>
    Effect.gen(function* () {
      ...
      const io2 = Effect.suspend(() =>
        Effect.either(Effect.suspend(() => ExampleErrorFail))
      )
  ...
  it.effect("catch - recovers from one of several tagged errors", () =>
    Effect.gen(function* () {
      ...
      const effect: Effect.Effect<never, ErrorA | ErrorB, never> = Effect.fail({
        _tag: "ErrorA"
      })
  ...
  it.effect(
    "catchIf - does not recover from one of several tagged errors",
    () =>
      Effect.gen(function* () {
        interface ErrorA {
          readonly _tag: "ErrorA"
        ...
        const effect: Effect.Effect<never, ErrorA | ErrorB, never> =
          Effect.fail({ _tag: "ErrorB" })
        const result = yield* pipe(
          Effect.catchIf(
            effect,
  ...
  it.effect(
    "catchTags - does not recover from one of several tagged errors",
    () =>
      Effect.gen(function* () {
        interface ErrorA {
          readonly _tag: "ErrorA"
  ...
  it.effect("orElse - does not recover from defects", () =>
    Effect.gen(function* () {
      ...
      const bothCause = Cause.parallel(
        Cause.interrupt(fiberId),
        Cause.die(error)
      )
      const thenCause = Cause.sequential(
        Cause.interrupt(fiberId),
        Cause.die(error)
      )
      const plain = yield* pipe(
        Effect.die(error),
        Effect.orElse(() => Effect.void),
        Effect.exit
      )
      const both = yield* pipe(
        Effect.failCause(bothCause),
        Effect.orElse(() => Effect.void),
        Effect.exit
      )
      const then = yield* pipe(
        Effect.failCause(thenCause),
        Effect.orElse(() => Effect.void),
        Effect.exit
      )
      const fail = yield* pipe(
        Effect.fail(error),
        Effect.orElse(() => Effect.void),
        Effect.exit
      )
          ...
          if (Cause.isDie(cause)) {
            ...
            if (Chunk.isNonEmpty(defects)) {
              ...
              return Effect.succeed(
                (head as Cause.RuntimeException).message === "2"
              )
            }
          }
  ...
  it("orElse - is associative", async () => {
    ...
    const exits = fc
      .oneof(
        causes.map(
          (s): Either.Either<Effect.Effect<number>, Cause.Cause<number>> =>
            Either.left(s)
        ),
        successes.map(
          (s): Either.Either<Effect.Effect<number>, Cause.Cause<number>> =>
    ...
    await fc.assert(
      fc.asyncProperty(exits, exits, exits, async (exit1, exit2, exit3) => {
        const leftEffect = pipe(
          exit1,
          Effect.orElse(() => exit2),
          Effect.orElse(() => exit3)
        )
        const rightEffect = pipe(
          exit1,
          Effect.orElse(() =>
            pipe(
              exit2,
  ...
  it.effect(
    "orElseFail - executes this effect and returns its value if it succeeds",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Effect.succeed(true),
          Effect.orElseFail(constFalse)
        )
        assertTrue(result)
      })
  )
  ...
  it.effect(
    "orElseSucceed - executes this effect and returns its value if it succeeds",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Effect.succeed(true),
  ...
  it.effect("orElseSucceed - otherwise succeeds with the specified value", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Effect.fail(false),
        Effect.orElseSucceed(constTrue)
      )
      assertTrue(result)
    })
  )
  ...
  it.effect("no information is lost during composition", () =>
    Effect.gen(function* () {
      ...
      const result = yield* cause(
        pipe(Effect.failCause(expectedCause), Effect.sandbox, Effect.unsandbox)
      )
