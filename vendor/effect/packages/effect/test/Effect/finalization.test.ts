...
describe("Effect", () => {
  ...
  it.effect("finalizer errors not caught", () =>
    Effect.gen(function* () {
      ...
      const expected = Cause.sequential(
        Cause.sequential(Cause.fail(ExampleError), Cause.die(e2)),
        Cause.die(e3)
      )
  ...
  it.effect(
    "onExit - executes that a cleanup function runs when effect succeeds",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make(false)
        yield* pipe(
  ...
  it.effect(
    "onExit - ensures that a cleanup function runs when an effect fails",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make(false)
        yield* pipe(
  ...
  it.effect(
    "onExit - ensures that a cleanup function runs when an effect is interrupted",
    () =>
      Effect.gen(function* () {
        ...
        const fiber = yield* pipe(
          ...
          Effect.onExit((exit) =>
            Exit.isFailure(exit) &&
            Cause.isInterrupted(exit.effect_instruction_i0)
