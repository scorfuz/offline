...
describe("Effect.Service", () => {
  ...
  it.effect("promises", () =>
    Effect.gen(function* () {
      ...
      const withUse = yield* Service.use((_) => _.foo()).pipe(
        Effect.flip,
      ...
      const accessor = yield* Service.foo().pipe(
        Effect.flip,
        Effect.provide(Service.Default)
      )
      deepStrictEqual(
      ...
      const accessorSuccess = yield* Service.bar().pipe(
        Effect.provide(Service.Default)
      )
  ...
  it.effect("scoped with arguments & deps", () =>
    Effect.gen(function* () {
      ...
      const x = yield* Service.x.pipe(Effect.provide(Service.Default(42)))
      strictEqual(x, 42)
      const x2 = yield* Service.x.pipe(
        Effect.provide(Service.DefaultWithoutDependencies(42))
      )
      strictEqual(x2, 42)
