...
describe("Effect", () => {
  it.effect("timeout produces a useful error message", () =>
    Effect.gen(function* () {
      ...
      assertTrue(
        result
          .toString()
          .includes("TimeoutException: Operation timed out after '1s 500ms'")
      )
    })
  )
  ...
  it.effect(
    "timeout - disconnect - returns with the produced value if the effect completes before the timeout elapses",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Effect.void,
  ...
  it.effect(
    "timeout - disconnect - returns `NoSuchElementException` otherwise",
    () =>
      Effect.gen(function* () {
