...
describe("Effect", () => {
  ...
  it.effect("memoized returns the same instance on repeated calls", () =>
    it.flakyTest(
      Effect.gen(function* () {
        ...
        const [first, second] = yield* pipe(
          memo,
          Effect.flatMap((effect) => pipe(effect, Effect.zip(effect)))
        )
  ...
  it.effect(
    "memoized function returns the same instance on repeated calls",
    () =>
      it.flakyTest(
        Effect.gen(function* () {
          const randomNumber = (n: number) =>
            Random.nextIntBetween(n, n + n + 1)
  ...
  it.effect("once returns an effect that will only be executed once", () =>
    Effect.gen(function* () {
      ...
      const effect: Effect.Effect<void> = yield* pipe(
        Ref.update(ref, (n) => n + 1),
        Effect.once
      )
      yield* Effect.all(Effect.replicate(effect, 100), {
