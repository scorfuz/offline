...
describe("Effect", () => {
  ...
  it.effect("mapAccum", () =>
    Effect.gen(function* () {
      const result = yield* Effect.mapAccum(["a", "b"], "", (prev, cur, i) =>
        Effect.succeed([prev + cur + i, cur])
      )
      deepStrictEqual(result, ["a0b1", ["a", "b"]])
    })
  )
  it.effect(
    "tryMap - returns an effect whose success is mapped by the specified side effecting function",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Effect.succeed("123"),
  ...
  it.effect(
    "tryMap - translates any thrown exceptions into typed failed effects",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
  ...
  it.effect("summarized - returns summary and value", () =>
    Effect.gen(function* () {
      ...
      const [[start, end], value] = yield* pipe(
        increment,
        Effect.summarized(increment, (start, end) => [start, end] as const)
      )
      strictEqual(start, 1)
      strictEqual(value, 2)
      strictEqual(end, 3)
  ...
  it.effect("point, bind, map", () =>
    Effect.gen(function* () {
      const fibEffect = (n: number): Effect.Effect<number> => {
        ...
        return pipe(
          fibEffect(n - 1),
          Effect.zipWith(fibEffect(n - 2), (a, b) => a + b)
        )
      }
