...
describe("Effect", () => {
  describe("all", () => {
    ...
    it.effect("should work with one iterable argument", () =>
      Effect.gen(function* () {
        const result = yield* Effect.all(
          new Set([Effect.succeed(0), Effect.succeed(1)])
        )
  ...
  describe("all/ concurrency", () => {
    ...
    it.effect("should work with one record argument", () =>
      Effect.gen(function* () {
        const result = yield* Effect.all(
          { a: Effect.succeed(0), b: Effect.succeed(1) },
          {
            concurrency: "unbounded"
          }
        )
    ...
    it.effect("failure should work with one array argument", () =>
      Effect.gen(function* () {
        const res = yield* Effect.flip(
          Effect.all([Effect.fail(0), Effect.succeed(1)], { mode: "validate" })
        )
    ...
    it.effect("failure should work with one record argument", () =>
      Effect.gen(function* () {
        const result = yield* Effect.flip(
          Effect.all(
            { a: Effect.fail(0), b: Effect.succeed(1) },
            { mode: "validate" }
          )
        )
        const { a, b } = result
  ...
  describe("all/ either mode", () => {
    ...
    it.effect("failure should work with one array argument", () =>
      Effect.gen(function* () {
        const res = yield* Effect.all([Effect.fail(0), Effect.succeed(1)], {
          mode: "either"
        })
