...
describe("Effect", () => {
  it.effect("andThen", () =>
    Effect.gen(function* () {
      ...
      const a3 = Effect.succeed(0).pipe(
  ...
  it.effect("tap", () =>
    Effect.gen(function* () {
      ...
      const a2 = Effect.succeed(0).pipe(
        Effect.tap(Effect.succeed(1), { onlyEffect: true })
      )
      const a3 = Effect.tap(Effect.succeed(0), (n) => Effect.succeed(n + 1))
      const a4 = Effect.tap(Effect.succeed(0), (n) => Effect.succeed(n + 1), {
        onlyEffect: true
      })
      const a5 = Effect.succeed(0).pipe(
        Effect.tap((n) => Effect.succeed(n + 1))
      )
      const a6 = Effect.succeed(0).pipe(
        Effect.tap((n) => Effect.succeed(n + 1), { onlyEffect: true })
      )
  ...
  describe("", () => {
    it.effect(
      "tapErrorCause - effectually peeks at the cause of the failure of this effect",
      () =>
        Effect.gen(function* () {
          const ref = yield* Ref.make(false)
          const result = yield* pipe(
            Effect.dieMessage("die"),
            Effect.tapErrorCause(() => Ref.set(ref, true)),
            Effect.exit
          )
          const effect = yield* Ref.get(ref)
          assertTrue(
            Exit.isFailure(result) &&
              Option.isSome(Cause.dieOption(result.effect_instruction_i0))
          )
          assertTrue(effect)
        })
    )
  ...
  it.effect("unless - executes correct branch only", () =>
    Effect.gen(function* () {
      ...
      const failed = yield* pipe(
        Effect.fail(failure),
        Effect.unless(constFalse),
        Effect.either
      )
      strictEqual(v1, 0)
      strictEqual(v2, 2)
  ...
  it.effect("unlessEffect - executes condition effect and correct branch", () =>
    Effect.gen(function* () {
      ...
      const conditionTrue = pipe(
        Ref.update(conditionRef, (n) => n + 1),
        Effect.as(true)
      )
      const conditionFalse = pipe(
        Ref.update(conditionRef, (n) => n + 1),
        Effect.as(false)
      )
      ...
      const failed = yield* pipe(
        Effect.fail(failure),
        Effect.unlessEffect(conditionFalse),
        Effect.either
      )
      strictEqual(v1, 0)
      strictEqual(c1, 1)
  ...
  it.effect("whenEffect - executes condition effect and correct branch", () =>
    Effect.gen(function* () {
      ...
      const failed = yield* pipe(
        Effect.fail(failure),
        Effect.whenEffect(conditionTrue),
        Effect.either
      )
      strictEqual(v1, 0)
      strictEqual(c1, 1)
  ...
  it.effect(
    "zip/parallel - does not report failure when interrupting loser after it succeeded",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          Effect.interrupt,
        ...
        return n === 0
          ...
          : pipe(
              ...
              Effect.flatMap((tuple) =>
                pipe(
                  countdown(n - 1),
                  Effect.map((y) => tuple[0] + tuple[1] + y)
                )
              )
            )
      }
  ...
  it.effect(
    "zip/parallel - does not kill fiber when forked on parent scope",
    () =>
      Effect.gen(function* () {
            ...
            Effect.zipRight(
              restore(
                pipe(
                  ...
                  Effect.zipRight(Effect.succeed("foo"))
                )
              )
            ),
        ...
        const result = yield* pipe(
          Effect.fork(left),
          Effect.zip(right, { concurrent: true })
        )
