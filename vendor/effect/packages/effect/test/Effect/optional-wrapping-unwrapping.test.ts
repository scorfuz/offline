...
describe("Effect", () => {
  describe("transposeOption", () => {
    ...
    it.effect("Some", () =>
      Effect.gen(function* () {
        const result = yield* Effect.transposeOption(
          Option.some(Effect.succeed(42))
        )
        ...
        it.effect("Failure", () =>
          Effect.gen(function* () {
            const resultDataFirst = yield* pipe(
              Effect.transposeMapOption(Option.some(42), () =>
                Effect.fail("error")
              ),
              Effect.flip
            )
...
describe("Either", () => {
  describe("transposeOption", () => {
    ...
    it.effect("Some", () =>
      Effect.gen(function* () {
        const result = yield* Either.transposeOption(
          Option.some(Either.right(42))
        )
        assert.deepStrictEqual(result, Option.some(42))
      })
    )
