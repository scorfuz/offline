...
describe("Effect", () => {
  it.effect("TaggedError has a stack", () =>
    Effect.gen(function* () {
      const cause = yield* Effect.flip(
        Effect.sandbox(Effect.withSpan("A")(new TestError()))
      )
      ...
      if (typeof window === "undefined") {
        assertInclude(
          log.replaceAll("\\", "/"),
          "test/Effect/error.test.ts:10:77"
        )
      }
      ...
      const cause = yield* pipe(
        Effect.tryPromise({
          try: () => Promise.reject("fail"),
            ...
            new MessageError({ name: "Failure", message: "some message" })
        }),
        Effect.withSpan("A"),
        Effect.sandbox,
        Effect.flip
  ...
  if (typeof window === "undefined") {
    ...
    it("inspect", () => {
      ...
      assertInclude(
        inspect(err).replaceAll("\\", "/"),
        "test/Effect/error.test.ts:70"
      )
    })
    ...
    it("cause", () => {
      ...
      assertInclude(
        inspect(new MessageError({ cause: new Error("boom") })),
        "[cause]: Error: boom"
      )
    })
  }
