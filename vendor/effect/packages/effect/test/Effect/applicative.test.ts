...
describe("Effect", () => {
  ...
  it.effect("two successes should succeed", () =>
    Effect.gen(function* () {
      const result = yield* Effect.succeed(add).pipe(
  ...
  it.effect("one failure in data-last position should fail", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Effect.succeed(add).pipe(
          Effect.ap(Effect.succeed(1)),
          Effect.ap(Effect.fail("c"))
        ),
        Effect.either
      )
      assertLeft(result, "c")
  ...
  it.effect(
    "an applicative operation that starts with a failure should fail",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
