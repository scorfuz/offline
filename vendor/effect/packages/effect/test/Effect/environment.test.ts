...
class NumberRepo extends Context.Tag("NumberRepo")<
  NumberRepo,
  {
...
describe("Effect", () => {
  ...
  it.effect("effect tag with primitives", () =>
    Effect.gen(function* () {
      ...
      Effect.provide(Layer.mergeAll(DateTag.Live, NumberTag.Live, MapTag.Live))
    )
  )
  it.effect("class tag", () =>
    Effect.gen(function* () {
      yield* Effect.flatMap(NumberRepo.numbers, (_) =>
        Effect.log(`Numbers: ${_}`)
      ).pipe(Effect.provideService(NumberRepo, { numbers: [0, 1, 2] }))
    })
  )
  ...
  it.effect(
    "environment - provideSomeContext provides context in the right order",
    () =>
      pipe(
        Effect.gen(function* () {
          const v1 = yield* NumberService
  ...
  it.effect("environment - async can use environment", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Effect.async<number, never, NumberService>((cb) =>
          cb(Effect.map(NumberService, ({ n }) => n))
        ),
  ...
  it.effect(
    "serviceWith - effectfully accesses a service in the environment",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
  ...
  it.effect("updateService - updates a service in the environment", () =>
    pipe(
      Effect.gen(function* () {
        const a = yield* pipe(
          NumberService,
          Effect.updateService(NumberService, ({ n }) => ({ n: n + 1 }))
        )
  ...
  it.effect(
    "serviceMembers - expose both service functions and constants",
    () => {
      interface Service {
        foo: (x: string, y: number) => Effect.Effect<string>
        baz: Effect.Effect<string>
