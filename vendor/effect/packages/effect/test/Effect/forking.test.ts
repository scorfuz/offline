...
describe("Effect", () => {
  ...
  it.effect("fork - propagates interruption with zip of defect", () =>
    Effect.gen(function* () {
      ...
      const result = yield* pipe(
        ...
        Effect.map(Exit.mapErrorCause((cause) => cause))
      )
      assertTrue(Exit.isInterrupted(result))
    })
  )
  ...
  it.effect(
    "forkWithErrorHandler - calls provided function when task fails",
    () =>
      Effect.gen(function* () {
        const deferred = yield* Deferred.make<void>()
        yield* pipe(
          Effect.fail<void>(void 0),
          Effect.forkWithErrorHandler((e) =>
            pipe(Deferred.succeed(deferred, e), Effect.asVoid)
          )
        )
  ...
  it.effect("forkAll - propagate failures", () =>
    Effect.gen(function* () {
      ...
      const result = yield* pipe(
        [fail],
        Effect.forkAll(),
        Effect.flatMap((fiber) => Effect.flip(Fiber.join(fiber)))
      )
  ...
  it.effect("forkAll - propagates defects", () =>
    Effect.gen(function* () {
      ...
      const fiber3 = yield* Effect.forkAll([
        die,
        Effect.succeed(42),
        Effect.never
      ])
