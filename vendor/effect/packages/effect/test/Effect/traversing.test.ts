...
describe("Effect", () => {
  ...
  it.effect(
    "exists - determines whether any element satisfies the effectual predicate",
    () =>
      Effect.gen(function* () {
        const array = [1, 2, 3, 4, 5]
        const result1 = yield* pipe(
          array,
          Effect.exists((n) => Effect.succeed(n > 3))
        )
  ...
  it.effect(
    "forAll - determines whether all elements satisfy the effectual predicate",
    () =>
      Effect.gen(function* () {
        const array = [1, 2, 3, 4, 5, 6]
        const result1 = yield* pipe(
          array,
          Effect.every((n) => Effect.succeed(n > 3))
        )
        const result2 = yield* pipe(
          array,
          Effect.every((n) => Effect.succeed(n > 0))
        )
        assertFalse(result1)
        assertTrue(result2)
      })
  ...
  it.effect("replicate - zero", () =>
    Effect.gen(function* () {
      const result = yield* Effect.all(
        pipe(Effect.succeed(12), Effect.replicate(0))
      )
  ...
  it.effect("replicate - negative", () =>
    Effect.gen(function* () {
      const result = yield* Effect.all(
        pipe(Effect.succeed(12), Effect.replicate(-2))
      )
      strictEqual(result.length, 0)
    })
  )
  ...
  it.effect(
    "forEach - both evaluates effects and returns results in the same order",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("forEach/discard - runs effects in order", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(
        [1, 2, 3, 4, 5],
        Effect.forEach((n) => Ref.update(ref, Array.append(n)), {
          discard: true
        })
      )
      const result = yield* Ref.get(ref)
  ...
  it.effect("forEach/discard - can be run twice", () =>
    Effect.gen(function* () {
      ...
      const effect = pipe(
        [1, 2, 3, 4, 5],
        Effect.forEach((n) => Ref.update(ref, (_) => _ + n), { discard: true })
      )
      yield* effect
      yield* effect
      const result = yield* Ref.get(ref)
  ...
  it.effect("forEach/concurrency - runs a task that dies", () =>
    Effect.gen(function* () {
      ...
      assertTrue(
        Exit.isFailure(result) && Cause.isDie(result.effect_instruction_i0)
      )
    })
  )
  it.effect("forEach/concurrency - runs a task that is interrupted", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Array.makeBy(10, (i) => i + 1),
        Effect.forEach(
          (n) => (n === 5 ? Effect.interrupt : Effect.succeed(n * 2)),
          { concurrency: "unbounded" }
        ),
        Effect.exit
      )
  ...
  it.effect(
    "forEach/concurrency - runs a task that throws an unsuspended exception",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          [1],
  ...
  it.effect("forEach/concurrency - returns results in the same order", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        ["1", "2", "3"],
        Effect.forEach((s) => Effect.sync(() => Number.parseInt(s)), {
          concurrency: "unbounded"
        })
      )
      deepStrictEqual(result, [1, 2, 3])
  ...
  it.effect("forEach/concurrency - propagates error", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        [1, 2, 3, 4, 5, 6],
        Effect.forEach(
          (n) => (n % 2 !== 0 ? Effect.succeed(n) : Effect.fail("not odd")),
          { concurrency: "unbounded" }
        ),
        Effect.flip
      )
  ...
  it.effect(
    "forEach/concurrency - does not kill fiber when forked on the parent scope",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make(0)
  ...
  it.effect(
    "forEach/concurrency - parallelism - returns the results in the appropriate order",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          [1, 2, 3],
          Effect.forEach((n) => Effect.succeed(n.toString()), {
            concurrency: 2
          })
        )
        deepStrictEqual(result, ["1", "2", "3"])
  ...
  it.effect(
    "forEach/concurrency - parallelism - runs effects in parallel",
    () =>
      Effect.gen(function* () {
        const deferred = yield* Deferred.make<void>()
        yield* pipe(
  ...
  it.effect(
    "forEach/concurrency - parallelism - interrupts effects on first failure",
    () =>
      Effect.gen(function* () {
        const actions = [Effect.never, Effect.succeed(1), Effect.fail("C")]
        const result = yield* pipe(
          actions,
  ...
  it.effect("forEach/concurrency+discard - accumulates errors", () =>
    Effect.gen(function* () {
      ...
      const result = yield* pipe(
        [1, 2, 3],
        Effect.forEach(
          (n) => pipe(task(started, trigger, n), Effect.uninterruptible),
          {
            concurrency: "unbounded",
            discard: true
          }
  ...
  it.effect(
    "forEach/concurrency+discard - parallelism - runs all effects",
    () =>
      Effect.gen(function* () {
        const ref = yield* Ref.make(Chunk.empty<number>())
        yield* pipe(
  ...
  it.effect("mergeAll - return error if it exists in list", () =>
    Effect.gen(function* () {
      const effects: ReadonlyArray<Effect.Effect<void, number>> = [
        Effect.void,
        Effect.fail(1)
      ]
      const result = yield* pipe(
  ...
  it.effect("partition - evaluates effects in correct order", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(
        array,
        Effect.partition((n) => Ref.update(ref, Chunk.prepend(n)))
      )
      const result = yield* pipe(Ref.get(ref), Effect.map(Chunk.reverse))
  ...
  it.effect(
    "partition/concurrency - parallelism - collects failures and successes",
    () =>
      Effect.gen(function* () {
        const array = Array.makeBy(10, (i) => i)
        const [left, right] = yield* pipe(
  ...
  it.effect(
    "reduce - with a successful step function sums the list properly",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          [1, 2, 3, 4, 5],
  ...
  it.effect("reduce - run sequentially from left to right", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        [1, 2, 3, 4, 5],
        Effect.reduce([] as ReadonlyArray<number>, (acc, curr) =>
  ...
  it.effect(
    "reduceRight - with a successful step function sums the list properly",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          [1, 2, 3, 4, 5],
  ...
  it.effect(
    "reduceRight - with a failing step function returns a failed IO",
    () =>
      Effect.gen(function* () {
        const result = yield* pipe(
          [1, 2, 3, 4, 5],
  ...
  it.effect("reduceRight - run sequentially from right to left", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        [1, 2, 3, 4, 5],
        Effect.reduceRight([] as ReadonlyArray<number>, (curr, acc) =>
          Effect.succeed([curr, ...acc])
        )
      )
      deepStrictEqual(result, [1, 2, 3, 4, 5])
    })
  ...
  it.effect(
    "reduceEffect/concurrency - return zero element on empty input",
    () =>
      Effect.gen(function* () {
        const zeroElement = 42
        const nonZero = 43
  ...
  it.effect(
    "reduceEffect/concurrency - return error if it exists in list",
    () =>
      Effect.gen(function* () {
        const effects: ReadonlyArray<Effect.Effect<void, number>> = [
          Effect.void,
        ...
        const result = yield* pipe(
          effects,
          Effect.reduceEffect(
            Effect.void as Effect.Effect<void, number>,
            constVoid,
            {
              concurrency: "unbounded"
            }
  ...
  it.effect("takeUntil - happy path", () =>
    Effect.gen(function* () {
      const result = yield* Effect.takeUntil([1, 2, 3, 4, 5], (n) =>
        Effect.succeed(n >= 3)
      )
      deepStrictEqual(result, [1, 2, 3])
    })
