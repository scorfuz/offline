...
describe("Effect", () => {
      ...
      const cause = yield* Effect.flip(
        Effect.sandbox(
          Effect.withSpan("spanB")(
            Effect.withSpan("spanA")(Effect.fail(new Error("ok")))
          )
        )
      )
  ...
  it.effect("includes span name in stack", () =>
    Effect.gen(function* () {
      ...
      const cause = yield* fn(0).pipe(Effect.sandbox, Effect.flip)
      const prettyErrors = Cause.prettyErrors(cause)
  ...
  it.effect("multiline message", () =>
    Effect.gen(function* () {
      const cause = yield* Effect.fail(
        new Error("Multi-line\nerror\nmessage")
      ).pipe(Effect.sandbox, Effect.flip)
      const pretty = Cause.pretty(cause)
      assertTrue(
        pretty.startsWith(`Error: Multi-line
error
message
    at`)
      )
  ...
  it.effect("pretty nested cause", () =>
    Effect.gen(function* () {
      const cause = yield* Effect.fail(
        new Error("parent", {
          cause: new Error("child", { cause: new Error("child2") })
        })
      ).pipe(Effect.sandbox, Effect.flip)
      const pretty = Cause.pretty(cause, { renderErrorCause: true })
      assertInclude(pretty, "[cause]: Error: child")
