...
describe("Effect", () => {
  ...
  it.effect(
    "repeat/until - repeats until the effectful condition is true",
    () =>
      Effect.gen(function* () {
        const input = yield* Ref.make(10)
        const output = yield* Ref.make(0)
  ...
  it.effect("repeat/until - always evaluates the effect at least once", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(
        Ref.update(ref, (n) => n + 1),
        Effect.repeat({ until: () => Effect.succeed(true) })
      )
      const result = yield* Ref.get(ref)
      strictEqual(result, 1)
    })
  ...
  it.effect("repeat/while - always evaluates the effect at least once", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(
        Ref.update(ref, (n) => n + 1),
        Effect.repeat({ while: constFalse })
      )
      const result = yield* Ref.get(ref)
      strictEqual(result, 1)
    })
  ...
  it.effect("repeat/while - always evaluates effect at least once", () =>
    Effect.gen(function* () {
      ...
      yield* pipe(
        Ref.update(ref, (n) => n + 1),
        Effect.repeat({ while: () => Effect.succeed(false) })
      )
      const result = yield* Ref.get(ref)
      strictEqual(result, 1)
    })
