...
describe("Effect", () => {
  ...
  it.live("supervise fibers", () =>
    Effect.gen(function* () {
      const makeChild = (n: number): Effect.Effect<Fiber.Fiber<void>> => {
        return pipe(
          Effect.sleep(Duration.millis(20 * n)),
          Effect.zipRight(Effect.never),
          Effect.fork
        )
      }
  ...
  it.effect(
    "race in uninterruptible region",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("firstSuccessOf of values", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        Effect.firstSuccessOf([Effect.fail(0), Effect.succeed(100)]),
        Effect.either
      )
  ...
  it.effect("raceFirst interrupts loser on failure", () =>
    Effect.gen(function* () {
      ...
      const winner = pipe(
        ...
        Effect.zipRight(Either.left(new Error()))
      )
      const loser = Effect.acquireUseRelease(
        Deferred.succeed(deferred, void 0),
        () => Effect.never,
  ...
  it.effect("mergeAll", () =>
    Effect.gen(function* () {
      const result = yield* pipe(
        ["a", "aa", "aaa", "aaaa"].map((a) => Effect.succeed(a)),
        Effect.mergeAll(0, (b, a) => b + a.length)
      )
