  ...
  describe.concurrent("fixed-window", () => {
    ...
    it.scoped("will use the provided cost", () =>
      Effect.gen(function* () {
        ...
        const fib = yield* Effect.replicateEffect(
          rl(Clock.currentTimeMillis).pipe(RateLimiter.withCost(10)),
          20
        ).pipe(Effect.fork)
    ...
    it.scoped(
      "will respect different costs per effect and interleave them.",
      () =>
        Effect.gen(function* () {
          const rl = yield* RateLimiter.make({
            limit: 10,
          ...
          const tasks = [
            rl1(Clock.currentTimeMillis).pipe(
              Effect.map((x) => ["rl1", x] as const)
            ),
            rl1(Clock.currentTimeMillis).pipe(
              Effect.map((x) => ["rl1", x] as const)
            ),
            rl2(Clock.currentTimeMillis).pipe(
              Effect.map((x) => ["rl2", x] as const)
            ),
            rl2(Clock.currentTimeMillis).pipe(
              Effect.map((x) => ["rl2", x] as const)
            )
          ...
          const fib = yield* Effect.all(tasks, {
    ...
    it.scoped(
      "will be composable with other `RateLimiters`",
      () =>
        Effect.gen(function* () {
...
const RateLimiterTestSuite = (algorithm: "fixed-window" | "token-bucket") => {
  it.scoped(`${algorithm} - execute up to max calls immediately`, () =>
    Effect.gen(function* () {
      ...
      const now = yield* Clock.currentTimeMillis
      const times = yield* Effect.forEach(Array.range(1, 10), () =>
  ...
  it.scoped(`${algorithm} - is not affected by stream chunk size`, () =>
    Effect.gen(function* () {
      ...
      yield* TestClock.adjust("1 seconds")
      const times2 = yield* Effect.forEach(fibers, Fiber.join, {
        concurrency: "unbounded"
      })
  ...
  it.scoped(`${algorithm} - holds back up calls after the max`, () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* pipe(
        Effect.replicateEffect(limit(Clock.currentTimeMillis), 20),
        Effect.fork
      )
  ...
  it.scoped(
    `${algorithm} - will interrupt the effect when a call is interrupted`,
    () =>
      Effect.gen(function* () {
        const limit = yield* RateLimiter.make({
          limit: 10,
  ...
  it.scoped(
    `${algorithm} - will not start execution of an effect when it is interrupted before getting its turn to execute`,
    () =>
      Effect.gen(function* () {
  ...
  it.scoped(
    `${algorithm} - will wait for interruption to complete of an effect that is already executing`,
    () =>
      Effect.gen(function* () {
        const limit = yield* RateLimiter.make({
          limit: 1,
  ...
  it.scoped(
    `${algorithm} - will make effects wait for interrupted effects to pass through the rate limiter`,
    () =>
      Effect.gen(function* () {
        const limit = yield* RateLimiter.make({
          limit: 1,
  ...
  it.scoped(
    "will not include interrupted effects in the throttling",
    () =>
      Effect.gen(function* () {
        const rate = 10
        const limit = yield* RateLimiter.make({
