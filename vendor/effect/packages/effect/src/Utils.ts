 ...
 * @since 2.0.0
 */
export interface GenKind<F extends TypeLambda, R, O, E, A> extends Variance<
  F,
  R,
  O,
  E
 ...
 * @since 3.0.6
 */
export const isGenKind = (u: unknown): u is GenKind<any, any, any, any, any> =>
  isObject(u) && GenKindTypeId in u

/**
 * @category constructors
 * @since 2.0.0
 */
export class GenKindImpl<F extends TypeLambda, R, O, E, A> implements GenKind<
  F,
  R,
  O,
  E,
...
export interface Gen<F extends TypeLambda, Z> {
  <
    ...
    K extends
      ...
      | YieldWrap<Kind<F, any, any, any, any>>,
    A
  >(
    ...args:
...
export interface Adapter<Z extends TypeLambda> {
  ...
  <A, _R, _O, _E, _A>(
    a: A,
    ab: (a: A) => Kind<Z, _R, _O, _E, _A>
  ): GenKind<Z, _R, _O, _E, _A>
  <A, B, _R, _O, _E, _A>(
    a: A,
    ab: (a: A) => B,
    bc: (b: B) => Kind<Z, _R, _O, _E, _A>
  ): GenKind<Z, _R, _O, _E, _A>
  <A, B, C, _R, _O, _E, _A>(
    a: A,
    ab: (a: A) => B,
 ...
 * @since 3.0.6
 */
export const YieldWrapTypeId: unique symbol = Symbol.for(
  "effect/Utils/YieldWrap"
)
...
export const structuralRegion = <A>(
  body: () => A,
  tester?: (a: unknown, b: unknown) => boolean
): A => {
...
const isNotOptimizedAway =
  standard
    ...
    ?.includes("effect_internal_function") === true
...
export const internalCall = isNotOptimizedAway
  ...
  : forced.effect_internal_function
...
export const isGeneratorFunction = (
  u: unknown
): u is (...args: Array<any>) => Generator<any, any, any> =>
