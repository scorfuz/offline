 ...
 * @category models
 */
export interface Micro<out A, out E = never, out R = never> extends Effect<
  A,
  E,
  R
> {
 ...
 * @since 3.4.3
 */
export interface MicroUnify<
  A extends { [Unify.typeSymbol]?: any }
> extends EffectUnify<A> {
  Micro?: () => A[Unify.typeSymbol] extends
    | Micro<infer A0, infer E0, infer R0>
    | infer _
    ? Micro<A0, E0, R0>
    : never
}
...
export declare namespace Micro {
  ...
  export type Success<T> =
    T extends Micro<infer _A, infer _E, infer _R> ? _A : never
   ...
   */
  export type Error<T> =
    T extends Micro<infer _A, infer _E, infer _R> ? _E : never
  ...
  export type Context<T> =
    T extends Micro<infer _A, infer _E, infer _R> ? _R : never
}
 ...
 * @category guards
 */
export const isMicro = (u: unknown): u is Micro<any, any, any> =>
...
export interface MicroIterator<T extends Micro<any, any, any>> {
  ...
  ): IteratorResult<YieldWrap<T>, Micro.Success<T>>
}
 ...
 * @category guards
 */
export const isMicroCause = (self: unknown): self is MicroCause<unknown> =>
  hasProperty(self, MicroCauseTypeId)
...
export declare namespace MicroCause {
   ...
   * @experimental
   */
  export interface Proto<Tag extends string, E>
    extends Pipeable, globalThis.Error {
...
abstract class MicroCauseImpl<Tag extends string, E>
  extends globalThis.Error
  implements MicroCause.Proto<Tag, E>
{
  readonly [MicroCauseTypeId]: {
    _E: Covariant<E>
  }
...
class Interrupt
  extends MicroCauseImpl<"Interrupt", never>
  implements MicroCause.Interrupt
{
 ...
 * @category MicroCause
 */
export const causeIsDie = <E>(self: MicroCause<E>): self is MicroCause.Die =>
  self._tag === "Die"
...
class MicroFiberImpl<in out A = any, in out E = any> implements MicroFiber<
  A,
  E
> {
  readonly [MicroFiberTypeId]: MicroFiber.Variance<A, E>
  ...
  getCont<S extends successCont | failureCont>(
    ...
    | (Primitive & Record<S, (value: any, fiber: MicroFiberImpl) => Primitive>)
    | undefined {
    while (true) {
...
const fiberMiddleware = globalValue("effect/Micro/fiberMiddleware", () => ({
  interruptChildren: undefined as
    | ((fiber: MicroFiberImpl) => Micro<void> | undefined)
    | undefined
}))
 ...
 * @category MicroFiber
 */
export const fiberJoin = <A, E>(self: MicroFiber<A, E>): Micro<A, E> =>
 ...
 * @category MicroFiber
 */
export const fiberInterruptAll = <A extends Iterable<MicroFiber<any, any>>>(
  fibers: A
): Micro<void> =>
  suspend(() => {
...
>(options: {
  ...
  readonly prop: Prop
  readonly eval: (
    this: MicroExit<unknown, unknown> & { [args]: Parameters<Fn>[0] },
 ...
 * @category constructors
 */
export const fail = <E>(error: E): Micro<never, E> =>
 ...
 * @category constructors
 */
export const succeedSome = <A>(a: A): Micro<Option.Option<A>> =>
...
): Micro<never, E> => suspend(() => failCause(evaluate()))
 ...
 * @category constructors
 */
export const failSync = <E>(error: LazyArg<E>): Micro<never, E> =>
  suspend(() => fail(error()))
...
): Micro<A, NoSuchElementException> =>
  option._tag === "Some"
...
const asyncFinalizer: (onInterrupt: () => Micro<void, any, any>) => Primitive =
  makePrimitive({
    op: "AsyncFinalizer",
    ensure(fiber) {
...
export const gen = <Self, Eff extends YieldWrap<Micro<any, any, any>>, AEff>(
  ...
  [Eff] extends [never]
    ? never
    : [Eff] extends [YieldWrap<Micro<infer _A, infer E, infer _R>>]
      ? E
      : never,
  [Eff] extends [never]
    ? never
    : [Eff] extends [YieldWrap<Micro<infer _A, infer _E, infer R>>]
      ? R
      : never
> =>
  suspend(() =>
    fromIterator(args.length === 1 ? args[0]() : (args[1].call(args[0]) as any))
  )
...
export const asSome = <A, E, R>(
  self: Micro<A, E, R>
): Micro<Option.Option<A>, E, R> => map(self, Option.some)
...
export const andThen: {
  <A, X>(
    ...
    self: Micro<A, E, R>
  ) => [X] extends [Micro<infer A1, infer E1, infer R1>]
    ? Micro<A1, E | E1, R | R1>
    : Micro<X, E, R>
  <X>(
    f: NotFunction<X>
...
export const tap: {
  <A, X>(
    ...
    self: Micro<A, E, R>
  ) => [X] extends [Micro<infer _A1, infer E1, infer R1>]
    ? Micro<A, E | E1, R | R1>
    : Micro<A, E, R>
  <X>(
    f: NotFunction<X>
 ...
 * @category mapping & sequencing
 */
export const asVoid = <A, E, R>(self: Micro<A, E, R>): Micro<void, E, R> =>
...
export const sandbox = <A, E, R>(
  self: Micro<A, E, R>
): Micro<A, MicroCause<E>, R> => catchAllCause(self, fail)
...
export const race: {
  ...
  <A, E, R, A2, E2, R2>(
    self: Micro<A, E, R>,
    that: Micro<A2, E2, R2>
  ): Micro<A | A2, E | E2, R | R2> => raceAll([self, that])
)
...
export const raceFirst: {
  ...
  <A, E, R, A2, E2, R2>(
    self: Micro<A, E, R>,
    that: Micro<A2, E2, R2>
  ): Micro<A | A2, E | E2, R | R2> => raceAllFirst([self, that])
)
 ...
 * @category MicroExit
 */
export const isMicroExit = (u: unknown): u is MicroExit<unknown, unknown> =>
  hasProperty(u, MicroExitTypeId)
 ...
 * @category MicroExit
 */
export const exitFailCause: <E>(cause: MicroCause<E>) => MicroExit<never, E> =
  failCause as any
 ...
 * @category MicroExit
 */
export const exitFail = <E>(e: E): MicroExit<never, E> =>
 ...
 * @category MicroExit
 */
export const exitDie = (defect: unknown): MicroExit<never> =>
...
export const exitVoidAll = <I extends Iterable<MicroExit<any, any>>>(
  ...
  void,
  I extends Iterable<MicroExit<infer _A, infer _E>> ? _E : never
> => {
...
export const serviceOption = <I, S>(
  ...
  withMicroFiber((fiber) => succeed(Context.getOption(fiber.context, tag)))
...
export class MaxOpsBeforeYield extends Context.Reference<MaxOpsBeforeYield>()<
  ...
  number
>("effect/Micro/currentMaxOpsBeforeYield", { defaultValue: () => 2048 }) {}
...
export class CurrentConcurrency extends Context.Reference<CurrentConcurrency>()<
  ...
  "unbounded" | number
>("effect/Micro/currentConcurrency", { defaultValue: () => "unbounded" }) {}
...
export const when: {
  ...
  <A, E, R, E2 = never, R2 = never>(
    ...
    flatMap(isMicro(condition) ? condition : sync(condition), (pass) =>
...
export const scheduleExponential =
  (baseMillis: number, factor = 2): MicroSchedule =>
  (attempt) =>
...
export const catchTag: {
  <K extends E extends { _tag: string } ? E["_tag"] : never, E, A1, E1, R1>(
    ...
    self: Micro<A, E, R>
  ) => Micro<A1 | A, E1 | Exclude<E, { _tag: K }>, R1 | R>
  <
    A,
    E,
    R,
...
export const mapErrorCause: {
  <E, E2>(
    f: (e: MicroCause<E>) => MicroCause<E2>
  ): <A, R>(self: Micro<A, E, R>) => Micro<A, E2, R>
  <A, E, R, E2>(
    self: Micro<A, E, R>,
    f: (e: MicroCause<E>) => MicroCause<E2>
  ): Micro<A, E2, R>
  ...
  <A, E, R, E2>(
    self: Micro<A, E, R>,
    f: (e: MicroCause<E>) => MicroCause<E2>
  ): Micro<A, E2, R> => catchAllCause(self, (cause) => failCause(f(cause)))
)
...
export const either = <A, E, R>(
  self: Micro<A, E, R>
): Micro<Either.Either<A, E>, never, R> =>
...
export const timeout: {
  (
    millis: number
  ): <A, E, R>(self: Micro<A, E, R>) => Micro<A, E | TimeoutException, R>
  <A, E, R>(
    self: Micro<A, E, R>,
    millis: number
  ): Micro<A, E | TimeoutException, R>
} = dual(
  2,
  <A, E, R>(
    self: Micro<A, E, R>,
...
export const timeoutOption: {
  (
    millis: number
  ): <A, E, R>(self: Micro<A, E, R>) => Micro<Option.Option<A>, E, R>
 ...
 */
export const MicroScopeTypeId: unique symbol = Symbol.for(
  "effect/Micro/MicroScope"
)
 ...
 * @category resources & finalization
 */
export const MicroScope: Context.Tag<MicroScope, MicroScope> =
  Context.GenericTag<MicroScope>("effect/Micro/MicroScope")
 ...
 * @category resources & finalization
 */
export const scopeMake: Micro<MicroScope.Closeable> = sync(
  () => new MicroScopeImpl()
)
...
export const provideScope: {
  (
    scope: MicroScope
  ): <A, E, R>(self: Micro<A, E, R>) => Micro<A, E, Exclude<R, MicroScope>>
  <A, E, R>(
    self: Micro<A, E, R>,
    scope: MicroScope
  ): Micro<A, E, Exclude<R, MicroScope>>
} = dual(
  2,
  <A, E, R>(
    self: Micro<A, E, R>,
...
export const scoped = <A, E, R>(
  ...
  suspend(() => {
    ...
    return onExit(provideService(self, MicroScope, scope), (exit) =>
      scope.close(exit)
    )
  })
...
export declare namespace All {
  ...
  export type ReturnIterable<
    T extends Iterable<MicroAny>,
    Discard extends boolean
  > = [T] extends [Iterable<Micro<infer A, infer E, infer R>>]
    ...
    Micro<
      Discard extends true
        ...
        : T[number] extends never
          ? []
          : {
              -readonly [K in keyof T]: T[K] extends Micro<
                infer _A,
                infer _E,
                infer _R
              >
      ...
      T[number] extends never
        ? never
        : T[number] extends Micro<infer _A, infer _E, infer _R>
          ? _E
          : never,
      T[number] extends never
        ? never
  ...
  export type ReturnObject<T, Discard extends boolean> = [T] extends [
    ...
    ? Micro<
        Discard extends true
          ...
          : {
              -readonly [K in keyof T]: [T[K]] extends [
                Micro<infer _A, infer _E, infer _R>
              ]
                ? _A
                : never
            },
        keyof T extends never
          ? never
          : T[keyof T] extends Micro<infer _A, infer _E, infer _R>
            ? _E
            : never,
        keyof T extends never
          ? never
  ...
  export type Return<
    ...
    O extends NoExcessProperties<
      ...
      O
    >
  > = [Arg] extends [ReadonlyArray<MicroAny>]
...
export const all = <
  const Arg extends
    | Iterable<Micro<any, any, any>>
    | Record<string, Micro<any, any, any>>,
  O extends NoExcessProperties<
    {
      readonly concurrency?: Concurrency | undefined
      readonly discard?: boolean | undefined
...
export const forkDaemon = <A, E, R>(
  ...
  withMicroFiber((fiber) => succeed(unsafeFork(fiber, self, false, true)))
...
export const forkIn: {
  (
    scope: MicroScope
  ): <A, E, R>(self: Micro<A, E, R>) => Micro<MicroFiber<A, E>, never, R>
  <A, E, R>(
    self: Micro<A, E, R>,
    scope: MicroScope
  ): Micro<MicroFiber<A, E>, never, R>
...
export const forkScoped = <A, E, R>(
  self: Micro<A, E, R>
): Micro<MicroFiber<A, E>, never, R | MicroScope> =>
...
export const runFork = <A, E>(
  ...
  const fiber = new MicroFiberImpl<A, E>(
    CurrentScheduler.context(options?.scheduler ?? new MicroSchedulerDefault())
  )
  fiber.evaluate(effect as any)
  if (options?.signal) {
...
export interface YieldableError extends Pipeable, Inspectable, Readonly<Error> {
  ...
  readonly [Effectable.SinkTypeId]: Sink.VarianceStruct<
    never,
    unknown,
    never,
    this,
