 ...
 * @category models
 */
export interface STMUnify<
  A extends { [Unify.typeSymbol]?: any }
> extends Effect.EffectUnify<A> {
  STM?: () => A[Unify.typeSymbol] extends
    | STM<infer A0, infer E0, infer R0>
    | infer _
    ? STM<A0, E0, R0>
    : never
}
 ...
 */
export const isSTM: (u: unknown) => u is STM<unknown, unknown, unknown> =
  core.isSTM
...
export const acquireUseRelease: {
  <A, A2, E2, R2, A3, E3, R3>(
    ...
    acquire: STM<A, E, R>
  ) => Effect.Effect<A2, E2 | E3 | E, R2 | R3 | R>
...
export declare namespace All {
  ...
  type ReturnIterable<T extends Iterable<STMAny>, Discard extends boolean> = [
    T
  ] extends [Iterable<STM.Variance<infer A, infer E, infer R>>]
    ? STM<Discard extends true ? void : Array<A>, E, R>
    : never

  type ReturnObject<
    T extends Record<string, STMAny>,
    Discard extends boolean
  > = STM<
    Discard extends true
      ? void
      : {
          -readonly [K in keyof T]: [T[K]] extends [
            STM.Variance<infer A, infer _E, infer _R>
          ]
            ? A
            : never
        },
    keyof T extends never
      ? never
      : [T[keyof T]] extends [{ [STMTypeId]: { _E: (_: never) => infer E } }]
        ? E
        : never,
    keyof T extends never
      ? never
      : [T[keyof T]] extends [{ [STMTypeId]: { _R: (_: never) => infer R } }]
        ? R
        : never
  >
  ...
  type IsDiscard<A> = [Extract<A, { readonly discard: true }>] extends [never]
    ? false
    : true
...
export const asSome: <A, E, R>(
  self: STM<A, E, R>
) => STM<Option.Option<A>, E, R> = stm.asSome
...
export const asSomeError: <A, E, R>(
  self: STM<A, E, R>
) => STM<A, Option.Option<E>, R> = stm.asSomeError
 ...
 * @category mapping
 */
export const asVoid: <A, E, R>(self: STM<A, E, R>) => STM<void, E, R> =
  stm.asVoid
...
export const catchAll: {
  <E, B, E1, R1>(
    f: (e: E) => STM<B, E1, R1>
  ): <A, R>(self: STM<A, E, R>) => STM<B | A, E1, R1 | R>
  <A, E, R, B, E1, R1>(
    self: STM<A, E, R>,
    f: (e: E) => STM<B, E1, R1>
  ): STM<A | B, E1, R | R1>
...
export const catchTag: {
  <K extends E["_tag"] & string, E extends { _tag: string }, A1, E1, R1>(
    ...
    self: STM<A, E, R>
  ) => STM<A1 | A, E1 | Exclude<E, { _tag: K }>, R1 | R>
...
export const collectSTM: {
  <A, A2, E2, R2>(
    pf: (a: A) => Option.Option<STM<A2, E2, R2>>
...
export const commitEither: <A, E, R>(
  self: STM<A, E, R>
) => Effect.Effect<A, E, R> = stm.commitEither
 ...
 * @category constructors
 */
export const contextWith: <R0, R>(
  f: (environment: Context.Context<R0>) => R
) => STM<R, never, R0> = core.contextWith
...
export const either: <A, E, R>(
  self: STM<A, E, R>
) => STM<Either.Either<A, E>, never, R> = stm.either
...
export const ensuring: {
  <R1, B>(
    finalizer: STM<B, never, R1>
  ): <A, E, R>(self: STM<A, E, R>) => STM<A, E, R1 | R>
  <A, E, R, R1, B>(
    self: STM<A, E, R>,
    finalizer: STM<B, never, R1>
  ): STM<A, E, R | R1>
 ...
 * @category mutations
 */
export const eventually: <A, E, R>(self: STM<A, E, R>) => STM<A, E, R> =
  stm.eventually
...
export const flatten: <A, E2, R2, E, R>(
  self: STM<STM<A, E2, R2>, E, R>
) => STM<A, E2 | E, R2 | R> = stm.flatten
 ...
 * @category constructors
 */
export const fromEither: <A, E>(either: Either.Either<A, E>) => STM<A, E> =
  stm.fromEither
...
) => STM<A, Option.Option<never>> = stm.fromOption
...
export const head: <A, E, R>(
  self: STM<Iterable<A>, E, R>
) => STM<A, Option.Option<E>, R> = stm.head
 ...
 * @category mutations
 */
export const ignore: <A, E, R>(self: STM<A, E, R>) => STM<void, never, R> =
  stm.ignore
...
export const isFailure: <A, E, R>(
  self: STM<A, E, R>
) => STM<boolean, never, R> = stm.isFailure
...
export const isSuccess: <A, E, R>(
  self: STM<A, E, R>
) => STM<boolean, never, R> = stm.isSuccess
 ...
 * @category mutations
 */
export const merge: <A, E, R>(self: STM<A, E, R>) => STM<E | A, never, R> =
  stm.merge
 ...
 * @category mutations
 */
export const negate: <E, R>(self: STM<boolean, E, R>) => STM<boolean, E, R> =
  stm.negate
...
export const none: <A, E, R>(
  self: STM<Option.Option<A>, E, R>
) => STM<void, Option.Option<E>, R> = stm.none
...
export const option: <A, E, R>(
  self: STM<A, E, R>
) => STM<Option.Option<A>, never, R> = stm.option
 ...
 * @category error handling
 */
export const orDie: <A, E, R>(self: STM<A, E, R>) => STM<A, never, R> =
  stm.orDie
...
export const orElseEither: {
  <A2, E2, R2>(
    that: LazyArg<STM<A2, E2, R2>>
  ): <A, E, R>(self: STM<A, E, R>) => STM<Either.Either<A2, A>, E2, R2 | R>
  <A, E, R, A2, E2, R2>(
    self: STM<A, E, R>,
    that: LazyArg<STM<A2, E2, R2>>
  ): STM<Either.Either<A2, A>, E2, R | R2>
} = stm.orElseEither
...
export const orElseSucceed: {
  <A2>(
    value: LazyArg<A2>
  ): <A, E, R>(self: STM<A, E, R>) => STM<A2 | A, never, R>
...
export const provideSomeContext: {
  ...
  ): <R1, E, A>(self: STM<A, E, R1>) => STM<A, E, Exclude<R1, R>>
  <R, R1, E, A>(
    self: STM<A, E, R1>,
    context: Context.Context<R>
  ): STM<A, E, Exclude<R1, R>>
...
export const rejectSTM: {
  <A, E2, R2>(
    pf: (a: A) => Option.Option<STM<E2, E2, R2>>
...
export const some: <A, E, R>(
  self: STM<Option.Option<A>, E, R>
) => STM<A, Option.Option<E>, R> = stm.some
 ...
 * @category constructors
 */
export const succeedSome: <A>(value: A) => STM<Option.Option<A>> =
  stm.succeedSome
...
export const suspend: <A, E, R>(
  evaluate: LazyArg<STM<A, E, R>>
) => STM<A, E, R> = stm.suspend
...
export const unless: {
  (
    predicate: LazyArg<boolean>
  ): <A, E, R>(self: STM<A, E, R>) => STM<Option.Option<A>, E, R>
  <A, E, R>(
    self: STM<A, E, R>,
    predicate: LazyArg<boolean>
  ): STM<Option.Option<A>, E, R>
} = stm.unless
...
export const unlessSTM: {
  ...
  ): <A, E, R>(self: STM<A, E, R>) => STM<Option.Option<A>, E2 | E, R2 | R>
  <A, E, R, E2, R2>(
    self: STM<A, E, R>,
    predicate: STM<boolean, E2, R2>
  ): STM<Option.Option<A>, E | E2, R | R2>
} = stm.unlessSTM
...
export const unsome: <A, E, R>(
  self: STM<A, Option.Option<E>, R>
) => STM<Option.Option<A>, E, R> = stm.unsome
...
export const validateFirst: {
  <A, B, E, R>(
    f: (a: A) => STM<B, E, R>
  ): (elements: Iterable<A>) => STM<B, Array<E>, R>
  <A, B, E, R>(
    elements: Iterable<A>,
    f: (a: A) => STM<B, E, R>
  ): STM<B, Array<E>, R>
...
export const zipLeft: {
  <A1, E1, R1>(
    that: STM<A1, E1, R1>
  ): <A, E, R>(self: STM<A, E, R>) => STM<A, E1 | E, R1 | R>
  <A, E, R, A1, E1, R1>(
    self: STM<A, E, R>,
    that: STM<A1, E1, R1>
  ): STM<A, E | E1, R | R1>
...
export const zipRight: {
  <A1, E1, R1>(
    that: STM<A1, E1, R1>
  ): <A, E, R>(self: STM<A, E, R>) => STM<A1, E1 | E, R1 | R>
  <A, E, R, A1, E1, R1>(
    self: STM<A, E, R>,
    that: STM<A1, E1, R1>
  ): STM<A1, E | E1, R | R1>
 ...
 * @category elements
 */
export const firstSuccessOf = <A, E, R>(
...
export const bind: {
  <N extends string, K, A, E2, R2>(
    ...
    self: STM<K, E, R>
  ) => STM<MergeRecord<K, { [k in N]: A }>, E2 | E, R2 | R>
...
export const bindTo: {
  <N extends string>(
    tag: N
  ): <A, E, R>(self: STM<A, E, R>) => STM<Record<N, A>, E, R>
