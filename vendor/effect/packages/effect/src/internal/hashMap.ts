...
/** @internal */
export const HashMapTypeId: HM.TypeId = Symbol.for(
  HashMapSymbolKey
) as HM.TypeId
...
export const setTree = Dual.dual<
  <K, V>(
    ...
    newSize: number
  ) => (self: HM.HashMap<K, V>) => HM.HashMap<K, V>,
  <K, V>(
    self: HM.HashMap<K, V>,
    newRoot: Node.Node<K, V>,
    newSize: number
...
>(
  3,
  <K, V>(self: HM.HashMap<K, V>, newRoot: Node.Node<K, V>, newSize: number) => {
...
/** @internal */
export const size = <K, V>(self: HM.HashMap<K, V>): number =>
...
export const map = Dual.dual<
  <A, V, K>(
    f: (value: V, key: K) => A
  ) => (self: HM.HashMap<K, V>) => HM.HashMap<K, A>,
  <K, V, A>(
    self: HM.HashMap<K, V>,
    f: (value: V, key: K) => A
  ) => HM.HashMap<K, A>
>(2, (self, f) =>
  reduce(self, empty(), (map, value, key) => set(map, key, f(value, key)))
)
...
>(2, (self, f) =>
  reduce(self, empty(), (zero, value, key) =>
    mutate(zero, (map) =>
      forEach(f(value, key), (value, key) => set(map, key, value))
    )
  )
)
...
>(
  ...
  <Z, V, K>(
    ...
    if (root._tag === "LeafNode") {
      return Option.isSome(root.value)
...
/** @internal */
export const compact = <K, A>(self: HM.HashMap<K, Option.Option<A>>) =>
...
export const findFirst: {
  <K, A, B extends A>(
    predicate: (a: NoInfer<A>, k: K) => a is B
  ): (self: HM.HashMap<K, A>) => Option.Option<[K, B]>
  <K, A>(
    predicate: (a: NoInfer<A>, k: K) => boolean
  ): (self: HM.HashMap<K, A>) => Option.Option<[K, A]>
  <K, A, B extends A>(
    self: HM.HashMap<K, A>,
    predicate: (a: A, k: K) => a is B
  ): Option.Option<[K, B]>
  <K, A>(
    self: HM.HashMap<K, A>,
