...
export class StreamImpl<
  ...
  out E = never,
  out R = never
> implements Stream.Stream<A, E, R> {
...
): Stream.Stream<Chunk.Chunk<A>, E, R> => chunks(accumulateChunks(self))

/** @internal */
export const accumulateChunks = <A, E, R>(
  self: Stream.Stream<A, E, R>
): Stream.Stream<A, E, R> => {
  const accumulator = (
    s: Chunk.Chunk<A>
  ...
  return new StreamImpl(
    core.pipeTo(toChannel(self), accumulator(Chunk.empty()))
  )
}
...
export const acquireRelease = <A, E, R, R2, X>(
  ...
  scoped(Effect.acquireRelease(acquire, release))
...
>(
  ...
  <A, E, R, B, A2, E2, R2, C, R3>(
    ...
    filterMap(aggregateWithinEither(self, sink, schedule), (_) =>
      Either.match(_, {
        onLeft: Option.none,
        onRight: Option.some
      })
    ...
    return fromEffect(layer).pipe(
      flatMap(
        ([
          ...
          const handoffProducer: Channel.Channel<
            never,
            Chunk.Chunk<A>,
            never,
            E | E2,
          ...
          const handoffConsumer: Channel.Channel<
            Chunk.Chunk<A | A2>,
            unknown,
            E | E2,
            unknown,
                  ...
                  switch (signal._tag) {
                    case HandoffSignal.OP_EMIT: {
                      return pipe(
                        ...
                        channel.zipRight(
                          core.fromEffect(Ref.get(endAfterEmit))
                        ),
                        core.flatMap((bool) =>
                          bool ? core.void : handoffConsumer
                        )
                      )
                    }
                    ...
                    case HandoffSignal.OP_END: {
                      if (
                        signal.reason._tag === SinkEndReason.OP_SCHEDULE_END
                      ) {
                        return pipe(
                          Ref.get(consumed),
                          Effect.map((bool) =>
                      ...
                      return pipe(
                        Ref.set<SinkEndReason.SinkEndReason>(
                          sinkEndReason,
                          signal.reason
                        ),
                        Effect.zipRight(Ref.set(endAfterEmit, true)),
          ...
          ): Effect.Effect<C, Option.Option<never>, R2 | R3> =>
            scheduleDriver.next(lastB)
          const scheduledAggregator = (
            sinkFiber: Fiber.RuntimeFiber<
              readonly [Chunk.Chunk<Chunk.Chunk<A | A2>>, B],
              E | E2
            >,
            scheduleFiber: Fiber.RuntimeFiber<C, Option.Option<never>>,
            scope: Scope.Scope
          ): Channel.Channel<
            Chunk.Chunk<Either.Either<B, C>>,
            unknown,
            E | E2,
            unknown,
            unknown,
                    ...
                    switch (reason._tag) {
                      case SinkEndReason.OP_SCHEDULE_END: {
                        return pipe(
                          ...
                          Effect.map(
                            ([wasConsumed, sinkFiber, scheduleFiber]) => {
                              const toWrite = pipe(
                                c,
                                Option.match({
                                  onNone: (): Chunk.Chunk<
                                    Either.Either<B, C>
                                  > => Chunk.of(Either.right(b)),
                                  onSome: (
                                    c
                                  ): Chunk.Chunk<Either.Either<B, C>> =>
                      ...
                      case SinkEndReason.OP_UPSTREAM_END: {
                        return pipe(
                          ...
                          Effect.map((wasConsumed) =>
                            wasConsumed
                              ? core.write(
                                  Chunk.of<Either.Either<B, C>>(Either.right(b))
                                )
                              : core.void
                          ),
                          channel.unwrap
                    ...
                    pipe(
                      ...
                      Effect.zipRight(
                        pipe(
                          ...
                          Effect.map(([leftovers, b]) =>
                            handleSide(leftovers, b, Option.none())
                          )
                        )
                      )
                    ),
                          ...
                          Either.match(Cause.failureOrCause(cause), {
                            onLeft: () =>
                              pipe(
                                handoff,
                                Handoff.offer<
                                  HandoffSignal.HandoffSignal<A, E | E2>
                          ...
                          pipe(
                            ...
                            Effect.zipRight(
                              pipe(
                                Fiber.join(sinkFiber),
                                Effect.map(([leftovers, b]) =>
                                  handleSide(leftovers, b, Option.some(c))
                                )
                              )
                            )
                          )
          ...
          return unwrapScopedWith((scope) =>
            core.pipeTo(toChannel(self), handoffProducer).pipe(
              ...
              Effect.zipRight(
                channel
                  .pipeToOrFail(handoffConsumer, sink_.toChannel(sink))
                  .pipe(
                    core.collectElements,
                    channel.run,
                    Effect.forkIn(scope),
...
export const as = dual<
  <B>(
    value: B
  ) => <A, E, R>(self: Stream.Stream<A, E, R>) => Stream.Stream<B, E, R>,
...
>(
  ...
  <A, E, R, B>(
    self: Stream.Stream<A, E, R>,
    value: B
  ): Stream.Stream<B, E, R> => map(self, () => value)
)
...
export const _async = <A, E = never, R = never>(
  ...
  Effect.acquireRelease(queueFromBufferOptions<A, E>(bufferSize), (queue) =>
      ...
      Effect.runtime<R>().pipe(
        Effect.flatMap((runtime) =>
          Effect.sync(() => {
            ...
            const canceler = register(
              emit.make<R, E, A, void>((resume) =>
                InternalTake.fromPull(resume)
                  .pipe(
        ...
        Effect.map((value) => {
          ...
          > = Queue.take(output).pipe(
            ...
            Effect.match({
              ...
              onSuccess: (chunk) =>
                core.write(chunk).pipe(core.flatMap(() => loop))
            }),
            channel.unwrap
          )
...
const queueFromBufferOptionsPush = <A, E>(
  ...
  if (
    ...
    (options?.bufferSize === undefined && options?.strategy === undefined)
  ) {
    return Queue.unbounded()
  }
...
export const asyncPush = <A, E = never, R = never>(
  ...
  Effect.acquireRelease(
    ...
    Effect.tap((queue) =>
      FiberRef.getWith(FiberRef.currentScheduler, (scheduler) =>
        register(emit.makePush(queue, scheduler))
      )
    ),
    Effect.map((queue) => {
...
>(
  ...
  <A, E, R, A2, E2, R2>(
    ...
    suspend(() => {
      ...
      > =>
        core.pipeTo(
          channel.zipRight(core.write(leftover), channel.identityChannel()),
          toChannel(f(prefix))
        )
      return new StreamImpl(
        pipe(toChannel(self), channel.pipeToOrFail(buffering(Chunk.empty<A>())))
      )
    })
)
...
export const broadcast = dual<
  <N extends number>(
    ...
    Types.TupleOf<N, Stream.Stream<A, E>>,
    never,
    Scope.Scope | R
  >,
  ...
  > =>
    pipe(
      ...
      Effect.map(
        (tuple) =>
          tuple.map((queue) =>
            flattenTake(fromQueue(queue, { shutdown: true }))
          ) as Types.TupleOf<N, Stream.Stream<A, E>>
      )
    )
)
...
>(
  ...
  <A, E, R>(
    ...
    Effect.map(toPubSub(self, maximumLag), (pubsub) =>
      flattenTake(fromPubSub(pubsub))
    )
)
...
>(
  ...
  > =>
    Effect.flatMap(pubsubFromOptions(maximumLag), (pubsub) =>
      pipe(
        Effect.all(
          Array.from({ length: n }, () => PubSub.subscribe(pubsub))
        ) as Effect.Effect<
          Types.TupleOf<N, Queue.Dequeue<Take.Take<A, E>>>,
          never,
          R
...
export const broadcastedQueuesDynamic = dual<
  (
    ...
    Effect.Effect<Queue.Dequeue<Take.Take<A, E>>, never, Scope.Scope>,
    never,
    Scope.Scope | R
  >,
  <A, E, R>(
          ...
          > = pipe(
            ...
            core.flatMap(
              Exit.match({
                ...
                onSuccess: (value) =>
                  core.flatMap(core.write(Chunk.of(value)), () => process)
              })
            )
          )
          return process
...
>(
  ...
  <A, E, R>(
    ...
    const queue = Effect.acquireRelease(
      Queue.dropping<readonly [Take.Take<A, E>, Deferred.Deferred<void>]>(
        capacity
      ),
...
>(
  ...
  <A, E, R>(
    ...
    const queue = Effect.acquireRelease(
      Queue.sliding<readonly [Take.Take<A, E>, Deferred.Deferred<void>]>(
        capacity
      ),
      (queue) => Queue.shutdown(queue)
    )
...
>(
  ...
  <A, E, R>(
    ...
    return new StreamImpl(
      bufferSignal(queue, toChannel(pipe(self, rechunk(1))))
    )
  }
)

const bufferUnbounded = <A, E, R>(
  self: Stream.Stream<A, E, R>
): Stream.Stream<A, E, R> => {
        ...
        > = pipe(
          ...
          core.flatMap(
            InternalTake.match({
              ...
              onFailure: core.failCause,
              onSuccess: (value) =>
                core.flatMap(core.write(value), () => process)
            })
          )
        )
        return process
...
const bufferSignal = <A, E, R>(
  scoped: Effect.Effect<
    Queue.Queue<readonly [Take.Take<A, E>, Deferred.Deferred<void>]>,
    never,
    Scope.Scope
  >,
  bufferChannel: Channel.Channel<
    Chunk.Chunk<A>,
    unknown,
    E,
    unknown,
      ...
      onInput: (input: Chunk.Chunk<A>) =>
        pipe(
          ...
          Effect.flatMap((deferred) =>
            pipe(
              Queue.offer(queue, [
                InternalTake.chunk(input),
                deferred
              ] as const),
              Effect.flatMap((added) =>
                pipe(
                  Ref.set(ref, deferred),
                  Effect.when(() => added)
                )
              )
            )
...
>(
  ...
  <A, E, R, A2, E2, R2>(
    ...
    new StreamImpl<A | A2, E2, R | R2>(
      pipe(
        toChannel(self),
        core.catchAllCause((cause) => toChannel(f(cause)))
      )
    )
)
  ...
  <A, E, R, A2, E2, R2>(
    ...
    pipe(
      ...
      catchAll((error) =>
        pipe(
          pf(error),
          Option.getOrElse(() => fail<E | E2>(error))
        )
      )
    )
)
  ...
  <A, E, R, A2, E2, R2>(
    ...
    pipe(
      ...
      catchAllCause((cause) =>
        pipe(
          pf(cause),
          Option.getOrElse(() => failCause<E | E2>(cause))
        )
      )
    )
)
...
export const catchTags: {
  ...
  >(
    ...
    | {
        [K in keyof Cases]: Cases[K] extends (
          ...args: Array<any>
        ) => Stream.Stream.Variance<infer A, infer _E, infer _R>
          ? A
          : never
      }[keyof Cases],
    ...
    | {
        [K in keyof Cases]: Cases[K] extends (
          ...args: Array<any>
        ) => Stream.Stream.Variance<infer _A, infer E, infer _R>
          ? E
          : never
      }[keyof Cases],
    | R
    | {
        [K in keyof Cases]: Cases[K] extends (
          ...args: Array<any>
        ) => Stream.Stream.Variance<infer _A, infer _E, infer R>
          ? R
          : never
      }[keyof Cases]
  >
  ...
  >(
    ...
    | {
        [K in keyof Cases]: Cases[K] extends (
          ...args: Array<any>
        ) => Stream.Stream.Variance<infer _R, infer _E, infer A>
          ? A
          : never
      }[keyof Cases],
    | Exclude<E, { _tag: keyof Cases }>
    | {
        [K in keyof Cases]: Cases[K] extends (
          ...args: Array<any>
        ) => Stream.Stream.Variance<infer _R, infer E, infer _A>
          ? E
          : never
      }[keyof Cases],
    | R
    | {
        [K in keyof Cases]: Cases[K] extends (
          ...args: Array<any>
        ) => Stream.Stream.Variance<infer R, infer _E, infer _A>
          ? R
          : never
      }[keyof Cases]
  >
...
/** @internal */
export const changes = <A, E, R>(
  self: Stream.Stream<A, E, R>
): Stream.Stream<A, E, R> =>
  pipe(
    self,
    changesWith((x, y) => Equal.equals(y)(x))
  )
...
>(
  ...
  <A, E, R>(
    ...
    return new StreamImpl(
      pipe(toChannel(self), core.pipeTo(writer(Option.none())))
    )
  }
)
...
export const chunks = <A, E, R>(
  self: Stream.Stream<A, E, R>
): Stream.Stream<Chunk.Chunk<A>, E, R> => pipe(self, mapChunks(Chunk.of))

/** @internal */
export const chunksWith = dual<
  <A, E, R, A2, E2, R2>(
    f: (
      stream: Stream.Stream<Chunk.Chunk<A>, E, R>
    ) => Stream.Stream<Chunk.Chunk<A2>, E2, R2>
...
const unsome = <A, E, R>(
  ...
  Effect.catchAll(Effect.asSome(effect), (o) =>
    o._tag === "None" ? Effect.succeedNone : Effect.fail(o.value)
  )
...
export const combine = dual<
  <A2, E2, R2, S, R3, E, A, R4, R5, A3>(
    ...
    f: (
      ...
      Exit.Exit<readonly [A3, S], Option.Option<E2 | E>>,
      never,
      R5
    >
  ) => <R>(
        ...
        Effect.all([
          ...
          Effect.map(([left, right, latchL, latchR]) => {
            ...
            const pullRight = Handoff.offer<void>(latchR, void 0).pipe(
              Effect.zipRight(
                Handoff.take(right).pipe(Effect.flatMap(identity))
              )
            )
            return toChannel(
              unfoldEffect(s, (s) =>
                Effect.flatMap(f(s, pullLeft, pullRight), unsome)
              )
            )
          })
        )
...
export const combineChunks = dual<
  <A2, E2, R2, S, R3, E, A, R4, R5, A3>(
    ...
    f: (
      ...
      Exit.Exit<readonly [Chunk.Chunk<A3>, S], Option.Option<E2 | E>>,
      never,
      R5
    >
  ) => <R>(
    ...
    > =>
      channel.zipRight(
        ...
        core.readWithCause({
          ...
          > =>
            core.fromEffect(
              Handoff.offer<Take.Take<Elem, Err>>(handoff, InternalTake.end)
            )
        })
      )
    return new StreamImpl(
      channel.unwrapScopedWith((scope) =>
        Effect.all([
          ...
          Effect.tap(([left, _, latchL]) =>
            core
              .pipeTo(toChannel(self), producer(left, latchL))
              .pipe(channelExecutor.runIn(scope), Effect.forkIn(scope))
          ),
          Effect.tap(([_, right, __, latchR]) =>
            core
              .pipeTo(toChannel(that), producer(right, latchR))
              .pipe(channelExecutor.runIn(scope), Effect.forkIn(scope))
          ),
          Effect.map(([left, right, latchL, latchR]) => {
            const pullLeft = Handoff.offer<void>(latchL, void 0).pipe(
              Effect.zipRight(
                Handoff.take(left).pipe(Effect.flatMap(InternalTake.done))
              )
            )
            const pullRight = Handoff.offer<void>(latchR, void 0).pipe(
              Effect.zipRight(
                Handoff.take(right).pipe(Effect.flatMap(InternalTake.done))
              )
            )
            return toChannel(
              unfoldChunkEffect(s, (s) =>
                Effect.flatMap(f(s, pullLeft, pullRight), unsome)
              )
            )
          })
...
>(
  ...
  <A, E, R, A2, E2, R2>(
    ...
    new StreamImpl<A2 | A, E2 | E, R2 | R>(
      pipe(toChannel(self), channel.zipRight(toChannel(that)))
    )
)

/** @internal */
export const concatAll = <A, E, R>(
  streams: Chunk.Chunk<Stream.Stream<A, E, R>>
): Stream.Stream<A, E, R> =>
  suspend(() =>
    pipe(
      streams,
      Chunk.reduce(empty as Stream.Stream<A, E, R>, (x, y) => concat(y)(x))
    )
  )
    ...
    unwrapScopedWith((scope) =>
      Effect.gen(function* () {
          ...
          onInput: (input: Chunk.Chunk<A>) =>
            Option.match(Chunk.last(input), {
              ...
              onSome: (elem) =>
                core
                  .fromEffect(
                    Handoff.offer(handoff, HandoffSignal.emit(Chunk.of(elem)))
                  )
                  .pipe(core.flatMap(() => producer))
            }),
          onFailure: (cause) =>
            core.fromEffect(
                                  ...
                                  Effect.succeed(
                                    core
                                      ...
                                      .pipe(
                                        core.flatMap(() =>
                                          consumer(
                                            DebounceState.current(handoffFiber)
                                          )
                                        )
                                      )
                                  )
                                ...
                                case HandoffSignal.OP_END: {
                                  return Fiber.join(previous).pipe(
                                    Effect.map((chunk) =>
                                      core
                                        .write(chunk)
                                        .pipe(channel.zipRight(core.void))
                                    )
                                  )
                                }
                              }
        ...
        return scopedWith((scope) =>
          core
            .pipeTo(toChannel(self), producer)
            .pipe(channelExecutor.runIn(scope), Effect.forkIn(scope))
...
/** @internal */
export const die = (defect: unknown): Stream.Stream<never> =>
  fromEffect(Effect.die(defect))

/** @internal */
export const dieSync = (evaluate: LazyArg<unknown>): Stream.Stream<never> =>
  fromEffect(Effect.dieSync(evaluate))

/** @internal */
export const dieMessage = (message: string): Stream.Stream<never> =>
      ...
      Effect.flatMap((deferred) =>
        pipe(
          ...
          Effect.flatMap((next) =>
            pipe(
              Effect.all(
                Chunk.map(Chunk.range(0, options.size - 1), (id) =>
                  Effect.map(
                    next,
                    ([key, queue]) => [[key, id], queue] as const
                  )
                )
              ),
              ...
              Effect.flatMap((entries) => {
                ...
                return pipe(
                  Deferred.succeed(deferred, (a: A) =>
                    Effect.map(
                      options.decide(a),
                      (f) => (key: number) => f(mappings.get(key)!)
                    )
                  ),
                  Effect.as(
                    Array.from(queues) as Types.TupleOf<
                      N,
                      Queue.Dequeue<Exit.Exit<A, Option.Option<E>>>
                    >
                  )
                )
              })
...
>(
  ...
  > =>
    pipe(
      Effect.acquireRelease(
        Ref.make<Map<number, Queue.Queue<Exit.Exit<A, Option.Option<E>>>>>(
          new Map()
        ),
        (ref, _) =>
          pipe(
            ...
            Effect.flatMap((queues) =>
              pipe(queues.values(), Effect.forEach(Queue.shutdown))
            )
          )
      ),
      Effect.flatMap((queuesRef) =>
        Effect.gen(function* () {
          ...
          const newQueue = yield* Ref.make<
            Effect.Effect<[number, Queue.Queue<Exit.Exit<A, Option.Option<E>>>]>
          >(
            pipe(
          ...
          yield* pipe(
            ...
            Effect.matchCauseEffect({
              onFailure: (cause) =>
                finalize(Exit.failCause(pipe(cause, Cause.map(Option.some)))),
...
/** @internal */
export const drain = <A, E, R>(
  self: Stream.Stream<A, E, R>
): Stream.Stream<never, E, R> => new StreamImpl(channel.drain(toChannel(self)))
...
>(
  ...
  <A, E, R, A2, E2, R2>(
    ...
    fromEffect(Deferred.make<never, E2>()).pipe(
        ...
        scopedWith((scope) =>
          toChannel(that).pipe(
            ...
            Effect.catchAllCause((cause) =>
              Deferred.failCause(backgroundDied, cause)
            ),
            Effect.forkIn(scope)
          )
...
export const either = <A, E, R>(
  self: Stream.Stream<A, E, R>
): Stream.Stream<Either.Either<A, E>, never, R> =>
  pipe(
    self,
    map(Either.right),
    catchAll((error) => make(Either.left(error)))
  )
...
>(
  2,
  (self, finalizer) =>
    new StreamImpl(core.ensuringWith(toChannel(self), finalizer))
)

/** @internal */
export const context = <R>(): Stream.Stream<Context.Context<R>, never, R> =>
  fromEffect(Effect.context<R>())

/** @internal */
export const contextWith = <R, A>(
  f: (env: Context.Context<R>) => A
): Stream.Stream<A, never, R> => pipe(context<R>(), map(f))
...
/** @internal */
export const execute = <X, E, R>(
  effect: Effect.Effect<X, E, R>
): Stream.Stream<never, E, R> => drain(fromEffect(effect))

/** @internal */
export const fail = <E>(error: E): Stream.Stream<never, E> =>
  fromEffectOption(Effect.fail(Option.some(error)))
...
/** @internal */
export const failCause = <E>(cause: Cause.Cause<E>): Stream.Stream<never, E> =>
  fromEffect(Effect.failCause(cause))

/** @internal */
export const failCauseSync = <E>(
  evaluate: LazyArg<Cause.Cause<E>>
): Stream.Stream<never, E> => fromEffect(Effect.failCauseSync(evaluate))
...
} = dual(2, <A, E, R>(self: Stream.Stream<A, E, R>, predicate: Predicate<A>) =>
  mapChunks(self, Chunk.filter(predicate))
)
...
>(
  ...
  <A, E, R, B>(
    self: Stream.Stream<A, E, R>,
    pf: (a: A) => Option.Option<B>
  ): Stream.Stream<B, E, R> => mapChunks(self, Chunk.filterMap(pf))
)
        ...
        if (next.done) {
          ...
          return pipe(
            ...
            Option.match({
              ...
              onSome: Effect.map((a2) =>
                core.flatMap(core.write(Chunk.of(a2)), () => loop(iterator))
              )
            }),
            channel.unwrap
          )
  ...
  <A, E, R, A2, E2, R2>(
    ...
    suspend(() => {
      ...
      return new StreamImpl(
        pipe(
          toChannel(self),
          channel.pipeToOrFail(loop(Chunk.empty<A>()[Symbol.iterator]()))
        )
      )
    })
)
...
/** @internal */
export const finalizer = <R, X>(
  finalizer: Effect.Effect<X, never, R>
): Stream.Stream<void, never, R> => acquireRelease(Effect.void, () => finalizer)
...
>(
  ...
  <A, E, R, A2, E2, R2>(
    ...
    return matchConcurrency(
      ...
      () =>
        new StreamImpl(
          channel.concatMap(toChannel(self), (as) =>
            pipe(
              as,
              Chunk.map((a) => toChannel(f(a))),
              Chunk.reduce(
                core.void as Channel.Channel<
                  Chunk.Chunk<A2>,
                  unknown,
                  E2,
                  unknown,
...
>(
  (args) => isStream(args[0]),
  (self, options) => flatMap(self, identity, options)
)

/** @internal */
export const flattenChunks = <A, E, R>(
  self: Stream.Stream<Chunk.Chunk<A>, E, R>
): Stream.Stream<A, E, R> => {
  const flatten: Channel.Channel<
    Chunk.Chunk<A>,
  ...
  > = core.readWithCause({
    onInput: (chunks: Chunk.Chunk<Chunk.Chunk<A>>) =>
      core.flatMap(channel.writeChunk(chunks), () => flatten),
    onFailure: core.failCause,
    onDone: () => core.void
  })
...
export const flattenExitOption = <A, E2, E, R>(
  ...
  const processChunk = (
    ...
    const [toEmit, rest] = pipe(
      chunk,
      Chunk.splitWhere((exit) => !Exit.isSuccess(exit))
    )
    const next = pipe(
      Chunk.head(rest),
      Option.match({
  ...
  const process: Channel.Channel<
    ...
    R
  > = core.readWithCause({
    onInput: (chunk: Chunk.Chunk<Exit.Exit<A, Option.Option<E2>>>) =>
...
/** @internal */
export const flattenIterables = <A, E, R>(
  self: Stream.Stream<Iterable<A>, E, R>
): Stream.Stream<A, E, R> => pipe(self, map(Chunk.fromIterable), flattenChunks)

/** @internal */
export const flattenTake = <A, E2, E, R>(
...
export const forever = <A, E, R>(
  self: Stream.Stream<A, E, R>
): Stream.Stream<A, E, R> => new StreamImpl(channel.repeated(toChannel(self)))
...
export const fromAsyncIterable = <A, E>(
  ...
  pipe(
    Effect.acquireRelease(
      ...
      (iterator) =>
        iterator.return
          ? Effect.promise(async () => iterator.return!())
          : Effect.void
    ),
    Effect.map((iterator) =>
      repeatEffectOption(
...
export const fromChunkPubSub: {
  ...
  if (options?.scoped) {
    ...
    return options.shutdown
      ? Effect.map(effect, ensuring(PubSub.shutdown(pubsub)))
      : effect
  }
...
/** @internal */
export const fromEffectOption = <A, E, R>(
  effect: Effect.Effect<A, Option.Option<E>, R>
): Stream.Stream<A, E, R> =>
  new StreamImpl(
    channel.unwrap(
...
export const fromPubSub: {
  ...
  if (options?.scoped) {
    const effect = Effect.map(PubSub.subscribe(pubsub), (queue) =>
      fromQueue(queue, { maxChunkSize, shutdown: true })
    )
  ...
  const stream = flatMap(scoped(PubSub.subscribe(pubsub)), (queue) =>
...
/** @internal */
export const fromTPubSub = <A>(
  pubsub: TPubSub.TPubSub<A>
): Stream.Stream<A> => {
  return unwrapScoped(
    Effect.map(TPubSub.subscribeScoped(pubsub), (queue) => fromTQueue(queue))
  )
}
...
export const fromPull = <R, R2, E, A>(
  effect: Effect.Effect<
    ...
    R | Scope.Scope
  >
): Stream.Stream<A, E, Exclude<R, Scope.Scope> | R2> =>
  pipe(effect, Effect.map(repeatEffectChunkOption), unwrapScoped)
...
export const fromReadableStream: {
  ...
  ...args:
    ...
    | [evaluate: LazyArg<ReadableStream<A>>, onError: (error: unknown) => E]
): Stream.Stream<A, E> => {
  const evaluate = args.length === 1 ? args[0].evaluate : args[0]
  const onError = args.length === 1 ? args[0].onError : args[1]
  const releaseLockOnEnd =
    args.length === 1 ? args[0].releaseLockOnEnd === true : false
  return unwrapScoped(
    Effect.map(
      Effect.acquireRelease(
...
>(
  ...
  <A, E, R, K>(
    ...
    const groupAdjacentByChunk = (
        ...
        case "Some": {
          ...
          while (loop && until < chunk.length) {
            ...
            if (!Equal.equals(key, updatedKey)) {
              ...
              const additionalChunk = Chunk.unsafeFromArray(
                Array.from(chunk).slice(from, until)
              )
      ...
      const nonEmptyChunk = Chunk.appendAll(
        previousChunk,
        Chunk.unsafeFromArray(Array.from(chunk).slice(from, until))
      )
      const output = Chunk.unsafeFromArray(builder)
      return [
        Option.some([key, nonEmptyChunk as Chunk.NonEmptyChunk<A>]),
        output
      ]
    }
      ...
      core.readWithCause({
        onInput: (input: Chunk.Chunk<A>) => {
          ...
          return Chunk.isEmpty(output)
            ...
            : core.flatMap(core.write(output), () =>
                groupAdjacent(updatedState)
              )
        },
        onFailure: (cause) =>
          Option.match(state, {
            ...
            onSome: (output) =>
              core.flatMap(core.write(Chunk.of(output)), () =>
    ...
    return new StreamImpl(
      channel.pipeToOrFail(toChannel(self), groupAdjacent(Option.none()))
    )
  }
)
        ...
        Effect.map(
          Option.match({
            onNone: () =>
              core.readWith({
                onInput: (input: Chunk.Chunk<A>) =>
                  core.flatMap(core.write(input), () => writer(fiber)),
                onFailure: core.fail,
                onDone: () => core.void
              }),
            onSome: Exit.match({
    ...
    return new StreamImpl(
      channel.unwrapScopedWith((scope) =>
        effect.pipe(
          Effect.forkIn(scope),
          Effect.map((fiber) =>
            toChannel(self).pipe(core.pipeTo(writer(fiber)))
          )
        )
      )
    )
...
>(
  ...
  <A, E, R>(
    self: Stream.Stream<A, E, R>,
    duration: Duration.DurationInput
  ): Stream.Stream<A, E, R> => pipe(self, haltWhen(Clock.sleep(duration)))
)

/** @internal */
export const haltWhenDeferred = dual<
  <X, E2>(
    deferred: Deferred.Deferred<X, E2>
...
/** @internal */
export const identityStream = <A, E = never, R = never>(): Stream.Stream<
  A,
  E,
  R
> =>
  new StreamImpl(
    channel.identityChannel() as Channel.Channel<
      Chunk.Chunk<A>,
      unknown,
      E,
      unknown,
                  ...
                  if (bool && !leftDone) {
                    return pipe(
                      ...
                      core.flatMap(
                        InternalTake.match({
                          onEnd: () =>
                            rightDone ? core.void : process(true, rightDone),
                          onFailure: core.failCause,
                          onSuccess: (chunk) =>
                            pipe(
                              core.write(chunk),
                              core.flatMap(() => process(leftDone, rightDone))
                            )
                        })
                      )
                    )
                  ...
                  if (!bool && !rightDone) {
                    return pipe(
                      ...
                      core.flatMap(
                        InternalTake.match({
                          onEnd: () =>
                            leftDone ? core.void : process(leftDone, true),
                          onFailure: core.failCause,
                          onSuccess: (chunk) =>
                            pipe(
                              core.write(chunk),
...
export const intersperse = dual<
  <A2>(
    element: A2
  ) => <A, E, R>(self: Stream.Stream<A, E, R>) => Stream.Stream<A2 | A, E, R>,
  <A, E, R, A2>(
    self: Stream.Stream<A, E, R>,
    element: A2
  ) => Stream.Stream<A2 | A, E, R>
>(
  2,
  <A, E, R, A2>(
    self: Stream.Stream<A, E, R>,
...
>(
  ...
  <A, E, R>(
    self: Stream.Stream<A, E, R>,
    duration: Duration.DurationInput
  ): Stream.Stream<A, E, R> => pipe(self, interruptWhen(Clock.sleep(duration)))
)
...
>(
  ...
  <A, E, R, X, E2, R2>(
    ...
    new StreamImpl(pipe(toChannel(self), channel.interruptWhen(effect)))
)
...
>(
  ...
  <A, E, R, X, E2>(
    ...
    new StreamImpl(
      pipe(toChannel(self), channel.interruptWhenDeferred(deferred))
    )
)
...
/** @internal */
export const make = <As extends Array<any>>(
  ...as: As
): Stream.Stream<As[number]> => fromIterable(as)
...
>(
  ...
  <A, E, R, S, A2>(
    ...
    const accumulator = (
      s: S
    ): Channel.Channel<Chunk.Chunk<A2>, Chunk.Chunk<A>, E, E, void, unknown> =>
      core.readWith({
        onInput: (input: Chunk.Chunk<A>) => {
...
>(
  ...
  <A, E, R, S, A2, E2, R2>(
    ...
    suspend(() => {
                ...
                return pipe(
                  ...
                  Effect.match({
                    onFailure: (error) => {
                      if (outputs.length !== 0) {
                        return channel.zipRight(
                          core.write(Chunk.unsafeFromArray(outputs)),
                          core.fail(error)
                        )
                      }
      ...
      return new StreamImpl(
        pipe(toChannel(self), channel.pipeToOrFail(accumulator(s)))
      )
    })
)
...
>(
  ...
  <A, E, R, E2, A2>(
    ...
    pipe(self, mapError(options.onFailure), map(options.onSuccess))
)
...
>(
  ...
  <A, E, R, B, E2, R2>(
    ...
    new StreamImpl(pipe(toChannel(self), channel.mapOutEffect(f)))
)
...
>(
  ...
  <A, E, R, A2>(
    self: Stream.Stream<A, E, R>,
    f: (a: A) => Chunk.Chunk<A2>
  ): Stream.Stream<A2, E, R> => pipe(self, mapChunks(Chunk.flatMap(f)))
)
...
>(
  ...
  <A, E, R, A2, E2, R2>(
    ...
    pipe(self, mapEffectSequential(f), mapConcatChunk(identity))
)
...
>(
  ...
  <A, E, R, A2, E2, R2>(
    ...
    pipe(
      self,
      mapEffectSequential((a) => pipe(f(a), Effect.map(Chunk.fromIterable))),
  ...
  <A, E, R, A2, E2, R2>(
    ...
    return new StreamImpl(
      pipe(
        toChannel(self),
        core.pipeTo(
          core.suspend(() => loop(Chunk.empty<A>()[Symbol.iterator]()))
        )
      )
    )
  }
...
export const mergeAll = dual<
  (options: {
    ...
    streams: Iterable<Stream.Stream<A, E, R>>
  ) => Stream.Stream<A, E, R>,
  <A, E, R>(
    streams: Iterable<Stream.Stream<A, E, R>>,
    options: {
...
>(
  ...
  (streams, options) => flatten(fromIterable(streams), options)
)
...
export const mergeWithTag: {
  <S extends { [k in string]: Stream.Stream<any, any, any> }>(
    ...
    {
      [K in keyof S]: { _tag: K; value: Stream.Stream.Success<S[K]> }
    }[keyof S],
  ...
  const values = keys.map((key) =>
    streams[key].pipe(map((value) => ({ _tag: key, value })))
  ) as any
...
>(
  ...
  <A, E, R, A2, E2, R2, A3, A4>(
    ...
    const strategy = options.haltStrategy
      ? haltStrategy.fromInput(options.haltStrategy)
      : HaltStrategy.Both
    const handler =
      (terminate: boolean) =>
      (
        exit: Exit.Exit<unknown, E | E2>
    ...
    return new StreamImpl<A3 | A4, E | E2, R | R2>(
      channel.mergeWith(toChannel(map(self, options.onSelf)), {
        ...
        onSelfDone: handler(
          strategy._tag === "Either" || strategy._tag === "Left"
        ),
        onOtherDone: handler(
          strategy._tag === "Either" || strategy._tag === "Right"
        )
      })
    )
  }
...
): Effect.Effect<string, E, R> => run(self, sink_.mkString)
...
>(
  ...
  <A, E, R, X, R2>(
    ...
    pipe(
      ...
      catchAllCause((cause) =>
        fromEffect(
          pipe(cleanup(cause), Effect.zipRight(Effect.failCause(cause)))
        )
      )
    )
)
    ...
    new StreamImpl<A, E, R | R2>(
      pipe(
        ...
        core.ensuringWith((exit) =>
          Exit.isSuccess(exit) ? cleanup() : Effect.void
        )
      )
    )
)
...
export const orDie = <A, E, R>(
  self: Stream.Stream<A, E, R>
): Stream.Stream<A, never, R> => pipe(self, orDieWith(identity))

/** @internal */
export const orDieWith = dual<
  <E>(
    f: (e: E) => unknown
  ) => <A, R>(self: Stream.Stream<A, E, R>) => Stream.Stream<A, never, R>,
  <A, E, R>(
    self: Stream.Stream<A, E, R>,
    f: (e: E) => unknown
  ) => Stream.Stream<A, never, R>
...
>(
  ...
  <A, E, R, A2, E2, R2>(
    ...
    pipe(
      self,
      map(Either.left),
      orElse(() => pipe(that(), map(Either.right)))
    )
)
...
export const orElseFail = dual<
  <E2>(
    error: LazyArg<E2>
  ) => <A, E, R>(self: Stream.Stream<A, E, R>) => Stream.Stream<A, E2, R>,
  <A, E, R, E2>(
    self: Stream.Stream<A, E, R>,
    error: LazyArg<E2>
  ) => Stream.Stream<A, E2, R>
...
>(
  ...
  <A, E, R, A2>(
    self: Stream.Stream<A, E, R>,
    element: LazyArg<A2>
  ): Stream.Stream<A | A2, E, R> =>
    pipe(
      self,
      orElseIfEmptyChunk(() => Chunk.of(element()))
    )
)
...
>(
  ...
  <A, E, R, A2>(
    ...
    pipe(
      self,
      orElseIfEmptyStream(() => new StreamImpl(core.write(chunk())))
    )
)
    ...
    > = core.readWith({
      onInput: (input: Chunk.Chunk<A>) => {
        ...
        return pipe(
          core.write(input),
          channel.zipRight(
            channel.identityChannel<Chunk.Chunk<A>, E, unknown>()
          )
        )
      },
      onFailure: core.fail,
...
export const paginateEffect = <S, A, E, R>(
  ...
  paginateChunkEffect(s, (s) =>
    pipe(
      f(s),
      Effect.map(([a, s]) => [Chunk.of(a), s] as const)
    )
  )
        ...
        pipe(
          ...
          Effect.map((handoff) => {
              ...
              onSuccess: ([z, leftovers]) => {
                ...
                > = core.readWithCause({
                  ...
                  onFailure: (cause) =>
                    channel.zipRight(
                      core.fromEffect(
                        Handoff.offer<Signal>(handoff, { _tag: OP_HALT, cause })
                      ),
                      core.failCause(cause)
                    ),
                  onDone: (_) =>
                    channel.zipRight(
                      core.fromEffect(
                        Handoff.offer<Signal>(handoff, { _tag: OP_END })
                      ),
                      core.void
                    )
                })
            ...
            return pipe(
              ...
              Effect.zipRight(Deferred.await(deferred)),
              Effect.map(
                (z) => [z, new StreamImpl(producer)] as [A2, StreamImpl<A, E>]
              )
            )
          })
        )
...
export const partition: {
  <C extends A, B extends A, A = C>(
    ...
    [
      excluded: Stream.Stream<Exclude<C, B>, E>,
      satisfying: Stream.Stream<B, E>
    ],
    E,
    Scope.Scope | R
  >
  <A>(
    ...
    [excluded: Stream.Stream<A, E>, satisfying: Stream.Stream<A, E>],
    E,
    Scope.Scope | R
  >
...
export const partitionEither = dual<
  <A, A3, A2, E2, R2>(
    predicate: (
      a: Types.NoInfer<A>
    ) => Effect.Effect<Either.Either<A3, A2>, E2, R2>,
    options?: {
      readonly bufferSize?: number | undefined
    }
  ) => <E, R>(
...
>(
  ...
  <A, E, R, A2, L, E2, R2>(
    ...
    new StreamImpl(
      pipe(toChannel(self), channel.pipeToOrFail(sink_.toChannel(sink)))
    )
)
...
>(
  ...
  <R, R2, E, E2, A, A2>(
    ...
    new StreamImpl(core.pipeTo(toChannel(self), channel))
)
...
>(
  ...
  <R, R2, E, E2, A, A2>(
    ...
    new StreamImpl(pipe(toChannel(self), channel.pipeToOrFail(chan)))
)

/** @internal */
export const prepend = dual<
  <B>(
    values: Chunk.Chunk<B>
...
>(
  ...
  (self, values) =>
    new StreamImpl(
      channel.zipRight(core.write(values as Chunk.Chunk<any>), toChannel(self))
    )
)
...
export const provideContext = dual<
  ...
  ) => <A, E>(self: Stream.Stream<A, E, R>) => Stream.Stream<A, E>,
  <A, E, R>(
    self: Stream.Stream<A, E, R>,
    context: Context.Context<R>
  ) => Stream.Stream<A, E>
...
export const provideSomeContext = dual<
  <R2>(
    ...
    self: Stream.Stream<A, E, R>
  ) => Stream.Stream<A, E, Exclude<R, R2>>,
  <A, E, R, R2>(
    self: Stream.Stream<A, E, R>,
    context: Context.Context<R2>
  ) => Stream.Stream<A, E, Exclude<R, R2>>
...
>(
  ...
  <A, E, R, I, S, E2, R2>(
    ...
    contextWithStream((env: Context.Context<R2 | Exclude<R, I>>) =>
      flatMap(stream, (service) =>
        pipe(
          self,
          provideContext(
            Context.add(env, tag, service) as Context.Context<R | R2>
          )
        )
      )
    )
  ...
  core.readWithCause({
    onInput: (chunk: Chunk.Chunk<A>) => {
      if (chunk.length === target && rechunker.isEmpty()) {
        return core.flatMap(core.write(chunk), () =>
      ...
      if (chunk.length > 0) {
        ...
        return core.flatMap(channel.writeAll(...chunks), () =>
          rechunkProcess(rechunker, target)
        )
      }
    ...
    onFailure: (cause) =>
      channel.zipRight(rechunker.emitIfNotEmpty(), core.failCause(cause)),
...
class StreamRechunker<out A, in out E> {
  ...
  emitIfNotEmpty(): Channel.Channel<
    Chunk.Chunk<A>,
    unknown,
    E,
    E,
...
>(
  ...
  <A, E, R, E2>(
    self: Stream.Stream<A, E, R>,
    pf: (error: E) => Option.Option<E2>
  ): Stream.Stream<A, E2, R> => pipe(self, refineOrDieWith(pf, identity))
)
...
>(
  ...
  <A, E, R, B, R2>(
    ...
    filterMap(repeatEither(self, schedule), (_) =>
      Either.match(_, {
        onLeft: Option.none,
        onRight: Option.some
      })
...
/** @internal */
export const repeatEffectChunk = <A, E, R>(
...
/** @internal */
export const repeatEffectOption = <A, E, R>(
  effect: Effect.Effect<A, Option.Option<E>, R>
): Stream.Stream<A, E, R> =>
...
>(
  ...
  <A, E, R, B, R2>(
    ...
    filterMap(
      repeatElementsWith(self, schedule, {
...
export const repeatValue = <A>(value: A): Stream.Stream<A> =>
  new StreamImpl(channel.repeated(core.write(Chunk.of(value))))
        ...
        > = channel.unwrap(
          Effect.match(driver.next(void 0), {
            ...
            onSuccess: (output) =>
              core.flatMap(process, () =>
                channel.zipRight(
                  core.write(Chunk.of(options.onSchedule(output))),
                  loop
                )
              )
          })
...
const repeatWithSchedule = <A, R, X>(
  ...
  repeatEffectWithSchedule(Effect.succeed(value), schedule)
      ...
      return concat(
        ...
        unfoldEffect(a, (s) =>
          Effect.matchEffect(driver.next(s as A0), {
            ...
            onSuccess: () =>
              Effect.map(provideLastIterationInfo(effect), (nextA) =>
                Option.some([nextA, nextA] as const)
              )
          })
        )
      )
...
export const withExecutionPlan: {
  ...
  <A, E extends Input, R, R2, Input, Provides, PolicyE>(
    ...
    suspend(() => {
      ...
      const loop: Stream.Stream<A, E | PolicyE, R2 | Exclude<R, Provides>> =
        suspend(() => {
          ...
          let nextStream: Stream.Stream<
            A,
            E | PolicyE,
            R2 | Exclude<R, Provides>
          > = Context.isContext(step.provide)
            ? provideSomeContext(self, step.provide)
            : provideSomeLayer(
                self,
                step.provide as Layer.Layer<Provides, E | PolicyE, R2>
              )
          ...
          if (Option.isSome(lastError)) {
            ...
            nextStream = scheduleDefectRefail(
              retry(
                nextStream,
                internalExecutionPlan.scheduleFromStep(step, false)!
              )
            )
          } else {
            const schedule = internalExecutionPlan.scheduleFromStep(step, true)
            nextStream = schedule
              ? scheduleDefectRefail(retry(nextStream, schedule))
              : nextStream
          }
...
const scheduleDefectRefail = <A, E, R>(self: Stream.Stream<A, E, R>) =>
  catchAllCause(self, (cause) =>
    failCause(InternalSchedule.scheduleDefectRefailCause(cause))
  )
...
export const runCount = <A, E, R>(
  self: Stream.Stream<A, E, R>
): Effect.Effect<number, E, R> => run(self, sink_.count)

/** @internal */
export const runDrain = <A, E, R>(
  self: Stream.Stream<A, E, R>
): Effect.Effect<void, E, R> => run(self, sink_.drain)
...
>(
  ...
  <A, E, R, S>(
    ...
    s: S,
    f: (s: S, a: A) => S
  ): Effect.Effect<S, E, R> => runFoldWhile(self, s, constTrue, f)
)
...
>(
  ...
  <A, E, R, S, E2, R2>(
    ...
    runFoldWhileEffect(self, s, constTrue, f)
)
...
>(
  ...
  <A, E, R, S, E2, R2>(
    ...
    pipe(self, runFoldWhileScopedEffect(s, constTrue, f))
)
...
>(
  ...
  <A, E, R, S>(
    ...
    pipe(self, runScoped(sink_.fold(s, cont, f)))
)
...
>(
  ...
  <A, E, R, S, E2, R2>(
    ...
    pipe(self, runScoped(sink_.foldEffect(s, cont, f)))
)
...
>(
  ...
  <A, E, R, X, E2, R2>(
    ...
    pipe(self, runScoped(sink_.forEachChunk(f)))
)
...
>(
  ...
  <A, E, R, X, E2, R2>(
    ...
    pipe(self, runScoped(sink_.forEach(f)))
)
...
>(
  ...
  <A, E, R, E2, R2>(
    ...
    pipe(self, runScoped(sink_.forEachWhile(f)))
)
...
>(
  ...
  <A, E, R>(
    ...
    pipe(self, runIntoQueueScoped(pubsub))
)
  ...
  <A, E, R>(
    ...
    > = core.readWithCause({
      onInput: (input: Chunk.Chunk<A>) =>
        core.flatMap(
          core.fromEffect(
            Queue.offerAll(queue, Chunk.map(input, Exit.succeed))
          ),
          () => writer
        ),
      onFailure: (cause) =>
        core.fromEffect(
          Queue.offer(queue, Exit.failCause(Cause.map(cause, Option.some)))
        ),
      onDone: () =>
...
>(
  ...
  <A, E, R>(
    ...
    > = core.readWithCause({
      onInput: (input: Chunk.Chunk<A>) =>
        core.flatMap(core.write(InternalTake.chunk(input)), () => writer),
...
): Effect.Effect<number, E, R> => run(self, sink_.sum)
...
export const scan = dual<
  <S, A>(
    s: S,
    f: (s: S, a: A) => S
  ) => <E, R>(self: Stream.Stream<A, E, R>) => Stream.Stream<S, E, R>,
  <A, E, R, S>(
    self: Stream.Stream<A, E, R>,
    s: S,
    f: (s: S, a: A) => S
...
>(
  ...
  <A, E, R, S>(
    ...
    pipe(
      self,
      scanEffect(s, (s, a) => Effect.succeed(f(s, a)))
    )
)
...
export const scanReduce = dual<
  <A2, A>(
    f: (a2: A2 | A, a: A) => A2
  ) => <E, R>(self: Stream.Stream<A, E, R>) => Stream.Stream<A2 | A, E, R>,
  <A, E, R, A2>(
    self: Stream.Stream<A, E, R>,
    f: (a2: A2 | A, a: A) => A2
  ) => Stream.Stream<A2 | A, E, R>
>(
  ...
  <A, E, R, A2>(
    ...
    pipe(
      self,
      scanReduceEffect((a2, a) => Effect.succeed(f(a2, a)))
    )
)
...
>(
  ...
  <A, E, R, A2, E2, R2>(
    ...
    pipe(
      self,
      mapAccumEffect<Option.Option<A | A2>, A, A | A2, E2, R2>(
        Option.none() as Option.Option<A | A2>,
        (option, a) => {
          switch (option._tag) {
            case "None": {
  ...
  <A, E, R, B, A0 extends A, R2, C>(
    ...
    > => {
      ...
      if (next.done) {
        return core.readWithCause({
          onInput: (chunk: Chunk.Chunk<A>) =>
            loop(driver, chunk[Symbol.iterator]()),
          onFailure: core.failCause,
          onDone: core.succeedNow
        })
      }
            ...
            pipe(
              ...
              Effect.map((b) =>
                pipe(
                  core.write(
                    Chunk.make(
                      options.onElement(next.value),
                      options.onSchedule(b)
                    )
                  ),
...
export const scoped = <A, E, R>(
  ...
  new StreamImpl(
    channel.ensuring(
      channel.scoped(pipe(effect, Effect.map(Chunk.of))),
      Effect.void
    )
  )
...
export const scopedWith = <A, E, R>(
  ...
  new StreamImpl(
    channel.scopedWith((scope) => f(scope).pipe(Effect.map(Chunk.of)))
  )

/** @internal */
export const some = <A, E, R>(
  self: Stream.Stream<Option.Option<A>, E, R>
): Stream.Stream<A, Option.Option<E>, R> =>
  pipe(
    self,
    mapError(Option.some),
    someOrFail(() => Option.none())
  )
...
>(
  ...
  <A, E, R, A2>(
    self: Stream.Stream<Option.Option<A>, E, R>,
    fallback: LazyArg<A2>
  ): Stream.Stream<A | A2, E, R> => pipe(self, map(Option.getOrElse(fallback)))
)
...
>(
  ...
  <A, E, R>(
    self: Stream.Stream<A, E, R>,
    chunkSize: number
  ): Stream.Stream<Chunk.Chunk<A>, E, R> => slidingSize(self, chunkSize, 1)
)
      ...
      core.suspend(() => {
        ...
        const emitOnStreamEnd = (
          ...
          if (queueSize < chunkSize) {
            ...
            const result = Chunk.isEmpty(items)
              ? Chunk.empty<Chunk.Chunk<A>>()
              : Chunk.of(items)
            return pipe(
              core.write(result),
              core.flatMap(() => channelEnd)
            )
          }
          ...
          const result = Chunk.isEmpty(lastItems)
            ? Chunk.empty<Chunk.Chunk<A>>()
            : Chunk.of(lastItems)
          return pipe(
            core.write(result),
        ...
        > =>
          core.readWithCause({
            ...
            onFailure: (cause) =>
              emitOnStreamEnd(queueSize, core.failCause(cause)),
...
export const split: {
  ...
  <A, E, R>(
    ...
    const split = (
      ...
      R
    > => {
      const [chunk, remaining] = pipe(
        leftovers,
        Chunk.appendAll(input),
        Chunk.splitWhere(predicate)
      )
      if (Chunk.isEmpty(chunk) || Chunk.isEmpty(remaining)) {
        return loop(
          pipe(chunk, Chunk.appendAll(pipe(remaining, Chunk.drop(1))))
        )
      }
      return pipe(
    ...
    return new StreamImpl(
      pipe(toChannel(self), core.pipeTo(loop(Chunk.empty())))
    )
  }
)
      ...
      core.readWithCause({
        onInput: (inputChunk: Chunk.Chunk<A>) => {
          ...
          const [carry, delimiterCursor] = pipe(
            ...
            Chunk.reduce(
              [
                pipe(
                  leftover,
                  Option.getOrElse(() => Chunk.empty<A>())
                ),
                delimiterIndex
              ] as const,
              ([carry, delimiterCursor], a) => {
                ...
                if (
                  ...
                  Equal.equals(
                    a,
                    pipe(delimiter, Chunk.unsafeGet(delimiterCursor))
                  )
                ) {
                  if (delimiterCursor + 1 === delimiter.length) {
                    ...
                    buffer.push(
                      pipe(
                        concatenated,
                        Chunk.take(concatenated.length - delimiter.length)
                      )
                    )
                ...
                return [
                  concatenated,
                  Equal.equals(a, pipe(delimiter, Chunk.unsafeGet(0))) ? 1 : 0
                ] as const
              }
            )
          )
          const output =
            buffer === undefined
              ? Chunk.empty<Chunk.Chunk<A>>()
              : Chunk.unsafeFromArray(buffer)
          return core.flatMap(core.write(output), () =>
            next(
        ...
        onDone: (done) =>
          Option.match(leftover, {
            ...
            onSome: (chunk) =>
              channel.zipRight(core.write(Chunk.of(chunk)), core.succeed(done))
          })
      })
    return new StreamImpl(
      pipe(toChannel(self), core.pipeTo(next(Option.none(), 0)))
    )
  }
)
...
export const splitLines = <E, R>(
  self: Stream.Stream<string, E, R>
): Stream.Stream<string, E, R> => pipeThroughChannel(self, channel.splitLines())

/** @internal */
export const succeed = <A>(value: A): Stream.Stream<A> =>
  fromChunk(Chunk.of(value))

/** @internal */
export const sync = <A>(evaluate: LazyArg<A>): Stream.Stream<A> =>
...
>(
  ...
  <A, E, R>(
    ...
    > =>
      core.readWith({
        onInput: (input: Chunk.Chunk<A>) => {
          const taken = pipe(
            input,
            Chunk.take(Math.min(n, Number.POSITIVE_INFINITY))
          )
    ...
    return new StreamImpl(
      pipe(toChannel(self), channel.pipeToOrFail(0 < n ? loop(n) : core.void))
    )
  }
)
      ...
      pipe(
        ...
        Effect.map((queue) => {
          ...
          > = core.readWith({
            ...
            onFailure: core.fail,
            onDone: () =>
              pipe(core.write(queue.toChunk()), channel.zipRight(core.void))
          })
...
export const tap: {
  ...
  <A, E, R, X, E2, R2>(
    ...
    mapEffectSequential(self, (a) => Effect.as(f(a), a))
)
...
export const tapBoth: {
  ...
  <A, E, R, X1, E2, R2, X2, E3, R3>(
    ...
    pipe(self, tapError(options.onFailure), tap(options.onSuccess))
)
...
export const tapError: {
  ...
  <A, E, R, X, E2, R2>(
    ...
    catchAll(self, (error) =>
      fromEffect(Effect.zipRight(f(error), Effect.fail(error)))
    )
)
...
export const tapErrorCause: {
  ...
  <A, E, R, X, E2, R2>(
    ...
    > = core.readWithCause({
      ...
      onFailure: (cause) =>
        core.fromEffect(Effect.zipRight(f(cause), Effect.failCause(cause))),
      onDone: core.succeedNow
    })
...
>(
  ...
  <A, E, R, E2, R2>(
    ...
    pipe(
      fromEffect(
        Effect.all([
          Queue.bounded<Take.Take<A, E | E2>>(1),
          Deferred.make<void>()
        ])
      ),
      flatMap(([queue, deferred]) => {
        ...
        > = core.readWithCause({
          ...
          onFailure: (cause: Cause.Cause<E | E2>) =>
            pipe(
              core.fromEffect(
                Queue.offer(queue, InternalTake.failCause(cause))
              ),
              core.foldCauseChannel({
      ...
      onInput: (input: Chunk.Chunk<A>) =>
        pipe(
          ...
          Effect.map(([weight, currentTimeMillis]) => {
            ...
            const max =
              units + burst < 0 ? Number.POSITIVE_INFINITY : units + burst
        ...
        pipe(
          ...
          Effect.map(([weight, currentTimeMillis]) => {
            ...
            const delay = Duration.millis(
              Math.max(0, waitCycles * Duration.toMillis(duration))
            )
            ...
            return core.flatMap(core.write(input), () =>
...
>(
  ...
  <A, E, R>(
    ...
    pipe(
      toPull(self),
      Effect.map(
        Effect.timeoutFail<Option.Option<E>>({
...
>(
  ...
  <A, E, R>(
    ...
    pipe(
      Effect.acquireRelease(pubsubFromOptions<A, E>(capacity), (pubsub) =>
        PubSub.shutdown(pubsub)
      ),
      Effect.tap((pubsub) =>
        pipe(self, runIntoPubSubScoped(pubsub), Effect.forkScoped)
      )
    )
)
...
export const toPull = <A, E, R>(
  ...
  Effect.Effect<Chunk.Chunk<A>, Option.Option<E>, R>,
  never,
  R | Scope.Scope
> =>
...
>(
  ...
  <A, E, R>(
    ...
    Effect.map(Effect.runtime<R>(), (runtime) =>
      toReadableStreamRuntime(self, runtime, options)
    )
)
  ...
  <A, E, R, A2, E2, R2>(
    ...
    const newChannel = core.suspend(() => {
      ...
      const concatAndGet = (
        ...
        const leftover = leftovers.ref
        const concatenated = Chunk.appendAll(
          leftover,
          Chunk.filter(chunk, (chunk) => chunk.length !== 0)
        )
      ...
      > = core.readWith({
        onInput: (input: Chunk.Chunk<A>) =>
          core.flatMap(core.write(input), () => upstreamMarker),
        onFailure: core.fail,
        onDone: (done) =>
          channel.zipRight(
            core.sync(() => {
      ...
      const transducer: Channel.Channel<
        Chunk.Chunk<A2>,
        Chunk.Chunk<A>,
        E | E2,
        never,
        ...
        core.flatMap(([leftover, z]) =>
          pipe(
            ...
            core.flatMap(([done, newLeftovers]) => {
              const nextChannel =
                done && Chunk.isEmpty(newLeftovers) ? core.void : transducer
              return pipe(
                core.write(Chunk.of(z)),
                core.flatMap(() => nextChannel)
              )
            })
          )
        )
  ...
  <A, E, XR, R extends XR>(
    ...
    return {
      [Symbol.asyncIterator]() {
        let currentResolve: ((value: IteratorResult<A>) => void) | undefined =
          undefined
        ...
        return {
          ...
          return() {
            ...
            return Effect.runPromise(
              Effect.as(Fiber.interrupt(fiber), { done: true, value: void 0 })
            )
          }
        }
      }
...
export const toAsyncIterable = <A, E>(
  self: Stream.Stream<A, E>
): AsyncIterable<A> => toAsyncIterableRuntime(self, Runtime.defaultRuntime)
...
export const toAsyncIterableEffect = <A, E, R>(
  ...
  Effect.map(Effect.runtime<R>(), (runtime) =>
    toAsyncIterableRuntime(self, runtime)
  )
      ...
      channel.unwrap(
        Effect.map(
          ...
          Option.match({
            onNone: () => core.void,
            onSome: ([chunk, s]) =>
              core.flatMap(core.write(chunk), () => loop(s))
          })
        )
      )
...
export const unfoldEffect = <S, A, E, R>(
  ...
  unfoldChunkEffect(s, (s) =>
    pipe(f(s), Effect.map(Option.map(([a, s]) => [Chunk.of(a), s])))
  )
...
export const unwrapScoped = <A, E2, R2, E, R>(
  effect: Effect.Effect<Stream.Stream<A, E2, R2>, E, R>
): Stream.Stream<A, E | E2, Exclude<R, Scope.Scope> | R2> =>
  flatten(scoped(effect))
...
>(
  ...
  <A, E, R>(
    self: Stream.Stream<A, E, R>,
    test: LazyArg<boolean>
  ): Stream.Stream<A, E, R> => pipe(self, whenEffect(Effect.sync(test)))
)
...
export const withSpan: {
  ...
  return (self: Stream.Stream<any, any, any>) =>
    new StreamImpl(channel.withSpan(toChannel(self), name, options))
} as any
...
>(
  ...
  <K, A, E, R, A2, E2, R2, A3>(
    ...
    const pull = (
      ...
      pullLeft: Effect.Effect<
        Chunk.Chunk<readonly [K, A]>,
        Option.Option<E>,
        R
      >,
      pullRight: Effect.Effect<
        Chunk.Chunk<readonly [K, A2]>,
        Option.Option<E2>,
        R2
      >
    ): Effect.Effect<
              ...
              onSuccess: ([leftOption, rightOption]) => {
                if (Option.isSome(leftOption) && Option.isSome(rightOption)) {
                  ...
                  if (Chunk.isEmpty(rightOption.value)) {
                    return pull(
                      ZipAllState.PullRight(leftOption.value),
                      pullLeft,
                      pullRight
                    )
                  }
                  return Effect.succeed(
                    Exit.succeed(merge(leftOption.value, rightOption.value))
                  )
                }
                if (Option.isSome(leftOption) && Option.isNone(rightOption)) {
                  ...
                  return Effect.succeed(
                    Exit.succeed([
                      pipe(
                        leftOption.value,
                        Chunk.map(([k, a]) => [k, options.onSelf(a)])
                      ),
                      ZipAllState.DrainLeft
                    ] as const)
                  )
                ...
                if (Option.isNone(leftOption) && Option.isSome(rightOption)) {
                  ...
                  return Effect.succeed(
                    Exit.succeed([
                      pipe(
                        rightOption.value,
                        Chunk.map(([k, a2]) => [k, options.onOther(a2)])
                      ),
                      ZipAllState.DrainRight
                    ] as const)
                  )
                ...
                return Effect.succeed(
                  Exit.fail<Option.Option<E | E2>>(Option.none())
                )
              }
            })
          )
          ...
          return Effect.matchEffect(pullLeft, {
            ...
            onSuccess: (leftChunk) =>
              Chunk.isEmpty(leftChunk)
                ? pull(
                    ZipAllState.PullLeft(state.rightChunk),
                    pullLeft,
                    pullRight
                  )
                : Effect.succeed(
                    Exit.succeed(merge(leftChunk, state.rightChunk))
                  )
          })
        }
          ...
          return Effect.matchEffect(pullRight, {
            ...
            onSuccess: (rightChunk) =>
              Chunk.isEmpty(rightChunk)
                ...
                : Effect.succeed(
                    Exit.succeed(merge(state.leftChunk, rightChunk))
                  )
          })
        }
      }
    ...
    const merge = (
      ...
      const hasNext = <T>(chunk: Chunk.Chunk<T>, index: number) =>
      ...
      while (loop) {
        ...
        if (compare === 0) {
          ...
          if (
            hasNext(leftChunk, leftIndex) &&
            hasNext(rightChunk, rightIndex)
          ) {
            ...
            state = ZipAllState.PullRight(
              pipe(leftChunk, Chunk.drop(leftIndex + 1))
            )
            loop = false
          } else if (hasNext(rightChunk, rightIndex)) {
            state = ZipAllState.PullLeft(
              pipe(rightChunk, Chunk.drop(rightIndex + 1))
            )
            loop = false
          } else {
            ...
            Effect.matchEffect({
              ...
              onSuccess: ([leftOption, rightOption]) => {
                if (Option.isSome(leftOption) && Option.isSome(rightOption)) {
                  ...
                  return Effect.succeed(
                    Exit.succeed(
                      zip(leftOption.value, rightOption.value, options.onBoth)
                    )
                  )
                }
                if (Option.isSome(leftOption) && Option.isNone(rightOption)) {
          ...
          return Effect.matchEffect(pullLeft, {
            ...
            onSuccess: (leftChunk) => {
              ...
              if (Chunk.isEmpty(state.rightChunk)) {
                return pull(
                  ZipAllState.PullRight(leftChunk),
                  pullLeft,
                  pullRight
                )
              }
              return Effect.succeed(
                Exit.succeed(zip(leftChunk, state.rightChunk, options.onBoth))
              )
            }
          })
        }
        case ZipAllState.OP_PULL_RIGHT: {
          return Effect.matchEffect(pullRight, {
            ...
            onSuccess: (rightChunk) => {
              ...
              return Effect.succeed(
                Exit.succeed(zip(state.leftChunk, rightChunk, options.onBoth))
              )
            }
          })
        }
...
export const zipLatest: {
  ...
  <AL, EL, RL, AR, ER, RR>(
    ...
    pipe(
      left,
      zipLatestWith(right, (a, a2) => [a, a2])
    )
)

export const zipLatestAll = <
  T extends ReadonlyArray<Stream.Stream<any, any, any>>
>(
  ...streams: T
): Stream.Stream<
  [T[number]] extends [never]
    ? never
    : {
        [K in keyof T]: T[K] extends Stream.Stream<infer A, infer _E, infer _R>
          ? A
          : never
      },
  [T[number]] extends [never]
    ...
    const pullNonEmpty = <_R, _E, _A>(
      ...
      pipe(
        ...
        Effect.flatMap((chunk) =>
          Chunk.isEmpty(chunk) ? pullNonEmpty(pull) : Effect.succeed(chunk)
        )
      )
    return pipe(
      toPull(left),
      ...
      Effect.flatMap(([left, right]) =>
        pipe(
          fromEffectOption<
            readonly [Chunk.Chunk<AL>, Chunk.Chunk<AR>, boolean],
            EL | ER,
            RL | RR
          >(
            Effect.raceWith(left, right, {
              onSelfDone: (leftDone, rightFiber) =>
                pipe(
                  ...
                  Effect.zipWith(
                    Fiber.join(rightFiber),
                    (l, r) => [l, r, true] as const
                  )
                ),
              onOtherDone: (rightDone, leftFiber) =>
                pipe(
                  ...
                  Effect.zipWith(
                    Fiber.join(leftFiber),
                    (l, r) => [r, l, false] as const
                  )
                )
            })
          ),
                ...
                pipe(
                  fromChunk(
                    leftFirst
                      ? pipe(
                          r,
                          Chunk.map((a2) => f(Chunk.unsafeLast(l), a2))
                        )
                      : pipe(
                          l,
                          Chunk.map((a) => f(a, Chunk.unsafeLast(r)))
                        )
                  ),
                  concat(
                    pipe(
                            ...
                            Ref.modify(
                              ...
                              ([_, rightLatest]) =>
                                [
                                  pipe(
                                    leftChunk,
                                    Chunk.map((a) => f(a, rightLatest))
                                  ),
                                  [
                                    Chunk.unsafeLast(leftChunk),
                                    rightLatest
                                  ] as const
                                ] as const
                            ),
                            ...
                            Ref.modify(
                              ...
                              ([leftLatest, _]) =>
                                [
                                  pipe(
                                    rightChunk,
                                    Chunk.map((a2) => f(leftLatest, a2))
                                  ),
                                  [
                                    leftLatest,
                                    Chunk.unsafeLast(rightChunk)
                                  ] as const
                                ] as const
                            )
                        })
...
export const zipWith: {
  ...
  <AL, EL, RL, AR, ER, RR, A>(
    ...
    pipe(
      left,
      zipWithChunks(right, (leftChunk, rightChunk) =>
...
export const zipWithChunks = dual<
  <A2, E2, R2, A, A3>(
    ...
    f: (
      ...
      Chunk.Chunk<A3>,
      Either.Either<Chunk.Chunk<A2>, Chunk.Chunk<A>>
    ]
  ) => <E, R>(
    self: Stream.Stream<A, E, R>
  ) => Stream.Stream<A3, E2 | E, R2 | R>,
            ...
            Effect.matchEffect({
              ...
              onSuccess: ([leftOption, rightOption]) => {
                if (Option.isSome(leftOption) && Option.isSome(rightOption)) {
                  ...
                  if (Chunk.isEmpty(leftOption.value)) {
                    return pull(
                      ZipChunksState.PullLeft(rightOption.value),
                      pullLeft,
                      pullRight
                    )
                  }
                  if (Chunk.isEmpty(rightOption.value)) {
                    return pull(
                      ZipChunksState.PullRight(leftOption.value),
                      pullLeft,
                      pullRight
                    )
                  }
                  return Effect.succeed(
                    Exit.succeed(zip(leftOption.value, rightOption.value))
                  )
                }
        ...
        case ZipChunksState.OP_PULL_LEFT: {
          return Effect.matchEffect(pullLeft, {
            ...
            onSuccess: (leftChunk) => {
              if (Chunk.isEmpty(leftChunk)) {
                return pull(
                  ZipChunksState.PullLeft(state.rightChunk),
                  pullLeft,
                  pullRight
                )
              }
              if (Chunk.isEmpty(state.rightChunk)) {
                return pull(
                  ZipChunksState.PullRight(leftChunk),
                  pullLeft,
                  pullRight
                )
              }
              return Effect.succeed(
                Exit.succeed(zip(leftChunk, state.rightChunk))
              )
            }
          })
        }
        case ZipChunksState.OP_PULL_RIGHT: {
          return Effect.matchEffect(pullRight, {
            ...
            onSuccess: (rightChunk) => {
              if (Chunk.isEmpty(rightChunk)) {
                return pull(
                  ZipChunksState.PullRight(state.leftChunk),
                  pullLeft,
                  pullRight
                )
              }
              if (Chunk.isEmpty(state.leftChunk)) {
                return pull(
                  ZipChunksState.PullLeft(rightChunk),
                  pullLeft,
                  pullRight
                )
              }
              return Effect.succeed(
                Exit.succeed(zip(state.leftChunk, rightChunk))
              )
            }
          })
        }
...
export const zipWithNext = <A, E, R>(
        ...
        const [newLast, chunk] = Chunk.mapAccum(
          ...
          (prev, curr) =>
            [
              ...
              pipe(
                prev,
                Option.map((a) => [a, curr] as const)
              )
            ] as const
        )
        const output = Chunk.filterMap(chunk, (option) =>
      ...
      onDone: () =>
        Option.match(last, {
          ...
          onSome: (value) =>
            channel.zipRight(
              core.write(
                Chunk.of<readonly [A, Option.Option<A>]>([value, Option.none()])
              ),
              core.void
            )
        })
  ...
  return new StreamImpl(
    pipe(toChannel(self), channel.pipeToOrFail(process(Option.none())))
  )
}
...
export const fromEventListener = <A = unknown>(
  ...
  asyncPush<A>(
    (emit) =>
      Effect.acquireRelease(
        Effect.sync(() =>
          target.addEventListener(type, emit.single as any, options)
        ),
        () =>
          Effect.sync(() =>
