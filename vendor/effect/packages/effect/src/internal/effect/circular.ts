  ...
  readonly withPermits =
    ...
    <A, E, R>(self: Effect.Effect<A, E, R>) =>
      core.uninterruptibleMask((restore) =>
        core.flatMap(restore(this.take(n)), (permits) =>
          fiberRuntime.ensuring(restore(self), this.release(permits))
        )
      )
...
/** @internal */
export const unsafeMakeSemaphore = (permits: number): Effect.Semaphore =>
  new Semaphore(permits)

/** @internal */
export const makeSemaphore = (permits: number) =>
  core.sync(() => unsafeMakeSemaphore(permits))
...
export const makeLatch = (open?: boolean | undefined) =>
  core.sync(() => unsafeMakeLatch(open))

/** @internal */
export const awaitAllChildren = <A, E, R>(
  self: Effect.Effect<A, E, R>
): Effect.Effect<A, E, R> => ensuringChildren(self, fiberRuntime.fiberAwaitAll)
...
export const cachedInvalidateWithTTL: {
  ...
  <A, E, R>(
    ...
    return core.flatMap(core.context<R>(), (env) =>
      core.map(
        makeSynchronized<
          Option.Option<readonly [number, Deferred.Deferred<A, E>]>
        >(Option.none()),
        (cache) =>
          [
...
const computeCachedValue = <A, E, R>(
  ...
  Option.Option<[number, Deferred.Deferred<A, E>]>,
  never,
  R
> => {
...
const getCachedValue = <A, E, R>(
  self: Effect.Effect<A, E, R>,
  timeToLive: Duration.DurationInput,
  cache: Synchronized.SynchronizedRef<
    Option.Option<readonly [number, Deferred.Deferred<A, E>]>
  >
): Effect.Effect<A, E, R> =>
  core.uninterruptibleMask((restore) =>
...
export const ensuringChild = dual<
  <X, R2>(
    f: (
      fiber: Fiber.Fiber<ReadonlyArray<unknown>, any>
...
>(2, (self, f) =>
  ensuringChildren(self, (children) => f(fiberRuntime.fiberAll(children)))
)
...
export const forkAll: {
  ...
  <A, E, R>(
    ...
    options?.discard
      ...
      : core.map(
          core.forEachSequential(effects, fiberRuntime.fork),
          fiberRuntime.fiberAll
        )
)
...
>(2, (self, scope) =>
  core.withFiberRuntime((parent, parentStatus) => {
    ...
    const fiber = fiberRuntime.unsafeFork(
      self,
      parent,
...
/** @internal */
export const fromFiber = <A, E>(
  fiber: Fiber.Fiber<A, E>
): Effect.Effect<A, E> => internalFiber.join(fiber)

/** @internal */
export const fromFiberEffect = <A, E, R>(
  fiber: Effect.Effect<Fiber.Fiber<A, E>, E, R>
): Effect.Effect<A, E, R> =>
...
>(
  ...
  <A, E, R, A2, E2, R2>(
    ...
    pipe(
      ...
      (effect: Effect.Effect<Exit.Exit<A | A2, E | E2>, never, R | R2>) =>
        core.flatten(effect)
    )
)
...
>(2, (self, supervisor) => {
  const supervise = core.fiberRefLocallyWith(
...
/** @internal */
export const SynchronizedTypeId: Synchronized.SynchronizedRefTypeId =
  Symbol.for(SynchronizedSymbolKey) as Synchronized.SynchronizedRefTypeId
...
/** @internal */
class SynchronizedImpl<in out A>
  extends Effectable.Class<A>
  implements Synchronized.SynchronizedRef<A>
{
...
/** @internal */
export const makeSynchronized = <A>(
  value: A
): Effect.Effect<Synchronized.SynchronizedRef<A>> =>
  core.sync(() => unsafeMakeSynchronized(value))

/** @internal */
export const unsafeMakeSynchronized = <A>(
  value: A
...
export const zipFiber = dual<
  <A2, E2>(
    that: Fiber.Fiber<A2, E2>
  ) => <A, E>(self: Fiber.Fiber<A, E>) => Fiber.Fiber<[A, A2], E | E2>,
  <A, E, A2, E2>(
    self: Fiber.Fiber<A, E>,
    that: Fiber.Fiber<A2, E2>
  ) => Fiber.Fiber<[A, A2], E | E2>
...
export const zipLeftFiber = dual<
  <A2, E2>(
    that: Fiber.Fiber<A2, E2>
  ) => <A, E>(self: Fiber.Fiber<A, E>) => Fiber.Fiber<A, E | E2>,
  <A, E, A2, E2>(
    self: Fiber.Fiber<A, E>,
    that: Fiber.Fiber<A2, E2>
  ) => Fiber.Fiber<A, E | E2>
...
export const zipRightFiber = dual<
  <A2, E2>(
    that: Fiber.Fiber<A2, E2>
  ) => <A, E>(self: Fiber.Fiber<A, E>) => Fiber.Fiber<A2, E | E2>,
  <A, E, A2, E2>(
    self: Fiber.Fiber<A, E>,
    that: Fiber.Fiber<A2, E2>
  ) => Fiber.Fiber<A2, E | E2>
...
>(3, (self, that, f) => ({
  ...
  await: pipe(
    self.await,
    core.flatten,
    fiberRuntime.zipWithOptions(core.flatten(that.await), f, {
      concurrent: true
    }),
    core.exit
  ),
  ...
  poll: core.zipWith(self.poll, that.poll, (optionA, optionB) =>
    pipe(
      optionA,
  ...
  interruptAsFork: (id) =>
    core.zipRight(self.interruptAsFork(id), that.interruptAsFork(id)),
  pipe() {
...
export const bindAll: {
  ...
  >(
    ...
    Effect.Effect<infer Success, infer Error, infer Context>
  ]
    ? Effect.Effect<
        {
  ...
  >(
    ...
    core.flatMap(self, (a) =>
      (
        ...
        >
      ).pipe(core.map((record) => Object.assign({}, a, record)))
    )
)
