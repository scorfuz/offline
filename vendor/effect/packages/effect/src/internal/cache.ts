 ...
 * @internal
 */
export const initialCacheState = <Key, Value, Error>(): CacheState<
  Key,
  Value,
  Error
> =>
...
/** @internal */
export const makeCacheStats = (options: {
...
class CacheImpl<in out Key, in out Value, in out Error> implements Cache.Cache<
  Key,
  Value,
  Error
> {
  ...
  getOptionComplete(key: Key): Effect.Effect<Option.Option<Value>> {
    return core.suspend(() =>
      Option.match(MutableHashMap.get(this.cacheState.map, key), {
        ...
        onSome: (value) =>
          this.resolveMapValue(value, true) as Effect.Effect<
            Option.Option<Value>
          >
      })
    )
  ...
  getEither(key: Key): Effect.Effect<Either.Either<Value, Value>, Error> {
    return core.suspend(
      (): Effect.Effect<Either.Either<Value, Value>, Error> => {
        const k = key
        ...
        let value = Option.getOrUndefined(
          MutableHashMap.get(this.cacheState.map, k)
        )
        if (value === undefined) {
  ...
  refresh(key: Key): Effect.Effect<void, Error> {
    return effect.clockWith((clock) =>
      core.suspend(() => {
        const k = key
        const deferred: Deferred.Deferred<Value, Error> = Deferred.unsafeMake(
          this.fiberId
        )
        let value = Option.getOrUndefined(
          MutableHashMap.get(this.cacheState.map, k)
              ...
              return pipe(
                ...
                effect.when(() => {
                  ...
                  if (Equal.equals(current, value)) {
                    const mapValue = refreshing(
                      deferred,
                      value as Complete<Key, Value, Error>
                    )
    ...
    return effect.clockWith((clock) =>
      core.sync(() => {
        ...
        const mapValue = complete(
          ...
          makeEntryStats(now),
          now +
            Duration.toMillis(Duration.decode(this.timeToLive(lookupResult)))
        )
        MutableHashMap.set(
          this.cacheState.map,
          k,
