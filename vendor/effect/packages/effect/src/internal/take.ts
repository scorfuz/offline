...
/** @internal */
export const chunk = <A>(chunk: Chunk.Chunk<A>): Take.Take<A> =>
  new TakeImpl(Exit.succeed(chunk))

/** @internal */
export const die = (defect: unknown): Take.Take<never> =>
  new TakeImpl(Exit.die(defect))
...
/** @internal */
export const fail = <E>(error: E): Take.Take<never, E> =>
  new TakeImpl(Exit.fail(Option.some(error)))
...
export const fromExit = <A, E>(exit: Exit.Exit<A, E>): Take.Take<A, E> =>
  new TakeImpl(
    pipe(exit, Exit.mapBoth({ onFailure: Option.some, onSuccess: Chunk.of }))
  )
...
>(
  ...
  <A, E, Z, E2, R, Z2, R2, Z3, E3, R3>(
    ...
    Exit.matchEffect(self.exit, {
      onFailure: (cause) =>
        Option.match<Cause.Cause<E>, Effect.Effect<Z | Z2, E2 | E, R | R2>>(
...
/** @internal */
export const of = <A>(value: A): Take.Take<A> =>
  new TakeImpl(Exit.succeed(Chunk.of(value)))
...
>(
  ...
  <A, E, X, E2, R>(
    ...
    pipe(self.exit, Exit.forEachEffect(f), Effect.asVoid)
)
