...
/** @internal */
export const FiberRefsSym: FiberRefs.FiberRefsSym = Symbol.for(
  "effect/FiberRefs"
) as FiberRefs.FiberRefsSym
...
const findAncestor = (
  ...
  while (ret === undefined) {
    if (
      Arr.isNonEmptyReadonlyArray(parentStack) &&
      Arr.isNonEmptyReadonlyArray(childStack)
    ) {
    ...
    if (!childStack[0][0][Equal.symbol](fiberId)) {
      ...
      if (wasModified) {
        ...
        if (!Equal.equals(oldValue, newValue)) {
          let newStack: Arr.NonEmptyReadonlyArray<
            readonly [FiberId.Single, unknown]
          >
          const parentFiberId = parentStack[0][0]
          if (parentFiberId[Equal.symbol](fiberId)) {
            newStack = [
              [parentFiberId, newValue] as const,
              ...parentStack.slice(1)
            ]
          } else {
...
export const forkAs = dual<
  (
    childId: FiberId.Single
  ) => (self: FiberRefs.FiberRefs) => FiberRefs.FiberRefs,
  (self: FiberRefs.FiberRefs, childId: FiberId.Single) => FiberRefs.FiberRefs
>(2, (self, childId) => {
  const map = new Map<
    FiberRef.FiberRef<any>,
    Arr.NonEmptyReadonlyArray<readonly [FiberId.Single, unknown]>
  >()
...
/** @internal */
export const fiberRefs = (self: FiberRefs.FiberRefs) =>
  HashSet.fromIterable(self.locals.keys())

/** @internal */
export const setAll = (self: FiberRefs.FiberRefs): Effect.Effect<void> =>
  core.forEachSequentialDiscard(fiberRefs(self), (fiberRef) =>
    core.fiberRefSet(fiberRef, getOrDefault(self, fiberRef))
  )
...
>(2, (self, fiberRef) =>
  pipe(
    get(self, fiberRef),
    Option.getOrElse(() => fiberRef.initial)
  )
)
...
const unsafeUpdateAs = (
  ...
  const oldStack: ReadonlyArray<readonly [FiberId.Single, any]> =
    locals.get(fiberRef) ?? []
  let newStack:
    | Arr.NonEmptyReadonlyArray<readonly [FiberId.Single, any]>
    | undefined
  ...
  if (Arr.isNonEmptyReadonlyArray(oldStack)) {
    ...
    if (currentId[Equal.symbol](fiberId)) {
      if (Equal.equals(currentValue, value)) {
        ...
        newStack = [[fiberId, value] as const, ...oldStack.slice(1)]
      }
    } else {
      newStack = [[fiberId, value] as const, ...oldStack]
    }
  } else {
