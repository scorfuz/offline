      ...
      Ref.modify(self.ref, (state) =>
        pipe(
          ...
          handoffStateMatch(
            ...
            (_, notifyProducer) => [
              Effect.flatMap(Deferred.await(notifyProducer), () =>
      ...
      Ref.modify(self.ref, (state) =>
        pipe(
          ...
          handoffStateMatch(
            (notifyConsumer) =>
              [
                Effect.flatMap(Deferred.await(notifyConsumer), () =>
                  take(self)
                ),
                state
              ] as const,
            (value, notifyProducer) => [
              Effect.as(Deferred.succeed(notifyProducer, void 0), value),
    ...
    Effect.flatten(
      Ref.modify(self.ref, (state) =>
        pipe(
          state,
          handoffStateMatch(
            () => [Effect.succeed(Option.none<A>()), state] as const,
            (value, notifyProducer) => [
              Effect.as(
