...
/** @internal */
export class Zip<out T0, out T1> implements Supervisor.Supervisor<
  readonly [T0, T1]
> {
  readonly _tag = "Zip"
  readonly [SupervisorTypeId] = supervisorVariance
...
/** @internal */
export class Track implements Supervisor.Supervisor<
...
> {
  ...
  map<B>(
    f: (a: Array<Fiber.RuntimeFiber<any, any>>) => B
...
class FibersIn implements Supervisor.Supervisor<
  SortedSet.SortedSet<Fiber.RuntimeFiber<any, any>>
> {
  readonly [SupervisorTypeId] = supervisorVariance

  constructor(
    readonly ref: MutableRef.MutableRef<
      SortedSet.SortedSet<Fiber.RuntimeFiber<any, any>>
    >
  ) {}

  get value(): Effect.Effect<
    SortedSet.SortedSet<Fiber.RuntimeFiber<any, any>>
  > {
  ...
  onStart<A, E, R>(
    ...
    pipe(
      this.ref,
      MutableRef.set(pipe(MutableRef.get(this.ref), SortedSet.add(fiber)))
    )
  }
  ...
  onEnd<A, E>(_value: Exit.Exit<A, E>, fiber: Fiber.RuntimeFiber<A, E>): void {
    pipe(
      this.ref,
      MutableRef.set(pipe(MutableRef.get(this.ref), SortedSet.remove(fiber)))
    )
  }
  ...
  map<B>(
    f: (a: SortedSet.SortedSet<Fiber.RuntimeFiber<any, any>>) => B
  ): Supervisor.Supervisor<B> {
    return new ProxySupervisor(this, pipe(this.value, core.map(f)))
  }
  ...
  zip<A>(
    ...
    [SortedSet.SortedSet<Fiber.RuntimeFiber<any, any>>, A]
  > {
    return new Zip(this, right)
  }
...
/** @internal */
export const unsafeTrack = (): Supervisor.Supervisor<
  Array<Fiber.RuntimeFiber<any, any>>
> => {
  return new Track()
}
...
/** @internal */
export const none = globalValue("effect/Supervisor/none", () =>
  fromEffect(core.void)
)
