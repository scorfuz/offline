...
export const fromAcquire = <A, E, R>(
  ...
  core.uninterruptible(
    fiberRuntime.scopeMake().pipe(
      core.flatMap((newScope) =>
        acquire.pipe(
          core.mapInputContext<R, Scope.Scope | R>(
            Context.add(fiberRuntime.scopeTag, newScope)
          ),
...
export const make = <A>(
  evaluate: LazyArg<A>
): Effect.Effect<ScopedRef.ScopedRef<A>, never, Scope.Scope> =>
...
>(2, <A, R, E>(self: ScopedRef.ScopedRef<A>, acquire: Effect.Effect<A, E, R>) =>
  core.flatten(
            ...
            core.exit(fiberRuntime.scopeExtend(acquire, newScope)).pipe(
              core.flatMap((exit) =>
                core.exitMatch(exit, {
                  onFailure: (cause) =>
                    core
                      .scopeClose(newScope, core.exitVoid)
                      .pipe(
                        core.as([
