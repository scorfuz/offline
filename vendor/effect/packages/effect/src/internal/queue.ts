...
/** @internal */
export const EnqueueTypeId: Queue.EnqueueTypeId = Symbol.for(
  EnqueueSymbolKey
) as Queue.EnqueueTypeId

/** @internal */
const DequeueSymbolKey = "effect/QueueDequeue"

/** @internal */
export const DequeueTypeId: Queue.DequeueTypeId = Symbol.for(
  DequeueSymbolKey
) as Queue.DequeueTypeId
...
/** @internal */
class QueueImpl<in out A>
  extends Effectable.Class<A>
  implements Queue.Queue<A>
{
  ...
  get size(): Effect.Effect<number> {
    return core.suspend(() =>
      core.catchAll(this.unsafeSize(), () => core.interrupt)
    )
  }
...
export const bounded = <A>(
  ...
  pipe(
    ...
    core.flatMap((queue) =>
      make(backingQueueFromMutableQueue(queue), backPressureStrategy())
    )
  )
...
export const dropping = <A>(
  ...
  pipe(
    core.sync(() => MutableQueue.bounded<A>(requestedCapacity)),
    core.flatMap((queue) =>
      make(backingQueueFromMutableQueue(queue), droppingStrategy())
    )
  )
...
export const sliding = <A>(
  ...
  pipe(
    core.sync(() => MutableQueue.bounded<A>(requestedCapacity)),
    core.flatMap((queue) =>
      make(backingQueueFromMutableQueue(queue), slidingStrategy())
    )
  )
...
export class BackingQueueFromMutableQueue<
  in out A
> implements Queue.BackingQueue<A> {
...
/** @internal */
export const backingQueueFromMutableQueue = <A>(
  mutable: MutableQueue.MutableQueue<A>
): Queue.BackingQueue<A> => new BackingQueueFromMutableQueue(mutable)

/** @internal */
export const capacity = <A>(
  self: Queue.Dequeue<A> | Queue.Enqueue<A>
): number => self.capacity()

/** @internal */
export const size = <A>(
  self: Queue.Dequeue<A> | Queue.Enqueue<A>
): Effect.Effect<number> => self.size

/** @internal */
export const isFull = <A>(
  self: Queue.Dequeue<A> | Queue.Enqueue<A>
): Effect.Effect<boolean> => self.isFull

/** @internal */
export const isEmpty = <A>(
  self: Queue.Dequeue<A> | Queue.Enqueue<A>
): Effect.Effect<boolean> => self.isEmpty

/** @internal */
export const isShutdown = <A>(
  self: Queue.Dequeue<A> | Queue.Enqueue<A>
): Effect.Effect<boolean> => self.isShutdown

/** @internal */
export const awaitShutdown = <A>(
  self: Queue.Dequeue<A> | Queue.Enqueue<A>
): Effect.Effect<void> => self.awaitShutdown

/** @internal */
export const shutdown = <A>(
  self: Queue.Dequeue<A> | Queue.Enqueue<A>
): Effect.Effect<void> => self.shutdown
...
export const offerAll = dual<
  ...
  <A>(self: Queue.Enqueue<A>, iterable: Iterable<A>) => Effect.Effect<boolean>
>(2, (self, iterable) => self.offerAll(iterable))

/** @internal */
export const poll = <A>(
  self: Queue.Dequeue<A>
): Effect.Effect<Option.Option<A>> => core.map(self.takeUpTo(1), Chunk.head)
...
export const takeAll = <A>(
  self: Queue.Dequeue<A>
): Effect.Effect<Chunk.Chunk<A>> => self.takeAll
...
export const takeBetween = dual<
  (
    min: number,
    max: number
  ) => <A>(self: Queue.Dequeue<A>) => Effect.Effect<Chunk.Chunk<A>>,
  <A>(
    self: Queue.Dequeue<A>,
    min: number,
    max: number
...
class BackPressureStrategy<in out A> implements Queue.Strategy<A> {
  ...
  readonly putters =
    MutableQueue.unbounded<readonly [A, Deferred.Deferred<boolean>, boolean]>()
  ...
  onCompleteTakersWithEmptyQueue(
    ...
    while (
      !MutableQueue.isEmpty(this.putters) &&
      !MutableQueue.isEmpty(takers)
    ) {
    ...
    while (
      ...
      if (putter === MutableQueue.EmptyMutableQueue) {
        ...
        if (offered && putter[2]) {
          ...
          unsafeOfferAll(
            this.putters,
            pipe(unsafePollAll(this.putters), Chunk.prepend(putter))
          )
        }
        unsafeCompleteTakers(this, queue, takers)
      }
...
/** @internal */
const unsafeCompleteDeferred = <A>(
  ...
  while (keepPolling && queue.length() !== 0) {
    ...
    if (taker !== MutableQueue.EmptyMutableQueue) {
      ...
      if (element !== MutableQueue.EmptyMutableQueue) {
        ...
        unsafeOfferAll(
          takers,
          pipe(unsafePollAll(takers), Chunk.prepend(taker))
        )
      }
      keepPolling = true
    } else {
