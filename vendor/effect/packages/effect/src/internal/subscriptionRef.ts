...
/** @internal */
export const SubscriptionRefTypeId: SubscriptionRef.SubscriptionRefTypeId =
  Symbol.for(SubscriptionRefSymbolKey) as SubscriptionRef.SubscriptionRefTypeId
...
/** @internal */
class SubscriptionRefImpl<in out A>
  extends Effectable.Class<A>
  implements SubscriptionRef.SubscriptionRef<A>
{
  ...
  get changes(): Stream<A> {
    return pipe(
      Ref.get(this.ref),
      Effect.flatMap((a) =>
        Effect.map(stream.fromPubSub(this.pubsub, { scoped: true }), (s) =>
          stream.concat(stream.make(a), s)
        )
      ),
...
): Effect.Effect<A> => Ref.get(self.ref)

/** @internal */
export const make = <A>(
  value: A
): Effect.Effect<SubscriptionRef.SubscriptionRef<A>> =>
  pipe(
    Effect.all([
      PubSub.unbounded<A>(),
      Ref.make(value),
...
export const modify = dual<
  <A, B>(
    f: (a: A) => readonly [B, A]
  ) => (self: SubscriptionRef.SubscriptionRef<A>) => Effect.Effect<B>,
  <A, B>(
    self: SubscriptionRef.SubscriptionRef<A>,
    f: (a: A) => readonly [B, A]
  ) => Effect.Effect<B>
...
export const set = dual<
  <A>(
    value: A
  ) => (self: SubscriptionRef.SubscriptionRef<A>) => Effect.Effect<void>,
  <A>(self: SubscriptionRef.SubscriptionRef<A>, value: A) => Effect.Effect<void>
>(
  2,
  <A>(
    self: SubscriptionRef.SubscriptionRef<A>,
