...
export const make = <A, R>(
  ...
  new core.RequestResolverImpl((requests) =>
    runAll(requests.map((_) => _.map((_) => _.request)))
  )
...
>(
  ...
  (self, before, after) =>
    new core.RequestResolverImpl(
      (requests) =>
        core.acquireUseRelease(before, () => self.runAll(requests), after),
...
export const aroundRequests = dual<
  <A, A2, R2, X, R3>(
    before: (
      requests: ReadonlyArray<NoInfer<A>>
    ) => Effect.Effect<A2, never, R2>,
    after: (
      requests: ReadonlyArray<NoInfer<A>>,
      _: A2
...
>(
  ...
  (self, before, after) =>
    new core.RequestResolverImpl(
      (requests) => {
        const flatRequests = requests.flatMap((chunk) =>
        ...
        return n < 1
          ? core.die(
              new Cause.IllegalArgumentException(
                "RequestResolver.batchN: n must be at least 1"
              )
            )
          : self.runAll(
              Array.from(
                Chunk.map(
                  RA.reduce(
                    ...
                    (acc, chunk) =>
                      Chunk.appendAll(
                        acc,
                        Chunk.chunksOf(Chunk.unsafeFromArray(chunk), n)
                      )
                  ),
...
export const eitherWith = dual<
  ...
  >(
    ...
    f: (
      _: Request.Entry<C>
    ) => Either.Either<Request.Entry<B>, Request.Entry<A>>
  ) => <R>(
    self: RequestResolver.RequestResolver<A, R>
...
export const fromFunction = <A extends Request.Request<any>>(
  ...
  makeBatched((requests: RA.NonEmptyArray<A>) =>
    core.forEachSequentialDiscard(requests, (request) =>
      complete(request, core.exitSucceed(f(request)) as any)
    )
...
export const fromEffectTagged =
  ...
  >(
    ...
    A,
    ReturnType<Fns[keyof Fns]> extends Effect.Effect<
      infer _A,
      infer _E,
      infer R
    >
      ...
      return Effect.forEach(
        ...
        (tag) =>
          Effect.matchCauseEffect(
            (fns[tag] as any)(grouped[tag]) as Effect.Effect<
              Array<any>,
              unknown,
              unknown
            >,
            {
              onFailure: (cause) =>
                Effect.forEach(
                  grouped[tag],
                  (req) => complete(req, core.exitFail(cause) as any),
                  { discard: true }
                ),
              onSuccess: (res) =>
                Effect.forEach(
                  grouped[tag],
                  (req, i) => complete(req, core.exitSucceed(res[i]) as any),
                  { discard: true }
                )
            }
          ),
...
/** @internal */
export const never: RequestResolver.RequestResolver<never> = make(
  () => Effect.never
).identified("Never")
...
>(2, (self, context) =>
  mapInputContext(self, (_: Context.Context<never>) => context).identified(
    "ProvideContext",
    self,
    context
  )
...
export const race = dual<
  ...
  <
    A extends Request.Request<any, any>,
    R,
    A2 extends Request.Request<any, any>,
    R2
  >(
    self: RequestResolver.RequestResolver<A, R>,
    that: RequestResolver.RequestResolver<A2, R2>
