...
/** @internal */
export const EffectTypeId: Effect.EffectTypeId = Symbol.for(
  "effect/Effect"
) as Effect.EffectTypeId
...
/** @internal */
export interface Async extends Op<
  OpCodes.OP_ASYNC,
  {
...
/** @internal */
export interface Blocked<out E = any, out A = any> extends Op<
  "Blocked",
  {
...
/** @internal */
export interface RunBlocked extends Op<
  "RunBlocked",
  {
    readonly effect_instruction_i0: BlockedRequests.RequestBlock
  }
...
/** @internal */
export interface Failure extends Op<
  OpCodes.OP_FAILURE,
  {
...
/** @internal */
export interface Commit extends Op<
  OpCodes.OP_COMMIT,
  {
...
/** @internal */
export interface OnFailure extends Op<
  OpCodes.OP_ON_FAILURE,
  {
...
/** @internal */
export interface OnStep extends Op<
  "OnStep",
  { readonly effect_instruction_i0: Primitive }
> {}

/** @internal */
export interface OnSuccessAndFailure extends Op<
...
/** @internal */
export interface Success extends Op<
  OpCodes.OP_SUCCESS,
  {
...
/** @internal */
export interface Sync extends Op<
  OpCodes.OP_SYNC,
  {
...
/** @internal */
export interface UpdateRuntimeFlags extends Op<
...
/** @internal */
export interface While extends Op<
  OpCodes.OP_WHILE,
  {
...
/** @internal */
export interface FromIterator extends Op<
  OpCodes.OP_ITERATOR,
  {
...
/** @internal */
export interface WithRuntime extends Op<
  OpCodes.OP_WITH_RUNTIME,
  {
    effect_instruction_i0(
      fiber: FiberRuntime.FiberRuntime<unknown, unknown>,
...
export const isEffect = (
  u: unknown
): u is Effect.Effect<unknown, unknown, unknown> => hasProperty(u, EffectTypeId)

/* @internal */
export const withFiberRuntime = <A, E = never, R = never>(
  withRuntime: (
    fiber: FiberRuntime.FiberRuntime<A, E>,
    status: FiberStatus.Running
  ) => Effect.Effect<A, E, R>
): Effect.Effect<A, E, R> => {
...
export const acquireUseRelease: {
  ...
  <A, E, R, A2, E2, R2, X, R3>(
    ...
    uninterruptibleMask((restore) =>
      flatMap(acquire, (a) =>
        flatMap(
          exit(suspend(() => restore(use(a)))),
          (exit): Effect.Effect<A2, E | E2, R | R2 | R3> => {
...
export const as: {
  <B>(
    value: B
  ): <A, E, R>(self: Effect.Effect<A, E, R>) => Effect.Effect<B, E, R>
  ...
  <A, E, R, B>(
    ...
    value: B
  ): Effect.Effect<B, E, R> => flatMap(self, () => succeed(value))
)

/* @internal */
export const asVoid = <A, E, R>(
  self: Effect.Effect<A, E, R>
): Effect.Effect<void, E, R> => as(self, void 0)

/* @internal */
export const custom: {
  <X, A, E, R>(
    i0: X,
    body: (this: { effect_instruction_i0: X }) => Effect.Effect<A, E, R>
  ): Effect.Effect<A, E, R>
  <X, Y, A, E, R>(
    i0: X,
    i1: Y,
...
export const unsafeAsync = <A, E = never, R = never>(
  ...
  effect.effect_instruction_i0 = (
    resume: (_: Effect.Effect<A, E, R>) => void
  ) => {
    cancelerRef = register(resume)
  }
  effect.effect_instruction_i1 = blockingOn
  return onInterrupt(effect, (_) =>
    isEffect(cancelerRef) ? cancelerRef : void_
  )
}
...
const async_ = <A, E = never, R = never>(
  ...
  return custom(resume, function () {
    let backingResume: ((_: Effect.Effect<A, E, R>) => void) | undefined =
      undefined
    ...
    if (this.effect_instruction_i0.length !== 1) {
      ...
      cancelerRef = internalCall(() =>
        (this.effect_instruction_i0 as any)(proxyResume)
      )
    }
    ...
    catchAllCause(self, (cause): Effect.Effect<A2 | A, E | E2, R2 | R> => {
      ...
      switch (either._tag) {
        case "Left":
          return pipe(
            pf(either.left),
            Option.getOrElse(() => failCause(cause))
          )
        case "Right":
          return failCause(either.right)
      }
...
export const checkInterruptible = <A, E, R>(
  ...
  withFiberRuntime((_, status) =>
    f(runtimeFlags_.interruption(status.runtimeFlags))
  )
...
export const capture = <E>(
  ...
  if (Option.isSome(span)) {
    return new Proxy(obj, {
      has(target, p) {
        return (
          p === internalCause.spanSymbol || p === originalSymbol || p in target
        )
      },
      get(target, p) {
...
export const die = (defect: unknown): Effect.Effect<never> =>
  isObject(defect) && !(internalCause.spanSymbol in defect)
    ? withFiberRuntime((fiber) =>
        failCause(
          internalCause.die(capture(defect, currentSpanFromFiber(fiber)))
        )
      )
...
export const fail = <E>(error: E): Effect.Effect<never, E> =>
  isObject(error) && !(internalCause.spanSymbol in error)
    ? withFiberRuntime((fiber) =>
        failCause(
          internalCause.fail(capture(error, currentSpanFromFiber(fiber)))
        )
      )
...
/* @internal */
export const fiberId: Effect.Effect<FiberId.FiberId> = withFiberRuntime(
...
export const andThen: {
  <A, X>(
    ...
    : [X] extends [PromiseLike<infer A1>]
      ? Effect.Effect<A1, E | Cause.UnknownException, R>
      : Effect.Effect<X, E, R>
  <X>(
    f: NotFunction<X>
  ): <A, E, R>(
            ...
            resume(
              fail(
                new UnknownException(
                  e,
                  "An unknown error occurred in Effect.andThen"
                )
              )
            )
        )
...
>(
  ...
  <A1, E1, R1, A2, E2, R2, E = never, R = never>(
    ...
    isEffect(self)
      ? flatMap(
          self,
          (b): Effect.Effect<A1 | A2, E1 | E2, R1 | R2> =>
...
/* @internal */
export const interrupt: Effect.Effect<never> = flatMap(fiberId, (fiberId) =>
  interruptWith(fiberId)
)
...
export const intoDeferred: {
  ...
  ): <R>(self: Effect.Effect<A, E, R>) => Effect.Effect<boolean, never, R>
  <A, E, R>(
    self: Effect.Effect<A, E, R>,
    deferred: Deferred.Deferred<A, E>
  ...
  <A, E, R>(
    ...
    uninterruptibleMask((restore) =>
      flatMap(exit(restore(self)), (exit) => deferredDone(deferred, exit))
    )
)
...
export const map: {
  ...
  <A, E, R, B>(
    self: Effect.Effect<A, E, R>,
    f: (a: A) => B
  ): Effect.Effect<B, E, R> => flatMap(self, (a) => sync(() => f(a)))
)
...
export const onError: {
  ...
  <A, E, R, X, R2>(
    ...
    onExit(self, (exit) =>
      exitIsSuccess(exit) ? void_ : cleanup(exit.effect_instruction_i0)
    )
)
      ...
      matchCauseEffect(restore(self), {
        onFailure: (cause1) => {
          ...
          return matchCauseEffect(cleanup(result), {
            onFailure: (cause2) =>
              exitFailCause(internalCause.sequential(cause1, cause2)),
            onSuccess: () => result
          })
        },
        onSuccess: (success) => {
...
export const orDie = <A, E, R>(
  self: Effect.Effect<A, E, R>
): Effect.Effect<A, never, R> => orDieWith(self, identity)
...
/* @internal */
export const runtimeFlags: Effect.Effect<RuntimeFlags.RuntimeFlags> =
  withFiberRuntime((_, status) => succeed(status.runtimeFlags))
...
export const tap = dual<
  {
    ...
    ): <E, R>(self: Effect.Effect<A, E, R>) => Effect.Effect<A, E | E1, R | R1>
    <X>(
      f: NotFunction<X>
    ): <A, E, R>(
      self: Effect.Effect<A, E, R>
...
>(
  (args) =>
    args.length === 3 ||
    (args.length === 2 && !(isObject(args[1]) && "onlyEffect" in args[1])),
  ...
  <A, E, R, A2, E2, R2, A3, E3, R3>(
    ...
    matchCauseEffect(self, {
      onFailure: (cause) => {
        ...
        if (defects.length > 0) {
          return failCause(
            Option.getOrThrow(internalCause.keepDefectsAndElectFailures(cause))
          )
        }
        return that()
      },
...
>(2, (effect, enabled) =>
  fiberRefLocally(effect, currentTracerEnabled, enabled)
)
...
>(2, (effect, enabled) =>
  fiberRefLocally(effect, currentTracerTimingEnabled, enabled)
)
...
/* @internal */
export const interruptFiber = <A, E>(
...
export const interruptAsFiber = dual<
  ...
  ) => <A, E>(self: Fiber.Fiber<A, E>) => Effect.Effect<Exit.Exit<A, E>>,
  <A, E>(
    self: Fiber.Fiber<A, E>,
    fiberId: FiberId.FiberId
  ) => Effect.Effect<Exit.Exit<A, E>>
>(2, (self, fiberId) =>
  flatMap(self.interruptAsFork(fiberId), () => self.await)
)
...
>(2, (self, pf) =>
  fiberRefModify(self, (v) => [v, Option.getOrElse(pf(v), () => v)] as const)
)
...
export const fiberRefGetWith = dual<
  ...
  ) => (self: FiberRef.FiberRef<A>) => Effect.Effect<B, E, R>,
  <A, B, E, R>(
    self: FiberRef.FiberRef<A>,
    f: (a: A) => Effect.Effect<B, E, R>
  ) => Effect.Effect<B, E, R>
...
/* @internal */
export const fiberRefReset = <A>(
  self: FiberRef.FiberRef<A>
): Effect.Effect<void> => fiberRefSet(self, self.initial)

/* @internal */
export const fiberRefModify = dual<
  <A, B>(
    f: (a: A) => readonly [B, A]
  ) => (self: FiberRef.FiberRef<A>) => Effect.Effect<B>,
  <A, B>(
    self: FiberRef.FiberRef<A>,
    f: (a: A) => readonly [B, A]
  ) => Effect.Effect<B>
...
export const fiberRefModifySome = <A, B>(
  ...
  fiberRefModify(self, (v) => Option.getOrElse(f(v), () => [def, v] as const))
...
export const fiberRefUpdateSome = dual<
  <A>(
    pf: (a: A) => Option.Option<A>
  ) => (self: FiberRef.FiberRef<A>) => Effect.Effect<void>,
  <A>(
    self: FiberRef.FiberRef<A>,
    pf: (a: A) => Option.Option<A>
  ) => Effect.Effect<void>
>(2, (self, pf) =>
  fiberRefModify(
    self,
    (v) => [void 0, Option.getOrElse(pf(v), () => v)] as const
  )
)
...
/** @internal */
export const RequestResolverTypeId: RequestResolver.RequestResolverTypeId =
  Symbol.for(RequestResolverSymbolKey) as RequestResolver.RequestResolverTypeId
...
export class RequestResolverImpl<
  in A,
  out R
> implements RequestResolver.RequestResolver<A, R> {
  ...
  [Hash.symbol](): number {
    return Hash.cached(
      this,
      this.target ? Hash.hash(this.target) : Hash.random(this)
    )
  }
  [Equal.symbol](that: unknown): boolean {
    return this.target
      ? isRequestResolver(that) &&
          Equal.equals(
            this.target,
            (that as RequestResolverImpl<any, any>).target
          )
      : this === that
  }
...
/** @internal */
export const isRequestResolver = (
  u: unknown
): u is RequestResolver.RequestResolver<unknown, unknown> =>
...
>(
  ...
  <R, B extends Request.Request<any, any>, A>(
    ...
    new RequestResolverImpl<B, R>(
      (requests) => fiberRefLocally(use.runAll(requests), self, value),
...
export const requestBlockLocally = <A>(
  ...
  blockedRequests_.reduce(self, LocallyReducer(ref, value))
...
export const fiberRefLocally: {
  <A>(
    self: FiberRef.FiberRef<A>,
    value: A
  ): <B, E, R>(use: Effect.Effect<B, E, R>) => Effect.Effect<B, E, R>
  <B, E, R, A>(
    use: Effect.Effect<B, E, R>,
    self: FiberRef.FiberRef<A>,
    value: A
  ): Effect.Effect<B, E, R>
} = dual(
...
export const fiberRefUnsafeMakePatch = <Value, Patch>(
  ...
  const _fiberRef = {
    ...
    diff: (oldValue: Value, newValue: Value) =>
      options.differ.diff(oldValue, newValue),
    combine: (first: Patch, second: Patch) =>
      options.differ.combine(first, second),
    patch: (patch: Patch) => (oldValue: Value) =>
      options.differ.patch(patch, oldValue),
    fork: options.fork,
...
/** @internal */
export const currentContext: FiberRef.FiberRef<Context.Context<never>> =
  globalValue(Symbol.for("effect/FiberRef/currentContext"), () =>
...
/** @internal */
export const currentLogAnnotations: FiberRef.FiberRef<
  HashMap.HashMap<string, unknown>
> = globalValue(Symbol.for("effect/FiberRef/currentLogAnnotation"), () =>
  fiberRefUnsafeMake(HashMap.empty())
)

/** @internal */
export const currentLogLevel: FiberRef.FiberRef<LogLevel.LogLevel> =
  globalValue(Symbol.for("effect/FiberRef/currentLogLevel"), () =>
    fiberRefUnsafeMake<LogLevel.LogLevel>(logLevelInfo)
  )

/** @internal */
export const currentLogSpan: FiberRef.FiberRef<List.List<LogSpan.LogSpan>> =
  globalValue(Symbol.for("effect/FiberRef/currentLogSpan"), () =>
...
>(2, (self, scheduler) =>
  fiberRefLocally(self, currentMaxOpsBeforeYield, scheduler)
)

/** @internal */
export const currentConcurrency: FiberRef.FiberRef<"unbounded" | number> =
  globalValue(Symbol.for("effect/FiberRef/currentConcurrency"), () =>
...
/** @internal */
export const currentUnhandledErrorLogLevel: FiberRef.FiberRef<
...
/** @internal */
export const currentVersionMismatchErrorLogLevel: FiberRef.FiberRef<
  Option.Option<LogLevel.LogLevel>
> = globalValue(
...
>(2, (self, level) =>
  fiberRefLocally(self, currentUnhandledErrorLogLevel, level)
)

/** @internal */
export const currentMetricLabels: FiberRef.FiberRef<
  ReadonlyArray<MetricLabel.MetricLabel>
> = globalValue(Symbol.for("effect/FiberRef/currentMetricLabels"), () =>
...
export const metricLabels: Effect.Effect<
  ReadonlyArray<MetricLabel.MetricLabel>
> = fiberRefGet(currentMetricLabels)

/** @internal */
export const currentForkScopeOverride: FiberRef.FiberRef<
  Option.Option<fiberScope.FiberScope>
> = globalValue(Symbol.for("effect/FiberRef/currentForkScopeOverride"), () =>
...
/** @internal */
export const currentInterruptedCause: FiberRef.FiberRef<Cause.Cause<never>> =
  globalValue(Symbol.for("effect/FiberRef/currentInterruptedCause"), () =>
...
/** @internal */
export const currentTracerTimingEnabled: FiberRef.FiberRef<boolean> =
  globalValue(Symbol.for("effect/FiberRef/currentTracerTiming"), () =>
    fiberRefUnsafeMake(true)
  )

/** @internal */
export const currentTracerSpanAnnotations: FiberRef.FiberRef<
  HashMap.HashMap<string, unknown>
> = globalValue(
...
/** @internal */
export const currentTracerSpanLinks: FiberRef.FiberRef<
  Chunk.Chunk<Tracer.SpanLink>
> = globalValue(Symbol.for("effect/FiberRef/currentTracerSpanLinks"), () =>
...
/** @internal */
export const ScopeTypeId: Scope.ScopeTypeId = Symbol.for(
  "effect/Scope"
) as Scope.ScopeTypeId
  ...
  switch (option._tag) {
    case "None": {
      return pipe(
        ...
        Option.match({
          onNone: () => {
            const interrupts = Arr.fromIterable(
              internalCause.interruptors(self)
            ).flatMap((fiberId) =>
...
export const isRuntimeException = (u: unknown): u is Cause.RuntimeException =>
  hasProperty(u, RuntimeExceptionTypeId)

/** @internal */
export const InterruptedExceptionTypeId: Cause.InterruptedExceptionTypeId =
  Symbol.for(
...
/** @internal */
export const isInterruptedException = (
  u: unknown
): u is Cause.InterruptedException => hasProperty(u, InterruptedExceptionTypeId)

/** @internal */
export const IllegalArgumentExceptionTypeId: Cause.IllegalArgumentExceptionTypeId =
  Symbol.for(
    "effect/Cause/errors/IllegalArgument"
  ) as Cause.IllegalArgumentExceptionTypeId

/** @internal */
export const IllegalArgumentException =
  makeException<Cause.IllegalArgumentException>(
    {
...
/** @internal */
export const isIllegalArgumentException = (
  u: unknown
): u is Cause.IllegalArgumentException =>
  hasProperty(u, IllegalArgumentExceptionTypeId)

/** @internal */
export const NoSuchElementExceptionTypeId: Cause.NoSuchElementExceptionTypeId =
  Symbol.for(
    "effect/Cause/errors/NoSuchElement"
  ) as Cause.NoSuchElementExceptionTypeId

/** @internal */
export const NoSuchElementException =
  makeException<Cause.NoSuchElementException>(
    {
...
/** @internal */
export const isNoSuchElementException = (
  u: unknown
): u is Cause.NoSuchElementException =>
  hasProperty(u, NoSuchElementExceptionTypeId)

/** @internal */
export const InvalidPubSubCapacityExceptionTypeId: Cause.InvalidPubSubCapacityExceptionTypeId =
  Symbol.for(
    "effect/Cause/errors/InvalidPubSubCapacityException"
  ) as Cause.InvalidPubSubCapacityExceptionTypeId

/** @internal */
export const InvalidPubSubCapacityException =
  makeException<Cause.InvalidPubSubCapacityException>(
    {
...
/** @internal */
export const ExceededCapacityExceptionTypeId: Cause.ExceededCapacityExceptionTypeId =
  Symbol.for(
    "effect/Cause/errors/ExceededCapacityException"
  ) as Cause.ExceededCapacityExceptionTypeId

/** @internal */
export const ExceededCapacityException =
  makeException<Cause.ExceededCapacityException>(
    {
...
/** @internal */
export const isExceededCapacityException = (
  u: unknown
): u is Cause.ExceededCapacityException =>
  hasProperty(u, ExceededCapacityExceptionTypeId)

/** @internal */
export const isInvalidCapacityError = (
  u: unknown
): u is Cause.InvalidPubSubCapacityException =>
...
/** @internal */
export const timeoutExceptionFromDuration = (
  duration: Duration.DurationInput
): Cause.TimeoutException =>
  new TimeoutException(
    `Operation timed out after '${Duration.format(duration)}'`
  )

/** @internal */
export const isTimeoutException = (u: unknown): u is Cause.TimeoutException =>
  hasProperty(u, TimeoutExceptionTypeId)
...
) => Cause.UnknownException = (function () {
...
/** @internal */
export const isUnknownException = (u: unknown): u is Cause.UnknownException =>
  hasProperty(u, UnknownExceptionTypeId)
...
/** @internal */
export const exitCauseOption = <A, E>(
  self: Exit.Exit<A, E>
...
export const exitCollectAll = <A, E>(
  ...
  exitCollectAllInternal(
    exits,
    options?.parallel ? internalCause.parallel : internalCause.sequential
  )
...
export const exitExists: {
  ...
  ): <E>(self: Exit.Exit<A, E>) => self is Exit.Exit<B>
...
export const exitFlatMap = dual<
  <A, A2, E2>(
    f: (a: A) => Exit.Exit<A2, E2>
  ) => <E>(self: Exit.Exit<A, E>) => Exit.Exit<A2, E | E2>,
  <A, E, E2, A2>(
    self: Exit.Exit<A, E>,
    f: (a: A) => Exit.Exit<A2, E2>
  ) => Exit.Exit<A2, E | E2>
...
/** @internal */
export const exitFromEither = <R, L>(
...
export const exitGetOrElse = dual<
  <E, A2>(
    orElse: (cause: Cause.Cause<E>) => A2
  ) => <A>(self: Exit.Exit<A, E>) => A | A2,
  <A, E, A2>(
    self: Exit.Exit<A, E>,
    orElse: (cause: Cause.Cause<E>) => A2
  ) => A | A2
  ...
  switch (self._tag) {
    case OpCodes.OP_FAILURE:
      return exitFailCause(
        pipe(self.effect_instruction_i0, internalCause.map(onFailure))
      )
    case OpCodes.OP_SUCCESS:
...
>(2, (self, f) => {
  switch (self._tag) {
    case OpCodes.OP_FAILURE:
      return exitFailCause(
        pipe(self.effect_instruction_i0, internalCause.map(f))
      )
    case OpCodes.OP_SUCCESS:
...
export const exitZip = dual<
  ...
  ) => <A, E>(self: Exit.Exit<A, E>) => Exit.Exit<[A, A2], E | E2>,
  <A, E, A2, E2>(
    self: Exit.Exit<A, E>,
    that: Exit.Exit<A2, E2>
  ) => Exit.Exit<[A, A2], E | E2>
...
export const exitZipLeft = dual<
  <A2, E2>(
    that: Exit.Exit<A2, E2>
  ) => <A, E>(self: Exit.Exit<A, E>) => Exit.Exit<A, E | E2>,
  <A, E, A2, E2>(
    self: Exit.Exit<A, E>,
    that: Exit.Exit<A2, E2>
  ) => Exit.Exit<A, E | E2>
...
>(3, (self, that, { onFailure, onSuccess }) => {
  switch (self._tag) {
    case OpCodes.OP_FAILURE: {
      switch (that._tag) {
        case OpCodes.OP_SUCCESS:
...
const exitCollectAllInternal = <A, E>(
  exits: Iterable<Exit.Exit<A, E>>,
  combineCauses: (
    causeA: Cause.Cause<E>,
    causeB: Cause.Cause<E>
  ) => Cause.Cause<E>
): Option.Option<Exit.Exit<Array<A>, E>> => {
...
/** @internal */
export const deferredUnsafeMake = <A, E = never>(
  fiberId: FiberId.FiberId
...
/* @internal */
export const deferredMake = <A, E = never>(): Effect.Effect<
  Deferred.Deferred<A, E>
> => flatMap(fiberId, (id) => deferredMakeAs<A, E>(id))

/* @internal */
export const deferredMakeAs = <A, E = never>(
  fiberId: FiberId.FiberId
): Effect.Effect<Deferred.Deferred<A, E>> =>
  sync(() => deferredUnsafeMake<A, E>(fiberId))

/* @internal */
export const deferredAwait = <A, E>(
...
export const deferredComplete: {
  ...
  ): (self: Deferred.Deferred<A, E>) => Effect.Effect<boolean>
  <A, E>(
    self: Deferred.Deferred<A, E>,
    effect: Effect.Effect<A, E>
  ...
  <A, E>(
    self: Deferred.Deferred<A, E>,
    effect: Effect.Effect<A, E>
  ): Effect.Effect<boolean> => intoDeferred(effect, self)
)
...
export const deferredFailSync = dual<
  <E>(
    evaluate: LazyArg<E>
  ) => <A>(self: Deferred.Deferred<A, E>) => Effect.Effect<boolean>,
  <A, E>(
    self: Deferred.Deferred<A, E>,
    evaluate: LazyArg<E>
  ) => Effect.Effect<boolean>
...
export const deferredDie = dual<
  (
    defect: unknown
  ) => <A, E>(self: Deferred.Deferred<A, E>) => Effect.Effect<boolean>,
  <A, E>(
    self: Deferred.Deferred<A, E>,
    defect: unknown
  ) => Effect.Effect<boolean>
...
/* @internal */
export const deferredInterrupt = <A, E>(
  self: Deferred.Deferred<A, E>
): Effect.Effect<boolean> =>
  flatMap(fiberId, (fiberId) =>
...
/* @internal */
export const deferredIsDone = <A, E>(
  self: Deferred.Deferred<A, E>
): Effect.Effect<boolean> =>
...
export const deferredSync = dual<
  <A>(
    evaluate: LazyArg<A>
  ) => <E>(self: Deferred.Deferred<A, E>) => Effect.Effect<boolean>,
  <A, E>(
    self: Deferred.Deferred<A, E>,
    evaluate: LazyArg<A>
  ) => Effect.Effect<boolean>
...
/** @internal */
export const deferredUnsafeDone = <A, E>(
  self: Deferred.Deferred<A, E>,
  effect: Effect.Effect<A, E>
): void => {
  const state = MutableRef.get(self.state)
...
const constContext = withFiberRuntime((fiber) =>
  exitSucceed(fiber.currentContext)
)

/* @internal */
export const context = <R>(): Effect.Effect<Context.Context<R>, never, R> =>
  constContext as any
...
>(2, <A, E, R>(self: Effect.Effect<A, E, R>, context: Context.Context<R>) =>
  fiberRefLocally(currentContext, context)(self as Effect.Effect<A, E>)
)
...
export const provideSomeContext = dual<
  ...
  ) => Effect.Effect<A, E, Exclude<R1, R>>,
  <A, E, R1, R>(
    self: Effect.Effect<A, E, R1>,
    context: Context.Context<R>
  ) => Effect.Effect<A, E, Exclude<R1, R>>
>(
  2,
  <A, E, R1, R>(self: Effect.Effect<A, E, R1>, context: Context.Context<R>) =>
...
>(
  ...
  <A, E, R, R2>(
    ...
    contextWithEffect((context: Context.Context<R2>) =>
      provideContext(self, f(context))
    )
)
...
/** @internal */
export const currentSpanFromFiber = <A, E>(
