...
export const makeWith = <A, E, R>(options: {
  ...
  core.uninterruptibleMask((restore) =>
    core.flatMap(core.context<R | Scope>(), (context) => {
      ...
      const acquire = core.mapInputContext(options.acquire, (input) =>
      ...
      const initialize = core.tap(
        fiberRuntime.forkDaemon(restore(pool.resize)),
        (fiber) => scope.addFinalizer(() => core.interruptFiber(fiber))
      )
      const runStrategy = core.tap(
        fiberRuntime.forkDaemon(restore(options.strategy.run(pool))),
        (fiber) => scope.addFinalizer(() => core.interruptFiber(fiber))
      )
      return core
        .succeed(pool)
        .pipe(
          core.zipLeft(scope.addFinalizer(() => pool.shutdown)),
...
export const invalidate: {
  ...
  2,
  <A, E>(self: Pool<A, E>, item: A): Effect<void> => self.invalidate(item)
)
...
interface Strategy<A, E> {
  ...
  readonly reclaim: (
    pool: PoolImpl<A, E>
  ) => Effect<Option.Option<PoolItem<A, E>>>
}

class PoolImpl<A, E>
  extends Effectable.Class<A, E, Scope>
  implements Pool<A, E>
{
  ...
  readonly get: Effect<A, E, Scope> = core.flatMap(
    core.suspend(() =>
      this.isShuttingDown ? core.interrupt : this.getPoolItem
    ),
    (_) => _.exit
  )
  ...
  get shutdown(): Effect<void> {
    return core.suspend(() => {
      ...
      return core
        .forEachSequentialDiscard(this.items, (item) => {
          if (item.refCount > 0) {
...
const reportUnhandledError = <E>(cause: Cause<E>) =>
  core.withFiberRuntime<void>((fiber) => {
    const unhandledLogLevel = fiber.getFiberRef(
