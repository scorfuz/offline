...
/** @internal */
export const DifferTypeId: Differ.TypeId = Symbol.for(
  "effect/Differ"
) as Differ.TypeId
...
/** @internal */
export const environment = <A>(): Differ.Differ<
  Context<A>,
  Differ.Differ.Context.Patch<A, A>
> =>
  make({
...
> =>
  make({
    ...
    diff: (oldValue, newValue) =>
      HashMapPatch.diff({ oldValue, newValue, differ }),
    patch: (patch, oldValue) => HashMapPatch.patch(oldValue, differ)(patch)
  })

/** @internal */
export const hashSet = <Value>(): Differ.Differ<
  HashSet<Value>,
  Differ.Differ.HashSet.Patch<Value>
> =>
  make({
...
export const orElseEither = Dual.dual<
  <Value2, Patch2>(
    ...
    Either<Value2, Value>,
    Differ.Differ.Or.Patch<Value, Value2, Patch, Patch2>
  >,
...
> =>
  make({
    ...
    diff: (oldValue, newValue) =>
      ReadonlyArrayPatch.diff({ oldValue, newValue, differ }),
    patch: (patch, oldValue) =>
      ReadonlyArrayPatch.patch(patch, oldValue, differ)
  })
...
/** @internal */
export const update = <A>(): Differ.Differ<A, (a: A) => A> =>
  updateWith((_, a) => a)

/** @internal */
export const updateWith = <A>(
  f: (x: A, y: A) => A
): Differ.Differ<A, (a: A) => A> =>
  make({
    empty: identity,
