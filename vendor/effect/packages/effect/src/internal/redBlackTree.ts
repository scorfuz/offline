...
/** @internal */
export const RedBlackTreeTypeId: RBT.TypeId = Symbol.for(
  RedBlackTreeSymbolKey
) as RBT.TypeId

/** @internal */
export interface RedBlackTreeImpl<in out K, out V> extends RBT.RedBlackTree<
  K,
  V
> {
...
const RedBlackTreeProto: RBT.RedBlackTree<unknown, unknown> = {
  ...
  [Equal.symbol]<K, V>(this: RedBlackTreeImpl<K, V>, that: unknown): boolean {
    if (isRedBlackTree(that)) {
      if (
        (this._root?.count ?? 0) !==
        ((that as RedBlackTreeImpl<K, V>)._root?.count ?? 0)
      ) {
        return false
      }
...
const makeImpl = <K, V>(
  ord: Order.Order<K>,
  root: Node.Node<K, V> | undefined
): RedBlackTreeImpl<K, V> => {
...
export const isRedBlackTree: {
  ...
  hasProperty(u, RedBlackTreeTypeId)

/** @internal */
export const empty = <K, V = never>(
  ord: Order.Order<K>
): RBT.RedBlackTree<K, V> => makeImpl<K, V>(ord, undefined)
...
/** @internal */
export const first = <K, V>(
  self: RBT.RedBlackTree<K, V>
): Option.Option<[K, V]> => {
  let node: Node.Node<K, V> | undefined = (self as RedBlackTreeImpl<K, V>)._root
  let current: Node.Node<K, V> | undefined = (self as RedBlackTreeImpl<K, V>)
    ._root
  while (node !== undefined) {
    current = node
    node = node.left
...
export const getAt = dual<
  (
    index: number
  ) => <K, V>(self: RBT.RedBlackTree<K, V>) => Option.Option<[K, V]>,
...
export const insert = dual<
  <K, V>(
    key: K,
    value: V
  ) => (self: RBT.RedBlackTree<K, V>) => RBT.RedBlackTree<K, V>,
  <K, V>(
    self: RBT.RedBlackTree<K, V>,
    key: K,
    value: V
...
/** @internal */
export const keysForward = <K, V>(
  self: RBT.RedBlackTree<K, V>
): IterableIterator<K> => keys(self, Direction.Forward)

/** @internal */
export const keysBackward = <K, V>(
  self: RBT.RedBlackTree<K, V>
): IterableIterator<K> => keys(self, Direction.Backward)

const keys = <K, V>(
  self: RBT.RedBlackTree<K, V>,
  direction: RBT.RedBlackTree.Direction
): IterableIterator<K> => {
  const begin: RedBlackTreeIterator<K, V> = self[
    Symbol.iterator
  ]() as RedBlackTreeIterator<K, V>
  let count = 0
  return {
...
/** @internal */
export const valuesForward = <K, V>(
  self: RBT.RedBlackTree<K, V>
): IterableIterator<V> => values(self, Direction.Forward)

/** @internal */
export const valuesBackward = <K, V>(
  self: RBT.RedBlackTree<K, V>
): IterableIterator<V> => values(self, Direction.Backward)
