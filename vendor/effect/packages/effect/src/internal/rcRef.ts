...
class RcRefImpl<A, E>
  extends Effectable.Class<A, E, Scope.Scope>
  implements RcRef.RcRef<A, E>
{
  ...
  core.withFiberRuntime<RcRef.RcRef<A, E>, never, R | Scope.Scope>((fiber) => {
    ...
    const ref = new RcRefImpl<A, E>(
      ...
      options.idleTimeToLive
        ? Duration.decode(options.idleTimeToLive)
        : undefined
    )
    return core.as(
      scope.addFinalizer(() =>
        ref.semaphore.withPermits(1)(
          core.suspend(() => {
            const close =
...
export const get = <A, E>(
  ...
  const isInfinite =
    self.idleTimeToLive && !Duration.isFinite(self.idleTimeToLive)
  return core
          ...
          core.suspend(() => {
            ...
            return coreEffect.sleep(self.idleTimeToLive).pipe(
              ...
              core.zipRight(
                core.suspend(() => {
                  if (
                    self.state._tag === "Acquired" &&
                    self.state.refCount === 0
                  ) {
                    self.state = stateEmpty
                    return core.scopeClose(state.scope, core.exitVoid)
                  }
...
/** @internal */
export const invalidate = <A, E>(self_: RcRef.RcRef<A, E>): Effect<void> => {
  const self = self_ as RcRefImpl<A, E>
