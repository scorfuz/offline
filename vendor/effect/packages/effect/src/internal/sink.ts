...
export const isSink = (
  u: unknown
): u is Sink.Sink<unknown, unknown, unknown, unknown, unknown> =>
...
/** @internal */
export const collectAll = <In>(): Sink.Sink<Chunk.Chunk<In>, In> =>
...
> =>
  core.readWithCause({
    onInput: (chunk: Chunk.Chunk<In>) =>
      collectAllLoop(pipe(acc, Chunk.appendAll(chunk))),
    onFailure: core.failCause,
    onDone: () => core.succeed(acc)
  })

/** @internal */
export const collectAllN = <In>(
  n: number
): Sink.Sink<Chunk.Chunk<In>, In, In> =>
  ...
  core.readWithCause({
    onInput: (chunk: Chunk.Chunk<In>) => {
      ...
      if (collected.length < n) {
        return collectAllNLoop(
          n - collected.length,
          Chunk.appendAll(acc, collected)
        )
      }
      if (Chunk.isEmpty(leftovers)) {
        return core.succeed(Chunk.appendAll(acc, collected))
      }
      return core.flatMap(core.write(leftovers), () =>
        core.succeed(Chunk.appendAll(acc, collected))
      )
    },
    onFailure: core.failCause,
    onDone: () => core.succeed(acc)
...
export const collectAllToSet = <In>(): Sink.Sink<HashSet.HashSet<In>, In> =>
  foldLeftChunks<HashSet.HashSet<In>, In>(HashSet.empty(), (acc, chunk) =>
    pipe(
      chunk,
      Chunk.reduce(acc, (acc, input) => pipe(acc, HashSet.add(input)))
    )
  )
...
export const collectAllToSetN = <In>(
  n: number
): Sink.Sink<HashSet.HashSet<In>, In, In> =>
  ...
  return pipe(
    foldEffect<[Chunk.Chunk<In>, boolean], In, E, R>(
      ...
      ([chunk, _], input) =>
        pipe(
          p(input),
          Effect.map((bool) => [pipe(chunk, Chunk.append(input)), !bool])
        )
    ),
    map((tuple) => tuple[0])
  )
...
export const collectAllWhile: {
  ...
  ): Sink.Sink<Chunk.Chunk<Out>, In, In>
...
> =>
  core.readWith({
    onInput: (input: Chunk.Chunk<In>) => {
      const [collected, leftovers] = pipe(
...
): Sink.Sink<Chunk.Chunk<In>, In, In, E, R> =>
  fromChannel(collectAllWhileEffectReader(predicate, Chunk.empty()))
        ...
        core.flatMap((collected) => {
          ...
          if (Chunk.isEmpty(leftovers)) {
            return collectAllWhileEffectReader(
              predicate,
              pipe(done, Chunk.appendAll(collected))
            )
          }
          return pipe(
            core.write(leftovers),
            channel.zipRight(
              core.succeed(pipe(done, Chunk.appendAll(collected)))
            )
          )
        })
      ),
...
export const collectAllWhileWith: {
  ...
  <A, In, L extends In, E, R, S>(
    ...
    const refs = pipe(Ref.make(Chunk.empty<In>()), Effect.zip(Ref.make(false)))
      ...
      core.flatMap(([leftoversRef, upstreamDoneRef]) => {
        ...
        > = core.readWith({
          onInput: (input) =>
            pipe(
              core.write(input),
              core.flatMap(() => upstreamMarker)
            ),
          onFailure: core.fail,
          onDone: (done) =>
            pipe(
              core.fromEffect(Ref.set(upstreamDoneRef, true)),
              channel.as(done)
            )
        })
        return pipe(
        ...
        p(doneValue)
          ? pipe(
              core.fromEffect(
                Ref.set(
                  leftoversRef,
                  Chunk.flatten(leftovers as Chunk.Chunk<Chunk.Chunk<In>>)
                )
              ),
              core.flatMap(() =>
                pipe(
                  ...
                  core.flatMap((upstreamDone) => {
                    ...
                    return upstreamDone
                      ? pipe(
                          core.write(Chunk.flatten(leftovers)),
                          channel.as(accumulatedResult)
                        )
                      : collectAllWhileWithLoop(
                          self,
                          leftoversRef,
...
export const collectLeftover = <A, In, L, E, R>(
  ...
  new SinkImpl(
    pipe(
      ...
      channel.map(([chunks, z]) => [z, Chunk.flatten(chunks)])
    )
  )
...
>(
  ...
  <A, In, L, E, R, In0>(
    self: Sink.Sink<A, In, L, E, R>,
    f: (input: In0) => In
  ): Sink.Sink<A, In0, L, E, R> => pipe(self, mapInputChunks(Chunk.map(f)))
)
...
>(
  ...
  <A, In, L, E, R, In0, E2, R2>(
    ...
    mapInputChunksEffect(self, (chunk) =>
      Effect.map(
...
/** @internal */
export const die = (defect: unknown): Sink.Sink<never, unknown> =>
...
): Sink.Sink<never, unknown> => failCauseSync(() => Cause.die(evaluate()))
...
>(
  ...
  <A, In, L, E, R, In0, A2>(
    ...
    map(mapInput(self, options.onInput), options.onDone)
)
...
>(2, (self, options) =>
  mapEffect(mapInputEffect(self, options.onInput), options.onDone)
)
...
>(2, (self, options) =>
  map(mapInputChunks(self, options.onInput), options.onDone)
)
...
/** @internal */
export const drop = <In>(n: number): Sink.Sink<unknown, In, In> =>
  ...
  core.readWith({
    onInput: (input: Chunk.Chunk<In>) => {
      ...
      return pipe(
        core.write(dropped),
        channel.zipRight(
          channel.identityChannel<Chunk.Chunk<In>, never, unknown>()
        )
      )
    },
    onFailure: core.fail,
...
export const dropUntil = <In>(
  predicate: Predicate<In>
): Sink.Sink<unknown, In, In> =>
  new SinkImpl(
    pipe(
      toChannel(dropWhile((input: In) => !predicate(input))),
      channel.pipeToOrFail(toChannel(drop<In>(1)))
    )
  )
...
export const dropUntilEffect = <In, E, R>(
  predicate: (input: In) => Effect.Effect<boolean, E, R>
): Sink.Sink<unknown, In, In, E, R> =>
  suspend(() => new SinkImpl(dropUntilEffectReader(predicate)))
        ...
        Effect.map((leftover) => {
          ...
          return more
            ...
            : pipe(
                ...
                channel.zipRight(
                  channel.identityChannel<Chunk.Chunk<In>, E, unknown>()
                )
              )
        }),
        channel.unwrap
...
export const dropWhile = <In>(
  predicate: Predicate<In>
): Sink.Sink<unknown, In, In> => new SinkImpl(dropWhileReader(predicate))
...
export const dropWhileEffect = <In, E, R>(
  ...
  suspend(() => new SinkImpl(dropWhileEffectReader(predicate)))
...
>(
  2,
  (self, finalizer) =>
    new SinkImpl(pipe(self, toChannel, channel.ensuring(finalizer)))
)
...
>(
  2,
  (self, finalizer) =>
    new SinkImpl(pipe(self, toChannel, core.ensuringWith(finalizer)))
)

/** @internal */
export const context = <R>(): Sink.Sink<
  Context.Context<R>,
  unknown,
  never,
  never,
...
export const contextWithSink = <R0, A, In, L, E, R>(
  ...
  new SinkImpl(
    channel.unwrap(Effect.contextWith((context) => toChannel(f(context))))
  )

/** @internal */
export const every = <In>(
  predicate: Predicate<In>
): Sink.Sink<boolean, In, In> =>
  fold(true, identity, (acc, input) => acc && predicate(input))

/** @internal */
export const fail = <E>(e: E): Sink.Sink<never, unknown, never, E> =>
  new SinkImpl(core.fail(e))

/** @internal */
export const failSync = <E>(
  evaluate: LazyArg<E>
): Sink.Sink<never, unknown, never, E> => new SinkImpl(core.failSync(evaluate))

/** @internal */
export const failCause = <E>(
  cause: Cause.Cause<E>
): Sink.Sink<never, unknown, never, E> => new SinkImpl(core.failCause(cause))

/** @internal */
export const failCauseSync = <E>(
  evaluate: LazyArg<Cause.Cause<E>>
): Sink.Sink<never, unknown, never, E> =>
...
export const filterInput: {
  ...
  <In, In1 extends In>(
    f: Predicate<In1>
  ): <A, L, E, R>(self: Sink.Sink<A, In, L, E, R>) => Sink.Sink<A, In1, L, E, R>
} = <In, In1 extends In>(f: Predicate<In1>) => {
  return <A, L, E, R>(
    self: Sink.Sink<A, In, L, E, R>
  ): Sink.Sink<A, In1, L, E, R> => pipe(self, mapInputChunks(Chunk.filter(f)))
}
...
>(2, (self, f) =>
  mapInputChunksEffect(self, (chunk) =>
    Effect.map(Effect.filter(chunk, f), Chunk.unsafeFromArray)
  )
)
...
>(
  ...
  <A, In, L extends In, E, R, E2, R2>(
    ...
    const newChannel = pipe(
      core.fromEffect(
        pipe(Ref.make(Chunk.empty<In>()), Effect.zip(Ref.make(false)))
      ),
      core.flatMap(([leftoversRef, upstreamDoneRef]) => {
        const upstreamMarker: Channel.Channel<
          Chunk.Chunk<In>,
          ...
          onSuccess: ([leftovers, doneValue]) =>
            pipe(
              ...
              core.flatMap((satisfied) =>
                pipe(
                  core.fromEffect(
                    Ref.set(leftoversRef, Chunk.flatten(leftovers))
                  ),
                  channel.zipRight(
                    pipe(
                      core.fromEffect(Ref.get(upstreamDoneRef)),
                      core.flatMap((upstreamDone) => {
                        if (satisfied) {
                          return pipe(
                            core.write(Chunk.flatten(leftovers)),
                            channel.as(Option.some(doneValue))
                          )
                        }
                        if (upstreamDone) {
                          return pipe(
                            core.write(Chunk.flatten(leftovers)),
                            channel.as(Option.none())
                          )
                        }
                        return loop
                      })
...
> => {
  ...
  return core.readWith({
    onInput: (input: Chunk.Chunk<In>) => {
      const [nextS, leftovers] = foldChunkSplit(
        s,
        input,
        contFn,
        f,
        ...
        onSuccess: ([leftovers, z]) =>
          core.suspend(() => {
            const leftoversRef = {
              ...
              ) as Chunk.Chunk<Chunk.Chunk<L1 | L2>>
            }
            const refReader = pipe(
              ...
              core.flatMap((chunk) =>
                channel.writeChunk(chunk as Chunk.Chunk<Chunk.Chunk<In1 & In2>>)
              )
            )
...
export const foldChunks = <S, In>(
  ...
  suspend(() => new SinkImpl(foldChunksReader(s, contFn, f)))
...
const foldChunksReader = <S, In>(
  ...
  return core.readWith({
    onInput: (input: Chunk.Chunk<In>) =>
      foldChunksReader(f(s, input), contFn, f),
    onFailure: core.fail,
    onDone: () => core.succeedNow(s)
  })
}
...
export const foldChunksEffect = <S, In, E, R>(
  ...
  suspend(() => new SinkImpl(foldChunksEffectReader(s, contFn, f)))
...
export const foldEffect = <S, In, E, R>(
  ...
  suspend(() => new SinkImpl(foldEffectReader(s, contFn, f)))
        ...
        core.flatMap(([nextS, leftovers]) =>
          pipe(
            ...
            Option.match({
              ...
              onSome: (leftover) =>
                pipe(core.write(leftover), channel.as(nextS))
            })
          )
        )
      ),
...
/** @internal */
export const foldLeft = <S, In>(
  s: S,
  f: (s: S, input: In) => S
): Sink.Sink<S, In> => ignoreLeftover(fold(s, constTrue, f))
...
export const foldLeftChunksEffect = <S, In, E, R>(
  s: S,
  f: (s: S, chunk: Chunk.Chunk<In>) => Effect.Effect<S, E, R>
): Sink.Sink<S, In, never, E, R> =>
  ignoreLeftover(foldChunksEffect(s, constTrue, f))
...
export const foldUntilEffect = <S, In, E, R>(
  ...
  pipe(
    foldEffect(
      ...
      (tuple) => tuple[1] < max,
      ([output, count], input: In) =>
        pipe(
          f(output, input),
          Effect.map((s) => [s, count + 1] as const)
        )
    ),
    map((tuple) => tuple[0])
  )
  ...
  core.readWith({
    onInput: (input: Chunk.Chunk<In>) => {
      ...
      if (Chunk.isNonEmpty(leftovers)) {
        return pipe(
          core.write(leftovers),
          channel.zipRight(core.succeedNow(nextS))
        )
      }
      if (cost > max) {
        return core.succeedNow(nextS)
      }
      return foldWeightedDecomposeLoop(
        nextS,
        nextCost,
        nextDirty,
        max,
    ...
    onInput: (input: Chunk.Chunk<In>) =>
      pipe(
        ...
        core.flatMap(([nextS, nextCost, nextDirty, leftovers]) => {
          ...
          return foldWeightedDecomposeEffectLoop(
            nextS,
            max,
            costFn,
            decompose,
    ...
    Effect.flatMap((total) => {
      ...
      return pipe(
        ...
        Effect.flatMap((decomposed) => {
          ...
          const next = pipe(
            decomposed,
            Chunk.appendAll(pipe(input, Chunk.drop(index + 1)))
          )
          return foldWeightedDecomposeEffectFold(
            s,
            max,
            costFn,
            decompose,
...
>(2, (self, f) => foldSink(self, { onFailure: fail, onSuccess: f }))
...
export const forEach = <In, X, E, R>(
  ...
  > = core.readWithCause({
    onInput: (input: Chunk.Chunk<In>) =>
      pipe(
        core.fromEffect(Effect.forEach(input, (v) => f(v), { discard: true })),
...
export const forEachWhile = <In, E, R>(
  ...
  > = core.readWithCause({
    onInput: (input: Chunk.Chunk<In>) =>
      forEachWhileReader(f, input, 0, input.length, process),
    onFailure: core.failCause,
    onDone: () => core.void
  })
  return new SinkImpl(process)
...
> => {
  ...
  return pipe(
    ...
    channel.catchAll((error) =>
      pipe(
        core.write(pipe(input, Chunk.drop(index))),
        channel.zipRight(core.fail(error))
      )
    )
  )
}
...
): Sink.Sink<A, unknown, never, E, R> => new SinkImpl(core.fromEffect(effect))
    ...
    onInput: (input: Chunk.Chunk<In>) =>
      channel.foldChannel(core.fromEffect(push(Option.some(input))), {
        onFailure: ([either, leftovers]) =>
          Either.match(either, {
            onLeft: (error) =>
              pipe(core.write(leftovers), channel.zipRight(core.fail(error))),
            onRight: (z) =>
              pipe(core.write(leftovers), channel.zipRight(core.succeedNow(z)))
          }),
...
export const head = <In>(): Sink.Sink<Option.Option<In>, In, In> =>
  fold(Option.none() as Option.Option<In>, Option.isNone, (option, input) =>
    Option.match(option, {
...
export const ignoreLeftover = <A, In, L, E, R>(
  self: Sink.Sink<A, In, L, E, R>
): Sink.Sink<A, In, never, E, R> => new SinkImpl(channel.drain(toChannel(self)))

/** @internal */
export const last = <In>(): Sink.Sink<Option.Option<In>, In, In> =>
  foldLeftChunks(Option.none<In>(), (s, input) =>
    Option.orElse(Chunk.last(input), () => s)
  )

/** @internal */
export const leftover = <L>(
  chunk: Chunk.Chunk<L>
): Sink.Sink<void, unknown, L> =>
...
>(2, (self, f) => new SinkImpl(pipe(toChannel(self), channel.mapEffect(f))))
...
>(2, (self, f) => new SinkImpl(pipe(toChannel(self), channel.mapError(f))))
...
export const provideContext = dual<
  ...
  ) => <A, In, L, E>(self: Sink.Sink<A, In, L, E, R>) => Sink.Sink<A, In, L, E>,
  <A, In, L, E, R>(
    self: Sink.Sink<A, In, L, E, R>,
    context: Context.Context<R>
  ) => Sink.Sink<A, In, L, E>
...
>(2, (self, that) => pipe(self, raceBoth(that), map(Either.merge)))
...
>(
  ...
  (self, that, options) =>
    raceWith(self, {
      other: that,
      onSelfDone: (selfDone) =>
        mergeDecision.Done(Effect.map(selfDone, Either.left)),
      onOtherDone: (thatDone) =>
        mergeDecision.Done(Effect.map(thatDone, Either.right)),
  ...
  <A, In, L, E, R, A2, In2, L2, E2, R2, A3, A4>(
    ...
    function race(scope: Scope.Scope) {
      return Effect.gen(function* () {
        ...
        >(options?.capacity ?? 16)
        const subscription1 = yield* Scope.extend(
          PubSub.subscribe(pubsub),
          scope
        )
        const subscription2 = yield* Scope.extend(
          PubSub.subscribe(pubsub),
          scope
        )
...
>(2, (self, pf) => pipe(self, refineOrDieWith(pf, identity)))
...
export const serviceWithSink = <I, S, R, E, In, L, Z>(
  ...
  new SinkImpl(
    pipe(
      Effect.map(tag, (service) => toChannel(f(service))),
      channel.unwrap
    )
  )

/** @internal */
export const some = <In>(
  predicate: Predicate<In>
): Sink.Sink<boolean, In, In> =>
  fold(
    false,
    (bool) => !bool,
    (acc, input) => acc || predicate(input)
  )
          ...
          core.flatMap(([leftovers, z]) =>
            pipe(
              ...
              core.flatMap((leftover) =>
                pipe(
                  core.write<Chunk.Chunk<In>>(
                    pipe(leftover, Chunk.appendAll(Chunk.flatten(leftovers)))
                  ),
...
/** @internal */
export const succeed = <A>(a: A): Sink.Sink<A, unknown> =>
  new SinkImpl(core.succeed(a))
...
/** @internal */
export const sync = <A>(evaluate: LazyArg<A>): Sink.Sink<A, unknown> =>
...
export const take = <In>(n: number): Sink.Sink<Chunk.Chunk<In>, In, In> =>
  pipe(
    ...
    flatMap((acc) => {
      ...
      return new SinkImpl(
        pipe(core.write(leftover), channel.zipRight(core.succeedNow(taken)))
      )
    })
  )
...
export const unwrapScoped = <A, In, L, E, R>(
  ...
  new SinkImpl(
    channel.unwrapScoped(effect.pipe(Effect.map((sink) => toChannel(sink))))
  )
...
export const unwrapScopedWith = <A, In, L, E, R>(
  ...
  new SinkImpl(
    channel.unwrapScopedWith((scope) =>
      f(scope).pipe(Effect.map((sink) => toChannel(sink)))
    )
  )
...
export const withDuration = <A, In, L, E, R>(
  ...
  pipe(
    self,
    summarized(Clock.currentTimeMillis, (start, end) =>
...
>(
  ...
  <A, In, L, E, R, A2, In2 extends In, L2, E2, R2>(
    ...
    zipWith(self, that, (z, z2) => [z, z2], options)
)
...
>(
  ...
  <A, In, L, E, R, A2, In2 extends In, L2, E2, R2>(
    ...
    zipWith(self, that, (z, _) => z, options)
)
...
>(
  ...
  <A, In, L, E, R, A2, In2 extends In, L2, E2, R2>(
    ...
    zipWith(self, that, (_, z2) => z2, options)
)
