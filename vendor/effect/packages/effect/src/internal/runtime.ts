  ...
  <R, A, E>(
    ...
    const fiberRefUpdates: ReadonlyArray.NonEmptyArray<
      readonly [
        ...
        ReadonlyArray.NonEmptyReadonlyArray<readonly [FiberId.Runtime, any]>
      ]
    > = [[core.currentContext, [[fiberId, runtime.context]]]]

    if (options?.scheduler) {
      fiberRefUpdates.push([
        scheduler_.currentScheduler,
    ...
    const fiberRuntime: FiberRuntime.FiberRuntime<A, E> =
      new FiberRuntime.FiberRuntime<A, E>(
        fiberId,
        fiberRefs,
            ...
            core.scopeAddFinalizer(
              ...
              core.fiberIdWith((id) =>
                equals(id, fiberRuntime.id())
                  ? core.void
                  : core.interruptAsFiber(fiberRuntime, id)
              )
            ),
...
class AsyncFiberExceptionImpl<A, E = never>
  extends Error
  implements Runtime.AsyncFiberException<A, E>
{
...
const asyncFiberException = <A, E>(
  fiber: Fiber.RuntimeFiber<A, E>
): Runtime.AsyncFiberException<A, E> => {
...
export const isAsyncFiberException = (
  u: unknown
): u is Runtime.AsyncFiberException<unknown, unknown> =>
  Predicate.isTagged(u, "AsyncFiberException") && "fiber" in u

/** @internal */
export const FiberFailureId: Runtime.FiberFailureId = Symbol.for(
...
class FiberFailureImpl extends Error implements Runtime.FiberFailure {
  ...
  toString(): string {
    return (
      "(FiberFailure) " +
      InternalCause.pretty(this[FiberFailureCauseId], {
...
/** @internal */
export const fiberFailure = <E>(
  cause: Cause.Cause<E>
): Runtime.FiberFailure => {
  const limit = Error.stackTraceLimit
  Error.stackTraceLimit = 0
...
export const isFiberFailure = (u: unknown): u is Runtime.FiberFailure =>
  Predicate.hasProperty(u, FiberFailureId)

const fastPath = <A, E, R>(
  effect: Effect.Effect<A, E, R>
...
export const unsafeRunSyncExit: {
  ...
  return core.exitDie(
    core.capture(
      asyncFiberException(fiberRuntime),
      core.currentSpanFromFiber(fiberRuntime)
    )
  )
})
...
export const runtime = <R>(): Effect.Effect<Runtime.Runtime<R>, never, R> =>
  core.withFiberRuntime((state, status) =>
    core.succeed(
      new RuntimeImpl<R>(
        state.getFiberRef(
          core.currentContext as unknown as FiberRef.FiberRef<
            Context.Context<R>
          >
        ),
        status.runtimeFlags,
...
} = dual(2, <R>(self: Runtime.Runtime<R>, flag: RuntimeFlags.RuntimeFlag) =>
  updateRuntimeFlags(self, runtimeFlags.disable(flag))
)
...
export const enableRuntimeFlag: {
  ...
  updateRuntimeFlags(self, runtimeFlags.enable(flag))
)
...
export const updateContext: {
  ...
  ): (self: Runtime.Runtime<R>) => Runtime.Runtime<R2>
  <R, R2>(
    self: Runtime.Runtime<R>,
    f: (context: Context.Context<R>) => Context.Context<R2>
  ): Runtime.Runtime<R2>
...
export const provideService: {
  <I, S>(
    tag: Context.Tag<I, S>,
    service: S
  ): <R>(self: Runtime.Runtime<R>) => Runtime.Runtime<R | I>
  <R, I, S>(
    self: Runtime.Runtime<R>,
    tag: Context.Tag<I, S>,
    service: S
...
export const deleteFiberRef: {
  ...
  <R, A>(
    self: Runtime.Runtime<R>,
    fiberRef: FiberRef.FiberRef<A>
  ): Runtime.Runtime<R> => updateFiberRefs(self, FiberRefs.delete(fiberRef))
)
...
export const asyncEffect = <A, E, R, R3, E2, R2>(
  ...
  core.suspend(() => {
    ...
    return core.flatMap(core.deferredMake<A, E | E2>(), (deferred) =>
            ...
            FiberRuntime.fork(
              restore(
                core.matchCauseEffect(
                  register((cb) =>
                    unsafeRunCallback(runtime)(core.intoDeferred(cb, deferred))
                  ),
                  {
                    onFailure: (cause) =>
