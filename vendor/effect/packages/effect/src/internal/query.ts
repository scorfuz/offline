...
export const currentCache = globalValue(
  ...
  () =>
    core.fiberRefUnsafeMake<RequestCache>(
      unsafeMakeWith<
        Request.Request<any, any>,
        {
          listeners: Request.Listeners
      ...
      >(
        65536,
        () =>
          core.map(core.deferredMake<any, any>(), (handle) => ({
...
>(
  ...
  [Ds] extends [Effect.Effect<any, any, any>]
    ? Effect.Effect.Context<Ds>
    : never
> =>
  core.flatMap(
    (core.isEffect(dataSource)
      ? dataSource
      : core.succeed(dataSource)) as Effect.Effect<
      RequestResolver.RequestResolver<A>
    >,
    (ds) =>
      core.fiberIdWith((id) => {
        ...
        return core.fiberRefGetWith(currentCacheEnabled, (cacheEnabled) => {
          if (cacheEnabled) {
            const cached: Effect.Effect<any, any> = core.fiberRefGetWith(
              currentCache,
              (cache) =>
...
>(2, (self, strategy) =>
  core.fiberRefLocally(self, currentCacheEnabled, strategy)
)
