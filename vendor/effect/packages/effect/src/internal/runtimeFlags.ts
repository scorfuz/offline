...
/** @internal */
export const Interruption: RuntimeFlags.RuntimeFlag = (1 <<
  0) as RuntimeFlags.RuntimeFlag

/** @internal */
export const OpSupervision: RuntimeFlags.RuntimeFlag = (1 <<
  1) as RuntimeFlags.RuntimeFlag

/** @internal */
export const RuntimeMetrics: RuntimeFlags.RuntimeFlag = (1 <<
  2) as RuntimeFlags.RuntimeFlag

/** @internal */
export const WindDown: RuntimeFlags.RuntimeFlag = (1 <<
  4) as RuntimeFlags.RuntimeFlag

/** @internal */
export const CooperativeYielding: RuntimeFlags.RuntimeFlag = (1 <<
  5) as RuntimeFlags.RuntimeFlag
...
export const cooperativeYielding = (self: RuntimeFlags.RuntimeFlags): boolean =>
  isEnabled(self, CooperativeYielding)

/** @internal */
export const disable = dual<
  (
    flag: RuntimeFlags.RuntimeFlag
  ) => (self: RuntimeFlags.RuntimeFlags) => RuntimeFlags.RuntimeFlags,
  (
...
export const interruptible = (self: RuntimeFlags.RuntimeFlags): boolean =>
  interruption(self) && !windDown(self)

/** @internal */
export const interruption = (self: RuntimeFlags.RuntimeFlags): boolean =>
  isEnabled(self, Interruption)

/** @internal */
export const isDisabled = dual<
  (
    flag: RuntimeFlags.RuntimeFlag
  ) => (self: RuntimeFlags.RuntimeFlags) => boolean,
...
export const enabledSet = (
  ...
  toSet(
    (runtimeFlagsPatch.active(self) &
      runtimeFlagsPatch.enabled(self)) as RuntimeFlags.RuntimeFlags
  )

/** @internal */
export const disabledSet = (
  self: RuntimeFlagsPatch.RuntimeFlagsPatch
): ReadonlySet<RuntimeFlags.RuntimeFlag> =>
  toSet(
    (runtimeFlagsPatch.active(self) &
...
export const diff = dual<
  ...
  ) => (self: RuntimeFlags.RuntimeFlags) => RuntimeFlagsPatch.RuntimeFlagsPatch,
  (
    self: RuntimeFlags.RuntimeFlags,
    that: RuntimeFlags.RuntimeFlags
...
>(
  ...
  (self, patch) =>
    ((self &
      (runtimeFlagsPatch.invert(runtimeFlagsPatch.active(patch)) |
        runtimeFlagsPatch.enabled(patch))) |
      (runtimeFlagsPatch.active(patch) &
        runtimeFlagsPatch.enabled(patch))) as RuntimeFlags.RuntimeFlags
)
