...
/** @internal */
type Op<Tag extends string, Body = {}> = STM.STM<never> &
  Body & {
...
/** @internal */
interface STMEffect extends Op<
  OpCodes.OP_WITH_STM_RUNTIME,
  {
...
/** @internal */
interface STMOnRetry extends Op<
  OpCodes.OP_ON_RETRY,
  {
    readonly effect_instruction_i1: STM.STM<unknown, unknown, unknown>
...
/** @internal */
interface STMOnSuccess extends Op<
...
/** @internal */
interface STMProvide extends Op<
  OpCodes.OP_PROVIDE,
  {
    readonly effect_instruction_i1: STM.STM<unknown, unknown, unknown>
    readonly effect_instruction_i2: (
...
/** @internal */
interface STMSucceed extends Op<
  OpCodes.OP_SUCCEED,
  {
    readonly effect_instruction_i1: unknown
  }
...
/** @internal */
interface STMInterrupt extends Op<
  OpCodes.OP_INTERRUPT,
  {
...
class STMPrimitive implements STM.STM<any, any, any> {
  ...
  constructor(
    readonly effect_instruction_i0: Primitive["effect_instruction_i0"]
  ) {
...
export const isSTM = (u: unknown): u is STM.STM<unknown, unknown, unknown> =>
  hasProperty(u, STMTypeId)

/** @internal */
export const commit = <A, E, R>(
  self: STM.STM<A, E, R>
): Effect.Effect<A, E, R> => unsafeAtomically(self, constVoid, constVoid)
  ...
  withFiberRuntime((state) => {
    ...
    switch (commitResult._tag) {
      ...
      case TryCommitOpCodes.OP_SUSPEND: {
        ...
        const state: { value: STMState.STMState<A, E> } = {
...
const tryCommitAsync = <A, E, R>(
  ...
  if (STMState.isRunning(state.value)) {
    ...
    switch (result._tag) {
      ...
      case TryCommitOpCodes.OP_SUSPEND: {
        Journal.addTodo(txnId, result.journal, () =>
          tryCommitAsync(
            fiberId,
            self,
            txnId,
...
): STM.STM<R, never, R0> => map(context<R0>(), f)
...
export class STMDriver<in out R, out E, out A> {
  ...
  nextSuccess() {
    ...
    while (
      current !== undefined &&
      current.effect_instruction_i0 !== OpCodes.OP_ON_SUCCESS
    ) {
  ...
  nextFailure() {
    ...
    while (
      current !== undefined &&
      current.effect_instruction_i0 !== OpCodes.OP_ON_FAILURE
    ) {
      current = this.popStack()
    }
    return current
  ...
  nextRetry() {
    ...
    while (
      current !== undefined &&
      current.effect_instruction_i0 !== OpCodes.OP_ON_RETRY
    ) {
      current = this.popStack()
    }
    return current
    ...
    while (exit === undefined && curr !== undefined) {
      try {
        ...
        if (current) {
          switch (current._op) {
            case "Tag": {
              curr = effect((_, __, env) =>
                Context.unsafeGet(env, current)
              ) as Primitive
              break
            }
            case "Left": {
              ...
              switch (current.effect_instruction_i0) {
                ...
                case OpCodes.OP_RETRY: {
                  ...
                  if (cont === undefined) {
                    ...
                    curr = internalCall(
                      () => cont.effect_instruction_i2() as Primitive
                    )
                  }
                  break
                }
                  ...
                  curr = internalCall(
                    () =>
                      current.effect_instruction_i1(
                        this as STMDriver<unknown, unknown, unknown>
                      ) as Primitive
                  )
                  break
                }
                ...
                case OpCodes.OP_PROVIDE: {
                  const env = this.env
                  this.env = internalCall(() =>
                    current.effect_instruction_i2(env)
                  )
                  curr = pipe(
                    current.effect_instruction_i1,
                ...
                case OpCodes.OP_SUCCEED: {
                  ...
                  if (cont === undefined) {
                    ...
                    curr = internalCall(
                      () => cont.effect_instruction_i2(value) as Primitive
                    )
                  }
                  break
                }
                case OpCodes.OP_SYNC: {
                  const value = internalCall(() =>
                    current.effect_instruction_i1()
                  )
                  const cont = this.nextSuccess()
                  if (cont === undefined) {
...
export const catchAll = dual<
  ...
  ) => <A, R>(self: STM.STM<A, E, R>) => STM.STM<B | A, E1, R1 | R>,
  <A, E, R, B, E1, R1>(
    self: STM.STM<A, E, R>,
    f: (e: E) => STM.STM<B, E1, R1>
  ) => STM.STM<B | A, E1, R1 | R>
...
export const mapInputContext = dual<
  ...
  ) => <A, E>(self: STM.STM<A, E, R>) => STM.STM<A, E, R0>,
  <A, E, R0, R>(
    self: STM.STM<A, E, R>,
    f: (context: Context.Context<R0>) => Context.Context<R>
  ) => STM.STM<A, E, R0>
...
export const effect = <R, A>(
  ...
  withSTMRuntime((_) => succeed(f(_.journal, _.fiberId, _.getEnv())))

/** @internal */
export const ensuring = dual<
  <R1, B>(
    finalizer: STM.STM<B, never, R1>
  ) => <A, E, R>(self: STM.STM<A, E, R>) => STM.STM<A, E, R1 | R>,
  <A, E, R, R1, B>(
    self: STM.STM<A, E, R>,
    finalizer: STM.STM<B, never, R1>
  ) => STM.STM<A, E, R1 | R>
...
export const flatMap = dual<
  <A, A2, E1, R1>(
    f: (a: A) => STM.STM<A2, E1, R1>
  ) => <E, R>(self: STM.STM<A, E, R>) => STM.STM<A2, E1 | E, R1 | R>,
  <A, E, R, A2, E1, R1>(
    self: STM.STM<A, E, R>,
    f: (a: A) => STM.STM<A2, E1, R1>
...
export const orTry = dual<
  ...
  ) => <A, E, R>(self: STM.STM<A, E, R>) => STM.STM<A1 | A, E1 | E, R1 | R>,
  <A, E, R, A1, E1, R1>(
    self: STM.STM<A, E, R>,
    that: LazyArg<STM.STM<A1, E1, R1>>
...
export const zip = dual<
  ...
  ) => <A, E, R>(self: STM.STM<A, E, R>) => STM.STM<[A, A1], E1 | E, R1 | R>,
  <A, E, R, A1, E1, R1>(
    self: STM.STM<A, E, R>,
    that: STM.STM<A1, E1, R1>
...
export const zipLeft = dual<
  <A1, E1, R1>(
    that: STM.STM<A1, E1, R1>
  ) => <A, E, R>(self: STM.STM<A, E, R>) => STM.STM<A, E1 | E, R1 | R>,
  <A, E, R, A1, E1, R1>(
    self: STM.STM<A, E, R>,
    that: STM.STM<A1, E1, R1>
  ) => STM.STM<A, E1 | E, R1 | R>
...
export const zipRight = dual<
  <A1, E1, R1>(
    that: STM.STM<A1, E1, R1>
  ) => <A, E, R>(self: STM.STM<A, E, R>) => STM.STM<A1, E1 | E, R1 | R>,
  <A, E, R, A1, E1, R1>(
    self: STM.STM<A, E, R>,
    that: STM.STM<A1, E1, R1>
  ) => STM.STM<A1, E1 | E, R1 | R>
>(2, (self, that) =>
  pipe(
    self,
    flatMap(() => that)
