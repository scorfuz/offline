...
/** @internal */
export const TReentrantLockTypeId: TReentrantLock.TReentrantLockTypeId =
  Symbol.for(TReentrantLockSymbolKey) as TReentrantLock.TReentrantLockTypeId
...
class TReentranLockImpl implements TReentrantLock.TReentrantLock {
  readonly [TReentrantLockTypeId]: TReentrantLock.TReentrantLockTypeId =
...
const noOtherHolder = (
  ...
  return (
    HashMap.isEmpty(readLock.readers) ||
    (HashMap.size(readLock.readers) === 1 &&
      HashMap.has(readLock.readers, fiberId))
  )
}
...
const adjustRead = (
  ...
  core.withSTMRuntime((runtime) => {
    ...
    if (isWriteLock(lock) && Equal.equals(runtime.fiberId)(lock.fiberId)) {
      ...
      if (newTotal < 0) {
        throw new Error(
          `Defect: Fiber ${FiberId.threadName(
            runtime.fiberId
...
export const acquireRead = (
  self: TReentrantLock.TReentrantLock
): STM.STM<number> => adjustRead(self, 1)
...
/** @internal */
export const fiberReadLocks = (
  self: TReentrantLock.TReentrantLock
): STM.STM<number> =>
  core.effect<never, number>((journal, fiberId) =>
    tRef.unsafeGet(self.state, journal).readLocksHeld(fiberId)
  )

/** @internal */
export const fiberWriteLocks = (
  self: TReentrantLock.TReentrantLock
): STM.STM<number> =>
  core.effect<never, number>((journal, fiberId) =>
    tRef.unsafeGet(self.state, journal).writeLocksHeld(fiberId)
  )

/** @internal */
export const lock = (
  self: TReentrantLock.TReentrantLock
): Effect.Effect<number, never, Scope.Scope> => writeLock(self)

/** @internal */
export const locked = (self: TReentrantLock.TReentrantLock): STM.STM<boolean> =>
  core.zipWith(readLocked(self), writeLocked(self), (x, y) => x || y)
...
export const readLock = (
  ...
  Effect.acquireRelease(core.commit(acquireRead(self)), () =>
...
export const readLocks = (
  self: TReentrantLock.TReentrantLock
): STM.STM<number> => core.map(tRef.get(self.state), (state) => state.readLocks)

/** @internal */
export const readLocked = (
  self: TReentrantLock.TReentrantLock
): STM.STM<boolean> =>
  core.map(tRef.get(self.state), (state) => state.readLocks > 0)
...
export const releaseWrite = (
  ...
  core.withSTMRuntime((runtime) => {
    ...
    if (
      isWriteLock(lock) &&
      lock.writeLocks === 1 &&
      Equal.equals(runtime.fiberId)(lock.fiberId)
    ) {
...
>(2, (effect, self) =>
  Effect.uninterruptibleMask((restore) =>
    Effect.zipRight(
      ...
      Effect.ensuring(effect, core.commit(releaseRead(self)))
    )
  )
)
...
>(2, (effect, self) =>
  Effect.uninterruptibleMask((restore) =>
    Effect.zipRight(
      ...
      Effect.ensuring(effect, core.commit(releaseWrite(self)))
    )
  )
)
...
export const writeLock = (
  ...
  Effect.acquireRelease(core.commit(acquireWrite(self)), () =>
...
export const writeLocked = (
  ...
  core.map(tRef.get(self.state), (state) => state.writeLocks > 0)

/** @internal */
export const writeLocks = (
  self: TReentrantLock.TReentrantLock
): STM.STM<number> =>
  core.map(tRef.get(self.state), (state) => state.writeLocks)
