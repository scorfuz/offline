...
/** @internal */
export const TPubSubTypeId: TPubSub.TPubSubTypeId = Symbol.for(
  TPubSubSymbolKey
) as TPubSub.TPubSubTypeId
...
class TPubSubImpl<in out A> implements TPubSub.TPubSub<A> {
  ...
  constructor(
    ...
    readonly publisherTail: TRef.TRef<
      TRef.TRef<Node<A> | undefined> | undefined
    >,
    ...
    readonly subscribers: TRef.TRef<
      HashSet.HashSet<TRef.TRef<TRef.TRef<Node<A>> | undefined>>
    >
  ) {}
  ...
  awaitShutdown: STM.STM<void> = core.flatMap(this.isShutdown, (isShutdown) =>
    isShutdown ? stm.void : core.retry
  )
  ...
  size: STM.STM<number> = core.withSTMRuntime((runtime) => {
    const currentPublisherTail = tRef.unsafeGet(
      this.publisherTail,
      runtime.journal
    )
  ...
  isFull: STM.STM<boolean> = core.map(
    this.size,
  ...
  offer(value: A): STM.STM<boolean> {
    return core.withSTMRuntime((runtime) => {
      ...
      const currentSubscriberCount = tRef.unsafeGet(
      ...
      if (currentPubSubSize < this.requestedCapacity) {
        const updatedPublisherTail: TRef.TRef<Node<A> | undefined> =
          new tRef.TRefImpl<Node<A> | undefined>(void 0)
      ...
      switch (this.strategy._tag) {
        ...
        case OpCodes.OP_SLIDING_STRATEGY: {
          if (this.requestedCapacity > 0) {
            let currentPublisherHead: TRef.TRef<Node<A> | undefined> =
              tRef.unsafeGet(this.publisherHead, runtime.journal)
            let loop = true
            while (loop) {
  ...
  shutdown: STM.STM<void> = core.effect<never, void>((journal) => {
    ...
    if (currentPublisherTail !== undefined) {
      ...
      HashSet.forEach(currentSubscribers, (subscriber) => {
        tRef.unsafeSet<TRef.TRef<Node<A>> | undefined>(
          subscriber,
          void 0,
          journal
        )
      ...
      tRef.unsafeSet(
        this.subscribers,
        HashSet.empty<TRef.TRef<TRef.TRef<Node<A>> | undefined>>(),
        journal
      )
    }
  })
...
class TPubSubSubscriptionImpl<in out A> implements TQueue.TDequeue<A> {
  ...
  constructor(
    ...
    readonly publisherHead: TRef.TRef<TRef.TRef<Node<A> | undefined>>,
    readonly requestedCapacity: number,
    readonly subscriberHead: TRef.TRef<
      TRef.TRef<Node<A> | undefined> | undefined
    >,
    readonly subscriberCount: TRef.TRef<number>,
    readonly subscribers: TRef.TRef<
  ...
  size: STM.STM<number> = core.withSTMRuntime((runtime) => {
    let currentSubscriberHead = tRef.unsafeGet(
      this.subscriberHead,
      runtime.journal
    )
...
const makeTPubSub = <A>(
  ...
  pipe(
    stm.all([tRef.make<Node<A> | undefined>(void 0), tRef.make(0)]),
    core.flatMap(([empty, pubsubSize]) =>
      pipe(
        ...
        core.map(
          ([publisherHead, publisherTail, subscriberCount, subscribers]) =>
            new TPubSubImpl(
              pubsubSize,
              publisherHead,
    ...
    core.flatMap((currentPublisherTail) =>
      pipe(
        ...
        stm.tap(([_, currentSubscriberCount]) =>
          pipe(subscriberCount, tRef.set(currentSubscriberCount + 1))
        ),
        stm.tap(([subscriberHead, _, currentSubscribers]) =>
          pipe(
            subscribers as any,
            tRef.set(
              pipe(currentSubscribers as any, HashSet.add(subscriberHead))
            )
          )
        ),
        core.map(
...
/** @internal */
export const awaitShutdown = <A>(self: TPubSub.TPubSub<A>): STM.STM<void> =>
  self.awaitShutdown
...
/** @internal */
export const isEmpty = <A>(self: TPubSub.TPubSub<A>): STM.STM<boolean> =>
  self.isEmpty

/** @internal */
export const isFull = <A>(self: TPubSub.TPubSub<A>): STM.STM<boolean> =>
  self.isFull

/** @internal */
export const isShutdown = <A>(self: TPubSub.TPubSub<A>): STM.STM<boolean> =>
  self.isShutdown
...
/** @internal */
export const shutdown = <A>(self: TPubSub.TPubSub<A>): STM.STM<void> =>
  self.shutdown
...
export const subscribeScoped = <A>(
  ...
  Effect.acquireRelease(subscribe(self), (dequeue) => tQueue.shutdown(dequeue))
