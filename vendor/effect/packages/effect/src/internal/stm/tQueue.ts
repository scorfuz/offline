...
/** @internal */
export const TEnqueueTypeId: TQueue.TEnqueueTypeId = Symbol.for(
  TEnqueueSymbolKey
) as TQueue.TEnqueueTypeId

const TDequeueSymbolKey = "effect/TQueue/TDequeue"

/** @internal */
export const TDequeueTypeId: TQueue.TDequeueTypeId = Symbol.for(
  TDequeueSymbolKey
) as TQueue.TDequeueTypeId
...
class TQueueImpl<in out A> implements TQueue.TQueue<A> {
  ...
  isFull: STM.STM<boolean> = core.map(
    this.size,
    (size) => size === this.requestedCapacity
  )
  ...
  takeUpTo(max: number): STM.STM<Array<A>> {
    return core.withSTMRuntime((runtime) => {
      ...
      const [toTake, remaining] = Chunk.splitAt(
        Chunk.unsafeFromArray(queue),
        max
      )
      tRef.unsafeSet<Array<A> | undefined>(
        this.ref,
...
export const dropping = <A>(
  requestedCapacity: number
): STM.STM<TQueue.TQueue<A>> => makeQueue<A>(requestedCapacity, Dropping)

/** @internal */
export const isEmpty = <A>(
  self: TQueue.TDequeue<A> | TQueue.TEnqueue<A>
): STM.STM<boolean> => self.isEmpty

/** @internal */
export const isFull = <A>(
  self: TQueue.TDequeue<A> | TQueue.TEnqueue<A>
): STM.STM<boolean> => self.isFull

/** @internal */
export const isShutdown = <A>(
  self: TQueue.TDequeue<A> | TQueue.TEnqueue<A>
): STM.STM<boolean> => self.isShutdown
...
): STM.STM<Option.Option<A>> => self.peekOption
...
const seekLoop = <A>(
  ...
  core.flatMap(self.take, (a) =>
    predicate(a) ? core.succeed(a) : seekLoop(self, predicate)
  )

/** @internal */
export const shutdown = <A>(
  self: TQueue.TDequeue<A> | TQueue.TEnqueue<A>
): STM.STM<void> => self.shutdown

/** @internal */
export const size = <A>(
  self: TQueue.TDequeue<A> | TQueue.TEnqueue<A>
): STM.STM<number> => self.size

/** @internal */
export const sliding = <A>(
  requestedCapacity: number
): STM.STM<TQueue.TQueue<A>> => makeQueue<A>(requestedCapacity, Sliding)

/** @internal */
export const take = <A>(self: TQueue.TDequeue<A>): STM.STM<A> => self.take

/** @internal */
export const takeAll = <A>(self: TQueue.TDequeue<A>): STM.STM<Array<A>> =>
  self.takeAll
...
export const takeBetween = dual<
  (
    min: number,
    max: number
  ) => <A>(self: TQueue.TDequeue<A>) => STM.STM<Array<A>>,
      ...
      const takeRemainder = (
        ...
        return pipe(
          ...
          core.flatMap((taken) => {
            ...
            if (remaining === 1) {
              return pipe(
                ...
                core.map((a) =>
                  pipe(
                    acc,
                    Chunk.appendAll(Chunk.unsafeFromArray(taken)),
                    Chunk.append(a)
                  )
                )
              )
            ...
            return core.succeed(
              pipe(acc, Chunk.appendAll(Chunk.unsafeFromArray(taken)))
            )
          })
        )
      }
      return core.map(takeRemainder(min, max, Chunk.empty<A>()), (c) =>
        Array.from(c)
      )
    })
)
...
const makeQueue = <A>(
  requestedCapacity: number,
  strategy: TQueueStrategy
): STM.STM<TQueue.TQueue<A>> =>
  core.map(
