...
/** @internal */
export const TPriorityQueueTypeId: TPriorityQueue.TPriorityQueueTypeId =
  Symbol.for(TPriorityQueueSymbolKey) as TPriorityQueue.TPriorityQueueTypeId
...
export class TPriorityQueueImpl<
  in out A
> implements TPriorityQueue.TPriorityQueue<A> {
...
export const empty = <A>(
  order: Order.Order<A>
): STM.STM<TPriorityQueue.TPriorityQueue<A>> =>
  pipe(
...
export const fromIterable =
  ...
  (iterable: Iterable<A>): STM.STM<TPriorityQueue.TPriorityQueue<A>> =>
    pipe(
      tRef.make(
...
export const isEmpty = <A>(
  self: TPriorityQueue.TPriorityQueue<A>
): STM.STM<boolean> => core.map(tRef.get(self.ref), SortedMap.isEmpty)

/** @internal */
export const isNonEmpty = <A>(
  self: TPriorityQueue.TPriorityQueue<A>
): STM.STM<boolean> => core.map(tRef.get(self.ref), SortedMap.isNonEmpty)

/** @internal */
export const make =
  <A>(order: Order.Order<A>) =>
  (...elements: Array<A>): STM.STM<TPriorityQueue.TPriorityQueue<A>> =>
...
export const offerAll = dual<
  <A>(
    values: Iterable<A>
  ) => (self: TPriorityQueue.TPriorityQueue<A>) => STM.STM<void>,
  <A>(
    self: TPriorityQueue.TPriorityQueue<A>,
    values: Iterable<A>
  ) => STM.STM<void>
...
>(2, <A>(self: TPriorityQueue.TPriorityQueue<A>, predicate: Predicate<A>) =>
  tRef.update(self.ref, (map) =>
    SortedMap.reduce(
      map,
      SortedMap.empty(SortedMap.getOrder(map)),
      (map, value, key) => {
...
export const size = <A>(
  ...
  tRef.modify(self.ref, (map) => [
    SortedMap.reduce(map, 0, (n, as) => n + as.length),
    map
  ])
...
export const takeUpTo = dual<
  (
    n: number
  ) => <A>(self: TPriorityQueue.TPriorityQueue<A>) => STM.STM<Array<A>>,
