...
/** @internal */
export const TSubscriptionRefTypeId: TSubscriptionRef.TSubscriptionRefTypeId =
  Symbol.for(
...
class TSubscriptionRefImpl<
  in out A
> implements TSubscriptionRef.TSubscriptionRef<A> {
...
export const make = <A>(
  value: A
): STM.STM<TSubscriptionRef.TSubscriptionRef<A>> =>
  pipe(
    STM.all([TPubSub.unbounded<A>(), TRef.make(value)]),
    STM.map(([pubsub, ref]) => new TSubscriptionRefImpl(ref, pubsub))
  )

/** @internal */
export const get = <A>(self: TSubscriptionRef.TSubscriptionRef<A>) =>
  TRef.get(self.ref)
...
export const set = dual<
  <A>(
    value: A
  ) => (self: TSubscriptionRef.TSubscriptionRef<A>) => STM.STM<void>,
...
export const getAndUpdate = dual<
  <A>(
    f: (a: A) => A
  ) => (self: TSubscriptionRef.TSubscriptionRef<A>) => STM.STM<A>,
