...
/** @internal */
export const TArrayTypeId: TArray.TArrayTypeId = Symbol.for(
  TArraySymbolKey
) as TArray.TArrayTypeId
...
export const collectFirst = dual<
  ...
  ) => (self: TArray.TArray<A>) => STM.STM<Option.Option<B>>,
  <A, B>(
    self: TArray.TArray<A>,
    pf: (a: A) => Option.Option<B>
  ) => STM.STM<Option.Option<B>>
>(2, (self, pf) =>
  collectFirstSTM(self, (a) => pipe(pf(a), Option.map(core.succeed)))
)
...
export const collectFirstSTM = dual<
  ...
  ) => (self: TArray.TArray<A>) => STM.STM<Option.Option<B>, E, R>,
  <A, B, E, R>(
    self: TArray.TArray<A>,
    pf: (a: A) => Option.Option<STM.STM<B, E, R>>
...
>(2, (self, predicate) => reduce(self, 0, (n, a) => (predicate(a) ? n + 1 : n)))

/** @internal */
export const countSTM = dual<
  <A, R, E>(
    predicate: (value: A) => STM.STM<boolean, E, R>
  ) => (self: TArray.TArray<A>) => STM.STM<number, E, R>,
  <A, R, E>(
    self: TArray.TArray<A>,
    predicate: (value: A) => STM.STM<boolean, E, R>
  ) => STM.STM<number, E, R>
>(2, (self, predicate) =>
  reduceSTM(self, 0, (n, a) =>
    core.map(predicate(a), (bool) => (bool ? n + 1 : n))
  )
)
...
export const everySTM = dual<
  <A, R, E>(
    predicate: (value: A) => STM.STM<boolean, E, R>
  ) => (self: TArray.TArray<A>) => STM.STM<boolean, E, R>,
  <A, R, E>(
    self: TArray.TArray<A>,
    predicate: (value: A) => STM.STM<boolean, E, R>
  ) => STM.STM<boolean, E, R>
>(2, (self, predicate) =>
  core.map(countSTM(self, predicate), (count) => count === self.chunk.length)
)
...
export const findFirst = dual<
  ...
  ) => (self: TArray.TArray<A>) => STM.STM<Option.Option<A>>,
  <A>(
    self: TArray.TArray<A>,
    predicate: Predicate<A>
  ) => STM.STM<Option.Option<A>>
>(2, (self, predicate) =>
  collectFirst(self, (a) => (predicate(a) ? Option.some(a) : Option.none()))
)
...
>(3, (self, value, from) =>
  findFirstIndexWhereFrom(self, (a) => Equal.equals(a)(value), from)
)
    ...
    const forIndex = (index: number): STM.STM<Option.Option<number>, E, R> =>
      index < self.chunk.length
        ? pipe(
            ...
            core.flatMap((bool) =>
              bool ? core.succeed(Option.some(index)) : forIndex(index + 1)
            )
          )
        : stm.succeedNone
          ...
          return pipe(
            ...
            core.flatMap((value) =>
              core.map(
                ...
                (bool) =>
                  [
                    bool ? Option.some(value) : Option.none(),
                    index + 1
                  ] as const
              )
            )
...
>(
  ...
  <A, R, E>(
    ...
    const cont = (state: readonly [Option.Option<A>, number]) =>
      Option.isNone(state[0]) && state[1] >= 0
    return core.map(
      stm.iterate(init, {
        while: cont,
        body: (state) => {
...
export const forEach = dual<
  ...
  ) => (self: TArray.TArray<A>) => STM.STM<void, E, R>,
  <A, R, E>(
    self: TArray.TArray<A>,
    f: (value: A) => STM.STM<void, E, R>
  ) => STM.STM<void, E, R>
...
export const fromIterable = <A>(
  iterable: Iterable<A>
): STM.STM<TArray.TArray<A>> =>
  core.map(stm.forEach(iterable, tRef.make), (chunk) => new TArrayImpl(chunk))
...
>(2, (self, order) => {
  ...
  return reduceOption(self, (acc, curr) =>
    greaterThan(acc)(curr) ? curr : acc
  )
})
...
export const reduce = dual<
  ...
  ) => (self: TArray.TArray<A>) => STM.STM<Z>,
  <Z, A>(
    self: TArray.TArray<A>,
    zero: Z,
    f: (accumulator: Z, current: A) => Z
...
>(2, <A>(self: TArray.TArray<A>, f: (x: A, y: A) => A) =>
  core.effect<never, Option.Option<A>>((journal) => {
    let index = 0
...
>(2, <A, R, E>(self: TArray.TArray<A>, f: (x: A, y: A) => STM.STM<A, E, R>) =>
...
>(3, (self, zero, f) => core.flatMap(toArray(self), stm.reduce(zero, f)))
...
export const some = dual<
  <A>(predicate: Predicate<A>) => (self: TArray.TArray<A>) => STM.STM<boolean>,
  <A>(self: TArray.TArray<A>, predicate: Predicate<A>) => STM.STM<boolean>
>(2, (self, predicate) => core.map(findFirst(self, predicate), Option.isSome))
...
/** @internal */
export const toArray = <A>(self: TArray.TArray<A>): STM.STM<Array<A>> =>
  stm.forEach(self.chunk, tRef.get)
...
>(2, <A, R, E>(self: TArray.TArray<A>, f: (value: A) => STM.STM<A, E, R>) =>
  core.flatMap(
    stm.forEach(self.chunk, (ref) => core.flatMap(tRef.get(ref), f)),
    (chunk) =>
      core.effect<never, void>((journal) => {
...
export const update = dual<
  <A>(
    index: number,
    f: (value: A) => A
  ) => (self: TArray.TArray<A>) => STM.STM<void>,
  <A>(
    self: TArray.TArray<A>,
    index: number,
    f: (value: A) => A
