...
/** @internal */
export const empty = <K, V>(): STM.STM<TMap.TMap<K, V>> =>
  fromIterable<K, V>([])
...
>(2, (self, f) =>
  reduceSTM(self, void 0 as void, (_, value, key) => stm.asVoid(f(key, value)))
)
...
>(3, (self, key, fallback) =>
  core.map(get(self, key), Option.getOrElse(fallback))
)
...
export const keys = <K, V>(self: TMap.TMap<K, V>): STM.STM<Array<K>> =>
  core.map(
    ...
    RA.map((entry) => entry[0])
  )

/** @internal */
export const make = <K, V>(
  ...entries: Array<readonly [K, V]>
): STM.STM<TMap.TMap<K, V>> => fromIterable(entries)
  ...
  <K, V, Z>(
    ...
    core.effect<never, Z>((journal) => {
      ...
      while (index < buckets.chunk.length) {
        ...
        result = Chunk.reduce(items, result, (acc, entry) =>
          f(acc, entry[1], entry[0])
        )
        index = index + 1
      }
      return result
...
>(2, (self, key) =>
  core.effect<never, void>((journal) => {
    ...
    const index = indexOf(key, buckets.chunk.length)
    const bucket = tRef.unsafeGet(buckets.chunk[index], journal)
    const [toRemove, toRetain] = Chunk.partition(bucket, (entry) =>
      Equal.equals(entry[1], key)
    )
...
>(2, <K, V>(self: TMap.TMap<K, V>, keys: Iterable<K>) =>
  core.effect<never, void>((journal) => {
    ...
    while ((next = iterator.next()) && !next.done) {
      ...
      const bucket = tRef.unsafeGet(buckets.chunk[index], journal)
      const [toRemove, toRetain] = Chunk.partition(bucket, (entry) =>
        Equal.equals(next.value)(entry[0])
      )
      if (Chunk.isNonEmpty(toRemove)) {
        const currentSize = tRef.unsafeGet(self.tSize, journal)
        tRef.unsafeSet(buckets.chunk[index], toRetain, journal)
...
export const retainIf: {
  ...
  (self, predicate, options) =>
    removeIf(self, (key, value) => !predicate(key, value), options)
)
...
>(3, <K, V>(self: TMap.TMap<K, V>, key: K, value: V) => {
  const resize = (
    ...
    buckets: TArray.TArray<Chunk.Chunk<readonly [K, V]>>
  ): void => {
    const capacity = buckets.chunk.length
    ...
    newBuckets[newIndex] = Chunk.prepend(newBuckets[newIndex], [
      key,
      value
    ] as const)
    ...
    const newTArray: TArray.TArray<Chunk.Chunk<readonly [K, V]>> =
      new tArray.TArrayImpl(newArray)
  ...
  return core.effect<never, void>((journal) => {
    ...
    const bucket = tRef.unsafeGet(buckets.chunk[index], journal)
    const shouldUpdate = Chunk.some(bucket, (entry) =>
      Equal.equals(key)(entry[0])
    )
    if (shouldUpdate) {
      const newBucket = Chunk.map(bucket, (entry) =>
        Equal.equals(key)(entry[0]) ? ([key, value] as const) : entry
      )
      tRef.unsafeSet(buckets.chunk[index], newBucket, journal)
    } else {
...
/** @internal */
export const size = <K, V>(self: TMap.TMap<K, V>): STM.STM<number> =>
  tRef.get(self.tSize)
    ...
    pipe(
      core.effect<never, Option.Option<A>>((journal) => {
        ...
        while (index < capacity && Option.isNone(result)) {
          const bucket = tRef.unsafeGet(buckets.chunk[index], journal)
          const recreate = Chunk.some(bucket, (entry) =>
            Option.isSome(pf(entry[0], entry[1]))
          )
          if (recreate) {
            const iterator = bucket[Symbol.iterator]()
            let newBucket = Chunk.empty<readonly [K, V]>()
      ...
      stm.collect((option) =>
        Option.isSome(option) ? Option.some(option.value) : Option.none<A>()
      )
    )
)
...
>(2, (self, pf) =>
  pipe(
    findSTM(self, (key, value) =>
      core.map(pf(key, value), (a) => [key, a] as const)
    ),
    stm.collect((option) =>
      Option.isSome(option) ? Option.some(option.value) : Option.none()
    ),
...
>(
  ...
  <K, V, A, E, R>(
    ...
    pipe(
      ...
      core.map((chunk) =>
        RA.isNonEmptyArray(chunk) ? Option.some(chunk) : Option.none()
      ),
      stm.collect((option) =>
        Option.isSome(option) ? Option.some(option.value) : Option.none()
      ),
...
const toReadonlyArray = <K, V>(
  self: TMap.TMap<K, V>
): STM.STM<ReadonlyArray<readonly [K, V]>> =>
...
/** @internal */
export const toChunk = <K, V>(
  self: TMap.TMap<K, V>
): STM.STM<Chunk.Chunk<[K, V]>> =>
  reduce(self, Chunk.empty<[K, V]>(), (acc, value, key) =>
    Chunk.append(acc, [key, value])
  )

/** @internal */
export const toHashMap = <K, V>(
  self: TMap.TMap<K, V>
): STM.STM<HashMap.HashMap<K, V>> =>
  reduce(self, HashMap.empty<K, V>(), (acc, value, key) =>
    pipe(acc, HashMap.set(key, value))
  )

/** @internal */
export const toArray = <K, V>(self: TMap.TMap<K, V>): STM.STM<Array<[K, V]>> =>
  reduce(self, [] as Array<[K, V]>, (acc, value, key) => {
...
export const toMap = <K, V>(self: TMap.TMap<K, V>): STM.STM<Map<K, V>> =>
  reduce(self, new Map<K, V>(), (acc, value, key) => acc.set(key, value))
...
>(2, <K, V>(self: TMap.TMap<K, V>, f: (key: K, value: V) => readonly [K, V]) =>
  core.effect<never, void>((journal) => {
    const buckets = pipe(self.tBuckets, tRef.unsafeGet(journal))
    const capacity = buckets.chunk.length
    const newBuckets = Array.from({ length: capacity }, () =>
      Chunk.empty<readonly [K, V]>()
    )
    let newSize = 0
    let index = 0
    while (index < capacity) {
      ...
      while ((next = iterator.next()) && !next.done) {
        ...
        if (
          !Chunk.some(newBucket, (entry) => Equal.equals(entry[0], newPair[0]))
        ) {
      ...
      core.flatMap((newData) =>
        core.effect<never, void>((journal) => {
          ...
          while ((next = iterator.next()) && !next.done) {
            ...
            const newBucket = newBuckets[index]
            if (
              !Chunk.some(newBucket, (entry) =>
                Equal.equals(entry[0])(next.value[0])
              )
            ) {
...
export const transformValuesSTM = dual<
  <V, R, E>(
    f: (value: V) => STM.STM<V, E, R>
  ) => <K>(self: TMap.TMap<K, V>) => STM.STM<void, E, R>,
  <K, V, R, E>(
    self: TMap.TMap<K, V>,
    f: (value: V) => STM.STM<V, E, R>
  ) => STM.STM<void, E, R>
>(2, (self, f) =>
  transformSTM(self, (key, value) =>
    core.map(f(value), (value) => [key, value])
  )
)
