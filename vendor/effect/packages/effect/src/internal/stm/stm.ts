          ...
          onFailure: (cause) => {
            if (STMState.isDone(state) && Exit.isSuccess(state.exit)) {
              return pipe(
                ...
                Effect.matchCauseEffect({
                  onFailure: (cause2) =>
                    Effect.failCause(Cause.parallel(cause, cause2)),
...
export const asSome = <A, E, R>(
  self: STM.STM<A, E, R>
): STM.STM<Option.Option<A>, E, R> => pipe(self, core.map(Option.some))

/** @internal */
export const asSomeError = <A, E, R>(
  self: STM.STM<A, E, R>
): STM.STM<A, Option.Option<E>, R> => pipe(self, mapError(Option.some))

/** @internal */
export const asVoid = <A, E, R>(self: STM.STM<A, E, R>): STM.STM<void, E, R> =>
  pipe(self, core.map(constVoid))
...
>(
  ...
  <K, E, R, N extends string, A, E2, R2>(
    ...
    core.flatMap(self, (k) =>
      core.map(
        f(k),
        (a): Types.MergeRecord<K, { [k in N]: A }> =>
          ({ ...k, [tag]: a }) as any
      )
    )
)
...
export const bindTo = dual<
  <N extends string>(
    tag: N
  ) => <A, E, R>(self: STM.STM<A, E, R>) => STM.STM<Record<N, A>, E, R>,
  <A, E, R, N extends string>(
    self: STM.STM<A, E, R>,
    tag: N
  ) => STM.STM<Record<N, A>, E, R>
>(
  2,
  <A, E, R, N extends string>(
    self: STM.STM<A, E, R>,
...
export const let_ = dual<
  <N extends string, K, A>(
    ...
    self: STM.STM<K, E, R>
  ) => STM.STM<Types.MergeRecord<K, { [k in N]: A }>, E, R>,
...
>(
  ...
  <K, E, R, N extends string, A>(
    ...
    core.map(
      self,
      (k): Types.MergeRecord<K, { [k in N]: A }> =>
        ({ ...k, [tag]: f(k) }) as any
    )
)
...
export const catchSome = dual<
  ...
  ) => <A, R>(self: STM.STM<A, E, R>) => STM.STM<A2 | A, E | E2, R2 | R>,
  <A, E, R, A2, E2, R2>(
    self: STM.STM<A, E, R>,
    pf: (error: E) => Option.Option<STM.STM<A2, E2, R2>>
...
>(
  ...
  <A, E, R, A2, E2, R2>(
    ...
    core.catchAll(
      self,
      (e): STM.STM<A | A2, E | E2, R | R2> =>
        Option.getOrElse(pf(e), () => core.fail(e))
    )
)
...
export const check = (predicate: LazyArg<boolean>): STM.STM<void> =>
  suspend(() => (predicate() ? void_ : core.retry))

/** @internal */
export const collect = dual<
  <A, A2>(
    pf: (a: A) => Option.Option<A2>
  ) => <E, R>(self: STM.STM<A, E, R>) => STM.STM<A2, E, R>,
  <A, E, R, A2>(
    self: STM.STM<A, E, R>,
    pf: (a: A) => Option.Option<A2>
  ) => STM.STM<A2, E, R>
>(2, (self, pf) => collectSTM(self, (a) => Option.map(pf(a), core.succeed)))

/** @internal */
export const collectSTM = dual<
  <A, A2, E2, R2>(
    pf: (a: A) => Option.Option<STM.STM<A2, E2, R2>>
  ) => <E, R>(self: STM.STM<A, E, R>) => STM.STM<A2, E2 | E, R2 | R>,
  <A, E, R, A2, E2, R2>(
    self: STM.STM<A, E, R>,
    pf: (a: A) => Option.Option<STM.STM<A2, E2, R2>>
...
/** @internal */
export const commitEither = <A, E, R>(
  self: STM.STM<A, E, R>
): Effect.Effect<A, E, R> => Effect.flatten(core.commit(either(self)))
...
export const either = <A, E, R>(
  self: STM.STM<A, E, R>
): STM.STM<Either.Either<A, E>, never, R> =>
    ...
    core.flatMap(
      ...
      (iterator) => {
        const loop: STM.STM<boolean, E, R> = suspend(() => {
          ...
          return core.flatMap(predicate(next.value), (bool) =>
            bool ? core.succeed(bool) : loop
          )
        })
        return loop
      }
...
/** @internal */
export const fiberId: STM.STM<FiberId.FiberId> = core.effect<
  never,
  FiberId.FiberId
>((_, fiberId) => fiberId)
...
>(
  ...
  <A, R, E>(
    iterable: Iterable<A>,
    predicate: (a: A) => STM.STM<boolean, E, R>
  ): STM.STM<Array<A>, E, R> => filter(iterable, (a) => negate(predicate(a)))
)
...
export const filterOrElse: {
  ...
  <A, E, R, B, E2, R2>(
    ...
    core.flatMap(
      self,
      (a): STM.STM<A | B, E2, R2> =>
        predicate(a) ? core.succeed(a) : orElse(a)
    )
)
...
export const filterOrFail: {
  ...
  <A, E, R, E2>(
    ...
    filterOrElse(self, predicate, (a) => core.failSync(() => orFailWith(a)))
)

/** @internal */
export const flatten = <A, E2, R2, E, R>(
  self: STM.STM<STM.STM<A, E2, R2>, E, R>
): STM.STM<A, E | E2, R | R2> => core.flatMap(self, identity)
...
export const flipWith = dual<
  ...
  ) => (self: STM.STM<A, E, R>) => STM.STM<A | A2, E | E2, R | R2>,
  <A, E, R, E2, A2, R2>(
    self: STM.STM<A, E, R>,
    f: (stm: STM.STM<E, A, R>) => STM.STM<E2, A2, R2>
  ...
  suspend(() => {
    ...
    const run = (
      ...
      state.done
        ...
        : core.flatMap(yieldWrapGet(state.value) as any, (val: any) =>
            run(iterator.next(val as never))
          )
    return run(state)
  })
...
export const head = <A, E, R>(
  self: STM.STM<Iterable<A>, E, R>
): STM.STM<A, Option.Option<E>, R> =>
  pipe(
    self,
    core.matchSTM({
...
>(
  ...
  <E, R, A, E1, R1, A2, E2, R2>(
    ...
    return core.flatMap(
      self,
      (bool): STM.STM<A | A2, E1 | E2 | E, R1 | R2 | R> =>
        bool ? onTrue : onFalse
    )
  }
)
...
export const loop: {
  ...
  options.discard
    ...
    : core.map(
        loopLoop(initial, options.while, options.step, options.body),
        (a) => Array.from(a)
      )
...
const loopLoop = <Z, A, E, R>(
  ...
  if (cont(initial)) {
    return pipe(
      body(initial),
      core.flatMap((a) =>
        pipe(loopLoop(inc(initial), cont, inc, body), core.map(Chunk.append(a)))
      )
    )
  }
...
export const mapAttempt = dual<
  <A, B>(
    f: (a: A) => B
  ) => <E, R>(self: STM.STM<A, E, R>) => STM.STM<B, unknown, R>,
...
export const mapError = dual<
  <E, E2>(
    f: (error: E) => E2
  ) => <A, R>(self: STM.STM<A, E, R>) => STM.STM<A, E2, R>,
  <A, E, R, E2>(
    self: STM.STM<A, E, R>,
    f: (error: E) => E2
  ) => STM.STM<A, E2, R>
...
export const mergeAll = dual<
  <A2, A>(
    zero: A2,
    f: (a2: A2, a: A) => A2
  ) => <E, R>(iterable: Iterable<STM.STM<A, E, R>>) => STM.STM<A2, E, R>,
  <A, E, R, A2>(
...
export const none = <A, E, R>(
  self: STM.STM<Option.Option<A>, E, R>
): STM.STM<void, Option.Option<E>, R> =>
  core.matchSTM(self, {
    onFailure: (e) => core.fail(Option.some(e)),
...
export const option = <A, E, R>(
  self: STM.STM<A, E, R>
): STM.STM<Option.Option<A>, never, R> =>
  match(self, { onFailure: () => Option.none(), onSuccess: Option.some })

/** @internal */
export const orDie = <A, E, R>(self: STM.STM<A, E, R>): STM.STM<A, never, R> =>
...
export const orDieWith = dual<
  <E>(
    f: (error: E) => unknown
  ) => <A, R>(self: STM.STM<A, E, R>) => STM.STM<A, never, R>,
  <A, E, R>(
    self: STM.STM<A, E, R>,
    f: (error: E) => unknown
  ) => STM.STM<A, never, R>
...
export const orElse = dual<
  ...
  ) => <A, E, R>(self: STM.STM<A, E, R>) => STM.STM<A2 | A, E2, R2 | R>,
  <A, E, R, A2, E2, R2>(
    self: STM.STM<A, E, R>,
    that: LazyArg<STM.STM<A2, E2, R2>>
  ) => STM.STM<A2 | A, E2, R2 | R>
>(
  2,
  <A, E, R, A2, E2, R2>(
    self: STM.STM<A, E, R>,
    ...
    core.flatMap(
      core.effect<R, LazyArg<void>>((journal) =>
        Journal.prepareResetJournal(journal)
      ),
      (reset) =>
        pipe(
...
export const orElseFail = dual<
  <E2>(
    error: LazyArg<E2>
  ) => <A, E, R>(self: STM.STM<A, E, R>) => STM.STM<A, E2, R>,
...
>(
  ...
  <A, E, R, E2>(
    self: STM.STM<A, E, R>,
    error: LazyArg<E2>
  ): STM.STM<A, E2, R> => orElse(self, () => core.failSync(error))
)
...
export const orElseSucceed = dual<
  <A2>(
    value: LazyArg<A2>
  ) => <A, E, R>(self: STM.STM<A, E, R>) => STM.STM<A2 | A, never, R>,
  <A, E, R, A2>(
    self: STM.STM<A, E, R>,
    value: LazyArg<A2>
  ) => STM.STM<A2 | A, never, R>
>(
  2,
  <A, E, R, A2>(
    self: STM.STM<A, E, R>,
    value: LazyArg<A2>
  ): STM.STM<A2 | A, never, R> => orElse(self, () => core.sync(value))
)
...
export const provideContext = dual<
  ...
  ) => <A, E>(self: STM.STM<A, E, R>) => STM.STM<A, E>,
  <A, E, R>(self: STM.STM<A, E, R>, env: Context.Context<R>) => STM.STM<A, E>
>(2, (self, env) =>
  core.mapInputContext(self, (_: Context.Context<never>) => env)
)
...
export const provideSomeContext = dual<
  ...
  ) => <R1, E, A>(self: STM.STM<A, E, R1>) => STM.STM<A, E, Exclude<R1, R>>,
  <R, R1, E, A>(
    self: STM.STM<A, E, R1>,
    context: Context.Context<R>
...
>(
  ...
  <R, R1, E, A>(
    ...
    core.mapInputContext(
      self,
      (parent: Context.Context<Exclude<R1, R>>): Context.Context<R1> =>
        Context.merge(parent, context) as any
    )
)
...
export const provideService = dual<
  ...
  ) => <A, E, R>(self: STM.STM<A, E, R>) => STM.STM<A, E, Exclude<R, I>>,
  <A, E, R, I, S>(
    self: STM.STM<A, E, R>,
    tag: Context.Tag<I, S>,
    resource: Types.NoInfer<S>
...
>(3, (self, tag, resource) =>
  provideServiceSTM(self, tag, core.succeed(resource))
)
...
export const reduce = dual<
  <S, A, E, R>(
    zero: S,
    f: (s: S, a: A) => STM.STM<S, E, R>
  ) => (iterable: Iterable<A>) => STM.STM<S, E, R>,
  <S, A, E, R>(
    iterable: Iterable<A>,
    zero: S,
    f: (s: S, a: A) => STM.STM<S, E, R>
...
export const reject = dual<
  <A, E2>(
    pf: (a: A) => Option.Option<E2>
  ) => <E, R>(self: STM.STM<A, E, R>) => STM.STM<A, E2 | E, R>,
  <A, E, R, E2>(
    self: STM.STM<A, E, R>,
    pf: (a: A) => Option.Option<E2>
  ) => STM.STM<A, E2 | E, R>
>(2, (self, pf) => rejectSTM(self, (a) => Option.map(pf(a), core.fail)))

/** @internal */
export const rejectSTM = dual<
  <A, E2, R2>(
    pf: (a: A) => Option.Option<STM.STM<E2, E2, R2>>
  ) => <E, R>(self: STM.STM<A, E, R>) => STM.STM<A, E2 | E, R2 | R>,
  <A, E, R, E2, R2>(
    self: STM.STM<A, E, R>,
    pf: (a: A) => Option.Option<STM.STM<E2, E2, R2>>
  ) => STM.STM<A, E2 | E, R2 | R>
...
const repeatUntilLoop = <A, E, R>(
  self: STM.STM<A, E, R>,
  predicate: Predicate<A>
): STM.STM<A, E, R> =>
  core.flatMap(self, (a) =>
    predicate(a) ? core.succeed(a) : repeatUntilLoop(self, predicate)
  )
...
const repeatWhileLoop = <A, E, R>(
  self: STM.STM<A, E, R>,
  predicate: Predicate<A>
): STM.STM<A, E, R> =>
  core.flatMap(self, (a) =>
    predicate(a) ? repeatWhileLoop(self, predicate) : core.succeed(a)
  )
...
>(2, <A, E, R>(self: STM.STM<A, E, R>, predicate: Predicate<A>) =>
  core.matchSTM(self, {
    onFailure: core.fail,
...
export const all = ((
  ...
  return core.map(
    forEach(Object.entries(input), ([_, e]) =>
      core.map(e, (a) => [_, a] as const)
    ),
    (values) => {
      const res = {}
...
/** @internal */
export const succeedNone: STM.STM<Option.Option<never>> = core.succeed(
  Option.none()
)

/** @internal */
export const succeedSome = <A>(value: A): STM.STM<Option.Option<A>> =>
...
export const summarized = dual<
  ...
  ) => <A, E, R>(self: STM.STM<A, E, R>) => STM.STM<[A3, A], E2 | E, R2 | R>,
  <A, E, R, A2, E2, R2, A3>(
    self: STM.STM<A, E, R>,
    summary: STM.STM<A2, E2, R2>,
    f: (before: A2, after: A2) => A3
...
export const suspend = <A, E, R>(
  evaluate: LazyArg<STM.STM<A, E, R>>
): STM.STM<A, E, R> => flatten(core.sync(evaluate))

/** @internal */
export const tap: {
  <A, X, E2, R2>(
    f: (a: A) => STM.STM<X, E2, R2>
  ): <E, R>(self: STM.STM<A, E, R>) => STM.STM<A, E2 | E, R2 | R>
  <A, E, R, X, E2, R2>(
    self: STM.STM<A, E, R>,
    f: (a: A) => STM.STM<X, E2, R2>
  ...
  <A, E, R, X, E2, R2>(
    self: STM.STM<A, E, R>,
    f: (a: A) => STM.STM<X, E2, R2>
  ): STM.STM<A, E | E2, R | R2> => core.flatMap(self, (a) => as(f(a), a))
)
...
export const tapBoth = dual<
  ...
  }) => <R>(self: STM.STM<A, E, R>) => STM.STM<A, E | E2 | E3, R2 | R3 | R>,
...
export const unless = dual<
  (
    predicate: LazyArg<boolean>
  ) => <A, E, R>(self: STM.STM<A, E, R>) => STM.STM<Option.Option<A>, E, R>,
  <A, E, R>(
    self: STM.STM<A, E, R>,
    predicate: LazyArg<boolean>
  ) => STM.STM<Option.Option<A>, E, R>
>(2, (self, predicate) =>
  suspend(() => (predicate() ? succeedNone : asSome(self)))
)
...
>(2, (self, predicate) =>
  core.flatMap(predicate, (bool) => (bool ? succeedNone : asSome(self)))
)

/** @internal */
export const unsome = <A, E, R>(
  self: STM.STM<A, Option.Option<E>, R>
): STM.STM<Option.Option<A>, E, R> =>
  core.matchSTM(self, {
...
export const validateAll = dual<
  <A, B, E, R>(
    f: (a: A) => STM.STM<B, E, R>
  ) => (elements: Iterable<A>) => STM.STM<Array<B>, RA.NonEmptyArray<E>, R>,
  <A, B, E, R>(
    elements: Iterable<A>,
    f: (a: A) => STM.STM<B, E, R>
...
>(2, (elements, f) =>
  core.flatMap(partition(elements, f), ([errors, values]) =>
    RA.isNonEmptyArray(errors) ? core.fail(errors) : core.succeed(values)
  )
)
...
export const validateFirst = dual<
  <A, B, E, R>(
    f: (a: A) => STM.STM<B, E, R>
  ) => (elements: Iterable<A>) => STM.STM<B, Array<E>, R>,
  <A, B, E, R>(
    elements: Iterable<A>,
    f: (a: A) => STM.STM<B, E, R>
  ) => STM.STM<B, Array<E>, R>
...
>(2, (self, predicate) =>
  suspend(() => (predicate() ? asSome(self) : succeedNone))
)
...
>(2, (self, predicate) =>
  core.flatMap(predicate, (bool) => (bool ? asSome(self) : succeedNone))
)
