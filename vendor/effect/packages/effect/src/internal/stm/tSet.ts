...
>(2, (self, other) =>
  core.flatMap(toHashSet(other), (values) =>
    removeIf(self, (value) => HashSet.has(values, value), { discard: true })
  )
)
...
export const forEach = dual<
  ...
  ) => (self: TSet.TSet<A>) => STM.STM<void, E, R>,
  <A, R, E>(
    self: TSet.TSet<A>,
    f: (value: A) => STM.STM<void, E, R>
  ) => STM.STM<void, E, R>
...
>(2, (self, other) =>
  core.flatMap(toHashSet(other), (values) =>
    pipe(
      self,
      retainIf((value) => pipe(values, HashSet.has(value)), { discard: true })
    )
  )
)

/** @internal */
export const isEmpty = <A>(self: TSet.TSet<A>): STM.STM<boolean> =>
  tMap.isEmpty(self.tMap)
...
>(3, (self, zero, f) =>
  tMap.reduce(self.tMap, zero, (acc, _, key) => f(acc, key))
)
...
export const reduceSTM = dual<
  ...
  ) => (self: TSet.TSet<A>) => STM.STM<Z, E, R>,
  <Z, A, R, E>(
    self: TSet.TSet<A>,
    zero: Z,
    f: (accumulator: Z, value: A) => STM.STM<Z, E, R>
...
>(3, (self, zero, f) =>
  tMap.reduceSTM(self.tMap, zero, (acc, _, key) => f(acc, key))
)
...
export const removeIf: {
  ...
  (self, predicate, options) =>
    options?.discard === true
      ? tMap.removeIf(self.tMap, (key) => predicate(key), { discard: true })
      : pipe(
...
export const takeFirstSTM = dual<
  ...
  ) => (self: TSet.TSet<A>) => STM.STM<B, E, R>,
  <A, B, E, R>(
    self: TSet.TSet<A>,
    pf: (a: A) => STM.STM<B, Option.Option<E>, R>
  ) => STM.STM<B, E, R>
...
export const takeSome = dual<
  ...
  ) => (self: TSet.TSet<A>) => STM.STM<RA.NonEmptyArray<B>>,
  <A, B>(
    self: TSet.TSet<A>,
    pf: (a: A) => Option.Option<B>
  ) => STM.STM<RA.NonEmptyArray<B>>
...
export const toHashSet = <A>(self: TSet.TSet<A>): STM.STM<HashSet.HashSet<A>> =>
  reduce(self, HashSet.empty<A>(), (acc, value) =>
    pipe(acc, HashSet.add(value))
  )

/** @internal */
export const toArray = <A>(self: TSet.TSet<A>): STM.STM<Array<A>> =>
  reduce<Array<A>, A>(self, [], (acc, value) => [...acc, value])
...
>(2, (self, f) =>
  tMap.transformSTM(self.tMap, (key, value) =>
    core.map(f(key), (a) => [a, value])
  )
)
