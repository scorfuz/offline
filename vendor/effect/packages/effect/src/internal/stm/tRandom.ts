      ...
      return pipe(
        ...
        stm.forEach(
          (n) =>
            pipe(
              nextIntBounded(n),
              core.flatMap((k) => swap(buffer, n - 1, k))
            ),
          { discard: true }
        ),
...
export const live: Layer.Layer<TRandom.TRandom> = Layer.effect(
  ...
  pipe(
    tRef.make(
      new Random.PCGRandom((Math.random() * 4294967296) >>> 0).getState()
    ),
...
/** @internal */
export const next: STM.STM<number, never, TRandom.TRandom> = core.flatMap(
  Tag,
  (random) => random.next
)

/** @internal */
export const nextBoolean: STM.STM<boolean, never, TRandom.TRandom> =
  core.flatMap(Tag, (random) => random.nextBoolean)

/** @internal */
export const nextInt: STM.STM<number, never, TRandom.TRandom> = core.flatMap(
  Tag,
...
export const nextIntBetween = (
  low: number,
  high: number
): STM.STM<number, never, TRandom.TRandom> =>
...
export const shuffle = <A>(
  elements: Iterable<A>
): STM.STM<Array<A>, never, TRandom.TRandom> =>
