...
/** @internal */
export const acquire = (self: TSemaphore.TSemaphore): STM.STM<void> =>
  acquireN(self, 1)
  ...
  core.withSTMRuntime((driver) => {
    if (n < 0) {
      throw new Cause.IllegalArgumentException(
        `Unexpected negative value ${n} passed to Semaphore.acquireN`
      )
    }
    ...
    if (value < n) {
      ...
      return STM.succeed(
        tRef.unsafeSet(self.permits, value - n, driver.journal)
      )
    }
  })
)
...
/** @internal */
export const release = (self: TSemaphore.TSemaphore): STM.STM<void> =>
  releaseN(self, 1)
...
>(2, (self, n) =>
  core.withSTMRuntime((driver) => {
    if (n < 0) {
      throw new Cause.IllegalArgumentException(
        `Unexpected negative value ${n} passed to Semaphore.releaseN`
      )
    }
    const current = tRef.unsafeGet(self.permits, driver.journal)
    return STM.succeed(
      tRef.unsafeSet(self.permits, current + n, driver.journal)
    )
  })
)
...
>(3, (self, semaphore, permits) =>
  Effect.uninterruptibleMask((restore) =>
    Effect.zipRight(
      ...
      Effect.ensuring(self, core.commit(releaseN(permits)(semaphore)))
    )
  )
)
...
): Effect.Effect<void, never, Scope.Scope> => withPermitsScoped(self, 1)

/** @internal */
export const withPermitsScoped = dual<
  (
    permits: number
  ) => (self: TSemaphore.TSemaphore) => Effect.Effect<void, never, Scope.Scope>,
  (
    self: TSemaphore.TSemaphore,
    permits: number
  ) => Effect.Effect<void, never, Scope.Scope>
>(2, (self, permits) =>
  Effect.acquireReleaseInterruptible(core.commit(acquireN(self, permits)), () =>
