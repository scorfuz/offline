...
export const isManagedRuntime = (
  u: unknown
): u is M.ManagedRuntime<unknown, unknown> => hasProperty(u, circular.TypeId)

function provide<R, ER, A, E>(
  managed: ManagedRuntimeImpl<R, ER>,
  effect: Effect.Effect<A, E, R>
): Effect.Effect<A, E | ER> {
  return core.flatMap(managed.runtimeEffect, (rt) =>
...
export const make = <R, ER>(
  ...
  const self: ManagedRuntimeImpl<R, ER> = Object.assign(
    Object.create(ManagedRuntimeProto),
    {
      memoMap,
      scope,
      runtimeEffect,
      ...
      disposeEffect: core.suspend(() => {
        ;(self as Mutable<ManagedRuntimeImpl<R, ER>>).runtimeEffect = core.die(
      ...
      runPromiseExit<A, E>(
        ...
        return self.cachedRuntime === undefined
          ? internalRuntime.unsafeRunPromiseExitEffect(
              provide(self, effect),
              options
            )
          : internalRuntime.unsafeRunPromiseExit(self.cachedRuntime)(
              effect,
              options
            )
      },
      runCallback<A, E>(
        ...
        return self.cachedRuntime === undefined
          ? internalRuntime.unsafeRunCallback(internalRuntime.defaultRuntime)(
              provide(self, effect),
              options
            )
          : internalRuntime.unsafeRunCallback(self.cachedRuntime)(
              effect,
              options
            )
      },
      runPromise<A, E>(
        ...
        return self.cachedRuntime === undefined
          ? internalRuntime.unsafeRunPromiseEffect(
              provide(self, effect),
              options
            )
          : internalRuntime.unsafeRunPromise(self.cachedRuntime)(
              effect,
              options
            )
      }
