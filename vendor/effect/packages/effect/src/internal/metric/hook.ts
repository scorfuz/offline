...
export const onModify = dual<
  <In, Out>(
    f: (input: In) => void
  ) => (self: MetricHook.MetricHook<In, Out>) => MetricHook.MetricHook<In, Out>,
  <In, Out>(
    self: MetricHook.MetricHook<In, Out>,
    f: (input: In) => void
  ) => MetricHook.MetricHook<In, Out>
>(2, (self, f) => ({
  [MetricHookTypeId]: metricHookVariance,
...
export const counter = <A extends number | bigint>(
  ...
  return make({
    get: () =>
      metricState.counter(
        sum as number
      ) as unknown as MetricState.MetricState.Counter<A>,
    update,
    modify: update
  })
}
...
export const gauge: {
  (
    ...
    startAt: number
  ): MetricHook.MetricHook.Gauge<number>
  (
    key: MetricKey.MetricKey.Gauge<bigint>,
    startAt: bigint
  ): MetricHook.MetricHook.Gauge<bigint>
  ...
  return make({
    get: () =>
      metricState.gauge(
        value as number
      ) as unknown as MetricState.MetricState.Gauge<A>,
    update: (v) => {
      value = v
    },
    modify: (v) => {
...
export const summary = (
  key: MetricKey.MetricKey.Summary
): MetricHook.MetricHook.Summary => {
  ...
  const snapshot = (
    ...
    while (i !== maxSize - 1) {
      ...
      if (item != null) {
        ...
        if (
          Duration.greaterThanOrEqualTo(age, Duration.zero) &&
          Duration.lessThanOrEqualTo(age, maxAge)
        ) {
          builder.push(v)
        }
      }
