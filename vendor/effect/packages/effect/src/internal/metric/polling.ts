...
/** @internal */
export const MetricPollingTypeId: MetricPolling.MetricPollingTypeId =
  Symbol.for(MetricPollingSymbolKey) as MetricPolling.MetricPollingTypeId
...
export const collectAll = <R, E, Out>(
  ...
  return {
    ...
    metric: metric.make(
      ...
      (extraTags) =>
        Array.from(
          metrics.map((pollingMetric) =>
            pollingMetric.metric.unsafeValue(extraTags)
          )
        ),
      (inputs: Array<any>, extraTags) => {
        for (let i = 0; i < inputs.length; i++) {
...
export const pollAndUpdate = <Type, In, R, E, Out>(
  ...
  core.flatMap(self.poll, (value) => metric.update(self.metric, value))
