...
/** @internal */
export const OrPatchTypeId: Differ.Or.TypeId = Symbol.for(
  "effect/DifferOrPatch"
) as Differ.Or.TypeId
...
/** @internal */
export const diff = <Value, Value2, Patch, Patch2>(options: {
  ...
  switch (options.oldValue._tag) {
    case "Left": {
      switch (options.newValue._tag) {
        case "Left": {
          const valuePatch = options.left.diff(
            options.oldValue.left,
            options.newValue.left
          )
    ...
    case "Right": {
      switch (options.newValue._tag) {
        ...
        case "Right": {
          const valuePatch = options.right.diff(
            options.oldValue.right,
            options.newValue.right
          )
...
>(
  ...
  <Value, Value2, Patch, Patch2>(
    ...
    let patches: Chunk.Chunk<Differ.Or.Patch<Value, Value2, Patch, Patch2>> =
      Chunk.of(self)
    let result = oldValue
