...
): Effect.Effect<Option.Option<A>, E, R> => core.map(self, Option.some)

/* @internal */
export const asSomeError = <A, E, R>(
  self: Effect.Effect<A, E, R>
): Effect.Effect<A, Option.Option<E>, R> => core.mapError(self, Option.some)
...
>(
  ...
  <A, E, R, A2, E2, R2>(
    ...
    core.catchAllCause(self, (cause): Effect.Effect<A | A2, E | E2, R | R2> => {
      const option = internalCause.find(cause, (_) =>
        internalCause.isDieType(_) ? Option.some(_) : Option.none()
      )
      switch (option._tag) {
        case "None": {
...
export const catchSomeCause: {
  <E, A2, E2, R2>(
    f: (
      cause: Cause.Cause<Types.NoInfer<E>>
    ) => Option.Option<Effect.Effect<A2, E2, R2>>
  ): <A, R>(
    self: Effect.Effect<A, E, R>
...
export const catchTag: {
  ...
  >(
    ...args: [
      ...tags: K,
      f: (
        e: Extract<Types.NoInfer<E>, { _tag: K[number] }>
      ) => Effect.Effect<A1, E1, R1>
    ]
  ): <A, R>(
    self: Effect.Effect<A, E, R>
  ...
  >(
    ...
    return core.catchIf(
      self,
      predicate as Predicate.Refinement<E, Extract<E, { _tag: K[number] }>>,
      f
    ) as any
  }
) as any
...
export const cause = <A, E, R>(
  self: Effect.Effect<A, E, R>
): Effect.Effect<Cause.Cause<E>, never, R> =>
  core.matchCause(self, {
    onFailure: identity,
...
) => Effect.Effect<A, E, R> = Clock.clockWith
...
export const descriptorWith = <A, E, R>(
  ...
  core.withFiberRuntime((state, status) =>
    f({
      id: state.id(),
      status,
      interruptors: internalCause.interruptors(
        state.getFiberRef(core.currentInterruptedCause)
      )
    })
...
export const allowInterrupt: Effect.Effect<void> = descriptorWith(
  (descriptor) =>
    HashSet.size(descriptor.interruptors) > 0 ? core.interrupt : core.void
)

/* @internal */
export const descriptor: Effect.Effect<Fiber.Fiber.Descriptor> = descriptorWith(
  core.succeed
)
...
export const diffFiberRefs = <A, E, R>(
  ...
  summarized(self, fiberRefs, fiberRefsPatch.diff)

/* @internal */
export const diffFiberRefsAndRuntimeFlags = <A, E, R>(
  self: Effect.Effect<A, E, R>
): Effect.Effect<
  [[FiberRefsPatch.FiberRefsPatch, runtimeFlagsPatch.RuntimeFlagsPatch], A],
  E,
  R
> =>
  summarized(
    ...
    ([refs, flags], [refsNew, flagsNew]) => [
      fiberRefsPatch.diff(refs, refsNew),
      runtimeFlags.diff(flags, flagsNew)
    ]
  )
...
): Effect.Effect<A, never, R> => core.map(core.context<R>(), f)
...
export const filterMap = dual<
  ...
  >
>(2, (elements, pf) =>
  core.map(core.forEachSequential(elements, identity), Arr.filterMap(pf))
)
...
export const filterOrDie: {
  ...
  <A, E, R>(
    ...
    filterOrElse(self, predicate, (a) => core.dieSync(() => orDieWith(a)))
)
...
export const filterOrElse: {
  <A, C, E2, R2, B extends A>(
    refinement: Predicate.Refinement<Types.NoInfer<A>, B>,
    orElse: (
      a: Types.EqualsWith<A, B, Types.NoInfer<A>, Exclude<Types.NoInfer<A>, B>>
  ...
  <A, E, R, C, E2, R2, B extends A>(
    ...
    refinement: Predicate.Refinement<A, B>,
    orElse: (
      a: Types.EqualsWith<A, B, A, Exclude<A, B>>
    ) => Effect.Effect<C, E2, R2>
  ...
  <A, E, R, B, E2, R2>(
    ...
    core.flatMap(self, (a) =>
      predicate(a) ? core.succeed<A | B>(a) : orElse(a)
    )
)
...
>(
  ...
  <A, E, B extends A = A>(
    ...
    core.suspend(() =>
      predicate(self)
        ? core.succeed(self as B)
        : core.fail(orFailWith(self as any))
    )
)
...
export const filterOrFail: {
  ...
  <A, B extends A>(
    ...
    self: Effect.Effect<A, E, R>
  ) => Effect.Effect<Types.NoInfer<B>, Cause.NoSuchElementException | E, R>
  <A>(
    predicate: Predicate.Predicate<Types.NoInfer<A>>
  ): <E, R>(
    self: Effect.Effect<A, E, R>
  ) => Effect.Effect<A, Cause.NoSuchElementException | E, R>
  <A, E, R, B extends A>(
    self: Effect.Effect<A, E, R>,
    refinement: Predicate.Refinement<A, B>
  ...
  <A, E, R, E2>(
    ...
    filterOrElse(
      ...
      (a): Effect.Effect<never, E2 | Cause.NoSuchElementException, never> =>
        orFailWith === undefined
          ? core.fail(new core.NoSuchElementException())
          : core.failSync(() => orFailWith(a))
    )
)
  ...
  core.suspend(() => {
    ...
    return pipe(
      ...
      Arr.reduce(
        ...
        (left, right) => core.orElse(left, () => right) as Eff
      )
    )
  })
...
export const every: {
  ...
  <A, E, R>(
    ...
    core.suspend(() => forAllLoop(elements[Symbol.iterator](), 0, predicate))
)
...
const forAllLoop = <A, E, R>(
  ...
  return next.done
    ? core.succeed(true)
    : core.flatMap(f(next.value, index), (b) =>
        b ? forAllLoop(iterator, index + 1, f) : core.succeed(b)
      )
}
...
export const forever = <A, E, R>(
  ...
  const loop: Effect.Effect<never, E, R> = core.flatMap(
    core.flatMap(self, () => core.yieldNow()),
    () => loop
  )
  return loop
}
...
/* @internal */
export const fiberRefs: Effect.Effect<FiberRefs.FiberRefs> =
  core.withFiberRuntime((state) => core.succeed(state.getFiberRefs()))
...
) => Effect.Effect<void, never, never> = logWithLevel()

/** @internal */
export const logTrace: (
  ...message: ReadonlyArray<any>
) => Effect.Effect<void, never, never> = logWithLevel(LogLevel.Trace)

/** @internal */
export const logDebug: (
  ...message: ReadonlyArray<any>
) => Effect.Effect<void, never, never> = logWithLevel(LogLevel.Debug)

/** @internal */
export const logInfo: (
  ...message: ReadonlyArray<any>
) => Effect.Effect<void, never, never> = logWithLevel(LogLevel.Info)

/** @internal */
export const logWarning: (
  ...message: ReadonlyArray<any>
) => Effect.Effect<void, never, never> = logWithLevel(LogLevel.Warning)

/** @internal */
export const logError: (
  ...message: ReadonlyArray<any>
) => Effect.Effect<void, never, never> = logWithLevel(LogLevel.Error)

/** @internal */
export const logFatal: (
  ...message: ReadonlyArray<any>
) => Effect.Effect<void, never, never> = logWithLevel(LogLevel.Fatal)
...
/* @internal */
export const logAnnotations: Effect.Effect<HashMap.HashMap<string, unknown>> =
  ...
  core.suspend(() =>
    cont(initial)
      ? core.flatMap(body(initial), (a) =>
          core.map(loopInternal(inc(initial), cont, inc, body), List.prepend(a))
        )
          ...
          core.zipRight(
            ...
            pipe(
              ...
              core.flatMap(([patch, a]) =>
                core.as(
                  core.zip(
                    patchFiberRefs(patch[0]),
                    core.updateRuntimeFlags(patch[1])
                  ),
                  a
                )
              )
...
): Effect.Effect<boolean, E, R> => core.map(self, (b) => !b)
...
export const once = <A, E, R>(
  ...
  core.map(Ref.make(true), (ref) =>
    core.asVoid(core.whenEffect(self, Ref.getAndSet(ref, false)))
  )
...
export const patchFiberRefs = (
  ...
  updateFiberRefs((fiberId, fiberRefs) =>
    pipe(patch, fiberRefsPatch.patch(fiberId, fiberRefs))
  )
...
export const promise = <A>(
  ...
  evaluate.length >= 1
    ...
    : core.async((resolve) => {
        try {
          ;(evaluate as LazyArg<PromiseLike<A>>)().then(
            (a) => resolve(core.succeed(a)),
            (e) => resolve(core.die(e))
          )
        } catch (e) {
  ...
  <A, E, R, I, S, E1, R1>(
    ...
    core.contextWithEffect((env: Context.Context<R1 | Exclude<R, I>>) =>
      core.flatMap(effect, (service) =>
        core.provideContext(
          self,
          pipe(env, Context.add(tag, service)) as Context.Context<R | R1>
        )
      )
    )
)
...
/* @internal */
export const random: Effect.Effect<Random.Random> = defaultServices.randomWith(
  core.succeed
)
...
const reduceWhileLoop = <A, R, E, Z>(
  ...
  if (!next.done && predicate(state)) {
    return core.flatMap(f(state, next.value, index), (nextState) =>
      reduceWhileLoop(iterator, index + 1, nextState, predicate, f)
    )
  }
...
/* @internal */
export const setFiberRefs = (
  fiberRefs: FiberRefs.FiberRefs
): Effect.Effect<void> => core.suspend(() => FiberRefs.setAll(fiberRefs))
...
/* @internal */
export const succeedNone: Effect.Effect<Option.Option<never>> = core.succeed(
  Option.none()
)

/* @internal */
export const succeedSome = <A>(value: A): Effect.Effect<Option.Option<A>> =>
  ...
  <A, E, R, B, E2, R2, C>(
    ...
    core.flatMap(summary, (start) =>
      core.flatMap(self, (value) =>
        core.map(summary, (end) => [f(start, end), value])
      )
    )
)
  ...
  function () {
    return labelMetrics(
      ...
      typeof arguments[1] === "string"
        ...
        : Object.entries<string>(arguments[1]).map(([k, v]) =>
            metricLabel.make(k, v)
          )
    )
  }
)
...
>(2, (self, labels) =>
  core.fiberRefLocallyWith(self, core.currentMetricLabels, (old) =>
    Arr.union(old, labels)
  )
)
...
export const timed = <A, E, R>(
  ...
  timedWith(self, Clock.currentTimeNanos)
...
>(2, (self, nanos) =>
  summarized(self, nanos, (start, end) => Duration.nanos(end - start))
)

/* @internal */
export const tracerWith: <A, E, R>(
  f: (tracer: Tracer.Tracer) => Effect.Effect<A, E, R>
) => Effect.Effect<A, E, R> = Tracer.tracerWith
...
>(2, (self, condition) =>
  core.suspend(() => (condition() ? succeedNone : asSome(self)))
)
...
>(2, (self, condition) =>
  core.flatMap(condition, (b) => (b ? succeedNone : asSome(self)))
)
...
>(
  ...
  <A, E, R, I, S>(
    ...
    core.mapInputContext(self, (context) =>
      Context.add(context, tag, f(Context.unsafeGet(context, tag)))
...
>(2, (self, condition) =>
  core.suspend(() =>
    condition() ? core.map(self, Option.some) : core.succeed(Option.none())
  )
)
...
export const serviceFunctionEffect =
  <T extends Effect.Effect<any, any, any>, Args extends Array<any>, A, E, R>(
    getService: T,
    f: (
      _: Effect.Effect.Success<T>
  ...
  (
    ...
    A,
    E | Effect.Effect.Error<T>,
    R | Effect.Effect.Context<T>
  > =>
    core.flatMap(getService, (a) => f(a)(...args))

/** @internal */
export const serviceFunction =
  <T extends Effect.Effect<any, any, any>, Args extends Array<any>, A>(
    getService: T,
    f: (_: Effect.Effect.Success<T>) => (...args: Args) => A
  ) =>
  (
    ...args: Args
  ): Effect.Effect<A, Effect.Effect.Error<T>, Effect.Effect.Context<T>> =>
  ...
  new Proxy({} as any, {
    get(_target: any, prop: any, _receiver) {
      return (...args: Array<any>) =>
        core.flatMap(getService, (s: any) => s[prop](...args))
    }
  })
...
export const serviceConstants = <S, SE, SR>(
  ...
  [k in { [k in keyof S]: k }[keyof S]]: S[k] extends Effect.Effect<
    infer A,
    infer E,
    infer R
  >
  ...
  new Proxy({} as any, {
    get(_target: any, prop: any, _receiver) {
      return core.flatMap(getService, (s: any) =>
        core.isEffect(s[prop]) ? s[prop] : core.succeed(s[prop])
      )
    }
  })
...
export const annotateCurrentSpan: {
  ...
  const args = arguments
  return ignore(
    core.flatMap(currentPropagatedSpan, (span) =>
      core.sync(() => {
...
export const linkSpanCurrent: {
  ...
  return ignore(
    core.flatMap(currentSpan, (span) => core.sync(() => span.addLinks(links)))
  )
}
...
> = serviceOptional(internalTracer.spanTag)

/** @internal */
export const currentSpan: Effect.Effect<
  Tracer.Span,
  Cause.NoSuchElementException
> = core.flatMap(core.context<never>(), (context) => {
  const span = context.unsafeMap.get(internalTracer.spanTag.key) as
    | Tracer.AnySpan
    | undefined
...
export const currentPropagatedSpan: Effect.Effect<
  Tracer.Span,
  Cause.NoSuchElementException
> = core.flatMap(core.context<never>(), (context) => {
...
export const filterDisablePropagation: (
  self: Option.Option<Tracer.AnySpan>
) => Option.Option<Tracer.AnySpan> = Option.flatMap((span) =>
...
export const unsafeMakeSpan = <XA, XE>(
  ...
  const disablePropagation =
    !fiber.getFiberRef(core.currentTracerEnabled) ||
    (options.context &&
      Context.get(options.context, internalTracer.DisablePropagation))
  ...
  if (disablePropagation) {
    span = core.noopSpan({
      ...
      context: Context.add(
        ...
        internalTracer.DisablePropagation,
        true
      )
    })
  } else {
    ...
    const annotationsFromEnv = FiberRefs.get(
      fiberRefs,
      core.currentTracerSpanAnnotations
    )
    ...
    if (annotationsFromEnv._tag === "Some") {
      HashMap.forEach(annotationsFromEnv.value, (value, key) =>
...
export const makeSpan = (
  ...
  return core.withFiberRuntime((fiber) =>
    core.succeed(unsafeMakeSpan(fiber, name, options))
  )
}

/* @internal */
export const spanAnnotations: Effect.Effect<HashMap.HashMap<string, unknown>> =
  core.fiberRefGet(core.currentTracerSpanAnnotations)

/* @internal */
export const spanLinks: Effect.Effect<Chunk.Chunk<Tracer.SpanLink>> =
...
export const withSpan: {
  ...
  return (self: Effect.Effect<any, any, any>) =>
    useSpan(name, options, (span) => withParentSpan(self, span))
} as any

export const functionWithSpan = <
  Args extends Array<any>,
  Ret extends Effect.Effect<any, any, any>
>(options: {
  ...
  function (this: any) {
    let captureStackTrace: LazyArg<string | undefined> | boolean =
    ...
    return core.suspend(() => {
      ...
      return withSpan(
        core.suspend(() =>
          internalCall(() => options.body.apply(this, arguments as any))
        ),
        opts.name,
        {
          ...opts,
...
export const fromNullable = <A>(
  value: A
): Effect.Effect<NonNullable<A>, Cause.NoSuchElementException> =>
  value == null
    ? core.fail(new core.NoSuchElementException())
    : core.succeed(value as NonNullable<A>)
...
export const optionFromOptional = <A, E, R>(
  ...
  R
> =>
  core.catchAll(core.map(self, Option.some), (error) =>
