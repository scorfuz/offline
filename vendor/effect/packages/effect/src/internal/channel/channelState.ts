...
/** @internal */
export interface ChannelState<out E, out R> extends ChannelState.Variance<
  E,
  R
> {}
...
/** @internal */
export type Primitive = Done | Emit | FromEffect | Read

/** @internal */
export type Op<Tag extends string, Body = {}> = ChannelState<never, never> &
  Body & {
    readonly _tag: Tag
  }
...
export const Read = <R>(
  ...
  onDone: (
    exit: Exit.Exit<unknown, unknown>
  ) => Effect.Effect<void, never, R> | undefined
...
/** @internal */
export const isChannelState = (
  u: unknown
): u is ChannelState<unknown, unknown> => hasProperty(u, ChannelStateTypeId)

/** @internal */
export const isDone = <E, R>(self: ChannelState<E, R>): self is Done =>
  (self as Primitive)._tag === OpCodes.OP_DONE

/** @internal */
export const isEmit = <E, R>(self: ChannelState<E, R>): self is Emit =>
  (self as Primitive)._tag === OpCodes.OP_EMIT

/** @internal */
export const isFromEffect = <E, R>(
  self: ChannelState<E, R>
): self is FromEffect => (self as Primitive)._tag === OpCodes.OP_FROM_EFFECT

/** @internal */
export const isRead = <E, R>(self: ChannelState<E, R>): self is Read =>
  (self as Primitive)._tag === OpCodes.OP_READ

/** @internal */
export const effect = <E, R>(
  self: ChannelState<E, R>
): Effect.Effect<void, E, R> =>
  isFromEffect(self) ? (self.effect as Effect.Effect<void, E, R>) : Effect.void

/** @internal */
export const effectOrUndefinedIgnored = <E, R>(
  self: ChannelState<E, R>
): Effect.Effect<void, E, R> | undefined =>
  isFromEffect(self)
    ? Effect.ignore(self.effect as Effect.Effect<void, E, R>)
    : undefined
