...
interface Emit<Err, Elem, Done> {
  ...
  readonly notifyConsumers: ReadonlyArray<
    Deferred.Deferred<Either.Either<Elem, Done>, Err>
  >
}
...
/** @internal */
const stateEmpty = (
  notifyProducer: Deferred.Deferred<void>
): State<never, never, never> => ({
  _tag: OP_STATE_EMPTY,
  notifyProducer
})
...
/** @internal */
const stateError = <Err>(
  cause: Cause.Cause<Err>
): State<Err, never, never> => ({
  _tag: OP_STATE_ERROR,
  cause
})
...
class SingleProducerAsyncInputImpl<
  ...
  in out Elem,
  in out Done
> implements SingleProducerAsyncInput.SingleProducerAsyncInput<
  Err,
  Elem,
  Done
> {
    ...
    return Effect.flatten(
      Ref.modify(this.ref, (state) =>
        state._tag === OP_STATE_EMPTY
          ? [
              Deferred.await(state.notifyProducer),
              state as State<Err, Elem, Done>
            ]
          : [Effect.void, state]
      )
    )
  ...
  takeWith<A>(
    ...
    return Effect.flatMap(
      Deferred.make<Either.Either<Elem, Done>, Err>(),
      (deferred) =>
        Effect.flatten(
          Ref.modify(this.ref, (state) => {
            switch (state._tag) {
...
> =>
  pipe(
    ...
    Effect.flatMap((deferred) =>
      Ref.make(stateEmpty(deferred) as State<Err, Elem, Done>)
    ),
