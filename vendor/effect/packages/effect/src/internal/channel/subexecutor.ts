...
export interface Subexecutor<in out R> {
  ...
  ): Effect.Effect<unknown, never, R> | undefined
  enqueuePullFromChild(child: PullFromChild<R>): Subexecutor<R>
}

/** @internal */
export type Primitive<Env> =
  | PullFromChild<Env>
  | PullFromUpstream<Env>
  | DrainChildExecutors<Env>
  | Emit<Env>
  ...
  close(
    ...
    const fins = [
      ...this.activeChildExecutors.map((child) =>
        child !== undefined ? child.childExecutor.close(exit) : undefined
      ),
      fin1
    ]
    const result = fins.reduce(
      (
        acc: Effect.Effect<Exit.Exit<unknown, unknown>, never, R> | undefined,
        next
      ) => {
        if (acc !== undefined && next !== undefined) {
          return Effect.zipWith(acc, Effect.exit(next), (exit1, exit2) =>
            Exit.zipRight(exit1, exit2)
          )
