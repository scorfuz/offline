...
export class ChannelExecutor<
  ...
  in out Env = never
> {
  private _activeSubexecutor: Subexecutor.Subexecutor<Env> | undefined =
    undefined

  private _cancelled: Exit.Exit<OutErr, OutDone> | undefined = undefined

  private _closeLastSubstream: Effect.Effect<unknown, never, Env> | undefined =
    undefined
  ...
  private _inProgressFinalizer: Effect.Effect<unknown, never, Env> | undefined =
    undefined
        ...
        try {
          if (this._currentChannel === undefined) {
            ...
            if (Effect.isEffect(this._currentChannel)) {
              this._currentChannel = core.fromEffect(
            ...
            switch (this._currentChannel._tag) {
              ...
              case ChannelOpCodes.OP_BRIDGE: {
                ...
                this._currentChannel = this._currentChannel
                  .channel as core.Primitive
                ...
                if (this._input !== undefined) {
                  ...
                  const drainer = (): Effect.Effect<unknown, never, Env> =>
                    Effect.flatMap(bridgeInput.awaitRead(), () =>
                      Effect.suspend(() => {
                        const state =
                          inputExecutor.run() as ChannelState.Primitive
                        switch (state._tag) {
              ...
              case ChannelOpCodes.OP_EMIT: {
                ...
                this._currentChannel = (
                  this._activeSubexecutor !== undefined ? undefined : core.void
                ) as core.Primitive | undefined
              ...
              case ChannelOpCodes.OP_FOLD: {
                this._doneStack.push(
                  this._currentChannel.k as ErasedContinuation<Env>
                )
                this._currentChannel = this._currentChannel
                  .channel as core.Primitive
                break
                  ...
                  Effect.matchCauseEffect(effect, {
                    onFailure: (cause) => {
                      ...
                      return state !== undefined &&
                        ChannelState.isFromEffect(state)
                        ? state.effect
                        : Effect.void
                    },
                    onSuccess: (value) => {
              ...
              case ChannelOpCodes.OP_PIPE_TO: {
                ...
                this._currentChannel =
                  this._currentChannel.right() as core.Primitive
              ...
              case ChannelOpCodes.OP_READ: {
                ...
                result = ChannelState.Read(
                  ...
                  (emitted) => {
                    try {
                      this._currentChannel = read.more(
                        emitted
                      ) as core.Primitive
                    } catch (error) {
              ...
              case ChannelOpCodes.OP_SUSPEND: {
                this._currentChannel =
                  this._currentChannel.channel() as core.Primitive
                break
              }
            }
          }
        } catch (error) {
          this._currentChannel = core.failCause(
            Cause.die(error)
          ) as core.Primitive
        }
      }
  ...
  storeInProgressFinalizer(
    finalizer: Effect.Effect<unknown, never, Env> | undefined
  ): void {
  ...
  popAllFinalizers(
    ...
    const effect = (
      finalizers.length === 0 ? Effect.void : runFinalizers(finalizers, exit)
    ) as Effect.Effect<unknown, never, Env>
    this.storeInProgressFinalizer(effect)
    return effect
  }

  popNextFinalizers(): Array<
    Continuation.ContinuationFinalizer<Env, unknown, unknown>
  > {
    const builder: Array<
      Continuation.ContinuationFinalizer<Env, unknown, unknown>
    > = []
    while (this._doneStack.length !== 0) {
      const cont = this._doneStack[
        this._doneStack.length - 1
      ] as Continuation.Primitive
      if (cont._tag === ContinuationOpCodes.OP_CONTINUATION_K) {
        return builder
      }
      builder.push(
        cont as Continuation.ContinuationFinalizer<Env, unknown, unknown>
      )
      this._doneStack.pop()
    }
    return builder
  ...
  close(
    ...
    return pipe(
      ...
      Effect.map(([[exit1, exit2], exit3]) =>
        pipe(exit1, Exit.zipRight(exit2), Exit.zipRight(exit3))
      ),
  ...
  runBracketOut(
    bracketOut: core.BracketOut
  ): ChannelState.ChannelState<unknown, Env> {
    const effect = Effect.uninterruptible(
      Effect.matchCauseEffect(
        this.provide(
          bracketOut.acquire() as Effect.Effect<OutDone, OutErr, Env>
        ),
        {
          onFailure: (cause) =>
            Effect.sync(() => {
    ...
    return ChannelState.fromEffect(effect) as ChannelState.ChannelState<
      unknown,
      Env
    >
  }
          ...
          (closeFunc) =>
            pipe(
              ...
              Effect.flatMap((closeEffect) =>
                closeEffect !== undefined ? closeEffect : Effect.void
              )
            ),
          { discard: true }
        )
  ...
  applyUpstreamPullStrategy(
    ...
    ReadonlyArray<Subexecutor.PullFromChild<Env> | undefined>
  ] {
    switch (strategy._tag) {
      case UpstreamPullStrategyOpCodes.OP_PULL_AFTER_NEXT: {
        const shouldPrepend =
          !upstreamFinished ||
          queue.some((subexecutor) => subexecutor !== undefined)
        return [
          strategy.emitSeparator,
          shouldPrepend ? [undefined, ...queue] : queue
        ]
      }
      case UpstreamPullStrategyOpCodes.OP_PULL_AFTER_ALL_ENQUEUED: {
        ...
        return [
          strategy.emitSeparator,
          shouldEnqueue ? [...queue, undefined] : queue
        ]
      }
    }
  }
  ...
  pullFromChild(
    ...
    return ChannelState.Read(
      ...
      (emitted) => {
        ...
        this._activeSubexecutor = new Subexecutor.Emit(
          emitted,
      ...
      (exit) => {
        if (
          subexecutor.activeChildExecutors.some(
            (subexecutor) => subexecutor !== undefined
          )
        ) {
        ...
        return state === undefined
          ...
          : // NOTE: assuming finalizers cannot fail
            ChannelState.effectOrUndefinedIgnored(
              state as ChannelState.ChannelState<never, Env>
            )
      }
    )
  }
        ...
        this.applyUpstreamPullStrategy(
          ...
          subexecutor.onPull(
            upstreamPullRequest.NoUpstream(
              rest.reduce((n, curr) => (curr !== undefined ? n + 1 : n), 0)
            )
          )
        )
...
const ifNotNull = <Env>(
  effect: Effect.Effect<unknown, never, Env> | undefined
): Effect.Effect<unknown, never, Env> =>
  ...
  const read = (): Effect.Effect<A, E2, R> => {
    ...
    switch (state._tag) {
      ...
      case ChannelStateOpCodes.OP_FROM_EFFECT: {
        ...
        return pipe(
          current.onEffect(
            state.effect as Effect.Effect<void>
    ...
    const run = (
      ...
      Effect.acquireUseRelease(
        ...
        (exec, exit) => {
          ...
          return Effect.tapErrorCause(finalize, (cause) =>
            Scope.addFinalizer(scope, Effect.failCause(cause))
          )
        }
      )
        ...
        Effect.flatMap(([child, channelDeferred, scopeDeferred]) =>
          restore(run(channelDeferred, scopeDeferred, child)).pipe(
            ...
            Effect.flatMap((fiber) =>
              scope
                .addFinalizer((exit) => {
                  const interruptors = Exit.isFailure(exit)
                        ...
                        : Deferred.succeed(scopeDeferred, void 0).pipe(
                            Effect.zipRight(
                              interruptors && HashSet.size(interruptors) > 0
                                ? Fiber.interruptAs(
                                    fiber,
                                    FiberId.combineAll(interruptors)
                                  )
