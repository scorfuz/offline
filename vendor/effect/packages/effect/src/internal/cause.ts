...
/** @internal */
export const isCause = (u: unknown): u is Cause.Cause<unknown> =>
  hasProperty(u, CauseTypeId)

/** @internal */
export const isEmptyType = <E>(self: Cause.Cause<E>): self is Cause.Empty =>
  self._tag === OpCodes.OP_EMPTY

/** @internal */
export const isFailType = <E>(self: Cause.Cause<E>): self is Cause.Fail<E> =>
  self._tag === OpCodes.OP_FAIL

/** @internal */
export const isDieType = <E>(self: Cause.Cause<E>): self is Cause.Die =>
  self._tag === OpCodes.OP_DIE

/** @internal */
export const isInterruptType = <E>(
  self: Cause.Cause<E>
): self is Cause.Interrupt => self._tag === OpCodes.OP_INTERRUPT

/** @internal */
export const isSequentialType = <E>(
  self: Cause.Cause<E>
): self is Cause.Sequential<E> => self._tag === OpCodes.OP_SEQUENTIAL

/** @internal */
export const isParallelType = <E>(
  self: Cause.Cause<E>
): self is Cause.Parallel<E> => self._tag === OpCodes.OP_PARALLEL
...
export const size = <E>(self: Cause.Cause<E>): number =>
  reduceWithContext(self, void 0, SizeCauseReducer)
...
export const failures = <E>(self: Cause.Cause<E>): Chunk.Chunk<E> =>
  Chunk.reverse(
    reduce<Chunk.Chunk<E>, E>(self, Chunk.empty<E>(), (list, cause) =>
...
export const interruptors = <E>(
  self: Cause.Cause<E>
): HashSet.HashSet<FiberId.FiberId> =>
...
export const failureOption = <E>(self: Cause.Cause<E>): Option.Option<E> =>
  find<E, E>(self, (cause) =>
    cause._tag === OpCodes.OP_FAIL ? Option.some(cause.error) : Option.none()
  )

/** @internal */
export const failureOrCause = <E>(
  self: Cause.Cause<E>
): Either.Either<Cause.Cause<never>, E> => {
...
export const dieOption = <E>(self: Cause.Cause<E>): Option.Option<unknown> =>
  find(self, (cause) =>
    cause._tag === OpCodes.OP_DIE ? Option.some(cause.defect) : Option.none()
  )
...
/** @internal */
export const keepDefects = <E>(
  self: Cause.Cause<E>
): Option.Option<Cause.Cause<never>> =>
  match(self, {
    onEmpty: Option.none(),
    onFail: () => Option.none(),
    onDie: (defect) => Option.some(die(defect)),
...
/** @internal */
export const keepDefectsAndElectFailures = <E>(
  self: Cause.Cause<E>
): Option.Option<Cause.Cause<never>> =>
  match(self, {
    onEmpty: Option.none(),
...
/** @internal */
export const linearize = <E>(
  self: Cause.Cause<E>
): HashSet.HashSet<Cause.Cause<E>> =>
  match(self, {
    ...
    onSequential: (leftSet, rightSet) =>
      HashSet.flatMap(leftSet, (leftCause) =>
        HashSet.map(rightSet, (rightCause) => sequential(leftCause, rightCause))
      ),
    onParallel: (leftSet, rightSet) =>
      HashSet.flatMap(leftSet, (leftCause) =>
        HashSet.map(rightSet, (rightCause) => parallel(leftCause, rightCause))
      )
  })
...
export const stripSomeDefects = dual<
  ...
  ) => <E>(self: Cause.Cause<E>) => Option.Option<Cause.Cause<E>>,
  <E>(
    self: Cause.Cause<E>,
    pf: (defect: unknown) => Option.Option<unknown>
  ) => Option.Option<Cause.Cause<E>>
...
export const andThen: {
  ...
  <E, E2>(
    self: Cause.Cause<E>,
    f: ((e: E) => Cause.Cause<E2>) | Cause.Cause<E2>
  ): Cause.Cause<E2> =>
...
/** @internal */
const causeEquals = (
  left: Cause.Cause<unknown>,
  right: Cause.Cause<unknown>
): boolean => {
  ...
  while (Chunk.isNonEmpty(leftStack) && Chunk.isNonEmpty(rightStack)) {
    const [leftParallel, leftSequential] = pipe(
      ...
      reduce(
        [
          HashSet.empty<unknown>(),
          Chunk.empty<Cause.Cause<unknown>>()
        ] as const,
...
export const find = dual<
  <E, Z>(
    pf: (cause: Cause.Cause<E>) => Option.Option<Z>
  ) => (self: Cause.Cause<E>) => Option.Option<Z>,
  <E, Z>(
    self: Cause.Cause<E>,
    pf: (cause: Cause.Cause<E>) => Option.Option<Z>
  ) => Option.Option<Z>
...
export const filter: {
  ...
  <E>(
    predicate: Predicate<Cause.Cause<NoInfer<E>>>
  ): (self: Cause.Cause<E>) => Cause.Cause<E>
  <E, EB extends E>(
    self: Cause.Cause<E>,
    refinement: Refinement<Cause.Cause<E>, Cause.Cause<EB>>
  ): Cause.Cause<EB>
  <E>(
    self: Cause.Cause<E>,
...
const evaluateCause = (
  ...
  while (cause !== undefined) {
    switch (cause._tag) {
      ...
      case OpCodes.OP_INTERRUPT: {
        _parallel = HashSet.add(
          _parallel,
          Chunk.make(cause._tag, cause.fiberId as unknown)
        )
        if (stack.length === 0) {
          return [_parallel, _sequential]
        }
...
/** @internal */
const IsInterruptedOnlyCauseReducer: Cause.CauseReducer<
  unknown,
  unknown,
  boolean
> = {
...
>(
  2,
  (self, { onDie, onEmpty, onFail, onInterrupt, onParallel, onSequential }) => {
...
export const reduceWithContext = dual<
  <C, E, Z>(
    context: C,
    reducer: Cause.CauseReducer<C, E, Z>
  ) => (self: Cause.Cause<E>) => Z,
  <C, E, Z>(
    self: Cause.Cause<E>,
    context: C,
  ...
  <C, E, Z>(
    ...
    while (input.length > 0) {
      ...
      switch (cause._tag) {
        ...
        case OpCodes.OP_INTERRUPT: {
          output.push(
            Either.right(reducer.interruptCase(context, cause.fiberId))
          )
          break
        }
        case OpCodes.OP_SEQUENTIAL: {
...
export const makePrettyError = (originalError: unknown): Cause.PrettyError => {
  ...
  const error = new Error(
    ...
    originalErrorIsObject &&
      ...
      typeof originalError.cause !== "undefined"
...
/** @internal */
export const spanToTrace = globalValue(
  "effect/Tracer/spanToTrace",
  () => new WeakMap()
)

const prettyErrorStack = (
  message: string,
  stack: string,
  span?: Span | undefined
): string => {
  const out: Array<string> = [message]
  const lines = stack.startsWith(message)
...
/** @internal */
export const prettyErrors = <E>(
  cause: Cause.Cause<E>
): Array<Cause.PrettyError> =>
