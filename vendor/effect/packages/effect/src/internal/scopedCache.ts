  ...
  Exit.matchEffect(self.exit, {
    ...
    onSuccess: ([value]) =>
      fiberRuntime.acquireRelease(
        core.as(
          core.sync(() => MutableRef.incrementAndGet(self.ownerCount)),
          value
        ),
        () => releaseOwner(self)
      )
...
> implements ScopedCache.ScopedCache<Key, Value, Error> {
  ...
  getOptionComplete(
    key: Key
  ): Effect.Effect<Option.Option<Value>, never, Scope.Scope> {
    return core.suspend(() =>
      Option.match(MutableHashMap.get(this.cacheState.map, key), {
        onNone: () => effect.succeedNone,
        onSome: (value) =>
          core.flatten(this.resolveMapValue(value, true)) as Effect.Effect<
            Option.Option<Value>,
            never,
            Scope.Scope
          >
  ...
  entryStats(key: Key): Effect.Effect<Option.Option<Cache.EntryStats>> {
    return core.sync(() => {
      ...
      switch (value._tag) {
        case "Complete": {
          return Option.some(
            cache_.makeEntryStats(value.entryStats.loadedMillis)
          )
        }
        case "Pending": {
          return Option.none()
        ...
        case "Refreshing": {
          return Option.some(
            cache_.makeEntryStats(value.complete.entryStats.loadedMillis)
          )
        }
      }
    })
    ...
    return pipe(
      ...
      core.flatMap((lookupValue) =>
        core.suspend(() => {
          ...
          let value = Option.getOrUndefined(
            MutableHashMap.get(this.cacheState.map, key)
          )
          if (value === undefined) {
  ...
  invalidate(key: Key): Effect.Effect<void> {
    return core.suspend(() => {
      if (MutableHashMap.has(this.cacheState.map, key)) {
        const mapValue = Option.getOrUndefined(
          MutableHashMap.get(this.cacheState.map, key)
        )!
        MutableHashMap.remove(this.cacheState.map, key)
  ...
  refresh(key: Key): Effect.Effect<void, Error> {
    return pipe(
      ...
      core.flatMap((scoped) => {
        ...
        return core.flatMap(finalScoped, (s) =>
          fiberRuntime.scopedEffect(core.asVoid(s))
        )
      })
    )
  }
  ...
  lookupValueOf(
    key: Key
  ): Effect.Effect<Effect.Effect<Value, Error, Scope.Scope>> {
    return pipe(
      core.onInterrupt(
        core.flatMap(Scope.make(), (scope) =>
          pipe(
            ...
            core.provideContext(
              pipe(this.context, Context.add(Scope.Scope, scope))
            ),
            core.exit,
            core.map(
              (exit) =>
                [
                  exit,
                  ((exit) => Scope.close(scope, exit)) as Scope.Scope.Finalizer
                ] as const
            )
          )
        ),
      ...
      core.flatMap(([exit, release]) => {
        ...
        switch (exit._tag) {
          case "Success": {
            const exitWithFinalizer: Exit.Exit<[Value, Scope.Scope.Finalizer]> =
              Exit.succeed([exit.value, release])
            ...
            let previousValue: MapValue<Key, Value, Error> | undefined =
              undefined
            if (MutableHashMap.has(this.cacheState.map, key)) {
              previousValue = Option.getOrUndefined(
                MutableHashMap.get(this.cacheState.map, key)
              )
            }
      ...
      while (loop) {
        ...
        if (key === undefined) {
          ...
          if (MutableHashMap.has(this.cacheState.map, key.current)) {
            const removed = Option.getOrUndefined(
              MutableHashMap.get(this.cacheState.map, key.current)
            )!
...
>(options: {
  ...
  Environment | Scope.Scope
> =>
  core.flatMap(effect.clock, (clock) =>
    buildWith(options.capacity, options.lookup, clock, (exit) =>
      Duration.decode(options.timeToLive(exit))
    )
  )
...
> =>
  fiberRuntime.acquireRelease(
    core.flatMap(core.context<Environment>(), (context) =>
      core.sync(
        () =>
          new ScopedCacheImpl(
            capacity,
