...
const patchLoop = (
  ...
  while (Chunk.isNonEmpty(patches)) {
    ...
    switch (head._tag) {
      ...
      case OP_AND_THEN: {
        patches = Chunk.prepend(head.first)(
          Chunk.prepend(head.second)(Chunk.tailNonEmpty(patches))
        )
        break
      }
    }
...
const toSet = (
  self: Supervisor.Supervisor<any>
): HashSet.HashSet<Supervisor.Supervisor<any>> => {
...
export const diff = (
  ...
  const added = pipe(
    ...
    HashSet.reduce(empty as SupervisorPatch, (patch, supervisor) =>
      combine(patch, { _tag: OP_ADD_SUPERVISOR, supervisor })
    )
  )
  const removed = pipe(
    oldSupervisors,
    ...
    HashSet.reduce(empty as SupervisorPatch, (patch, supervisor) =>
      combine(patch, { _tag: OP_REMOVE_SUPERVISOR, supervisor })
    )
  )
