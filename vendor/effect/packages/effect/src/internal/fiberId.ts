...
export const composite = (
  left: FiberId.FiberId,
  right: FiberId.FiberId
): FiberId.Composite => {
...
export const isFiberId = (self: unknown): self is FiberId.FiberId =>
  hasProperty(self, FiberIdTypeId)
...
export const combineAll = (
  fiberIds: HashSet.HashSet<FiberId.FiberId>
): FiberId.FiberId => {
  return pipe(
    fiberIds,
    HashSet.reduce(none as FiberId.FiberId, (a, b) => combine(b)(a))
  )
}
...
export const toSet = (
  self: FiberId.FiberId
): HashSet.HashSet<FiberId.Runtime> => {
  switch (self._tag) {
    case OP_NONE: {
      return HashSet.empty()
    }
