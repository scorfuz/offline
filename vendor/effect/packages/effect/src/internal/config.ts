...
/** @internal */
export type Op<Tag extends string, Body = {}> = Config.Config<never> &
  Body & {
    readonly _tag: Tag
  }

/** @internal */
export interface Constant extends Op<
  OpCodes.OP_CONSTANT,
  {
...
/** @internal */
export interface Described extends Op<
  OpCodes.OP_DESCRIBED,
  {
...
/** @internal */
export interface Fallback extends Op<
  OpCodes.OP_FALLBACK,
  {
...
/** @internal */
export interface Fail extends Op<
  OpCodes.OP_FAIL,
  {
...
/** @internal */
export interface Lazy extends Op<
  OpCodes.OP_LAZY,
  {
...
/** @internal */
export interface MapOrFail extends Op<
  OpCodes.OP_MAP_OR_FAIL,
  {
...
/** @internal */
export interface Nested extends Op<
  OpCodes.OP_NESTED,
  {
...
/** @internal */
export interface Primitive extends Op<
  OpCodes.OP_PRIMITIVE,
  {
    readonly description: string
    parse(text: string): Either.Either<unknown, ConfigError.ConfigError>
  }
...
/** @internal */
export interface Sequence extends Op<
  OpCodes.OP_SEQUENCE,
  {
    readonly config: Config.Config<unknown>
  }
...
/** @internal */
export interface Table extends Op<
  OpCodes.OP_HASHMAP,
  {
...
/** @internal */
export interface Zipped extends Op<
  OpCodes.OP_ZIP_WITH,
  {
...
export const url = (name?: string): Config.Config<URL> => {
  const config = primitive("an URL property", (text) =>
    Either.try({
      try: () => new URL(text),
      catch: (_) =>
        configError.InvalidData(
          [],
          `Expected an URL value but received ${formatUnknown(text)}`
        )
    })
  )
  return name === undefined ? config : nested(config, name)
...
export const chunk = <A>(
  config: Config.Config<A>,
  name?: string
): Config.Config<Chunk.Chunk<A>> => {
  return map(
    name === undefined ? repeat(config) : nested(repeat(config), name),
...
export const literal =
  <Literals extends ReadonlyArray<Config.LiteralValue>>(
    ...literals: Literals
  ) =>
  (name?: string): Config.Config<Literals[number]> => {
  ...
  const config = mapOrFail(string(), (value) => {
    ...
    return level === undefined
      ? Either.left(
          configError.InvalidData(
            [],
            `Expected a log level but received ${formatUnknown(value)}`
          )
        )
      : Either.right(level)
  })
...
export const duration = (name?: string): Config.Config<Duration.Duration> => {
  const config = mapOrFail(string(), (value) => {
    ...
    return Either.fromOption(duration, () =>
      configError.InvalidData(
        [],
        `Expected a duration but received ${formatUnknown(value)}`
      )
    )
  })
  return name === undefined ? config : nested(config, name)
...
export const secret = (name?: string): Config.Config<Secret.Secret> => {
  const config = primitive("a secret property", (text) =>
    Either.right(InternalSecret.fromString(text))
  )
  return name === undefined ? config : nested(config, name)
}
...
export const redacted = <A>(
  ...
  const config: Config.Config<A | string> = isConfig(nameOrConfig)
    ? nameOrConfig
    : string(nameOrConfig)
...
export const branded: {
  ...
  2,
  <A, B extends Brand.Brand.Constructor<any>>(
    nameOrConfig: Config.Config<NoInfer<A>> | string | undefined,
    constructor: B
  ) => {
    const config: Config.Config<string | A> = isConfig(nameOrConfig)
      ? nameOrConfig
      : string(nameOrConfig)
      ...
      constructor
        ...
        .pipe(
          Either.mapLeft((brandErrors) =>
            configError.InvalidData(
              [],
              brandErrors.map((brandError) => brandError.message).join("\n")
            )
          )
        )
    )
...
export const hashSet = <A>(
  config: Config.Config<A>,
  name?: string
): Config.Config<HashSet.HashSet<A>> => {
...
export const nonEmptyString = (name?: string): Config.Config<string> => {
  const config = primitive(
    ...
    Either.liftPredicate(
      (text) => text.length > 0,
      () => configError.MissingData([], "Expected a non-empty string")
    )
  )
...
export const all = <
  const Arg extends
    | Iterable<Config.Config<any>>
    | Record<string, Config.Config<any>>
>(
  arg: Arg
): Config.Config<
  [Arg] extends [ReadonlyArray<Config.Config<any>>]
    ? {
        -readonly [K in keyof Arg]: [Arg[K]] extends [Config.Config<infer A>]
          ? A
          : never
      }
    : [Arg] extends [Iterable<Config.Config<infer A>>]
      ? Array<A>
      : [Arg] extends [Record<string, Config.Config<any>>]
        ? {
            -readonly [K in keyof Arg]: [Arg[K]] extends [
              Config.Config<infer A>
            ]
              ? A
              : never
          }
...
const struct = <NER extends Record<string, Config.Config<any>>>(
  r: NER
): Config.Config<{
  [K in keyof NER]: [NER[K]] extends [
    { [ConfigTypeId]: { _A: (_: never) => infer A } }
  ]
    ? A
    : never
}> => {
...
export const hashMap = <A>(
  config: Config.Config<A>,
  name?: string
): Config.Config<HashMap.HashMap<string, A>> => {
...
export const isConfig = (u: unknown): u is Config.Config<unknown> =>
  hasProperty(u, ConfigTypeId)

/** @internal */
const tuple = <T extends ArrayLike<Config.Config<any>>>(
  tuple: T
): Config.Config<{
...
export const zip = dual<
  <B>(
    that: Config.Config<B>
  ) => <A>(self: Config.Config<A>) => Config.Config<[A, B]>,
  <A, B>(
    self: Config.Config<A>,
    that: Config.Config<B>
  ) => Config.Config<[A, B]>
...
export const zipWith = dual<
  <B, A, C>(
    that: Config.Config<B>,
    f: (a: A, b: B) => C
  ) => (self: Config.Config<A>) => Config.Config<C>,
  <A, B, C>(
    self: Config.Config<A>,
    that: Config.Config<B>,
    f: (a: A, b: B) => C
