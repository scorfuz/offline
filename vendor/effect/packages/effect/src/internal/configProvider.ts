...
const concat = <A, B>(
  l: ReadonlyArray<A>,
  r: ReadonlyArray<B>
): ReadonlyArray<A | B> => [...l, ...r]

/** @internal */
const ConfigProviderSymbolKey = "effect/ConfigProvider"

/** @internal */
export const ConfigProviderTypeId: ConfigProvider.ConfigProviderTypeId =
  Symbol.for(ConfigProviderSymbolKey) as ConfigProvider.ConfigProviderTypeId

/** @internal */
export const configProviderTag: Context.Tag<
  ConfigProvider.ConfigProvider,
  ConfigProvider.ConfigProvider
> = Context.GenericTag("effect/ConfigProvider")

/** @internal */
const FlatConfigProviderSymbolKey = "effect/ConfigProviderFlat"

/** @internal */
export const FlatConfigProviderTypeId: ConfigProvider.FlatConfigProviderTypeId =
  Symbol.for(
...
export const fromEnv = (
  ...
  const load = <A>(
    ...
    const valueOpt =
      pathString in current ? Option.some(current[pathString]!) : Option.none()
    return pipe(
      valueOpt,
      core.mapError(() =>
        configError.MissingData(
...
export const fromMap = (
  ...
  const enumerateChildren = (
    ...
    core.sync(() => {
      const keyPaths = Arr.fromIterable(mapWithIndexSplit.keys()).map(
        unmakePathString
      )
      const filteredKeyPaths = keyPaths
        .filter((keyPath) => {
...
const extend = <A, B>(
  ...
  const leftPad = Arr.unfold(left.length, (index) =>
    index >= right.length
      ? Option.none()
      : Option.some([leftDef(index), index + 1])
  )
  const rightPad = Arr.unfold(right.length, (index) =>
    index >= left.length
      ? Option.none()
...
const fromFlatLoop = <A>(
  ...
  switch (op._tag) {
    ...
    case OpCodes.OP_FAIL: {
      return core.fail(
        configError.MissingData(prefix, op.message)
      ) as Effect.Effect<Array<A>, ConfigError.ConfigError>
    }
        ...
        core.catchAll((error1) => {
          if (op.condition(error1)) {
            return pipe(
              ...
              core.catchAll((error2) =>
                core.fail(configError.Or(error1, error2))
              )
            )
          }
          ...
          core.flatMap(
            core.forEachSequential((a) =>
              pipe(
                op.mapOrFail(a),
                core.mapError(
                  configError.prefixed(appendConfigPath(prefix, op.original))
                )
              )
            )
          )
    ...
    case OpCodes.OP_NESTED: {
      return core.suspend(() =>
        fromFlatLoop(flat, concat(prefix, Arr.of(op.name)), op.config, split)
      ) as unknown as Effect.Effect<Array<A>, ConfigError.ConfigError>
    }
        ...
        core.flatMap((patchedPrefix) =>
          pipe(
            ...
            core.flatMap((indices) => {
              ...
              return pipe(
                core.forEachSequential(indices, (index) =>
                  fromFlatLoop(
                    flat,
                    Arr.append(prefix, `[${index}]`),
                    op.config,
                    true
                  )
                ),
                    ...
                    return pipe(
                      ...
                      Arr.map((values) =>
                        HashMap.fromIterable(
                          Arr.zip(Arr.fromIterable(keys), values)
                        )
                      )
                    )
                  })
...
const fromFlatLoopFail =
  ...
  (index: number): Either.Either<unknown, ConfigError.ConfigError> =>
    Either.left(
      configError.MissingData(
        prefix,
...
>(2, (self, that) =>
  fromFlat(orElseFlat(self.flattened, () => that().flattened))
)
            ...
            core.flatMap((that) =>
              pipe(
                ...
                core.flatMap((right) => {
                  ...
                  if (Either.isRight(left) && Either.isRight(right)) {
                    return core.succeed(
                      pipe(left.right, HashSet.union(right.right))
                    )
                  }
                  throw new Error(
...
export const constantCase = (
  self: ConfigProvider.ConfigProvider
): ConfigProvider.ConfigProvider => mapInputPath(self, StringUtils.constantCase)

/** @internal */
export const kebabCase = (
  self: ConfigProvider.ConfigProvider
): ConfigProvider.ConfigProvider => mapInputPath(self, StringUtils.kebabCase)

/** @internal */
export const lowerCase = (
  self: ConfigProvider.ConfigProvider
): ConfigProvider.ConfigProvider => mapInputPath(self, StringUtils.lowerCase)

/** @internal */
export const snakeCase = (
  self: ConfigProvider.ConfigProvider
): ConfigProvider.ConfigProvider => mapInputPath(self, StringUtils.snakeCase)

/** @internal */
export const upperCase = (
  self: ConfigProvider.ConfigProvider
): ConfigProvider.ConfigProvider => mapInputPath(self, StringUtils.upperCase)
...
>(3, (self, path, f) => {
  const unnest = Arr.reduce(path, self, (provider, name) =>
    unnested(provider, name)
  )
  const nest = Arr.reduceRight(path, f(unnest), (provider, name) =>
...
const transpose = <A>(
  array: ReadonlyArray<ReadonlyArray<A>>
): Array<Array<A>> => {
  return Object.keys(array[0]).map((column) =>
    array.map((row) => row[column as any])
  )
}
...
const getIndexedEntries = (
  ...
  const loopAny = (
    ...
    if (typeof value === "number" || typeof value === "boolean") {
      return Arr.make([path, String(value)] as [
        ReadonlyArray<KeyComponent>,
        string
      ])
    }
  ...
  const loopArray = (
    ...
    Arr.match(values, {
      onEmpty: () =>
        Arr.make([path, "<nil>"] as [ReadonlyArray<KeyComponent>, string]),
      onNonEmpty: Arr.flatMap((value, index) =>
        loopAny(Arr.append(path, keyIndex(index)), value)
      )
    })
  const loopObject = (
    path: ReadonlyArray<KeyComponent>,
