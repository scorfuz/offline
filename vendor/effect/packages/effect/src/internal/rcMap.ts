...
export const make: {
  ...
  <K, A, E, R>(options: {
    ...
    RcMap.RcMap<K, A, E | Cause.ExceededCapacityException>,
    never,
    Scope.Scope | R
  >
} = <K, A, E, R>(options: {
  ...
  core.withFiberRuntime<RcMap.RcMap<K, A, E>, never, R | Scope.Scope>(
    (fiber) => {
      const context = fiber.getFiberRef(core.currentContext) as Context.Context<
        R | Scope.Scope
      >
      ...
      return core.as(
        scope.addFinalizer(() =>
          core.suspend(() => {
            ...
            return core
              .forEachSequentialDiscard(map, ([, entry]) =>
                core.scopeClose(entry.scope, core.exitVoid)
              )
              .pipe(
                core.tap(() => {
...
export const get: {
  ...
  <K, A, E>(self_: RcMap.RcMap<K, A, E>, key: K): Effect<A, E, Scope.Scope> => {
    ...
    return core.uninterruptibleMask((restore) =>
      getImpl(self, key, restore as any)
    )
  }
)

const getImpl = core.fnUntraced(function* <K, A, E>(
  self: RcMapImpl<K, A, E>,
  key: K,
  restore: <A>(a: A) => A
) {
  if (self.state._tag === "Closed") {
  ...
  if (o._tag === "Some") {
    ...
    return yield* core.fail(
      new core.ExceededCapacityException(
...
const acquire = core.fnUntraced(function* <K, A, E>(
  self: RcMapImpl<K, A, E>,
  key: K,
  restore: <A>(a: A) => A
) {
...
const release = <K, A, E>(
  ...
  coreEffect.clockWith((clock) => {
    ...
    entry.expiresAt =
      clock.unsafeCurrentTimeMillis() + Duration.toMillis(entry.idleTimeToLive)
    if (entry.fiber) return core.void

    return core
      .interruptibleMask(function loop(restore): Effect<void> {
        ...
        return core.flatMap(clock.sleep(Duration.millis(remaining)), () =>
          loop(restore)
        )
      })
      .pipe(
...
export const keys = <K, A, E>(self: RcMap.RcMap<K, A, E>): Effect<Array<K>> => {
  ...
  return core.suspend(() =>
    impl.state._tag === "Closed"
      ? core.interrupt
      : core.succeed(MutableHashMap.keys(impl.state.map))
  )
}
...
export const touch: {
  <K>(key: K): <A, E>(self: RcMap.RcMap<K, A, E>) => Effect<void>
  <K, A, E>(self: RcMap.RcMap<K, A, E>, key: K): Effect<void>
} = dual(2, <K, A, E>(self_: RcMap.RcMap<K, A, E>, key: K) =>
  coreEffect.clockWith((clock) => {
    const self = self_ as RcMapImpl<K, A, E>
