...
export const get = <I, S>(
  ...
  core.flatMap(reloadableTag(tag), (reloadable) =>
    scopedRef.get(reloadable.scopedRef)
  )
      ...
      core.flatMap((env) =>
        pipe(
          scopedRef.fromAcquire(
            pipe(layer_.build(options.layer), core.map(Context.unsafeGet(tag)))
          ),
          core.map((ref) => ({
            ...
            reload: pipe(
              scopedRef.set(
                ref,
                pipe(
                  layer_.build(options.layer),
                  core.map(Context.unsafeGet(tag))
                )
              ),
...
export const reloadableTag = <I, S>(
  ...
  return Context.GenericTag<Reloadable.Reloadable<I>, Reloadable.Reloadable<S>>(
...
export const reload = <I, S>(
  ...
  core.flatMap(reloadableTag(tag), (reloadable) => reloadable.reload)
