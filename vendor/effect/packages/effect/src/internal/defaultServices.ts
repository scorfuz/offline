...
/** @internal */
export const liveServices: Context.Context<DefaultServices.DefaultServices> =
  pipe(
    Context.empty(),
...
export const defaultServicesWith = <A, E, R>(
  f: (
    services: Context.Context<DefaultServices.DefaultServices>
  ) => Effect.Effect<A, E, R>
...
export const clockWith = <A, E, R>(
  ...
  defaultServicesWith((services) =>
    f(services.unsafeMap.get(clock.clockTag.key))
  )
...
export const configProviderWith = <A, E, R>(
  ...
  defaultServicesWith((services) =>
    f(services.unsafeMap.get(configProvider.configProviderTag.key))
  )

/** @internal */
export const config = <A>(config: Config.Config<A>) =>
  configProviderWith((_) => _.load(config))

/** @internal */
export const configOrDie = <A>(config: Config.Config<A>) =>
  core.orDie(configProviderWith((_) => _.load(config)))
...
export const randomWith = <A, E, R>(
  ...
  defaultServicesWith((services) =>
    f(services.unsafeMap.get(random.randomTag.key))
  )
...
/** @internal */
export const nextInt: Effect.Effect<number> = randomWith(
...
/** @internal */
export const choice = <Self extends Iterable<unknown>>(elements: Self) => {
  const array = Array.fromIterable(elements)
  return core.map(
    array.length === 0
      ? core.fail(
          new core.NoSuchElementException(
            "Cannot select a random element from an empty array"
          )
        )
...
export const tracerWith = <A, E, R>(
  ...
  defaultServicesWith((services) =>
    f(services.unsafeMap.get(tracer.tracerTag.key))
  )
