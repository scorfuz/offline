...
class KeyedPoolImpl<
  ...
  in out A,
  out E = never
> implements KeyedPool.KeyedPool<K, A, E> {
  ...
  invalidate(item: A): Effect.Effect<void> {
    return core.flatMap(
      this.activePools,
      core.forEachSequentialDiscard((pool) => pool.invalidate(item))
    )
  }
  pipe() {
...
class Complete<in out A, out E> implements Equal.Equal {
  ...
  readonly [KeyedPoolMapValueSymbol]: KeyedPoolMapValueSymbol =
  ...
  pipe(
    ...
    core.map(([context, fiberId, map, scope]) => {
      const getOrCreatePool = (key: K): Effect.Effect<Pool.Pool<A, E>> =>
        core.suspend(() => {
          let value: MapValue<A, E> | undefined = Option.getOrUndefined(
            HashMap.get(MutableRef.get(map), key)
          )
          if (value === undefined) {
            return core.uninterruptibleMask((restore) => {
              ...
              if (HashMap.has(MutableRef.get(map), key)) {
                ...
                MutableRef.update(
                  map,
                  HashMap.set(key, value as MapValue<A, E>)
                )
              }
                ...
                return pipe(
                  ...
                  core.matchCauseEffect({
                    ...
                    onSuccess: (pool) => {
                      MutableRef.update(
                        map,
                        HashMap.set(key, new Complete(pool) as MapValue<A, E>)
                      )
      ...
      const activePools: Effect.Effect<Array<Pool.Pool<A, E>>> = core.suspend(
        () =>
          core.forEachSequential(
            HashMap.toValues(MutableRef.get(map)),
            (value) => {
              switch (value._tag) {
                case "Complete": {
                  return core.succeed(value.pool)
...
/** @internal */
export const makeWith = <K, A, E, R>(options: {
  readonly acquire: (key: K) => Effect.Effect<A, E, R>
...
/** @internal */
export const makeWithTTL = <K, A, E, R>(options: {
  readonly acquire: (key: K) => Effect.Effect<A, E, R>
  ...
  return makeImpl(options.acquire, options.min, options.max, () =>
    Option.some(timeToLive)
  )
}

/** @internal */
export const makeWithTTLBy = <K, A, E, R>(options: {
  readonly acquire: (key: K) => Effect.Effect<A, E, R>
  readonly min: (key: K) => number
  readonly max: (key: K) => number
  readonly timeToLive: (key: K) => Duration.DurationInput
}): Effect.Effect<KeyedPool.KeyedPool<K, A, E>, never, R | Scope.Scope> =>
  makeImpl(options.acquire, options.min, options.max, (key) =>
    Option.some(Duration.decode(options.timeToLive(key)))
  )
...
export const get = dual<
  <K>(
    ...
    self: KeyedPool.KeyedPool<K, A, E>
...
export const invalidate = dual<
  <A>(
    item: A
  ) => <K, E>(self: KeyedPool.KeyedPool<K, A, E>) => Effect.Effect<void>,
