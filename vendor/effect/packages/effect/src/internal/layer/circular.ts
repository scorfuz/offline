...
>(2, (self, level) =>
  core.fiberRefLocally(fiberRuntime.currentMinimumLogLevel, level)(self)
)

/** @internal */
export const addLogger = <A>(
  logger: Logger.Logger<unknown, A>
): Layer.Layer<never> =>
  layer.scopedDiscard(
...
): Layer.Layer<never, E, R> => layer.unwrapEffect(core.map(effect, addLogger))

/** @internal */
export const addLoggerScoped = <A, E, R>(
  effect: Effect.Effect<Logger.Logger<unknown, A>, E, R>
): Layer.Layer<never, E, Exclude<R, Scope>> =>
  layer.unwrapScoped(core.map(effect, addLogger))

/** @internal */
export const removeLogger = <A>(
  logger: Logger.Logger<unknown, A>
): Layer.Layer<never> =>
  layer.scopedDiscard(
    fiberRuntime.fiberRefLocallyScopedWith(
...
export const replaceLogger = dual<
  <B>(
    that: Logger.Logger<unknown, B>
  ) => <A>(self: Logger.Logger<unknown, A>) => Layer.Layer<never>,
  <A, B>(
...
>(2, (self, that) =>
  layer.flatMap(removeLogger(self), () => addLoggerEffect(that))
)
...
>(2, (self, that) =>
  layer.flatMap(removeLogger(self), () => addLoggerScoped(that))
)
