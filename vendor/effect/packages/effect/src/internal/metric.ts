...
export const make: Metric.MetricApply = function <Type, In, Out>(
  ...
  const metric: Metric.Metric<Type, In, Out> = Object.assign(
    ...
    ): Effect.Effect<A, E, R> => core.tap(effect, (a) => update(metric, a)),
    {
...
export const counter: {
  ...
  fromMetricKey(metricKey.counter(name, options as any)) as any
...
export const frequency = (
  ...
  fromMetricKey(metricKey.frequency(name, options))
...
export const gauge: {
  ...
  fromMetricKey(metricKey.gauge(name, options as any)) as any
...
export const increment = (
  ...
  metricKeyType.isCounterKey(self.keyType)
    ? update(
        self as Metric.Metric.Counter<number>,
        self.keyType.bigint ? (BigInt(1) as any) : 1
      )
    : modify(
        self as Metric.Metric.Gauge<number>,
        self.keyType.bigint ? (BigInt(1) as any) : 1
      )
...
export const incrementBy = dual<
  {
    (
      ...
      self: Metric.Metric.Counter<number> | Metric.Metric.Counter<number>
    ) => Effect.Effect<void>
    (
      amount: bigint
    ): (
      self: Metric.Metric.Counter<bigint> | Metric.Metric.Gauge<bigint>
    ) => Effect.Effect<void>
  },
  {
    (
      self: Metric.Metric.Counter<number> | Metric.Metric.Gauge<number>,
      amount: number
    ): Effect.Effect<void>
    (
      self: Metric.Metric.Counter<bigint> | Metric.Metric.Gauge<bigint>,
...
>(2, (self, f) =>
  make(f(self.keyType), self.unsafeUpdate, self.unsafeValue, self.unsafeModify)
)
...
export const modify = dual<
  <In>(
    input: In
  ) => <Type, Out>(self: Metric.Metric<Type, In, Out>) => Effect.Effect<void>,
  <Type, In, Out>(
    self: Metric.Metric<Type, In, Out>,
    input: In
  ) => Effect.Effect<void>
>(2, (self, input) =>
  core.fiberRefGetWith(core.currentMetricLabels, (tags) =>
    core.sync(() => self.unsafeModify(input, tags))
  )
)
...
/** @internal */
export const sync = <Out>(
  evaluate: LazyArg<Out>
): Metric.Metric<void, unknown, Out> =>
...
export const summaryTimestamp = (options: {
  ...
  fromMetricKey(metricKey.summary(options))
...
>(3, (self, key, value) =>
  taggedWithLabels(self, [metricLabel.make(key, value)])
)
...
>(2, (self, extraTags) => {
  return make(
    self.keyType,
    (input, extraTags1) =>
      self.unsafeUpdate(input, Arr.union(extraTags, extraTags1)),
    (extraTags1) => self.unsafeValue(Arr.union(extraTags, extraTags1)),
    (input, extraTags1) =>
      self.unsafeModify(input, Arr.union(extraTags, extraTags1))
  )
})
...
>(
  ...
  (self, input) => (effect) =>
    core.matchCauseEffect(effect, {
      onFailure: (cause) =>
        core.zipRight(update(self, input), core.failCause(cause)),
      onSuccess: (value) =>
        core.zipRight(update(self, input), core.succeed(value))
    })
)
...
>(3, (self, metric, f) => {
  ...
  return effect_.tapDefect(self, (cause) =>
    core.forEachSequentialDiscard(Cause.defects(cause), updater)
  )
})
...
>(2, (self, input) =>
  core.fiberRefGetWith(core.currentMetricLabels, (tags) =>
    core.sync(() => self.unsafeUpdate(input, tags))
  )
)
...
export const value = <Type, In, Out>(
  ...
  core.fiberRefGetWith(core.currentMetricLabels, (tags) =>
    core.sync(() => self.unsafeValue(tags))
  )
...
export const withNow = <Type, In, Out>(
  ...
  mapInput(self, (input: In) => [input, Date.now()] as const)
...
/** @internal */
export const unsafeSnapshot = (): Array<MetricPair.MetricPair.Untyped> =>
  globalMetricRegistry.snapshot()

/** @internal */
export const snapshot: Effect.Effect<Array<MetricPair.MetricPair.Untyped>> =
  core.sync(unsafeSnapshot)
