...
/** @internal */
export const isGroupBy = (
  u: unknown
): u is GroupBy.GroupBy<unknown, unknown, unknown, unknown> =>
...
>(
  ...
  <K, V, E, R, A, E2, R2>(
    ...
    stream.flatMap(
      self.grouped,
      ([key, queue]) =>
        f(key, stream.flattenTake(stream.fromQueue(queue, { shutdown: true }))),
      ...
      stream.unwrapScoped(
        Effect.gen(function* () {
          ...
          const output = yield* Effect.acquireRelease(
            Queue.bounded<
              Exit.Exit<
                readonly [K, Queue.Dequeue<Take.Take<V, E | E2>>],
                Option.Option<E | E2>
              >
          ...
          const add = yield* pipe(
            ...
            stream.distributedWithDynamicCallback(
              options?.bufferSize ?? 16,
              ([key, value]) =>
                Effect.flatMap(Deferred.await(decider), (f) => f(key, value)),
            ...
            pipe(
              ...
              Effect.flatMap(
                Option.match({
                  ...
                  onSome: (index) =>
                    Effect.succeed<Predicate<number>>(
                      (n: number) => n === index
                    )
                })
              )
          ...
          return stream.flattenExitOption(
            stream.fromQueue(output, { shutdown: true })
          )
        })
      )
    )
...
>((args) => typeof args[0] !== "function", (<A, E, R, A2, E2, R2, K>(
  self: Stream.Stream<A, E, R>,
  f: (a: A) => Effect.Effect<A2, E2, R2>,
  options?: {
  ...
  return stream.matchConcurrency(
    ...
    (n) =>
      options?.unordered
        ? stream.flatMap(self, (a) => stream.fromEffect(f(a)), {
            concurrency: n
          })
...
const mapDequeue = <A, B>(
  dequeue: Queue.Dequeue<A>,
  f: (a: A) => B
): Queue.Dequeue<B> => new MapDequeue(dequeue, f)

class MapDequeue<in out A, out B>
  extends Effectable.Class<B>
  implements Queue.Dequeue<B>
{
  ...
  get take(): Effect.Effect<B> {
    return pipe(
      Queue.take(this.dequeue),
      Effect.map((a) => this.f(a))
    )
  }

  get takeAll(): Effect.Effect<Chunk.Chunk<B>> {
    return pipe(
      Queue.takeAll(this.dequeue),
      Effect.map(Chunk.map((a) => this.f(a)))
    )
  }

  takeUpTo(max: number): Effect.Effect<Chunk.Chunk<B>> {
    return pipe(
      Queue.takeUpTo(this.dequeue, max),
      Effect.map(Chunk.map((a) => this.f(a)))
    )
  }
  ...
  takeBetween(min: number, max: number): Effect.Effect<Chunk.Chunk<B>> {
    return pipe(
      Queue.takeBetween(this.dequeue, min, max),
      Effect.map(Chunk.map((a) => this.f(a)))
    )
  }
  ...
  poll(): Effect.Effect<Option.Option<B>> {
    return pipe(
      Queue.poll(this.dequeue),
      Effect.map(Option.map((a) => this.f(a)))
    )
  }
...
>(
  ...
  <A, E, R, K>(
    ...
    const loop = (
      ...
      outerQueue: Queue.Queue<
        Take.Take<readonly [K, Queue.Queue<Take.Take<A, E>>], E>
      >
      ...
      core.readWithCause({
                      ...
                      Effect.flatMap((innerQueue) =>
                        pipe(
                          ...
                          Effect.zipRight(
                            Queue.offer(
                              outerQueue,
                              take.of([key, innerQueue] as const)
                            )
                          ),
                          Effect.zipRight(
                            pipe(
        ...
        onFailure: (cause) =>
          core.fromEffect(Queue.offer(outerQueue, take.failCause(cause))),
        onDone: () =>
          core.fromEffect(
            pipe(
              Effect.forEach(
        ...
        Effect.gen(function* () {
          ...
          return yield* stream
            ...
            .pipe(
              ...
              Effect.forkIn(scope),
              Effect.as(
                stream.flattenTake(stream.fromQueue(queue, { shutdown: true }))
              )
            )
        })
      )
...
const groupByIterable = dual<
  <V, K>(
    f: (value: V) => K
  ) => (iterable: Iterable<V>) => Chunk.Chunk<[K, Chunk.Chunk<V>]>,
  <V, K>(
    iterable: Iterable<V>,
    f: (value: V) => K
  ) => Chunk.Chunk<[K, Chunk.Chunk<V>]>
>(
  2,
  <V, K>(
    iterable: Iterable<V>,
