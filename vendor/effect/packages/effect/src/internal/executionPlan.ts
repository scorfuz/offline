...
/** @internal */
export const isExecutionPlan = (u: unknown): u is Api.ExecutionPlan<any> =>
    ...
    core.suspend(() => {
      ...
      return core.flatMap(
        core.whileLoop({
          while: () =>
            i < plan.steps.length &&
            (result === undefined || Either.isLeft(result)),
          body: () => {
            const step = plan.steps[i]
            let nextEffect: Effect<A, any, any> = layer.effect_provide(
              effect,
              step.provide as any
            )
            if (result) {
              ...
              nextEffect = schedule
                ? InternalSchedule.scheduleDefectRefail(
                    InternalSchedule.retry_Effect(nextEffect, schedule)
                  )
                : nextEffect
            }
