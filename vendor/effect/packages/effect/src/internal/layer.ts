...
/** @internal */
export type Op<Tag extends string, Body = {}> = Layer.Layer<
  unknown,
  unknown,
  unknown
> &
...
/** @internal */
export interface ExtendScope extends Op<
  OpCodes.OP_EXTEND_SCOPE,
  {
...
/** @internal */
export interface Fold extends Op<
  OpCodes.OP_FOLD,
  {
    readonly layer: Layer.Layer<unknown>
...
/** @internal */
export interface Fresh extends Op<
  OpCodes.OP_FRESH,
  {
    readonly layer: Layer.Layer<unknown>
  }
...
/** @internal */
export interface FromEffect extends Op<
  OpCodes.OP_FROM_EFFECT,
  {
...
/** @internal */
export interface Scoped extends Op<
  OpCodes.OP_SCOPED,
  {
    readonly effect: Effect.Effect<unknown, unknown, Context.Context<unknown>>
  }
...
/** @internal */
export interface Suspend extends Op<
  OpCodes.OP_SUSPEND,
  {
...
/** @internal */
export interface Locally extends Op<
  "Locally",
  {
...
/** @internal */
export interface ProvideTo extends Op<
  OpCodes.OP_PROVIDE,
  {
...
/** @internal */
export interface ZipWith extends Op<
  OpCodes.OP_PROVIDE_MERGE,
  {
    readonly first: Layer.Layer<unknown>
    readonly second: Layer.Layer<unknown>
    zipK(
...
/** @internal */
export interface ZipWithPar extends Op<
  OpCodes.OP_ZIP_WITH,
  {
    readonly first: Layer.Layer<unknown>
    readonly second: Layer.Layer<unknown>
...
/** @internal */
export interface MergeAll extends Op<
  OpCodes.OP_MERGE_ALL,
  {
...
/** @internal */
export const isLayer = (
  u: unknown
): u is Layer.Layer<unknown, unknown, unknown> => hasProperty(u, LayerTypeId)

/** @internal */
export const isFresh = <RIn, E, ROut>(
        ...
        if (inMap !== undefined) {
          ...
          const cached: Effect.Effect<Context.Context<ROut>, E> = pipe(
            acquire as Effect.Effect<
              readonly [FiberRefsPatch.FiberRefsPatch, Context.Context<ROut>],
              E
            >,
                              ...
                              switch (exit._tag) {
                                case EffectOpCodes.OP_FAILURE: {
                                  return pipe(
                                    ...
                                    core.zipRight(
                                      core.failCause(exit.effect_instruction_i0)
                                    )
                                  )
                                }
...
/** @internal */
export const unsafeMakeMemoMap = (): Layer.MemoMap =>
  new MemoMapImpl(circular.unsafeMakeSynchronized(new Map()))
...
>(2, (self, scope) =>
  core.flatMap(makeMemoMap, (memoMap) => buildWithMemoMap(self, memoMap, scope))
)
...
>(3, (self, memoMap, scope) =>
  core.flatMap(makeBuilder(self, scope), (run) =>
    effect.provideService(run(memoMap), CurrentMemoMap, memoMap)
  )
)
...
const makeBuilder = <RIn, E, ROut>(
  ...
  (memoMap: Layer.MemoMap) => Effect.Effect<Context.Context<ROut>, E, RIn>
> => {
  ...
  switch (op._op_layer) {
    case "Locally": {
      return core.sync(
        () => (memoMap: Layer.MemoMap) =>
          op.f(memoMap.getOrElseMemoize(op.self, scope))
      )
    }
    case "ExtendScope": {
      return core.sync(
    ...
    case "Fresh": {
      return core.sync(
        () => (_: Layer.MemoMap) => pipe(op.layer, buildWithScope(scope))
      )
    }
    case "FromEffect": {
      return inMemoMap
        ...
        : core.sync(
            () => (memoMap: Layer.MemoMap) =>
              memoMap.getOrElseMemoize(self, scope)
          )
    }
    case "Provide": {
      return core.sync(
    ...
    case "Suspend": {
      return core.sync(
        () => (memoMap: Layer.MemoMap) =>
          memoMap.getOrElseMemoize(op.evaluate(), scope)
      )
    }
    case "ProvideMerge": {
      return core.sync(
        () => (memoMap: Layer.MemoMap) =>
          pipe(
            memoMap.getOrElseMemoize(op.first, scope),
            core.zipWith(memoMap.getOrElseMemoize(op.second, scope), op.zipK)
          )
      )
    }
    case "ZipWith": {
      return core.gen(function* () {
        const parallelScope = yield* core.scopeFork(
          scope,
          ExecutionStrategy.parallel
        )
        const firstScope = yield* core.scopeFork(
          parallelScope,
          ExecutionStrategy.sequential
        )
        const secondScope = yield* core.scopeFork(
          parallelScope,
          ExecutionStrategy.sequential
        )
          ...
          return core.map(
            fiberRuntime.forEachConcurrentDiscard(
              ...
              core.fnUntraced(function* (layer, i) {
                const scope = yield* core.scopeFork(
                  parallelScope,
                  ExecutionStrategy.sequential
                )
...
>(2, (self, onFailure) =>
  matchCause(self, { onFailure, onSuccess: succeedContext })
)

/** @internal */
export const die = (defect: unknown): Layer.Layer<unknown> =>
  failCause(Cause.die(defect))

/** @internal */
export const dieSync = (evaluate: LazyArg<unknown>): Layer.Layer<unknown> =>
  failCauseSync(() => Cause.die(evaluate()))

/** @internal */
export const discard = <RIn, E, ROut>(
  self: Layer.Layer<ROut, E, RIn>
): Layer.Layer<never, E, RIn> => map(self, () => Context.empty())

/** @internal */
export const context = <R>(): Layer.Layer<R, never, R> =>
  fromEffectContext(core.context<R>())
...
/** @internal */
export const fail = <E>(error: E): Layer.Layer<unknown, E> =>
...
/** @internal */
export const failCause = <E>(cause: Cause.Cause<E>): Layer.Layer<unknown, E> =>
  fromEffectContext(core.failCause(cause))

/** @internal */
export const failCauseSync = <E>(
  evaluate: LazyArg<Cause.Cause<E>>
): Layer.Layer<unknown, E> => fromEffectContext(core.failCauseSync(evaluate))
...
export const flatten = dual<
  ...
  ) => <E, R>(self: Layer.Layer<I, E, R>) => Layer.Layer<A, E | E2, R | R2>,
  <I, E, R, A, E2, R2>(
...
>(2, (a, b) => {
  ...
  return fromEffectContext(
    core.map(effect, (service) => Context.make(tag, service))
  )
})
...
>(3, (self, ref, value) =>
  locallyEffect(self, core.fiberRefLocally(ref, value))
)
...
>(3, (self, ref, value) =>
  locallyEffect(self, core.fiberRefLocallyWith(ref, value))
)
...
export const mock: {
  <I, S extends object>(
    tag: Context.Tag<I, S>
  ): (service: Layer.PartialEffectful<S>) => Layer.Layer<I>
  <I, S extends object>(
    tag: Context.Tag<I, S>,
    service: Layer.PartialEffectful<S>
  ): Layer.Layer<I>
} = function () {
  if (arguments.length === 1) {
    return (service: Layer.PartialEffectful<any>) =>
      mockImpl(arguments[0], service)
  }
    ...
    new Proxy({ ...(service as object) } as S, {
      get(target, prop, _receiver) {
        ...
        const error = new Error(
          `${tag.key}: Unimplemented method "${prop.toString()}"`
        )
...
export const mapError = dual<
  <E, E2>(
    f: (error: E) => E2
  ) => <A, R>(self: Layer.Layer<A, E, R>) => Layer.Layer<A, E2, R>,
  <A, E, R, E2>(
    self: Layer.Layer<A, E, R>,
    f: (error: E) => E2
  ) => Layer.Layer<A, E2, R>
...
export const memoize = <RIn, E, ROut>(
  ...
  fiberRuntime.scopeWith((scope) =>
    core.map(effect.memoize(buildWithScope(self, scope)), fromEffectContext)
  )
...
export const orDie = <A, E, R>(
  self: Layer.Layer<A, E, R>
): Layer.Layer<A, never, R> => catchAll(self, (defect) => die(defect))
...
export const passthrough = <RIn, E, ROut>(
  self: Layer.Layer<ROut, E, RIn>
): Layer.Layer<RIn | ROut, E, RIn> => merge(context<RIn>(), self)
...
>(4, (self, tagA, tagB, f) =>
  map(self, (context) =>
    Context.make(tagB, f(Context.unsafeGet(context, tagA)))
  )
)
...
>(2, (self, schedule) =>
  suspend(() => {
    const stateTag = Context.GenericTag<{ state: unknown }>(
      "effect/Layer/retry/{ state: unknown }"
    )
    return pipe(
      ...
      flatMap((env: Context.Context<{ state: unknown }>) =>
        retryLoop(
          ...
          schedule,
          stateTag,
          pipe(env, Context.get(stateTag)).state
        )
      )
    )
  })
...
): Layer.Layer<never, E, Exclude<R, Scope.Scope>> =>
  scopedContext(pipe(effect, core.as(Context.empty())))
...
export const scope: Layer.Layer<Scope.Scope> = scopedContext(
  core.map(
    fiberRuntime.acquireRelease(fiberRuntime.scopeMake(), (scope, exit) =>
      scope.close(exit)
    ),
...
export const succeed = dual<
  <I, S>(
    tag: Context.Tag<I, S>
  ) => (resource: Types.NoInfer<S>) => Layer.Layer<I>,
  <I, S>(tag: Context.Tag<I, S>, resource: Types.NoInfer<S>) => Layer.Layer<I>
>(2, (a, b) => {
  const tagFirst = Context.isTag(a)
  const tag = (tagFirst ? a : b) as Context.Tag<unknown, unknown>
...
export const sync = dual<
  <I, S>(
    tag: Context.Tag<I, S>
  ) => (evaluate: LazyArg<Types.NoInfer<S>>) => Layer.Layer<I>,
  <I, S>(
    tag: Context.Tag<I, S>,
...
>(2, (self, f) =>
  flatMap(self, (context) => fromEffectContext(core.as(f(context), context)))
)
...
>(2, (self, f) =>
  catchAll(self, (e) =>
    fromEffectContext(core.flatMap(f(e as any), () => core.fail(e)))
  )
)
...
>(2, (self, f) =>
  catchAllCause(self, (cause) =>
    fromEffectContext(
      core.flatMap(f(cause as any), () => core.failCause(cause))
    )
  )
)
...
export const toRuntime = <RIn, E, ROut>(
  ...
  pipe(
    ...
    core.flatMap((context) =>
      pipe(runtime.runtime<ROut>(), core.provideContext(context))
    )
  )
...
>(2, (self, memoMap) =>
  core.flatMap(
    ...
    (context) => pipe(runtime.runtime<any>(), core.provideContext(context))
  )
)
  ...
  (
    ...
    suspend(() => {
      ...
      provideTo.first = Object.create(proto, {
        ...
        second: {
          value: Array.isArray(that) ? mergeAll(...(that as any)) : that
        },
        zipK: {
          value: (a: Context.Context<any>, b: Context.Context<any>) =>
            pipe(a, Context.merge(b))
        }
      })
...
>(
  ...
  <RIn2, E2, ROut2, RIn, E, ROut>(
    ...
    zipWith.zipK = (
      a: Context.Context<ROut>,
      b: Context.Context<ROut2>
    ): Context.Context<ROut | ROut2> => {
...
export const unwrapEffect = <A, E1, R1, E, R>(
  ...
  const tag = Context.GenericTag<Layer.Layer<A, E1, R1>>(
    "effect/Layer/unwrapEffect/Layer.Layer<R1, E1, A>"
  )
...
export const unwrapScoped = <A, E1, R1, E, R>(
  ...
  const tag = Context.GenericTag<Layer.Layer<A, E1, R1>>(
    "effect/Layer/unwrapScoped/Layer.Layer<R1, E1, A>"
  )
...
export const withSpan: {
  ...
  if (dataFirst) {
    ...
    return unwrapScoped(
      core.map(
        options?.onEnd
          ? core.tap(fiberRuntime.makeSpanScoped(name, options), (span) =>
...
>(2, (self, span) =>
  provide(self, succeedContext(Context.make(tracer.spanTag, span)))
)
...
>(2, (self, layer) =>
  fiberRuntime.scopedWith((scope) =>
    core.flatMap(buildWithScope(layer, scope), (context) =>
      core.provideSomeContext(self, context)
    )
  )
)
...
>(2, (self, rt) => {
  const patchRefs = FiberRefsPatch.diff(
    runtime.defaultRuntime.fiberRefs,
    rt.fiberRefs
  )
  const patchFlags = runtimeFlags.diff(
    runtime.defaultRuntime.runtimeFlags,
    rt.runtimeFlags
  )
    ...
    core.withFiberRuntime((fiber) => {
      ...
      return fiberRuntime.ensuring(
        core.provideSomeContext(
          restore(self),
          Context.merge(oldContext, rt.context)
        ),
        core.withFiberRuntime((fiber) => {
          fiber.setFiberRefs(
            FiberRefsPatch.patch(fiber.id(), fiber.getFiberRefs())(rollbackRefs)
          )
          fiber.currentRuntimeFlags = runtimeFlags.patch(rollbackFlags)(
