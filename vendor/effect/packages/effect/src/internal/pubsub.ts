...
/** @internal */
export const awaitShutdown = <A>(self: PubSub.PubSub<A>): Effect.Effect<void> =>
  self.awaitShutdown
...
export const publishAll = dual<
  <A>(
    elements: Iterable<A>
  ) => (self: PubSub.PubSub<A>) => Effect.Effect<boolean>,
...
): Effect.Effect<Queue.Dequeue<A>, never, Scope.Scope> => self.subscribe
...
/** @internal */
class SubscriptionImpl<in out A>
  extends Effectable.Class<A>
  implements Queue.Dequeue<A>
{
    ...
    return core.withFiberRuntime((state) => {
      ...
      if (message === MutableQueue.EmptyMutableQueue) {
        ...
        return pipe(
          ...
          core.onInterrupt(() =>
            core.sync(() => unsafeRemove(this.pollers, deferred))
          )
        )
      } else {
  ...
  takeUpTo(this: this, max: number): Effect.Effect<Chunk.Chunk<A>> {
    return core.suspend(() => {
      ...
      this.strategy.unsafeOnPubSubEmptySpace(this.pubsub, this.subscribers)
      return replay
        ? core.succeed(Chunk.appendAll(replay, as))
        : core.succeed(as)
    })
  }
...
class PubSubImpl<in out A> implements PubSub.PubSub<A> {
  ...
  get subscribe(): Effect.Effect<Queue.Dequeue<A>, never, Scope.Scope> {
    ...
    return core.map(
      fiberRuntime.acquireRelease(acquire, (tuple, exit) =>
        tuple[0].close(exit)
      ),
      (tuple) => tuple[1]
    )
...
const ensureCapacity = (capacity: number): void => {
  if (capacity <= 0) {
    throw new core.InvalidPubSubCapacityException(
      `Cannot construct PubSub with capacity of ${capacity}`
    )
  }
}
...
/** @internal */
const unsafePollAllSubscription = <A>(
  subscription: Subscription<A>
): Chunk.Chunk<A> => {
...
class BackPressureStrategy<in out A> implements PubSubStrategy<A> {
  ...
  unsafeRemove(deferred: Deferred.Deferred<boolean>): void {
    unsafeOfferAll(
      this.publishers,
      pipe(
        unsafePollAllQueue(this.publishers),
        Chunk.filter(([_, a]) => a !== deferred)
      )
    )
  }
}
  ...
  while (keepPolling && !subscription.isEmpty()) {
    ...
    if (poller === MutableQueue.EmptyMutableQueue) {
      ...
      if (pollResult === MutableQueue.EmptyMutableQueue) {
        unsafeOfferAll(
          pollers,
          pipe(unsafePollAllQueue(pollers), Chunk.prepend(poller))
        )
      } else {
