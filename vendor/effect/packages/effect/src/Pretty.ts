...
): ((a: A) => string) => compile(schema.ast, [])

const getPrettyAnnotation = AST.getAnnotation<PrettyAnnotation<any, any>>(
  AST.PrettyAnnotationId
)

const getMatcher =
  (defaultPretty: Pretty<any>) =>
  (ast: AST.AST): Pretty<any> =>
...
export const match: AST.Match<Pretty<any>> = {
  ...
  TypeLiteral: (ast, go, path) => {
    ...
    const indexSignatureTypes = ast.indexSignatures.map((is) =>
      go(is.type, path)
    )
    ...
    return (input: { readonly [x: PropertyKey]: unknown }) => {
      ...
      if (indexSignatureTypes.length > 0) {
        for (let i = 0; i < indexSignatureTypes.length; i++) {
          ...
          const keys = util_.getKeysForIndexSignature(
            input,
          ...
          for (const key of keys) {
            ...
            output.push(
              `${Inspectable.formatPropertyKey(key)}: ${type(input[key])}`
            )
          }
        }
      }

      return Arr.isNonEmptyReadonlyArray(output)
        ? "{ " + output.join(", ") + " }"
        : "{}"
    }
  },
  Union: (ast, go, path) => {
    ...
    const types = ast.types.map(
      (ast) => [ParseResult.is({ ast } as any), go(ast, path)] as const
    )
    return (a) => {
      const index = types.findIndex(([is]) => is(a))
      if (index === -1) {
        throw new Error(
          errors_.getPrettyNoMatchingSchemaErrorMessage(a, path, ast)
        )
      }
