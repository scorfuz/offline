...
export const productMany: {
  <A>(
    collection: Iterable<Order<A>>
  ): (self: Order<A>) => Order<readonly [A, ...Array<A>]> // readonly because invariant
  <A>(
    self: Order<A>,
    collection: Iterable<Order<A>>
  ): Order<readonly [A, ...Array<A>]> // readonly because invariant
} = dual(
  2,
  <A>(
    self: Order<A>,
    collection: Iterable<Order<A>>
  ): Order<readonly [A, ...Array<A>]> => {
...
export const tuple = <T extends ReadonlyArray<Order<any>>>(
  ...
  Readonly<{ [I in keyof T]: [T[I]] extends [Order<infer A>] ? A : never }>
> => all(elements) as any
...
export const between = <A>(
  ...
  dual(
    ...
    (
      ...
      !lessThan(O)(self, options.minimum) &&
      !greaterThan(O)(self, options.maximum)
  )
