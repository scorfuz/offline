...
>(
  ...
  PreloadKeys extends undefined
    ? never
    : L extends Layer.Layer<infer _A, infer _E, infer _R>
      ? _E
      : never,
  | Scope.Scope
  | (L extends Layer.Layer<infer _A, infer _E, infer _R> ? _R : never)
> = Effect.fnUntraced(function* <I, K, EL, RL>(
    ...
    lookup: (key: K) =>
      Effect.scopeWith((scope) =>
        Effect.diffFiberRefs(
          Layer.buildWithMemoMap(lookup(key), memoMap, scope)
        )
      ).pipe(
        Effect.map(
          ([patch, context]) =>
            ({
              layer: Layer.scopedContext(
                core.withFiberRuntime<Context.Context<I>, any, Scope.Scope>(
                  (fiber) => {
                    ...
                    const newRefs = FiberRefsPatch.patch(
                      patch,
                      fiber.id(),
                      oldRefs
                    )
              ...
              runtimeEffect: Effect.withFiberRuntime<
                Runtime.Runtime<I>,
                any,
                Scope.Scope
              >((fiber) => {
                const fiberRefs = FiberRefsPatch.patch(
                  patch,
                  fiber.id(),
                  fiber.getFiberRefs()
                )
  ...
  return identity<LayerMap<K, Exclude<I, Scope.Scope>, any>>({
    ...
    get: (key) =>
      Layer.unwrapScoped(
        Effect.map(RcMap.get(rcMap, key), ({ layer }) => layer)
      ),
    runtime: (key) =>
      Effect.flatMap(
        RcMap.get(rcMap, key),
...
export const Service =
  ...
  >(
    ...
    Options extends { readonly preload: true }
      ...
      : Options extends { readonly preloadKey: Iterable<any> }
        ? Service.Error<Options>
        : never,
    Options extends { readonly dependencies: ReadonlyArray<any> }
  ...
  > => {
    ...
    TagClass_.get = (key: string) =>
      Layer.unwrapScoped(Effect.map(TagClass_, (layerMap) => layerMap.get(key)))
    TagClass_.runtime = (key: string) =>
      Effect.flatMap(TagClass_, (layerMap) => layerMap.runtime(key))
