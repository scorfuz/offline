...
> =>
  Effect.acquireRelease(
    Effect.map(Deferred.make<void, unknown>(), (deferred) =>
      unsafeMake(new Set(), deferred)
    ),
    (set) =>
      Effect.withFiberRuntime((parent) => {
        ...
        ).pipe(Effect.intoDeferred(set.deferred))
      })
  )
...
const runImpl = <A, E, R, XE extends E, XA extends A>(
  ...
  Effect.fiberIdWith((fiberId) => {
    ...
    return Effect.tap(Effect.forkDaemon(effect), (fiber) =>
      unsafeAdd(self, fiber, {
        ...options,
        interruptAs: fiberId
      })
