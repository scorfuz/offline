 ...
 * @category models
 */
export interface RequestResolver<in A, out R = never>
  extends RequestResolver.Variance<A, R>, Equal.Equal, Pipeable {
  /**
 ...
 * @category utils
 */
export const contextFromEffect = <R, A extends Request.Request<any, any>>(
...
export const contextFromServices =
  ...
  <R, A extends Request.Request<any, any>>(
    ...
    { [k in keyof Services]: Effect.Effect.Context<Services[k]> }[number]
  > =>
    Effect.contextWith((_) =>
      provideContext(self as any, Context.pick(...services)(_ as any))
    )
...
export const isRequestResolver: (
  u: unknown
) => u is RequestResolver<unknown, unknown> = core.isRequestResolver
...
export const aroundRequests: {
  <A, A2, R2, X, R3>(
    before: (
      requests: ReadonlyArray<Types.NoInfer<A>>
...
export const fromEffectTagged: <
  A extends Request.Request<any, any> & { readonly _tag: string }
>() => <
  Fns extends {
    readonly [Tag in A["_tag"]]: [Extract<A, { readonly _tag: Tag }>] extends [
      infer Req
    ]
...
>(
  ...
  A,
  ReturnType<Fns[keyof Fns]> extends Effect.Effect<infer _A, infer _E, infer R>
    ? R
    : never
