...
export const isReadable = (
  u: unknown
): u is Readable<unknown, unknown, unknown> => hasProperty(u, TypeId)
...
export const map: {
  <A, B>(
    f: (a: NoInfer<A>) => B
  ): <E, R>(fa: Readable<A, E, R>) => Readable<B, E, R>
  <A, E, R, B>(
    self: Readable<A, E, R>,
    f: (a: NoInfer<A>) => B
  ): Readable<B, E, R>
  ...
  <A, E, R, B>(
    self: Readable<A, E, R>,
    f: (a: NoInfer<A>) => B
  ): Readable<B, E, R> => make(core.map(self.get, f))
)
...
): Readable<A, E | E1, R | R1> => make(core.flatMap(effect, (s) => s.get))
