...
export declare namespace Request {
  ...
  export interface Constructor<
    R extends Request<any, any>,
    T extends keyof R = never
  > {
    (
      args: Omit<
        R,
        T | keyof Request.Variance<Request.Success<R>, Request.Error<R>>
      >
    ): R
  }
   ...
   * @category type-level
   */
  export type Error<T extends Request<any, any>> = [T] extends [
   ...
   * @category type-level
   */
  export type Success<T extends Request<any, any>> = [T] extends [
    Request<infer _A, infer _E>
  ]
    ? _A
    : never
   ...
   * @category type-level
   */
  export type Result<T extends Request<any, any>> =
    T extends Request<infer A, infer E> ? Exit.Exit<A, E> : never
 ...
 */
export const isRequest: (u: unknown) => u is Request<unknown, unknown> =
  internal.isRequest
...
export const Class: new <Success, Error, A extends Record<string, any>>(
  args: Types.Equals<Omit<A, keyof Request<unknown, unknown>>, {}> extends true
    ? void
    : {
        readonly [P in keyof A as P extends keyof Request<unknown, unknown>
          ? never
          : P]: A[P]
      }
  ...
  args: Types.Equals<Omit<A, keyof Request<unknown, unknown>>, {}> extends true
    ...
    : {
        readonly [P in keyof A as P extends
          | "_tag"
          | keyof Request<unknown, unknown>
          ? never
          : P]: A[P]
      }
) => Request<Success, Error> & Readonly<A> & { readonly _tag: Tag } =
  internal.TaggedClass as any
 ...
 * @since 2.0.0
 */
export interface Cache extends _Cache.ConsumerCache<
  Request<any, any>,
  {
    listeners: Listeners
 ...
 * @category models
 */
export const makeCache = (options: {
  ...
  cache.make({
    ...options,
    lookup: () =>
      core.map(core.deferredMake<unknown, unknown>(), (handle) => ({
 ...
 * @category symbols
 */
export const EntryTypeId: unique symbol = Symbol.for(
