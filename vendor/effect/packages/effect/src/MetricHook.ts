 ...
 * @category models
 */
export interface MetricHook<in In, out Out>
  extends MetricHook.Variance<In, Out>, Pipeable {
  get(): Out
...
export declare namespace MetricHook {
   ...
   * @category models
   */
  export type Counter<A extends number | bigint> = MetricHook<
    A,
   ...
   * @category models
   */
  export type Gauge<A extends number | bigint> = MetricHook<
    A,
   ...
   * @category models
   */
  export type Summary = MetricHook<
...
) => MetricHook.Counter<A> = internal.counter
...
export const frequency: (
  _key: MetricKey.MetricKey.Frequency
) => MetricHook.Frequency = internal.frequency
...
export const gauge: {
  (
    key: MetricKey.MetricKey.Gauge<number>,
    startAt: number
  ): MetricHook.Gauge<number>
  (
    key: MetricKey.MetricKey.Gauge<bigint>,
    startAt: bigint
  ): MetricHook.Gauge<bigint>
} = internal.gauge
...
) => MetricHook.Histogram = internal.histogram

/**
 * @since 2.0.0
 * @category constructors
 */
export const summary: (key: MetricKey.MetricKey.Summary) => MetricHook.Summary =
  internal.summary
...
export const onUpdate: {
  <In, Out>(
    f: (input: In) => void
  ): (self: MetricHook<In, Out>) => MetricHook<In, Out>
  <In, Out>(
