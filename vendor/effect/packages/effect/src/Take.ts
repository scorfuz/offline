 ...
 * @category models
 */
export interface Take<out A, out E = never>
  extends Take.Variance<A, E>, Pipeable {
  /** @internal */
...
export const done: <A, E>(
  self: Take<A, E>
) => Effect.Effect<Chunk.Chunk<A>, Option.Option<E>> = internal.done
 ...
 * @category constructors
 */
export const failCause: <E>(cause: Cause.Cause<E>) => Take<never, E> =
  internal.failCause
...
) => Effect.Effect<Take<A, E>, never, R> = internal.fromEffect
 ...
 * @category constructors
 */
export const fromExit: <A, E>(exit: Exit.Exit<A, E>) => Take<A, E> =
  internal.fromExit
