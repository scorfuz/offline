...
export const liveServices: Context.Context<TestServices> = pipe(
  Context.make(
    Annotations.TestAnnotations,
    Annotations.make(ref.unsafeMake(TestAnnotationMap.empty()))
  ),
  Context.add(Live.TestLive, Live.make(defaultServices.liveServices)),
  Context.add(Sized.TestSized, Sized.make(100)),
  Context.add(
    TestConfig.TestConfig,
    TestConfig.make({ repeats: 100, retries: 100, samples: 200, shrinks: 1000 })
  )
)
 ...
 * @since 2.0.0
 */
export const currentServices: FiberRef.FiberRef<Context.Context<TestServices>> =
 ...
 * @since 2.0.0
 */
export const annotations = (): Effect.Effect<Annotations.TestAnnotations> =>
  annotationsWith(core.succeed)
...
export const annotationsWith = <A, E, R>(
  ...
  core.fiberRefGetWith(currentServices, (services) =>
    f(Context.get(services, Annotations.TestAnnotations))
  )
...
): Effect.Effect<A> => annotationsWith((annotations) => annotations.get(key))
 ...
 * @since 2.0.0
 */
export const liveWith = <A, E, R>(
  f: (live: Live.TestLive) => Effect.Effect<A, E, R>
): Effect.Effect<A, E, R> =>
  core.fiberRefGetWith(currentServices, (services) =>
    f(Context.get(services, Live.TestLive))
  )
 ...
 * @since 2.0.0
 */
export const provideLive = <A, E, R>(
  effect: Effect.Effect<A, E, R>
): Effect.Effect<A, E, R> => liveWith((live) => live.provide(effect))
...
>(2, (self, f) =>
  core.fiberRefGetWith(defaultServices.currentServices, (services) =>
    provideLive(
      f(core.fiberRefLocally(defaultServices.currentServices, services)(self))
    )
  )
)
 ...
 * @since 2.0.0
 */
export const sizedWith = <A, E, R>(
  f: (sized: Sized.TestSized) => Effect.Effect<A, E, R>
): Effect.Effect<A, E, R> =>
  core.fiberRefGetWith(currentServices, (services) =>
    f(Context.get(services, Sized.TestSized))
  )
...
export const testConfigWith = <A, E, R>(
  ...
  core.fiberRefGetWith(currentServices, (services) =>
    f(Context.get(services, TestConfig.TestConfig))
  )
 ...
 * @since 2.0.0
 */
export const testConfig: Effect.Effect<TestConfig.TestConfig> = testConfigWith(
  core.succeed
)
 ...
 * @since 2.0.0
 */
export const withTestConfigScoped = (
  config: TestConfig.TestConfig
): Effect.Effect<void, never, Scope.Scope> =>
  fiberRuntime.fiberRefLocallyScopedWith(
    currentServices,
 ...
 * @since 2.0.0
 */
export const repeats: Effect.Effect<number> = testConfigWith((config) =>
  core.succeed(config.repeats)
)
 ...
 * @since 2.0.0
 */
export const retries: Effect.Effect<number> = testConfigWith((config) =>
  core.succeed(config.retries)
)
 ...
 * @since 2.0.0
 */
export const samples: Effect.Effect<number> = testConfigWith((config) =>
  core.succeed(config.samples)
)
 ...
 * @since 2.0.0
 */
export const shrinks: Effect.Effect<number> = testConfigWith((config) =>
  core.succeed(config.shrinks)
)
