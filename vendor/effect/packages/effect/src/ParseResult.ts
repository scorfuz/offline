...
export const isParseError = (u: unknown): u is ParseError =>
  Predicate.hasProperty(u, ParseErrorTypeId)
 ...
 * @since 3.10.0
 */
export const parseError = (issue: ParseIssue): ParseError =>
  new ParseError({ issue })
 ...
 * @since 3.10.0
 */
export const fail: (issue: ParseIssue) => Either.Either<never, ParseIssue> =
  Either.left
...
export const fromOption: {
  (
    onNone: () => ParseIssue
  ): <A>(self: Option.Option<A>) => Either.Either<A, ParseIssue>
  <A>(
...
const isEither: <A, E, R>(
  self: Effect.Effect<A, E, R>
) => self is Either.Either<A, E> = Either.isEither as any
...
export const getOption = (
  ...
  return (
    input: unknown,
    overrideOptions?: AST.ParseOptions
  ): Option.Option<any> => Option.getRight(parser(input, overrideOptions))
}
...
const getEffect = <R>(
  ...
  return (
    input: unknown,
    overrideOptions?: AST.ParseOptions
  ): Effect.Effect<any, ParseIssue, R> =>
    parser(input, {
...
export const decodeUnknownSync = <A, I>(
  ...
  getSync(schema.ast, true, options)
...
export const decodeUnknownEither = <A, I>(
  ...
  u: unknown,
  overrideOptions?: AST.ParseOptions
) => Either.Either<A, ParseIssue>) => getEither(schema.ast, true, options)
...
export const decodeUnknownPromise = <A, I>(
  ...
  return (u: unknown, overrideOptions?: AST.ParseOptions): Promise<A> =>
...
export const decodeUnknown = <A, I, R>(
  ...
  u: unknown,
  overrideOptions?: AST.ParseOptions
) => Effect.Effect<A, ParseIssue, R>) => getEffect(schema.ast, true, options)
...
export const encodeUnknownSync = <A, I>(
  ...
  getSync(schema.ast, false, options)
...
export const encodeUnknownEither = <A, I>(
  ...
  u: unknown,
  overrideOptions?: AST.ParseOptions
) => Either.Either<I, ParseIssue>) => getEither(schema.ast, false, options)
...
export const encodeUnknownPromise = <A, I>(
  ...
  return (u: unknown, overrideOptions?: AST.ParseOptions): Promise<I> =>
    Effect.runPromise(parser(u, overrideOptions))
}
...
export const encodeUnknown = <A, I, R>(
  ...
  u: unknown,
  overrideOptions?: AST.ParseOptions
) => Effect.Effect<I, ParseIssue, R>) => getEffect(schema.ast, false, options)
...
export const decodeEither: <A, I>(
  ...
  i: I,
  overrideOptions?: AST.ParseOptions
) => Either.Either<A, ParseIssue> = decodeUnknownEither
...
export const decode: <A, I, R>(
  ...
  i: I,
  overrideOptions?: AST.ParseOptions
) => Effect.Effect<A, ParseIssue, R> = decodeUnknown
...
export const encodeEither: <A, I>(
  ...
  a: A,
  overrideOptions?: AST.ParseOptions
) => Either.Either<I, ParseIssue> = encodeUnknownEither
...
export const encode: <A, I, R>(
  ...
  a: A,
  overrideOptions?: AST.ParseOptions
) => Effect.Effect<I, ParseIssue, R> = encodeUnknown
...
const goMemo = (ast: AST.AST, isDecoding: boolean): Parser => {
  ...
  const parserWithOptions: Parser = Option.isSome(parseOptionsAnnotation)
    ? (i, options) =>
        raw(i, mergeInternalOptions(options, parseOptionsAnnotation.value))
    : raw
  const decodingFallbackAnnotation = AST.getDecodingFallbackAnnotation(ast)
  const parser: Parser =
    isDecoding && Option.isSome(decodingFallbackAnnotation)
      ? (i, options) =>
          handleForbidden(
            orElse(
...
const go = (ast: AST.AST, isDecoding: boolean): Parser => {
  switch (ast._tag) {
      ...
      if (isDecoding) {
        ...
        return (i, options) => {
          ...
          const result = flatMap(
            ...
            (a) =>
              Option.match(ast.filter(a, options, ast), {
                onNone: () => Either.right(a),
                onSome: (ep) =>
                  Either.left(new Refinement(ast, i, "Predicate", ep))
              })
          )
        ...
        handleForbidden(
          flatMap(
            mapError(
              from(i, options),
              (e) =>
                new Transformation(ast, i, isDecoding ? "Encoded" : "Type", e)
            ),
            (a) =>
              flatMap(
                mapError(
    ...
    case "Declaration": {
      ...
      return (i, options) =>
        handleForbidden(
          parse(i, options ?? AST.defaultParseOption, ast),
          ast,
          i,
          options
        )
    ...
    case "TemplateLiteral": {
      ...
      return fromRefinement(
        ast,
        (u): u is any => Predicate.isString(u) && regex.test(u)
      )
    }
    case "TupleType": {
      ...
      return (input: unknown, options) => {
            ...
            if (isEither(te)) {
              if (Either.isLeft(te)) {
                ...
                if (allErrors) {
                  ...
                  return Either.left(
                    new Composite(ast, input, e, sortByIndex(output))
                  )
                }
              }
        ...
        const computeResult = ({ es, output }: State) =>
          Arr.isNonEmptyArray(es)
            ? Either.left(
                new Composite(ast, input, sortByIndex(es), sortByIndex(output))
              )
    ...
    case "TypeLiteral": {
      ...
      const propertySignatures: Array<
        readonly [Parser, AST.PropertySignature]
      > = []
      ...
      const expectedAST = AST.Union.make(
        ast.indexSignatures
          .map((is): AST.AST => is.parameter)
          .concat(
            expectedKeys.map((key) =>
              Predicate.isSymbol(key)
                ? new AST.UniqueSymbol(key)
                : new AST.Literal(key)
            )
          )
      )
      ...
      return (input: unknown, options) => {
            ...
            if (isEither(keu) && Either.isRight(keu)) {
              ...
              if (isEither(vpr)) {
                if (Either.isLeft(vpr)) {
                  ...
                  if (
                    !Object.prototype.hasOwnProperty.call(expectedKeysMap, key)
                  ) {
        ...
        const computeResult = ({ es, output }: State) => {
          if (Arr.isNonEmptyArray(es)) {
            return Either.left(
              new Composite(ast, input, sortByIndex(es), output)
            )
          }
          ...
          if (Predicate.isRecordOrArray(input)) {
            for (let i = 0; i < ownKeysLen; i++) {
              ...
              if (Object.prototype.hasOwnProperty.call(input, name)) {
                  ...
                  es.push([
                    ...
                    new Composite(
                      ...
                      new Pointer(
                        name,
                        input,
                        new Type(literalsUnion, input[name])
                      )
                    )
                  ])
                }
                ...
                const { candidates, literals } = searchTree.keys[name]
                const fakePropertySignature = new AST.PropertySignature(
                  name,
                  AST.Union.make(literals),
                  false,
                  true
                )
                const errorAst =
                ...
                es.push([
                  ...
                  new Composite(
                    errorAst,
                    input,
                    new Pointer(name, input, new Missing(fakePropertySignature))
                  )
                ])
              }
            }
...
const fromRefinement =
  <A>(ast: AST.AST, refinement: (u: unknown) => u is A): Parser =>
  (u) =>
  ...
  switch (ast._tag) {
    ...
    case "TypeLiteral": {
      ...
      for (let i = 0; i < ast.propertySignatures.length; i++) {
        ...
        const type = isDecoding
          ? AST.encodedAST(propertySignature.type)
          : AST.typeAST(propertySignature.type)
    ...
    case "TupleType": {
      ...
      for (let i = 0; i < ast.elements.length; i++) {
        ...
        const type = isDecoding
          ? AST.encodedAST(element.type)
          : AST.typeAST(element.type)
...
const handleForbidden = <A, R>(
  ...
  const fiber = Effect.runFork(effect as Effect.Effect<A, ParseIssue>, {
    scheduler
  })
  scheduler.flush()
...
const compare = (
  [a]: [number, ...Array<unknown>],
  [b]: [number, ...Array<unknown>]
) => (a > b ? 1 : a < b ? -1 : 0)
...
const getCurrentMessage = (
  issue: ParseIssue
): Effect.Effect<CurrentMessage | undefined> =>
...
const createParseIssueGuard =
  <T extends ParseIssue["_tag"]>(tag: T) =>
  (issue: ParseIssue): issue is Extract<ParseIssue, { _tag: T }> =>
    issue._tag === tag
  ...
  flatMap(getCurrentMessage(issue), (currentMessage) => {
    if (currentMessage !== undefined) {
      ...
      return useInnerMessage
        ? isTransformation(issue) || isRefinement(issue)
...
function getDefaultTypeMessage(issue: Type): string {
  ...
  const expected = AST.isRefinement(issue.ast)
    ? getRefinementExpected(issue.ast)
    : String(issue.ast)
...
const formatForbiddenMessage = (issue: Forbidden): string =>
  issue.message ?? "is forbidden"

const formatUnexpectedMessage = (issue: Unexpected): string =>
  issue.message ?? "is unexpected"
...
const formatMissingMessage = (issue: Missing): Effect.Effect<string> => {
  ...
  if (Option.isSome(missingMessageAnnotation)) {
    ...
    return Predicate.isString(annotation)
      ? Either.right(annotation)
      : annotation
  }
  ...
  switch (issue._tag) {
    ...
    case "Transformation":
      return flatMap(getMessage(issue), (message) => {
        ...
        return map(formatTree(issue.issue), (tree) =>
          makeTree(getParseIssueTitle(issue), [
    ...
    case "Pointer":
      return map(formatTree(issue.issue), (tree) =>
        makeTree(util_.formatPath(issue.path), [tree])
      )
    case "Composite":
      return flatMap(getMessage(issue), (message) => {
        if (message !== undefined) {
        ...
        return util_.isNonEmpty(issue.issues)
          ? map(Effect.forEach(issue.issues, formatTree), (forest) =>
              makeTree(parseIssueTitle, forest)
            )
          : map(formatTree(issue.issues), (tree) =>
...
const getArrayFormatterIssues = (
  ...
  switch (_tag) {
    ...
    case "Pointer":
      return getArrayFormatterIssues(
        issue.issue,
        undefined,
        path.concat(issue.path)
      )
    case "Composite":
      return flatMap(getMessage(issue), (message) => {
        if (message !== undefined) {
        ...
        return util_.isNonEmpty(issue.issues)
          ? map(
              Effect.forEach(issue.issues, (issue) =>
                getArrayFormatterIssues(issue, undefined, path)
              ),
              Arr.flatten
            )
    ...
    case "Refinement":
      return flatMap(getMessage(issue), (message) => {
        ...
        return getArrayFormatterIssues(
          issue.issue,
          issue.kind === "Predicate" ? _tag : undefined,
          path
        )
      })
    case "Transformation":
      return flatMap(getMessage(issue), (message) => {
        ...
        return getArrayFormatterIssues(
          issue.issue,
          issue.kind === "Transformation" ? _tag : undefined,
          path
        )
      })
  }
