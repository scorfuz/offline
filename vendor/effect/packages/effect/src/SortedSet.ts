 ...
 * @category models
 */
export interface SortedSet<in out A>
...
export const fromIterable: {
  ...
  <A extends B, B>(iterable: Iterable<A>, ord: Order<B>): SortedSet<A> =>
    fromTree(
      RBT.fromIterable(
        Array.from(iterable).map((k) => [k, true]),
        ord
      )
    )
)
...
export const make =
  <K>(ord: Order<K>) =>
  <Entries extends ReadonlyArray<K>>(
    ...entries: Entries
  ): SortedSet<Entries[number]> =>
...
export const partition: {
  ...
  ): [excluded: SortedSet<A>, satisfying: SortedSet<A>]
} = Dual.dual(
  2,
  <A>(
    self: SortedSet<A>,
 ...
 * @category getters
 */
export const values = <A>(self: SortedSet<A>): IterableIterator<A> =>
  RBT.keys(self.keyTree)
...
export const getEquivalence =
  <A>(): Equivalence.Equivalence<SortedSet<A>> =>
  (a, b) =>
