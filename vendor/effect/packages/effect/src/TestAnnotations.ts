 ...
 * @since 2.0.0
 */
export const TestAnnotationsTypeId: unique symbol = Symbol.for(
...
class AnnotationsImpl implements TestAnnotations {
  ...
  get supervisedFibers(): Effect.Effect<
    SortedSet.SortedSet<Fiber.RuntimeFiber<unknown, unknown>>
  > {
      ...
      core.flatMap(this.get(TestAnnotation.fibers), (either) => {
        switch (either._tag) {
          ...
          case "Right": {
            return pipe(
              either.right,
              core.forEachSequential((ref) =>
                core.sync(() => MutableRef.get(ref))
              ),
              core.map(
                RA.reduce(SortedSet.empty(fiber.Order), (a, b) =>
                  SortedSet.union(a, b)
                )
              ),
              core.map(
                SortedSet.filter(
                  (fiber) => !Equal.equals(fiber.id(), descriptor.id)
                )
              )
            )
          }
 ...
 * @since 2.0.0
 */
export const TestAnnotations: Context.Tag<TestAnnotations, TestAnnotations> =
  Context.GenericTag<TestAnnotations>("effect/Annotations")

/**
 * @since 2.0.0
 */
export const isTestAnnotations = (u: unknown): u is TestAnnotations =>
  hasProperty(u, TestAnnotationsTypeId)
