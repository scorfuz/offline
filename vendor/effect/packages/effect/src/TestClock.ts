...
export const TestClock: Context.Tag<TestClock, TestClock> =
  Context.GenericTag<TestClock>("effect/TestClock")
 ...
 */
const warning =
  "Warning: A test is using time, but is not advancing " +
...
const suspendedWarning =
  "Warning: A test is advancing the test clock, " +
...
export class TestClockImpl implements TestClock {
  ...
  constructor(
    ...
    this.currentTimeNanos = core.map(ref.get(this.clockState), (data) =>
      BigInt(data.instant * 1000000)
    )
  }
  ...
  get save(): Effect.Effect<Effect.Effect<void>> {
    return core.map(ref.get(this.clockState), (data) =>
      ref.set(this.clockState, data)
    )
  }
  /**
      ...
      pipe(
        ref.modify(this.clockState, (data) => {
          ...
          if (end > data.instant) {
            return [
              ...
              makeData(
                data.instant,
                pipe(data.sleeps, Chunk.prepend([end, deferred] as const))
              )
            ] as const
          }
        ...
        core.flatMap((shouldAwait) =>
          shouldAwait
            ? pipe(
                this.warningStart(),
                core.zipRight(core.deferredAwait(deferred))
              )
  ...
  adjust(durationInput: Duration.DurationInput): Effect.Effect<void> {
    ...
    return core.zipRight(
      this.warningDone(),
      this.run((n) => n + Duration.toMillis(duration))
    )
  }
  /**
   * Increments the current clock time by the specified duration. Any effects
  ...
  adjustWith(durationInput: Duration.DurationInput) {
    ...
    return <A, E, R>(effect: Effect.Effect<A, E, R>): Effect.Effect<A, E, R> =>
      fiberRuntime.zipLeftOptions(effect, this.adjust(duration), {
        concurrent: true
      })
  }
  /**
   ...
   */
  freeze(): Effect.Effect<
    HashMap.HashMap<FiberId.FiberId, FiberStatus.FiberStatus>,
    void
  > {
  ...
  > {
    return pipe(
      ...
      core.flatMap(([first, last]) =>
        Equal.equals(first, last) ? core.succeed(first) : core.fail(void 0)
      )
    )
  }
  /**
  ...
  suspendedWarningStart(): Effect.Effect<void> {
    return synchronized.updateSomeEffect(
...
export const live = (
  data: Data
): Layer.Layer<TestClock, never, Annotations.TestAnnotations | Live.TestLive> =>
  layer.scoped(
    TestClock,
    core.gen(function* () {
      ...
      const suspendedWarningState = yield* circular.makeSynchronized(
        SuspendedWarningData.start
      )
      const testClock = new TestClockImpl(
        clockState,
        live,
        annotations,
        warningState,
 ...
 * @since 2.0.0
 */
export const defaultTestClock: Layer.Layer<
  TestClock,
  never,
  Annotations.TestAnnotations | Live.TestLive
> = live(makeData(new Date(0).getTime(), Chunk.empty()))
 ...
 * @since 2.0.0
 */
export const save = (): Effect.Effect<Effect.Effect<void>> =>
...
export const setTime = (input: DateTime.DateTime.Input): Effect.Effect<void> =>
  testClockWith((testClock) =>
    testClock.setTime(
      typeof input === "number" ? input : DateTime.unsafeMake(input).epochMillis
    )
  )
 ...
 * @since 2.0.0
 */
export const sleeps = (): Effect.Effect<Chunk.Chunk<number>> =>
 ...
 * @since 2.0.0
 */
export const testClock = (): Effect.Effect<TestClock> =>
  testClockWith(core.succeed)
 ...
 * @since 2.0.0
 */
export const currentTimeMillis: Effect.Effect<number> = testClockWith(
