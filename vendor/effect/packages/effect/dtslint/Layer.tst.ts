...
describe("Layer", () => {
  it("merge", () => {
    ...
    expect(Layer.merge(layer1, layer2)).type.toBe<
      Layer.Layer<Out1 | Out2, Err1 | Err2, In1 | In2>
    >()
    expect(layer1.pipe(Layer.merge(layer2))).type.toBe<
      Layer.Layer<Out1 | Out2, Err1 | Err2, In1 | In2>
    >()
  })
  ...
  it("mergeAll", () => {
    ...
    expect(Layer.mergeAll(layer1, layer2)).type.toBe<
      Layer.Layer<Out1 | Out2, Err1 | Err2, In1 | In2>
    >()
    expect(Layer.mergeAll(layer1, layer2, layer3)).type.toBe<
      Layer.Layer<Out1 | Out2 | Out3, Err1 | Err2 | Err3, In1 | In2 | In3>
    >()
  })

  it("retry", () => {
    expect(Layer.retry(layer1, Schedule.recurs(1))).type.toBe<
      Layer.Layer<Out1, Err1, In1>
    >()
    expect(layer1.pipe(Layer.retry(Schedule.recurs(1)))).type.toBe<
      Layer.Layer<Out1, Err1, In1>
    >()
  })

  it("ensureSuccessType", () => {
    expect(layer1.pipe(Layer.ensureSuccessType<Out1>())).type.toBe<
      Layer.Layer<Out1, Err1, In1>
    >()
  })
  ...
  it("ensureErrorType", () => {
    ...
    expect(withoutError.pipe(Layer.ensureErrorType<never>())).type.toBe<
      Layer.Layer<TestService1, never, never>
    >()

    const withError = layer1
    expect(withError.pipe(Layer.ensureErrorType<Err1>())).type.toBe<
      Layer.Layer<Out1, Err1, In1>
    >()
  })
  ...
  it("ensureRequirementsType", () => {
    ...
    expect(
      withoutRequirements.pipe(Layer.ensureRequirementsType<never>())
    ).type.toBe<Layer.Layer<TestService1, never, never>>()
