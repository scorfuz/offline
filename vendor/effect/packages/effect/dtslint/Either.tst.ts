...
describe("Either", () => {
  it("flip", () => {
    expect(Either.flip(number$string)).type.toBe<
      Either.Either<string, number>
    >()
    expect(pipe(number$string, Either.flip)).type.toBe<
      Either.Either<string, number>
    >()
  })
  ...
  it("try", () => {
    ...
    expect(Either.try({ try: () => 1, catch: () => new Error() })).type.toBe<
  ...
  describe("all", () => {
    it("tuple", () => {
      ...
      expect(Either.all([number$string])).type.toBe<
        Either.Either<[number], string>
      >()
      expect(Either.all([number$string, boolean$string])).type.toBe<
        Either.Either<[number, boolean], string>
      >()
      expect(Either.all([number$string, boolean$Error])).type.toBe<
        Either.Either<[number, boolean], string | Error>
      >()
      expect(
        pipe([number$string, boolean$string] as const, Either.all)
      ).type.toBe<Either.Either<[number, boolean], string>>()
      expect(
        pipe([number$string, boolean$Error] as const, Either.all)
      ).type.toBe<Either.Either<[number, boolean], string | Error>>()
    })

    it("struct", () => {
      expect(Either.all({})).type.toBe<Either.Either<{}, never>>()
      expect(Either.all({ a: number$string })).type.toBe<
        Either.Either<{ a: number }, string>
      >()
      ...
      >()
      expect(
        pipe({ a: number$string, b: boolean$string }, Either.all)
      ).type.toBe<Either.Either<{ a: number; b: boolean }, string>>()
      expect(
        pipe({ a: number$string, b: boolean$Error }, Either.all)
      ).type.toBe<Either.Either<{ a: number; b: boolean }, string | Error>>()
    })

    it("array", () => {
      const eitherArray = hole<Array<Either.Either<number, string>>>()
      expect(Either.all(eitherArray)).type.toBe<
        Either.Either<Array<number>, string>
      >()
      expect(pipe(eitherArray, Either.all)).type.toBe<
        Either.Either<Array<number>, string>
      >()
    })
    ...
    it("record", () => {
      ...
      expect(Either.all(eitherRecord)).type.toBe<
        Either.Either<{ [x: string]: number }, string>
      >()
      expect(pipe(eitherRecord, Either.all)).type.toBe<
        Either.Either<{ [x: string]: number }, string>
      >()
    })
  })
  ...
  it("andThen", () => {
    expect(Either.andThen(string$string, number$string)).type.toBe<
      Either.Either<number, string>
    >()
    expect(string$string.pipe(Either.andThen(number$string))).type.toBe<
      Either.Either<number, string>
    >()
  ...
  it("fromNullable", () => {
    ...
    expect(Either.fromNullable(nullableString, () => new Error())).type.toBe<
    ...
    expect(Either.fromNullable(nullableObject, () => new Error())).type.toBe<
  ...
  it("filterOrLeft", () => {
    ...
    const readonlyArrayOfStrings$Error =
      hole<Either.Either<ReadonlyArray<string>, Error>>()
    expect(
      Either.filterOrLeft(
      ...
      Either.Either<readonly [string, ...Array<string>], "b" | Error>
    >()
    expect(
      pipe(
        readonlyArrayOfStrings$Error,
...
it("transposeMapOption", () => {
  expect(
    Either.transposeMapOption(Option.none(), (value) => {
      expect(value).type.toBe<never>()
      return string$string
    })
  ).type.toBe<Either.Either<Option.Option<string>, string>>()
  expect(
    pipe(
      Option.none(),
  ...
  expect(
    Either.transposeMapOption(Option.some(42), (value) => {
