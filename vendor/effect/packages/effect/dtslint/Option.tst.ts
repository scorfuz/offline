...
describe("Option", () => {
  it("liftPredicate", () => {
    ...
    expect(
      Option.liftPredicate(primitiveNumberOrString, (n): n is number => {
    ...
    expect(
      pipe(
        ...
        Option.liftPredicate((n): n is number => {
          expect(n).type.toBe<string | number>()
          return typeof n === "number"
        })
      )
  ...
  it("getOrElse", () => {
    expect(Option.getOrElse(Option.some("a"), () => null)).type.toBe<
      string | null
    >()
    expect(
      pipe(
  ...
  it("filter", () => {
    expect(Option.filter(number, predicateNumbersOrStrings)).type.toBe<
      Option.Option<number>
    >()
    expect(pipe(number, Option.filter(predicateNumbersOrStrings))).type.toBe<
      Option.Option<number>
    >()

    expect(pipe(numberOrString, Option.filter(Predicate.isString))).type.toBe<
      Option.Option<string>
    >()
    expect(Option.filter(numberOrString, Predicate.isString)).type.toBe<
      Option.Option<string>
    >()
  ...
  describe("all", () => {
    it("tuple", () => {
      ...
      expect(Option.all([Option.some(1), Option.some("b")])).type.toBe<
    ...
    it("struct", () => {
      expect(Option.all({})).type.toBe<Option.Option<{}>>()
      expect(Option.all({ a: Option.some(1) })).type.toBe<
    ...
    it("array", () => {
      ...
      expect(pipe(optionArray, Option.all)).type.toBe<
    ...
    it("record", () => {
      ...
      expect(Option.all(optionRecord)).type.toBe<
        Option.Option<{ [x: string]: string }>
      >()
      expect(pipe(optionRecord, Option.all)).type.toBe<
        Option.Option<{ [x: string]: string }>
      >()
    })
  })
  ...
  it("andThen", () => {
    expect(Option.andThen(numberOrString, numberOrString)).type.toBe<
      Option.Option<string | number>
    >()
    expect(Option.andThen(numberOrString, () => numberOrString)).type.toBe<
      Option.Option<string | number>
    >()
    expect(numberOrString.pipe(Option.andThen(numberOrString))).type.toBe<
      Option.Option<string | number>
    >()
    expect(numberOrString.pipe(Option.andThen(() => numberOrString))).type.toBe<
      Option.Option<string | number>
  ...
  describe("firstSomeOf", () => {
    ...
    it("should work for heterogeneous usage", () => {
      expect(Option.firstSomeOf([number, string])).type.toBe<
        Option.Option<string | number>
      >()
      expect(pipe([number, string], Option.firstSomeOf)).type.toBe<
        Option.Option<string | number>
      >()
    })
