...
describe("Stream", () => {
  it("filter", () => {
    ...
    ).type.toBe<Stream.Stream<string | number, never, never>>()
  ...
  it("takeWhile", () => {
    ...
    ).type.toBe<Stream.Stream<number, never, never>>()
  ...
  it("Do Notation", () => {
    expect(
      ...
      Stream.Stream<{ a: number; b: string; c: boolean }, never, never>
    >()
  ...
  it("zipLatestAll", () => {
    expect(Stream.zipLatestAll()).type.toBe<
      Stream.Stream<never, never, never>
    >()
    ...
    ).type.toBe<Stream.Stream<[number, string | number, never], Error, never>>()
  })
      ...
      Stream.mergeWithTag(
        {
          a: pipe(
            Stream.make(0),
            Stream.tap(() => new Cause.NoSuchElementException())
          ),
          b: Stream.make("")
        },
        { concurrency: 1 }
