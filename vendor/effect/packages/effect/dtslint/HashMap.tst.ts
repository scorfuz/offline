...
describe("HashMap", () => {
  ...
  it("filter", () => {
    ...
    // Refinement
    expect(HashMap.filter(string$numberOrString, Predicate.isNumber)).type.toBe<
  ...
  it("findFirst", () => {
    // Predicate
    expect(
      HashMap.findFirst(string$numberOrString, (value, key) => {
        expect(value).type.toBe<string | number>()
        expect(key).type.toBe<string>()
        return true
      })
    ...
    // Refinement
    expect(
      HashMap.findFirst(string$numberOrString, Predicate.isNumber)
