...
const successSchema = <Req extends Schema.TaggedRequest.All>(req: Req) =>
  Schema.successSchema(Schema.asWithResult(req))

const failureSchema = <Req extends Schema.TaggedRequest.All>(req: Req) =>
  Schema.failureSchema(Schema.asWithResult(req))
...
describe("Schema Serializable", () => {
  it("Serializable type-level helpers", () => {
    ...
    ).type.toBe<"payload-context">()
  })

  it("successSchema", () => {
    expect(successSchema(new TR({ id: 1 }))).type.toBe<
      Schema.Schema<number, string, never>
    >()
  })

  it("failureSchema", () => {
    expect(failureSchema(new TR({ id: 1 }))).type.toBe<
