...
describe("Schema Context", () => {
  ...
  it("declare: with contexts and options", () => {
    ...
    expect(S.asSchema(schema)).type.toBe<
      S.Schema<string, number, "aContext" | "bContext">
    >()
    expect(schema).type.toBe<
      S.declare<string, number, readonly [aContext, bContext]>
    >()
  })

  it("declare errors", () => {
    expect(S.declare).type.not.toBeCallableWith([aContext, bContext], {
      decode:
        (_a: S.Schema<string, string>, _b: S.Schema<number, number>) => () =>
  ...
  it("Union", () => {
    expect(S.asSchema(S.Union(aContext, bContext))).type.toBe<
      S.Schema<string | number, string | number, "aContext" | "bContext">
    >()
    expect(S.Union(aContext, bContext)).type.toBe<
  ...
  it("TupleType", () => {
    expect(S.asSchema(S.Tuple([aContext], bContext))).type.toBe<
      S.Schema<
      ...
      >
    >()
    expect(S.Tuple([aContext], bContext)).type.toBe<
      S.TupleType<readonly [aContext], [bContext]>
    >()
  })
  ...
  it("OptionalElement", () => {
    expect(
      ...
      >
    >()
    expect(S.Tuple(aContext, S.optionalElement(bContext))).type.toBe<
      S.Tuple<[aContext, S.Element<bContext, "?">]>
    >()
  })

  it("Array", () => {
    expect(S.asSchema(S.Array(aContext))).type.toBe<
      S.Schema<ReadonlyArray<string>, ReadonlyArray<string>, "aContext">
    >()
    expect(S.Array(aContext)).type.toBe<S.Array$<aContext>>()
  })

  it("NonEmptyArray", () => {
    expect(S.asSchema(S.NonEmptyArray(aContext))).type.toBe<
      S.Schema<
      ...
      >
    >()
    expect(S.NonEmptyArray(aContext)).type.toBe<S.NonEmptyArray<aContext>>()
  })

  it("propertySignatureDeclaration", () => {
    expect(S.propertySignature(aContext)).type.toBe<
      S.propertySignature<aContext>
    >()
    expect(S.propertySignature(aContext).annotations({})).type.toBe<
      S.propertySignature<aContext>
    >()
  })
  ...
  it("optionalToOptional", () => {
    expect(
      S.optionalToOptional(aContext, S.String, {
        decode: (o) => o,
        encode: (o) => o
      })
    ).type.toBe<
      S.PropertySignature<"?:", string, never, "?:", string, false, "aContext">
    >()
  })

  it("optionalToRequired", () => {
    expect(
      S.optionalToRequired(aContext, S.String, {
  ...
  it("requiredToOptional", () => {
    expect(
      S.requiredToOptional(aContext, S.String, {
        decode: Option.some,
  ...
  it("optional", () => {
    expect(S.optional(aContext)).type.toBe<S.optional<aContext>>()
  })

  it("Struct", () => {
    expect(S.asSchema(S.Struct({ a: aContext, b: bContext }))).type.toBe<
      S.Schema<
      ...
      >
    >()
    expect(S.Struct({ a: aContext, b: bContext })).type.toBe<
      S.Struct<{ a: aContext; b: bContext }>
    >()
  })

  it("pick", () => {
    expect(S.Struct({ a: aContext, b: bContext }).pipe(S.pick("a"))).type.toBe<
      S.SchemaClass<
        { readonly a: string },
        { readonly a: string },
        "aContext" | "bContext"
  ...
  it("omit", () => {
    expect(S.Struct({ a: aContext, b: bContext }).pipe(S.omit("b"))).type.toBe<
      S.SchemaClass<
        { readonly a: string },
        { readonly a: string },
        "aContext" | "bContext"
  ...
  it("mutable", () => {
    expect(
      ...
      >
    >()
    expect(S.mutable(S.Struct({ a: aContext, b: bContext }))).type.toBe<
      S.mutable<S.Struct<{ a: aContext; b: bContext }>>
    >()
  })

  it("Record", () => {
    expect(S.asSchema(S.Record({ key: aContext, value: bContext }))).type.toBe<
      S.Schema<
      ...
      >
    >()
    expect(S.Record({ key: aContext, value: bContext })).type.toBe<
  ...
  it("extend", () => {
    expect(
      S.asSchema(
        S.Struct({ a: aContext, b: bContext }).pipe(
          S.extend(S.Struct({ c: cContext }))
        )
      )
    ).type.toBe<
      S.Schema<
  ...
  it("compose", () => {
    expect(
      S.asSchema(aContext.pipe(S.compose(bContext, { strict: false })))
    ).type.toBe<S.Schema<number, string, "aContext" | "bContext">>()
  })

  it("suspend", () => {
    expect(S.suspend(() => aContext)).type.toBe<
      S.suspend<string, string, "aContext">
    >()
  })

  it("filter", () => {
    expect(aContext.pipe(S.filter(() => false))).type.toBe<S.filter<aContext>>()
  })
  ...
  it("annotations", () => {
    expect(aContext.annotations({})).type.toBe<
      S.Schema<string, string, "aContext">
    >()
  })
...
describe("Class", () => {
  it("Class", () => {
    expect<S.Schema.Context<typeof MyClass>>().type.toBe<"aContext">()
  })
    ...
    class MyClassWithTransform extends MyClass.transformOrFail<MyClassWithTransform>(
      ...
      { b: bContext },
      {
        decode: (i) =>
          Tag1.pipe(
            Effect.flatMap((a) =>
              ParseResult.succeed(i.a === a ? { ...i, b: 1 } : { ...i, b: 2 })
            )
          ),
        encode: (a) =>
          Tag2.pipe(
            Effect.flatMap((b) =>
              ParseResult.succeed(a.b === b ? { a: "a1" } : { a: "a2" })
            )
          )
      }
    ) {}
    expect<S.Schema.Context<typeof MyClassWithTransform>>().type.toBe<
      "aContext" | "bContext" | "Tag1" | "Tag2"
    >()
    expect(MyClassWithTransform.fields).type.toBe<{
      readonly a: aContext
      readonly b: bContext
    }>()
  })
  ...
  it("Class.transformFrom", () => {
    ...
    expect<S.Schema.Context<typeof MyClassWithTransformFrom>>().type.toBe<
      "aContext" | "bContext" | "Tag1" | "Tag2"
    >()
    expect(MyClassWithTransformFrom.fields).type.toBe<{
      readonly a: aContext
      readonly b: bContext
    }>()
  })
...
describe("TaggedRequest", () => {
  it("TaggedRequest", () => {
    expect<S.Schema.Context<typeof MyRequest>>().type.toBe<"aContext">()
    expect(MyRequest.fields).type.toBe<{
      readonly _tag: S.tag<"MyRequest">
      readonly a: aContext
    }>()
  })

  it("exitSchema", () => {
    expect(S.exitSchema(myRequest)).type.toBe<
      S.Schema<
        Exit.Exit<boolean, number>,
        S.ExitEncoded<boolean, number, unknown>,
        "bContext" | "cContext"
      >
    >()
  })
...
describe("TemplateLiteralParser", () => {
  it("TemplateLiteralParser", () => {
    expect(
      ...
      S.Schema<readonly [string, "a", string], `${string}a${string}`, "a" | "b">
    >()
  })
})
