...
class WithContext extends S.Class<WithContext>("WithContext")({
  a: aContext,
  b: bContext
}) {}
...
describe("Schema.Class", () => {
  it("should check conflicts with fields/from keys", () => {
    ...
    expect(
      checkForConflicts(
        S.Struct({ fields: S.String }).pipe(S.filter(() => true))
      )
    ...
    expect(
      checkForConflicts({
        fields: S.Struct({ a: S.String }).pipe(S.filter(() => true))
      })
    ).type.toBe<
    ...
    expect(
      checkForConflicts({
        fields: S.Struct({ a: S.String }).pipe(
          S.filter(() => true),
          S.filter(() => true)
        )
      })
    ).type.toBe<
      S.Struct<{ fields: S.filter<S.filter<S.Struct<{ a: typeof S.String }>>> }>
    >()
    ...
    expect(
      checkForConflicts({
        from: S.Struct({ a: S.String }).pipe(S.filter(() => true))
      })
    ).type.toBe<
    ...
    expect(
      checkForConflicts({
        from: S.Struct({ a: S.String }).pipe(
          S.filter(() => true),
          S.filter(() => true)
        )
      })
    ).type.toBe<
      S.Struct<{ from: S.filter<S.filter<S.Struct<{ a: typeof S.String }>>> }>
    >()
  })
  ...
  it("A class with all fields with a default should permit an empty argument in the constructor.", () => {
    class AllDefaultedFields extends S.Class<AllDefaultedFields>(
      "AllDefaultedFields"
    )({
      a: S.String.pipe(
        S.propertySignature,
  ...
  it("test Context", () => {
    expect<S.Schema.Type<typeof WithContext>>().type.toBe<WithContext>()
    expect<S.Schema.Encoded<typeof WithContext>>().type.toBe<{
      readonly a: string
      readonly b: number
    }>()
    expect<S.Schema.Context<typeof WithContext>>().type.toBe<"a" | "b">()
  })

  it("should be a constructor", () => {
    expect<ConstructorParameters<typeof WithContext>>().type.toBe<
      [
  ...
  it("should expose a `fields` field", () => {
    expect(WithContext.fields).type.toBe<{
      readonly a: typeof aContext
      readonly b: typeof bContext
    }>()
  })
  ...
  it("can be extended with Class.extend", () => {
    ...
    expect<S.Schema.Type<typeof Extended>>().type.toBe<Extended>()

    expect<S.Schema.Encoded<typeof Extended>>().type.toBe<{
      readonly a: string
      readonly b: number
      readonly c: boolean
    }>()
  ...
  it("can be extended with another Class `fields` field", () => {
    class ExtendedFromClassFields extends S.Class<ExtendedFromClassFields>(
      "ExtendedFromClassFields"
    )({
      ...WithContext.fields,
      b: S.String,
      c: cContext
    ...
    expect<
      S.Schema.Type<typeof ExtendedFromClassFields>
    >().type.toBe<ExtendedFromClassFields>()

    expect<S.Schema.Encoded<typeof ExtendedFromClassFields>>().type.toBe<{
      readonly a: string
      readonly b: string
      readonly c: boolean
    }>()

    expect<S.Schema.Context<typeof ExtendedFromClassFields>>().type.toBe<
      "a" | "c"
    >()

    expect(ExtendedFromClassFields.fields).type.toBe<{
      readonly b: typeof S.String
      readonly c: S.Schema<boolean, boolean, "c">
      readonly a: S.Schema<string, string, "a">
    }>()

    expect<ConstructorParameters<typeof ExtendedFromClassFields>>().type.toBe<
      [
        props: { readonly a: string; readonly b: string; readonly c: boolean },
        options?: S.MakeOptions | undefined
      ]
    >()
  })
  ...
  it("can be extended with another TaggedClass `fields` field", () => {
    class ExtendedFromTaggedClassFields extends S.TaggedClass<ExtendedFromTaggedClassFields>()(
      "ExtendedFromTaggedClassFields",
      {
        ...WithContext.fields,
        b: S.String,
        c: cContext
    ...
    expect<
      S.Schema.Type<typeof ExtendedFromTaggedClassFields>
    >().type.toBe<ExtendedFromTaggedClassFields>()

    expect<S.Schema.Encoded<typeof ExtendedFromTaggedClassFields>>().type.toBe<{
      readonly a: string
      readonly b: string
      readonly c: boolean
      readonly _tag: "ExtendedFromTaggedClassFields"
    }>()

    expect<S.Schema.Context<typeof ExtendedFromTaggedClassFields>>().type.toBe<
      "a" | "c"
    >()

    expect(ExtendedFromTaggedClassFields.fields).type.toBe<{
    ...
    expect<
      ConstructorParameters<typeof ExtendedFromTaggedClassFields>
    >().type.toBe<
      [
        props: { readonly a: string; readonly b: string; readonly c: boolean },
        options?: S.MakeOptions | undefined
  ...
  it("should accept a HasFields as argument", () => {
    ...
    class _B extends S.Class<_B>("B")(
      S.Struct({ a: S.String }).pipe(S.filter(() => true))
    ) {}
    class _C extends S.Class<_C>("C")(
      S.Struct({ a: S.String }).pipe(
        S.filter(() => true),
        S.filter(() => true)
      )
    ) {}
  ...
  it("users can override an instance member property", () => {
    ...
    expect(new B({ a: "a", c: "c" }).b).type.toBe<2>()
  })
  ...
  it("users can override a field with an instance member property", () => {
    ...
    expect(new B({ a: "a", c: "c" }).a).type.toBe<"default">()
  })
  ...
  it(`users can't override an instance member property with a field`, () => {
    ...
    expect(new B({ a: "a", b: 2 }).b).type.toBe<1>()
  })
})
