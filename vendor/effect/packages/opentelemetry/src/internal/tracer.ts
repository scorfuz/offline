...
export class OtelSpan implements EffectTracer.Span {
  ...
  constructor(
    ...
    this.sampled =
      (spanContext.traceFlags & OtelApi.TraceFlags.SAMPLED) ===
...
/** @internal */
export const TracerProvider = Context.GenericTag<
  OtelTracerProvider,
  OtelApi.TracerProvider
>("@effect/opentelemetry/Tracer/OtelTracerProvider")

/** @internal */
export const Tracer = Context.GenericTag<OtelTracer, OtelApi.Tracer>(
...
/** @internal */
export const traceFlagsTag = Context.GenericTag<
  OtelTraceFlags,
  OtelApi.TraceFlags
>("@effect/opentelemetry/Tracer/OtelTraceFlags")

/** @internal */
export const traceStateTag = Context.GenericTag<
  OtelTraceState,
  OtelApi.TraceState
...
const makeOtelSpan = (
  ...
  const spanContext: OtelApi.SpanContext = {
    ...
    spanId: span.spanId,
    traceFlags: span.sampled
      ? OtelApi.TraceFlags.SAMPLED
      : OtelApi.TraceFlags.NONE,
    isRemote: false
  }
...
const convertOtelTimeInput = (
  input: OtelApi.TimeInput | undefined,
  clock: Clock.Clock
): bigint => {
  if (input === undefined) {
...
/** @internal */
export const layerGlobalProvider = Layer.sync(TracerProvider, () =>
...
export const layerTracer = Layer.effect(
  Tracer,
  Effect.flatMap(Effect.zip(Resource, TracerProvider), ([resource, provider]) =>
    Effect.sync(() =>
      provider.getTracer(
...
/** @internal */
export const layerGlobal = Layer.unwrapEffect(
  Effect.map(make, Layer.setTracer)
).pipe(Layer.provideMerge(layerGlobalTracer))

/** @internal */
export const layerWithoutOtelTracer = Layer.unwrapEffect(
  Effect.map(make, Layer.setTracer)
)
...
const extractTraceTag = <I, S>(
  ...
  Option.orElse(Context.getOption(context, tag), () =>
