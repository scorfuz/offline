    ...
    for (let i = 0, len = snapshot.length; i < len; i++) {
      ...
      if (MetricState.isCounterState(metricState)) {
        ...
        if (metricDataByName.has(descriptor.name)) {
          metricDataByName
            .get(descriptor.name)!
            .dataPoints.push(dataPoint as any)
        } else {
          addMetricData({
        ...
        if (metricDataByName.has(`${descriptor.name}_quantiles`)) {
          // eslint-disable-next-line no-restricted-syntax
          metricDataByName
            .get(`${descriptor.name}_quantiles`)!
            .dataPoints.push(...(dataPoints as any))
          metricDataByName
            .get(`${descriptor.name}_count`)!
            .dataPoints.push(countDataPoint as any)
          metricDataByName
            .get(`${descriptor.name}_sum`)!
...
const descriptorFromKey = (
  ...
  valueType:
    "bigint" in metricKey.keyType && metricKey.keyType.bigint === true
      ? ValueType.INT
      : ValueType.DOUBLE
})

const instrumentTypeFromKey = (
  ...
  if (MetricKeyType.isHistogramKey(key.keyType)) {
    ...
    MetricKeyType.isCounterKey(key.keyType) &&
    key.keyType.incremental
  ) {
    return InstrumentType.COUNTER
  }
...
export const layer = (
  ...
  Layer.scopedDiscard(
    Effect.flatMap(makeProducer, (producer) =>
