    ...
    for (let i = 0, len = snapshot.length; i < len; i++) {
      ...
      if (MetricState.isCounterState(metricState)) {
        ...
        if (metricDataByName.has(metricKey.name)) {
          metricDataByName
            .get(metricKey.name)!
            .gauge!.dataPoints.push(dataPoint)
        } else {
          addMetricData({
            name: metricKey.name,
        ...
        if (metricDataByName.has(`${metricKey.name}_quantiles`)) {
          // eslint-disable-next-line no-restricted-syntax
          metricDataByName
            .get(`${metricKey.name}_quantiles`)!
            .sum!.dataPoints.push(...dataPoints)
          metricDataByName
            .get(`${metricKey.name}_count`)!
            .sum!.dataPoints.push(countDataPoint)
          metricDataByName
            .get(`${metricKey.name}_sum`)!
...
export const layer = (options: {
  ...
  Layer.scopedDiscard(make(options))
