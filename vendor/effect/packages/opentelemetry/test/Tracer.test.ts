...
describe("Tracer", () => {
    ...
    it.effect("supervisor sets context", () =>
      Effect.provide(
        Effect.withSpan("ok")(
          Effect.sync(() => {
            expect(
              OtelApi.trace.getSpan(OtelApi.context.active())
            ).toBeDefined()
          })
        ),
        TracingLive
            ...
            async (span) => {
              try {
                const parent = await Runtime.runPromise(
                  runtime,
                  Tracer.withSpanContext(effect, span.spanContext())
                )
  ...
  describe("OTLP tracer", () => {
      ...
      Effect.provide(
        Effect.withSpan("root")(
          Effect.gen(function* () {
            const otelSpan = yield* Tracer.currentOtelSpan
            expect(() =>
              otelSpan.addEvent("test-event", { foo: "bar", count: 42 })
            ).not.toThrow()
          })
        ),
        OtlpTracingLive
    ...
    it.effect(
      "addEvent with HrTime tuple (2-arg overload) does not throw",
      () =>
        Effect.provide(
          Effect.withSpan("root")(
            Effect.gen(function* () {
              const otelSpan = yield* Tracer.currentOtelSpan
              expect(() =>
                otelSpan.addEvent("test-event", [1, 2])
              ).not.toThrow()
            })
          ),
          OtlpTracingLive
    ...
    it.effect(
      "addEvent with number timestamp (2-arg overload) does not throw",
      () =>
        Effect.provide(
          Effect.withSpan("root")(
            Effect.gen(function* () {
              const otelSpan = yield* Tracer.currentOtelSpan
              expect(() =>
                otelSpan.addEvent("test-event", Date.now())
              ).not.toThrow()
            })
          ),
          OtlpTracingLive
    ...
    it.effect(
      "addEvent with Date timestamp (2-arg overload) does not throw",
      () =>
        Effect.provide(
          Effect.withSpan("root")(
            Effect.gen(function* () {
              const otelSpan = yield* Tracer.currentOtelSpan
              expect(() =>
                otelSpan.addEvent("test-event", new Date())
              ).not.toThrow()
            })
          ),
          OtlpTracingLive
    ...
    it.effect(
      "addEvent with attributes and timestamp (3-arg overload) does not throw",
      () =>
        Effect.provide(
          Effect.withSpan("root")(
            Effect.gen(function* () {
              const otelSpan = yield* Tracer.currentOtelSpan
              expect(() =>
                otelSpan.addEvent("test-event", { foo: "bar" }, Date.now())
              ).not.toThrow()
            })
          ),
          OtlpTracingLive
  ...
  describe("Log Attributes", () => {
    it.effect("propagates attributes with Effect.fnUntraced", () =>
      Effect.gen(function* () {
        ...
        assert.isNotEmpty(
          exporter
            .getFinishedSpans()[0]
            .events.filter((_) => _.name === "FooBar")
        )
        assert.isNotEmpty(
          exporter
            .getFinishedSpans()[0]
            .events.filter((_) => _.name === "exception")
        )
      }).pipe(Effect.provide(TracingLive))
    )
    ...
    it.effect("propagates attributes with Effect.fn", () =>
      Effect.gen(function* () {
        ...
        yield* Console.log(
          Array.from(yield* FiberRef.get(FiberRef.currentLoggers))
        )
