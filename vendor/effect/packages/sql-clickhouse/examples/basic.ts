...
const ClickhouseLive = ClickhouseClient.layerConfig({
  url: Config.succeed(
    "https://r8raccaqh3.ap-southeast-2.aws.clickhouse.cloud:8443"
  ),
...
Effect.gen(function* () {
  ...
  yield* sql.asCommand(
    sql`INSERT INTO clickhouse_js_example_cloud_table ${sql.insert({
      id: 3,
      name: "Charlie"
    })}`
  )
...
}).pipe(Effect.provide(ClickhouseLive), NodeRuntime.runMain)
